...
{ NOTAS
  ...
  Esta versión de dxmdataset la he creado para evitar utilizar directamente la librería DevExpress.
  El motivo principal es que incluye ficheros que utilizan el UI.
  Tampoco la actualizo porque, de momento, no necesito los pocos cambios introducidos y las modificaciones
  que he hecho incluyen la creación de funciones para Variants que evitan usar cxVariants y
...
uses
  ...
  Types, DB, SqlTimSt, Classes, SysUtils,
  Core, CoreClasses;

type
  TCustomMemData = class;
  TMemFields = class;
  TMemBlobData = AnsiString;

  TMemDataBuffer = TRecordBuffer;
  TMemDataFieldList = TList<TField>;
  ...
  private
    ...
    function GetDataSet: TCustomMemData;
    function GetMemFields: TMemFields;
  ...
  protected
    ...
    property MemFields: TMemFields read GetMemFields;
  public
    constructor Create(AOwner : TMemFields);
    ...
    property DataSet: TCustomMemData read GetDataSet;
  ...
  private
    FItems : TList;
    FCalcFields : TList;
    FDataSet : TCustomMemData;
    FValues : TList;
    FValuesSize : Integer;

    function GetRecordCount : Integer;
    function GetItem(Index : Integer)  : TMemField;
  protected
    function Add(AField : TField) : TMemField;
    procedure Clear;
  ...
  public
    constructor Create(ADataSet : TCustomMemData);
    ...
    function IndexOf(Field: TField) : TMemField;

    function GetValue(mField: TMemField; Index: Integer): TRecordBuffer;
    function GetHasValue(mField: TMemField; Index: Integer): AnsiChar;
    procedure SetValue(mField: TMemField; Index: Integer; Buffer: TRecordBuffer);
    procedure SetHasValue(mField: TMemField; Index: Integer; Value: AnsiChar);

    //For the guys from AQA.
    property Values: TList read FValues;

    property DataSet : TCustomMemData read FDataSet;
    property Count : Integer read GetCount;
    property Items[Index : Integer] : TMemField read GetItem;
  ...
  TMemBlobStream = class(TMemoryStream)
  private
    FField: TBlobField;
    FDataSet: TCustomMemData;
  ...
  TSortOption = (soDesc, soCaseInsensitive);
  TSortOptions = set of TSortOption;

  TMemIndex = class(TCollectionItem)
  private
    fIsDirty: Boolean;
    fField: TField;
  ...
  protected
    function GetMemData: TCustomMemData;
    procedure Prepare;
  ...
  public
    ...
    property MemData: TCustomMemData read GetMemData;
  published
    property FieldName: String read fFieldName write SetFieldName;
    property SortOptions: TSortOptions read FSortOptions write SetSortOptions;
  end;

  TMemIndexes = class(TCollection)
  private
    FMemData: TCustomMemData;
  protected
  ...
  public
    function Add: TMemIndex;
    function GetIndexByField(AField: TField): TMemIndex;

    property MemData: TCustomMemData read FMemData;
  end;
  ...
  public
    constructor Create(AMemData: TCustomMemData);
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    procedure SaveData;
    procedure LoadData;
  ...
  private
    ...
    property CalcBuffer: TMemDataBuffer read GetCalcBuffer;
    property IsBinaryDataLoading: Boolean read FLoadFlag;
    property RecIdMemField: TMemField read GetRecIdMemField;
  protected
    function ActiveBuffer: TMemDataBuffer; inline;
  ...
  protected
    ...
    function InternalCompareValues(const ABuffer1, ABuffer2: Pointer; AMemField: TMemField; AIsCaseInSensitive: Boolean; ACount: Integer = -1): Integer;
    function CompareValues(const ABuffer1, ABuffer2: TRecordBuffer; AMemField: TMemField; ASortOptions: TSortOptions): Integer; overload;
    function CompareValues(const ABuffer1, ABuffer2: TRecordBuffer; AMemField: TMemField): Integer; overload;
    ...
    function InternalGotoNearest(AValues: TList; AField: TField;
      const ABuffer: TRecordBuffer; ASortOptions: TSortOptions; ALocateOptions: TLocateOptions; out AIndex: Integer): Boolean;
    function GotoNearest(const ABuffer: TRecordBuffer; ASortOptions: TSortOptions; ALocateOptions: TLocateOptions; out AIndex: Integer): Boolean;
  ...
  public
    ...
    property Data : TMemFields read FData;
    property DelimiterChar : Char read FDelimiterChar write FDelimiterChar;
    property Filter;

    property Indexes: TMemIndexes read fIndexes write SetIndexes;
...
resourceString
   IncorrectedData = 'Datos incorrectos.';
   DataInvalidStreamFormat = 'Formato de stream no válido.';
...
function TFakeDataSet.GetFieldData(Field: TField; var Buffer: TValueBuffer): Boolean;
begin
  Result := FBuffer <> nil;
  if Buffer <> nil then
...
type
  ...
  public
    ...
    procedure CreateField(AMemData: TCustomMemData); virtual;
  ...
  public
    procedure CreateField(AMemData: TCustomMemData); override;
    function ReadFieldValue(AStream: TStream; AVerNo: Double): Boolean; override;
  end;
  ...
  protected
    FMemData: TCustomMemData;
    procedure WriteFieldValue(AStream: TStream; AMemField: TMemField; ARecordIndex: Integer); virtual; abstract;

    property MemData: TCustomMemData read FMemData;
  public
    constructor Create(AMemData: TCustomMemData; AField: TField); virtual;
  end;
  ...
  protected
    procedure WriteFieldValue(AStream: TStream; AMemField: TMemField; ARecordIndex: Integer); override;
  end;
  ...
  protected
    ...
    function MemDataField(AField: TField): TMemField;

    property Fields[Index: Integer]: TField read GetField;
  public
    constructor Create(AMemData: TCustomMemData; AStream: TStream); virtual;
    destructor Destroy; override;
  ...
  { TMemDataStreamReader }

  TMemDataStreamReader = class(TMemDataStreamer)
  private
    FVerNo: Double;
  ...
  public
    constructor Create(AMemData: TCustomMemData; AStream: TStream); override;

    procedure CreateFields(AMemData: TCustomMemData);
    procedure LoadData;
  end;

  { TMemDataStreamWriter }

  TMemDataStreamWriter = class(TMemDataStreamer)
  private
...
constructor TMemDataStreamReader.Create(AMemData: TCustomMemData; AStream: TStream);
begin
  inherited;
  FVerNo := -1;
end;

function TMemDataStreamReader.GetFieldReader(Index: Integer): TdxFieldReader;
begin
  Result := TdxFieldReader(FFieldStreamers[Index]);
end;

function TMemDataStreamReader.GetFieldReaderClass(AFieldTypeNo: Integer): TdxFieldReaderClass;
var
...
function TMemDataStreamReader.GetFieldReadersByField(AField : TField) : TdxFieldReader;
begin
  Result := TdxFieldReader(GetFieldStreamersByField(AField));
end;

procedure TMemDataStreamReader.AddRecord;
var
...
function TMemDataStreamReader.ReadVerNoFromStream: Boolean;
var
...
function TMemDataStreamReader.ReadFieldsFromStream: Boolean;
var
...
function TMemDataStreamReader.ReadRecordFromStream: Boolean;
var
  I: Integer;
begin
  Result := True;
...
procedure TMemDataStreamReader.CreateFields(AMemData: TCustomMemData);
var
  I : Integer;
begin
...
procedure TMemDataStreamReader.LoadData;
begin
...
begin
  ...
  FMode := Mode;
  FField := Field;
  FDataSet := TCustomMemData(FField.DataSet);
...
constructor TdxFieldWriter.Create(AMemData: TCustomMemData; AField: TField);
begin
  inherited Create;
  FMemData := AMemData;
  FField := AField;
...
procedure TdxBlobFieldWriter.WriteFieldValue(AStream: TStream; AMemField: TMemField; ARecordIndex: Integer);
var
  ABlobLength : Integer;
...
procedure TdxStringFieldWriter.WriteFieldValue(AStream: TStream; AMemField: TMemField; ARecordIndex: Integer);
var
  AStrLength: Integer;
begin
...
procedure TdxOrdinalFieldWriter.WriteFieldValue(AStream: TStream; AMemField: TMemField; ARecordIndex: Integer);
begin
  WriteCharToStream(AStream, AMemField.HasValues[ARecordIndex]);
...
procedure TMemDataStreamWriter.WriteMemDataVersion;
begin
  WriteStringToStream(Stream, MemDataVerString);
  WriteDoubleToStream(Stream, MemDataVer);
end;

function TMemDataStreamWriter.GetFieldWriterClass(AFieldType: TFieldType): TdxFieldWriterClass;
begin
  case BaseFieldType(AFieldType) of
...
function TMemDataStreamWriter.GetFieldWritersByField(AField: TField): TdxFieldWriter;
begin
  Result := TdxFieldWriter(GetFieldStreamersByField(AField));
end;

procedure TMemDataStreamWriter.WriteFields;
var
  I: Integer;
begin
...
procedure TMemDataStreamWriter.WriteRecord(ARecordIndex: Integer);
var
  I: Integer;
begin
  for I := 0 to FieldCount - 1 do
...
procedure TMemDataStreamWriter.SaveData;
var
  I : Integer;
begin
  WriteMemDataVersion;
...
constructor TMemDataStreamer.Create(AMemData: TCustomMemData; AStream: TStream);
begin
  inherited Create;
  FMemData := AMemData;
  FStream := AStream;
...
destructor TMemDataStreamer.Destroy;
begin
...
function TMemDataStreamer.BaseFieldType(AFieldType: TFieldType): TdxBaseFieldType;
begin
...
function TMemDataStreamer.FieldCount: Integer;
begin
  Result := FFields.Count;
end;

function TMemDataStreamer.FieldStreamersCount: Integer;
begin
  Result := FFieldStreamers.Count;
end;

procedure TMemDataStreamer.PopulateFieldList;
var
  I: Integer;
begin
...
function TMemDataStreamer.GetField(Index: Integer): TField;
begin
  Result := TField(FFields[Index]);
end;

function TMemDataStreamer.GetFieldStreamersByField(AField: TField): TdxFieldStreamer;
var
  I: Integer;
begin
  Result := nil;
...
function TMemDataStreamer.MemDataField(AField: TField): TMemField;
begin
  Result := MemData.Data.IndexOf(AField);
end;

{ TCustomMemData }
constructor TCustomMemData.Create(AOwner : TComponent);
begin
  inherited Create(AOwner);
  FData := TMemFields.Create(self);
  FData.FDataSet := self;
  ...
  fIndexes := TMemIndexes.Create(TMemIndex);
...
destructor TCustomMemData.Destroy;
begin
    try
      Close;
      ...
      // No relanzo la excepción para que no se interrumpa la limpieza el contenedor de este objeto
      end;

end;

procedure TCustomMemData.CreateRecIDField;
begin
...
procedure TCustomMemData.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
function TCustomMemData.BookmarkValid(Bookmark: TBookmark): Boolean;
var
  AIndex : Integer;
begin
  Result := Bookmark <> nil;
...
function TCustomMemData.CompareBookmarks(Bookmark1, Bookmark2: TBookmark): Integer;
const
...
procedure TCustomMemData.CheckFields(FieldsName: string);
var
  AFieldList: TMemDataFieldList;
  i: Integer;
begin
  AFieldList := TMemDataFieldList.Create;
  try
...
function TCustomMemData.GetStringLength(AFieldType: TFieldType; const ABuffer: Pointer): Integer;
begin
  Result := 0;
  if ABuffer <> nil then
    case AFieldType of
...
function TCustomMemData.InternalLocate(const KeyFields: string; const KeyValues: Variant;
  ...
  function CompareLocStr(AmField: TMemField; buf1, buf2 : TRecordBuffer; AStSize: Integer) : Integer;
  var
    ATempBuffer: Pointer;
    fStr2Len : Integer;
  begin
  ...
  function LocateByIndexField(AIndex: TMemIndex; AField: TField; AValue: Variant) : Integer;
  var
    FStSize : Integer;
    mField: TMemField;
    ABuf: TRecordBuffer;
        ...
        begin
          ...
          for k := 0 to AFieldList.Count - 1 do
            ...
            begin
              ...
              begin
                if TMemField(AmFieldList[k]).HasValue[RealRec] then
                  j := -1;
              end
              else
              begin
                ...
                begin
                  ...
                  j := CompareLocStr(TMemField(AmFieldList[k]),
                      TRecordBuffer(AValueList[k]), TMemField(AmFieldList[k]).Values[RealRec], StSize)
                end
                else
                  j := InternalCompareValues(TRecordBuffer(AValueList[k]), TMemField(AmFieldList[k]).Values[RealRec], TMemField(AmFieldList[k]), loCaseInsensitive in Options);
              end;
...
function TCustomMemData.Locate(const KeyFields: string; const KeyValues: Variant;
...
function TCustomMemData.Lookup(const KeyFields: string; const KeyValues: Variant;
...
function TCustomMemData.GetRecNoByFieldValue(Value : Variant; FieldName : String) : Integer;
begin
...
function TCustomMemData.SupportedFieldType(AType: TFieldType): Boolean;
begin
  Result := GetNoByFieldType(AType) <> -1;
end;

function TCustomMemData.GetFieldClass(FieldType: TFieldType): TFieldClass;
begin
     case FieldType of
...
procedure TCustomMemData.InternalOpen;
var
  i : Integer;
begin
  for i := 0 to FieldCount - 1 do
...
procedure TCustomMemData.InternalClose;
begin
  FData.Clear;
  FBookMarks.Clear;
  FFilterList.Clear;
...
function TCustomMemData.IsCursorOpen: Boolean;
begin
  Result := FActive;
end;

procedure TCustomMemData.InternalInitFieldDefs;
var
  i : Integer;
begin
  FieldDefs.Clear;
...
procedure TCustomMemData.InternalHandleException;
begin
  HandleException(Self);
end;

procedure TCustomMemData.InternalGotoBookmark(Bookmark: Pointer);
var
...
procedure TCustomMemData.InternalSetToRecord(Buffer: TRecordBuffer);
begin
  InternalGotoBookmark(@PRecInfo(Buffer + FRecInfoOfs).Bookmark);
end;

function TCustomMemData.GetBookmarkFlag(Buffer: TRecordBuffer): TBookmarkFlag;
begin
  Result := PRecInfo(Buffer + FRecInfoOfs).BookmarkFlag;
end;

procedure TCustomMemData.SetBookmarkFlag(Buffer: TRecordBuffer; Value: TBookmarkFlag);
begin
  PRecInfo(Buffer + FRecInfoOfs).BookmarkFlag := Value;
end;

procedure TCustomMemData.GetBookmarkData(Buffer: TRecordBuffer; Data: Pointer);
begin
  PInteger(Data)^ := PRecInfo(Buffer + FRecInfoOfs).Bookmark;
end;

procedure TCustomMemData.GetBookmarkData(Buffer: TRecordBuffer; Data: TBookmark);
begin
  Assert(Length(Data) = BookmarkSize);
  GetBookmarkData(Buffer, @Data[0]);
end;

procedure TCustomMemData.SetBookmarkData(Buffer: TRecordBuffer; Data: Pointer);
begin
  PRecInfo(Buffer + FRecInfoOfs).Bookmark := PInteger(Data)^;
end;

procedure TCustomMemData.SetBookmarkData(Buffer: TRecordBuffer; Data: TBookmark);
begin
  Assert(Length(Data) = BookmarkSize);
  SetBookmarkData(Buffer, @Data[0]);
end;

function TCustomMemData.GetCurrentRecord(Buffer: TRecordBuffer): Boolean;
begin
...
function TCustomMemData.GetRecordSize: Word;
begin
  Result := FRecInfoOfs;
end;

procedure TCustomMemData.Loaded;
begin
  inherited Loaded;
...
function TCustomMemData.AllocRecordBuffer: TRecordBuffer;
begin
  Result := AllocMem(FRecBufSize + BlobFieldCount * SizeOf(Pointer));
  InitializeBlobData(GetRecordData(Result));
end;

procedure TCustomMemData.FreeRecordBuffer(var Buffer: TRecordBuffer);
begin
...
function TCustomMemData.GetRecord(Buffer: TRecordBuffer; GetMode: TGetMode;
...
procedure TCustomMemData.InternalInitRecord(Buffer: TRecordBuffer);
begin
...
function TCustomMemData.GetActiveRecBuf(var ARecordBuffer: TRecordBuffer): Boolean;
begin
  case State of
    dsBrowse:
      if IsEmpty then
...
function TCustomMemData.GetFieldData(Field: TField; var Buffer: TValueBuffer): Boolean;
var
...
function TCustomMemData.GetFieldData(Field: TField; var Buffer: TValueBuffer; NativeFormat: Boolean): Boolean;
begin
...
procedure TCustomMemData.SetFieldData(Field: TField; Buffer: TValueBuffer);
var
  RecBuf : TRecordBuffer;
begin
...
procedure TCustomMemData.SetFieldData(Field: TField; Buffer: TValueBuffer; NativeFormat: Boolean);
begin
  if (Field.DataType = ftWideString) then
...
function TCustomMemData.GetStateFieldValue(State: TDataSetState; Field: TField): Variant;
var
  mField: TMemField;
begin
...
procedure TCustomMemData.InternalFirst;
begin
  FCurRec := -1;
  FFilterCurRec := -1;
end;

procedure TCustomMemData.InternalLast;
begin
  if not FIsFiltered then
...
procedure TCustomMemData.DoAfterCancel;
begin
  if not IsBinaryDataLoading then
    inherited DoAfterCancel;
end;

procedure TCustomMemData.DoAfterClose;
begin
  if not IsBinaryDataLoading then
    inherited DoAfterClose;
end;

procedure TCustomMemData.DoAfterInsert;
begin
  if not IsBinaryDataLoading then
    inherited DoAfterInsert;
end;

procedure TCustomMemData.DoAfterOpen;
begin
  if not IsBinaryDataLoading then
  begin
...
procedure TCustomMemData.DoAfterPost;
begin
  if not IsBinaryDataLoading then
    inherited DoAfterPost;
end;

procedure TCustomMemData.DoBeforeClose;
begin
  if not IsBinaryDataLoading then
    inherited DoBeforeClose;
end;

procedure TCustomMemData.DoBeforeInsert;
begin
  if not IsBinaryDataLoading then
    inherited ;
end;

procedure TCustomMemData.DoBeforeOpen;
begin
  if not IsBinaryDataLoading then
    inherited ;
end;

procedure TCustomMemData.DoBeforePost;
begin
  if not IsBinaryDataLoading then
    inherited DoBeforePost;
end;

procedure TCustomMemData.DoOnNewRecord;
begin
  if not IsBinaryDataLoading then
    inherited DoOnNewRecord;
end;

procedure TCustomMemData.InternalAddFilterRecord;
var
  i : Integer;
begin
...
procedure TCustomMemData.MakeRecordSort;
var
  mField : TMemField;
...
procedure TCustomMemData.UpdateRecordFilteringAndSorting(AIsMakeSort : Boolean);
begin
...
function TCustomMemData.InternalIsFiltering: Boolean;
begin
  Result := Assigned(OnFilterRecord) and Filtered;
end;

procedure TCustomMemData.InternalPost;
var
  ABuf: TRecordBuffer;
  AIsMakeSort: Boolean;
  AmField: TMemField;
...
procedure TCustomMemData.InternalInsert;
var
  buf: TRecordBuffer;
  Value: Integer;
  mField: TMemField;
...
procedure TCustomMemData.InternalAddRecord(ABuffer: Pointer; AAppend: Boolean);
begin
  FSaveChanges := True;
  Inc(FLastBookmark);
  if AAppend then
...
procedure TCustomMemData.InternalAddRecord(ABuffer: TRecordBuffer; AAppend: Boolean);
begin
  InternalAddRecord(Pointer(ABuffer), AAppend);
end;

procedure TCustomMemData.InternalDelete;
var
  i : Integer;
  p : TRecordBuffer;
begin
...
procedure TCustomMemData.GetCalcFields(Buffer: TRecordBuffer);
begin
...
function TCustomMemData.GetRecordCount: Integer;
begin
  if Not FIsFiltered then
...
function TCustomMemData.GetRecNo: Integer;
begin
...
function TCustomMemData.InternalSetRecNo(const Value: Integer): Integer;
begin
  if not FIsFiltered then
    Result := Value - 1
  else
...
procedure TCustomMemData.SetRecNo(Value: Integer);
var
  NewCurRec : Integer;
begin
  if Active then
...
procedure TCustomMemData.SetFilteredRecNo(Value: Integer);
var
  Index : Integer;
begin
...
function TCustomMemData.GetCanModify: Boolean;
begin
  Result := not FReadOnly or IsBinaryDataLoading;
end;

procedure TCustomMemData.ClearCalcFields(Buffer: TRecordBuffer);
var
  i : Integer;
  mField: TMemField;
begin
...
procedure TCustomMemData.SetFiltered(Value: Boolean);
var
...
function TCustomMemData.GetAnsiStringValue(const ABuffer: TRecordBuffer): AnsiString;
begin
  Result := AnsiString(PAnsiChar(ABuffer));
end;

function TCustomMemData.GetWideStringValue(const ABuffer: TRecordBuffer): WideString;
begin
  Result := WideString(PWideChar(ABuffer));
end;

function TCustomMemData.GetVariantValue(const ABuffer: TRecordBuffer; AField: TField): Variant;
var
  AFakeDataSet: TFakeDataSet;
  AFakeField: TField;
begin
...
function TCustomMemData.GetIntegerValue(const ABuffer : TRecordBuffer;
...
function TCustomMemData.GetLargeIntValue(const ABuffer: TRecordBuffer): Largeint;
begin
  Move(ABuffer^, Result, SizeOf(Largeint));
end;

function TCustomMemData.GetFloatValue(const ABuffer: TRecordBuffer): Double;
begin
  Move(ABuffer^, Result, SizeOf(Double));
end;

function TCustomMemData.GetExtendedValue(const ABuffer: TRecordBuffer): Extended;
begin
  Move(ABuffer^, Result, SizeOf(Extended));
end;

function TCustomMemData.GetCurrencyValue(const ABuffer: TRecordBuffer): System.Currency;
begin
  Move(ABuffer^, Result, SizeOf(System.Currency));
end;

function TCustomMemData.GetDateTimeValue(const ABuffer: TRecordBuffer; AField: TField): TDateTime;
begin
{$WARNINGS OFF}
...
function TCustomMemData.GetTimeStampValue(const ABuffer: TRecordBuffer; AField: TField): TSQLTimeStamp;
begin
{$WARNINGS OFF}
  DataConvert(AField, ABuffer, @Result, False);
{$WARNINGS ON}
...
function TCustomMemData.CompareValues(const ABuffer1, ABuffer2: TRecordBuffer; AMemField: TMemField; ASortOptions: TSortOptions): Integer;
begin
  Result := InternalCompareValues(ABuffer1, ABuffer2, AMemField, soCaseInsensitive in ASortOptions);
end;

function TCustomMemData.CompareValues(const ABuffer1, ABuffer2: TRecordBuffer; AMemField: TMemField): Integer;
begin
  Result := CompareValues(ABuffer1, ABuffer2, AMemField, FSortOptions);
end;

function TCustomMemData.CompareValues(const ABuffer1, ABuffer2: TRecordBuffer; AField: TField): Integer;
begin
  Result := CompareValues(ABuffer1, ABuffer2, Data.IndexOf(AField));
end;

function TCustomMemData.InternalCompareValues(const ABuffer1, ABuffer2: Pointer;
  AMemField: TMemField; AIsCaseInsensitive: Boolean; ACount: Integer = -1): Integer;
...
function TCustomMemData.GetCalcBuffer: TMemDataBuffer;
begin
  Result := TMemDataBuffer(inherited CalcBuffer);
end;

function TCustomMemData.GetRecIdMemField: TMemField;
begin
  Result := Data.IndexOf(RecIdField)
end;

function TCustomMemData.AllocBufferForFieldValue(AValue: Variant; AField: TField): Pointer;
begin
  Result := AllocMem(GetDataSize(AValue, AField));
end;

function TCustomMemData.AllocBufferForField(AField: TField): Pointer;
begin
  Result := AllocMem(GetDataSize(Null, AField));
end;

function TCustomMemData.GetSortOptions : TSortOptions;
begin
  Result := FSortOptions;
end;

procedure TCustomMemData.PopulateValueList(const AList: TList);
var
  I: Integer;
begin
  AList.Clear;
...
procedure TCustomMemData.SetSortedField(Value : String);
begin
...
procedure TCustomMemData.SetSortOptions(Value : TSortOptions);
begin
...
procedure TCustomMemData.SetIndexes(Value : TMemIndexes);
begin
  fIndexes.Assign(Value);
end;

procedure TCustomMemData.SetPersistent(Value: TdxMemPersistent);
begin
  fPersistent.Assign(Value);
end;

procedure TCustomMemData.MakeSort;
var
  mField : TMemField;
...
procedure TCustomMemData.DoSort(ASortedList : TList; AmdField: TMemField;
  ASortOptions: TSortOptions; ASynchronizationLists: TList);
...
function TCustomMemData.InternalGotoNearest(AValues: TList; AField: TField;
  const ABuffer: TRecordBuffer; ASortOptions: TSortOptions; ALocateOptions: TLocateOptions; out AIndex: Integer) : Boolean;
var
  AMemField: TMemField;
...
function TCustomMemData.GotoNearest(const ABuffer: TRecordBuffer; ASortOptions: TSortOptions; ALocateOptions: TLocateOptions; out AIndex: Integer): Boolean;
begin
  AIndex := -1;
  Result := False;
...
procedure TCustomMemData.SetOnFilterRecord(const Value: TFilterRecordEvent);
begin
  inherited SetOnFilterRecord(Value);
  UpdateFilters;
end;

procedure TCustomMemData.UpdateFilterRecord;
var
  Accepted : Boolean;
begin
...
procedure TCustomMemData.UpdateFilters;
var
...
function TCustomMemData.GetValueCount(AFieldName: string; AValue: Variant): Integer;
var
  ABuf: TRecordBuffer;
  I: Integer;
  AMemField: TMemField;
...
procedure TCustomMemData.FillBookMarks;
var
  I: Integer;
begin
  FBookMarks.Clear;
...
procedure TCustomMemData.MoveCurRecordTo(AIndex: Integer);
var
...
procedure TCustomMemData.LoadFromTextFile(const AFileName: string);
var
...
procedure TCustomMemData.SaveToTextFile(const AFileName: string);
var
  AStrings: TStringList;
begin
  if not Active then
...
procedure TCustomMemData.LoadFromStrings(AStrings: TStrings);
  ...
  begin
    ...
    AOffset := 1;
    repeat
      ADelimiterPos := Pos(FDelimiterChar, S, AOffset);
...
begin
  ...
  if AError then
    raise EdxException.Create(DataInvalidStreamFormat);
end;

procedure TCustomMemData.SaveToStrings(AStrings: TStrings);
type
...
procedure TCustomMemData.CreateFieldsFromBinaryFile(const AFileName: string);
var
...
procedure TCustomMemData.CreateFieldsFromStream(Stream: TStream);
var
  AMemStreamReader: TMemDataStreamReader;
begin
  Close;
  AMemStreamReader := TMemDataStreamReader.Create(Self, Stream);
  try
...
procedure TCustomMemData.LoadFromStream(AStream: TStream);
var
  AMemReader: TMemDataStreamReader;
begin
  DisableControls;
  try
    FLoadFlag := True;
    try
      Close;
      Open;
      AMemReader := TMemDataStreamReader.Create(Self, AStream);
      try
        AMemReader.LoadData;
      finally
        AMemReader.Free;
...
procedure TCustomMemData.LoadFromBinaryFile(const AFileName: string);
var
  AStream : TMemoryStream;
begin
  AStream := TMemoryStream.Create;
...
procedure TCustomMemData.SaveToStream(AStream: TStream);
var
  AMemDataStreamWriter: TMemDataStreamWriter;
begin
  if not Active then
    Exit;
  CheckBrowseMode;
  AMemDataStreamWriter := TMemDataStreamWriter.Create(Self, AStream);
  try
...
procedure TCustomMemData.SaveToBinaryFile(const AFileName: string);
var
...
function TCustomMemData.CreateBlobStream(Field: TField; Mode: TBlobStreamMode): TStream;
begin
  Result := TMemBlobStream.Create(TBlobField(Field), Mode);
end;

procedure TCustomMemData.CloseBlob(Field: TField);
begin
...
procedure TCustomMemData.BlobClear;
var
  i : Integer;
  p : TRecordBuffer;
begin
...
function TCustomMemData.ActiveBuffer: TMemDataBuffer;
begin
  Result := TMemDataBuffer(inherited ActiveBuffer);
end;

procedure TCustomMemData.InitializeBlobData(ABuffer: TRecordBuffer);
begin
  if BlobFieldCount = 0 then Exit;
  cxZeroMemory(ABuffer, BlobFieldCount * SizeOf(TdxNativeInt));
end;

procedure TCustomMemData.FinalizeBlobData(ABuffer: TRecordBuffer);
var
  I: Integer;
  ptr: TRecordBuffer;
begin
...
function TCustomMemData.GetRecordData(ARecordBuffer: TRecordBuffer): TRecordBuffer;
begin
  Result := ShiftPointer(ARecordBuffer, FRecBufSize);
end;

function TCustomMemData.GetBlobInfo(ADataBuffer: TRecordBuffer; AOffSet: Integer; out ABlobData: TRecordBuffer; out ABlobSize: Integer): Boolean;
var
  APtr: TRecordBuffer;
begin
...
function TCustomMemData.GetBlobInfo(ARecordBuffer: TRecordBuffer; AField: TField; out ABlobData: TRecordBuffer; out ABlobSize: Integer): Boolean;
begin
  Result := GetBlobInfo(GetRecordData(ARecordBuffer), AField.Offset, ABlobData, ABlobSize);
end;

function TCustomMemData.GetBlobPointer(ADataBuffer: TRecordBuffer; AOffSet: Integer): TRecordBuffer;
var
  APtr: TRecordBuffer;
begin
  Result := nil;
...
function TCustomMemData.GetBlobData(ADataBuffer: TRecordBuffer; AOffSet: Integer): TMemBlobData;
var
  ASize: Integer;
  AData: TRecordBuffer;
begin
...
function TCustomMemData.GetBlobData(ARecordBuffer: TRecordBuffer; AField: TField): TMemBlobData;
begin
  Result := GetBlobData(GetRecordData(ARecordBuffer), AField.Offset);
end;

procedure TCustomMemData.SetInternalBlobData(ADataBuffer: TRecordBuffer; AOffSet: Integer; AValue: TRecordBuffer; ASize: Integer);
var
...
procedure TCustomMemData.SetInternalBlobData(ADataBuffer: TRecordBuffer; AOffSet: Integer; AValue: TMemBlobData);
begin
  SetInternalBlobData(ADataBuffer, AOffset, TRecordBuffer(AValue), Length(AValue));
end;

procedure TCustomMemData.SetBlobData(ADataBuffer: TRecordBuffer; AOffSet: Integer; const AValue: TRecordBuffer; ASize: Integer);
begin
...
procedure TCustomMemData.SetBlobData(ARecordBuffer: TRecordBuffer; AField: TField; const AValue: TMemBlobData);
begin
  SetBlobData(GetRecordData(ARecordBuffer), AField.Offset, TRecordBuffer(AValue), Length(AValue));
end;

procedure TCustomMemData.SetBlobData(ARecordBuffer: TRecordBuffer; AField: TField; const AValue: TRecordBuffer; ASize: Integer);
begin
  SetBlobData(GetRecordData(ARecordBuffer), AField.Offset, AValue, ASize);
end;

function TCustomMemData.GetActiveBlobData(Field: TField): TMemBlobData;
var
  i : Integer;
begin
  Result := '';
...
procedure TCustomMemData.GetMemBlobData(Buffer : TRecordBuffer);
var
  i : Integer;
begin
  if BlobFieldCount > 0 then
...
procedure TCustomMemData.SetMemBlobData(Buffer : TRecordBuffer);
var
  p : TRecordBuffer;
  i, Pos : Integer;
begin
...
procedure TCustomMemData.CreateFieldsFromDataSet(ADataSet: TDataSet; AOwner: TComponent);
begin
...
procedure TCustomMemData.AddFieldsFromDataSet(ADataSet: TDataSet; AOwner: TComponent);
...
procedure TCustomMemData.CopyFromDataSet(DataSet : TDataSet);
begin
  Close;
...
procedure TCustomMemData.LoadFromDataSet(DataSet : TDataSet);
...
{TMemIndex}
constructor TMemIndex.Create(Collection: TCollection);
begin
...
procedure TMemIndex.Assign(Source: TPersistent);
begin
  if Source is TMemIndex then
  begin
    FieldName := TMemIndex(Source).FieldName;
    SortOptions := TMemIndex(Source).SortOptions;
  end
  else
    inherited Assign(Source);
end;

procedure TMemIndex.Prepare;
var
  I: Integer;
  mField: TMemField;
  ASynchronizationLists: TList;
...
function TMemIndex.GotoNearest(const ABuffer: TRecordBuffer; ALocateOptions: TLocateOptions; out AIndex: Integer) : Boolean;
begin
  Result := False;
  Prepare;
...
procedure TMemIndex.SetIsDirty(Value: Boolean);
begin
...
procedure TMemIndex.DeleteRecord(ABuffer: TRecordBuffer);
begin
  IsDirty := True;
end;
...
procedure TMemIndex.UpdateRecord(ABuffer: TRecordBuffer);
var
  AValueIndex: Integer;
...
procedure TMemIndex.SetFieldName(Value: String);
var
  AField : TField;
begin
...
procedure TMemIndex.SetSortOptions(Value: TSortOptions);
begin
...
procedure TMemIndex.SetFieldNameAfterMemdataLoaded;
begin
  if (fLoadedFieldName <> '') then
    FieldName := fLoadedFieldName;
end;

function TMemIndex.GetMemData: TCustomMemData;
begin
  Result := TMemIndexes(Collection).fMemData;
end;

{ TdxReadStringField }

procedure TdxStringFieldReader.CreateField(AMemData: TCustomMemData);
begin
  inherited;
...
{TMemIndexes}
function TMemIndexes.GetOwner: TPersistent;
begin
  Result := fMemData;
end;

procedure TMemIndexes.SetIsDirty;
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
    TMemIndex(Items[i]).IsDirty := True;
end;

procedure TMemIndexes.DeleteRecord(ABuffer: TRecordBuffer);
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
    TMemIndex(Items[i]).DeleteRecord(ABuffer);
end;

procedure TMemIndexes.UpdateRecord(ABuffer: TRecordBuffer);
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
    TMemIndex(Items[i]).UpdateRecord(ABuffer);
end;

procedure TMemIndexes.RemoveField(AField: TField);
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
    if(TMemIndex(Items[i]).fField = AField) then
    begin
      TMemIndex(Items[i]).fField := nil;
      TMemIndex(Items[i]).IsDirty := True;
    end;
end;

procedure TMemIndexes.CheckFields;
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
  begin
    TMemIndex(Items[i]).fField := fMemData.FindField(TMemIndex(Items[i]).FieldName);
    TMemIndex(Items[i]).IsDirty := True;
  end;
end;

procedure TMemIndexes.AfterMemdataLoaded;
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
    TMemIndex(Items[i]).SetFieldNameAfterMemdataLoaded;
end;

function TMemIndexes.Add: TMemIndex;
begin
  Result := TMemIndex(inherited Add);
end;

function TMemIndexes.GetIndexByField(AField: TField): TMemIndex;
var
  i: Integer;
begin
  Result := nil;
...
constructor TMemField.Create(AOwner: TMemFields);
begin
  inherited Create;
  FOwner := AOwner;
...
procedure TMemField.CreateField(AField: TField);
var
  I: Integer;
  mField: TMemField;
  AIsRecId: Boolean;
begin
  ...
  begin
    mField := TMemField(FOwner.FItems[FIndex - 1]);
...
function TMemField.GetActiveBuffer(AActiveBuffer, ABuffer: TRecordBuffer): Boolean;
var
  AData: Pointer;
begin
...
procedure TMemField.SetActiveBuffer(AActiveBuffer, ABuffer: TRecordBuffer);
var
  AData: Pointer;
begin
  AData := GetDataFromBuffer(AActiveBuffer);
...
procedure TMemField.SetAutoIncValue(const AValue, ADataPointer: TRecordBuffer);
var
  ANewValue: Integer;
begin
  if (AValue <> nil) then
...
procedure TMemField.AddValue(AValue: TRecordBuffer);
begin
  if FIndex = 0 then
...
procedure TMemField.InsertValue(AIndex: Integer; ABuffer: TRecordBuffer);
begin
...
function TMemField.GetDataFromBuffer(ABuffer: TRecordBuffer): TRecordBuffer;
begin
  Result := ShiftPointer(ABuffer, FOffSet);
end;

function TMemField.GetHasValueFromBuffer(ABuffer: TRecordBuffer): AnsiChar;
begin
  Result := AnsiChar(ReadByte(ABuffer, FOffSet));
end;

function TMemField.GetValueFromBuffer(ABuffer: TRecordBuffer): TRecordBuffer;
begin
...
function TMemField.DataPointer(AIndex, AOffset: Integer): TRecordBuffer;
begin
  Result := ShiftPointer(FOwner.FValues[AIndex], AOffset);
end;

procedure TMemField.InternalAddValue(AValue: TRecordBuffer);
var
  AData: Pointer;
begin
...
function TMemField.GetValues(AIndex: Integer): TRecordBuffer;
begin
...
function TMemField.GetHasValue(AIndex: Integer): Boolean;
begin
  Result := HasValues[AIndex] <> #0;
end;

function TMemField.GetHasValues(AIndex: Integer): AnsiChar;
begin
  Result := AnsiChar(ReadByte(DataPointer(AIndex, FOffSet)));
end;

procedure TMemField.SetHasValue(AIndex: Integer; AValue: Boolean);
const
...
procedure TMemField.SetHasValues(AIndex: Integer; AValue: AnsiChar);
begin
  WriteByte(DataPointer(AIndex, FOffSet), Byte(AValue));
end;

function TMemField.GetDataSet : TCustomMemData;
begin
  Result := MemFields.DataSet;
end;

function TMemField.GetMemFields : TMemFields;
begin
  Result := FOwner;
end;
...
procedure TdxFieldReader.CreateField(AMemData: TCustomMemData);
begin
...
constructor TdxMemPersistent.Create(AMemData: TCustomMemData);
begin
  inherited Create;
...
{TMemFields}
constructor TMemFields.Create(ADataSet : TCustomMemData);
begin
  inherited Create;
  FDataSet := ADataSet;
  FItems := TList.Create;
...
function TMemFields.AllocateRecordMemory(ANewRecordIndex: Integer = -1): TRecordBuffer;
begin
...
procedure TMemFields.DeleteRecord(AIndex : Integer);
begin
  FreeMem(Pointer(FValues[AIndex]));
  FValues.Delete(AIndex);
end;

function TMemFields.Add(AField : TField) : TMemField;
begin
  Result := TMemField.Create(self);
  FItems.Add(Result);
  Result.CreateField(AField);
end;

function TMemFields.GetItem(Index : Integer)  : TMemField;
begin
  Result := TMemField(FItems[Index]);
end;

function TMemFields.IndexOf(Field : TField) : TMemField;
var
  i : Integer;
begin
  Result := Nil;
...
function TMemFields.GetValue(mField : TMemField; Index : Integer) : TRecordBuffer;
begin
  Result := mField.Values[Index];
end;

function TMemFields.GetHasValue(mField: TMemField; Index: Integer) : AnsiChar;
begin
  Result := mField.GetHasValues(Index);
end;

procedure TMemFields.SetValue(mField: TMemField; Index: Integer; Buffer: TRecordBuffer);
const
...
procedure TMemFields.SetHasValue(mField: TMemField; Index: Integer; Value: AnsiChar);
begin
...
procedure TMemFields.GetBuffer(Buffer : TRecordBuffer; AIndex : Integer);
begin
  cxCopyData(Pointer(FValues[AIndex]), Buffer, FValuesSize);
end;

procedure TMemFields.SetBuffer(Buffer : TRecordBuffer; AIndex : Integer);
begin
  if AIndex = -1 then exit;
  cxCopyData(Buffer, Pointer(FValues[AIndex]), FValuesSize);
end;

function TMemFields.GetActiveBuffer(AActiveBuffer, ABuffer: TRecordBuffer; Field : TField) : Boolean;
var
  mField : TMemField;
begin
...
procedure TMemFields.SetActiveBuffer(AActiveBuffer, ABuffer: TRecordBuffer; Field : TField);
var
  mField : TMemField;
begin
  mField := IndexOf(Field);
...
function TMemFields.GetRecordCount : Integer;
begin
  if(FValues = nil) then
    Result := 0
...
procedure TMemFields.InsertRecord(const ABuffer: TRecordBuffer; AIndex: Integer; AAppend: Boolean);
var
  I: Integer;
  AData: TRecordBuffer;
  mField : TMemField;
...
procedure TMemFields.AddField(Field : TField);
var
  mField : TMemField;
begin
  mField := IndexOf(Field);
...
procedure TMemFields.RemoveField(Field : TField);
var
  mField : TMemField;
begin
  mField := IndexOf(Field);
