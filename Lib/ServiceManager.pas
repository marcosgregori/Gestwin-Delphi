...
type
  ...
  public
    ...
    function ServiceStop(Wait: Boolean) : DWORD;
...
begin
     If   FHandle<>0
     then begin
          If   not QueryServiceStatus( FHandle, Status )
...
begin
     ...
     try
       P := nil;
       If   not StartService( FHandle, 0, P )
       then RaiseLastOSError;
       If   Wait
       then WaitFor( SERVICE_RUNNING );
     finally
       CleanupHandle;
       end;
end;

function TServiceInfo.ServiceStop( Wait : Boolean ) : DWORD;
...
begin
     ...
     try
       If   not ( saStop in Accepts )
       then ErrorCode := ERROR_INVALID_ACCESS       // raise Exception.Create( 'No se ha podido detener el servicio' );
       else If   not ControlService( FHandle, SERVICE_CONTROL_STOP, Status )
            then ErrorCode := GetLastError;         // RaiseLastOSError;
       If   ( ErrorCode=ERROR_SUCCESS ) and Wait
       then WaitFor( SERVICE_STOPPED );
     finally
       CleanupHandle;
       Result := ErrorCode;
       end;
...
begin
     ...
     While State<>FServiceStatus.dwCurrentState do
       ...
       Sleep( Wait );
       Query;
       If   State=FServiceStatus.dwCurrentState
       then Break;
       If   FServiceStatus.dwCheckPoint<>OldCheckPoint
       then raise Exception.Create( 'El servicio no ha reaccionado en el tiempo esperado' );
       end;
end;
...
begin
     ...
     then Result := Result + [ saPauseContinue ];
     If   FServiceStatus.dwControlsAccepted and SERVICE_ACCEPT_STOP<>0
     then Result := Result + [ saStop ];
     If   FServiceStatus.dwControlsAccepted and SERVICE_ACCEPT_SHUTDOWN<>0
     then Result := Result + [ saShutdown ];
end;

procedure TServiceInfo.SetState( const Value : TServiceState );

var  OldState: TServiceState;
...
begin
     ...
     case Value of
       ssStopped :
         If   OldState<>ssStopped
         then ServiceStop( True );
       ssRunning :
         case OldState of
           ssStopped :
             ServiceStart( True );
       ...
       ssPaused :
         case OldState of
           ...
           ssRunning :
             ...
             raise Exception.Create( 'No se puede fijar un estado de transición a TServiceInfo.' );
           end;
end;
