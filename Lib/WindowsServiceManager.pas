...
var   ServiceApplication : TServiceApplication = nil;

const DefaultServiceName : String = 'GestwinRPCServerV11';

function WSMOpenService(const aServiceName: string; QueryOnly: boolean = false): SC_HANDLE;
function WSMIsService(const aServiceName: string): Boolean;
...
{$ENDIF}
  System.UITypes, Forms, Dialogs, Consts;

resourceString
    SServiceFailed = 'El servicio ha fallado en %s: %s';
    SExecute = 'ejecución';
    SStart = 'inicio';
    SStop = 'parada';
    SPause = 'pausa';
    ...
    SShutdown = 'desconectar';
    SCustomError = 'El servicio ha fallado con el mensaje (%d): %s';
    SServiceInstallOK = 'Servicio instalado con éxito.';
    SServiceInstallFailed = 'El servicio "%s" no se ha podido instalar. Error : "%s"';
    SServiceUninstallOK = 'Servicio des-instalado con éxito.';
    SServiceUninstallFailed = 'El servicio "%s" no se ha podido des-instalar. Error : "%s"';
...
begin
  ...
  if (FServiceType = stWin32) and (ServiceApplication.ServiceCount > 1) then
...
begin
     ...
     ServiceController := Self.GetServiceController();
     FStatusHandle := RegisterServiceCtrlHandler( Name, ServiceController );
     If   FStatusHandle=0
     then LogMessage( SysErrorMessage( GetLastError ) )
     else DoStart;
end;
...
var   I : Integer;
      ServiceController: THandlerFunction;
...
{$ENDIF}
begin
  ServiceApplication.DispatchServiceMain(Argc, Argv);
end;
...
begin
  ...
  FInitialized := False;
  // Forms.ServiceApplication.HookMainWindow(Hook);
end;

destructor TServiceApplication.Destroy;
begin
  FreeAndNil(FEventLogger);
  If   Assigned( Forms.Application )
  then Forms.Application.OnException := nil;
  // Forms.ServiceApplication.UnhookMainWindow(Hook);
  inherited Destroy;
end;
...
{$ENDIF}
  ...
  begin
    if IsConsole then
      ...
      MessageDlg(Msg, MsgType, [ TMsgDlgBtn.mbOk ], 0);
  end;
      ...
      try
        ...
        begin
          ...
          with TService(Components[i]) do
            DisplayMessage(Format(Msg, [DisplayName, E.Message]), TMsgDlgType.mtError);
        end;
      end;
    if Success and not Silent then
      if Install then
        DisplayMessage(SServiceInstallOK, TMsgDlgType.mtInformation)
      else
        DisplayMessage(SServiceUninstallOK, TMsgDlgType.mtInformation);
  finally
...
begin
  ...
  begin
    FInitialized := True;
    // Forms.ServiceApplication.ShowMainForm :=False;
...
begin
  ...
  // PostMessage(Forms.ServiceApplication.Handle, WM_QUIT, 0, 0);
end;
...
begin
     ...
     {$ENDIF}
     If   FindSwitch( 'INSTALL' )
     then RegisterServices( True, FindSwitch( 'SILENT' ) )
     else If   FindSwitch( 'UNINSTALL' )
          then RegisterServices( False, FindSwitch( 'SILENT' ) )
          else begin
               Forms.Application.OnException := OnExceptionHandler;
               SC := 0;
               For I := 0 to ComponentCount - 1 do
                 If   Components[ I ] is TService
                 then Inc( SC );
               SetLength( ServiceStartTable, SC + 1 );
               {$IF NOT DEFINED(CLR)}
               FillChar( ServiceStartTable[ 0 ], SizeOf( TServiceTableEntry ) * ( SC + 1 ), 0 );
               {$ENDIF}
               J := 0;
               For I := 0 to ComponentCount - 1 do
                 If   Components[ I ] is TService
                 ...
                 try
                   While not Forms.Application.Terminated do
                     ...
                     {$ENDIF}
                     If   StartThread.ReturnValue<>0
                     then FEventLogger.LogMessage(SysErrorMessage(StartThread.ReturnValue));
                 finally
                   StartThread.Free;
                   end;
...
begin
     ServiceApplication.Free;
     ServiceApplication := nil;
end;

// Funciones

function WSMOpenService( const AServiceName : String;
                               QueryOnly    : Boolean = false): SC_HANDLE;

var ServiceManager: THandle;

begin
     If   QueryOnly
     then ServiceManager := OpenSCManager(nil, nil, SC_MANAGER_CONNECT )
     else ServiceManager := OpenSCManager(nil, nil, SC_MANAGER_ALL_ACCESS );
     If   ServiceManager=0
     then Result := 0
     else try
            If   QueryOnly
            then Result := WinAPI.WinSvc.OpenService(ServiceManager, PChar(aServiceName), SERVICE_QUERY_STATUS )
            else Result := WinAPI.WinSvc.OpenService(ServiceManager, PChar(aServiceName), SERVICE_ALL_ACCESS );
          finally
...
begin
     Result := True;
     try
       ServiceHandle := WSMOpenService( AServiceName, True );
       If   ServiceHandle=0
       then Result := False
