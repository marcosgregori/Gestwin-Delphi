...
uses
  ...
  dxUIABase, dxUIAClasses, dxUIATextControl;
...
{ TcxCustomTextEditProperties }
  ...
  private
    ...
    // UI Automation
    function GetUIACaretState: TUIACaretState;
  ...
  protected
    ...
    function GetInnerEditUIAProvider: IdxUIAProvider; override;
    function GetUIATextControlHelper: IdxUIATextControl;
    procedure UIATextChanged;
...
type
  ...
  TcxCustomTextEditUIATextControl = class(TInterfacedObject, IdxUIATextControl)
  private
    FControl: TcxCustomTextEdit;
    FInner: TcxCustomInnerTextEdit;
    function ConvertToPasswordCharIfNeeded(const AText: string): string;
  protected
    // IdxUIATextControl
    function GetLineCount: Integer;
    function GetLineFromCharIndex(ACharIndex: Integer): Integer;
    function GetCharIndexFromLine(ALine: Integer): Integer;
    function IsPassword: Boolean;
    function FindText(AInRange: TdxUIATextRange; AText: string; AFromEnd: Boolean;
      AIgnoreCase: Boolean; out AResultRange: TdxUIATextRange): Boolean;
    procedure FixRange(var ARange: TdxUIATextRange; ASoft: Boolean);
    function GetBoundingRectangles(ARange: TdxUIATextRange): TRects;
    function GetTextWithinRange(ARange: TdxUIATextRange): string;
    procedure Select(ARange: TdxUIATextRange);

    function GetCaretPosition: Integer;
    function GetCaretPositionInfo: Integer;
    function GetFocused: Boolean;
    function GetSelection: TdxUIATextRange;
    function GetMaxEnpointValue: Integer;
    function GetAllTextRange: TdxUIATextRange;
    function GetText: string;
    function GetInstance: TWinControl;
    function GetInnerControl: TWinControl;
...
begin
  Result := TdxUIAInnerEditProvider.Create(Container.GetUIATextControlHelper);
end;
...
begin
  Result := TdxUIACustomTextEditProvider.Create(Self);
end;
...
begin
  if dxUIARaiseEvent(GetInnerEditUIAProvider, UIA_Text_TextSelectionChangedEventId) <> S_OK then
    ;
end;
...
function TcxCustomTextEdit.GetInnerEditUIAProvider: IdxUIAProvider;
begin
...
function TcxCustomTextEdit.GetCaretPosition: Integer;
var
  P: TPoint;
begin
  if Winapi.Windows.GetCaretPos(P) then
  begin
    InnerControl.ScreenToClient(P);
    Result := LoWord(SendMessage(InnerControl.Handle, EM_CHARFROMPOS, 0, MakeLong(Word(P.X), Word(P.Y))));
  end
  else
    Result := 0;
end;
...
function TcxCustomTextEdit.GetSelection: TSelection;
begin
  SendMessage(InnerControl.Handle, EM_GETSEL, WPARAM(@Result.StartPos), LPARAM(@Result.EndPos));
end;
...
function TcxCustomTextEdit.GetUIACaretState: TUIACaretState;
var
  ASelection: TSelection;
begin
  ASelection := GetSelection;
  Result.Selection.First := ASelection.StartPos;
  Result.Selection.Last := ASelection.EndPos;
  Result.Caret := GetCaretPosition;
end;
...
var
  AProvider: IdxUIAProvider;
begin
  if HandleAllocated and dxUIAHasActiveClients then
  begin
    AProvider := GetInnerEditUIAProvider;
    if Assigned(AProvider) then
    begin
      if dxUIARaiseEvent(AProvider, UIA_Text_TextChangedEventId) <> S_OK then
        ;
    end;
  end;
end;
...
function TcxCustomTextEditUIATextControl.ConvertToPasswordCharIfNeeded(
  const AText: string): string;
begin
  if IsPassword then
    Result := StringOfChar(GetWindowsPasswordChar(FControl.LookAndFeel.NativeStyle, FControl.ActiveProperties.PasswordChar), Length(AText))
  else
    Result := AText;
end;

constructor TcxCustomTextEditUIATextControl.Create(
  AControl: TcxCustomTextEdit);
begin
  inherited Create;
  FControl := AControl;
  FInner := AControl.InnerControl as TcxCustomInnerTextEdit;
end;

function TcxCustomTextEditUIATextControl.FindText(
  AInRange: TdxUIATextRange; AText: string; AFromEnd,
  AIgnoreCase: Boolean; out AResultRange: TdxUIATextRange): Boolean;
var
  AInText: string;
  AIdx: Integer;
begin
  FixRange(AInRange, False);
  AInText := GetTextWithinRange(AInRange);
  if AIgnoreCase then
  begin
    AInText := ConvertToPasswordCharIfNeeded(AInText.ToLower);
    AText := ConvertToPasswordCharIfNeeded(AText.ToLower);
  end;
  if AFromEnd then
    AIdx := AInText.LastIndexOf(AText)
  else
    AIdx := AInText.IndexOf(AText);
  Result := AIdx <> -1;
  if Result then
  begin
    AResultRange := AInRange;
    AResultRange.First := AInRange.First + AIdx;
    AResultRange.Last := AResultRange.First + Length(AText);
  end;
end;

procedure TcxCustomTextEditUIATextControl.FixRange(
  var ARange: TdxUIATextRange; ASoft: Boolean);
var
  AMax: Integer;
begin
  ...
  if ARange.Last < 0 then
    ARange.Last := 0;
  AMax := GetMaxEnpointValue;
  if ARange.First > AMax then
    ARange.First := AMax;
  if ARange.Last > AMax then
    ARange.Last := AMax;
  if ARange.First > ARange.Last then
    ARange.First := ARange.Last;
  if not ASoft then
  begin
  end;
end;

function TcxCustomTextEditUIATextControl.GetAllTextRange: TdxUIATextRange;
begin
  Result.First := 0;
  Result.Last := Length(FControl.Text);
end;

function TcxCustomTextEditUIATextControl.GetBoundingRectangles(
  ARange: TdxUIATextRange): TRects;

  procedure CalcClientAreas(AControl: TWinControl; out AClientRect: TRect; out ANCAreaSizes: TRect);
  var
    AScreenClientRect: TRect;
    AWindowRect: TRect;
  begin
    AClientRect := AControl.ClientRect;
    AScreenClientRect := dxClientToScreen(AControl, AClientRect);
    GetWindowRect(AControl.Handle, AWindowRect);
    ANCAreaSizes := TRect.Create(
      AScreenClientRect.Left - AWindowRect.Left,
      AScreenClientRect.Top - AWindowRect.Top,
      AWindowRect.Right - AScreenClientRect.Right,
      AWindowRect.Bottom - AScreenClientRect.Bottom);
  end;

  function PosFromChar(AHandle: HWND; ACharPosition: Integer): TPoint;
  var
    ARes: Integer;
  begin
    ARes := Integer(SendMessage(AHandle, EM_POSFROMCHAR, ACharPosition, 0));
    Result.X := LongRec(ARes).Lo;
    Result.Y := LongRec(ARes).Hi;
  end;
...
var
  ...
  AFirst: Integer;
  ALast: Integer;
  AControlClientRect: TRect;
  AControlNCAreaSizes: TRect;
  ATextLen: Integer;
  AText: string;
  ATextWindow: TWinControl;
begin
  ATextWindow := FControl.InnerControl;
  if (not Assigned(ATextWindow)) or (not ATextWindow.HandleAllocated) then
  begin
    Result := [cxNullRect];
    Exit;
  end;
  ...
  if AFirst < 0 then
    AFirst := 0;
  ALast := ARange.Last;
  if ALast > ATextLen then
    ALast := ATextLen - 1;
  CalcClientAreas(ATextWindow, AControlClientRect, AControlNCAreaSizes);

  if ATextLen <> 0 then
  begin
    if (AFirst = ATextLen) and (ALast = ATextLen) then
      AFirst := ATextLen - 1;
  end;

  if AFirst > 0 then
    R.Left := PosFromChar(ATextWindow.Handle, AFirst).X
  else
    R.Left := 0;
  if R.Left < 0 then
    R.Left := 0;
  ...
  if ALast < ATextLen then
  begin
    R.Right := PosFromChar(ATextWindow.Handle, ALast).X;
  end
  else
  begin
    R.Right := PosFromChar(ATextWindow.Handle, ALast - 1).X;
    R.Right := R.Right +
      TdxTextMeasurer.TextWidthTO(TControlAccess(ATextWindow).Font, Copy(AText, ATextLen, 1), TControlAccess(ATextWindow).Font.Style);
  end;

  if R.Right > AControlNCAreaSizes.Left + AControlClientRect.Width + AControlNCAreaSizes.Right then
    R.Right := AControlNCAreaSizes.Left + AControlClientRect.Width + AControlNCAreaSizes.Right;

  R.Top := 0;
  R.Bottom := AControlClientRect.Height;

  R := dxClientToScreen(ATextWindow, R);
  Result := [R];
end;
...
function TcxCustomTextEditUIATextControl.GetIsReadOnly: Boolean;
begin
  Result := FControl.ActiveProperties.ReadOnly;
end;

function TcxCustomTextEditUIATextControl.GetCaretPosition: Integer;
begin
  Result := FControl.GetUIACaretState.Caret;
end;

function TcxCustomTextEditUIATextControl.GetCaretPositionInfo: Integer;
var
  ACaretPosition: Integer;
begin
  ACaretPosition := GetCaretPosition;
  if ACaretPosition = 0 then
    Result := CaretPosition_BeginningOfLine
  else if ACaretPosition = Length(FControl.Text) then
    Result := CaretPosition_EndOfLine
  else
    Result := CaretPosition_Unknown;
end;

function TcxCustomTextEditUIATextControl.GetSelection: TdxUIATextRange;
begin
  Result := FControl.GetUIACaretState.Selection;
end;

function TcxCustomTextEditUIATextControl.GetLineFromCharIndex(
  ACharIndex: Integer): Integer;
begin
  Result := 0; 
end;

function TcxCustomTextEditUIATextControl.GetCharIndexFromLine(
  ALine: Integer): Integer;
begin
  Result := 0; 
end;

function TcxCustomTextEditUIATextControl.GetLineCount: Integer;
begin
  Result := 1; 
end;

function TcxCustomTextEditUIATextControl.GetMaxEnpointValue: Integer;
begin
  Result := Length(FControl.Text);
end;

function TcxCustomTextEditUIATextControl.GetText: string;
begin
  Result := ConvertToPasswordCharIfNeeded(FControl.Text);
end;

function TcxCustomTextEditUIATextControl.GetTextWithinRange(
  ARange: TdxUIATextRange): string;
begin
  Result := ConvertToPasswordCharIfNeeded(Copy(FControl.Text, ARange.First + 1, ARange.Last - ARange.First));
end;
...
procedure TcxCustomTextEditUIATextControl.Select(
  ARange: TdxUIATextRange);
begin
  FControl.SelStart := ARange.First;
  FControl.SelLength := ARange.Last - ARange.First;
end;

procedure TcxCustomTextEditUIATextControl.SetText(const AValue: string);
begin
  FControl.Text := AValue;
end;
