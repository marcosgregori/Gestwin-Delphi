...
uses
  ...
  Winapi.ActiveX, dxThreading, dxUIA;
...
type
  ...
  protected
    ...
    function Get_SelectionContainer(out pRetVal: dxUIA.IRawElementProviderSimple): HResult; stdcall;
  end;
...
procedure TcxRadioButton.SetChecked(Value: Boolean);
  ...
  procedure NotifyUIAClientsAboutNewState;
  var
    AOldValue, ANewValue: OleVariant;
  begin
    if Assigned(FUIAProvider) and FUIAProvider.IsLive and dxUIAHasActiveClients then
    begin
      InitVariantFromBoolean(Checked, ANewValue);
      dxUIARaisePropertyChangedEvent(FUIAProvider, dxUIA.UIA_SelectionItemIsSelectedPropertyId, AOldValue, ANewValue);
    end;
  end;
...
function TdxUIARadioButtonProvider.AddToSelection: HResult;
begin
  Result := Select;
end;
...
function TdxUIARadioButtonProvider.Get_IsSelected(
  out pRetVal: Integer): HResult;
begin
  if not IsLive(Result) then
    Exit;
...
function TdxUIARadioButtonProvider.Get_SelectionContainer(
  out pRetVal: dxUIA.IRawElementProviderSimple): HResult;
begin
  if not IsLive(Result) then
    Exit;
  pRetVal := GetParentProvider;
  dxUIAPrepareForReturnToUIACore(pRetVal);
  Result := S_OK;
end;

function TdxUIARadioButtonProvider.RemoveFromSelection: HResult;
begin
  if not IsLive(Result) then
    Exit;
...
function TdxUIARadioButtonProvider.Select: HResult;
begin
  if not IsLive(Result) then
    Exit;
...
function TdxUIACustomRadioGroupProvider.GetSelection(
  out pRetVal: PSafeArray): HResult;
var
  AProviders: TArray<IUnknown>;
  I: Integer;
  ACount: Integer;
...
begin
  if not IsLive(Result) then
    Exit;
  SetLength(AProviders, Control.ButtonCount);
  ACount := 0;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          AProviders[ACount] := AButtonProvider as IUnknown;
          Inc(ACount);
        end;
      end;
    end;
  end;
  SetLength(AProviders, ACount);
  if not dxCreateIUnknownSafeArray(AProviders, pRetVal) then
    Result := S_FALSE
  else
    Result := S_OK;
end;
...
function TdxUIACustomRadioGroupProvider.Get_CanSelectMultiple(
  out pRetVal: Integer): HResult;
begin
  pRetVal := 0;
  if not IsLive(Result) then
...
function TdxUIACustomRadioGroupProvider.Get_IsSelectionRequired(
  out pRetVal: Integer): HResult;
begin
  pRetVal := 0;
  if not IsLive(Result) then
