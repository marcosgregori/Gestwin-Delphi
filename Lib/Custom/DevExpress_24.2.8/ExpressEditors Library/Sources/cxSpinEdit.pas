...
uses
  System.Types, Winapi.Windows, System.Variants, System.Classes, Vcl.Controls, Vcl.Forms, Vcl.Graphics, System.SysUtils, cxClasses,
  cxContainer, cxControls, cxDataStorage, cxDataUtils, cxEdit, cxGraphics, cxMaskEdit, cxGeometry,
  cxTextEdit, cxVariants, cxLookAndFeelPainters, cxFilterControlUtils, dxUxTheme, cxAccessibility;
...
type
  ...
  { TdxSpinEditAccessibilityHelper }

  TdxSpinEditAccessibilityHelper = class(TdxEditAccessibilityHelper) // for internal use
  protected
...
procedure TcxSpinEditViewInfo.DrawNativeButtonBackground(
  ACanvas: TcxCanvas; AButtonViewInfo: TcxEditButtonViewInfo; const ARect: TRect);
begin
  dxDrawThemeBackground(ACanvas.Handle, totSpin, ScaleFactor, ARect,
    AButtonViewInfo.Data.NativePart, AButtonViewInfo.Data.NativeState);
end;
...
function TdxSpinEditAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_SPINBUTTON;
end;

function TdxSpinEditAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_NORMAL;
end;

function TdxSpinEditAccessibilityHelper.GetValue(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := VarToStr(TcxCustomSpinEdit(Edit).Value);
end;
...
function TcxCustomSpinEdit.GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass;
begin
  Result := TdxSpinEditAccessibilityHelper;
end;
