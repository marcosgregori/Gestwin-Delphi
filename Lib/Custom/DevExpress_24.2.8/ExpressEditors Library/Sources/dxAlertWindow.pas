...
uses
  System.UITypes,
  System.Classes, Winapi.Windows, Vcl.Forms, Vcl.Controls, System.SysUtils, cxClasses, cxControls, cxGraphics,
  Vcl.Graphics, cxGeometry, Winapi.Messages, cxLookAndFeelPainters, System.Math, dxFormattedText,
  cxDrawTextUtils, Vcl.ImgList, dxCoreClasses, cxLookAndFeels, Vcl.Menus, dxForms, dxCore,
  dxDrawRichTextUtils;  //..
...
type
  ...
  private
    ...
    function GetItem(AIndex: TdxListIndex): TdxAlertWindowMessage;
  protected
    procedure Changed; virtual;
  ...
  public
    ...
    property Items[Index: TdxListIndex]: TdxAlertWindowMessage read GetItem; default;
  end;
...
// #BA: Don't change properties order
  ...
  public
    ...
    property Controller: TdxAlertWindowController read GetController; // for internal use
    ...
    property Owner: IdxAlertWindow read FOwner; // for internal use
  ...
  private
    function GetItem(AIndex: TdxListIndex): TdxAlertWindowCustomButtonViewInfo;
  public
    ...
    property Items[Index: TdxListIndex]: TdxAlertWindowCustomButtonViewInfo read GetItem; default; // for internal use
  end;
  ...
  public
    ...
    property Buttons: TdxAlertWindowButtonsViewInfoList read FButtons; // for internal use
  ...
  TdxAlertWindowFormattedTextViewInfo = class(TdxAlertWindowCustomTextViewInfo)
  protected
    FFormattedText: TdxFormattedText;
    procedure DoDraw(ACanvas: TcxCanvas); override;
    function GetHint: string; override;
    function IsMouseOverHyperlink(APoint: TPoint; out AURL: string): Boolean;
  public
    constructor Create(AOwner: IdxAlertWindow); override;
    destructor Destroy; override;
    procedure Calculate(const ABounds: TRect); override;
    function CalculateFormattedText(const ARect: TRect; out ATextSize: TSize): Boolean;
  end;

  { TdxAlertWindowCaptionTextViewInfo }

  TdxAlertWindowCaptionTextViewInfo = class(TdxAlertWindowFormattedTextViewInfo)
  protected
    function CustomDraw(ACanvas: TcxCanvas): Boolean; override;
  ...
  TdxAlertWindowMessageTextViewInfo = class(TdxAlertWindowFormattedTextViewInfo)
  protected
    function CustomDraw(ACanvas: TcxCanvas): Boolean; override;
    function GetDrawTextFlags: Cardinal; override;
    function GetFont: TFont; override;
  ...
  public
    ...
    property InfoPanelText: TdxAlertWindowNavigationPanelTextViewInfo read FInfoPanelText; // for internal use
    property NextButton: TdxAlertWindowNextButtonViewInfo read FNextButton; // for internal use
    property PreviousButton: TdxAlertWindowPreviousButtonViewInfo read FPreviousButton; // for internal use
  ...
  public
    ...
    property ButtonsViewInfo: TdxAlertWindowButtonsViewInfo read FButtonsViewInfo; // for internal use
    property CaptionButtonsViewInfo: TdxAlertWindowCaptionButtonsViewInfo read FCaptionButtonsViewInfo; // for internal use
    property CaptionTextViewInfo: TdxAlertWindowCaptionTextViewInfo read FCaptionTextViewInfo; // for internal use
    property ImageViewInfo: TdxAlertWindowImageViewInfo read FImageViewInfo; // for internal use
    property MessageTextViewInfo: TdxAlertWindowMessageTextViewInfo read FMessageTextViewInfo; // for internal use
    property NavigationPanelViewInfo: TdxAlertWindowNavigationPanelViewInfo read FNavigationPanelViewInfo; // for internal use
  ...
  private
    ...
    FHotViewInfoURL: string;
    FIsMouseOverHyperlink: Boolean;
  ...
  protected
    ...
    property HotViewInfoURL: string read FHotViewInfoURL write FHotViewInfoURL;
    property IsMouseOverHyperlink: Boolean read FIsMouseOverHyperlink write FIsMouseOverHyperlink;
  ...
  TdxAlertWindowHyperlinkClickEvent = procedure(Sender: TObject; const AURI: string; var AHandled: Boolean) of object;
  ...
  private
    ...
    FOnHyperlinkClick: TdxAlertWindowHyperlinkClickEvent;
  ...
  protected
    ...
    procedure SetCurrentMessageIndex(AValue: Integer);
    procedure SetIsPopupMenuShown(AValue: Boolean);
    procedure SetIsUrlProcessing(const AValue: Boolean);
    procedure SetPinned(AValue: Boolean);
    ...
    property IsPopupMenuShown: Boolean read GetIsPopupMenuShown write SetIsPopupMenuShown;
    property IsUrlProcessing: Boolean read FIsUrlProcessing write SetIsUrlProcessing;
    property ViewInfo: TdxAlertWindowViewInfo read GetViewInfo;
    ...
    property OnHyperlinkClick: TdxAlertWindowHyperlinkClickEvent  read FOnHyperlinkClick write FOnHyperlinkClick;
  ...
  private
    function GetItem(AIndex: TdxListIndex): TdxAlertWindowPositionInfo;
  public
    ...
    property Items[Index: TdxListIndex]: TdxAlertWindowPositionInfo read GetItem; default;
  end;
  ...
  private
    function GetItem(AIndex: TdxListIndex): TdxAlertWindow;
  public
    function Add(AClass: TdxAlertWindowClass): TdxAlertWindow;

    property Items[Index: TdxListIndex]: TdxAlertWindow read GetItem; default;
  end;
  ...
  private
    ...
    FOnHyperlinkClick: TdxAlertWindowManagerHyperlinkClickEvent;
    ...
    procedure AlertWindowHyperlinkClickHandler(Sender: TObject; const AURI: string; var AHandled: Boolean);
  ...
  published
    ...
    property OnHyperlinkClick: TdxAlertWindowManagerHyperlinkClickEvent read FOnHyperlinkClick write FOnHyperlinkClick;
...
uses
  System.Types, cxEditConsts, Winapi.ShellAPI, dxDPIAwareUtils, dxCoreGraphics,
  dxCustomHint, dxFormattedTextConverterBBCode, dxFormattedTextConverterRTF;

var  //..
   FTextHelper : TdxRichTextHelper = nil;

function GetTextHelper : TdxRichTextHelper;
begin
     If   not Assigned( FTextHelper )
     then FTextHelper := TdxRichTextHelper.Create;
     Result := FTextHelper;
end;
...
begin
  ...
  begin
    ...
    cxMeasureCanvas.Font := AFont;
    ATextParams := cxCalcTextParams(cxMeasureCanvas.Canvas, AFormat);
    cxMakeTextRows(cxMeasureCanvas.Canvas, PChar(AText), Length(AText), ARect, ATextParams, ATextRows, Result);
    Result := TdxTextMeasurer.TextLineHeight(AFont) * Result;
  end
  else
    Result := 0;
end;
...
function TdxAlertWindowMessageList.GetItem(AIndex: TdxListIndex): TdxAlertWindowMessage;
begin
...
begin
  dxSetFontAsDpiUnrelated(TFont(Sender));
  Changed;
end;
...
begin
  ...
  Caption.Font.Height := dxGetFontHeightForDefaultDPI(dxAlertWindowDefaultCaptionFontSize);
...
begin
  ARect := Bounds;  
  If   dxIsRichText( Text )  //..
  then With GetTextHelper do
         begin
         Init( ACanvas.Canvas, Text, dxDefaultScaleFactor );
         DrawText( ACanvas.Canvas, ARect );
         end       
  else cxTextOut( ACanvas.Canvas, Text, ARect, DrawTextFlags );
end;
...
function TdxAlertWindowCustomTextViewInfo.GetTextColor: TColor;
begin
...
begin
  ...
  if IsImageAssigned then
    TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, cxRectCenter(Bounds, GlyphSize), Bounds, nil, OptionsButtons.Images,
...
function TdxAlertWindowButtonsViewInfoList.GetItem(AIndex: TdxListIndex): TdxAlertWindowCustomButtonViewInfo;
begin
...
begin
  if IsImageAssigned then
    TdxImageDrawer.DrawImage(ACanvas, Bounds, nil, OptionsMessage.Images, ImageIndex, ifmNormal, idmNormal, True, nil, Owner.GetScaleFactor);
end;
...
var
  ATextSize: TSize;
begin
  if CalculateFormattedText(Rect(0, 0, 1, 1), ATextSize) then
    Result := ATextSize.cy
  else
    ...
    If   (AWidth > 0) 
	   then begin
         ...
         If   dxIsRichText( Text )   //..
 	       then With GetTextHelper do  //..
                begin
                Init( cxScreenCanvas.Canvas, Text, dxDefaultScaleFactor );
                CalculateTextHeight( cxScreenCanvas.Canvas, ARect );
                Result := ARect.Bottom - ARect.Top;
                end                  //..
           else begin
                If   CalculateFormattedText(ARect, ATextSize)
			             then Result := ATextSize.cy
			             else begin
			                  cxMeasureCanvas.Font := Font;
				                 ATextParams := cxCalcTextParams(cxMeasureCanvas.Canvas, CXTO_WORDBREAK or CXTO_EDITCONTROL or CXTO_END_ELLIPSIS);
                     cxMakeTextRows(cxMeasureCanvas.Canvas, PChar(Text), Length(Text), ARect, ATextParams, ATextRows, Result);
                     Result := TdxTextMeasurer.TextLineHeight(Font) * Result;
                     end;
                end;
          end 
   else Result := 0;
...
begin
  ...
  begin
    if CalculateFormattedText(Rect(0, 0, 1, AHeight), ATextSize) then
      Result := ATextSize.cx
    else
      Result := cxRectWidth(cxGetTextRect(Font, Text, AHeight div TdxTextMeasurer.TextLineHeight(Font), True));
  end;
end;
...
var
  APreviousPoint: TPoint;
begin
  APreviousPoint := GetCurrentPoint;
  ...
  if not cxPointIsEqual(APreviousPoint, GetCurrentPoint) then
    DoAnimation;
end;
...
var
  APreviousAlphaValue: Byte;
begin
  APreviousAlphaValue := CurrentAlphaValue;
  FWorkingAlphaValue := FWorkingAlphaValue + FAlphaStep;
  if APreviousAlphaValue <> CurrentAlphaValue then
    DoAnimation;
end;
...
begin
  AViewInfo := ViewInfo.GetHitTest(P);
  if AViewInfo is TdxAlertWindowCustomButtonViewInfo then
    HotButton := TdxAlertWindowCustomButtonViewInfo(AViewInfo)
  else
    HotButton := nil;
  IsMouseOverHyperlink := Assigned(AViewInfo) and (AViewInfo is TdxAlertWindowFormattedTextViewInfo) and
      TdxAlertWindowFormattedTextViewInfo(AViewInfo).IsMouseOverHyperlink(P, FHotViewInfoURL);
end;
...
var
  AUrl: string;
begin
  if Controller.IsMouseOverHyperlink then
  begin
    AUrl := Controller.HotViewInfoURL;
    if not DoHyperlinkClick(Self, AUrl) and (AUrl <> '') then
      dxShellExecute(AUrl, SW_SHOWMAXIMIZED);
  end
  else
...
begin
  inherited Resize;
  if not TdxFormHelper.IsScaleChanging(Self) then
...
begin
  ...
  Cursor := IfThen(Controller.IsMouseOverHyperlink, crHandPoint, crDefault);
end;
...
function TdxAlertWindow.DoHyperlinkClick(ASender: TObject; const AURI: string): Boolean;
begin
  Result := False;
...
procedure TdxAlertWindow.SetIsUrlProcessing(const AValue: Boolean);
begin
  if FIsUrlProcessing <> AValue then
  begin
    FIsUrlProcessing := AValue;
    UpdateDisplayTimerState;
  end;
end;
...
procedure TdxAlertWindow.SetVisible(AValue: Boolean);   //..
begin
     try
       FChangingVisibility := True;
       If   AValue
       then ShowWindow(Handle, SW_SHOWNA)
...
begin
  if VisibilityTransition <> awvtHiding then
  begin
    if MouseInControl or Pinned or IsPopupMenuShown or IsUrlProcessing then
      StopDisplayTimer
    else
      StartDisplayTimer;
...
function TdxAlertWindowPositionInfoList.GetItem(AIndex: TdxListIndex): TdxAlertWindowPositionInfo;
begin
...
begin
  ...
  AAlertWindow.ScaleForPPI(dxGetMonitorDPI(AMonitor));
  AColumnIndex := 0;
  AActualPosition := GetActualPosition(AAlertWindow);
  AAlertWindow.BoundsRect := GetNewPosition(AAlertWindow.BoundsRect, GetLastInitialGrid(AActualPosition,
    AMonitor.MonitorNum), AActualPosition, GetMonitorWorkArea(AMonitor.Handle), AColumnIndex);
...
function TdxAlertWindowList.GetItem(AIndex: TdxListIndex): TdxAlertWindow;
begin
...
procedure TdxAlertWindowManager.AlertWindowHyperlinkClickHandler(Sender: TObject; const AURI: string;
  var AHandled: Boolean);
begin
  if Assigned(FOnHyperlinkClick) then
    FOnHyperlinkClick(Self, AURI, AHandled);
end;
...
begin
  ...
  Result.OnHyperlinkClick := AlertWindowHyperlinkClickHandler;
...
constructor TdxAlertWindowFormattedTextViewInfo.Create(AOwner: IdxAlertWindow);
begin
  inherited Create(AOwner);
  FFormattedText := TdxFormattedText.Create;
end;

destructor TdxAlertWindowFormattedTextViewInfo.Destroy;
begin
  FreeAndNil(FFormattedText);
  inherited;
end;

procedure TdxAlertWindowFormattedTextViewInfo.Calculate(const ABounds: TRect);
var
  AFont: TFont;
begin
  inherited;
  ...
  try
    AFont.Assign(Font);
    AFont.Color := GetTextColor;
    FFormattedText.Import(Text, AFont);
    if FFormattedText.HasFormatting then
      FFormattedText.CalculateLayout(TdxTextMeasurer.MeasureCanvas, AFont, Bounds, GetDrawTextFlags, ScaleFactor);
  finally
    AFont.Free;
  end;
end;

function TdxAlertWindowFormattedTextViewInfo.CalculateFormattedText(const ARect: TRect; out ATextSize: TSize): Boolean;
var
  AFormattedText: TdxFormattedText;
begin
  AFormattedText := TdxFormattedText.Create;
  try
    AFormattedText.Import(Text, Font);
    Result := AFormattedText.HasFormatting;
    if Result then
    begin
      TdxTextMeasurer.MeasureCanvas.Font.Assign(Font);
      AFormattedText.CalculateLayout(TdxTextMeasurer.MeasureCanvas, Font, ARect, GetDrawTextFlags and not CXTO_END_ELLIPSIS or CXTO_CALCRECT, ScaleFactor);
      ATextSize := AFormattedText.TextSize;
    end;
  finally
    AFormattedText.Free;
  end;
...
procedure TdxAlertWindowFormattedTextViewInfo.DoDraw(ACanvas: TcxCanvas);
begin
  if FFormattedText.HasFormatting then
    FFormattedText.Draw(ACanvas.Canvas, Bounds.TopLeft, True)
  else
    inherited DoDraw(ACanvas);
end;

function TdxAlertWindowFormattedTextViewInfo.GetHint: string;
begin
  if IsTextClipped then
  begin
    if cxGetHintStyleController = nil then
      Result := FFormattedText.GetDisplayText
    else
      Result := Text;
  end
  else
...
function TdxAlertWindowFormattedTextViewInfo.IsMouseOverHyperlink(APoint: TPoint; out AURL: string): Boolean;
var
  ALayoutBox: TdxFormattedTextLayoutBox;
begin
  ALayoutBox := FFormattedText.HitTest(Point(APoint.X - Bounds.Left, APoint.Y - Bounds.Top));
  Result := Assigned(ALayoutBox) and ALayoutBox.IsHyperlink;
  if Result then
    AURL := FFormattedText.GetUrl(ALayoutBox.HyperlinkIndex)
  else
    AURL := '';
end;
