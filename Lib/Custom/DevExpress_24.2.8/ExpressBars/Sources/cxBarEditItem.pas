...
type
  ...
  {$HINTS ON}
    ...
    procedure SynchronizeItemIndexAndEditValueWhenLoaded;
...
begin
  Result := (AProvider <> nil) and not IsLoading and AProvider.SupportsItemIndexSynchronization(AInitiator);
end;

procedure TcxCustomBarEditItem.SynchronizeItemIndexAndEditValueWhenLoaded;
...
begin
  ...
  if AProvider = nil then
    Exit;
  if NeedSynchronizeItemIndexAndEditValue(AProvider, TcxItemIndexSynchronizationInitiator.PropertiesHandler) then
  begin
    if FItemIndex = -1 then
      EditValue := Null
    else
      if FItemIndex >= AProvider.ItemCount then
        ItemIndex := -1
    else
      if AProvider.FindValueByItemIndex(FItemIndex, ANewValue) then
      begin
...
begin
  if CaptionIsEditValue then
    ...
    dxBarCustomizingForm.PainterClass.DrawEdit(ACanvas, ARect, Self, odSelected in AState, UseBarPaintingStyle(not dxBarCustomizingForm.LookAndFeelPainter.SupportsEditorBorders));
end;
...
begin
  ...
  try
    ...
    ADrawAsStandalone := not dxBarCustomizingForm.LookAndFeelPainter.SupportsEditorBorders; 
...
begin
  inherited Loaded;
  SynchronizeItemIndexAndEditValueWhenLoaded;
  FItemIndexAssigned := False;
  FEditValueAssigned := False;
end;

procedure TcxCustomBarEditItem.ReadState(AReader: TReader);
begin
  FItemIndexAssigned := False;
  FEditValueAssigned := False;
