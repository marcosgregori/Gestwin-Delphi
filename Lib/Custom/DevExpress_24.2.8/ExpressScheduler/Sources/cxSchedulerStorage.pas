...
type
  ...
  public
    ...
    Timezone: Integer; 
    TimezoneDaylightSaving: Boolean; 
  ...
  ['{62D132ED-D331-4FB3-AB9E-6B16676148FF}']
    ...
    function GetTimeZoneDaylightSaving: Boolean; 
  ...
  private
    ...
    procedure SetValue(AEvent: TcxSchedulerEvent; const AValue: Variant); inline;
  ...
  private
    ...
    procedure AddRecurrenceLink(AEvent: TcxSchedulerEvent); inline;

    function GetActualFinish: Integer;
    function GetActualStart: Integer;
    function GetActualLabelColor: Integer; inline;
    ...
    function GetFinish: TDateTime; inline;
    ...
    function GetStart: TDateTime; inline;
    ...
    function GetValueDef(AFieldIndex: Integer): TcxValueDef; inline;

    procedure DoSetOptionsFlag(const AMask: Integer; AValue: Boolean); inline;
  ...
  protected
    ...
    function CheckRecurrenceLink(AEvent: TcxSchedulerEvent): Boolean;
    ...
    function GetResourcesDisplayNames: string;
  ...
  protected
    ...
    function CanResize: Boolean; overload;
    function CanResize(AResource: TcxSchedulerStorageResourceItem): Boolean; overload;
  ...
  TcxSchedulerResourceEvent = procedure(Sender: TObject;
    AResource: TcxSchedulerStorageResourceItem) of object;
  ...
  private
    ...
    FOnResourceVisibleChanged: TcxSchedulerResourceEvent;
  ...
  protected
    ...
    procedure DoResourceVisibleChanged(AItem: TcxSchedulerStorageResourceItem);
    function GetOwner: TPersistent; override;
    function GetResourceItemByID(const AResource: Variant): TcxSchedulerStorageResourceItem;
  ...
  published
    ...
    property OnResourceVisibleChanged: TcxSchedulerResourceEvent read FOnResourceVisibleChanged write FOnResourceVisibleChanged;
  end;
  ...
  private
    ...
    function GetField(AIndex: Integer): TcxCustomSchedulerStorageField; inline;
    function GetFieldCount: Integer; inline;
    function GetInternalFieldCount: Integer; inline;
    ...
    function GetSortedList: TdxSortedList<TcxSchedulerEvent>;
  ...
  protected
    ...
    FSortedList: TdxSortedList<TcxSchedulerEvent>;
    LockCount: Integer;
    // internal fields
    ...
    property SortedList: TdxSortedList<TcxSchedulerEvent> read GetSortedList;
  ...
  protected
    ...
    procedure Changed; virtual;
    function CheckEvent(AEvent: TcxSchedulerEvent; const AResourceID: Variant; AddAll: Boolean): Boolean; virtual;
  ...
  protected
    ...
    function CheckEvent(AEvent: TcxSchedulerEvent; const AResourceID: Variant; AddAll: Boolean): Boolean; override;
 ...
 public
  ...
  public
    constructor Create(AOwner: TcxSchedulerReminders; AEvent: TcxSchedulerControlEvent); virtual;
    destructor Destroy; override;
    procedure Dismiss;
  ...
  private
    ...
    FUpdateEventsTimer: TcxTimer;
    FUpdateRemindersTimer: TcxTimer;
  ...
  protected
    ...
    procedure UpdateReminderList; virtual;
    procedure UpdateTimer(ATimer: TcxTimer; AInterval: TDateTime);
    procedure DoValidateReminderList;
    ...
    property UpdateEventsTimer: TcxTimer read FUpdateEventsTimer;
    property UpdateRemindersTimer: TcxTimer read FUpdateRemindersTimer;
...
function CompareEvents(const AEvent1, AEvent2: TcxSchedulerEvent): Integer;
begin
...
const
  ...
  DueInFormatTexts: array[Boolean] of string = ('%d %s%s', '%d %s%s atrasado');
begin
  with AInfo do
  begin
...
constructor TcxSchedulerEvent.Create(AStorage: TcxCustomSchedulerStorage; ADisableEditing: Boolean = False);
begin
  Create(AStorage, nil);
...
constructor TcxSchedulerEvent.Create(AStorage: TcxCustomSchedulerStorage; ARecordHandle: Pointer);
begin
  Create;
  FStorage := AStorage;
...
function TcxSchedulerEvent.CheckRecurrenceLink(AEvent: TcxSchedulerEvent): Boolean;
begin
  Result := (AEvent.EventType in [etCustom, etException]) and VarEquals(ID, AEvent.ParentID);
  if Result then
    AddRecurrenceLink(AEvent);
end;
...
function TcxSchedulerEvent.GetResourcesDisplayNames: string;
var
  I: Integer;
...
begin
  ...
  begin
    ...
    if VarIsNull(AResource.ResourceID) or VarIsEmpty(AResource.ResourceID) or
      not(AResource.Visible and IsSharedWithResource(AResource)) then
        Continue;
    if Result <> '' then
      Result := Result + dxFormatSettings.ListSeparator + ' ';
    Result := Result + AResource.DisplayName;
  end;
end;
...
begin
  ...
  begin
    for I := 0 to Storage.ResourceCount - 1 do
      if IsSharedWithResource(Storage.Resources.ResourceItems[I]) then
      begin
        Result := Storage.Resources.ResourceItems[I].ActualEventColor;
        Break;
      end;
  end;
end;
...
procedure TcxSchedulerEvent.DoSetOptionsFlag(const AMask: Integer; AValue: Boolean);
begin
  if AValue then
...
procedure TcxSchedulerEvent.AddRecurrenceLink(AEvent: TcxSchedulerEvent);
var
  ALink: TcxSchedulerEvent;
begin
...
begin
  DoSetOptionsFlag(omEnabled, AValue);
end;
...
begin
  DoSetOptionsFlag(omGroup, AValue);
end;
...
begin
  DoSetOptionsFlag(omReminder, AValue);
...
constructor TcxSchedulerControlEvent.Create(AStorage: TcxCustomSchedulerStorage; ADisableEditing: Boolean = False);
begin
  inherited Create;
  FStorage := AStorage;
  CreateSubClasses;
...
constructor TcxSchedulerControlEvent.Create(ASource: TcxSchedulerEvent; const AStart, AFinish: TDateTime);
begin
  Create(ASource);
  Start := AStart;
  Finish := AFinish;
...
constructor TcxSchedulerControlEvent.Create(ASource, AOriginal: TcxSchedulerEvent);
begin
  Create(ASource);
...
begin
  AResource := GetResourceItem;
  Result := CanResize(AResource);
end;

function TcxSchedulerControlEvent.CanResize(AResource: TcxSchedulerStorageResourceItem): Boolean;
begin
...
begin
  ...
  begin
    ...
    FWorkDays := TcxSchedulerStorageResourceItem(Source).WorkDays;
    FWorkFinish := TcxSchedulerStorageResourceItem(Source).WorkFinish;
    FWorkStart := TcxSchedulerStorageResourceItem(Source).WorkStart;
  end
  else
    inherited Assign(Source);
end;
...
begin
  if FVisible = AValue then
    Exit;
  FVisible := AValue;
  Resources.DoResourceVisibleChanged(Self);
  Changed(True);
end;
...
constructor TcxSchedulerStorageResources.Create(AOwner: TcxCustomSchedulerStorage);
begin
  FOwner := AOwner;
...
begin
  FreeAndNil(FItems);
  FreeAndNil(FItemsVisibility);
  inherited Destroy;
end;

procedure TcxSchedulerStorageResources.Assign(Source: TPersistent);
begin
...
function TcxSchedulerStorageResources.GetResourceItemByID(const AResource: Variant): TcxSchedulerStorageResourceItem;
var
  I: Integer;
...
function TcxSchedulerStorageResources.GetResourceName(AResource: TcxSchedulerStorageResourceItem): string;
begin
...
begin
  Result := '';
  AResourceItem := GetResourceItemByID(AResource);
      ...
      if SameText(ASubValue, 'WorkStart') then
        ...
        if SameText(ASubValue, 'WorkFinish') then
          ...
          if SameText(ASubValue, 'WorkDays') then
            AValue := cxDaysToInteger(AItem.WorkDays)
          else
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if P <> nil then
              P^.WorkDays := cxIntegerToDays(AValue)
            else
              AItem.WorkDays := cxIntegerToDays(AValue);
          end
          else
          begin
            if P <> nil then
...
procedure TcxSchedulerStorageResources.DoResourceVisibleChanged(AItem: TcxSchedulerStorageResourceItem);
begin
  if Assigned(FOnResourceVisibleChanged) then
    FOnResourceVisibleChanged(Storage, AItem);
end;
...
constructor TcxSchedulerEventConflictsInfo.Create(AEvent: TcxSchedulerEvent; AExceptEventsWithoutResources: Boolean);
begin
  inherited Create;
  FEvent := AEvent;
...
begin
  FreeAndNil(ExcludedEventID);
  FreeAndNil(FConflictEvents);
  FreeAndNil(FTimeRanges);
  inherited Destroy;
end;
...
var
  I: Integer;
  ASortedEvents: TdxSortedList<TcxSchedulerEvent>;
begin
  ...
  ASortedEvents := SortedList;
  for I := 0 to ASortedEvents.Count - 1 do
...
var
  AComparer: IComparer<TcxSchedulerEvent>;
begin
  ...
  AComparer := TDelegatedComparer<TcxSchedulerEvent>.Create(CompareEvents);
  FSortedList := TdxSortedList<TcxSchedulerEvent>.Create(AComparer);
    ...
    begin
      ...
      begin
        if AIsRemoved then
          ...
          if not IsUpdatingMode and not FInternalUpdate then
...
procedure TcxCustomSchedulerStorage.UpdateStructure;
  ...
  procedure CalculateRecurrenceLinks;
  var
    AList, APatterns: TdxFastList;
    ALinks: TObjectDictionary<string, TdxFastList>;
    I, J: Integer;
    AEvent: TcxSchedulerEvent;
    AEventType: TcxEventType;
    AKey: string;
  begin
    ALinks := TObjectDictionary<string, TdxFastList>.Create([doOwnsValues]);
    APatterns := TdxFastList.Create;
    try
      for I := 0 to EventCount - 1 do
      begin
        ...
        AEvent.FIndex := I;
        if AEventType in [etCustom, etException] then
        begin
          AKey := VarToStr(AEvent.ParentID);
          if not ALinks.TryGetValue(AKey, AList) then
          begin
            AList := TdxFastList.Create;
            ALinks.Add(AKey, AList);
          end;
          AList.Add(AEvent);
        end;
        if AEventType = etPattern then
        begin
          AEvent.FLink := nil;
          APatterns.Add(AEvent);
        end;
      end;
      for I := 0 to APatterns.Count - 1 do
      begin
        AEvent := TcxSchedulerEvent(APatterns[I]);
        if ALinks.TryGetValue(VarToStr(AEvent.ID), AList) then
        begin
          for J := 0 to AList.Count - 1 do
            AEvent.AddRecurrenceLink(AList[J]);
        end;
      end;
    finally
      ALinks.Free;
...
begin
  if IsLocked or Reminders.UpdatingReminderList then
    Exit;
  Reminders.StopTimers;
  CalculateRecurrenceLinks;
  BeginStructureUpdating;
  try
    FSortedList.Clear;
    for I := 0 to FEventsList.Count - 1 do
      FEventsList[I].ResetReferences;
    for I := 0 to FEventsList.Count - 1 do
      FEventsList[I].RefreshTaskLinks;
  finally
...
begin
  Result := not (FIsLoading or (ComponentState * [csDesigning, csDestroying, csLoading] <> []) or
    IsLocked or IsStructureUpdating);
end;
...
function TcxCustomSchedulerStorage.GetSortedList: TdxSortedList<TcxSchedulerEvent>;
begin
  Result := FSortedList;
      ...
      if FSortedList.Count <> FEventsList.Count then
        TdxMultithreadedListHelper.IterateItems(FEventsList.List,
          ...
          begin
            ...
            System.TMonitor.Enter(FSortedList);
            try
              FSortedList.Add(TcxSchedulerEvent(AEvent));
            finally
              System.TMonitor.Exit(FSortedList);
            end;
          end);
    finally
...
constructor TcxSchedulerEventSelection.Create(ASourceEvents: TcxSchedulerEventList);
begin
  inherited Create;
  FKeys := TList.Create;
...
procedure TcxSchedulerEventSelection.Add(AEvent: TcxSchedulerControlEvent; AShift: TShiftState);
var
  AIndex: Integer;
  ASelected: Boolean;
begin
...
begin
  ...
  begin
    if not ACalculator.Event.Reminder then Exit;
...
function TcxSchedulerFilteredEventList.CheckEvent(
  AEvent: TcxSchedulerEvent; const AResourceID: Variant; AddAll: Boolean): Boolean;
begin
  Result := False;
  if AEvent.EventType in [etNone, etCustom] then
    Result := CheckSimpleEvent(AEvent, AResourceID, AddAll)
  else
    if (AEvent.EventType = etPattern) or AEvent.SkipExceptions then
      Result := CheckRecurrenceEvent(AEvent, AResourceID, AddAll);
end;
...
function TcxSchedulerFilteredEventList.CheckRecurrenceEvent(
  AEvent: TcxSchedulerEvent; const AResourceID: Variant; AddAll: Boolean): Boolean;
var
  AFinish: TDateTime;
  AOccurrence: TcxSchedulerEvent;
    ...
    try
      ...
      begin
        if not AEvent.GetOccurrenceByIndex(ACalculator.Index, AOccurrence) then
          if AddOccurrence(ACalculator) <> nil then
            Result := True;
      end;
    finally
      ACalculator.Free;
...
function TcxSchedulerFilteredEventList.CheckSimpleEvent(
  AEvent: TcxSchedulerEvent; const AResourceID: Variant; AddAll: Boolean): Boolean;
begin
  Result := False;
...
begin
  ...
  Result.FTimeBias := Result.TimeBias;
  Result.FLink := Result.FLink;
  ValidateTimeBias(Result);
end;
...
function TcxSchedulerCachedEventList.CheckEvent(
  AEvent: TcxSchedulerEvent; const AResourceID: Variant; AddAll: Boolean): Boolean;
begin
  Result := (AEvent.FInfo.Visible and (AddAll or CheckEventVisibility(AEvent, ShowEventsWithoutResource))) and
...
var
  AProc: TdxThreadingListProc;
begin
  FAbsoluteItems.Count := 0;
  FAbsoluteItems.Capacity := GetAbsoluteCountInternal;
  AProc :=
    procedure(AItem: Pointer; AIndex: Integer)
    var
      AEvent: TcxSchedulerEvent;
    begin
      AEvent := TcxSchedulerEvent(AItem);
      if not UseTimeRange or ((AEvent.Start <= SelFinish) and (SelStart <= AEvent.Finish)) then
      begin
        System.TMonitor.Enter(FAbsoluteItems);
        try
          FAbsoluteItems.Add(AEvent);
        finally
          System.TMonitor.Exit(FAbsoluteItems);
        end;
      end;
    end;
  TdxMultithreadedListHelper.IterateItems(FItems, AProc);
  TdxMultithreadedListHelper.IterateItems(FClones.FItems, AProc);
end;
...
constructor TcxSchedulerCustomReminderForm.Create(AReminders: TcxSchedulerReminders);
begin
...
constructor TcxSchedulerReminder.Create(AOwner: TcxSchedulerReminders; AEvent: TcxSchedulerControlEvent);
begin
  inherited Create;
  FOwner := AOwner;
  FEvent := AEvent;
...
begin
  if not Owner.FItems.Clearing then
    Owner.FItems.Remove(Self);
  inherited Destroy;
end;
...
begin
  ...
  FreeAndNil(FUpdateEventsTimer);
  FreeAndNil(FUpdateRemindersTimer);
  FreeAndNil(FReminderWindow);
  FreeAndNil(FReminderWindowLookAndFeel);
  FreeAndNil(FEventRemindersList);
  FreeAndNil(FItems);
  FreeAndNil(FEvents);
  inherited Destroy;
end;
...
begin
  ...
  if not DoAlertReminder(AReminder) and AEvent.Reminder then
  begin
...
begin
  ...
  if Storage.CanUpdateReminders then
    DoValidateReminderList;
  UpdateReminderList;
...
begin
  if IsLocked or UpdatingReminderList then
    Exit;
  ...
  try
    ...
    begin
      ...
      begin
        if FNeedShowWindow and not ARestoreSelection then
        begin
          ReminderWindow.SaveSelection;
      ...
      if FNeedShowWindow and HasReminders and not IsLocked then
        ShowReminderWindow;
    FNeedShowWindow := False;
  finally
...
procedure TcxSchedulerReminders.UpdateTimer(ATimer: TcxTimer; AInterval: TDateTime);
begin
  if Active then
  begin
...
procedure TcxSchedulerReminders.DoValidateReminderList;
var
  AIndex: Integer;
  AEvent: TcxSchedulerControlEvent;
begin
...
begin
  FUpdateEventsTimer := cxCreateTimer(OnUpdateEventsTimer, 1000, False);
  FUpdateRemindersTimer := cxCreateTimer(OnUpdateRemindersTimer, 1000, False);
end;
...
procedure cxFieldValueToTaskLinks(const AValue: Variant; ALinks: TcxSchedulerEventLinks);
  ...
  procedure InternalAdd(ADataValue: Variant);
  begin
    System.TMonitor.Enter(ALinks);
    try
      TcxSchedulerEventItemLink(ALinks.Add).SetData(ADataValue);
    finally
      System.TMonitor.Exit(ALinks);
    end;
  end;
