...
uses
  System.UITypes,
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, System.StrUtils,
  Vcl.StdCtrls, Vcl.ExtCtrls, cxTextEdit, cxMemo, cxImageComboBox, cxCalendar,
  ...
  cxCheckComboBox, cxGroupBox, Vcl.Menus, cxGraphics, cxLabel, dxBevel, Vcl.ComCtrls,
  dxCore, cxDateUtils, cxSchedulerEditorFormManager, cxClasses, dxLayoutContainer,
  dxLayoutControl, dxLayoutLookAndFeels, dxLayoutControlAdapters,
  dxLayoutcxEditAdapters, dxForms, cxGeometry, dxFramedControl, dxPanel;
...
type
  ...
  private
    ...
    FOriginalEventType: TcxEventType;
  ...
  public
    ...
    property OriginalEventType: TcxEventType read FOriginalEventType write FOriginalEventType;
    ...
    property TimeBias: Double read FTimeBias write FTimeBias;
  ...
  private
    ...
    // IcxShedulerCustomEditorForm                                      //#ignoreTypo
  ...
  private
    ...
    procedure SetTimeBias(const Value: Double);

    procedure WMActivate(var Message: TWMActivate); message WM_ACTIVATE;
  protected
    function GetHelperClass: TcxSchedulerEditorHelperClass; override;
    // IcxShedulerEventEditorForm     //#ignoreTypo
    procedure CreateParams(var Params: TCreateParams); override;
    function GetForm: TForm;
    function GetTimeZone: Integer; virtual; abstract;
    function GetTimeZoneDaylightSaving: Boolean; virtual; abstract;
    procedure Initialize(AEventInfo: TcxSchedulerEventEditInfo;
  ...
  public
    ...
    property TimeBias: Double read GetTimeBias write SetTimeBias;
  ...
  TcxSchedulerEventEditorForm = class(TcxSchedulerCustomEventEditor, IcxSchedulerEventEditorForm)
    ...
    procedure cbReminderMinutesBeforeStartPropertiesValidate(
      Sender: TcxCustomEdit; var DisplayValue: Variant; var ErrorText: TCaption;
      var Error: Boolean); //..
  ...
  protected
    ...
    function GetTimeZone: Integer; override;
    function GetTimeZoneDaylightSaving: Boolean; override;
    procedure Initialize(AEventInfo: TcxSchedulerEventEditInfo;
...
uses
  System.Variants, System.DateUtils, System.Math, cxSchedulerStrs, cxSchedulerUtils,
  cxSchedulerDialogs, cxVariants, cxSchedulerRecurrenceSelectionDialog, dxMessageDialog;

const
  dxThisUnitName = 'cxSchedulerEventEditor';
...
begin
  ...
  begin
    ...
    begin
      ...
      dxMessageBox(Form.Handle, AMessage, '', MB_ICONEXCLAMATION or MB_OK);
      Result := False;
    end;
  end;
end;
...
begin
  ...
  FOriginalEventType := AEvent.EventType;
  RefreshRecurrenceInfo;
  if AEvent.Pattern <> nil then
  begin
    FPatternStart := AEvent.Pattern.Start - TimeBias + Storage.TimeBias;
    FPatternFinish := AEvent.Pattern.Finish - TimeBias + Storage.TimeBias;
...
begin
  ...
  begin
    ...
    begin
      AMsgRes := dxMessageDlg(cxGetResourceString(@scxExitConfirmation), mtWarning, [mbYes, mbNo, mbCancel], 0);
...
begin
  if (Event.Pattern <> nil) and not DeleteExceptions then
    ARecurrenceInfo := Event.Pattern.RecurrenceInfo
  else
    ARecurrenceInfo := Event.RecurrenceInfo;
  ARecurrenceInfo.DisplayTimeBias := IfThen(OriginalEventType = etPattern, TimeBias - Storage.TimeBias, 0);
  Result := cxGetRecurrenceDescriptionStringProc(ARecurrenceInfo, True);
end;
...
begin
  ...
  if Result and Modified then
    Result := dxMessageDlg(cxGetResourceString(@scxDeleteConfirmation), mtWarning, [mbYes, mbNo], 0) = mrYes
end;

procedure TcxSchedulerEventEditorFormHelper.PopulateLabelColor(AProperties: TcxImageComboBoxProperties);
var
...
function TcxSchedulerCustomEventEditor.GetTimeBias: Double;
begin
  Result := Helper.TimeBias;
end;
...
procedure TcxSchedulerCustomEventEditor.SetTimeBias(const Value: Double);
begin
  Helper.TimeBias := Value;
end;
...
begin
  ...
  deStart.Visible := not cbAllDayEvent.Checked;   //..
  deEnd.Visible := not cbAllDayEvent.Checked;
  CheckControlStates;
end;
...
function TcxSchedulerEventEditorForm.GetTimeZone: Integer;
begin
  if Helper.OriginalEventType = etPattern then
    Result := Helper.EditingInfo.EditInfo.Timezone
  else
    Result := -1; 
end;

function TcxSchedulerEventEditorForm.GetTimeZoneDaylightSaving: Boolean;
begin
  Result := Helper.EditingInfo.EditInfo.TimezoneDaylightSaving;
end;
...
begin
  ...
  if not Result then
  begin
    dxMessageDlg(cxGetResourceString(@scxWrongTimeBounds), mtWarning, [mbOk], 0);
  ...
  begin
    ...
    if not Result then
    begin
      dxMessageDlg(Format(cxGetResourceString(@scxTaskWrongTimeBounds),
...
begin
  if not FindAvailableTime then
    dxMessageBox(Handle, cxGetResourceString(@scxNoAvailableFreeTime), Caption, MB_ICONINFORMATION or MB_OK);
  CheckControlStates;
