...
//    function GetRecordCount: Integer; virtual;
  ...
  protected
    ...
    function GetFilterOptions: TcxFilterCriteriaOptions; virtual;
  ...
  TcxBeforeSummaryEvent = procedure(ASender: TcxDataSummary) of object;  //..
  ...
  private
    ...
    FOnBeforeSummary: TcxBeforeSummaryEvent;  //..
  ...
  protected
    ...
    procedure DoBeforeSummary; virtual; //..
  ...
  published
    ...
    property OnBeforeSummary: TcxBeforeSummaryEvent read FOnBeforeSummary write FOnBeforeSummary;  //..
  ...
  private
    ...
    function GetSearchingFromAnyPos: Boolean;
  ...
  public
    ...
    property SearchingFromAnyPos: Boolean read GetSearchingFromAnyPos;
...
//FIsResetHasChildrenFlag: Boolean;
    ...
    FIncrementalSearchingFromAnyPos: Boolean;
  ...
  protected
    ...
    function LockOnBeforeSummary: Boolean; virtual;  //..
...
uses
  System.RTLConsts, System.Math, cxDateUtils, Winapi.Windows, cxFormats, cxDataConsts, cxFilterConsts, dxThreading,
  dxStringHelper;
...
begin
  ...
  begin
    ...
    DataController.FIncrementalSearchingFromAnyPos := AIsAnywhere;
...
begin
  ...
  begin
    ...
    if TdxDiacriticStringOptions.DataCompareText(S, ASubText, True, AIsAnywhere, False) then
    begin
      Result := I;
      Break;
    end;
...
function TcxDataControllerSearch.GetSearchingFromAnyPos: Boolean;
begin
  Result := DataController.FIncrementalSearchingFromAnyPos;
end;

function TcxDataControllerSearch.GetSearchText: string;
begin
  Result := TdxDiacriticStringOptions.Normalize(DataController.FIncrementalSearchText);
end;
...
begin
  if Source is TcxDataSummary then
    begin
    OnBeforeSummary := TcxDataSummary(Source).OnBeforeSummary;  //..
...
begin
  if FInBeforeSummary or  //..
     FInAfterSummary then Exit;
  if LockCount = 0 then
    Update(ARedrawOnly);
end;
...
procedure TcxDataSummary.DoBeforeSummary;
begin
  if Assigned(FOnBeforeSummary) then
  begin
    if DataController.IsLoading then
      DataController.FBeforeSummaryFlag := True
    else
    begin
      DataController.FBeforeSummaryFlag := False;
      FInBeforeSummary := True;
      try
        if not DataController.IsPattern and not DataController.LockOnBeforeSummary then
          FOnBeforeSummary(Self);
      finally
        FInBeforeSummary := False;
      end;
    end;
  end;
end;
...
begin
  ...
  if FBeforeSummaryFlag then   //..
    Summary.DoBeforeSummary;
...
var
  ...
  begin
    ...
    begin
      ...
      if DataCompareText(S, AText, APartialCompare, { AIsAnyWhere } True, ACaseSensitive ) then    //..
      begin
        Result := ARecordIndex;
        Break;
      end;
...
//..
function TcxCustomDataController.LockOnBeforeSummary: Boolean;
begin
...
begin
  ...
  FIsStringConversionNeeded := FIsAnsiSort or ACaseInsensitiveSetting or TdxDiacriticStringOptions.ActualDiacriticInsensitive;
  if ACaseInsensitiveSetting then
    FStringCompareFunc := TdxDiacriticStringOptions.AnsiCompareText
  else if FIsAnsiSort then
    FStringCompareFunc := TdxDiacriticStringOptions.AnsiCompareStr
  else
    FStringCompareFunc := TdxDiacriticStringOptions.CompareStr;
end;
...
function TcxCustomDataFindCriteria.GetFilterOptions: TcxFilterCriteriaOptions;
begin
  Result := [fcoCaseInsensitive, fcoIgnoreNull];
end;
...
begin
  ...
  begin
    ...
    Result := TdxDiacriticStringOptions.AnsiPos(ASearchText, ADisplayText) - 1;
    if Result <> -1 then
    begin
...
begin
  ...
  if not DataController.LockOnBeforeSummary and  //..
     not DataController.LockOnAfterSummary then
