...
uses
  System.UITypes,
  System.Types, System.Classes, Vcl.Controls, Winapi.Messages, Winapi.Windows, Vcl.Graphics, Vcl.Forms, Vcl.StdCtrls, System.SysUtils, System.IniFiles,
  dxCore, cxGraphics, cxGeometry, cxControls, cxLookAndFeels, cxClasses, dxTouch, System.Generics.Defaults, System.Generics.Collections;
...
type
  ...
  strict private
    ...
    function GetHeaderPartVisible: Boolean;
  ...
  public
    ...
    property HeaderPartVisible: Boolean read GetHeaderPartVisible;
  ...
  strict private
    FStoredStructure: TArray<Integer>;
  public
    function Add(StartIndex, FinishIndex: Integer; const Bounds: TRect): TdxPreviewPagesRow;
    procedure OffsetBounds(X, Y: Integer);
    function HasStructureChanged: Boolean; // for internal use
    procedure StoreStructure; // for internal use
  end;
  ...
  public
    ...
    procedure GetVisibleSelectedPageIndex(out AIndex: Integer; ATargetPageIndex: Integer = -1);
    procedure Invalidate;
  ...
  protected
    ...
    function IsRightToLeftLayout: Boolean; inline;
  ...
  { TdxCustomPreviewLayoutCalculator }

  TdxCustomPreviewLayoutCalculator = class  // for internal use
  strict private
    FPreview: TdxCustomPreview;

    function GetPagesArea: TRect;
    function GetPageCount: Integer;
    function GetPages: TdxPreviewPageList;
    function GetPagesRows: TdxPreviewPagesRowList;
    function GetScaleFactor: TdxScaleFactor;
    function GetZoomFactor: Integer;
    function GetZoomMode: TdxPreviewZoomMode;
    procedure SetZoomFactor(const AValue: Integer);
  strict protected
    function CanAddRow(AColumnIndex: Integer; const ABounds: TRect; const APageSize: TPoint): Boolean; virtual;
    ...
    function GetScrollTopPos: Integer; virtual;
    procedure CalculateVisiblePageRange(out AFirstPageIndex, ALastPageIndex: Integer); virtual;
    ...
    procedure AlignPages(const ABounds: TRect);
    procedure AlignPagesToHorizontalCenter(const ABounds: TRect); virtual;
    procedure AlignPagesToTopLeftPos; virtual;
    procedure AlignPagesToVerticalCenter(const ABounds: TRect);
    procedure CalculatePagesRows(const ABounds: TRect);
    procedure OffsetContent(const AOffset: TPoint);

    function HandleAllocated: Boolean;
    procedure CalculateZoomFactor;
    procedure SetZoomFactorF(AValue: Single);

    property PagesArea: TRect read GetPagesArea;
    property PageCount: Integer read GetPageCount;
    property Pages: TdxPreviewPageList read GetPages;
    property PagesRows: TdxPreviewPagesRowList read GetPagesRows;
    property Preview: TdxCustomPreview read FPreview;
    property ScaleFactor: TdxScaleFactor read GetScaleFactor;
    property ZoomMode: TdxPreviewZoomMode read GetZoomMode;
  ...
  protected
    ...
    property ClientSize: TSize read GetClientSize;
    property ContentHeight: Integer read GetContentHeight;
    property ContentWidth: Integer read GetContentWidth;
    property ScrollStep: Integer read GetScrollStep;
  public
    constructor Create(APreview: TdxCustomPreview);
  ...
  private
    ...
    FLayoutCalculator: TdxCustomPreviewLayoutCalculator;
    ...
    procedure ChangeSelPageIndex(AStartPageIndex: Integer; ADirection: Integer);
  ...
  protected
    ...
    function GetClientSize: TSize; override;
    ...
    procedure ResyncSelPageIndex; virtual;
    procedure ResyncMargins; virtual;

    function CreateLayoutCalculator: TdxCustomPreviewLayoutCalculator; virtual;
    function CreatePage: TdxPreviewPage; virtual;
    function GetPageClass: TdxPreviewPageClass; virtual;
    function GetPageSizeOptionsClass: TdxPreviewPageSizeOptionsClass; virtual;
    procedure RecreateLayoutCalculator;
    ...
    procedure SetZoomFactor(Value: Integer); virtual;

    procedure DesignerModified;
    procedure CheckMargins; virtual;
    // Page navigation
    procedure DoSelectNextPage(AStartPageIndex: Integer);
    procedure DoSelectPrevPage(AStartPageIndex: Integer);
    ...
    procedure DoSelectPage(APageIndex: Integer); virtual;
    ...
    procedure DrawPages(ACanvas: TcxCanvas); virtual;
    ...
    function GetHitInfoAt(const Pt: TPoint): TdxPreviewHitTests; overload; virtual;
    ...
    procedure HideAllHints; virtual;
    procedure MakePageVisible(APageIndex: Integer; AAnimated: Boolean); virtual;
    ...
    property NeedUpdateZoomState: Boolean read FNeedUpdateZoomState write FNeedUpdateZoomState;
    ...
    property LayoutCalculator: TdxCustomPreviewLayoutCalculator read FLayoutCalculator;
    ...
    // property SelPageIndex: Integer read FSelPageIndex write SetSelPageIndex;  //.. Trasladado
    ...
    property ZoomFactor: Integer read GetZoomFactor write SetZoomFactor stored True default 100;
...
uses
  Vcl.Themes, System.Math, System.Win.Registry, dxCustomPreviewStrs, dxDPIAwareUtils, dxPrintUtils, dxMeasurementUnits, dxTypeHelpers,
  cxLibraryConsts;

const
  dxThisUnitName = 'dxCustomPreview';
...
begin
  ...
  ALineColor := TdxColorUtils.ColorToHSL(clWindow);
  ALineColor.L := IfThen(ALineColor.L > 0.5, ALineColor.L - 0.1, ALineColor.L + 0.1);
  ACanvas.Brush.Color := TdxColorUtils.HSLToColor(ALineColor);
...
begin
  ...
  Winapi.Windows.DrawText(Canvas.Handle, PChar(AHint), Length(AHint), R,
  ...
  MakeVisibleOnDesktop(R, FirstPos);
...
function TdxPreviewPage.GetHeaderPartVisible: Boolean;
begin
  Result := PartVisible and (Bounds.Top >= Preview.ClientRect.Top);
end;
...
function TdxPreviewPagesRowList.HasStructureChanged: Boolean;
var
  I: Integer;
  ARow: TdxPreviewPagesRow;
begin
  Result := Count <> Length(FStoredStructure);
  if not Result then
    for I := 0 to Count - 1 do
    begin
      ARow := Items[I];
      Result := FStoredStructure[I] <> ARow.FinishIndex - ARow.StartIndex + 1;
      if Result then
        Break;
    end;
end;

procedure TdxPreviewPagesRowList.StoreStructure;
var
  I: Integer;
  ARow: TdxPreviewPagesRow;
begin
  SetLength(FStoredStructure, Count);
  for I := 0 to Count - 1 do
  begin
    ARow := Items[I];
    FStoredStructure[I] := ARow.FinishIndex - ARow.StartIndex + 1;
  end;
end;
...
procedure TdxPreviewPageList.GetVisibleSelectedPageIndex(out AIndex: Integer; ATargetPageIndex: Integer = -1);
begin
  AIndex := -1;
  if Count > 0 then
  begin
    if InRange(ATargetPageIndex, 0, Count - 1) and Items[ATargetPageIndex].LargerPartVisible then
    begin
      AIndex := ATargetPageIndex;
      Exit;
    end;
    AIndex := 0;
...
begin
  ...
  begin
    ...
    try
      ...
      SetBkMode(DC, Winapi.Windows.OPAQUE);
      SelectObject(DC, Pen);
    end;
  end;
end;
...
function TdxPreviewPageMargin.IsRightToLeftLayout: Boolean;
begin
  Result := Preview.IsRightToLeft;
end;
...
begin
  Result := not IsRightToLeftLayout;
end;
...
constructor TdxCustomPreviewLayoutCalculator.Create(APreview: TdxCustomPreview);
begin
  inherited Create;
  FPreview := APreview;
end;
...
procedure TdxCustomPreviewLayoutCalculator.Calculate(AType: TdxChangeType);
begin
...
procedure TdxCustomPreviewLayoutCalculator.CalculatePages;
var
  AContentRect: TRect;
begin
  AContentRect := PagesArea;
  CalculatePagesRows(AContentRect);
  AlignPages(AContentRect);
end;

function TdxCustomPreviewLayoutCalculator.GetPageSiteBounds(APageIndex: Integer): TRect;
begin
  Result := Pages[APageIndex].SiteBounds;
end;

procedure TdxCustomPreviewLayoutCalculator.CalculateAbsoluteIndents;
begin
  FPreview.FAbsoluteIndentLeft := FPreview.GetAbsoluteIndentLeft;
  FPreview.FAbsoluteIndentRight := FPreview.GetAbsoluteIndentRight;
end;

procedure TdxCustomPreviewLayoutCalculator.CalculateIndent;
begin
  FPreview.FIndent := FPreview.AbsoluteIndent * FPreview.ZoomFactor div 100;
end;

function TdxCustomPreviewLayoutCalculator.CanAddRow(AColumnIndex: Integer; const ABounds: TRect;
  const APageSize: TPoint): Boolean;
begin
  Result := (FPreview.ZoomMode = pzmPages) and (AColumnIndex = FPreview.PageXCount) or
    (FPreview.ZoomMode <> pzmPages) and (ABounds.Left + APageSize.X > ABounds.Right);
end;

function TdxCustomPreviewLayoutCalculator.GetClientSize: TSize;
begin
  Result := cxSize(FPreview.ClientBounds);
end;

function TdxCustomPreviewLayoutCalculator.GetContentHeight: Integer;
begin
  Result := FPreview.AbsoluteIndent;
  if PageCount > 0 then
    Inc(Result, Pages.Bounds.Height);
end;

function TdxCustomPreviewLayoutCalculator.GetContentWidth: Integer;
begin
  if (PageCount = 0) or (ZoomMode = pzmPageWidth) then
    Result := FPreview.ClientWidth
  else
    Result := FPreview.AbsoluteIndent + Pages.Bounds.Width;
end;

function TdxCustomPreviewLayoutCalculator.GetScrollStep: Integer;
begin
  Result := dxPreviewScrollStep;
end;

function TdxCustomPreviewLayoutCalculator.GetScrollTopPos: Integer;
begin
  Result := FPreview.TopPos;
end;

procedure TdxCustomPreviewLayoutCalculator.CalculateVisiblePageRange(out AFirstPageIndex, ALastPageIndex: Integer);
begin
  Pages.GetPartVisiblePageRanges(AFirstPageIndex, ALastPageIndex);
end;

procedure TdxCustomPreviewLayoutCalculator.CalculateZoomFactorForPageWidthPreviewZoomMode(
...
procedure TdxCustomPreviewLayoutCalculator.CalculateZoomFactorForPagesPreviewZoomMode(
  AFirstPageIndex, ALastPageIndex: Integer);

  function CalcZoomFactorY(const R: TRect; AIndex, ACount: Integer): Single;
  var
    AZoom: Single;
    I: Integer;
  begin
    Result := FPreview.MaxZoomFactor;
    AIndex := Pages[AIndex].Cell.Y;
    for I := AIndex to AIndex + Min(PagesRows.Count - AIndex, ACount) - 1 do
    begin
      AZoom := ZoomFactor * cxRectHeight(R) / (PagesRows[I].Bounds.Bottom - PagesRows[AIndex].Bounds.Top);
      if AZoom < FPreview.MinZoomFactor then
        Break;
      Result := Min(Result, AZoom);
    end;
  end;

  function CalcZoomFactorX(AStartIndex, AFinishIndex, APagesCount: Integer): Single;
  begin
    Result := 0;
    AStartIndex := Pages[AStartIndex].Cell.Y;
    AFinishIndex := Pages[AFinishIndex].Cell.Y;
    ...
    Result := Max(ZoomFactor * cxRectWidth(GetPagesArea) / Result, FPreview.MinZoomFactor);
  end;
var
  AZoomX, AZoomY: Single;
begin
  AZoomX := CalcZoomFactorX(AFirstPageIndex, ALastPageIndex, FPreview.PageXCount);
  AZoomY := CalcZoomFactorY(GetPagesArea, AFirstPageIndex, FPreview.PageYCount);
  SetZoomFactorF(Min(AZoomX, AZoomY));
end;

procedure TdxCustomPreviewLayoutCalculator.AlignPages(const ABounds: TRect);
begin
  if not FPreview.NonCenterizePages then
  begin
    AlignPagesToHorizontalCenter(ABounds);
    if not FPreview.NonVerticalCenterizePages then
      AlignPagesToVerticalCenter(ABounds);
  end;
  AlignPagesToTopLeftPos;
end;

procedure TdxCustomPreviewLayoutCalculator.AlignPagesToHorizontalCenter(const ABounds: TRect);
var
  I, ABoundsWidth: Integer;
begin
  ABoundsWidth := ABounds.Width;
  for I := 0 to FPreview.PagesRows.Count - 1 do
    FPreview.FPages.CenterPagesInRow(FPreview.PagesRows[I], ABoundsWidth);
end;

procedure TdxCustomPreviewLayoutCalculator.AlignPagesToTopLeftPos;
begin
  OffsetContent(cxPoint(-FPreview.LeftPos, -GetScrollTopPos));
end;

procedure TdxCustomPreviewLayoutCalculator.AlignPagesToVerticalCenter(const ABounds: TRect);
var
  ADelta: Integer;
begin
  ADelta := (ABounds.Height - FPreview.FPages.Bounds.Height) div 2;
  if ADelta > 0 then
    FPreview.OffsetContent(cxPoint(0, ADelta));
end;

procedure TdxCustomPreviewLayoutCalculator.CalculatePagesRows(const ABounds: TRect);
var
  I, AColumnIndex, AFirstPageInRow: Integer;
  APage: TdxPreviewPage;
  APageSize: TPoint;
  ARect: TRect;
begin
  FPreview.PagesRows.Clear;
  ARect := cxRectSetHeight(ABounds, 0);
  AFirstPageInRow := 0;
  AColumnIndex := 0;

  for I := 0 to FPreview.PageCount - 1 do
  begin
    APage := FPreview.Pages[I];
    APageSize := cxPointScale(APage.PageSize.ActualSizeInPixels, FPreview.ActualZoomFactor, 100);
    if AFirstPageInRow <> I then
      if CanAddRow(AColumnIndex, ARect, APageSize) then
      begin
        FPreview.PagesRows.Add(AFirstPageInRow, I - 1, cxRect(ABounds.Left, ARect.Top, ARect.Left - FPreview.Indent, ARect.Bottom));
        ARect.Top := ARect.Bottom + FPreview.Indent;
    ...
    APage.Cell := Point(AColumnIndex, FPreview.PagesRows.Count);
    APage.Bounds := cxRectSetSize(ARect, APageSize.X, APageSize.Y);
    ARect.Bottom := Max(ARect.Bottom, APage.Bounds.Bottom);
    ARect.Left := APage.Bounds.Right + FPreview.Indent;
    Inc(AColumnIndex);
  end;
  FPreview.PagesRows.Add(AFirstPageInRow, FPreview.PageCount - 1, cxRect(ABounds.Left, ARect.Top,
    ARect.Left - FPreview.Indent, ARect.Bottom));
end;

procedure TdxCustomPreviewLayoutCalculator.OffsetContent(const AOffset: TPoint);
begin
  FPreview.OffsetContent(AOffset);
end;

function TdxCustomPreviewLayoutCalculator.HandleAllocated: Boolean;
begin
  Result := FPreview.HandleAllocated;
end;

procedure TdxCustomPreviewLayoutCalculator.CalculateZoomFactor;
var
  AFirstPageIndex: Integer;
  ALastPageIndex: Integer;
begin
  if (PageCount = 0) or not HandleAllocated or FPreview.IsZooming then
    Exit;

  FPreview.BeginZooming;
  try
    CalculateVisiblePageRange(AFirstPageIndex, ALastPageIndex);
...
function TdxCustomPreviewLayoutCalculator.GetPagesArea: TRect;
begin
  Result := FPreview.GetPagesArea;
end;

function TdxCustomPreviewLayoutCalculator.GetPageCount: Integer;
begin
  Result := FPreview.PageCount;
end;

function TdxCustomPreviewLayoutCalculator.GetPages: TdxPreviewPageList;
begin
  Result := FPreview.FPages;
end;

function TdxCustomPreviewLayoutCalculator.GetPagesRows: TdxPreviewPagesRowList;
begin
  Result := FPreview.PagesRows;
end;

function TdxCustomPreviewLayoutCalculator.GetScaleFactor: TdxScaleFactor;
begin
  Result := FPreview.ScaleFactor;
end;

function TdxCustomPreviewLayoutCalculator.GetZoomFactor: Integer;
begin
  Result := FPreview.ZoomFactor;
end;

function TdxCustomPreviewLayoutCalculator.GetZoomMode: TdxPreviewZoomMode;
begin
  Result := FPreview.ZoomMode;
end;

procedure TdxCustomPreviewLayoutCalculator.SetZoomFactorF(AValue: Single);
begin
  FPreview.ZoomFactor := Trunc(AValue);
  if FPreview.ZoomFactor = Trunc(AValue) then
    FPreview.FZoomFactorF := AValue
  else
    FPreview.FZoomFactorF := FPreview.ZoomFactor;
end;

procedure TdxCustomPreviewLayoutCalculator.SetZoomFactor(const AValue: Integer);
begin
  FPreview.ZoomFactor := AValue;
end;
...
begin
  inherited Create(AOwner);
  FNeedUpdateZoomState := True;
  CreateSubClasses;
  RecreateLayoutCalculator;
  Initialize;
end;
...
function TdxCustomPreview.IsZooming: Boolean;
begin
  Result := psZooming in State;
end;

procedure TdxCustomPreview.BeginZooming;
begin
  Include(FState, psZooming);
end;

procedure TdxCustomPreview.EndZooming;
begin
...
begin
  ...
  DoCalcPageCount;
  FLayoutCalculator.CalculateAbsoluteIndents;
  Invalidate;
end;
...
procedure TdxCustomPreview.MakePageVisible(APageIndex: Integer; AAnimated: Boolean);
...
begin
  ...
  begin
    ...
    ASiteBounds := FLayoutCalculator.GetPageSiteBounds(APageIndex);
    APagesArea := GetPagesArea;
    ALeft := LeftPos + GetDeltaX(APagesArea, ASiteBounds);
    ATop := TopPos + GetDeltaY(APagesArea, ASiteBounds);
    SetLeftTop(cxPoint(ALeft, ATop), AAnimated);
    CalculatePagesLayout;
  end;
end;
...
begin
  MakePageVisible(APageIndex, IsScrollAnimationEnabled and CanMakeVisibleAnimate);
end;
...
begin
  if SelPageIndex < PageCount then
    ChangeSelPageIndex(SelPageIndex, 1);
end;

procedure TdxCustomPreview.SelectPrevPage;
begin
  DoSelectPrevPage(SelPageIndex);
end;

function TdxCustomPreview.GetClientSize: TSize;
begin
  Result := FLayoutCalculator.ClientSize;
end;
...
function TdxCustomPreview.CanCalculate: Boolean;
begin
  Result := inherited CanCalculate and not IsUpdateLocked;
end;

procedure TdxCustomPreview.Calculate(AType: TdxChangeType);
begin
  FLayoutCalculator.Calculate(AType);
end;

procedure TdxCustomPreview.CalculatePagesLayout;
begin
  FLayoutCalculator.CalculatePages;
end;
...
begin
  Result := FLayoutCalculator.ScrollStep;
end;
...
function TdxCustomPreview.CreateLayoutCalculator: TdxCustomPreviewLayoutCalculator;
begin
  Result := TdxCustomPreviewLayoutCalculator.Create(Self);
end;
...
procedure TdxCustomPreview.RecreateLayoutCalculator;
begin
  FreeAndNil(FLayoutCalculator);
...
function TdxCustomPreview.GetZoomFactor: Integer;
begin
  Result := Trunc(FZoomFactorF);
end;
...
begin
  ...
  begin
    BeginUpdate;
    try
      FPrevZoomFactor := ZoomFactor;
      NeedUpdateZoomState := True;
      SetZoomFactorF(Value);
      if [psZooming] * State = [] then
      begin
        FZoomMode := pzmNone;
        FZoomed := ZoomFactor >= 100;
      end;
    finally
      EndUpdate;
    end;
...
procedure TdxCustomPreview.DoSelectNextPage(AStartPageIndex: Integer);
begin
  if AStartPageIndex < PageCount - 1 then
    ChangeSelPageIndex(AStartPageIndex, 1);
end;

procedure TdxCustomPreview.DoSelectPrevPage(AStartPageIndex: Integer);
begin
  if AStartPageIndex > 0 then
    ChangeSelPageIndex(AStartPageIndex, -1);
end;
...
procedure TdxCustomPreview.CreateSubClasses;
begin
...
procedure TdxCustomPreview.DoSelectPage(APageIndex: Integer);
begin
  SelPageIndex := APageIndex;
end;

procedure TdxCustomPreview.Initialize;
begin
  ...
  FUnzoomedFactor := 50;
  SetZoomFactorF(GetDefaultZoomFactor);
  FPrevZoomFactor := ZoomFactor;
  FZoomMode := pzmNone;
...
begin
  Result := FLayoutCalculator.ClientSize.cy;
end;
...
begin
  Result := FLayoutCalculator.ContentHeight;
end;

function TdxCustomPreview.GetContentWidth: Integer;
begin
  Result := FLayoutCalculator.ContentWidth;
end;
...
begin
  ...
  MakePageVisible(SelPageIndex, False);
  Update;
end;

procedure TdxCustomPreview.SetZoomFactorF(AValue: Single);
begin
  FZoomFactorF := AValue;
end;
  ...
  begin
    if IsVertical then
      ...
      if IsDragging then
        ...
        Pt.Y := Bounds.Top;
    Winapi.Windows.ClientToScreen(Handle, Pt);
  end;
...
begin
  ...
  if Result then
    DoSelectPage(APageIndex);
end;

procedure TdxCustomPreview.ChangeSelPageIndex(AStartPageIndex: Integer; ADirection: Integer);
var
  I: Integer;
begin
  I := 1;
  repeat
    if SelectPage(AStartPageIndex + ADirection * I) then
      Break;
    Inc(I);
  until (AStartPageIndex + ADirection * I = PageCount + 1) or (AStartPageIndex - ADirection * I = -2);
end;
...
initialization
  ...
  dxRegisterCursor(crdxPreviewHorzResize, HInstance, IDC_DXPREVIEW_MARGINSMOVEHORZ);
  dxRegisterCursor(crdxPreviewVertResize, HInstance, IDC_DXPREVIEW_MARGINSMOVEVERT);
  dxRegisterCursor(crdxPreviewZoomIn, HInstance, IDC_DXPREVIEW_ZOOMIN);
  dxRegisterCursor(crdxPreviewZoomOut, HInstance, IDC_DXPREVIEW_ZOOMOUT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.InitializationFinished(dxThisUnitName, SysInit.HInstance);{$ENDIF}
finalization
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.FinalizationStarted(dxThisUnitName, SysInit.HInstance);{$ENDIF}
  dxUnregisterCursor(crdxPreviewHorzResize);
  dxUnregisterCursor(crdxPreviewVertResize);
  dxUnregisterCursor(crdxPreviewZoomIn);
  dxUnregisterCursor(crdxPreviewZoomOut);
