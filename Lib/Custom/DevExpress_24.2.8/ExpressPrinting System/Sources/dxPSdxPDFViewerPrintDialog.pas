...
uses
  System.UITypes,
  System.SysUtils, Vcl.Forms, Vcl.Controls, System.Classes, Vcl.Menus, Vcl.ImgList, Vcl.ExtCtrls, Vcl.StdCtrls, System.Types,
  ...
  cxClasses, dxLayoutControl, cxImageList, cxCustomListBox, cxImageComboBox;

resourcestring
  sdxPDFViewerPagePlacementAndScaling = 'Situación y e&scala';  //..
  sdxPDFViewerPrintAsImage = 'Imprimir como imagen';
...
type
  ...
  protected
    ...
    function GetDefaultPosition: TPosition; override;
...
uses
  cxGeometry, dxCore, dxCustomPreview, dxPrintingStrs, dxPrnPg, dxPSUtl, dxPSGlbl, dxPSdxPDFViewerLnk, dxPSRes, dxPreVw,
  dxPDFViewer;

const
  dxThisUnitName = 'dxPSdxPDFViewerPrintDialog';
...
begin
  if (Preview <> nil) and (Preview.PrinterPage <> nil) then
...
function TdxPDFPrintDialog.GetDefaultPosition: TPosition;
begin
  Result := poOwnerFormCenter;
end;
...
begin
  inherited;
  if FIsPageRangesValid and not GetPageRangesHintVisible then
  begin
...
var
  AGrayShading: Boolean;
  AMode: TdxScaleMode;
  AOrientation: TdxPrinterOrientation;
begin
  AGrayShading := Preview.PrinterPage.GrayShading;
  AMode := Preview.PrinterPage.ScaleMode;
  AOrientation := Preview.PrinterPage.Orientation;
  ...
  try
    ...
    Preview.PrinterPage.GrayShading := AGrayShading;
    Preview.PrinterPage.Orientation := AOrientation;
...
begin
  ...
  Preview.BeginUpdate;
  TdxPreviewAccess(Preview.Preview).Zoomed := False;
  ...
  Preview.Preview.MaxZoomFactor := 200;
  Preview.EndUpdate;
