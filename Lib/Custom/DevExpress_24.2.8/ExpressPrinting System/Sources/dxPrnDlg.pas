...
uses
  ...
  System.Win.Registry, Vcl.ExtCtrls, Vcl.StdCtrls, Vcl.Menus, Vcl.Buttons, Vcl.ImgList, dxPSForm, dxPSESys, 
  ...
  cxGroupBox, cxLookAndFeels, System.IniFiles, cxClasses, dxLayoutContainer, dxLayoutControl,
  dxLayoutLookAndFeels, dxLayoutControlAdapters, dxLayoutcxEditAdapters, cxImageList, 
  cxCustomListBox, cxImageComboBox, dxSkinsCore, dxUIAClasses;
...
type
  ...
  TdxfmPrintDialog = class(TCustomdxPSForm)
    ...
    liPageNumbersWarningSign: TdxLayoutImageItem;
    liPageNumbersWarningText: TdxLayoutLabeledItem;
    lgPageNumbersWarning: TdxLayoutGroup;
    dxLayoutCxLookAndFeel_InfoBk: TdxLayoutStandardLookAndFeel;
  ...
  strict protected
    procedure SetPageRangesHint(const AText: string);
    function GetPageRangesHintVisible: Boolean;
  protected
...
uses
  System.Variants, Winapi.WinSpool, System.Math, System.Types, cxGeometry, dxShellDialogs, dxPrintingStrs, dxPSPopupMan, dxPSRes, dxPSEngn, dxPSEvnt,
  dxPSUtl, dxPSImgs;
...
var
  APageIndexes: TIntegers;
  ASrcOptions: TPrintDialogOptions;
  ADialog: Vcl.Dialogs.TPrintDialog;
begin
  if Safe.Cast<Vcl.Dialogs.TPrintDialog>(Dest, ADialog) then
  begin
    ADialog.Collate := Collate;
    ADialog.Copies := Copies;
    ADialog.MinPage := MinRange;
    ADialog.MaxPage := MaxRange;
    if DecodePageIndexes(Pages, APageIndexes) then
    try
      ADialog.FromPage := MinIntValue(APageIndexes);
      ADialog.ToPage := MaxIntValue(APageIndexes);
    finally
    ...
    ASrcOptions := ADialog.Options;
    XorOption(ASrcOptions, poPrintToFile, pdoPrintToFile in OptionsVisible);
    XorOption(ASrcOptions, poDisablePrintToFile, pdoPrintToFile in OptionsEnabled);
    XorOption(ASrcOptions, poPageNums, (pdoPageRange in OptionsVisible) and (pdoPageRange in OptionsEnabled));
    XorOption(ASrcOptions, poSelection, (pdoSelection in OptionsVisible) and (pdoSelection in OptionsEnabled));
    XorOption(ASrcOptions, poHelp, (pdbHelp in ButtonsVisible) and (pdbHelp in ButtonsEnabled));
    ADialog.Options := ASrcOptions;

    ADialog.PrintToFile := PrintToFile;

    if PageRanges = prSelection then
      ADialog.PrintRange := Vcl.Dialogs.prAllPages
    else
      ADialog.PrintRange := TPrintRange(PageRanges);
  end;
end;
...
begin
  inherited;
  Filer.DefineProperty('IsTitleAssigned', ReadIsTitleAssigned, WriteIsTitleAssigned, FIsTitleAssigned and (Title = ''));
end;
...
begin
  ...
  miPageSetup.ImageIndex := 3;
  miDefineStyles.ImageIndex := 4;
...
begin
  ...
  AIndex := cbxNumberOfPages.ItemIndex;
  AItems := cbxNumberOfPages.Properties.Items;
  AItems.BeginUpdate;
  try
    AItems.Clear;
    AItems.Add(cxGetResourceString(@sdxPrintDialogAllPages));
    AItems.Add(cxGetResourceString(@sdxPrintDialogEvenPages));
    AItems.Add(cxGetResourceString(@sdxPrintDialogOddPages));
  finally
    AItems.EndUpdate;
  end;
  cbxNumberOfPages.ItemIndex := AIndex;
...
//  FModified := False;
  ...
  try
    ...
    { visible }
    libtnPrinterProperties.Visible := pdbPrinterProperties in APrintDlgData.ButtonsVisible;
    libtnNetwork.Visible := (pdbNetwork in APrintDlgData.ButtonsVisible) and not IsWin95;
    lichbxPrintToFile.Visible := pdoPrintToFile in APrintDlgData.OptionsVisible;
    licbxFileName.Visible := pdoPrintToFile in APrintDlgData.OptionsVisible;
    libtnBrowse.Visible := pdoPrintToFile in APrintDlgData.OptionsVisible;

    lirbtnAllPages.Visible := pdoAllPages in APrintDlgData.OptionsVisible;
    lirbtnCurrentPage.Visible := pdoCurrentPage in APrintDlgData.OptionsVisible;
    lirbtnSelection.Visible := pdoSelection in APrintDlgData.OptionsVisible;
    lirbtnPageRanges.Visible := pdoPageRange in APrintDlgData.OptionsVisible;
    lblDescription.Visible := lirbtnPageRanges.Visible;
    liedPageRanges.Visible := pdoPageRange in APrintDlgData.OptionsVisible;
    ...
    libtnPageSetup2.Visible := pdbPageSetup in APrintDlgData.ButtonsVisible;
    libtnDefineStyles.Visible := pdbDefineStyles in APrintDlgData.ButtonsVisible;

    libtnPageSetup.Visible := pdbPageSetup in APrintDlgData.ButtonsVisible;
    libtnPreview.Visible := pdbPreview in APrintDlgData.ButtonsVisible;
    libtnHelp.Visible := pdbHelp in APrintDlgData.ButtonsVisible;

    if (DialogData.StyleManager = nil) or not (pdoPrintStyles in APrintDlgData.OptionsVisible) then
      gbxPrintStyles.Visible := False;

    {enable}
    if libtnPrinterProperties.Visible then
      btnPrinterProperties.Enabled := pdbPrinterProperties in APrintDlgData.ButtonsEnabled;
    if libtnNetwork.Visible then
      btnNetwork.Enabled := dxPSUtl.IsNetworkPresent and (pdbNetwork in APrintDlgData.ButtonsEnabled);
    if lichbxPrintToFile.Visible then
      chbxPrintToFile.Enabled := pdoPrintToFile in APrintDlgData.OptionsEnabled;
    if licbxFileName.Visible then
    begin
      cbxFileName.Enabled := pdoPrintToFile in APrintDlgData.OptionsEnabled;
    ...
    if libtnBrowse.Visible then
      btnBrowse.Enabled := pdoPrintToFile in APrintDlgData.OptionsEnabled;
    if lirbtnAllPages.Visible then
      rbtnAllPages.Enabled := pdoAllPages in APrintDlgData.OptionsEnabled;
    if lirbtnCurrentPage.Visible then
      rbtnCurrentPage.Enabled := pdoCurrentPage in APrintDlgData.OptionsEnabled;
    if lirbtnSelection.Visible then
      rbtnSelection.Enabled := pdoSelection in APrintDlgData.OptionsEnabled;
    if lirBtnPageRanges.Visible then
      rBtnPageRanges.Enabled := pdoPageRange in APrintDlgData.OptionsEnabled;
    if liedPageRanges.Visible then
      edPageRanges.Enabled := pdoPageRange in APrintDlgData.OptionsEnabled;
    ...
    if libtnPageSetup.Visible then
      btnPageSetup.Enabled := pdbPageSetup in APrintDlgData.ButtonsEnabled;
    if libtnPreview.Visible then
      btnPreview.Enabled := pdbPreview in APrintDlgData.ButtonsEnabled;
    if libtnHelp.Visible then
      btnHelp.Enabled := pdbHelp in APrintDlgData.ButtonsEnabled;
    Caption := APrintDlgData.Title;

    FOnClose := APrintDlgData.Events.OnClose;
    FOnShow := APrintDlgData.Events.OnShow;
    FOnPageSetup := APrintDlgData.Events.OnPageSetup;
  finally
    UpdateControlsState;
...
begin
  OpenDialog := TdxOpenFileDialog.Create(nil);
  try
    OpenDialog.Title := cxGetResourceString(@sdxPrintDialogOpenDlgTitle);
    OpenDialog.Filter := Format('%s (*.*)|*.*|%s (*.prn)|*.PRN',
      [cxGetResourceString(@sdxPrintDialogOpenDlgAllFiles),
       cxGetResourceString(@sdxPrintDialogOpenDlgPrinterFiles)]);
    OpenDialog.FilterIndex := 2;
    OpenDialog.DefaultExt := 'prn';
    OpenDialog.FileName := cbxFileName.Text;
    if OpenDialog.Execute(Handle) then
      cbxFileName.Text := OpenDialog.FileName;
  finally
    OpenDialog.Free;
  end;
end;
...
var
  APageRangeCorrect: Boolean;
begin
  TcxTextEdit(Sender).Text := StringReplace(TcxTextEdit(Sender).Text, ' ', '', [rfReplaceAll, rfIgnoreCase]);
  ...
  begin
    try
      ...
      APageRangeCorrect := CheckPageRanges or (FDialogData.MaxRange = -1);
      if not APageRangeCorrect then
        SetPageRangesHint(Format(cxGetResourceString(@sdxPrintDialogPageNumbersOutOfRange), [FDialogData.MinRange, FDialogData.MaxRange]));
    except
      APageRangeCorrect := False;
      SetPageRangesHint(Format(cxGetResourceString(@sdxPrintDialogInvalidPageRanges), [edPageRanges.Text]));
    end;
    if not APageRangeCorrect and not GetPageRangesHintVisible and (ActiveControl <> btnOK) and (ActiveControl <> btnCancel) then
      Beep;
    if APageRangeCorrect then
      SetPageRangesHint('');
  end;
end;
...
begin
  FDialogData.StyleManager.CurrentStyle := TBasedxPrintStyle(lbxPrintStyles.Items.Objects[lbxPrintStyles.ItemIndex]);
  btnCancel.Caption := cxGetResourceString(@sdxBtnClose);
end;
...
var
  ...
  R: TRect;
begin
  ADC := TPaintBox(Sender).Canvas.Handle;
  SelectObject(ADC, TPaintBox(Sender).Font.Handle);
  SetTextColor(ADC, GetSysColor(COLOR_WINDOWTEXT));
  R := TPaintBox(Sender).ClientRect;
  SetBkMode(ADC, TRANSPARENT);
  DrawCollatedPages(ADC, R, FDialogData.Collate);
  SelectClipRgn(ADC, 0);
end;
...
function TdxfmPrintDialog.GetPageRangesHintVisible: Boolean;
begin
  Result := lgPageNumbersWarning.Visible;
end;

procedure TdxfmPrintDialog.RefreshPrinterList;
var
  APrinters: TStringList;
  I: Integer;
  AImageItem: TcxImageComboBoxItem;
begin
  APrinters := TStringList.Create;
  cbxPrinters.Properties.BeginUpdate;
  try
    dxGetPrinterList(APrinters);
    cbxPrinters.Properties.Items.Clear;
    for I := 0 to APrinters.Count - 1 do
    begin
      AImageItem := cbxPrinters.Properties.Items.Add;
      AImageItem.Description := APrinters[I];
      AImageItem.Value := I;
      if TdxPrintDeviceInfo(APrinters.Objects[I]).IsFax then
        AImageItem.ImageIndex := 2
      else
        AImageItem.ImageIndex := IfThen(TdxPrintDeviceInfo(APrinters.Objects[I]).IsDefault, 1, 0);
    end;
    if cbxPrinters.Properties.Items.Count > 0 then
      cbxPrinters.ItemIndex := dxPrintDevice.PrinterIndex;
  finally
    APrinters.Free;
    cbxPrinters.Properties.EndUpdate;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      lbxPrintStyles.Items.AddObject(AStyle.StyleCaption, AStyle);
    end;
    lbxPrintStyles.ItemIndex := lbxPrintStyles.Items.IndexOfObject(FDialogData.StyleManager.CurrentStyle);
  finally
...
var
  AIndex: Integer;
begin
  FDialogData.Pages := StringReplace(edPageRanges.Text, ' ', '',  [rfReplaceAll, rfIgnoreCase]);
  FDialogData.FileName := GetFileName(Trim(cbxFileName.Text));
  if FDialogData.FileList <> nil then
  begin
    AIndex := FDialogData.FileList.IndexOf(FDialogData.FileName);
    if AIndex = -1 then
      FDialogData.FileList.Insert(0, FDialogData.FileName)
    else
      FDialogData.FileList.Exchange(0, AIndex);
  end;
end;
...
procedure TdxfmPrintDialog.SetPageRangesHint(const AText: string);
begin
  liPageNumbersWarningText.CaptionOptions.Text := AText;
  lgPageNumbersWarning.Visible := AText <> '';
end;
...
begin
  try                         
    if Winapi.WinSpool.OpenPrinter(dxPrintDevice.CurrentDevice, PrinterHandle, nil) then
    try
      Winapi.WinSpool.GetPrinter(PrinterHandle, 2, nil, 0, @Size);
      if Size > 0 then
      begin
        ...
        if PBuffer <> nil then
        try
          if Winapi.WinSpool.GetPrinter(PrinterHandle, 2, PBuffer, Size, @Size) then
          begin
...
procedure TdxfmPrintDialog.lbxPrintStylesDrawItem(AControl: TcxListBox; ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
  dxPSDrawStyleItem(TBasedxPrintStyle(AControl.Items.Objects[AIndex]),
    ACanvas, ARect, AControl.Items[AIndex], AState, True, False, ScaleFactor);
end;
