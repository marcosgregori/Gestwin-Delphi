...
begin
  case code of
   ...
   {mistyped pattern table}
    raise Exception.CreateFmt('%s: error interno', [self.ClassName]);  //..
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    for i := 1 to Length(S) do
      if ((S[i] > '9') or (S[i] < '0')) and not (Typ in [bcCode128C, bcCodeEAN128C]) and ((S[i] = '(') or (S[i] = ')'))  then
        raise Exception.Create( 'El código de barras debe ser numérico'); //..
  end;
  ...
  try  //..
    ...
    raise Exception.CreateFmt('%s: tipo de código de barras erróneo', [self.ClassName]); //..
    end;  //..
...
begin
  ...
  Assert(Length(tmp)=8, 'Longitud errónea del texto (EAN8)'); //..
...
begin
  ...
  Assert(Length(tmp) = 13, 'Longitud errónea del texto (EAN13)');  //..
...
begin
  ...
  begin
    ...
    if idx < 0 then
      raise Exception.CreateFmt('%s: datos incorrectos en Code93 <%s>', [self.ClassName,FText]);  //..
