...
begin
  ...
  FRowHeight := Canvas.TextHeight('Wg') + {$IFDEF NONWINFPC}6{$ELSE}6{$ENDIF};  //..
  with Box.VertScrollBar do
  begin
    Increment := FRowHeight;
    Tracking := True;
  end;
  ObjectsCB.ItemHeight := Round(18 * aNewPPI / frx_DefaultPPI);  //..
...
{$ELSE}
  FTabs.Font.Height := Round( -13 * CurrentFormPPI / frx_DefaultPPI);  //..
  FTabs.Height := Abs(FTabs.Font.Height) + Round( 10 * CurrentFormPPI / frx_DefaultPPI); //..
{$ENDIF}
  h := FTabs.Height + 4;
