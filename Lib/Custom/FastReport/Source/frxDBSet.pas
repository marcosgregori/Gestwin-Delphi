...
begin
     If   not FSavingBeforeClose
     then begin
          FSavingBeforeClose := True;
          try
            If    Assigned( FSaveCloseEvent ) and ( Sender<>nil )
            then FSaveCloseEvent(Sender);
            If FBookMark<>nil
            then FDS.FreeBookmark(FBookmark);
            FBookMark := nil;
            FInitialized := False;
          finally
            FSavingBeforeClose := False;
            end;
          end;
end;                                                    //..
