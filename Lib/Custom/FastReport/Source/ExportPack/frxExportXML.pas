...
{$IFNDEF Linux}
  ComObj,
{$ENDIF}
frxClass, frxExportMatrix, frxProgress,
...
{$ENDIF}
  ...
  function ConvertFormat(fstr, fdecsep, fthsep: string): string;  //..
  var
    err, p : integer;
    s: string;
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      FProgress.Execute(FMatrix.PagesCount, 'Exportando páginas', True, True);  //..
    end;
...
{$ENDIF}
    WriteExpLn('<?fr-application homesite="http://www.gestwin.com"?>');
...
begin
  ...
  if IEMPage <> nil then
  begin
    IEMPage.PageName := Page.Name;
    if Assigned(OnGenerateSheetName) then
      IEMPage.PageName := OnGenerateSheetName(IEMPage.PageName, Index);
    IEMPage.PrintOnPreviousPage := Page.PrintOnPreviousPage;
  end;
