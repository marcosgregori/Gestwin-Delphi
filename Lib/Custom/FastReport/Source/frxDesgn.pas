...
, System.UITypes
{$ENDIF}
, dxBar, cxClasses, cxStyles, cxDropDownEdit, cxBarEditItem,  //..
  cxPC, dxDockControl, dxDockPanel, Vcl.DockTabSet,
  cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters, dxStatusBar, System.ImageList, dxSkinsCore, dxSkinsDefaultPainters,
  System.Actions;
...
{$ENDIF}
    ...
    FicherosRecientesItem: TdxBarSubItem;
    dxBarLargeButton1: TdxBarLargeButton;
    dxBarLargeButton2: TdxBarLargeButton;
    NewMI: TdxBarButton;
    NewReportMI: TdxBarButton;
    NewPageMI: TdxBarButton;
    NewDialogMI: TdxBarButton;
    OpenMI: TdxBarButton;
    SaveMI: TdxBarButton;
    SaveAsMI: TdxBarButton;
    PreviewMI: TdxBarButton;
    PageSettingsMI: TdxBarButton;
    ExitMI: TdxBarButton;
    UndoMI: TdxBarButton;
    RedoMI: TdxBarButton;
    CutMI: TdxBarButton;
    CopyMI: TdxBarButton;
    PasteMI: TdxBarButton;
    DeleteMI: TdxBarButton;
    DeletePageMI: TdxBarButton;
    SelectAllMI: TdxBarButton;
    GroupMI: TdxBarButton;
    UngroupMI: TdxBarButton;
    EditMI: TdxBarButton;
    FindMI: TdxBarButton;
    ReplaceMI: TdxBarButton;
    FindNextMI: TdxBarButton;
    BringtoFrontMI: TdxBarButton;
    SendtoBackMI: TdxBarButton;
    ReportDataMI: TdxBarButton;
    VariablesMI: TdxBarButton;
    ReportStylesMI: TdxBarButton;
    ReportSettingsMI: TdxBarButton;
    ToolbarsMI: TdxBarSubItem;
    StandardMI: TdxBarButton;
    TextMI: TdxBarButton;
    FrameMI: TdxBarButton;
    AlignmentMI: TdxBarButton;
    ToolsMI: TdxBarButton;
    InspectorMI: TdxBarButton;
    DataTreeMI: TdxBarButton;
    ReportTreeMI: TdxBarButton;
    ShowRulersMI: TdxBarButton;
    ShowGuidesMI: TdxBarButton;
    AutoGuidesMI: TdxBarButton;
    OptionsMI: TdxBarButton;
    ConnectionsMI: TdxBarButton;
    HelpContentsMI: TdxBarButton;
    AboutMI: TdxBarButton;
    FileMenu: TdxBarSubItem;
    EditMenu: TdxBarSubItem;
    ReportMenu: TdxBarSubItem;
    ViewMenu: TdxBarSubItem;
    HelpMenu: TdxBarSubItem;
    FindB: TToolButton;
    DeleteHGuidesMI: TdxBarButton;
    DeleteVGuidesMI: TdxBarButton;
    EdConfigMI: TdxBarButton;
    WatchMI: TdxBarButton;
    BreakPoints: TdxBarButton;
...
{$ENDIF}
  protected
    ...
    procedure AttachDialogFormEvents(Attach: Boolean); override; //..
    procedure SetPreparing(const Value : Boolean ); override;  //..
  public
    destructor Destroy; override;
    { Public declarations }
...
uses
  ...
  frxTableObject, AppManager, AppContainer, AppForms;  //..
...
begin
  ...
  if not(FPrefEvent in [peAtShow, peAtShowOnce]) then
    ReloadReport;
  SetBarItemVisible( ConnectionsMI, False ); //..
  if frxDesignerComp <> nil then
  begin
    SetBarItemVisible( ConnectionsMI, frxDesignerComp.Standalone ); //..
...
begin
  ...
  SetBarItemVisible( ToolsMI, ExtraToolsTB.ButtonCount > 0 ); //..
...
{$ENDIF}
  ...
  begin
    Report.Errors.Add('El objeto página no es una página');  //..
  end;
...
var
  ...
  pmi: TMenuItem;
{ //..
  procedure AddItem(ParentMI: TdxBarButton; aEvent: TNotifyEvent; sStream: TfrxIOTransportStream);
  begin
...
begin
  ...
  If   Assigned( Report.PreviewForm)  //..
  then Report.PreviewForm.Destroy;
...
begin
  ...
  BarManager.ImageOptions.Images := MainImages; //..
...
{$ENDIF}
{$IFDEF Delphi11}
   StandardTB.Transparent := True;  //..
   AlignTB.Transparent := True;
   TextTB.Transparent := True;
   FrameTB.Transparent := True;
   ExtraToolsTB.Transparent := True;
   ObjectsTB1.Transparent := True;  //..
{$ENDIF}
end;
...
{$IFDEF UseTabset}
  ...
  //FTabs.ShrinkToFit := True;
   ...
   // FTabs.BackgroundColor := clWindow;  //..
  ...
  FTabs.TabPosition := TTabPosition.tpTop;  //..
{$ELSE}
...
{$ENDIF}
  ...
  FTabs.BackgroundColor := TabPanel.Color;
  FTabs.Align := alTop;
  FTabs.Margins.SetBounds( 0, 4, 0, 0 ); //..
  FTabs.AlignWithMargins := True;  //..
  FTabs.Font.Name := 'Segoe UI'; //..
  FTabs.Font.Size := 10;         //..
  FTabs.Height := Abs( FTabs.Font.Height ) + 8;
{$IFDEF UseTabset}
  TabPanel.Height := FTabs.Height + 8;
  // Panel1.SetBounds(0, FTabs.Height + 2, 2000, 2 );
{$ELSE}
  Panel1.BringToFront;
...
begin
     ...
     then begin
          ...
          If   FPage is TfrxDialogPage
          then begin

               // La tontería de reutilizar los DialogForms del diseñador en la previsualización hace que el comportamiento
               // sea errático, sobre todo en sistemas con varios monitores. De momento recoloco siempre las ventanas a mano y punto.

               FDialogForm.Top := Round( DefaultTop { FPage.Top } * FDialogForm.CurrentFormPPI / frx_DefaultPPI );
               FDialogForm.Left := Round( DefaultLeft { FPage.Left } * FDialogForm.CurrentFormPPI / frx_DefaultPPI );
          ...
          FDialogForm.BringToFront;
          end
     else If   FDialogForm <> nil
          then begin
...
procedure TfrxDesignerForm.SetPreparing(const Value : Boolean );
begin
     PreviewB.Enabled := not Value;
end;
...
{$ENDIF}
    ...
    if c is TfrxCustomMemoView then
      ...
      begin
        ...
        TextAlignTopB.Down := VAlign = frxClass.vaTop; //..
        TextAlignMiddleB.Down := VAlign = frxClass.vaCenter; //..
        TextAlignBottomB.Down := VAlign = frxClass.vaBottom; //..
...
begin
  ...
  CodeWindow.Font.Size := Ini.ReadInteger(lName, 'ScriptFontSize', 11); //..
  MemoFontName := Ini.ReadString(lName, 'MemoFontName',
    {$IFNDEF LCLGTK2}'Arial'{$ELSE}'FreeSans'{$ENDIF});
  MemoFontSize := Ini.ReadInteger(lName, 'MemoFontSize', 11); //..
...
{$ELSE}
  Caption := 'Diseñador de informes - ' + GetReportName;  //..
{$ENDIF}
end;
...
//..
  ...
  FicherosRecientesItem.ItemLinks.Clear;

  If   CheckOp( drDontShowRecentFiles )
  then begin
       BarManager.BeginUpdate;
       FicherosRecientesItem.Visible := ivAlways;
       For I := FRecentFiles.Count - 1 downto 0 do
         AddBarButtonItem( FicherosRecientesItem, FRecentFiles[ I ] ).OnClick := OpenRecentFile;
       BarManager.EndUpdate;
       end
  else FicherosRecientesItem.Visible := ivNever;
          ...
          if c is TfrxCustomMemoView then
            with TfrxCustomMemoView(c) do
              if TextAlignTopB.Down then
                VAlign := frxClass.vaTop  //..
              else if TextAlignMiddleB.Down then
                VAlign := frxClass.vaCenter  //..
              else
                VAlign := frxClass.vaBottom; //..
...
var
  i: Integer;
begin
     If  TObject( SelectedObjects[0] ) is TfrxCustomMemoView  //..
     then with TfrxHighlightEditorForm.Create(Self) do
            begin
...
begin
     If   CheckOp( drDontLoadReport ) 
     then LoadFile( TdxBarButton( Sender ).Caption, True );  //..
end;
...
begin
  ...
  { Debido a la reutilización de los DialogForms en la previsualización me veo obligado
    a cambiar la página activa, por si es una de diálogo (desaparece, se descoloca y un largo etc..) }

  If   ( FTabs.TabIndex>1 ) and not ( Report.Pages[ FTabs.TabIndex - 1 ] is TfrxReportPage )
  then begin

       var FirstReportPage : TfrxReportPage := nil;

       For var PageNbr := 1 to Report.PagesCount do
       If   Report.Pages[ PageNbr ] is TfrxReportPage
       then begin
            FirstReportPage := TfrxReportPage( Report.Pages[ PageNbr ] );
            Break;
            end;

       SetPage( FirstReportPage );
       end;
  ...
  Report.ScriptText := CodeWindow.Lines;  //.. Es importante reasignar el script porque se pierde la última asignación
  ...
  then begin
       ...
       If   Preview.Owner is TgxForm
       then PreviewForm := TForm( Preview.Owner )
       else PreviewForm := nil;

       If   Assigned( PreviewForm )
       then begin

            { If   Assigned( FDialogForm ) and FDialogForm.Visible
            then } AttachDialogFormEvents( False );

            If   ( PreviewForm.Owner<> nil ) and ( PreviewForm.Owner is TForm )
            then TForm( PreviewForm.Owner ).SetFocus;
...
begin
  ...
  try
    reg.RootKey := HKEY_CURRENT_USER;
     if reg.OpenKey('\Software\Gestwin\Reports\Resources', false) then //..
...
begin
  ...
  LeftDockSite1.ParentBackground := True;  //..
  ...
  DockTop.Color := ApplicationContainer.GetDefaultContentColor;  //..
{$IFDEF Delphi10}
  
  
  PanelTB1.ParentColor := True; //..
  PanelTB2.ParentColor := True;
  PanelTB3.ParentColor := True;
  frTBPanel1.ParentColor := True;
