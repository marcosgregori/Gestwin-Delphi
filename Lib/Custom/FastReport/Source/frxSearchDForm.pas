...
begin
  ...
  if not Result and (FCodePos > 1) and
    ( frxConfirmMsg( Format(frxResources.Get('dsTextNotFound'), [edtFind.Text]) + sLineBreak + frxResources.Get('dsContinueSearch') ) = mrYes ) then  //..
    {
...
begin
  ...
  begin
    ...
    if not bFound and ( frxConfirmMsg( Format(frxResources.Get('dsTextNotFound'), [edtFind.Text]) + sLineBreak + frxResources.Get('dsContinueSearch') ) = mrYes ) then  //..
    {
    MessageDlg(Format(frxResources.Get('dsTextNotFound'), [edtFind.Text]) + sLineBreak + frxResources.Get('dsContinueSearch'),
        mtConfirmation, [mbYes, mbNo], 0) = mrYes) then }
...
begin
  if not FReplaceAll then
    i := frxConfirmMsg( Format(frxResources.Get('dsReplace'), [edtFind.Text]) + sLineBreak + ReplaceIn ) //..

    {
    MessageDlg(Format(frxResources.Get('dsReplace'), [edtFind.Text]) + sLineBreak + ReplaceIn,
