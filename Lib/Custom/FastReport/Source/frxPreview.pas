...
{$ENDIF}
, AppForms,
  cxLookAndFeelPainters, cxButtons, AppContainer, dxSkinsCore,
  dxSkinsdxBarPainter, dxBar, cxClasses, cxBarEditItem, cxGraphics,
  dxSkinsDefaultPainters, System.ImageList, dxBarExtItems,
  cxImageList, cxControls, cxLookAndFeels, cxContainer, cxEdit,
  cxLabel, cxProgressBar, cxSplitter, cxGroupBox, GestwinSkin, dxUIAClasses;
...
{$ENDIF}
  ...
  public
    ...
    function RefreshReport : Boolean; override;  //..
  ...
  TfrxPreviewForm = class(TgxForm) //.. TfrxBaseForm
    ...
    ZoomComboBox: TdxBarCombo;
    FullScreenButton: TdxBarLargeButton;
    ...
    PageNumberEdit: TcxBarEditItem;
    NextButton: TdxBarLargeButton;
    LastButton: TdxBarLargeButton;
    ExportFiltersImageList: TcxImageList;
    BarHeigthBarStatic: TdxBarStatic;
    procedure FormDestroy(Sender: TObject);
    procedure FormCreate(Sender: TObject);

    procedure FormKeyPress(Sender: TObject; var Key: Char);
	procedure ZoomWholePageButtonClick(Sender: TObject);
    procedure ZoomPageWidthButtonClick(Sender: TObject);
    ...
    procedure DesignerButtonClick(Sender: TObject);
    procedure NewPageButtonClick(Sender: TObject);
    procedure DelPageButtonClick(Sender: TObject);
    ...
    procedure PDFExportButtonClick(Sender: TObject);
...
resourceString
  RsMsg1 = 'No se han podido copiar los ficheros temporales.';
  RsMsg2 = 'Puede que la salida de datos no sea la esperada, sin embargo esto no afectará a la edición del informe.';
  RsMsg3 = 'Cancelar el proceso de generación del informe.';
  RsMsg4 = 'Cerrar la ventana de pre-visualización';
{$IFNDEF FPC}
type
...
begin
  ...
  FFrameColor := clBlack;
  FActiveFrameColor := clActiveCaption; // $804020;

  With ApplicationContainer do
    If   SkinController.UseSkins and Assigned( AppSkinPainter )
    then begin
         FBackColor := AppSkinPainter.DefaultGroupColor;
         FFrameColor := AppSkinPainter.DefaultGridLineColor;
         // FActiveFrameColor := SkinPainter.DefaultSelectionColor;
         end;
...
var  EventParams: TfrxPreviewIntEventParams;

begin

     If   ( FPageList.Count=0 ) or Locked or FIsThumbnail or ( csDestroying in ComponentState ) //..
     then Exit;
...
{$ENDIF}
  ...
  begin
    ...
    FSearchFrm.Constraints.MinWidth:= 240;  //..
  end;
...
begin
  ...
  if Owner is TfrxPreviewForm then
    TfrxPreviewForm(Owner).OutlineButton.Down := Value;  //..
  if NeedChange then
    UpdatePages;
end;
...
begin
  ...
  if Owner is TfrxPreviewForm then
    TfrxPreviewForm(Owner).ThumbButton.Down := Value;  //..
  if NeedChange then
    UpdatePages;
end;
...
begin
  ...
  if not Value then
    TextFound := False;
  if Owner is TfrxPreviewForm then
    TfrxPreviewForm(Owner).FindButton.Down := Value; //..
  if NeedChange then
    UpdatePages;
end;
...
begin
  ...
  if not TfrxPreviewForm(Self.Parent).FindButton.Down then
     TfrxPreviewForm(Self.Parent).FindButton.Down := True
  else
     TfrxPreviewForm(Self.Parent).FindButton.Down := False;
  FindFmVisible := TfrxPreviewForm(Self.Parent).FindButton.Down;
end;
...
function TfrxPreview.RefreshReport : Boolean;  //..
var
...
begin
     Pages := nil;
     If   Assigned( Report)
     then Pages := Report.PreviewPages;

     If   not Assigned( FOutline.TreeView.Items )
     then Exit;

     FOutline.TreeView.Visible := False;
     ...
     then begin
          ...
          Outline.LevelRoot;
          DoUpdate( nil );
          If   Report.PreviewOptions.OutlineExpand
          then FOutline.TreeView.FullExpand;
          If   FOutline.TreeView.Items.Count>0
          then FOutline.TreeView.TopItem := FOutline.TreeView.Items[ 0 ];
          end;

     FOutline.TreeView.Items.EndUpdate;
     FOutline.TreeView.Visible := True;
end;
  ...
  begin
    ...
    begin
     ...
     then begin
          ...
          then UpdatePages;
          If   Owner is TfrxPreviewForm
          then TfrxPreviewForm(Owner).UpdateControls; //..
          FTick := GetTickCount;
          end
     else If   Progress=1
          then begin
          ...
          else If   Progress mod Integer( FTick )=0
               then begin
                    UpdatePageNumbers;
                    If   Report.Engine.FinalPass
                    then FWorkspace.UpdateScrollBars;
                    end;
...
begin
     Result := OutlineVisible and ( TfrxOutline( PreviewPages.Outline ).Root.Count > 0 );
end;
...
begin
  ...
  try
    ...
    if (Owner is TfrxPreviewForm) and TfrxPreviewForm(Owner).FindButton.Down then  //..
    begin
      TfrxPreviewForm(Owner).FindButton.Down := false;
      TfrxPreviewForm(Owner).FindButton.Click;
    end;
  finally
    Unlock;
  end;
...
begin
  ...
  ZoomComboBox.Items.Add(frxResources.Get('zmPageWidth'));
  ZoomComboBox.Items.Add(frxResources.Get('zmWholePage'));
{  //..
...
var  I, J : Integer;
     M, E : TdxBarButton;
     L1, L2 : TdxBarItemLink;
     W, H : Integer;
     ExportFilterClassName : String;

begin
  FIsClosing := False;
  FillItemsList(FFilterList, fvPreview);

   //.. Trasladado FPreview.Init(Report, Report.PreviewPages);
  with Report.PreviewOptions do
  begin

       {
       pbTools   = Herramientas de edición (cargar, guardar, suprimir página etc..)
       pbEdit    = Permitir editar la plantilla
       }

       If   Modal
       then BorderIcons := [ biSystemMenu ];

       If   not ApplicationContainer.UserCanEditReports
       then Buttons := Buttons - [ pbEdit ];

       // AdjustBarSize( ButtonsBar );

       AllowEdit := ApplicationContainer.UserCanModifyReports;
       If   not AllowEdit
       then Buttons := Buttons - [ pbLoad, pbSave, pbTools ];
       ...
       UpdateZoom;   // Actualiza el contenido del ComboBox

       try

         BarManager.BeginUpdate;

         SetBarItemVisible( ZoomPlusButton, pbZoom in Buttons );
         SetBarItemVisible( ZoomMinusButton, pbZoom in Buttons );
         SetBarItemVisible( FullScreenButton, pbZoom in Buttons );

         SetBarItemVisible( OutlineButton, pbOutline in Buttons );
         SetBarItemVisible( ThumbButton, pbOutline in Buttons );
         SetBarItemVisible( PageSettingsButton, pbPageSetup in Buttons );

         SetBarItemVisible( FirstButton, pbNavigator in Buttons );
         SetBarItemVisible( PriorButton, pbNavigator in Buttons );
         SetBarItemVisible( NextButton, pbNavigator in Buttons );
         SetBarItemVisible( LastButton, pbNavigator in Buttons );

         SetBarItemVisible( CancelButton, not ( pbNoClose in Buttons ) );

         SetBarItemVisible( PrintButton, pbPrint in Buttons );
         SetBarItemVisible( ExportButton, pbExport in Buttons );
         SetBarItemVisible( FindButton, pbFind in Buttons  );

         SetBarItemVisible( OpenButton, pbLoad in Buttons );
         SetBarItemVisible( SaveButton, pbSave in Buttons );
         SetBarItemsVisible( [ NewPageButton, DelPageButton, DesignerButton ], pbTools in Buttons );
         SetBarItemVisible( EditorButton, pbEdit in Buttons );

         If   ( FrxExportFilters.Count=0 ) or ( ( FrxExportFilters.Count=1 ) and ( FrxExportFilters[ 0 ].Filter=FrxDotMatrixExport ) )
         then SetBarItemVisible( ExportButton, False );

         For I := 0 to FrxExportFilters.Count - 1 do
           begin

           If   FrxExportFilters[ I ].Filter=FrxDotMatrixExport
           then Continue;

           M := TdxBarButton.Create( ExportPopupMenu );

           M.Caption := TfrxCustomExportFilter( FrxExportFilters[ I ].Filter ).GetDescription;
           M.OnClick := ExportMIClick;
           M.Tag := I;

           L1 := ExportPopupMenu.ItemLinks.Add;
           L1.Item := M;

           // Link.BeginGroup := NewGroup;

           ExportFilterClassName := TfrxCustomExportFilter( FrxExportFilters[ I ].Filter ).ClassName;

           If   ExportFilterClassName='TfrxHTMLExport'
           then M.ImageIndex := 0
           else If   ExportFilterClassName='TfrxXLSExport'
                then M.ImageIndex := 1
                else If   ExportFilterClassName='TfrxXMLExport'
                     then M.ImageIndex := 2
                     else If   ( ExportFilterClassName='TfrxRTFExport' ) or
                               ( ExportFilterClassName='TfrxDOCXExport' )
                          then M.ImageIndex := 3
                          else If   ( ExportFilterClassName='TfrxBMPExport' ) or
                                    ( ExportFilterClassName='TfrxJPEGExport' )
                               then M.ImageIndex := 4
                               else If   ExportFilterClassName='TfrxPDFExport'
                                    then begin
                                         M.ImageIndex := 5;
                                         FPDFExport := TfrxCustomExportFilter( frxExportFilters[ I ].Filter );
                                         SetBarItemVisible( PDFExportButton, pbExportQuick in Report.PreviewOptions.Buttons );
                                         end
                                    else If   ExportFilterClassName='TfrxSimpleTextExport'
                                         then M.ImageIndex := 6
                                         else If   ExportFilterClassName='TfrxCSVExport'
                                              then M.ImageIndex := 7
                                              else If   ExportFilterClassName='TfrxMailExport'
                                                   then M.ImageIndex := 8
                                                   else If   ExportFilterClassName='TfrxODSExport'
                                                        then M.ImageIndex := 9
                                                        else If   ExportFilterClassName='TfrxODTExport'
                                                             then M.ImageIndex := 10;


           If   not ( pbNoEmail in Report.PreviewOptions.Buttons )
           then begin
                If   ExportFilterClassName='TfrxMailExport'
                then begin
                     FEmailExport := TfrxCustomExportFilter( frxExportFilters[ I ].Filter );
                     SetBarItemVisible( EmailButton, pbExportQuick in Report.PreviewOptions.Buttons );
                     end;
                end
           else SetBarItemVisible( EmailButton, False );
     ...
     If   Report.ReportOptions.Name<>''
     then Caption := Report.ReportOptions.Name;

     FPreview.Init( Report, Report.PreviewPages );  //..

     {
     For I := 0 to ButtonsBar.ItemLinks.Count - 1 do
       If   ButtonsBar.ItemLinks[ I ].Item is TdxBarLargeButton
       then With TdxBarLargeButton( ButtonsBar.ItemLinks[ I ].Item ) do
              begin

              If   ( ButtonStyle<>bsDefault ) and
                   ( Visible=ivAlways ) and
                   ( Hint<>'' )
              then begin

                   M := TdxBarButton.Create( RightPopupMenu );
                   L1 := RightPopupMenu.ItemLinks.Add;
                   L1.Item := M;
                   L1.BeginGroup := ButtonsBar.ItemLinks[ I ].BeginGroup;

                   Tag := Integer( M );  // De ButtonsBar.ItemLinks[ I ].Control

                   M.Caption := Hint;
                   M.OnClick := OnClick;
                   M.LargeGlyph := LargeGlyph;

                   If   Assigned( DropdownMenu )
                   then For J := 0  to DropdownMenu.ItemLinks.Count - 1 do
                          begin
                          E := TdxBarButton.Create( M );
                          E.Caption := DropdownMenu.ItemLinks[ J ].Item.Caption;
                          E.Tag := DropdownMenu.ItemLinks[ J ].Item.Tag;
                          E.OnClick := DropdownMenu.ItemLinks[ J ].Item.OnClick;
                          L2 := ButtonsBar.ItemLinks.Add;
                          L2.Item := E;
                        end;
     ...
     // PopupMenu := RightPopupMenu;

     AdjustScale;    // Al hacer visible la ventana de forma prematura se escala cuando ya se ha nmostrado
     
     If   Report.PreviewOptions.MDIChild
     then try
             Visible := False;   // Ahora se pueden ocultar las MDIChildForms
             try
               FSwitchingToMDI := True;
               FormStyle := fsMDIChild;
             finally
               FSwitchingToMDI := False;
               end;
             FormType := fmBackground;
          finally
            Visible := True;
...
begin
     ...
     With Report.PreviewOptions do
       try
         ...
         CanProcess := not FPreview.FRunning and ( FPreview.PageCount>0 );

         PrintButton.Enabled := CanProcess;
         ExportButton.Enabled := CanProcess;
         FindButton.Enabled := CanProcess;
         PDFExportButton .Enabled := CanProcess;
         EmailButton.Enabled := CanProcess;
         PageSettingsButton.Enabled := CanProcess;

         CanModify := not FPreview.FRunning and Report.PreviewOptions.AllowEdit and not Assigned( Report.Designer );

         OpenButton.Enabled := CanModify;
         SaveButton.Enabled := CanModify;
         NewPageButton.Enabled := CanModify;
         DelPageButton.Enabled := CanModify;
         DesignerButton.Enabled := CanModify;
         EditorButton.Enabled := CanModify;

         With CancelButton do
           If   FPreview.FRunning
           then begin
                Hint := RsMsg3;
                LargeImageIndex := 1;
                end
     ...
     // EnableControls([ExpandMI, CollapseMI, N1], not FPreview.FRunning and HasDrillDown);
end;
...
procedure TfrxPreviewForm.PrintButtonClick(Sender: TObject);
begin
     If   not frxPrinters.hasPhysicalPrinters
     then frxErrorMsg(frxResources.Get('clNoPrinters'))
     else try
            fPrinting := true;
            enabled := false;
            setPrintReportOptions( preview.report, true, true );
            fPreview.print;
          finally
            fPrinting := false;
            enabled := true;
...
procedure TfrxPreviewForm.OpenButtonClick(Sender: TObject);
begin
...
procedure TfrxPreviewForm.SaveButtonClick(Sender: TObject);
begin
  FPreview.SaveToFile;
end;

procedure TfrxPreviewForm.FindButtonClick(Sender: TObject);
begin
  //FPreview.Find;
  FPreview.FindFmVisible := FindButton.Down;
  if (not FPreview.FindFmVisible) then
    FPreview.SetFocus;
end;

procedure TfrxPreviewForm.ZoomPlusButtonClick(Sender: TObject);
begin
  FPreview.Zoom := FPreview.Zoom + 0.25;
     UpdateZoom;
end;

procedure TfrxPreviewForm.ZoomMinusButtonClick(Sender: TObject);
begin
  FPreview.Zoom := FPreview.Zoom - 0.25;
     UpdateZoom;
end;
procedure TfrxPreviewForm.ZoomWholePageButtonClick(Sender: TObject);
begin
     FPreview.ZoomMode := zmWholePage;
end;

procedure TfrxPreviewForm.ZoomPageWidthButtonClick(Sender: TObject);
begin
     FPreview.ZoomMode := zmPageWidth;
     ZoomComboBox.ItemIndex := 6;
end;

procedure TfrxPreviewForm.Zoom100ButtonClick(Sender: TObject);
begin
     FPreview.Zoom := 1;
end;
...
begin
     ZoomComboBox.Text := IntToStr( Round(FPreview.Zoom * 100 ) ) + '%';
end;

procedure TfrxPreviewForm.ZoomComboBoxChange(Sender: TObject);
...
begin
     ...
     If   ZoomComboBox.ItemIndex=6
     then FPreview.ZoomMode := zmPageWidth
     else If   ZoomComboBox.ItemIndex=7
          then FPreview.ZoomMode := zmWholePage
          else begin
               S := ZoomComboBox.Text;
               If   Pos( '%', S )<>0
               then S[ Pos( '%', S ) ] := ' ';
               While Pos( ' ', S )<>0 do
                 Delete( S, Pos( ' ', S ), 1 );
               If   S<>''
               then FPreview.Zoom := frxStrToFloat( S ) / 100;
               end;
...
begin
     case Key of
       VK_ESCAPE : If   not ApplicationContainer.TDI
                   then CancelButtonClick( Self );
       VK_F11    : SwitchToFullScreen;
       VK_F1     : frxResources.Help( Self );
     end;
end;
...
begin
     If   Key=#13
     then begin
          If   Assigned( ZoomComboBox.FocusedItemLink )
          then ZoomComboBoxChange( nil )
          else If   Assigned( PageNumberEdit.FocusedItemLink )
               then PageEClick( nil );
          end;
end;
...
procedure TfrxPreviewForm.PageSettingsButtonClick(Sender: TObject);
begin
...
begin
     FirstPass := False;
     If   FPreview.PreviewPages<>nil
     then FirstPass := not FPreview.PreviewPages.Engine.FinalPass;

     if   FirstPass and FPreview.FRunning
     then ProgressLabel.Caption := frxResources.Get('clFirstPass') + ' ' + IntToStr(FPreview.PageCount) //..
     else ProgressLabel.Caption := Format(frxResources.Get('clPageOf'), [ PageNo, FPreview.PageCount ] );

     PageNumberEdit.EditValue := PageNo;
end;

procedure TfrxPreviewForm.PageEClick(Sender: TObject);
begin
  FPreview.PageNo := StrToIntDef(PageNumberEdit.EditValue, FPreview.PageNo);
  FPreview.SetFocus;
end;

procedure TfrxPreviewForm.FirstButtonClick(Sender: TObject);
begin
  FPreview.First;
end;

procedure TfrxPreviewForm.PriorButtonClick(Sender: TObject);
begin
  FPreview.Prior;
end;

procedure TfrxPreviewForm.NextButtonClick(Sender: TObject);
begin
  FPreview.Next;
end;

procedure TfrxPreviewForm.LastButtonClick(Sender: TObject);
begin
  FPreview.Last;
end;
...
procedure TfrxPreviewForm.DesignerButtonClick(Sender: TObject);
begin
  FPreview.Edit;
end;

procedure TfrxPreviewForm.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
procedure TfrxPreviewForm.NewPageButtonClick(Sender: TObject);
begin
  FPreview.AddPage;
end;

procedure TfrxPreviewForm.DelPageButtonClick(Sender: TObject);
begin
  FPreview.DeletePage;
end;
...
procedure TfrxPreviewForm.CancelButtonClick(Sender: TObject);
begin
     If   FPreview.FRunning
     then begin
          FPreview.Cancel;
          FPreview.FRunning := False;  // Por si acaso
          end
     else If   FPrinting or FExporting
          then ApplicationContainer.EndProgression
          else begin
               If   Assigned( Report ) and Assigned( Report.Designer ) and ( Report.PreviewOptions.MDIChild )
               then Report.Designer.SetFocus
               else Close;
               end;
end;

procedure TfrxPreviewForm.ExportButtonClick(Sender: TObject);

var  P : TPoint;
     ItemLink : TdxBarItemLink;

begin
     With ExportPopupMenu do
       begin
       ItemLink := GetItemLinkByBarItem( ButtonsBar, ExportButton );
       If   Assigned( ItemLink )
       then begin
            With ItemLink.Control.ViewInfo.Bounds do
              begin
              P.X := Left;
              P.Y := Bottom;
              end;
...
var
  pt: TPoint;
begin
     pt := DesignerButton.ClientToScreen(Point(0, 0));
     If   Button=mbRight
     then HiddenMenu.Popup(pt.X, pt.Y);
end;
}
...
begin
     ...
     then Preview.FSearchFrm.CleartrvFind;
     FreeAndNil( FFilterList );
     inherited;
end;
...
procedure TfrxPreviewForm.FullScreenButtonClick(Sender: TObject);
begin
  SwitchToFullScreen;
end;

procedure TfrxPreviewForm.PDFExportButtonClick(Sender: TObject);
begin
  if Assigned(FPDFExport) then
    FPreview.Export(FPDFExport);
end;

procedure TfrxPreviewForm.EmailButtonClick(Sender: TObject);
begin
...
begin
     case Msg.CmdType of
       {
       SC_MINIMIZE : If   Report.PreviewOptions.MDIChild // and Report.PreviewOptions.Modal
                     then begin
                          ShowWindow( Application.Handle, SW_MINIMIZE );
                          Exit;
                          end;
       }
       SC_CLOSE    : If   Report.PreviewOptions.MDIChild and Assigned( Report.Designer )
                     then begin
...
procedure TfrxPreviewForm.OutlineButtonClick(Sender: TObject);
begin
     ThumbButton.Down := False;  //..
     FPreview.OutlineVisible := OutlineButton.Down;
end;

procedure TfrxPreviewForm.ThumbButtonClick(Sender: TObject);
begin
     OutlineButton.Down := False;  //..
     FPreview.ThumbnailVisible := ThumbButton.Down;
end;

procedure TfrxPreviewForm.OnPreviewDblClick(Sender: TObject);
begin
     If   FFullScreen
     then SwitchToFullScreen;
end;
...
procedure TfrxPreviewForm.EditorButtonClick(Sender: TObject);

procedure CopyTemporalTables;
...
begin
     ...
     With FPreview.Report do
       try

         TmpPathSt := AddPathDelim( DataAccessModule.directTemporalpath ) + '%s.nx1';

         // Esto lo hago para que se pueda reutilizar la tabla temporal actual
         ...
         For index := 0 to dataSets.count - 1 do
           ...
           then begin
                With TfrxNXTable( frxDataset ) do
                  If   TableType=ttTemporal
                  then Table.TableType := ttStandard;
                end;
           end;
...
procedure TfrxPreviewForm.FormDestroy( sender : TObject);
begin
     If   Assigned( FPreview.Report )
     then If   Assigned( FPreview.Report.Designer )
          then begin
               FPreview.Report.PreviewForm := nil;
               FPreview.Report.Designer.Free;
               end;
end;
...
var
  Sz: Integer;
begin
  Sz := Round((Self.ClientWidth  -   StatusPanel.Panels[0].Width)/2);
