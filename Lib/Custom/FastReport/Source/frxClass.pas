...
{$ENDIF}
  ...
  private
    ...
    procedure SetPreparing(const Value: Boolean); //..
    {$IFDEF FPC}
  ...
  published
    ...
    property Preparing : Boolean read FPreparing write SetPreparing;  //..
  ...
  protected
    ...
    procedure AttachDialogFormEvents(Attach: Boolean); virtual; abstract; //..
    procedure SetPreparing(const Value : Boolean ); virtual; abstract;  //..
  public
    /// <summary>
  ...
  public
    ...
    /// </summary>
    function RefreshReport : Boolean; virtual; abstract; //..
    /// <summary>
...
const
  DefFontName =  {$IFNDEF LCLGTK2}'Arial'{$ELSE}'Segoe UI'{$ENDIF};  //..
  DefFontNameBand = 'Segoe UI';
  DefFontSize = 10;
...
//.. Añadido el cálculo y la variable fmtStr
...
begin
  ...
  try
    FmtStr := AFormat.FormatStr;  //..

    If   FmtStr<>''
    then If   CharInSet( FmtStr[ 1 ], [ '[', '<' ] )
         then FmtStr := Report.Calc( FmtStr );
		 
     case AFormat.Kind of
      fkNumeric:
        begin
          if Pos('#', FmtStr) <> 0 then  //..
            begin
            NumValue := Extended(Value);
            Pc := Pos( '.', FmtStr );
            If   Pc<>0
            then begin
                 Dec := FmtStr.Length - Pc;
                 If   Dec<5
                 then begin
                      CurrValue := Currency( Value );  //.. Si no se convierte a Currency aparecen errores de redondeo
                      NumValue := SimpleRoundTo( CurrValue, -Dec );
                      end
                 else NumValue := SimpleRoundTo( NumValue, -Dec );
                 end;
            Result := FormatFloat(FmtStr, NumValue );
            end
          else if (Pos('d', FmtStr) <> 0) or (Pos('u', FmtStr) <> 0) then
            Result := Format(FmtStr, [Integer(Value)])
          else
            Result := Format(FmtStr, [Extended(Value)]);
...
begin
  ...
  then FForm.Perform(CM_FOCUSCHANGED, 0, frxInteger(FForm.ActiveControl.Handle));  //..
...
begin
  ...
  begin
    case i of
      0: Value := ApplicationContainer.TodayDate; //.. 
...
begin
  ...
  Preparing := True;
  PreparingReport := True; //.. Es una variable global que me permite saber si hay algún report en preparación
  try
...
{$ENDIF}
          ...
          else If   FPreviewForm<>nil           //..
               then begin
                    If   Assigned( FDesigner )
                    then begin
                         // FDesigner.SetPage( FDesigner.Page );
                         FDesigner.BringToFront;
                         end;
                    If   FPreviewForm.Owner<>Application.MainForm
                    then FPreviewForm.Close;
                    end;
        end;
      except
        on e: Exception do
...
begin
  ...
  begin
    FPreviewForm := TfrxPreviewForm.Create(Application.MainForm { Designer } );  //..
...
{$IFDEF FR_RUN_DESIGNER}
  ...
  begin
    ...
    FDesigner.CreateDesigner(Application.MainForm { nil }, Self);  //..
...
begin
  Result := FEngine.Running or Assigned(Script.ProgRunning) or Preparing;
end;
...
procedure TfrxReport.SetPreparing(const Value: Boolean);  //..
begin
     FPreparing := Value;
     If   Assigned( FDesigner )
     then FDesigner.SetPreparing( Value );
end;
