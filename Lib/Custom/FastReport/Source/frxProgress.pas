...
uses
  ...
  ComCtrls, StdCtrls, ExtCtrls, frxBaseForm, cxGraphics, cxLookAndFeels,
  cxLookAndFeelPainters, Vcl.Menus, cxButtons, AppContainer, cxControls,
  cxContainer, cxEdit, cxProgressBar, cxGroupBox, cxLabel
  {$IFDEF FPC}
...
begin
  ...
  Bar.Properties.Min := 0;
  Bar.Properties.Max := 100;
  Position := 0;
...
begin
  if (Position < Bar.Properties.Max) and (Position >= Bar.Properties.Min) then  //..
    Position := Position + 1;
end;
