...
uses
  ...
  System.Types, System.Variants, System.SysUtils, System.Classes,  Data.DB, dxCore, cxDataUtils, cxCustomData,
...
type
  ...
  TcxDBCustomDataLink = class(TcxCustomDataLink)
  private
    function GetIsDataSetBusy: Boolean;
  protected
    function GetIsDataSetBusyState: Boolean; virtual;
    function IsDataLocate: Boolean; virtual;
    function IsDataUpdate: Boolean; virtual;
    function IsOtherDataControllerLocate: Boolean;
  ...
  strict private type
    TListeners = class(TDictionary<TcxDBDataProvider, Byte>);
  private
...
//    FNearestRecordID: Variant;
    ...
    function GetDataController: TcxDBDataController; inline;
  ...
  private
    ...
    function GetFilterCriteria: TcxDBDataFilterCriteria;
  protected
    function GetFilterOperatorClass: TcxFilterOperatorClass; override;
    procedure SetItemLink(Value: TObject); override;
    function UseSQLIgnoreTimeOperatorClass: Boolean; virtual;
  public
    property Criteria: TcxDBDataFilterCriteria read GetFilterCriteria;
    property DataController: TcxDBDataController read GetDataController;
  ...
  private
    ...
    function GetProvider: TcxDBDataProvider; inline;
  ...
  protected
    ...
    function CanIgnoreTimeForFiltering(AItemLink: TObject): Boolean; virtual;
    function CanLoadData: Boolean; override;
    procedure CheckEditingOnFindCriteriaChanged; override;
    ...
    procedure FindCriteriaChanged; override;
    ...
    function SkipLocateNotification: Boolean; virtual;
  ...
  public
    ...
    // Navigation
    function CanFocusedRecordIndexChangePostData: Boolean; override;  // for internal use
    ...
    property InCheckBrowseMode : Boolean read FInCheckBrowseMode; //..
...
uses
  Winapi.Windows, System.TypInfo, System.RTLConsts, System.Contnrs, cxDataConsts, dxCoreClasses;

const
  dxThisUnitName = 'cxDBData';

type
  TDataSetAccess = class(TDataSet);
  TcxCustomDataRelationListAccess = class(TcxCustomDataRelationList);
...
function CanCallDataSetLocate(ADataSet: TDataSet; const AKeyFieldNames: string;
  ...
  begin
    ...
    begin
      AField := FindField(ADataSet.Fields, ExtractFieldName(AFieldNames, APos));
      Result := AField <> nil;
      if not Result then
        Break;
      AList.Add(AField);
...
begin
  ...
  begin
    ...
    if Pos(';', AKeyFieldNames) = 0 then
      AField := FindField(ADataSet.Fields, AKeyFieldNames);
    ...
    begin
      ...
      try
        ...
        try
          while not IsEOF do
          begin
            if DataController.CanSelectRecord(-1) and DataController.CanSelectRow(-1) then
...
function TcxDBCustomDataLink.IsDataLocate: Boolean;
begin
  Result := False;
end;

function TcxDBCustomDataLink.IsDataUpdate: Boolean;
begin
  Result := False;
end;

function TcxDBCustomDataLink.IsOtherDataControllerLocate: Boolean;
begin
  Result := IsDataLocate;
end;

function TcxDBCustomDataLink.IsOtherDataControllerUpdating: Boolean;
begin
  Result := IsDataUpdate;
end;
    ...
    begin
      ...
      try
        ...
        try
          ...
          Provider.DataController.ClearExpressionFieldValues;
...
function TcxDBDataLink.FocusControl(Field: TField): Boolean;
...
begin
  for I := 0 to AFieldList.Count - 1 do
    if AFieldList.List[I] <> nil then
      FFields.Add(AFieldList.List[I]);
end;
...
begin
  ...
  begin
    ADBDataProvider := TcxDBDataProvider(DBDataProviders.List[I]);
    if (ADBDataProvider.DataSet = ADataSet) and ADBDataProvider.FInCanInitEditing then
      Exit(True);
  end;
  Result := False;
end;
...
begin
  Result := False;
  ADataSet := DataSet;
  for I := DBDataProviders.Count - 1 downto 0 do
  begin
    ADBDataProvider := TcxDBDataProvider(DBDataProviders.List[I]);
    if (ADBDataProvider.DataSet = ADataSet) and ((ADBDataProvider.LockCount <> 0) or ADBDataProvider.FInNotify or
       (ADBDataProvider.LocateCount <> 0) and (IsKeyNavigation or DataController.IsGridMode)) then
    begin
      ...
      begin
        ...
        if not FPostLocateFlag then
          Result := not (DataController.IsGridMode and (DataController.LockGridModeNotifyCount <> 0));
      Break;
    end;
  end;
end;
...
begin
  ...
  begin
    ADBDataProvider := TcxDBDataProvider(DBDataProviders.List[I]);
    if (ADBDataProvider.DataSet = ADataSet) and
...
begin
  ...
  begin
    ADBDataProvider := TcxDBDataProvider(DBDataProviders.List[I]);
    if ADBDataProvider <> Self then
    begin
      if (ADBDataProvider.DataLink.DataSet = ADataSet) and
        ((ADBDataProvider.LockCount <> 0) or (ADBDataProvider.LocateCount <> 0) and IsKeyNavigation) then
      begin
...
begin
  Result := Assigned(DataSet) and DataSet.IsUniDirectional;
end;
...
function TcxDBDataProvider.IsLocating: Boolean;
begin
  Result := (LocateCount <> 0) and not DataController.FInCheckBrowseMode;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not FAssignedFields.IsFieldExists(AField) and not VarIsNull(GetFieldValue(AField)) then
      begin
        Result := True;
        Break;
      end;
...
procedure TcxDBDataProvider.DeleteRecords(AList: TList);
  ...
  begin
    ...
    try
      ...
      begin
        New(P);
        P^ := DataController.GetInternalRecordId(Integer(AList.List[I]), AFieldList);
        AKeyList.Add(P);
      end;
      for I := AKeyList.Count - 1 downto 0 do
      begin
        P := PVariant(AKeyList.List[I]);
  ...
  begin
    ...
    if DataController.CanFocusRecord(AAnchorRecordIndex) then
      ...
      begin
        ARecordIndex := Integer(AList.List[I]);
...
begin
  ...
  if Assigned(ADataSetField) and ADataSetField.CanModify then
  begin
...
begin
  FListeners.AddOrSetValue(AProvider, 0);
end;
...
var
  ...
  begin
    ...
    try
      ...
      Result := TdxDiacriticStringOptions.DataCompareText(S, ASubText, True, False, False);
    finally
...
begin
  ...
  begin
    ...
    DataController.StorageRecordCount := GetDataRecordCount;
    FRecordIndex := DataLink.ActiveRecord;
  end;
...
begin
  ...
  for I := 0 to DBDataProviders.Count - 1 do
  begin
    ADBDataProvider := DBDataProviders.List[I];
    if (ADBDataProvider <> Self) and
      (ADBDataProvider.DataSet = ADataSet) then
...
procedure TcxValueDefDBReader.Read(ABuffer: PAnsiChar; AValueDef: TcxValueDef);
begin
  if TcxDBDataField(AValueDef.LinkObject).IsExpression then
...
begin
  ...
  begin
    for I := FFields.Count - 1 downto 0 do
      TcxDBDataField(FFields.List[I]).Free;
    FFields.Free;
    FFields := nil;
  end;
end;
...
begin
  ...
  if UseSQLIgnoreTimeOperatorClass and (cxGetFilterSQLIgnoreTimeOperatorClass(OperatorKind) <> nil) then
    Result := cxGetFilterSQLIgnoreTimeOperatorClass(OperatorKind)
  else
  begin
...
procedure TcxDBDataFilterCriteriaItem.SetItemLink(Value: TObject);
begin
  inherited SetItemLink(Value);
  if UseSQLIgnoreTimeOperatorClass then
    RecreateOperator;
end;

function TcxDBDataFilterCriteriaItem.UseSQLIgnoreTimeOperatorClass: Boolean;
begin
  Result := Criteria.AutoDataSetFilter and (ItemLink <> nil) and DataController.CanIgnoreTimeForFiltering(ItemLink);
end;
...
function TcxDBDataFilterCriteriaItem.GetFilterCriteria: TcxDBDataFilterCriteria;
begin
  Result := TcxDBDataFilterCriteria(inherited Criteria);
end;
...
function TcxDBDataFilterCriteria.IsExpressionsSupported: Boolean;
begin
  Result := inherited IsExpressionsSupported and not IsAutoDataSetFilter;
end;
  ...
  begin
    ...
    begin
      ...
      if fcoCaseInsensitive in Options then
        Include(AFilterOptions, foCaseInsensitive)
      else
        Exclude(AFilterOptions, foCaseInsensitive);
...
function TcxDBDataController.CanIgnoreTimeForFiltering(AItemLink: TObject): Boolean;
begin
  Result := False;
end;
...
function TcxDBDataController.CanLoadData: Boolean;
  ...
  procedure Error(const E: Exception);
  begin
...
begin
  Result := inherited CanLoadData;
  if Result and not (IsGridMode or (DetailMode <> dcdmClone)) then
  begin
    AMasterRelation := TcxDBDataRelation(GetMasterRelation);
    if (AMasterRelation <> nil) and AMasterRelation.IsLinked then
    try
      Result := (AMasterRelation.MasterKeyFieldCount = MasterDetailKeyFields.Count);
    except
      on E: Exception do
      begin
        Error(E);
...
procedure TcxDBDataController.CheckEditingOnFindCriteriaChanged;
begin
  if (Provider.DataSet <> nil) and (Provider.DataSet.State in dsEditModes) then
    try
      CheckBrowseMode;
    except
      FindCriteria.Text := '';
    ...
    inherited CheckEditingOnFindCriteriaChanged;
end;
...
function TcxDBDataController.CheckDetailsBrowseMode: Boolean;
var
...
begin
  ...
  begin
    IsBrowsing := True;
    AActiveLinks := TBits.Create;
    AActiveLinks.Size := ARelationCount;
    try
      for I := 0 to RecordCount - 1 do
      begin
        ADetailObject := TcxCustomDataRelationListAccess(Relations).GetValueAsDetailObject(I);
        if ADetailObject <> nil then
        begin
          for J := 0 to ARelationCount - 1 do
          begin
            ALinkObject := ADetailObject.LinkObjects[J];
            if (ALinkObject <> nil) and not AActiveLinks[J] then
            begin
              ADataController := GetDetailDataControllerByLinkObject(ALinkObject);
              ADataController.CheckBrowseMode;
              AActiveLinks[J] := True;
            end;
          end;
          if AActiveLinks.OpenBit = ARelationCount then
            Break;
        end;
      end;
    finally
...
procedure TcxDBDataController.FindCriteriaChanged;
var
  AHardChanges: Boolean;
begin
  AHardChanges := FindCriteriaChangesData;
  inherited FindCriteriaChanged;
  if AHardChanges and Provider.IsSyncMode then
    CheckFocusedRow;
end;
...
var
  ADataSet: TDataSetAccess;
begin
  if IsBrowsing then
    Exit;
  FInCheckBrowseMode := True;
  try
    ADataSet := TDataSetAccess(DataSet);
    if ADataSet <> nil then
    begin
      if ADataSet.State <> dsInactive then
        ADataSet.DataEvent(deCheckBrowseMode, 0);
      if ADataSet.State in [dsEdit, dsInsert] then
      begin
        ADataSet.UpdateRecord;
        if Provider.IsDataSetModified {DataSet.Modified} then
        begin
          if IsSmartRefresh or IsGridMode then
            ADataSet.Post
          else
          begin
            ADataSet.DisableControls; // support AfterPost Requery
            try
              ADataSet.Post;
            finally
              ADataSet.EnableControls;
...
function TcxDBDataController.CanFocusedRecordIndexChangePostData: Boolean;
begin
  Result := inherited CanFocusedRecordIndexChangePostData and Provider.IsDataSetModified and
    (DataSet.State in dsEditModes);
end;
...
function TcxDBDataController.IsFieldSupportsExpression(AField: TcxCustomDataField): Boolean;
var
  ADBField: TcxDBDataField absolute AField;
begin
  Result := inherited IsFieldSupportsExpression(AField) and (ADBField.FieldName = '') and (ADBField.Field = nil)
end;
...
var
  AProvider: TcxDBDataProvider;
  ANeedCheckNotified: Boolean;
  ADataSet: TDataSet;
begin
  if Provider.FInNotify then Exit;
  Provider.FInNotify := True;
  try
    ANeedCheckNotified := not IsPattern and IsDetailMode and not Active;
    ADataSet := DataSet;
    for AProvider in Provider.FListeners.Keys do
    begin
      if not (ANeedCheckNotified and (ADataSet = AProvider.DataSet)) then
        AProvider.DataLink.DataSetChanged;
    end;
    Provider.FListeners.Clear;
  finally
...
function TcxDBDataController.SkipLocateNotification: Boolean;
begin
  Result := False;
end;
...
begin
  for I := 0 to Relations.Count - 1 do
    ...
    begin
      ARelation := TcxDBDataRelation(Relations[I]);
      AMasterKeyFieldNames := ARelation.MasterKeyFieldNames;
      AMasterKeyField := ARelation.FMasterKeyField;
      UpdateInternalKeyFields(AMasterKeyFieldNames, AMasterKeyField);
      ARelation.FMasterKeyField := AMasterKeyField;
    end;
end;
