...
uses
  System.UITypes,
  System.Types, Winapi.Windows, System.Classes, Vcl.Controls, Vcl.Graphics, Winapi.Messages, dxCore, cxContainer, cxControls,
  cxDropDownEdit, cxEdit, cxEditConsts, cxGraphics, cxLookAndFeels, dxBar, cxGeometry, dxActions,
  dxBarAccessibility, dxUIABase;
...
type
  ...
  private
    FAlwaysSaveDisplayValue: Boolean;
    ...
    FWithoutLeftIndents: Boolean;

    FEditValueAssigned: Boolean;
    FIsMakeTopmostMRUItemProcessing: Boolean;
    FIsStoreDisplayValueProcessing: Boolean;
    FItemIndex: Integer;
    FItemIndexAppliedToControl: Boolean;
    FItemIndexAssigned: Boolean;
    FSavedItemIndex: Integer;
    FSetItemIndexProcessCount: Integer;
  ...
  {$HINTS ON}
    ...
    function UseBarPaintingStyle(AForceDrawAsStandalone: Boolean): Boolean;
    procedure WriteEditValue(AWriter: TWriter); overload;
  {$HINTS OFF}
    procedure WriteEditValue(AStream: TStream); overload;
  {$HINTS ON}

    function ApplyItemIndexFromControl: Boolean;
    function ApplyItemIndexToControl: Boolean;
    function CanApplyItemIndexToControl: Boolean;
    function IsItemIndexProviderReady(const AProvider: IcxPropertiesItemIndexProvider): Boolean;
    function NeedSynchronizeItemIndexAndEditValue(const AProvider: IcxPropertiesItemIndexProvider; AInitiator: TcxItemIndexSynchronizationInitiator): Boolean;
    procedure SynchronizeItemIndexAndEditValue;
    function TryUpdateItemIndexByEditValue: Boolean;
    procedure UpdateItemIndexWhenEditValueChanged;
    procedure UpdateItemIndexWhenLoaded;
    procedure UpdateItemIndexWhenPropertiesChanged;

    function IsItemIndexStored(AFiler: TFiler): Boolean;
    procedure ReadItemIndex(AReader: TReader);
    procedure WriteItemIndex(AWriter: TWriter);
  protected
    procedure RestoreItemIndex;
    procedure StoreItemIndex(ATakeItemIndexFromControl: Boolean);

    function GetActiveProperties: TcxCustomEditProperties;
    function GetEditorItemIndexHandler: IcxItemIndexHandler;
    function GetPropertiesItemIndexProvider: IcxPropertiesItemIndexProvider;
    function GetActiveEditor: TcxCustomEdit;
    function CanSetEditValue(const ANewValue: TcxEditValue): Boolean;
    function ForceSaveDisplayValue: Boolean; virtual;

    procedure Loaded; override;
    procedure DefineProperties(Filer: TFiler); override;
    procedure DrawCustomizingImage(ACanvas: TCanvas; const ARect: TRect; AState: TOwnerDrawState); override;
    procedure DrawCustomizingImageContent(ACanvas: TCanvas; const ARect: TRect; ASelected: Boolean; ScaleFactor: TdxScaleFactor); override;
    ...
    property AlwaysSaveDisplayValue: Boolean read FAlwaysSaveDisplayValue write FAlwaysSaveDisplayValue default False;
    property HasBorder: Boolean read FHasBorder write FHasBorder default True;
    property IsStoreDisplayValueProcessing: Boolean read FIsStoreDisplayValueProcessing write FIsStoreDisplayValueProcessing;
    property ItemIndexAppliedToControl: Boolean read FItemIndexAppliedToControl;
    property WithoutLeftIndents: Boolean read FWithoutLeftIndents write FWithoutLeftIndents default False;
  public
  ...
  private
    ...
    FIsItemIndexSynchronized: Boolean;
    FIsItemIndexInitialized: Boolean;
    procedure ClearEditEvents;
    procedure InitializeItemIndex;
    procedure InternalShowEdit;
    procedure SaveEditEvents;
    procedure SynchronizeItemIndex(AApplyNewItemIndex: Boolean);
    ...
    procedure DoValidate(Sender: TcxCustomEdit; var DisplayValue: TcxEditValue;  //..
    ...
    function GetProperties: TcxCustomEditProperties;
    function GetWithoutLeftIndents: Boolean;
  ...
  protected
    ...
    function UseBarPaintingStyle: Boolean; inline;
    ...
    function ForceSaveDisplayValue: Boolean; override;
    ...
    function SupportsDynamicWidth(AViewSize: TdxBarItemControlViewSize): Boolean; override;
    function GetDefaultSmallWidth: Integer; override;

    procedure ActivateEdit(AByMouse: Boolean; AKey: Char = #0); override;
    procedure InitEdit; override;
    procedure InitializeEditViewInfo(AViewInfo: TcxCustomEditViewInfo);
  ...
  public
    destructor Destroy; override;
    class function GetEditParentBarControl(AViewInfo: TcxCustomEditViewInfo): TCustomdxBarControl; // for internal use
  ...
  { TdxBarEditItemControlAccessibilityHelper }

  TdxBarEditItemControlAccessibilityHelper = class(TdxBarWinControlAccessibilityHelper) // for internal use
  private
    function GetEditItem: TcxCustomBarEditItem;
    function GetEditItemControl: TcxBarEditItemControl;
  protected
    function GetControlTypePropertyValue(out AValue: Integer): Boolean; override;
    function GetDefaultAriaRole: string; override;
    function GetDefaultIsContentElement: TdxDefaultBoolean; override;
    function GetSupportedProperties: TdxUIAAccessibilityHelperProperties; override;

    property EditItem: TcxCustomBarEditItem read GetEditItem;
    property EditItemControl: TcxBarEditItemControl read GetEditItemControl;
  end;

  function dxShowPropertiesAtDesignTime(APropertiesClass: TClass): Boolean; // for internal use

implementation

uses
  System.Variants, Vcl.Forms, Vcl.StdCtrls, System.SysUtils, cxBarEditItemValueEditor, cxClasses, cxEditPaintUtils,
  cxEditRepositoryItems, cxEditUtils, cxLookAndFeelPainters, cxTextEdit, dxSpellCheckerCore, dxForms,
  cxVariants, dxBarCustomCustomizationForm, dxOffice11, dxBarStrs, dxBarSkinConsts, dxFading, System.Math,
  dxDPIAwareUtils, dxUIA;

const
  dxThisUnitName = 'cxBarEditItem';
...
type
  ...
  TcxCustomEditPropertiesClassAccess = class of TcxCustomEditPropertiesAccess;
  ...
  private
    ...
    procedure SetBarControl(AValue: TCustomdxBarControl);
  protected
    // IdxVisualRefinementsTuner
    procedure AdjustPadding(var APadding: TdxPadding);
    // IdxInplaceEditParentBackgroundProvider
    procedure IdxInplaceEditParentBackgroundProvider.DrawParentBackground=DrawEditParentBackground;
    function IdxInplaceEditParentBackgroundProvider.UseParentBackground=UseEditParentBackground;
    procedure DrawEditParentBackground(ACanvas: TcxCanvas; const AViewInfo: TcxCustomEditViewInfo);
    function UseEditParentBackground: Boolean;
  ...
  public
    ...
    property BarControl: TCustomdxBarControl read FBarControl write SetBarControl;
    property BarItemControl: TdxBarItemControl read FBarItemControl write FBarItemControl;
  end;
...
function dxShowPropertiesAtDesignTime(APropertiesClass: TClass): Boolean;
begin
  Result := dxIsDesignTime and (APropertiesClass <> nil) and APropertiesClass.InheritsFrom(TcxCustomEditProperties) and
    TcxCustomEditPropertiesClassAccess(APropertiesClass).ShowAtDesignTime;
end;
...
procedure TEditorParentForm.AdjustPadding(var APadding: TdxPadding);
begin
  if FBarControl <> nil then
  begin
    TCustomdxBarControlAccess(BarControl).AdjustVisualRefinementsPadding(APadding);
  end;
end;

procedure TEditorParentForm.DrawEditParentBackground(ACanvas: TcxCanvas; const AViewInfo: TcxCustomEditViewInfo);
var
  ASaveOrg: TPoint;
begin
  ASaveOrg := ACanvas.WindowOrg;
  ACanvas.WindowOrg := dxMapWindowPoint(Handle, BarControl.Handle, AViewInfo.Bounds.TopLeft, True);
  AViewInfo.Association := BarItemControl;
  TCustomdxBarControlAccess(BarControl).DrawEditParentBackground(ACanvas, AViewInfo);
  ACanvas.WindowOrg := ASaveOrg;
end;

function TEditorParentForm.UseEditParentBackground: Boolean;
begin
  Result := TCustomdxBarControlAccess(BarControl).UseEditParentBackground;
end;
...
begin
  ...
  begin
    ...
    if AComponent = FBarControl then
      FBarControl := nil;
  end;
end;
...
procedure TEditorParentForm.SetBarControl(AValue: TCustomdxBarControl);
begin
  if FBarControl <> AValue then
  begin
    if FBarControl <> nil then
      FBarControl.RemoveFreeNotification(Self);
    FBarControl := AValue;
    if FBarControl <> nil then
      FBarControl.FreeNotification(Self);
  end;
  if AValue <> nil then
    ParentWindow := AValue.Handle;
end;
...
var
  I, AMouseDPI: Integer;
  ABarItem: TdxBarItemLink;
  ARegisteredClasses: TcxRegisteredClasses;
begin
  AMouseDPI := dxGetMonitorDPI(GetMouseCursorPos);
  ARegisteredClasses := GetRegisteredEditProperties;
  for I := 0 to ARegisteredClasses.Count - 1 do
    if (ARegisteredClasses.Descriptions[I] <> '') and
      (not dxIsDesignTime or dxShowPropertiesAtDesignTime(ARegisteredClasses.Items[I])) then
    begin
      ABarItem := BarDesignController.AddInternalItem(AItemLinks, TdxBarButton, ARegisteredClasses.Descriptions[I], OnButtonClick);
      LoadEditorComponentIcon(ARegisteredClasses.Items[I].ClassName, ABarItem.Glyph, AMouseDPI);
    end;
end;
...
function TcxCustomBarEditItem.GetItemIndex: Integer;
begin
  if not ApplyItemIndexFromControl then
    TryUpdateItemIndexByEditValue;
  Result := FItemIndex;
end;

function TcxCustomBarEditItem.GetActiveEditor: TcxCustomEdit;
var
 ACurItemLink: TdxBarItemLink;
 AItem: TcxBarEditItemControl;
begin
  Result := nil;
  ACurItemLink := CurItemLink;
  if ACurItemLink <> nil then
  begin
    AItem := Safe<TcxBarEditItemControl>.Cast(ACurItemLink.Control);
    if AItem <> nil then
      Result := AItem.Edit;
  end;
end;

function TcxCustomBarEditItem.ForceSaveDisplayValue: Boolean;
begin
  Result := FAlwaysSaveDisplayValue;  
end;

function TcxCustomBarEditItem.GetEditorItemIndexHandler: IcxItemIndexHandler;
begin
  if not Supports(TObject(GetActiveEditor), IcxItemIndexHandler, Result) then
    Exit;
  if not Result.SupportsItemIndex then
    Result := nil;
end;

function TcxCustomBarEditItem.GetActiveProperties: TcxCustomEditProperties;
begin
  if (RepositoryItem <> nil) and (RepositoryItem.Properties <> nil) then
    Result := RepositoryItem.Properties 
  else
    Result := Properties;
end;

function TcxCustomBarEditItem.GetPropertiesItemIndexProvider: IcxPropertiesItemIndexProvider;
begin
  if not Supports(TObject(GetActiveProperties), IcxPropertiesItemIndexProvider, Result) then
    Exit;
  if not Result.SupportsItemIndex then
    Result := nil;
end;

function TcxCustomBarEditItem.TryUpdateItemIndexByEditValue: Boolean;
var
  AProvider: IcxPropertiesItemIndexProvider;
  AItemIndex: Integer;
begin
  Result := False;
  AProvider := GetPropertiesItemIndexProvider;
  if AProvider = nil then
    Exit;
  if AProvider.SupportsItemIndexSynchronization(TcxItemIndexSynchronizationInitiator.ItemIndexGetter) then
  begin
    Result := True;
    if AProvider.FindItemIndexByValue(EditValue, AItemIndex) then
...
procedure TcxCustomBarEditItem.RestoreItemIndex;
begin
  TryUpdateItemIndexByEditValue;
  FItemIndexAppliedToControl := True;
  FSavedItemIndex := FItemIndex;
  ApplyItemIndexToControl;
end;

procedure TcxCustomBarEditItem.StoreItemIndex(ATakeItemIndexFromControl: Boolean);
begin
  if ATakeItemIndexFromControl then
    ApplyItemIndexFromControl
  else
    ItemIndex := FSavedItemIndex;
  FItemIndexAppliedToControl := False;
end;

function TcxCustomBarEditItem.ApplyItemIndexFromControl: Boolean;
var
  AHandler: IcxItemIndexHandler;
begin
  if not ItemIndexAppliedToControl then
    Exit(False);
  AHandler := GetEditorItemIndexHandler;
  Result := AHandler <> nil;
  if Result then
    FItemIndex := AHandler.ItemIndex;
end;

function TcxCustomBarEditItem.ApplyItemIndexToControl: Boolean;
var
  AHandler: IcxItemIndexHandler;
begin
  if not ItemIndexAppliedToControl or not CanApplyItemIndexToControl then
    Exit(False);
  AHandler := GetEditorItemIndexHandler;
  Result := AHandler <> nil;
  if Result then
    AHandler.ItemIndex := FItemIndex;
end;

function TcxCustomBarEditItem.CanApplyItemIndexToControl: Boolean;
var
  AProvider: IcxPropertiesItemIndexProvider;
begin
  Result := False;
  AProvider := GetPropertiesItemIndexProvider;
  if AProvider <> nil then
    Result := AProvider.CanApplyItemIndexToControl(FItemIndexAssigned);
end;

function TcxCustomBarEditItem.NeedSynchronizeItemIndexAndEditValue(const AProvider: IcxPropertiesItemIndexProvider;
  AInitiator: TcxItemIndexSynchronizationInitiator): Boolean;
begin
  Result := IsItemIndexProviderReady(AProvider) and AProvider.SupportsItemIndexSynchronization(AInitiator);
end;

function TcxCustomBarEditItem.IsItemIndexProviderReady(const AProvider: IcxPropertiesItemIndexProvider): Boolean;
begin
  Result := (AProvider <> nil) and not IsLoading and AProvider.Ready;
end;

procedure TcxCustomBarEditItem.SynchronizeItemIndexAndEditValue;

  procedure SynchronizeItemIndexToEditValue(const AProvider: IcxPropertiesItemIndexProvider);
  var
    ANewValue: Variant;
  begin
    if not NeedSynchronizeItemIndexAndEditValue(AProvider, TcxItemIndexSynchronizationInitiator.ItemIndexLoader) then
    begin
      Exit;
    end;
    ...
    if FItemIndex = -1 then
      ...
      if AProvider.FindValueByItemIndex(FItemIndex, ANewValue) then
        FEditValue := ANewValue
    else
    begin
      FItemIndex := -1;
      FEditValue := Null;
  ...
  procedure SynchronizeEditValueToItemIndex(const AProvider: IcxPropertiesItemIndexProvider);
  var
    ANewItemIndex: Integer;
  begin
    if not NeedSynchronizeItemIndexAndEditValue(AProvider, TcxItemIndexSynchronizationInitiator.EditValueLoader) then
      Exit;

    if VarIsNull(FEditValue) then
      FItemIndex := -1
    else
      if AProvider.FindItemIndexByValue(FEditValue, ANewItemIndex) then
        FItemIndex := ANewItemIndex
    else
    begin
      FItemIndex := -1;
      FEditValue := Null;
...
begin
  AProvider := GetPropertiesItemIndexProvider;
  if not IsItemIndexProviderReady(AProvider) then
    Exit;
  if FItemIndexAssigned and FEditValueAssigned and
    (AProvider.SelectedItemPrioritySource = TcxSelectedItemPrioritySource.EditValue) then
      SynchronizeEditValueToItemIndex(AProvider)
  else
    if FItemIndexAssigned then
      SynchronizeItemIndexToEditValue(AProvider)
  else
    if FEditValueAssigned then
      SynchronizeEditValueToItemIndex(AProvider);
end;

procedure TcxCustomBarEditItem.UpdateItemIndexWhenLoaded;
begin
  UpdateItemIndexWhenPropertiesChanged;
end;

procedure TcxCustomBarEditItem.UpdateItemIndexWhenPropertiesChanged;

  procedure UpdateValues(const AProvider: IcxPropertiesItemIndexProvider);
  var
    ANewValue: Variant;
    ANewItemIndex: Integer;
  begin
    if not NeedSynchronizeItemIndexAndEditValue(AProvider, TcxItemIndexSynchronizationInitiator.PropertiesHandler) then
    begin
      if (FItemIndex < -1) or (FItemIndex >= AProvider.ItemCount) then
        FItemIndex := -1;
      Exit;
    end;

    if AProvider.SelectedItemPrioritySource = TcxSelectedItemPrioritySource.EditValue then
    begin
      if VarIsNull(FEditValue) then
        ItemIndex := -1
      else
      ...
      if AProvider.SelectedItemPrioritySource = TcxSelectedItemPrioritySource.ItemIndex then
      begin
        if FItemIndex = -1 then
          EditValue := Null
        else
          if AProvider.FindValueByItemIndex(FItemIndex, ANewValue) then
          begin
            if not VarEquals(EditValue, ANewValue) then
              ItemIndex := -1
          end
          else
            ItemIndex := -1;
...
var
  AProvider: IcxPropertiesItemIndexProvider;
begin
  if FIsMakeTopmostMRUItemProcessing or IsLoading or ItemIndexAppliedToControl then
    Exit;
  AProvider := GetPropertiesItemIndexProvider;
  if not IsItemIndexProviderReady(AProvider) then
    Exit;
  if AProvider.IsItemLoadingComplete then
    SynchronizeItemIndexAndEditValue
  else
    UpdateValues(AProvider);
end;

procedure TcxCustomBarEditItem.UpdateItemIndexWhenEditValueChanged;
var
  AProvider: IcxPropertiesItemIndexProvider;
begin
  if ItemIndexAppliedToControl or (FSetItemIndexProcessCount > 0) or IsStoreDisplayValueProcessing then
    Exit;
  AProvider := GetPropertiesItemIndexProvider;
  if AProvider = nil then
    Exit;
  if not NeedSynchronizeItemIndexAndEditValue(AProvider, TcxItemIndexSynchronizationInitiator.EditValueSetter) then
    Exit;
  if (VarIsSoftNull(EditValue) or not AProvider.FindItemIndexByValue(EditValue, FItemIndex)) then
    FItemIndex := -1;
end;

procedure TcxCustomBarEditItem.SetItemIndex(AValue: Integer);
var
  AOldIndex: Integer;

  function UpdateEditValue(const ANewValue: Variant): Boolean;
  begin
    Result := CanSetEditValue(ANewValue); 
    if Result then
    begin
      EditValue := ANewValue;
      CurChange;
  ...
  begin
    ...
    if AProvider = nil then
      Exit;
    if NeedSynchronizeItemIndexAndEditValue(AProvider, TcxItemIndexSynchronizationInitiator.ItemIndexSetter) then
    begin
      if AProvider.FindValueByItemIndex(FItemIndex, ANewValue) then
        UpdateEditValue(ANewValue)
      else
        if FItemIndex <> -1 then
          FItemIndex := AOldIndex 
        else
          UpdateEditValue(Null);
    end
    else
      if not InRange(FItemIndex, 0, AProvider.ItemCount - 1) and (FItemIndex <> -1) then
        FItemIndex := AOldIndex; 
  end;

begin
  Inc(FSetItemIndexProcessCount);
  try
    AOldIndex := GetItemIndex;
    if AOldIndex <> AValue then
    begin
      FItemIndex := AValue;
      FItemIndexAssigned := True;
      if not IsLoading and not ApplyItemIndexToControl then
        UpdateValues;
    end;
  finally
    Dec(FSetItemIndexProcessCount);
  end;
end;
...
begin
  ...
  Filer.DefineProperty('ItemIndex', ReadItemIndex, WriteItemIndex, IsItemIndexStored(Filer));
...
function TcxCustomBarEditItem.IsItemIndexStored(AFiler: TFiler): Boolean;
  ...
  begin
    AProvider := GetPropertiesItemIndexProvider;
    Result := (AProvider <> nil) and AProvider.CanStoreItemIndex;
  end;
...
begin
  ...
  if AFiler.Ancestor <> nil then
    Result := not (AFiler.Ancestor is TcxCustomBarEditItem) or (ItemIndex <> TcxCustomBarEditItem(AFiler.Ancestor).ItemIndex)
  else
    Result := ItemIndex <> -1;
end;

procedure TcxCustomBarEditItem.ReadItemIndex(AReader: TReader);
begin
  ItemIndex := AReader.ReadInteger;
end;

procedure TcxCustomBarEditItem.WriteItemIndex(AWriter: TWriter);
begin
  AWriter.WriteInteger(ItemIndex);
end;
...
begin
  if CaptionIsEditValue then
    dxBarCustomizingForm.ItemsPainterClass.DrawButtonOrSubItem(ACanvas, ARect, Self, GetTextOf(Caption), odSelected in AState)
  else
    dxBarCustomizingForm.ItemsPainterClass.DrawEdit(ACanvas, ARect, Self, odSelected in AState,
      UseBarPaintingStyle(not dxBarCustomizingForm.ItemsLookAndFeelPainter.SupportsEditorBorders));
end;

procedure TcxCustomBarEditItem.DrawCustomizingImageContent(ACanvas: TCanvas; const ARect: TRect; ASelected: Boolean; ScaleFactor: TdxScaleFactor);
var
...
begin
  ...
  try
    ...
    AEditViewData := AEditProperties.CreateViewData(AEditStyle, True, ScaleFactor, True);
    ...
    ADrawAsStandalone := not dxBarCustomizingForm.ItemsLookAndFeelPainter.SupportsEditorBorders; 
    AEditViewInfo.ForceDrawAsStandalone := ADrawAsStandalone;
    if UseBarPaintingStyle(ADrawAsStandalone) then
      AEditStyle.ButtonTransparency := ebtHideInactive;
    if not ADrawAsStandalone then
...
procedure TcxCustomBarEditItem.ChangeScale(M: Integer; D: Integer);
begin
  inherited;
  FHeight := MulDiv(Height, M, D);
end;
...
begin
  ...
  begin
    ...
    AProperties.DefaultValuesProvider := nil;
...
begin
  ...
  if (ACanvas <> nil) and AHandled then
    FillRectByColor(ACanvas.Handle, ARect,
      dxBarCustomizingForm.ItemsPainterClass.GetButtonColor(Self, False));
end;
...
begin
  if IsBarCompatibleEdit(Sender.Properties) then
    ADefaultWidth := dxBarCustomizingForm.ItemsPainterClass.GetComboBoxButtonWidth;
end;
...
procedure TcxCustomBarEditItem.Loaded;
begin
  inherited Loaded;
  SynchronizeItemIndexAndEditValue;
  UpdateItemIndexWhenLoaded;
end;

function TcxCustomBarEditItem.CanSetEditValue(const ANewValue: TcxEditValue): Boolean;
begin
  Result := not (GetProperties.CanCompareEditValue and (VarType(ANewValue) = VarType(FEditValue)) and
    VarEqualsExact(ANewValue, FEditValue));
end;

procedure TcxCustomBarEditItem.SetEditValue(const AValue: TcxEditValue);

  procedure MakeTopmostMRUItem;
  var
    AProvider: IcxPropertiesItemIndexProvider;
    ANewItemIndex: Integer;
  begin
    if FIsMakeTopmostMRUItemProcessing or IsLoading or
      IsStoreDisplayValueProcessing or (FSetItemIndexProcessCount = 0) then
        Exit;
    AProvider := GetPropertiesItemIndexProvider;
    if (AProvider = nil) or not AProvider.SupportsMRU then
      Exit;
    FIsMakeTopmostMRUItemProcessing := True;
    try
      if AProvider.MakeTopmostItem(EditValue, ANewItemIndex) then
        FItemIndex := ANewItemIndex;
    finally
      FIsMakeTopmostMRUItemProcessing := False;
    end;
  end;

begin
  if CanSetEditValue(AValue) then
  begin
    FEditValue := AValue;
    FEditValueAssigned := True;
    if not IsLoading then
    begin
      UpdateItemIndexWhenEditValueChanged;
      MakeTopmostMRUItem;
    end;
...
function TcxCustomBarEditItem.UseBarPaintingStyle(AForceDrawAsStandalone: Boolean): Boolean;
begin
  Result := not AForceDrawAsStandalone and BarStyleDropDownButton and IsBarCompatibleEdit;
end;
...
class function TcxBarEditItemControl.GetEditParentBarControl(AViewInfo: TcxCustomEditViewInfo): TCustomdxBarControl;
begin
  if AViewInfo.ParentBackgroundProvider is TdxBarControl then
    Exit(TdxBarControl(AViewInfo.ParentBackgroundProvider));
  if AViewInfo.ParentBackgroundProvider is TEditorParentForm then
    Exit(TEditorParentForm(AViewInfo.ParentBackgroundProvider).BarControl);
  Result := nil;
end;
...
begin
  ...
  begin
    if IsChildControl(AActiveWinControl, TCustomdxBarControlAccess(Parent).DockControl) then
      FakeWinControl.Parent := TCustomdxBarControlAccess(Parent).DockControl
    else
...
{$ENDIF}
  TcxContainer.LockFormInactivation(True);
  try
   ...
   TcxContainer.LockFormInactivation(False);
  end;
  if Edit <> nil then
...
begin
  ...
  begin
    ...
    FDrawParams.ForceDrawAsStandAlone := Painter.DrawEditAsStandalone;
  end;
end;

function TcxBarEditItemControl.UseBarPaintingStyle: Boolean;
begin
  Result := Item.UseBarPaintingStyle(Painter.DrawEditAsStandalone);
end;
...
//  Focused := False;
  ...
  try
    SynchronizeItemIndex(NeedStoreDisplayValueOnDeactivate);
...
function TcxBarEditItemControl.HasBorder: Boolean;
begin
  Result := Item.HasBorder; 
end;
...
procedure TcxBarEditItemControl.DrawTextField;
  ...
  begin
    Result := Painter.EditButtonAllowCompositeFrame and UseBarPaintingStyle;
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      EditViewInfo.ParentBackgroundProvider := Parent; 
...
function TcxBarEditItemControl.GetAccessibilityHelperClass: TdxBarAccessibilityHelperClass;
begin
  Result := TdxBarEditItemControlAccessibilityHelper;
end;

function TcxBarEditItemControl.ForceSaveDisplayValue: Boolean;
begin
  Result := Item <> nil;
  if Result then
    Result := Item.ForceSaveDisplayValue;
end;

function TcxBarEditItemControl.DrawSelected: Boolean;
begin
  Result := inherited DrawSelected or not NeedFocusOnClick and IsSelected and not FSelectedByMouse;  //..
end;
...
begin
  if Edit <> nil then
    ...
    begin
      ...
      if not TcxCustomEditAccess(Edit).IsEditValidated and Edit.ModifiedAfterEnter then
        FEditValueBeforeHiding := TcxCustomEditAccess(Edit).InternalGetEditingValue
      else
        FEditValueBeforeHiding := Edit.EditValue;
      if FIsEditValueAssigned then
        SynchronizeItemIndex(AStoreDisplayValue);
      ...
      SetWindowPos(EditorParentForm.Handle, 0, 0, 0, 0, 0,
        SWP_NOZORDER or SWP_NOREDRAW or SWP_NOACTIVATE or SWP_NOCOPYBITS or SWP_DEFERERASE);
      DisableAppWindows(not IsApplicationActive);
      try
...
begin
  ...
  InitializeEditViewInfo(Edit.ViewInfo);
...
procedure TcxBarEditItemControl.InitializeEditViewInfo(AViewInfo: TcxCustomEditViewInfo);
begin
  AssignViewInfoEvents(AViewInfo);
  AViewInfo.ForceDrawAsStandalone := Painter.DrawEditAsStandalone;
end;
...
procedure TcxBarEditItemControl.InitializeItemIndex;
begin
  if not FIsItemIndexInitialized then
  begin
    FIsItemIndexInitialized := True;
    FIsItemIndexSynchronized := False;
    Item.RestoreItemIndex;    
  end;
end;

procedure TcxBarEditItemControl.SynchronizeItemIndex(AApplyNewItemIndex: Boolean);
begin
  if FIsItemIndexInitialized and not FIsItemIndexSynchronized then
  begin
    FIsItemIndexSynchronized := True;
    FIsItemIndexInitialized := False;
    Item.StoreItemIndex(AApplyNewItemIndex);
  end;
end;

procedure TcxBarEditItemControl.StoreDisplayValue;
begin
  Item.IsStoreDisplayValueProcessing := True;
  try
    PrepareEditForClose;
    SynchronizeItemIndex(True);
    if (Edit <> nil) and FIsEditValueAssigned then
      Item.EditValue := Edit.EditValue
    else
      Item.EditValue := FEditValueBeforeHiding;
  finally
    Item.IsStoreDisplayValueProcessing := False;
  end;
end;

function TcxBarEditItemControl.SupportsDynamicWidth(AViewSize: TdxBarItemControlViewSize): Boolean;
begin
  Result := AViewSize = GetViewSize;
end;

function TcxBarEditItemControl.GetDefaultSmallWidth: Integer;
begin
  if TdxBarItemLinkAccess(ItemLink).DynamicWidth > 0 then
    Result := TdxBarItemLinkAccess(ItemLink).DynamicWidth
  else
    Result := inherited GetDefaultSmallWidth;
end;
...
var
  AIntf: IdxVisualRefinementsTuner;
begin
  Result := Properties.CreateViewData(GetEditStyle(AFull), True, ScaleFactor);
  Result.Enabled := Enabled;
  if Supports(Parent, IdxVisualRefinementsTuner, AIntf) and not Painter.LookAndFeelPainter.ApplyEditorAdvancedMode then
    Result.RefinementsTuner := AIntf;
  if AFull then
  begin
  ...
  Result.ForceDrawAsStandalone := Painter.DrawEditAsStandalone;
end;
...
begin
  ...
  EditorParentForm.BarControl := Parent;
  EditorParentForm.BoundsRect := GetBoundsRect;
  EditorParentForm.ForceScaleForPPI(ScaleFactor.Apply(dxDefaultDPI));
  EditorParentForm.BarItemControl := Self;
  Edit.Align := alClient;
...
begin
  ACanStart := ACanStart and TCustomdxBarControlAccess(Parent).IsFadingAvailable and
    not (csDestroying in Parent.ComponentState) and Painter.IsFadingAvailable;
end;
...
begin
  if UseBarPaintingStyle and (AState = ebsNormal) then
  begin
...
procedure TcxBarEditItemControl.DoValidate(Sender: TcxCustomEdit;
...
function TcxBarEditItemControl.GetIndents(ADrawAreaType: TdxBarItemControlPart): TRect;
begin
  Result := inherited GetIndents(ADrawAreaType);
  if (ADrawAreaType = TdxBarItemControlPart.cpText) and GetWithoutLeftIndents then
    Result.Left := 0;
end;
...
function TcxBarEditItemControl.GetWithoutLeftIndents: Boolean;
begin
  Result := (Item <> nil) and (Item.Caption = '') and Item.WithoutLeftIndents;
end;
...
begin
  ...
  begin
    ...
    Painter.GetEditTextParams(AParams.Offsets, AParams.SizeCorrection.cy, Painter);
  end;
...
function TdxBarEditItemControlAccessibilityHelper.GetControlTypePropertyValue(
  out AValue: Integer): Boolean;
begin
  AValue := UIA_TextControlTypeId;
  Result := True;
end;

function TdxBarEditItemControlAccessibilityHelper.GetDefaultAriaRole: string;
begin
  Result := 'text';
end;

function TdxBarEditItemControlAccessibilityHelper.GetDefaultIsContentElement: TdxDefaultBoolean;
begin
  Result := bTrue;
end;

function TdxBarEditItemControlAccessibilityHelper.GetEditItem: TcxCustomBarEditItem;
begin
  Result := Unsafe.Cast<TcxCustomBarEditItem>(Item);
end;

function TdxBarEditItemControlAccessibilityHelper.GetEditItemControl: TcxBarEditItemControl;
begin
  Result := Unsafe.Cast<TcxBarEditItemControl>(ItemControl);
end;

function TdxBarEditItemControlAccessibilityHelper.GetSupportedProperties: TdxUIAAccessibilityHelperProperties;
begin
  Result := inherited GetSupportedProperties + [TdxUIAAccessibilityHelperProperty.Value];
end;
