...
uses
  ...
  Vcl.Dialogs, System.StrUtils, System.Contnrs, dxCore, cxClasses, cxGeometry, cxGraphics, cxControls,
...
type
  ...
  protected
    ...
    property OnPageChanging: TdxTabbedMDIPageChangingEvent read FOnPageChanging write FOnPageChanging;
  public
  ...
  protected
    ...
    procedure TabPropertiesPageChangingHandler(Sender: TObject; ANewPage: TdxTabbedMDIPage; var AAllowChange: Boolean);
...
uses
  System.Math, dxOffice11, dxTabbedMDIStrs, System.Types, dxMessages, dxDPIAwareUtils, dxForms;

const
  dxThisUnitName = 'dxTabbedMDI';
...
procedure TdxTabbedMDIChild.WndProc(var Message: TMessage);
  ...
  begin
    ...
    AWasClientFocused := Winapi.Windows.GetFocus = FParent.Handle;
    Default;
    ...
    if AWasClientFocused then
      Winapi.Windows.SetFocus(FParent.Handle);
...
begin
  ...
  { //.. No añado ninguna ventana existente al inicio porque añade las de fondo y están ocultas (scheduler, etc..)
  BeginUpdate;
  try
    SubscribeChildren;
  finally
  ...
  with Message do
    case Msg of
      ...
      WM_MDICASCADE, WM_MDIICONARRANGE:
        Result := 0;
      WM_MDICREATE:
         Default;   // Los tabs se asignan de forma explícita desde los mismos TgForms ()
...
var  CurrentPageHandle : THandle;

begin
  if TabbedMDIProperties.PageIndex <> -1 then
  begin
    UpdateActiveChildBounds;
    CurrentPageHandle := TabbedMDIProperties.ActivePage.Handle; 
    If   IsWindowVisible( CurrentPageHandle )  //.. Hace que se repinte todo el control cuando hay un skin activo y no es necesario
    then SetActiveChild( CurrentPageHandle );   
  end;
end;
...
begin
     If   TabbedMDIProperties.GetPageIndexByHandle(AMDIChild)=-1  //..  Si ya existe no la añado
     then TabbedMDIProperties.AddPage(AMDIChild);
end;
...
begin
  ...
  Tab.Visible := FForm.Visible and not ( csDestroying in FForm.ComponentState ); //..
end;
...
function TdxTabbedMDITabProperties.CanChangeActivePage(ANewPage: TdxTabbedMDIPage): Boolean;
begin
  Result := True;
...
procedure TdxTabbedMDITabProperties.DoChanging(ANewTabIndex: Integer; var AAllowChange: Boolean);
begin
  inherited;
...
procedure TdxTabbedMDIManager.SubscribeChild( AMDIChild: THandle ); //
begin
  if FClientFakeControl <> nil then
    With TdxTabbedMDIClientControl(FClientFakeControl) do
      begin
      SubscribeChild( AMDIChild );
      TabbedMDIProperties.SetActivePageByHandle(AMDIChild);
      end;
end;

procedure TdxTabbedMDIManager.UnsubscribeChild( AMDIChild: THandle ); //
begin
  if FClientFakeControl <> nil then
    TdxTabbedMDIClientControl(FClientFakeControl).UnsubscribeChild( AMDIChild );
end;
...
procedure TdxTabbedMDIManager.TabPropertiesPageChangingHandler(Sender: TObject; ANewPage: TdxTabbedMDIPage; var AAllowChange: Boolean);
begin
...
finalization
  ...
  dxFreeGlobalObject(dxTabbedMDIManagers);
