...
uses
  System.Types, System.Variants, Winapi.Windows, Winapi.Messages, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.ImgList, Vcl.Forms, System.Generics.Collections, System.Generics.Defaults,
  Vcl.Buttons, Vcl.StdCtrls, Vcl.ExtCtrls, Vcl.ComCtrls, cxClasses, cxControls, cxGraphics, dxTypeHelpers,
  cxLookAndFeels, cxLookAndFeelPainters, cxStyles, cxStorage, cxPC, cxListBox,
  cxContainer, cxEdit, cxTextEdit, cxGrid, cxGridCommon, cxGridLevel, cxFilterControl, dxScrollbarAnnotations,
  cxGridCustomView, cxGridCustomTableView, cxGridDetailsSite, cxCustomData, cxImageComboBox, dxFilterPopupWindow,
  cxData, cxDataStorage, cxFilter, Vcl.Menus, dxCoreClasses, cxGridInplaceEditForm, dxUIElementPopupWindow, cxFindPanel,
  ...
  dxScrollAnimation, cxGridRowLayout, dxCoreGraphics, dxUIABase, dxUIAClasses;
...
const
  ...
  htGroupByBoxSearchButton = htGridBase + 16;
  ...
  cxGridDefaultFixedColumnSeparatorWidth = 2;
  ...
  vsRowLayoutGroup = vsTableFirst + 14; //for internal use only

  vsTableLast = vsRowLayoutGroup;
...
type
  ...
  TcxGridGroupByBoxSearchButtonViewInfo = class;
  ...
  TcxGridTableRowLayoutContainer = class;
  TcxGridTableRowLayoutController = class;

  TcxGridColumnContainerKind = Integer;

  TcxGridColumnFixedKind = (fkNone, fkLeft, fkRight, fkLeftDynamic);
  ...
  TcxGridGroupByBoxSearchButtonHitTest = class(TcxCustomGridViewHitTest)
  protected
  ...
  TcxGridRowLayoutCustomDrawEvent = procedure(Sender: TcxGridTableView; ACanvas: TcxCanvas;
    AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean) of object;

  TcxGridRowLayoutOptions = class(TcxGridCustomRowLayoutOptions)
  strict private
    FSmartCellNavigation: Boolean;
    FOnCustomDrawRowBackground: TcxGridRowLayoutCustomDrawEvent;
    FOnCustomDrawRowHotTrack: TcxGridRowLayoutCustomDrawEvent;
    FOnCustomDrawRowSelection: TcxGridRowLayoutCustomDrawEvent;

    function GetGridView: TcxGridTableView;
    procedure SetOnCustomDrawRowBackground(AValue: TcxGridRowLayoutCustomDrawEvent);
    procedure SetOnCustomDrawRowHotTrack(AValue: TcxGridRowLayoutCustomDrawEvent);
    procedure SetOnCustomDrawRowSelection(AValue: TcxGridRowLayoutCustomDrawEvent);
  protected
    procedure ActiveChanged; override;
    procedure DoAssign(Source: TPersistent); override;
    procedure DoCustomDrawRowBackground(ACanvas: TcxCanvas; AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean); virtual;
    procedure DoCustomDrawRowHotTrack(ACanvas: TcxCanvas; AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean); virtual;
    procedure DoCustomDrawRowSelection(ACanvas: TcxCanvas; AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean); virtual;
    function GetLayoutController: TcxGridRowLayoutController; override;
    function HasCustomDrawRowHotTrack: Boolean; virtual;
    function HasCustomDrawRowSelection: Boolean; virtual;
    procedure UseDefaultLayoutChanged; override;
  public
    property GridView: TcxGridTableView read GetGridView;
  published
    property Active;
    property CaptionSuffix;
    property CellBorders;
    property DefaultColumnCount;
    property DefaultStretch;
    property MinValueWidth;
    property SmartCellNavigation: Boolean read FSmartCellNavigation write FSmartCellNavigation default False;
    property UseDefaultLayout;
    property OnCustomDrawRowBackground: TcxGridRowLayoutCustomDrawEvent read FOnCustomDrawRowBackground write SetOnCustomDrawRowBackground;
    property OnCustomDrawRowHotTrack: TcxGridRowLayoutCustomDrawEvent read FOnCustomDrawRowHotTrack write SetOnCustomDrawRowHotTrack;
    property OnCustomDrawRowSelection: TcxGridRowLayoutCustomDrawEvent read FOnCustomDrawRowSelection write SetOnCustomDrawRowSelection;
  end;
  TcxGridRowLayoutOptionsClass = class of TcxGridRowLayoutOptions;

  TcxGridTableRowLayoutDataCellViewInfo = class(TcxGridRowLayoutDataCellViewInfo)
  strict private
  ...
  protected
    function AlwaysDrawSelectedWhenFocused: Boolean; override;
    function CanAutoHeight: Boolean; override;
    function CanFocus: Boolean; override;
    function CanDrawSelectedWhenEditing: Boolean; override;
    ...
    function HasHotTrackBackground: Boolean; override;
    function HasSelectionBackground: Boolean; override;
    procedure InitEditSizeProperties(out AEditSizeProperties: TcxEditSizeProperties); override;
    function IsValueTransparent: Boolean; override;
  ...
  TcxGridTableRowLayoutContainerViewInfo = class(TcxGridRowLayoutContainerViewInfo)
  strict private
    function GetContainer: TcxGridTableRowLayoutContainer;
  protected
    function CanDrawRoot: Boolean; override;
    function FindGridItemViewInfo(AViewInfo: TcxGridCustomLayoutItemViewInfo): TcxGridViewLayoutItemDataCellViewInfo; override;
  public
    property Container: TcxGridTableRowLayoutContainer read GetContainer;
  end;

  TcxGridTableRowLayoutItem = class(TcxGridRowLayoutItem);
  TcxGridTableRowLayoutGroup = class(TdxLayoutGroup);

  TcxGridTableRowLayoutContainer = class(TcxGridRowLayoutContainer)
  strict private
    function GetGridView: TcxGridTableView;
    function GetLayoutController: TcxGridTableRowLayoutController;
  protected
    function CanCreateLayoutItem(AGridItem: TcxCustomGridTableItem): Boolean; override;
    function CanPlaceItem(AItem: TcxGridRowLayoutItem): Boolean; override;
    function GetActualViewInfo: TcxGridLayoutContainerViewInfo; virtual;
    ...
    procedure SetItemForGridItem(AGridItem: TcxCustomGridTableItem; AItem: TcxGridRowLayoutItem); override;

    property GridView: TcxGridTableView read GetGridView;
    property LayoutController: TcxGridTableRowLayoutController read GetLayoutController;
  end;

  TcxGridTableRowLayoutSerializationOwner = class(TcxComponent,
    IdxLayoutContainerOwner)
  strict private
    FContainer: TcxGridTableRowLayoutContainer;

    //IdxLayoutContainerOwner
    function GetContainer: TdxLayoutContainer;
  protected
    procedure GetChildren(Proc: TGetChildProc; Root: TComponent); override;
    procedure SetContainer(AValue: TcxGridTableRowLayoutContainer);
    procedure SetParentComponent(Value: TComponent); override;
  end;

  TcxGridTableRowLayoutCustomizationController = class(TcxGridRowLayoutCustomizationController)
  protected
    function CreateForm: TForm; override;
  end;

  TcxGridTableRowLayoutController = class(TcxGridRowLayoutController)
  strict private
    FContainerClientRect: TRect;
    FDefaultContainerViewInfo: TcxGridTableRowLayoutContainerViewInfo;
    FIsDefaultContainerCalculated: Boolean;
    FSerializationOwner: TcxGridTableRowLayoutSerializationOwner;

    function GetContainer: TcxGridTableRowLayoutContainer;
    function GetContainerDefaultHeight: Integer; virtual;
    function GetContainerDefaultWidth: Integer; virtual;
    function GetGridView: TcxGridTableView;
    function GetOptions: TcxGridRowLayoutOptions;
  protected
    procedure CalculateDefaultContainer(AWidth, AHeight: Integer); virtual;
    function CreateCustomizationController: TcxGridRowLayoutCustomizationController; override;
    function CreateDefaultContainerViewInfo: TcxGridTableRowLayoutContainerViewInfo; virtual;
    function CreateSerializationOwner: TcxGridTableRowLayoutSerializationOwner; virtual;
    function GetContainerClass: TcxGridRowLayoutContainerClass; override;
    function GetContainerHeight(AViewInfo: TcxGridTableRowLayoutContainerViewInfo): Integer; virtual;
    function GetContainerWidth(AViewInfo: TcxGridTableRowLayoutContainerViewInfo): Integer; virtual;
    function GetOptionsInstance: TcxGridCustomRowLayoutOptions; override;
    procedure SetSerializationOwner(AValue: TcxGridTableRowLayoutSerializationOwner); virtual;
    procedure Store(Proc: TGetChildProc); override;
    procedure UpdateLayoutLookAndFeel; override;

    property ContainerClientRect: TRect read FContainerClientRect;
    property DefaultContainerViewInfo: TcxGridTableRowLayoutContainerViewInfo read FDefaultContainerViewInfo;
    property IsDefaultContainerCalculated: Boolean read FIsDefaultContainerCalculated;
  public
    constructor Create(AGridView: TcxCustomGridTableView); reintroduce; virtual;
    destructor Destroy; override;

    procedure CalculateContainer(AViewInfo: TcxGridTableRowLayoutContainerViewInfo; AWidth, AHeight: Integer); virtual;
    function CalculateContainerAutoHeight(AViewInfo: TcxGridTableRowLayoutContainerViewInfo; AWidth: Integer): Integer; virtual;
    procedure CopyCustomizationSetting(AContainer: TdxLayoutContainer); override;
    procedure Init; override;
    procedure InvalidateDefaultContainer; virtual;

    property Container: TcxGridTableRowLayoutContainer read GetContainer;
    property ContainerDefaultHeight: Integer read GetContainerDefaultHeight;
    property ContainerDefaultWidth: Integer read GetContainerDefaultWidth;
    property GridView: TcxGridTableView read GetGridView;
    property Options: TcxGridRowLayoutOptions read GetOptions;
  end;
  TcxGridTableRowLayoutControllerClass = class of TcxGridTableRowLayoutController;
  ...
  private
    ...
    function GetItem: TcxGridColumn;
  protected
    function CalculateHotTracked: Boolean; override;
    ...
    function GetLayoutItemViewInfoInstance: TcxGridCustomLayoutItemViewInfo; override;
    function HasBorders: Boolean; override;
    function HotTrackSelection: Boolean; override;
    function InvalidateOnStateChange: Boolean; override;
    function RecalculateOnStateChange: Boolean; override;
  public
  ...
  TcxGridTableViewInplaceEditFormCustomizationController = class(TcxGridRowLayoutCustomizationController)
  protected
    function CreateForm: TForm; override;
  end;
  ...
  protected
    function CreateCustomizationController: TcxGridRowLayoutCustomizationController; override;
    procedure DetachedFormInitialize; override;
    procedure Edit(ARow: TcxGridDataRow); virtual;
    function GetContainerClass: TcxGridRowLayoutContainerClass; override;
    procedure InvalidateIndicator; virtual;
    procedure InvalidateUpdateButton; virtual;
    function GetOptionsInstance: TcxGridCustomRowLayoutOptions; override;
  ...
  TcxGridEditFormOptions = class(TcxGridCustomEditFormOptions)
  strict private
  ...
  public
    property GridView: TcxGridTableView read GetGridView;
  published
    property CaptionMask;
    property CellBorders;
    property DefaultColumnCount;
    property DefaultStretch;
    property ItemHotTrack;
    property MasterRowDblClickAction: TcxGridMasterRowDblClickAction read FMasterRowDblClickAction
      write SetMasterRowDblClickAction default dcaSwitchExpandedState;
    property UseDefaultLayout;
  end;
  TcxGridEditFormOptionsClass = class of TcxGridEditFormOptions;
  ...
  protected
    // UI Automation
    function CreateUIAProvider: IdxUIAProvider; override;
    function SupportsUIAutomation: Boolean; override;
    procedure UIANotifyRowsChanged; override;
    //inplace edit form
    ...
    function GetParentRecord: TcxCustomGridRecord; override;
  ...
  protected
    function GetIndicatorKind: TcxIndicatorKind; override;
    function HasErrorData: Boolean; override;
  ...
  protected
    ...
    function GetInternalActiveDetailGridView(out AView: TcxCustomGridView): Boolean;
    function GetViewInfoCacheItemClass: TcxCustomGridViewInfoCacheItemClass; override;
    function GetViewInfoClass: TcxCustomGridRecordViewInfoClass; override;
    procedure KeyDown(var Key: Word; Shift: TShiftState); override;
  ...
  private
    function GetCaptionPrefix(AIndex: Integer): string;
  ...
  protected
    ...
    function GetDisplayText(Index: Integer): string; override;
    function GetDisplayTextByGroupedColumn(AGroupedColumnIndex: Integer): string; virtual;
    ...
    property CaptionPrefix[AGroupedColumnIndex: Integer]: string read GetCaptionPrefix;
    property CaptionSeparator: string read GetCaptionSeparator;
    property DisplayTextByGroupedColumn[AGroupedColumnIndex: Integer]: string read GetDisplayTextByGroupedColumn;
  public
    function ExpandOnDblClick: Boolean; override;
  ...
  protected
    ...
    function GetDestZonesClientBounds: TRect; virtual;
    function GetGroupByBoxArrowAreaBounds: TRect; virtual;
    function GetHeaderArrowAreaBounds(APlace: TcxGridArrowPlace): TRect; virtual;
    function GetHeaderArrowAreaBoundsForLeftFixedSource: TRect; virtual;
    function GetHeaderArrowAreaBoundsForRightFixedSource: TRect; virtual;
    function GetHeaderArrowAreaBoundsForScrollableSource: TRect; virtual;
    function GetHeaderLeftFixedArrowAreaBounds: TRect; virtual;
    function GetHeaderRightFixedArrowAreaBounds: TRect; virtual;
    ...
    function IsValidDestinationFixedKind: Boolean; virtual;
    ...
    function NeedCheckScrolling: Boolean; override;
  ...
  TcxGridTableFindPanel = class(TcxGridFindPanel)
  strict private
    function GetGridView: TcxGridTableView;
  protected
    function GetActualLayout: TcxFindPanelLayout; override;
    function IsFocusingContentKey(Key: Word; Shift: TShiftState): Boolean; override;
  public
    procedure DisplayModeChanged; override;
  ...
  strict private
    ...
    FHasImmediateFilterValue: Boolean;
    function GetController: TcxGridTableController;
    function GetGridView: TcxGridTableView;
  protected
    ...
    property HasImmediateFilterValue: Boolean read FHasImmediateFilterValue write FHasImmediateFilterValue;
  public
    destructor Destroy; override;
  ...
  TcxGridTableCustomizationController = class(TcxGridCustomizationController)
  strict private
    function GetViewInfo: TcxGridTableViewInfo;
  protected
  ...
  TcxGridFocusedItemKind = (fikNone, fikGridItem, fikUpdateButton, fikCancelButton); //for internal use only
  TcxGridGetNextItemFunc = function(AItems: TList; AFocusedIndex: Integer; AGoForward, AGoOnCycle: Boolean; out ACycleChanged: Boolean): Integer of object; //for internal use only

  TcxGridTableColumnScrollAnimationHelper = class(TdxScrollAnimationHelper)
  strict private
    FOwner: TcxGridTableController;
  protected
  ...
  public
    constructor Create(AOwner: TcxGridTableController);
  end;

  TcxGridTableController = class(TcxCustomGridTableController)
  private
    FCellSelectionAnchor: TcxGridColumn;
    FColumnScrollAnimationHelper: TcxGridTableColumnScrollAnimationHelper;
    ...
    FLeftPos: Integer;
    FPostponedAdjustableRow: TcxCustomGridRow;
    ...
    function GetFindPanel: TcxGridTableFindPanel;
    ...
    procedure DoSetLeftPos(AValue: Integer);
    procedure MakeMasterRecordVisible(ARecord: TcxCustomGridRecord);
    procedure RemoveSelectedColumn(AColumn: TcxGridColumn);
  protected
    procedure AdjustOffsetForFixedGroupMode(ARow: TcxCustomGridRow); virtual;
    procedure AdjustRowVisibility(ARow: TcxCustomGridRow); virtual;
    ...
    function CanMakeItemVisible(AItem: TcxCustomGridColumn): Boolean;
    function CanRowLayoutSmartCellNavigate: Boolean; virtual;
    ...
    function CreateFindPanel: TcxGridFindPanel; override;
    ...
    function GetActuallyFocusedItemIndex: Integer; override;
    ...
    function GetLastVisibleRecordIndex: Integer; override;
    ...
    function IsScrollAnimationActive: Boolean; override;
    ...
    procedure ScrollAnimationEnded; override;
    ...
    procedure StopScrollAnimation; override;
    ...
    procedure SetLeftPosition(AValue: Integer; AAnimated: Boolean);
    ...
    //layout item focusing
    function CanCloseInplaceEditForm: Boolean;
    procedure CheckFocusedItem(AItemViewInfo: TcxGridTableViewInplaceEditFormDataCellViewInfo);
    function CloseInplaceEditFormOnRecordInserting: Boolean;
    function FocusNextEditFormItem(AGoForward: Boolean): Boolean; overload;
    function FocusNextEditFormItem(AFocusedItemIndex: Integer; AGoForward: Boolean): Boolean; overload;
    function FocusNextRowLayoutItem(AFocusedItemIndex: Integer; AGoForward, AGoOnCycle: Boolean; out ACycleChanged: Boolean): Boolean;
    function GetNextEditFormItemIndexFunc(AItems: TList; AFocusedIndex: Integer; AGoForward, AGoOnCycle: Boolean; out ACycleChanged: Boolean): Integer;
    function GetNextInplaceButton: TcxGridFocusedItemKind;
    function GetNextRowLayoutItemIndexFunc(AItems: TList; AFocusedIndex: Integer; AGoForward, AGoOnCycle: Boolean; out ACycleChanged: Boolean): Integer;
    function GetNextLayoutItemIndex(AContainerViewInfo: TcxGridLayoutContainerViewInfo;
      AFocusedIndex: Integer; AGoForward, AGoOnCycle: Boolean; AFunc: TcxGridGetNextItemFunc; out ACycleChanged: Boolean): Integer;
    function RowLayoutSmartCellNavigate(AFocusedItem: TcxCustomGridColumn; ADirection: TcxNeighbor; AStartNewCycle: Boolean): Boolean;
    ...
    // customization
    function CreateCustomizationController: TcxCustomGridCustomizationController; override;
    ...
    property FindPanel: TcxGridTableFindPanel read GetFindPanel;
  ...
  public
    ...
    procedure ShowRowLayoutCustomizationDialog;
  ...
  strict private
    function GetViewInfo: TcxGridHeaderViewInfo;
  protected
    procedure DrawItems; override;
    procedure DrawItemsByFixedKind(AKind: TcxGridColumnFixedKind); virtual;
    function DrawItemsFirst: Boolean; override;

    property ViewInfo: TcxGridHeaderViewInfo read GetViewInfo;
  end;

  // group by box

  TcxGridGroupByBoxSearchButtonPainter = class(TcxCustomGridCellPainter)
  strict private
    function GetViewInfo: TcxGridGroupByBoxSearchButtonViewInfo;
  protected
    procedure DrawContent; override;

    property ViewInfo: TcxGridGroupByBoxSearchButtonViewInfo read GetViewInfo;
  end;
  ...
  TcxGridFooterPainter = class(TcxGridHeaderPainter)
  private
    FLeftFixedRegion: TcxRegion;
    FRightFixedRegion: TcxRegion;
  ...
  protected
    ...
    procedure DrawBorders; override;
    procedure DrawItems; override;
    procedure DrawItemsByFixedKind(AKind: TcxGridColumnFixedKind); override;
    function DrawItemsFirst: Boolean; override;
    procedure DrawSeparator; virtual;
    procedure ExcludeLeftFixedRegion; virtual;
    procedure ExcludeRightFixedRegion; virtual;
  ...
  protected
    class procedure DrawDataRowCellsByFixedKind(ARowViewInfo: TcxCustomGridRowViewInfo; AKind: TcxGridColumnFixedKind); virtual;
    procedure Paint; override;
  ...
  protected
    ...
    procedure RepaintOnOffset; override;
  public
    constructor Create(AGridView: TcxCustomGridView); override;
    destructor Destroy; override;

    procedure AddGridLine(const R: TRect);
    procedure DrawFixedHighlight(ABounds: TRect); //for internal use only
    procedure DrawFixedSeparator(ABounds: TRect; AIsRight: Boolean; AIsHeader: Boolean); virtual; //for internal use only
  ...
  TcxGridTableFindPanelLocation = (fplSeparatePanel, fplGroupByBox);

  TcxGridTableFindPanelViewInfo = class(TcxGridFindPanelViewInfo)
  strict private
    FLocation: TcxGridTableFindPanelLocation;
    FWidthInGroupByBox: Integer;
    FWidthInGroupByBoxAssigned: Boolean;

    function GetGridView: TcxGridTableView;
    function GetGridViewInfo: TcxGridTableViewInfo;
    function GetWidthInGroupByBox: Integer;
  protected
    function CalculateBounds: TRect; override;
    function CalculateWidth: Integer; override;
    function CalculateWidthInGroupByBox: Integer; virtual;
    function GetButtonsOffset: Integer; override;
    function GetFirstOffset: Integer; override;
    function GetIsPart: Boolean; override;
    function GetLocation: TcxGridTableFindPanelLocation; virtual;
    function GetVisible: Boolean; override;
    function HasBackground: Boolean; override;
    function HasBorders: Boolean; override;

    property Location: TcxGridTableFindPanelLocation read FLocation;
    property WidthInGroupByBox: Integer read GetWidthInGroupByBox;
  public
  ...
  private
    FIndex: TDictionary<TcxCustomGridColumn, TcxGridColumnHeaderViewInfo>;
    FItemHeight: Integer;
    ...
    function GetGridView: TcxGridTableView;
    function GetGridViewInfo: TcxGridTableViewInfo;
    function GetInternalItem(Index: Integer): TcxGridColumnHeaderViewInfo; inline;
    function GetItem(Index: Integer): TcxGridColumnHeaderViewInfo; inline;
  ...
  protected
    ...
    function GetColumnIndentWidth(AColumn: TcxGridColumn): Integer; virtual;
    ...
    function GetZone(const P: TPoint; ATrackingBounds: TRect; AFirst, ALast: Integer): TcxGridItemContainerZone; overload; virtual;
    function GetZonesTrackingBounds: TRect; virtual;
    ...
    property Controller: TcxGridTableController read GetController;
    property ZonesTrackingBounds: TRect read GetZonesTrackingBounds;
  public
    constructor Create(AGridViewInfo: TcxCustomGridTableViewInfo); override;
    destructor Destroy; override;
    procedure BeforeRecalculation; override;
    function GetHitTest(const P: TPoint): TcxCustomGridHitTest; override;
    function GetZone(const P: TPoint): TcxGridItemContainerZone; overload; virtual;
  ...
  public
    ...
    property ColorPalette: IdxColorPalette read GetColorPalette;
  ...
  private
    ...
    FHiddenBorders: TcxBorders;
    FIsDynamicFixed: Boolean;
  ...
  protected
    ...
    function EmulateMouseMoveAfterCalculate: Boolean; override;
    function FindArea(AAreaClass: TcxGridColumnHeaderAreaViewInfoClass; ACompareExactClassTypes: Boolean = True): TcxGridColumnHeaderAreaViewInfo;
    procedure FixDynamically; virtual;
    ...
    class function GetCellBorderWidth(ALookAndFeelPainter: TcxCustomLookAndFeelPainter;
      AScaleFactor, ABordersScaleFactor: TdxScaleFactor;
      ABorder: TcxBorder): Integer; virtual;
    class function GetCellHeight(ATextHeight: Integer; ALookAndFeelPainter: TcxCustomLookAndFeelPainter;
      AScaleFactor, ABordersScaleFactor: TdxScaleFactor): Integer; override;
    function GetCheckBoxAreaWidth: Integer; virtual;
    function GetContentIndents: TRect; virtual;
    ...
    function ExcludeLeftBorder: Boolean; virtual;
    function ExcludeRightBorder: Boolean; virtual;
    // design
    ...
    property HiddenBorders: TcxBorders read FHiddenBorders;
  ...
  public
    ...
    property IsFixed: Boolean read GetIsFixedByWidth;
  ...
  private
    FSeparatorsBounds: array[TcxGridColumnFixedKind] of TRect;
    FDynamicFixedItems: TList;
    FSpecific: TcxGridHeaderViewInfoSpecific;

    function GetDynamicFixedItemCount: Integer;
    function GetDynamicFixedItem(AIndex: Integer): TcxGridColumnHeaderViewInfo;
    function GetFirstItem(AFixedKind: TcxGridColumnFixedKind): TcxGridColumnHeaderViewInfo;
    function GetFirstItemIndex(AFixedKind: TcxGridColumnFixedKind): Integer;
    function GetItemCount(AFixedKind: TcxGridColumnFixedKind): Integer;
    function GetLastItem(AFixedKind: TcxGridColumnFixedKind): TcxGridColumnHeaderViewInfo;
    function GetLastItemIndex(AFixedKind: TcxGridColumnFixedKind): Integer;
    function GetSeparatorBounds(AFixedKind: TcxGridColumnFixedKind): TRect;
    function GetSeparatorActualWidth(AFixedKind: TcxGridColumnFixedKind): Integer; virtual;
  protected
    function GetColumn(Index: Integer): TcxGridColumn; override;
    function GetColumnCount: Integer; override;

    procedure AddDynamicFixedItem(AItem: TcxGridColumnHeaderViewInfo); virtual;
    ...
    procedure CalculateDynamicFixedItems(ALeft, ATop, ACheckedLeft: Integer); virtual;
    function CalculateHeight: Integer; override;
    procedure CalculateInvisible; override;
    function CalculateItemHeight: Integer; override;
    procedure CalculateItems; virtual;
    procedure CalculateLeftFixedItems(var ALeft: Integer; ATop: Integer); virtual;
    procedure CalculateRightFixedItems(var ARight: Integer; ATop: Integer); virtual;
    procedure CalculateScrollableItems(ALeft, ATop: Integer); virtual;
    procedure CalculateSeparatorBounds(AFixedKind: TcxGridColumnFixedKind; var ALeft: Integer; ATop, AWidth: Integer); virtual;
    ...
    function GetDynamicFixedAreaLastBound: Integer; virtual;
    function GetHitTestClass: TcxCustomGridHitTestClass; override;
    ...
    function GetItemAreaBoundsByFixedKind(AKind: TcxGridColumnFixedKind): TRect; virtual;
    ...
    function GetItemsHitTestByFixedKind(const P: TPoint; AKind: TcxGridColumnFixedKind): TcxCustomGridHitTest; virtual;
    function GetKind: TcxGridColumnContainerKind; override;
    function GetLeftFixedAreaLastBound: Integer; virtual;
    function GetPainterClass: TcxCustomGridCellPainterClass; override;
    function GetRightFixedAreaFirstBound: Integer; virtual;
    function GetViewInfoSpecificClass: TcxGridHeaderViewInfoSpecificClass; virtual;
    procedure GetViewParams(var AParams: TcxViewParams); override;
    function GetVisible: Boolean; override;
    function GetWidth: Integer; override;
    function GetZoneByFixedKind(const P: TPoint; AKind: TcxGridColumnFixedKind): TcxGridItemContainerZone; virtual;
    function GetZonesClientBoundsByFixedKind(AKind: TcxGridColumnFixedKind): TRect; virtual;
    function GetZonesTrackingBounds: TRect; override;
    function GetZonesTrackingBoundsByFixedKind(AKind: TcxGridColumnFixedKind): TRect; virtual;
    function IsAlwaysVisibleForCalculation: Boolean; virtual;
    function IsColumnLeft(AColumn: TcxCustomGridColumn): Boolean; virtual;
    function IsHeightAssigned: Boolean; virtual;
    procedure Offset(DX, DY: Integer); override;
    procedure RecalculateItemVisibles; virtual;
    procedure ResetSeparatorsBounds; virtual;

    property ColumnBackgroundBitmap: TBitmap read GetColumnBackgroundBitmap;
    property DynamicFixedItemCount: Integer read GetDynamicFixedItemCount;
    property DynamicFixedItems[AIndex: Integer]: TcxGridColumnHeaderViewInfo read GetDynamicFixedItem;
    property FirstItem[AFixedKind: TcxGridColumnFixedKind]: TcxGridColumnHeaderViewInfo read GetFirstItem;
    property FirstItemIndex[AFixedKind: TcxGridColumnFixedKind]: Integer read GetFirstItemIndex;
    property ItemCount[AFixedKind: TcxGridColumnFixedKind]: Integer read GetItemCount;
    property LastItem[AFixedKind: TcxGridColumnFixedKind]: TcxGridColumnHeaderViewInfo read GetLastItem;
    property LastItemIndex[AFixedKind: TcxGridColumnFixedKind]: Integer read GetLastItemIndex;
    property SeparatorActualWidth[AFixedKind: TcxGridColumnFixedKind]: Integer read GetSeparatorActualWidth;
    property SeparatorBounds[AFixedKind: TcxGridColumnFixedKind]: TRect read GetSeparatorBounds;
  public
    constructor Create(AGridViewInfo: TcxCustomGridTableViewInfo); override;
    destructor Destroy; override;
  ...
  TcxGridGroupByBoxSearchButtonViewInfo = class(TcxGridPartCustomCellViewInfo)
  ...
  public
    constructor Create(AGroupByBoxViewInfo: TcxGridGroupByBoxViewInfo); reintroduce; virtual;
  ...
  private
    FColumnWidthCalculatingCount: Integer;
    FIsSearchButtonVisible: Boolean;
    FIsSearchButtonVisibleAssigned: Boolean;
    FSearchButtonViewInfo: TcxGridGroupByBoxSearchButtonViewInfo;

    function GetGroupByBoxVerOffset: Integer;
    function GetIsSearchButtonVisible: Boolean;
  ...
  protected
    ...
    function CalculateFitElementBounds(AElementHeight, AElementWidth: Integer): TRect; virtual;
    function CalculateHeaderAreaHeight(ALevelCount: Integer): Integer; virtual;
    function CalculateHeight: Integer; override;
    function CalculateItemHeight: Integer; override;
    function CalculateMinHeight: Integer; virtual;
    function CalculateWidth: Integer; override;
    function CanFitElement(AElementWidth: Integer): Boolean; virtual;
    function CanShowFindPanel: Boolean; virtual;
    function CanShowSearchButton: Boolean; virtual;
    function CreateSearchButtonViewInfo: TcxGridGroupByBoxSearchButtonViewInfo; virtual;
    function GetAlignment: TcxGridPartAlignment; override;
    function GetAlignmentVert: TcxAlignmentVert; override;
    function GetBackgroundBitmap: TBitmap; override;
    ...
    procedure GetViewParams(var AParams: TcxViewParams); override;
    function GetVisible: Boolean; override;
    function IsColumnWidthCalculating: Boolean; virtual;
    function IsSingleLine: Boolean; virtual;

    property GroupByBoxVerOffset: Integer read GetGroupByBoxVerOffset;
    property IsSearchButtonVisible: Boolean read GetIsSearchButtonVisible;
    property SearchButtonViewInfo: TcxGridGroupByBoxSearchButtonViewInfo read FSearchButtonViewInfo;
  public
    constructor Create(AGridViewInfo: TcxCustomGridTableViewInfo); override;
    destructor Destroy; override;
  ...
  protected
    ...
    function CalculateWidth: Integer; override;
    procedure CalculateTextAreaBounds; override;
    ...
    function GetBorderWidth(AIndex: TcxBorder): Integer; override;
    class function GetCellBorderWidth(ALookAndFeelPainter: TcxCustomLookAndFeelPainter;
      AScaleFactor, ABordersScaleFactor: TdxScaleFactor; ABorder: TcxBorder): Integer; override;
    class function GetCellHeight(ATextHeight: Integer; ALookAndFeelPainter: TcxCustomLookAndFeelPainter;
      AScaleFactor, ABordersScaleFactor: TdxScaleFactor): Integer; override;
    function GetContentIndents: TRect; override;
    function GetHitTestClass: TcxCustomGridHitTestClass; override;
    function GetIsDesignSelected: Boolean; override;
  ...
  TcxGridFooterViewInfoSpecificClass = class of TcxGridFooterViewInfoSpecific; // for internal use

  TcxGridFooterViewInfoSpecific = class(TcxGridHeaderViewInfoSpecific) // for internal use
  private
    function GetFooterContainerViewInfo: TcxGridFooterViewInfo;
  protected
    function CalcCellHeight(AIndex, ACellHeight: Integer): Integer; virtual;
    function CalcCellWidth(Column: TcxGridColumn; ACellWidth: Integer): Integer; virtual;
  public
    constructor Create(AContainerViewInfo: TcxGridHeaderViewInfo); override;
    function GetVerticalSpaceBetweenItems: Integer; virtual;
    function GetHorizontalSpaceBetweenItems: Integer; virtual;
    property ContainerViewInfo: TcxGridFooterViewInfo read GetFooterContainerViewInfo;
  end;
  ...
  private
    ...
    FFooterPanelContentOffsets: TRect;
    FAllBordersSizeRect: TRect;

    function GetCellData(AIndex: Integer): TcxGridFooterCellData;
    function GetMultipleSummaries: Boolean;
    function GetRowCount: Integer;
    function GetRowHeight: Integer;
    function GetSpecific: TcxGridFooterViewInfoSpecific;
  protected
    ...
    function GetBorderWidth(AIndex: TcxBorder): Integer; override;
    function GetColumnWidth(AColumn: TcxGridColumn): Integer; override;
    function GetDynamicFixedAreaLastBound: Integer; override;
    function GetHitTestClass: TcxCustomGridHitTestClass; override;
    function GetIsAutoWidth: Boolean; override;
    function GetIsScrollable: Boolean; override;
    function GetItemAreaBounds(AItem: TcxGridColumnHeaderViewInfo): TRect; override;
    ...
    function GetKind: TcxGridColumnContainerKind; override;
    function GetLeftFixedAreaLastBound: Integer; override;
    function GetPainterClass: TcxCustomGridCellPainterClass; override;
    function GetRightFixedAreaFirstBound: Integer; override;
    ...
    function GetViewInfoSpecificClass: TcxGridHeaderViewInfoSpecificClass; override;
    procedure GetViewParams(var AParams: TcxViewParams); override;
    function GetVisible: Boolean; override;
    function HasSeparator: Boolean; virtual;
    function IsAlwaysVisibleForCalculation: Boolean; override;
    function IsColumnLeft(AColumn: TcxCustomGridColumn): Boolean; override;
  ...
  public
    ...
    property AllBordersSizeRect: TRect read FAllBordersSizeRect; // for internal use
    property BordersBounds: TRect read GetBordersBounds;
    property FooterPanelContentOffsets: TRect read FFooterPanelContentOffsets; // for internal use
    ...
    property Specific: TcxGridFooterViewInfoSpecific read GetSpecific; // for internal use
  ...
  protected
    ...
    function GetVisible: Boolean; override;
    function GetWidth: Integer; override;
    procedure MoveItem(ACurIndex, ANewIndex: Integer); virtual;
  ...
  public
    ...
    property GridView: TcxGridTableView read GetGridView;
    property GridViewInfo: TcxGridTableViewInfo read GetGridViewInfo;
    property IndicatorAreaWidth: Integer read GetIndicatorAreaWidth; // for internal use
  ...
  protected
    ...
    function GetDataRelativeIndent: Integer; virtual;
    ...
    function IsSpecial: Boolean; virtual;
    procedure MarkAsOverlapped; virtual;
    ...
    property CheckBoxState: TcxCheckBoxState read GetCheckBoxState;
    property Controller: TcxGridTableController read GetController;
    property DataRelativeIndent: Integer read GetDataRelativeIndent;
  ...
  private
    ...
    FRestHeight: Integer;
    FRowHeight: Integer;
    FSpecialDataRowHeight: Integer;
    FSpecialRowHeight: Integer;
    ...
    procedure CheckOverlapRows(AFixedGroups: TdxFastList);
    procedure CorrectFixedGroupIndicatorsOrder(AFixedGroups: TdxFastList);
    procedure PopulateFixedGroups(AFixedGroups: TdxFastList);
    procedure PostFixedGroups(AFixedGroups: TdxFastList);
    procedure RecalculateFixedGroups(AFixedGroups: TdxFastList);
  ...
  protected
    ...
    function CalculateDataRowHeight(AConsideLayout: Boolean = True): Integer; virtual;
    function CalculateSpecialDataRowHeight: Integer; virtual;
    function CalculateSpecialRowHeight: Integer; virtual;
    ...
    function CalculateRowDefaultHeight(AConsideLayout: Boolean = True): Integer; virtual;
    function CalculateRowHeight(AConsideLayout: Boolean = True): Integer; virtual;
    ...
    function GetCellAreaBounds(ACellViewInfo: TcxGridTableDataCellViewInfo): TRect; override;
    function GetCommonDataRowHeight: Integer; virtual;
    function GetDataRowCellsHitTestByFixedKind(const P: TPoint; ARowViewInfo: TcxCustomGridRowViewInfo; AKind: TcxGridColumnFixedKind): TcxCustomGridHitTest; virtual;
    ...
    function GetNextNonOverlappedIndex(AIndex: Integer): Integer; virtual;
    function GetPainterClass: TcxCustomGridRecordsPainterClass; override;
    function GetRowFocusRectBounds(ARowViewInfo: TcxCustomGridRowViewInfo): TRect; virtual;
  ...
  public
    ...
    function GetDataRowCellsHitTest(const P: TPoint; ARowViewInfo: TcxCustomGridRowViewInfo): TcxCustomGridHitTest; virtual; //for internal use only
    ...
    property SeparatorWidth: Integer read GetSeparatorWidth;
    property SpecialDataRowHeight: Integer read FSpecialDataRowHeight; //for internal use only
    property SpecialRowHeight: Integer read FSpecialRowHeight; //for internal use only
  end;
  ...
  private
    ...
    FForceIncludeRightmostBorder: Boolean;
    ...
    function GetFindPanelViewInfo: TcxGridTableFindPanelViewInfo;
    function GetFixedSeparatorWidth: Integer;
    function GetForceIncludeRightmostBorder: Boolean;
    function GetGridView: TcxGridTableView; inline;
    function GetGridLines: TcxGridLines;
  ...
  protected
    ...
    function CreateFindPanelViewInfo: TcxGridFindPanelViewInfo; override;
    function DoGetHitTest(const P: TPoint): TcxCustomGridHitTest; override;
    function GetBoundsForMakeItemVisible(AColumn: TcxCustomGridColumn): TRect; virtual;
    ...
    function GetFixedHighlightColor: TdxAlphaColor; virtual;
    function GetFixedHighlightColorMaxAlpha: Byte; virtual;
    function GetFixedSeparatorActualWidth(ANeighborBorderWidth: Integer): Integer; virtual;
    function GetFixedSeparatorColor: TColor; virtual;
    ...
    function IsCellPartVisible(ACell: TcxGridTableDataCellViewInfo): Boolean; override;
    function NeedIncludeVerticalScrollBarAreaInScrollContent: Boolean; virtual;
    procedure Offset(DX, DY: Integer); override;
    ...
    function GetFooterViewInfoSpecificClass: TcxGridFooterViewInfoSpecificClass; virtual;
    ...
    property Controller: TcxGridTableController read GetController;
    property FixedHighlightColor: TdxAlphaColor read GetFixedHighlightColor;
    property FixedSeparatorColor: TColor read GetFixedSeparatorColor;
  ...
  public
    ...
    function GetCellLineIndex(AIndex: Integer): Integer; virtual;
    ...
    property FindPanelViewInfo: TcxGridTableFindPanelViewInfo read GetFindPanelViewInfo;
    ...
    property ForceIncludeRightmostBorder: Boolean read FForceIncludeRightmostBorder; // for internal use
  ...
  private
    ...
    function GetGridView: TcxGridTableView; inline;
    function GetItem: TcxCustomGridColumn; inline;
  ...
  published
    ...
    property FilteringFilteredItemsListShowFilteredItemsOnly;
  ...
  private
    ...
    function GetItem: TcxGridColumn; inline;
  ...
  private
    ...
    FFixedKind: TcxGridColumnFixedKind;
    ...
    FRowLayoutItem: TcxGridTableRowLayoutItem;
    FSelected: Boolean;
    FSummary: TcxGridColumnSummary;
    FVisibleForEditForm: TdxDefaultBoolean;
    FVisibleForRowLayout: TdxDefaultBoolean;
    ...
    procedure SetFixedKind(Value: TcxGridColumnFixedKind);
    ...
    procedure SetRowLayoutItem(const Value: TcxGridTableRowLayoutItem);
    procedure SetVisibleForEditForm(AValue: TdxDefaultBoolean);
    procedure SetVisibleForRowLayout(AValue: TdxDefaultBoolean);
  ...
  protected
    ...
    procedure CreateLayoutItem; virtual;
    procedure CreateRowLayoutItem; virtual;
    procedure DestroyLayoutItem; virtual;
    procedure DestroyRowLayoutItem; virtual;

    procedure AssignColumnWidths; virtual;
    function CanCellMerging: Boolean; virtual;
    function CanCreateLayoutItem: Boolean;
    function CanCreateRowLayoutItem: Boolean;
    ...
    function CanSearchInGroupRows: Boolean; virtual;
    ...
    procedure CheckAccessibilityForRowLayout;
    procedure CheckUsingInFindCriteria; override;
    ...
    function IsDefaultVisibleForEditForm: Boolean;
    function IsDefaultVisibleForRowLayout: Boolean;
    ...
    function IsRowLayoutItemStored: Boolean; virtual;
    function IsSearchable: Boolean; override;
    ...
    function IsVisibleForFocus(ARecord: TcxCustomGridRecord): Boolean; override;
    function IsVisibleForRowLayout: Boolean;
    function NeedFixedHighlight: Boolean; virtual;
    ...
    function TryResetFocusToNext: Boolean; override;
  ...
  public
    ...
    property IsPreview: Boolean read FIsPreview;
  ...
  published
    ...
    property FixedKind: TcxGridColumnFixedKind read FFixedKind write SetFixedKind default fkNone;
    ...
    property RowLayoutItem: TcxGridTableRowLayoutItem read FRowLayoutItem write SetRowLayoutItem stored IsRowLayoutItemStored;
    property SortIndex;
    property SortOrder;
    ...
    property VisibleForRowLayout: TdxDefaultBoolean read FVisibleForRowLayout write SetVisibleForRowLayout default bDefault;
    property Width;
  end;
  ...
  protected
    ...
    procedure ChangeGrouping(AGroupIndex: Integer; AMergeWithLeftColumn: Boolean = False;
      AMergeWithRightColumn: Boolean = False); override;
    function GetIsFixedByWidth: Boolean; override;
    function GetOptionsClass: TcxCustomGridTableItemOptionsClass; override;
  ...
  TcxGridTableFindPanelOptions = class(TcxGridFindPanelOptions)
  private
    FLocation: TcxGridTableFindPanelLocation;
    FSearchInGroupRows: Boolean;
    FSearchInPreview: Boolean;

    function GetGridView: TcxGridTableView;
    procedure SetLocation(const AValue: TcxGridTableFindPanelLocation);
    procedure SetSearchInGroupRows(AValue: Boolean);
    procedure SetSearchInPreview(AValue: Boolean);
  public
    procedure Assign(Source: TPersistent); override;

    property GridView: TcxGridTableView read GetGridView;
  published
    property Location: TcxGridTableFindPanelLocation read FLocation write SetLocation default fplSeparatePanel;
    property SearchInGroupRows: Boolean read FSearchInGroupRows write SetSearchInGroupRows default False;
    property SearchInPreview: Boolean read FSearchInPreview write SetSearchInPreview default False;
  end;
  ...
  TcxGridEditMode = (emInplace, emInplaceEditForm, emInplaceEditFormHideCurrentRow, emModalEditForm);
  ...
  private
    ...
    function GetColumnFilteredItemsListShowFilteredItemsOnly: Boolean;
    ...
    procedure SetColumnFilteredItemsListShowFilteredItemsOnly(AValue: Boolean);
  ...
  published
    ...
    property ColumnFilteredItemsListShowFilteredItemsOnly: Boolean read GetColumnFilteredItemsListShowFilteredItemsOnly
      write SetColumnFilteredItemsListShowFilteredItemsOnly default True;
  ...
  private
    ...
    function GetColumnExpressionEditing: Boolean;
    ...
    function GetGridView: TcxGridTableView;
    procedure SetColumnExpressionEditing(Value: Boolean);
  ...
  public
    ...
    property GridView: TcxGridTableView read GetGridView;
  published
    property ColumnExpressionEditing: Boolean read GetColumnExpressionEditing write SetColumnExpressionEditing default False;
  ...
  private
    ...
    FFixedColumnHighlightColor: TdxAlphaColor;
    FFixedColumnSeparatorColor: TColor;
    FFixedColumnSeparatorWidth: Integer;
    FFooter: Boolean;
    ...
    procedure SetFixedColumnHighlightColor(Value: TdxAlphaColor);
    procedure SetFixedColumnSeparatorColor(Value: TColor);
    procedure SetFixedColumnSeparatorWidth(Value: Integer);
    ...
    procedure SetHighlightFixedColumns(AValue: Boolean);
  ...
  published
    ...
    property FixedColumnHighlightColor: TdxAlphaColor read FFixedColumnHighlightColor write SetFixedColumnHighlightColor default TdxAlphaColors.Default;
    property FixedColumnSeparatorColor: TColor read FFixedColumnSeparatorColor write SetFixedColumnSeparatorColor default clDefault;
    property FixedColumnSeparatorWidth: Integer read FFixedColumnSeparatorWidth write SetFixedColumnSeparatorWidth default cxGridDefaultFixedColumnSeparatorWidth;
    ...
    property HighlightFixedColumns: Boolean read FHighlightFixedColumns write SetHighlightFixedColumns default False;
  ...
  private
    FIsInplaceEditFormItemSelectionParams: Boolean;
  ...
  protected
    procedure GetDefaultViewParams(Index: Integer; AData: TObject; out AParams: TcxViewParams); override;
    procedure GetRowLayoutGroupParams(out AParams: TcxViewParams); virtual;
    procedure GetRowLayoutItemParams(ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem;
      out AParams: TcxViewParams; AUseViewInfo: Boolean = False; ACellViewInfo: TcxGridTableCellViewInfo = nil); virtual;
    function IsOddEvenStylesSupported: Boolean; override;

    property IsInplaceEditFormItemSelectionParams: Boolean read FIsInplaceEditFormItemSelectionParams;
  public
    procedure Assign(Source: TPersistent); override;
  ...
  TdxGridTableViewAutomationSettings = class(TdxAutomationElementSettings)
  private
    FOnInitializeRowProperties: TdxGridInitializeRowAutomationPropertiesEvent;
    FOnCalculateRowProperty: TdxGridCalculateRowAutomationPropertyEvent;
  protected
    procedure AssignTo(ADest: TPersistent); override;
  public
    procedure DoCalculateRowAutomationProperty(ARow: TcxGridDataRow; AProperty: TdxAutomationProperty; AProperties: TdxAutomationProperties); // for internal use
    procedure DoInitializeRowAutomationProperties(ARow: TcxGridDataRow; AProperties: TdxAutomationProperties); // for internal use
    function IsAutomationUnitNeededInUses: Boolean; override; // for internal use
  published
    property OnCalculateRowProperty: TdxGridCalculateRowAutomationPropertyEvent read FOnCalculateRowProperty write FOnCalculateRowProperty;
    property OnInitializeRowProperties: TdxGridInitializeRowAutomationPropertiesEvent read FOnInitializeRowProperties write FOnInitializeRowProperties;
  end;
  ...
  private
    ...
    FDynamicFixedColumnCount: Integer;
    ...
    FLastAutoWidthSizableColumnVisibleIndex: Integer;
    FNewItemRow: TcxGridNewItemRowOptions;
    FPreview: TcxGridPreview;
    FRowLayout: TcxGridRowLayoutOptions;
    FRowLayoutController: TcxGridTableRowLayoutController;
    FVisibleDynamicFixedColumnCount: Integer;
    FVisibleLeftFixedColumnCount: Integer;
    FVisibleRightFixedColumnCount: Integer;
    ...
    FOnDetachedEditFormInitialize: TcxGridTableViewDetachedEditFormEvent;
    ...
    function GetAutomation: TdxGridTableViewAutomationSettings;
    ...
    function GetFindPanel: TcxGridTableFindPanelOptions;
    ...
    function GetVisibleColumnCountByFixedKind(AKind: TcxGridColumnFixedKind): Integer;
    function GetVisibleFixedColumnCount: Integer;
    function GetVisibleScrollableColumnCount: Integer;
    procedure SetAutomation(const AValue: TdxGridTableViewAutomationSettings);
    ...
    procedure SetOnDetachedEditFormInitialize(Value: TcxGridTableViewDetachedEditFormEvent);
  ...
  protected
    // UI Automation
    function CreateUIAProvider: IdxUIAProvider; override;
    function GetAutomationSettingsClass: TdxAutomationElementSettingsClass; override;
    function SupportsUIAutomation: Boolean; override;
    procedure UIANotify(ANotification: TdxUIAGridNotification); override;
    // IcxStoredObject
    function GetProperties(AProperties: TStrings): Boolean; override;
    procedure GetPropertyValue(const AName: string; var AValue: Variant); override;
    procedure SetPropertyValue(const AName: string; const AValue: Variant); override;
    procedure GetStoredChildren(AChildren: TStringList); override;
    function GetDataViewStateStorageClass: TCustomGridViewStorageClass; override;
    ...
    procedure CheckVisibleDynamicFixedColumnCount; virtual;
    procedure ColumnFixedKindChanged(AKind, APrevKind: TcxGridColumnFixedKind); virtual;
    function CreateFindPanel: TcxGridFindPanelOptions; override;
    ...
    procedure FocusedItemChanged(APrevFocusedItem, AFocusedItem: TcxCustomGridTableItem); override;
    function GetLastAutoWidthSizableColumnVisibleIndex: Integer; virtual;
    function GetInplaceEditFormClientBounds: TRect; virtual;
    ...
    function HasColumnAutoWidth: Boolean; virtual;
    function HasInplaceEditForm: Boolean; virtual;
    function HasRowLayoutController: Boolean; virtual;
    procedure Init; override;
    ...
    function IsInplaceEditFormVisible: Boolean; virtual;
    ...
    procedure NavigatorEditExecute; override;
    function NeedChangeLayoutOnSelectionChanged(AInfo: TcxSelectionChangedInfo): Boolean; override;
    function NeedFocusFirstItemAfterInit: Boolean; override;
    ...
    procedure ReorderVisibleItemListByFixedKind; virtual;
    procedure ChangeScale(M, D: Integer); override;
    procedure SetName(const NewName: TComponentName); override;
    function SupportsColumnFixing: Boolean; virtual;
    function SupportsHotTrack: Boolean; override;
    ...
    function GetEditFormClass: TcxGridEditFormOptionsClass; virtual;
    ...
    function GetRowLayoutClass: TcxGridRowLayoutOptionsClass; virtual;
    function GetRowLayoutControllerClass: TcxGridTableRowLayoutControllerClass; virtual;
    ...
    procedure DoCustomDrawRowLayoutRowBackground(ACanvas: TcxCanvas; AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean); virtual;
    procedure DoCustomDrawRowLayoutRowHotTrack(ACanvas: TcxCanvas; AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean); virtual;
    procedure DoCustomDrawRowLayoutRowSelection(ACanvas: TcxCanvas; AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean); virtual;
    procedure DoDetachedEditFormInitialize(AForm: TForm); virtual;
    ...
    function HasCustomDrawRowLayoutRowHotTrack: Boolean;
    function HasCustomDrawRowLayoutRowSelection: Boolean;
    ...
    property InplaceEditForm: TcxGridTableViewInplaceEditForm read GetInplaceEditForm;
    property LastAutoWidthSizableColumnVisibleIndex: Integer read FLastAutoWidthSizableColumnVisibleIndex;
    property RowLayoutController: TcxGridTableRowLayoutController read GetRowLayoutController;
    property VisibleFixedColumnCount: Integer read GetVisibleFixedColumnCount;
    property VisibleScrollableColumnCount: Integer read GetVisibleScrollableColumnCount;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure GetChildren(Proc: TGetChildProc; Root: TComponent); override;
    ...
    property VisibleColumnCountByFixedKind[AKind: TcxGridColumnFixedKind]: Integer read GetVisibleColumnCountByFixedKind;
    property VisibleColumns[Index: Integer]: TcxGridColumn read GetVisibleColumn;
  published
    property Automation: TdxGridTableViewAutomationSettings read GetAutomation write SetAutomation;
    ...
    property EditForm: TcxGridEditFormOptions read FEditForm write SetEditForm;
    ...
    property FindPanel: TcxGridTableFindPanelOptions read GetFindPanel write SetFindPanel;
    ...
    property RowLayout: TcxGridRowLayoutOptions read FRowLayout write SetRowLayout;
    property RowLayoutEvents: TNotifyEvent read FSubClassEvents write FSubClassEvents;
    ...
    property OnDetachedEditFormInitialize: TcxGridTableViewDetachedEditFormEvent read FOnDetachedEditFormInitialize
      write SetOnDetachedEditFormInitialize;
...
uses
  System.RTLConsts, System.SysUtils, System.Math, System.Contnrs, Vcl.Dialogs, dxOffice11, cxVariants, cxDataUtils, dxXMLDoc, cxDBData,
  cxGridTableRowLayoutCustomizationForm, cxGridTableViewInplaceEditFormCustomizationForm,
  cxFilterControlUtils, cxLibraryConsts, cxGridRows, cxGridStrs, cxNavigator, dxDPIAwareUtils,
  dxUIAGridProvider, dxThreading;

const
  dxThisUnitName = 'cxGridTableView';
...
type
  TcxGridCheckBoxViewInfoAccess = class(TcxGridCheckBoxViewInfo);
  TcxGridSiteAccess = class(TcxGridSite);
  TcxCustomGridAccess = class(TcxCustomGrid);
  TcxGridDataRowViewInfoAccess = class(TcxGridDataRowViewInfo);
  TcxGridDataCellViewInfoAccess = class(TcxGridDataCellViewInfo);
  TcxGridTableCellViewInfoAccess = class(TcxGridTableCellViewInfo);
  TcxControlMainScrollbarsHelperAccess = class(TcxControlMainScrollbarsHelper);

  TcxCustomGridTableViewAccess_ = class (TcxCustomGridTableView);

  { TGridTableViewStorage }

  TGridTableViewStorage = class(TCustomGridViewStorage)
  private type
    TcxTableDataViewState =  class;

    TRecordInfo = class
    public
      FOptions: TcxGridStorageOptions;
      IsData: Boolean;
      IsSelected: Boolean;
      IsFocused: Boolean;
      IsExpanded: Boolean;
      Level: Integer;
      FKeyValues: Variant;
      FViewInfos: TList<TcxTableDataViewState>;

      constructor Create(AOptions: TcxGridStorageOptions);
      destructor Destroy; override;

      function  FindViewRecord(AView: TcxCustomGridTableView): TcxCustomGridRecord;
      procedure SaveRecordInfo(ARecord: TcxCustomGridRecord; ATableView: TcxCustomGridTableView);

      procedure LoadFromNode(ANode: TdxXMLNode);
      procedure SaveToNode(AParentNode: TdxXMLNode);
    end;
    ...
    private
      ...
      FRecordInfos: TObjectList<TRecordInfo>;

      function FindViewRecord(AView: TcxCustomGridTableView; ARecordInfo: TRecordInfo): TcxCustomGridRecord;
    public
      constructor Create(AOptions: TcxGridStorageOptions);
      destructor Destroy; override;

      function  RestoreFocus(AView: TcxCustomGridTableView): Boolean;
      procedure RestoreTableView(AView: TcxCustomGridTableView);
      procedure RestoreTopRecord(AView: TcxCustomGridTableView);
      procedure SaveTableView(AView: TcxCustomGridTableView);
  ...
  protected
    function GetTableView: TcxCustomGridTableView;
    procedure RestoreStateFromNode(AParentNode: TdxXMLNode); override;
    procedure SaveStateToNode(AParentNode: TdxXMLNode); override;

    property TableView: TcxCustomGridTableView read GetTableView;
  end;

{ TGridTableViewStorage }

function TGridTableViewStorage.GetTableView: TcxCustomGridTableView;
begin
  Result := TcxCustomGridTableView(GridView);
end;

procedure TGridTableViewStorage.RestoreStateFromNode(AParentNode: TdxXMLNode);
var
  ADataViewState: TcxTableDataViewState;
  AGrid: TcxGrid;
begin
  ADataViewState := TcxTableDataViewState.Create(Options);
  try
    ADataViewState.LoadFromNode(AParentNode);
    AGrid := nil;
    Safe.Cast(TableView.Control, AGrid);
    if AGrid <> nil then
      AGrid.BeginUpdate;
    try
      ADataViewState.RestoreTableView(TableView);
    finally
      if AGrid <> nil then
        AGrid.EndUpdate;
    end;
    if gsoFocusedView in Options then
      ADataViewState.RestoreFocus(TableView);
    if gsoTopRecord in Options then
      ADataViewState.RestoreTopRecord(TableView);
  finally
    ADataViewState.Free;
  end;
end;

procedure TGridTableViewStorage.SaveStateToNode(AParentNode: TdxXMLNode);
var
  ADataViewState: TcxTableDataViewState;
begin
  ADataViewState := TcxTableDataViewState.Create(Options);
  try
    ADataViewState.SaveTableView(TableView);
    ADataViewState.SaveToNode(AParentNode);
  finally
    ADataViewState.Free;
  end;
end;
...
begin
  ...
  begin
    ...
    if not Safe.Cast(AParentGroup.ParentRecord, AParentGroup) then
      Break;
  end;
  Result := AValue;
end;

{ TGridTableViewStorage.TcxTableDataViewState }

constructor TGridTableViewStorage.TcxTableDataViewState.Create(AOptions: TcxGridStorageOptions);
begin
  FRecordInfos := TObjectList<TRecordInfo>.Create(True);
  FOptions := AOptions;
end;

destructor TGridTableViewStorage.TcxTableDataViewState.Destroy;
begin
  FreeAndNil(FRecordInfos);
  inherited Destroy;
end;

function TGridTableViewStorage.TcxTableDataViewState.FindViewRecord(AView: TcxCustomGridTableView;
  ARecordInfo: TRecordInfo): TcxCustomGridRecord;
var
  AKeyValue: Variant;
...
begin
  Result := nil;
  AKeyValue := ARecordInfo.FKeyValues;
  ...
  begin
    ...
    for I := 0 to AView.ViewData.RecordCount - 1 do
      if Safe.Cast(AView.ViewData.Records[I], AGroupRow)
        and (AGroupRow.Level = ARecordInfo.Level) and (GetGroupRowKeyValue(AGroupRow) = ARecordInfo.FKeyValues) then
      begin
...
procedure TGridTableViewStorage.TcxTableDataViewState.LoadFromNode(ANode: TdxXMLNode);
var
  I: Integer;
  ARecordInfo: TRecordInfo;
begin
  FFocusedItemIndex := ANode.Attributes.GetValueAsInteger('FocusedItemIndex');
  FIsFocused := ANode.Attributes.GetValueAsBoolean('IsFocused');
  FTopRecordIndex := ANode.Attributes.GetValueAsInteger('TopRecordIndex');

  for I := 0 to ANode.Count - 1 do
  begin
    ARecordInfo := TRecordInfo.Create(FOptions);
    FRecordInfos.Add(ARecordInfo);
    ARecordInfo.LoadFromNode(ANode.Items[I]);
  end;
end;

function TGridTableViewStorage.TcxTableDataViewState.RestoreFocus(AView: TcxCustomGridTableView): Boolean;
var
  I, J: Integer;
...
begin
  ...
  if FIsFocused then
  begin
    if not TcxCustomGridTableViewAccess_(AView).CanFocus then
      TcxGridLevel(AView.Level).MakeVisible;
    AView.Focused := True;
    Result := True;
  end;
  for I := 0 to FRecordInfos.Count - 1 do
  begin
    ARecordInfo := FRecordInfos[I];
    ARecord := FindViewRecord(AView, ARecordInfo);
    if ARecord <> nil then
    begin
      if (gsoFocusedRecord in FOptions) and ARecordInfo.IsFocused then
      begin
        ARecord.Focused := (gsoFocusedRecord in FOptions) and ARecordInfo.IsFocused;
        ARecord.Selected := (gsoSelected in FOptions) and ARecordInfo.IsSelected;
      end;
      if not FIsFocused then
        for J := 0 to ARecordInfo.FViewInfos.Count - 1 do
          if Safe.Cast(TcxGridMasterDataRow(ARecord).DetailGridViews[J], ADetailView) then
          begin
            Result := ARecordInfo.FViewInfos[J].RestoreFocus(ADetailView);
            if Result then
              Break;
          end;
    end;
...
procedure TGridTableViewStorage.TcxTableDataViewState.RestoreTableView(AView: TcxCustomGridTableView);
var
  I, J: Integer;
  ARecord: TcxCustomGridRecord;
  ARecordInfo: TRecordInfo;
...
begin
  ...
  begin
    ARecordInfo := FRecordInfos[I];
    ARecord := ARecordInfo.FindViewRecord(AView);
    if ARecord <> nil then
    begin
      if (gsoExpanded in FOptions) and ARecordInfo.IsExpanded then
        ARecord.Expand(False)
      else
        ARecord.Collapse(True);
      ARecord.Focused := (gsoFocusedRecord in FOptions) and ARecordInfo.IsFocused;
      ARecord.Selected := (gsoSelected in FOptions) and ARecordInfo.IsSelected;
      if gsoDetail in FOptions then
        for J := 0 to ARecordInfo.FViewInfos.Count - 1 do
          if Safe.Cast(TcxGridMasterDataRow(ARecord).DetailGridViews[J], TcxCustomGridTableView, ADetailView) then
            ARecordInfo.FViewInfos[J].RestoreTableView(ADetailView);
    end;
  end;
  if (gsoFocusedItem in FOptions) then
    AView.Controller.FocusedItemIndex := FFocusedItemIndex;
end;

procedure TGridTableViewStorage.TcxTableDataViewState.RestoreTopRecord(AView: TcxCustomGridTableView);
var
  I, J: Integer;
  ARecord: TcxCustomGridRecord;
  ARecordInfo: TRecordInfo;
...
begin
  if gsoDetail in FOptions then
    ...
    begin
      ARecordInfo := FRecordInfos[I];
      if ARecordInfo.FViewInfos.Count > 0 then
      begin
        ARecord := FindViewRecord(AView, ARecordInfo);
        if (ARecord <> nil) then
          for J := 0 to ARecordInfo.FViewInfos.Count - 1 do
            if Safe.Cast(TcxGridMasterDataRow(ARecord).DetailGridViews[J], ADetailView) then
              ARecordInfo.FViewInfos[J].RestoreTopRecord(ADetailView);
      end;
    end;
  AView.Controller.TopRecordIndex := FTopRecordIndex;
end;

procedure TGridTableViewStorage.TcxTableDataViewState.SaveTableView(AView: TcxCustomGridTableView);
var
  I: Integer;
  ARecordInfo: TRecordInfo;
  ARecord: TcxCustomGridRecord;
  AMasterData: TcxGridMasterDataRow;
  ANeedToSave: Boolean;
begin
  FTopRecordIndex := AView.Controller.TopRecordIndex;
  FIsFocused := AView.Focused;
  FFocusedItemIndex := AView.Controller.FocusedItemIndex;
  for I := 0 to AView.ViewData.RecordCount - 1 do
  begin
    ARecord := AView.ViewData.Records[I];
    ANeedToSave := ARecord.Selected or ARecord.Expanded or ARecord.Focused;
    if ANeedToSave or
      (Safe.Cast(ARecord, TcxGridMasterDataRow, AMasterData) and AMasterData.ActiveDetailGridViewExists) then
    begin 
      ARecordInfo := TRecordInfo.Create(FOptions);
      ARecordInfo.SaveRecordInfo(ARecord, AView);
      if ANeedToSave or (ARecordInfo.FViewInfos.Count > 0) then
        FRecordInfos.Add(ARecordInfo)
      else
        ARecordInfo.Free;
    end;
...
procedure TGridTableViewStorage.TcxTableDataViewState.SaveToNode(AParentNode: TdxXMLNode);
var
  ANode: TdxXMLNode;
  I: Integer;
begin
  ANode := AParentNode.AddChild('View');
  ANode.Attributes.SetValueAsInteger('FocusedItemIndex', FFocusedItemIndex);
  ANode.Attributes.SetValueAsString('IsFocused', TdxXMLHelper.EncodeBoolean(FIsFocused));
  ANode.Attributes.SetValueAsInteger('TopRecordIndex', FTopRecordIndex);
  for I := 0 to FRecordInfos.Count - 1 do
    FRecordInfos[I].SaveToNode(ANode);
end;

{ TGridTableViewStorage.TRecordInfo }

constructor TGridTableViewStorage.TRecordInfo.Create(AOptions: TcxGridStorageOptions);
begin
  FViewInfos := TList<TcxTableDataViewState>.Create;
  FOptions := AOptions;
end;

destructor TGridTableViewStorage.TRecordInfo.Destroy;
begin
  FreeAndNil(FViewInfos);
  inherited Destroy;
end;

function TGridTableViewStorage.TRecordInfo.FindViewRecord(AView: TcxCustomGridTableView): TcxCustomGridRecord;
var
  ARecordIndex: Integer;
  I: Integer;
  AGroupRow: TcxGridGroupRow;
begin
  ...
  begin
    if AView.DataController is TcxDBDataController then
      ARecordIndex := TcxDBDataController(AView.DataController).FindRecordIndexByKey(FKeyValues)
    else
      ARecordIndex := FKeyValues;
    Result := AView.ViewData.GetRecordByRecordIndex(ARecordIndex);
  end
  else
    for I := 0 to AView.ViewData.RecordCount - 1 do
      if Safe.Cast(AView.ViewData.Records[I], AGroupRow)
        and (AGroupRow.Level = Level)
        and (GetGroupRowKeyValue(AGroupRow) = FKeyValues) then
      begin
        Result := AView.ViewData.Records[I];
        Break;
      end;
...
procedure TGridTableViewStorage.TRecordInfo.LoadFromNode(ANode: TdxXMLNode);
var
  I: Integer;
  AInfo: TcxTableDataViewState;
  AVarType: Word;
begin
  AVarType := ANode.Attributes.GetValueAsInteger('KeyValueType');
  FKeyValues := VarAsType(ANode.Attributes.GetValue('KeyValue'), AVarType);
  IsData := ANode.Attributes.GetValueAsBoolean('IsData');
  IsSelected := ANode.Attributes.GetValueAsBoolean('IsSelected');
  IsFocused := ANode.Attributes.GetValueAsBoolean('IsFocused');
  IsExpanded := ANode.Attributes.GetValueAsBoolean('IsExpanded');
  Level := ANode.Attributes.GetValueAsInteger('Level');

  for I := 0 to ANode.Count - 1 do
  begin
    AInfo := TcxTableDataViewState.Create(FOptions);
    FViewInfos.Add(AInfo);
    AInfo.LoadFromNode(ANode.Items[I]);
  end;
end;

procedure TGridTableViewStorage.TRecordInfo.SaveRecordInfo(ARecord: TcxCustomGridRecord; ATableView: TcxCustomGridTableView);
var
  I: Integer;
  ADetailViewInfo: TcxTableDataViewState;
  ADetailView: TcxCustomGridTableView;
begin
  Level := ARecord.Level;
  IsSelected := ARecord.Selected;
  IsFocused := ARecord.Focused;
  IsExpanded := ARecord.Expanded;
  IsData := ARecord.IsData;
  if IsData then
  begin
    if ATableView.DataController is TcxDBDataController then
      FKeyValues := ATableView.DataController.GetRecordId(ARecord.RecordIndex)
    else
      FKeyValues := ARecord.RecordIndex;
    if ARecord.Expanded and (ARecord is TcxGridMasterDataRow) and
      TcxGridMasterDataRow(ARecord).ActiveDetailGridViewExists then
    begin
      for I := 0 to TcxGridMasterDataRow(ARecord).DetailGridViewCount - 1 do
        if Safe.Cast(TcxGridMasterDataRow(ARecord).DetailGridViews[I], ADetailView) then
        begin
          ADetailViewInfo := TcxTableDataViewState.Create(FOptions);
          ADetailViewInfo.SaveTableView(ADetailView);
          FViewInfos.Add(ADetailViewInfo)
        end;
    end;
  end
  else
    FKeyValues := GetGroupRowKeyValue(TcxGridGroupRow(ARecord));
end;

procedure TGridTableViewStorage.TRecordInfo.SaveToNode(AParentNode: TdxXMLNode);
var
  ANode: TdxXMLNode;
  I: Integer;
begin
  ANode := AParentNode.AddChild('Record');
  ANode.Attributes.SetValueAsString('KeyValue', VarToStrDef(FKeyValues, ''));
  ANode.Attributes.SetValueAsInteger('KeyValueType', VarType(FKeyValues));
  ANode.Attributes.SetValueAsString('IsData', TdxXMLHelper.EncodeBoolean(IsData));
  ANode.Attributes.SetValueAsString('IsSelected', TdxXMLHelper.EncodeBoolean(IsSelected));
  ANode.Attributes.SetValueAsString('IsFocused', TdxXMLHelper.EncodeBoolean(IsFocused));
  ANode.Attributes.SetValueAsString('IsExpanded', TdxXMLHelper.EncodeBoolean(IsExpanded));
  ANode.Attributes.SetValueAsInteger('Level', Level);

  for I := 0 to FViewInfos.Count - 1 do
    FViewInfos[I].SaveToNode(ANode)
end;
...
function TdxCustomLayoutItemViewInfoHelper.IsActuallyVisible: Boolean;
begin
  Result := ActuallyVisible;
end;

function TdxCustomLayoutItemViewInfoHelper.Enabled: Boolean;
begin
  Result := GetEnabled;
end;

function TdxCustomLayoutItemViewInfoHelper.Expanded: Boolean;
begin
  Result := IsExpanded;
end;
...
class function TcxGridGroupByBoxSearchButtonHitTest.GetHitTestCode: Integer;
begin
  Result := htGroupByBoxSearchButton;
end;
...
procedure TcxGridRowLayoutOptions.ActiveChanged;
begin
  GridView.Styles.RefreshCache;
  inherited ActiveChanged;
  GridView.Controller.FocusFirstAvailableItem;
  GridView.CheckItemsUsingInFindCriteria;
end;

procedure TcxGridRowLayoutOptions.DoAssign(Source: TPersistent);
var
  AOptions: TcxGridRowLayoutOptions absolute Source;
begin
  if Source is TcxGridRowLayoutOptions then
  begin
    Active := AOptions.Active;
    SmartCellNavigation := AOptions.SmartCellNavigation;
    OnCustomDrawRowBackground := AOptions.OnCustomDrawRowBackground;
    OnCustomDrawRowHotTrack := AOptions.OnCustomDrawRowHotTrack;
    OnCustomDrawRowSelection := AOptions.OnCustomDrawRowSelection;
  end;
  inherited DoAssign(Source);
end;

procedure TcxGridRowLayoutOptions.DoCustomDrawRowBackground(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean);
begin
  if Assigned(FOnCustomDrawRowBackground) then
    FOnCustomDrawRowBackground(GridView, ACanvas, AViewInfo, ADone);
end;

procedure TcxGridRowLayoutOptions.DoCustomDrawRowHotTrack(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean);
begin
  if HasCustomDrawRowHotTrack then
    FOnCustomDrawRowHotTrack(GridView, ACanvas, AViewInfo, ADone);
end;

procedure TcxGridRowLayoutOptions.DoCustomDrawRowSelection(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean);
begin
  if HasCustomDrawRowSelection then
    FOnCustomDrawRowSelection(GridView, ACanvas, AViewInfo, ADone);
end;

function TcxGridRowLayoutOptions.GetLayoutController: TcxGridRowLayoutController;
begin
  Result := GridView.RowLayoutController;
end;

function TcxGridRowLayoutOptions.HasCustomDrawRowHotTrack: Boolean;
begin
  Result := Assigned(FOnCustomDrawRowHotTrack);
end;

function TcxGridRowLayoutOptions.HasCustomDrawRowSelection: Boolean;
begin
  Result := Assigned(FOnCustomDrawRowSelection);
end;

procedure TcxGridRowLayoutOptions.UseDefaultLayoutChanged;
begin
  inherited UseDefaultLayoutChanged;
  GridView.CheckItemsUsingInFindCriteria;
end;

function TcxGridRowLayoutOptions.GetGridView: TcxGridTableView;
begin
  Result := TcxGridTableView(inherited GridView)
end;

procedure TcxGridRowLayoutOptions.SetOnCustomDrawRowBackground(AValue: TcxGridRowLayoutCustomDrawEvent);
begin
  if not dxSameMethods(FOnCustomDrawRowBackground, AValue) then
  begin
    FOnCustomDrawRowBackground := AValue;
    Changed(vcProperty);
  end;
end;

procedure TcxGridRowLayoutOptions.SetOnCustomDrawRowHotTrack(AValue: TcxGridRowLayoutCustomDrawEvent);
begin
  if not dxSameMethods(FOnCustomDrawRowHotTrack, AValue) then
  begin
    FOnCustomDrawRowHotTrack := AValue;
    Changed(vcProperty);
  end;
end;

procedure TcxGridRowLayoutOptions.SetOnCustomDrawRowSelection(AValue: TcxGridRowLayoutCustomDrawEvent);
begin
  if not dxSameMethods(FOnCustomDrawRowSelection, AValue) then
  begin
    FOnCustomDrawRowSelection := AValue;
    Changed(vcProperty);
  end;
end;

{ TcxGridTableRowLayoutDataCellViewInfo }

function TcxGridTableRowLayoutDataCellViewInfo.AlwaysDrawSelectedWhenFocused: Boolean;
begin
  Result := True;
end;

function TcxGridTableRowLayoutDataCellViewInfo.CanAutoHeight: Boolean;
begin
  Result := inherited CanAutoHeight and GridView.RowLayoutController.CanDataCellAutoHeight;
end;

function TcxGridTableRowLayoutDataCellViewInfo.CanFocus: Boolean;
begin
  Result := inherited CanFocus and Item.CanFocus(GridRecord);
end;

function TcxGridTableRowLayoutDataCellViewInfo.CanDrawSelectedWhenEditing: Boolean;
begin
  Result := True;
end;

procedure TcxGridTableRowLayoutDataCellViewInfo.GetCaptionParams(var AParams: TcxViewParams);
begin
  GridView.Styles.GetRowLayoutItemParams(GridRecord, Item, AParams, True, Self);
end;

function TcxGridTableRowLayoutDataCellViewInfo.GetLayoutItemViewInfoInstance: TcxGridCustomLayoutItemViewInfo;
begin
  Result := TcxGridDataRowViewInfo(RecordViewInfo).LayoutViewInfo.FindLayoutItemViewInfo(Item);
end;

function TcxGridTableRowLayoutDataCellViewInfo.GetMultiLine: Boolean;
begin
  Result := RecordViewInfo.AutoHeight;
end;

function TcxGridTableRowLayoutDataCellViewInfo.GetTransparent: Boolean;
begin
  Result := RecordViewInfo.Transparent;
end;

function TcxGridTableRowLayoutDataCellViewInfo.HasBorders: Boolean;
begin
  Result := inherited HasBorders and (GridView.OptionsSelection.InvertSelect or not GridView.OptionsSelection.CellSelect or
    GridView.OptionsSelection.MultiSelect) and (GridView.RowLayout.CellBorders or Editing);
end;

function TcxGridTableRowLayoutDataCellViewInfo.HasHotTrackBackground: Boolean;
begin
  Result := inherited HasHotTrackBackground and not GridView.OptionsSelection.InvertSelect and
    GridView.OptionsSelection.CellSelect and not GridView.OptionsSelection.MultiSelect;
end;

function TcxGridTableRowLayoutDataCellViewInfo.HasSelectionBackground: Boolean;
begin
  Result := inherited HasSelectionBackground and not GridView.OptionsSelection.InvertSelect and
    GridView.OptionsSelection.CellSelect and not GridView.OptionsSelection.MultiSelect;
end;

procedure TcxGridTableRowLayoutDataCellViewInfo.InitEditSizeProperties(out AEditSizeProperties: TcxEditSizeProperties);
var
  ADefaultItemsViewInfo: TdxLayoutGroupViewInfo;
  ADefaultLayoutItemViewInfo: TcxGridRowLayoutItemViewInfo;
begin
  inherited InitEditSizeProperties(AEditSizeProperties);
  if GridView.RowLayoutController.IsDefaultContainerCalculated then
  begin
    ADefaultItemsViewInfo := GridView.RowLayoutController.DefaultContainerViewInfo.ItemsViewInfo;
    ADefaultLayoutItemViewInfo := TcxGridRowLayoutItemViewInfo(ADefaultItemsViewInfo.FindItemViewInfo(Item.RowLayoutItem));
    if ADefaultLayoutItemViewInfo <> nil then
      AEditSizeProperties.Width := ADefaultLayoutItemViewInfo.ControlViewInfo.Bounds.Width - 2 * cxGridEditOffset;
  end;
end;

function TcxGridTableRowLayoutDataCellViewInfo.IsValueTransparent: Boolean;
begin
  Result := Transparent or not HasBorders;
end;

function TcxGridTableRowLayoutDataCellViewInfo.SupportsEditing: Boolean;
begin
  Result := GridView.OptionsBehavior.EditMode = emInplace;
end;

function TcxGridTableRowLayoutDataCellViewInfo.GetGridView: TcxGridTableView;
begin
  Result := TcxGridTableView(inherited GridView);
end;

function TcxGridTableRowLayoutDataCellViewInfo.GetItem: TcxGridColumn;
begin
  Result := TcxGridColumn(inherited Item);
end;

function TcxGridTableRowLayoutDataCellViewInfo.GetRecordViewInfo: TcxCustomGridRowViewInfo;
begin
  Result := TcxCustomGridRowViewInfo(inherited RecordViewInfo);
end;

{ TcxGridTableRowLayoutContainerViewInfo }

function TcxGridTableRowLayoutContainerViewInfo.CanDrawRoot: Boolean;
begin
  Result := False;
end;

function TcxGridTableRowLayoutContainerViewInfo.FindGridItemViewInfo(
...
begin
  ARowViewInfo := TcxGridDataRowViewInfo(RecordViewInfo);
  if (ARowViewInfo <> nil) and (ARowViewInfo.LayoutViewInfo <> nil) then
  begin
    AColumn := TcxCustomGridColumn(AViewInfo.Item.GridViewItem);
    Result := ARowViewInfo.LayoutViewInfo.FindCellViewInfo(AColumn);
  end
  else
    Result := nil;
end;

function TcxGridTableRowLayoutContainerViewInfo.GetContainer: TcxGridTableRowLayoutContainer;
begin
  Result := TcxGridTableRowLayoutContainer(inherited Container);
end;

{ TcxGridTableRowLayoutContainer }

function TcxGridTableRowLayoutContainer.CanCreateLayoutItem(AGridItem: TcxCustomGridTableItem): Boolean;
var
  AColumn: TcxCustomGridColumn absolute AGridItem;
begin
  Result := inherited CanCreateLayoutItem(AGridItem) and AColumn.IsVisibleForRowLayout and (AColumn.RowLayoutItem = nil);
end;

function TcxGridTableRowLayoutContainer.CanPlaceItem(AItem: TcxGridRowLayoutItem): Boolean;
begin
  Result := inherited CanPlaceItem(AItem) and (AItem.GridViewItem <> nil) and
    TcxGridColumn(AItem.GridViewItem).IsVisibleForRowLayout;
end;

function TcxGridTableRowLayoutContainer.GetActualViewInfo: TcxGridLayoutContainerViewInfo;
var
  ARow: TcxCustomGridRow;
  ARowViewInfo: TcxGridDataRowViewInfo;
begin
  ARowViewInfo := nil;
  ARow := GridView.Controller.FocusedRow;
  if (ARow <> nil) and ARow.IsData and not ARow.IsSpecial then
    ARowViewInfo := TcxGridDataRowViewInfo(ARow.ViewInfo);
  if (ARowViewInfo <> nil) and (ARowViewInfo.LayoutViewInfo <> nil) then
    Result := ARowViewInfo.LayoutViewInfo.ContainerViewInfo
  else
    Result := ViewInfo;
end;

function TcxGridTableRowLayoutContainer.GetClientRect: TRect;
begin
  Result := LayoutController.ContainerClientRect;
end;

function TcxGridTableRowLayoutContainer.GetDefaultGroupClass: TdxLayoutGroupClass;
begin
  Result := TcxGridTableRowLayoutGroup;
end;

function TcxGridTableRowLayoutContainer.GetItemClass: TcxGridRowLayoutItemClass;
begin
  Result := TcxGridTableRowLayoutItem;
end;

procedure TcxGridTableRowLayoutContainer.SetItemForGridItem(
  AGridItem: TcxCustomGridTableItem; AItem: TcxGridRowLayoutItem);
begin
  TcxGridColumn(AGridItem).RowLayoutItem := TcxGridTableRowLayoutItem(AItem);
end;

function TcxGridTableRowLayoutContainer.GetGridView: TcxGridTableView;
begin
  Result := TcxGridTableView(inherited GridView);
end;

function TcxGridTableRowLayoutContainer.GetLayoutController: TcxGridTableRowLayoutController;
begin
  Result := TcxGridTableRowLayoutController(inherited LayoutController);
end;

{ TcxGridTableRowLayoutContainerOwner }

procedure TcxGridTableRowLayoutSerializationOwner.GetChildren(Proc: TGetChildProc; Root: TComponent);
begin
  inherited GetChildren(Proc, Root);
  FContainer.StoreChildren(Proc);
end;

procedure TcxGridTableRowLayoutSerializationOwner.SetContainer(AValue: TcxGridTableRowLayoutContainer);
begin
  FContainer := AValue;
end;

procedure TcxGridTableRowLayoutSerializationOwner.SetParentComponent(Value: TComponent);
begin
  inherited SetParentComponent(Value);
  if Value is TcxGridTableView then
    TcxGridTableView(Value).RowLayoutController.SetSerializationOwner(Self);
end;

function TcxGridTableRowLayoutSerializationOwner.GetContainer: TdxLayoutContainer;
begin
  Result := FContainer;
end;

{ TcxGridTableRowLayoutCustomizationController }

function TcxGridTableRowLayoutCustomizationController.CreateForm: TForm;
begin
  Result := TcxGridTableRowLayoutCustomizationForm.Create(nil);
end;

{ TcxGridTableRowLayoutController }

constructor TcxGridTableRowLayoutController.Create(AGridView: TcxCustomGridTableView);
begin
  inherited Create(AGridView);
  FDefaultContainerViewInfo := CreateDefaultContainerViewInfo;
end;

destructor TcxGridTableRowLayoutController.Destroy;
begin
  FreeAndNil(FDefaultContainerViewInfo);
  inherited Destroy;
end;

procedure TcxGridTableRowLayoutController.CalculateContainer(AViewInfo: TcxGridTableRowLayoutContainerViewInfo;
  AWidth, AHeight: Integer);
begin
  CalculateDefaultContainer(AWidth, AHeight);
  if CanDataCellAutoHeight then
  begin
    FContainerClientRect := cxRect(cxSize(AWidth, AHeight));
    AViewInfo.Calculate(False);
  end
  else
  begin
    AViewInfo.PrepareData(False);
    AViewInfo.AssignBounds(DefaultContainerViewInfo);
  end;
end;

function TcxGridTableRowLayoutController.CalculateContainerAutoHeight(
  AViewInfo: TcxGridTableRowLayoutContainerViewInfo; AWidth: Integer): Integer;
begin
  CalculateDefaultContainer(AWidth, 0);
  Result := GetContainerHeight(AViewInfo);
end;

procedure TcxGridTableRowLayoutController.CopyCustomizationSetting(AContainer: TdxLayoutContainer);
begin
  inherited CopyCustomizationSetting(AContainer);
  GridView.CheckItemsUsingInFindCriteria;
end;

procedure TcxGridTableRowLayoutController.Init;
begin
  inherited Init;
  FreeAndNil(FSerializationOwner);
  GridView.CheckItemsUsingInFindCriteria;
end;

procedure TcxGridTableRowLayoutController.InvalidateDefaultContainer;
begin
  FIsDefaultContainerCalculated := False;
  if DefaultContainerViewInfo <> nil then
    DefaultContainerViewInfo.DestroyViewInfos;
end;

procedure TcxGridTableRowLayoutController.CalculateDefaultContainer(AWidth, AHeight: Integer);
begin
  if not IsDefaultContainerCalculated then
  begin
    FContainerClientRect := cxRect(cxSize(AWidth, AHeight));
    DefaultContainerViewInfo.Calculate(False);
    FIsDefaultContainerCalculated := True;
  end;
end;

function TcxGridTableRowLayoutController.CreateCustomizationController: TcxGridRowLayoutCustomizationController;
begin
  Result := TcxGridTableRowLayoutCustomizationController.Create(GridView);
end;

function TcxGridTableRowLayoutController.CreateDefaultContainerViewInfo: TcxGridTableRowLayoutContainerViewInfo;
begin
  Result := TcxGridTableRowLayoutContainerViewInfo.Create(Container, nil);
end;

function TcxGridTableRowLayoutController.CreateSerializationOwner: TcxGridTableRowLayoutSerializationOwner;
begin
  Result := TcxGridTableRowLayoutSerializationOwner.Create(nil);
end;

function TcxGridTableRowLayoutController.GetContainerClass: TcxGridRowLayoutContainerClass;
begin
  Result := TcxGridTableRowLayoutContainer;
end;

function TcxGridTableRowLayoutController.GetContainerHeight(AViewInfo: TcxGridTableRowLayoutContainerViewInfo): Integer;
begin
  if AViewInfo.ItemsViewInfo = nil then
     AViewInfo.PrepareData(False);
  Result := AViewInfo.ItemsViewInfo.MinHeight
end;

function TcxGridTableRowLayoutController.GetContainerWidth(AViewInfo: TcxGridTableRowLayoutContainerViewInfo): Integer;
begin
  if AViewInfo.ItemsViewInfo = nil then
     AViewInfo.PrepareData(False);
  Result := Container.ViewInfo.ItemsViewInfo.MinWidth;
end;

function TcxGridTableRowLayoutController.GetOptionsInstance: TcxGridCustomRowLayoutOptions;
begin
  Result := GridView.RowLayout;
end;

procedure TcxGridTableRowLayoutController.SetSerializationOwner(AValue: TcxGridTableRowLayoutSerializationOwner);
begin
  FSerializationOwner := AValue;
  FSerializationOwner.SetContainer(Container);
end;

procedure TcxGridTableRowLayoutController.Store(Proc: TGetChildProc);
begin
  SetSerializationOwner(CreateSerializationOwner);
  try
    FSerializationOwner.SetContainer(Container);
    Proc(FSerializationOwner);
  finally
    FreeAndNil(FSerializationOwner);
  end;
end;

procedure TcxGridTableRowLayoutController.UpdateLayoutLookAndFeel;
...
begin
  LayoutLookAndFeel.BeginUpdate;
  try
    LayoutLookAndFeel.LookAndFeel.MasterLookAndFeel := GridView.LookAndFeel;
    GridView.Styles.GetRowLayoutItemParams(nil, nil, AParams);
    UpdateCaptionOptions(LayoutLookAndFeel.ItemOptions.CaptionOptions, AParams);
    GridView.Styles.GetRowLayoutGroupParams(AParams);
    UpdateCaptionOptions(LayoutLookAndFeel.GroupOptions.CaptionOptions, AParams);
    LayoutLookAndFeel.GroupOptions.Color := AParams.Color;
  finally
    LayoutLookAndFeel.EndUpdate;
  end;
end;

function TcxGridTableRowLayoutController.GetContainer: TcxGridTableRowLayoutContainer;
begin
  Result := TcxGridTableRowLayoutContainer(inherited Container);
end;

function TcxGridTableRowLayoutController.GetContainerDefaultHeight: Integer;
begin
  Result := GetContainerHeight(DefaultContainerViewInfo);
end;

function TcxGridTableRowLayoutController.GetContainerDefaultWidth: Integer;
begin
  Result := GetContainerWidth(DefaultContainerViewInfo);
end;

function TcxGridTableRowLayoutController.GetGridView: TcxGridTableView;
begin
  Result := TcxGridTableView(inherited GridView);
end;

function TcxGridTableRowLayoutController.GetOptions: TcxGridRowLayoutOptions;
begin
  Result := TcxGridRowLayoutOptions(inherited Options);
end;

{ TcxGridTableViewInplaceEditFormDataCellViewInfo }

function TcxGridTableViewInplaceEditFormDataCellViewInfo.CanDrawHotTracked: Boolean;
begin
  Result := GridView.EditForm.ItemHotTrack;
end;

function TcxGridTableViewInplaceEditFormDataCellViewInfo.CanDrawSelected: Boolean;
begin
  Result := CanDrawHotTracked;
end;

function TcxGridTableViewInplaceEditFormDataCellViewInfo.CalculateHotTracked: Boolean;
begin
  Result := CanDrawHotTracked and IsHotTracked;
end;

function TcxGridTableViewInplaceEditFormDataCellViewInfo.CalculateSelected: Boolean;
begin
  Result := CanDrawSelected and Focused and not HotTracked;
end;

function TcxGridTableViewInplaceEditFormDataCellViewInfo.CanAutoHeight: Boolean;
begin
  Result := inherited CanAutoHeight and GridView.InplaceEditForm.CanDataCellAutoHeight;
end;
...
function TcxGridTableViewInplaceEditFormDataCellViewInfo.GetEditBounds: TRect;
begin
  Result := TcxGridDataRowViewInfo(RecordViewInfo).InplaceEditFormAreaViewInfo.GetBoundsForEdit(inherited GetEditBounds);
end;

function TcxGridTableViewInplaceEditFormDataCellViewInfo.GetLayoutItemViewInfoInstance: TcxGridCustomLayoutItemViewInfo;
begin
  if Item.LayoutItem <> nil then
    Result := Item.LayoutItem.ViewInfo
  else
    Result := nil;
end;

function TcxGridTableViewInplaceEditFormDataCellViewInfo.HasBorders: Boolean;
begin
  Result := inherited HasBorders and GridView.EditForm.CellBorders and not GridView.EditForm.ItemHotTrack;
end;

function TcxGridTableViewInplaceEditFormDataCellViewInfo.HotTrackSelection: Boolean;
begin
  Result := True;
end;
...
begin
  Result := not IsDestroying and CanDrawHotTracked;
end;

function TcxGridTableViewInplaceEditFormDataCellViewInfo.RecalculateOnStateChange: Boolean;
begin
  Result := InvalidateOnStateChange;
end;
...
begin
  ARowViewInfo := TcxGridDataRowViewInfo(RecordViewInfo);
  if (ARowViewInfo <> nil) and (ARowViewInfo.InplaceEditFormAreaViewInfo <> nil) then
  begin
    AColumn := TcxCustomGridColumn(AViewInfo.Item.GridViewItem);
    Result := ARowViewInfo.InplaceEditFormAreaViewInfo.FindCellViewInfo(AColumn);
  end
  else
    Result := nil;
end;
...
function TcxGridTableViewInplaceEditFormContainer.CanCreateLayoutItem(AGridItem: TcxCustomGridTableItem): Boolean;
var
  AColumn: TcxCustomGridColumn absolute AGridItem;
begin
  Result := inherited CanCreateLayoutItem(AGridItem) and AColumn.IsVisibleForEditForm and (AColumn.LayoutItem = nil);
end;

function TcxGridTableViewInplaceEditFormContainer.CanPlaceItem(AItem: TcxGridRowLayoutItem): Boolean;
begin
  Result := inherited CanPlaceItem(AItem) and (AItem.GridViewItem <> nil) and
...
procedure TcxGridTableViewInplaceEditFormContainer.SetItemForGridItem(
  AGridItem: TcxCustomGridTableItem; AItem: TcxGridRowLayoutItem);
begin
  TcxGridColumn(AGridItem).LayoutItem := TcxGridInplaceEditFormLayoutItem(AItem);
end;
...
{ TcxGridTableViewInplaceEditFormCustomizationController }

function TcxGridTableViewInplaceEditFormCustomizationController.CreateForm: TForm;
begin
...
function TcxGridTableViewInplaceEditForm.GetContainerClass: TcxGridRowLayoutContainerClass;
begin
  Result := TcxGridTableViewInplaceEditFormContainer;
end;

procedure TcxGridTableViewInplaceEditForm.InvalidateIndicator;
begin
  GridView.ViewInfo.IndicatorViewInfo.Invalidate;
end;

procedure TcxGridTableViewInplaceEditForm.InvalidateUpdateButton;
var
  ARecord: TcxCustomGridRecord;
begin
  ARecord := GridView.Controller.FocusedRecord;
  if ARecord.IsData and (ARecord.ViewInfo <> nil) then
    TcxGridDataRowViewInfo(ARecord.ViewInfo).InplaceEditFormAreaViewInfo.InvalidateUpdateButtonInfo;
end;

function TcxGridTableViewInplaceEditForm.CreateCustomizationController: TcxGridRowLayoutCustomizationController;
begin
  Result := TcxGridTableViewInplaceEditFormCustomizationController.Create(GridView);
end;

procedure TcxGridTableViewInplaceEditForm.DetachedFormInitialize;
var
  AHeight, AWidth: Integer;
begin
  inherited DetachedFormInitialize;
  AHeight := DetachedForm.Height;
  AWidth := DetachedForm.Width;
  GridView.DoDetachedEditFormInitialize(DetachedForm);
  DetachedForm.UseDefaultHeight := DetachedForm.UseDefaultHeight and (AHeight = DetachedForm.Height);
  DetachedForm.UseDefaultWidth := DetachedForm.UseDefaultWidth and (AWidth = DetachedForm.Width);
end;

procedure TcxGridTableViewInplaceEditForm.Edit(ARow: TcxGridDataRow);
begin
  if GridView.Controller.FocusedRow <> ARow then
    GridView.Controller.FocusedRow := ARow;
  EditingRecordIndex := GridView.Controller.FocusedRowIndex;
  if GridView.OptionsBehavior.EditMode <> emModalEditForm then
    GridView.Controller.MakeFocusedRecordVisible;
end;

function TcxGridTableViewInplaceEditForm.GetOptionsInstance: TcxGridCustomRowLayoutOptions;
begin
  Result := GridView.EditForm;
end;

procedure TcxGridTableViewInplaceEditForm.ModifiedChanged;
begin
  inherited ModifiedChanged;
  if (DisplayMode = efdmInplace) and Visible then
    InvalidateUpdateButton;
  InvalidateIndicator;
end;

procedure TcxGridTableViewInplaceEditForm.UpdateLayoutLookAndFeel;
...
begin
  ...
  try
    LayoutLookAndFeel.LookAndFeel.MasterLookAndFeel := GridView.LookAndFeel;
    GridView.Styles.GetInplaceEditFormItemParams(nil, nil, AParams);
    UpdateCaptionOptions(LayoutLookAndFeel.ItemOptions.CaptionOptions, AParams);
    GridView.Styles.GetInplaceEditFormGroupParams(nil, nil, AParams);
    UpdateCaptionOptions(LayoutLookAndFeel.GroupOptions.CaptionOptions, AParams);
    LayoutLookAndFeel.GroupOptions.Color := AParams.Color;
  finally
    LayoutLookAndFeel.EndUpdate;
...
procedure TcxGridEditFormOptions.DoAssign(Source: TPersistent);
var
  AOptions: TcxGridEditFormOptions absolute Source;
begin
  inherited DoAssign(Source);
...
function TcxGridEditFormOptions.GetLayoutController: TcxGridRowLayoutController;
begin
...
procedure TcxGridEditFormOptions.SetMasterRowDblClickAction(AValue: TcxGridMasterRowDblClickAction);
begin
...
function TcxGridDataRow.CreateUIAProvider: IdxUIAProvider;
begin
  Result := dxUIACreateGridTableViewRowProvider(Self);
end;

function TcxGridDataRow.ExpandOnDblClick: Boolean;
begin
  Result := (GridView.OptionsBehavior.EditMode <> emInplace) and not IsSpecial and (FixedState = rfsNotFixed);
end;
...
function TcxGridDataRow.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;
...
begin
  Result := GridView.IsInplaceEditFormVisible and (InplaceEditForm.EditingRecordIndex = Index);
end;
...
begin
  case Key of
    VK_RETURN, VK_F2:
      if (GridView.OptionsBehavior.EditMode <> emInplace) and not EditFormVisible then
      begin
  ...
  if ((Key = VK_RETURN) or (Key = VK_SPACE)) and EditFormVisible and
    (Controller.FocusedItemKind in [fikUpdateButton, fikCancelButton]) then
...
begin
  if not AValue then
    ...
    if not GridView.IsDesigning and (GridView.OptionsBehavior.EditMode <> emInplace) then
      InplaceEditForm.Edit(Self);
end;
...
begin
  if EditFormVisible and not (dceInsert in DataController.EditState) and
    not Controller.InplaceEditForm.Modified then
    Result := ikInplaceEdit
  else
    Result := inherited GetIndicatorKind;
end;

function TcxGridDataRow.GetParentRecord: TcxCustomGridRecord;
begin
  if FixedState = rfsNotFixed then
    Result := inherited GetParentRecord
  else
...
procedure TcxGridDataRow.UIANotifyRowsChanged;
begin
  GridView.UIANotify(TdxUIAGridNotification.RowsChanged);
end;
...
function TcxGridNewItemRow.HasErrorData: Boolean;
begin
  Result := False;
end;
...
begin
  if (FSelected <> Value) and not GridView.IsInplaceEditFormVisible then
  begin
...
begin
  Result := (GridView.OptionsView.ExpandButtonsForEmptyDetails or HasChildren or Expanded) and (FixedState = rfsNotFixed);
end;
...
function TcxGridMasterDataRow.GetInternalActiveDetailGridView(out AView: TcxCustomGridView): Boolean;
var
  AIndex: Integer;
begin
  Result := False;
  if IsValid then
  begin
    AIndex := InternalActiveDetailIndex;
    if AIndex >= 0 then
    begin
      Result := DetailGridViewExists[AIndex];
      if Result then
        AView := DetailGridViews[AIndex];
    end;
  end;
end;
...
begin
  Result := (GridView.MasterRowDblClickAction = dcaSwitchExpandedState) and GridView.OptionsBehavior.ExpandMasterRowOnDblClick and
    Expandable or (GridView.MasterRowDblClickAction = dcaShowEditForm) and inherited ExpandOnDblClick;
end;
...
function TcxGridGroupRow.GetCaptionPrefix(AIndex: Integer): string;
begin
  Result := GroupedColumns[AIndex].GetAlternateCaption;
  if Result <> '' then
    Result := Result + CaptionSeparator;
end;

function TcxGridGroupRow.GetCaptionSeparator: string;
begin
  Result := ' : ';
end;
...
begin
  Result := CaptionPrefix[AIndex] + DisplayTextByGroupedColumn[AIndex];
end;
...
function TcxGridGroupRow.ExpandOnDblClick: Boolean;
begin
  Result := Expandable;
end;
...
function TcxGridGroupRow.HasErrorData: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  begin
    ...
    if AZone = nil then
    begin
      AZone := ViewInfo.HeaderViewInfo.GetZoneByFixedKind(AHitTest.Pos, SourceItem.FixedKind);
      if AZone <> nil then
...
var
  AIndex: Integer;
begin
  if (SourceItem.FixedKind = fkLeft) and (ViewInfo.HeaderViewInfo.ItemCount[fkLeft] = 0) or
    (SourceItem.FixedKind = fkRight) and (ViewInfo.HeaderViewInfo.ItemCount[fkRight] = 0)  or
    (SourceItem.FixedKind in [fkNone, fkLeftDynamic]) and (ViewInfo.HeaderViewInfo.ItemCount[fkNone] = 0) then
    Exit;
  if (SourceItem.FixedKind = fkLeft) and (DestZone.ItemIndex = ViewInfo.HeaderViewInfo.LastItemIndex[fkLeft] + 1) or
    (SourceItem.FixedKind = fkRight) and (DestZone.ItemIndex = ViewInfo.HeaderViewInfo.LastItemIndex[fkRight] + 1) or
    (SourceItem.FixedKind in [fkNone, fkLeftDynamic]) and (DestZone.ItemIndex = ViewInfo.HeaderViewInfo.LastItemIndex[fkNone] + 1) then
  begin
    AIndex := GridView.VisibleColumns[DestZone.ItemIndex - 1].Index;
...
begin
  if DestItemContainerKind = ckGroupByBox then
    Result := GetGroupByBoxArrowAreaBounds
  else
    Result := GetHeaderArrowAreaBounds(APlace);
end;

function TcxGridColumnHeaderMovingObject.GetDestZonesClientBounds: TRect;
begin
  if DestItemContainerKind = ckHeader then
    Result := ViewInfo.HeaderViewInfo.GetZonesClientBoundsByFixedKind(SourceItem.FixedKind)
  else
    Result := ViewInfo.ClientBounds;
end;

function TcxGridColumnHeaderMovingObject.GetGroupByBoxArrowAreaBounds: TRect;
var
...
function TcxGridColumnHeaderMovingObject.GetHeaderArrowAreaBounds(APlace: TcxGridArrowPlace): TRect;
begin
  if GridView.VisibleColumnCount = 0 then
    Result := ViewInfo.HeaderViewInfo.Bounds
  else
    case SourceItem.FixedKind of
      fkLeft:
        Result := GetHeaderArrowAreaBoundsForLeftFixedSource;
      fkRight:
        Result := GetHeaderArrowAreaBoundsForRightFixedSource;
      else 
        Result := GetHeaderArrowAreaBoundsForScrollableSource;
    end;
end;

function TcxGridColumnHeaderMovingObject.GetHeaderArrowAreaBoundsForLeftFixedSource: TRect;
var
  AHeader: TcxGridHeaderViewInfo;
  AIsArrowLocatedToRight: Boolean;
begin
  AIsArrowLocatedToRight := False;
  AHeader := ViewInfo.HeaderViewInfo;
  if AHeader.ItemCount[fkLeft] > 0 then
    if DestZone.ItemIndex = AHeader.LastItemIndex[fkLeft] + 1 then
      if AHeader.SeparatorBounds[fkLeft].IsEmpty then
      begin
        Result := AHeader[DestZone.ItemIndex - 1].Bounds;
        AIsArrowLocatedToRight := True;
      end
      else
        Result := AHeader.SeparatorBounds[fkLeft]
    else
      Result := AHeader[DestZone.ItemIndex].Bounds
  else
    if AHeader.ItemCount[fkNone] > 0 then
      Result := AHeader[DestZone.ItemIndex].Bounds
    else
      Result := GetHeaderRightFixedArrowAreaBounds;
  Result.Left := Max(Result.Left, ViewInfo.ClientBounds.Left);
  Result.Right := Min(Result.Right, ViewInfo.ClientBounds.Right);
  if AIsArrowLocatedToRight then
    if AHeader.UseRightToLeftAlignment then
      Result.Right := Result.Left
    else
      Result.Left := Result.Right;
end;

function TcxGridColumnHeaderMovingObject.GetHeaderArrowAreaBoundsForRightFixedSource: TRect;
var
  AHeader: TcxGridHeaderViewInfo;
  AIsArrowLocatedToRight: Boolean;
begin
  AIsArrowLocatedToRight := True;
  AHeader := ViewInfo.HeaderViewInfo;
  if AHeader.ItemCount[fkRight] > 0 then
    if DestZone.ItemIndex = AHeader.LastItemIndex[fkRight] + 1 then
      Result := AHeader[DestZone.ItemIndex - 1].Bounds
    else
    begin
      Result := AHeader[DestZone.ItemIndex].Bounds;
    ...
    if AHeader.ItemCount[fkNone] > 0 then
      ...
      Result := GetHeaderLeftFixedArrowAreaBounds;
  Result.Left := Max(Result.Left, ViewInfo.ClientBounds.Left);
  Result.Right := Min(Result.Right, ViewInfo.ClientBounds.Right);
  if AIsArrowLocatedToRight then
    if AHeader.UseRightToLeftAlignment then
...
function TcxGridColumnHeaderMovingObject.GetHeaderArrowAreaBoundsForScrollableSource: TRect;
var
  AHeader: TcxGridHeaderViewInfo;
  AIsArrowLocatedToRight: Boolean;
begin
  ...
  AHeader := ViewInfo.HeaderViewInfo;
  if AHeader.ItemCount[fkNone] > 0 then
    if DestZone.ItemIndex = AHeader.LastItemIndex[fkNone] + 1 then
      if AHeader.ItemCount[fkRight] > 0 then
        Result := GetHeaderRightFixedArrowAreaBounds
      else
      begin
...
function TcxGridColumnHeaderMovingObject.GetHeaderLeftFixedArrowAreaBounds: TRect;
begin
  Result := ViewInfo.HeaderViewInfo.SeparatorBounds[fkLeft];
  if Result.IsEmpty then
    Result := ViewInfo.HeaderViewInfo.LastItem[fkLeft].Bounds;
end;

function TcxGridColumnHeaderMovingObject.GetHeaderRightFixedArrowAreaBounds: TRect;
begin
  Result := ViewInfo.HeaderViewInfo.SeparatorBounds[fkRight];
  if Result.IsEmpty then
    Result := ViewInfo.HeaderViewInfo.FirstItem[fkRight].Bounds;
end;
...
begin
  ...
  ABounds := GetDestZonesClientBounds;
...
begin
  ...
  if Result then
    case DestItemContainerKind of
      ckGroupByBox:
        Result := (SourceItem.GroupIndex = -1) and (not IsMerging or AllowMergeWithLeftColumn or AllowMergeWithRightColumn) or
          IsValidDestinationForVisibleSource;
      ckHeader:
        Result := SourceItem.Visible and IsValidDestinationForVisibleSource or not SourceItem.Visible and IsValidDestinationFixedKind;
    end;
end;

function TcxGridColumnHeaderMovingObject.IsValidDestinationFixedKind: Boolean;
var
  AFirstIndex: Integer;
  AHeader: TcxGridHeaderViewInfo;
begin
  ...
  case SourceItem.FixedKind of
    fkLeft:
      if AHeader.ItemCount[fkLeft] > 0 then
        Result := DestZone.ItemIndex <= AHeader.LastItemIndex[fkLeft] + 1
      else
        Result := DestZone.ItemIndex = 0;
    fkRight:
      if AHeader.ItemCount[fkRight] > 0 then
        Result := DestZone.ItemIndex >= AHeader.FirstItemIndex[fkRight]
      else
        Result := DestZone.ItemIndex = GridView.VisibleColumnCount;
    else 
      if AHeader.ItemCount[fkNone] > 0 then
      begin
        if AHeader.ItemCount[fkLeftDynamic] > 0 then
          AFirstIndex := AHeader.LastItemIndex[fkLeftDynamic] + 1
        else
          AFirstIndex := AHeader.FirstItemIndex[fkNone];
        Result := (DestZone.ItemIndex = -1) or InRange(DestZone.ItemIndex, AFirstIndex, ViewInfo.HeaderViewInfo.LastItemIndex[fkNone] + 1)
      end
      else
        if AHeader.ItemCount[fkLeft] > 0 then
          Result := DestZone.ItemIndex = AHeader.LastItemIndex[fkLeft] + 1
...
begin
  case DestItemContainerKind of
    ...
    ckHeader:
      Result := ((SourceItemContainerKind = ckGroupByBox) or (DestZone.ItemIndex < SourceItem.VisibleIndex) or
        (SourceItem.VisibleIndex + 1 < DestZone.ItemIndex)) and IsValidDestinationFixedKind;
  else
    Result := False;
  end;
end;
...
function TcxGridColumnHeaderMovingObject.NeedCheckScrolling: Boolean;
begin
  Result := inherited NeedCheckScrolling and not (SourceItem.FixedKind in [fkLeft, fkRight]) and
    (ViewInfo.HeaderViewInfo.ItemCount[fkLeftDynamic] = 0);
end;
...
procedure TcxGridTableItemsListBox.UpdateItemHeight;
var
  ANewItemHeight: Integer;
  AHeaderBorderSize: Integer;
begin
  AHeaderBorderSize := LookAndFeelPainter.HeaderBorderSize;
  if GridView.UseLightBorders then
    AHeaderBorderSize := GridView.BordersScaleFactor.Apply(AHeaderBorderSize);

  ANewItemHeight := 2 * (AHeaderBorderSize + ScaleFactor.Apply(cxGridCellTextOffset)) + TdxTextMeasurer.TextLineHeight(Font);
  Inc(ANewItemHeight, TdxVisualRefinements.HeaderPadding.ScaledHeight(ScaleFactor));
  dxAdjustToTouchableSize(ANewItemHeight, ScaleFactor);
  ItemHeight := ANewItemHeight;
end;
...
begin
  ...
  ASelectionR.Deflate(AHeaderBorderSize);
  ATextR := cxRectInflate(ASelectionR, -ScaleFactor.Apply(cxGridCellTextOffset), 0);
  TdxVisualRefinements.HeaderPadding.Deflate(ATextR, ScaleFactor);
...
{ TcxGridTableFindPanel }

procedure TcxGridTableFindPanel.DisplayModeChanged;
var
  AWasVisible: Boolean;
begin
  AWasVisible := Visible;
  inherited DisplayModeChanged;
  if (GridView.FindPanel.Location = fplGroupByBox) and not Visible and not AWasVisible then
    Changed;
end;

function TcxGridTableFindPanel.GetActualLayout: TcxFindPanelLayout;
begin
  if GridView.FindPanel.Location = fplGroupByBox then
    Result := fplCompact
  else
    Result := inherited GetActualLayout;
end;

function TcxGridTableFindPanel.IsFocusingContentKey(Key: Word;
  Shift: TShiftState): Boolean;
begin
  if not (ssAlt in Shift) then
  begin
    if (Key = VK_DOWN) and ((Options.Position = fppTop) or (GridView.ViewInfo.FindPanelViewInfo.Location = fplGroupByBox)) then
      Exit(True);
    if (Key = VK_UP) and (Options.Position = fppBottom) and (GridView.ViewInfo.FindPanelViewInfo.Location = fplSeparatePanel) then
      Exit(True);
  end;
  Result := False;
end;

function TcxGridTableFindPanel.GetGridView: TcxGridTableView;
begin
  Result := TcxGridTableView(inherited GridView);
end;
...
begin
  ...
  begin
    ...
    if GridView.IsInplaceEditFormVisible then
      GridView.InplaceEditForm.UpdateModified;
end;
...
begin
  ...
  if (AKey = VK_ESCAPE) and GridView.IsInplaceEditFormVisible then
    GridView.InplaceEditForm.UpdateModified;
end;
...
{ TcxGridTableCustomizationController }

procedure TcxGridTableCustomizationController.CheckFormBounds(var R: TRect);
var
  AHeaderBottomBound: Integer;
begin
  inherited CheckFormBounds(R);
...
function TcxGridTableCustomizationController.GetViewInfo: TcxGridTableViewInfo;
begin
  Result := TcxGridTableViewInfo(inherited ViewInfo);
end;

{ TcxGridTableColumnScrollAnimationHelper }

constructor TcxGridTableColumnScrollAnimationHelper.Create(
  AOwner: TcxGridTableController);
begin
  inherited Create;
  FOwner := AOwner;
end;

procedure TcxGridTableColumnScrollAnimationHelper.AfterAnimationEnded(AIsRaiseEvent: Boolean);
begin
  inherited AfterAnimationEnded(AIsRaiseEvent);
  FOwner.ScrollAnimationEnded;
end;

procedure TcxGridTableColumnScrollAnimationHelper.BeforeAnimationStarted;
begin
  FOwner.ScrollAnimationStarted;
  inherited BeforeAnimationStarted;
end;

procedure TcxGridTableColumnScrollAnimationHelper.CheckPosition(var AValue: Integer);
begin
  FOwner.CheckLeftPos(AValue);
end;

function TcxGridTableColumnScrollAnimationHelper.GetOwnerPosition: Integer;
begin
  Result := FOwner.LeftPos;
end;

procedure TcxGridTableColumnScrollAnimationHelper.PositionChanged;
begin
  FOwner.GridView.DoLeftPosChanged;
end;

procedure TcxGridTableColumnScrollAnimationHelper.UpdateOwnerPosition(AValue: Integer);
begin
  FOwner.DoSetLeftPos(AValue);
end;
...
begin
  ...
  FColumnScrollAnimationHelper := TcxGridTableColumnScrollAnimationHelper.Create(Self);
end;

destructor TcxGridTableController.Destroy;
begin
  FreeAndNil(FColumnScrollAnimationHelper);
...
function TcxGridTableController.GetFindPanel: TcxGridTableFindPanel;
begin
  Result := TcxGridTableFindPanel(inherited FindPanel);
end;
...
function TcxGridTableController.GetRowLayoutController: TcxGridTableRowLayoutController;
begin
  Result := GridView.RowLayoutController;
end;
...
procedure TcxGridTableController.SetFocus(ANotifyMaster: Boolean);
begin
  inherited SetFocus(ANotifyMaster);
end;
...
begin
  ...
  begin
    ...
    if GridView.Visible and not GridView.IsUpdateLocked and GridView.IsInplaceEditFormVisible and
      (GridView.OptionsBehavior.EditMode <> emModalEditForm) then
    begin
      ARecord := ViewData.GetRecordByIndex(InplaceEditForm.EditingRecordIndex);
      ARowViewInfo := Safe<TcxGridDataRowViewInfo>.Cast(ARecord.ViewInfo);
      if ARecord.IsData and (ARecord.ViewInfo <> nil) and (ARowViewInfo.InplaceEditFormAreaViewInfo <> nil) then
        ARowViewInfo.InplaceEditFormAreaViewInfo.ButtonsPanelViewInfo.Invalidate;
    end;
  end;
end;
...
begin
  if FColumnScrollAnimationHelper.IsAnimationActive then
  begin
    APrevLeftPos := FColumnScrollAnimationHelper.StartPosition;
    FColumnScrollAnimationHelper.StopAnimation(False);
  end
  else
    APrevLeftPos := FLeftPos;
  DoSetLeftPos(Value);
  if FLeftPos <> APrevLeftPos then
...
procedure TcxGridTableController.DoSetLeftPos(AValue: Integer);
var
  APrevLeftPos: Integer;
begin
  CheckLeftPos(AValue);
  ...
  begin
    ...
    if GridView.CanOffsetHorz then
      ...
      LeftPosChanged;
    GridView.UIANotify(TdxUIAGridNotification.FocusedCellChanged);
  end;
end;
...
procedure TcxGridTableController.AdjustOffsetForFixedGroupMode(ARow: TcxCustomGridRow);
var
  AViewInfo: TcxCustomGridRowViewInfo;
begin
  if ViewInfo.RecordsViewInfo.GetFixedGroupsCount = 0 then
    Exit;
  AViewInfo := TcxCustomGridRowViewInfo(ViewInfo.RecordsViewInfo.GetRealItem(ARow));
  if (AViewInfo <> nil) and (AViewInfo.FixedState = rfsNotFixed) then
    PixelScrollRecordOffset := ViewInfo.RecordsViewInfo.GetFixedGroupsBottomBound - AViewInfo.Bounds.Top;
end;

procedure TcxGridTableController.AdjustRowVisibility(ARow: TcxCustomGridRow);
var
  APosition, AAdjustedPosition: Integer;
begin
  if not GridView.IsFixedGroupsMode or ARow.IsSpecial then
    Exit;
  if IsRecordScrollAnimationActive then
    FPostponedAdjustableRow := ARow
  else
  begin
    ...
    begin
      ...
      if IsRecordPixelScrolling and not IsPixelBasedScrollDataPos then
        AdjustOffsetForFixedGroupMode(ARow);
    end;
  end;
end;
...
begin
  Result := inherited CanDataPost and (not GridView.IsInplaceEditFormVisible or (InplaceEditForm.CloseQuery = mrYes));
end;
...
function TcxGridTableController.CanMakeItemVisible(AItem: TcxCustomGridColumn): Boolean;
begin
  Result := (AItem <> nil) and ((AItem.VisibleIndex <> -1) and (AItem.VisibleIndex < ViewInfo.HeaderViewInfo.Count) and
    (not GridView.RowLayout.Active or (FocusedRow <> nil) and FocusedRow.IsSpecial) or
    GridView.IsInplaceEditFormVisible and TcxGridColumn(AItem).IsVisibleForEditForm or
    GridView.RowLayout.Active and (FocusedRow <> nil) and FocusedRow.IsData and not FocusedRow.IsSpecial and
    (AItem.RowLayoutItem <> nil) and AItem.RowLayoutItem.ActuallyVisible);
end;

function TcxGridTableController.CanRowLayoutSmartCellNavigate: Boolean;
begin
  Result := GridView.Focused and GridView.RowLayout.Active and GridView.RowLayout.SmartCellNavigation and
    (FocusedRow <> nil) and FocusedRow.IsData and not IsSpecialRowFocused;
end;
...
begin
  ...
  if Result and GridView.IsInplaceEditFormVisible then
  begin
    ARecord := ViewData.GetRecordByIndex(ANewRecordIndex);
    if (ARecord <> nil) and (not ARecord.IsData or not TcxGridDataRow(ARecord).EditFormVisible) then
    begin
...
function TcxGridTableController.CreateFindPanel: TcxGridFindPanel;
begin
  Result := TcxGridTableFindPanel.Create(Self);
end;
...
procedure TcxGridTableController.FocusedItemChanged(APrevFocusedItem: TcxCustomGridTableItem);
  ...
  begin
    if FocusedItem <> nil then
      ...
      if not GridView.IsInplaceEditFormVisible then
...
function TcxGridTableController.GetActuallyFocusedItemIndex: Integer;
begin
  if (FocusedColumn <> nil) and GridView.RowLayout.Active and
    ((FocusedRow = nil) or FocusedRow.IsData and not FocusedRow.IsSpecial) then
      Result := FocusedColumn.Index
  else
    Result := inherited GetActuallyFocusedItemIndex;
end;
...
function TcxGridTableController.GetLastVisibleRecordIndex: Integer;
begin
  Result := inherited GetLastVisibleRecordIndex;
  if GridView.IsFixedGroupsMode and IsRecordPixelScrolling then
    Dec(Result, ViewInfo.RecordsViewInfo.GetFixedGroupsCount);
end;
...
begin
  Result := (AColumn = ForcingWidthItem) or (AColumn.VisibleIndex < ForcingWidthItem.VisibleIndex) and
    (ForcingWidthItem.VisibleIndex < GridView.LastAutoWidthSizableColumnVisibleIndex);
end;
...
function TcxGridTableController.IsScrollAnimationActive: Boolean;
begin
  Result := inherited IsScrollAnimationActive or FColumnScrollAnimationHelper.IsAnimationActive;
end;
...
procedure TcxGridTableController.ScrollAnimationEnded;
begin
  inherited ScrollAnimationEnded;
  if FPostponedAdjustableRow <> nil then
  begin
    AdjustRowVisibility(FPostponedAdjustableRow);
    FPostponedAdjustableRow := nil;
  end;
end;
...
begin
  if FColumnScrollAnimationHelper.IsAnimationActive then
    ALeftPos := FColumnScrollAnimationHelper.TargetPosition
  else
    ALeftPos := LeftPos;
  case ADirection of
    dirLeft:
      SetLeftPosition(ALeftPos - HScrollDelta, IsScrollAnimationEnabled);
    dirRight:
      SetLeftPosition(ALeftPos + HScrollDelta, IsScrollAnimationEnabled);
    dirUp:
...
begin
  ...
  begin
    if IsRecordScrollAnimationActive then
      ATopRecordIndex := TargetRecordScrollIndex + Max(1, GetPageVisibleRecordCount(TargetRecordScrollIndex, TargetRecordScrollOffset) - 1)
    else
    ...
    begin
      ...
      CheckTopRecordIndexAndOffset(ATopRecordIndex, APixelScrollRecordOffset);
      SetTopRecordIndexWithOffset(ATopRecordIndex, APixelScrollRecordOffset, IsScrollAnimationEnabled);
    end
    else
...
var
  ...
  ATopRecordIndex, ATopRecordOffset: Integer;
begin
  if IsRecordScrollAnimationActive then
    ATopRecordIndex := TargetRecordScrollIndex
  else
    ATopRecordIndex := InternalTopRecordIndex;
  ATopRecordOffset := 0;
  if ATopRecordIndex = -1 then Exit;
  if ATopRecordIndex = 0 then
    if DataController.IsGridMode then
      AVisibleRowCount := GetPageRecordCount
    else
    begin
      if IsRecordPixelScrolling then
        SetTopRecordIndexWithOffset(0, 0, IsScrollAnimationEnabled);
      Exit;
    end
  else
  begin
    AVisibleRowCount := GetPageVisibleRecordCount(ATopRecordIndex, 0, False);
    if DataController.IsGridMode and (ATopRecordIndex - (AVisibleRowCount - 1) = 0) and
  ...
  begin
    Dec(ATopRecordIndex);
    CheckTopRecordIndexAndOffset(ATopRecordIndex, ATopRecordOffset);
    SetTopRecordIndexWithOffset(ATopRecordIndex, ATopRecordOffset, IsScrollAnimationEnabled);
  end
  else
    if DataController.IsGridMode then
      TopRecordIndex := InternalTopRecordIndex - (AVisibleRowCount - 1)
    else
      SetTopRecordIndexWithOffset(ATopRecordIndex - (AVisibleRowCount - 1), 0, IsScrollAnimationEnabled);
end;

procedure TcxGridTableController.StopScrollAnimation;
begin
  inherited StopScrollAnimation;
  if FColumnScrollAnimationHelper.IsAnimationActive then
    FColumnScrollAnimationHelper.StopAnimation(False);
end;
...
begin
  Result := inherited WantSpecialKey(AKey) or
    ((AKey = VK_RETURN) and (IsFilterRowFocused or (GridView.OptionsBehavior.EditMode <> emInplace)));
end;
  ...
  begin
    case AScrollCode of
      ...
      scLineDown:
        ScrollData(dirRight);
      scPageUp:
        SetLeftPosition(LeftPos - ViewInfo.ScrollableAreaWidth, IsScrollAnimationEnabled);
      scPageDown:
        SetLeftPosition(LeftPos + ViewInfo.ScrollableAreaWidth, IsScrollAnimationEnabled);
      scTrack:
        LeftPos := AScrollPos;
...
procedure TcxGridTableController.SetLeftPosition(AValue: Integer; AAnimated: Boolean);
begin
  if AAnimated then
    FColumnScrollAnimationHelper.SetPosition(AValue)
  else
    LeftPos := AValue;
end;
...
begin
  Result := inherited CanProcessMultiSelect(AHitTest, AShift) and
    ...
    (not CellMultiSelect or (ssLeft in AShift) or
     not Safe.Cast<TcxGridTableDataCellViewInfo>(AHitTest.ViewInfo, ACell) or
     not (ACell.Selected or ACell.HotTracked and TcxGridColumn(ACell.Item).Selected));
end;
...
begin
  Result := GridView.IsInplaceEditFormVisible and not EditingController.IsErrorOnPost;
end;
...
procedure TcxGridTableController.CheckFocusedItem(AItemViewInfo: TcxGridTableViewInplaceEditFormDataCellViewInfo);
  ...
  var
    I: Integer;
    ALayoutItemViewInfo: TcxGridCustomLayoutItemViewInfo;
  begin
    Result := False;
    AFocusedItem := nil;
    if not AGroupViewInfo.Expanded or not AGroupViewInfo.Enabled then
      Exit;
    ...
    begin
      if AGroupViewInfo.ItemViewInfos[I] is TdxLayoutGroupViewInfo then
        ...
        if (AGroupViewInfo.ItemViewInfos[I] is TcxGridCustomLayoutItemViewInfo) then
        begin
          ALayoutItemViewInfo := TcxGridCustomLayoutItemViewInfo(AGroupViewInfo.ItemViewInfos[I]);
          Result := TcxGridTableViewInplaceEditFormDataCellViewInfo(ALayoutItemViewInfo.GridItemViewInfo).CanFocus;
          if Result then
            AFocusedItem := ALayoutItemViewInfo.GridItemViewInfo.Item;
        end;
      if Result then
        Break;
    end;
  ...
  begin
    ...
    if Result then
    begin
      Result := AItemViewInfo.ParentViewInfo.Expanded and AItemViewInfo.ParentViewInfo.IsActuallyVisible;
      if Result then
...
begin
  Result := not GridView.IsInplaceEditFormVisible;
  if not Result and CanCloseInplaceEditForm then
    Result := InplaceEditForm.CloseOnRecordInserting;
end;

function TcxGridTableController.FocusNextEditFormItem(AGoForward: Boolean): Boolean;
var
...
begin
  if FocusedItem = nil then
    ...
    AFocusedItemIndex := FocusedItem.Index;
  Result := FocusNextEditFormItem(AFocusedItemIndex, AGoForward);
end;

function TcxGridTableController.FocusNextEditFormItem(AFocusedItemIndex: Integer; AGoForward: Boolean): Boolean;
var
  ACycleChanged: Boolean;
begin
  AFocusedItemIndex := GetNextLayoutItemIndex(InplaceEditForm.Container.ViewInfo, AFocusedItemIndex,
    AGoForward, False, GetNextEditFormItemIndexFunc, ACycleChanged);
  if AFocusedItemIndex <> -1 then
    GridView.Items[AFocusedItemIndex].Focused := True
  else
    FocusedItem := nil;
  Result := FocusedItemKind <> fikNone;
end;

function TcxGridTableController.FocusNextRowLayoutItem(AFocusedItemIndex: Integer; AGoForward, AGoOnCycle: Boolean; out ACycleChanged: Boolean): Boolean;
begin
  AFocusedItemIndex := GetNextLayoutItemIndex(RowLayoutController.Container.GetActualViewInfo, AFocusedItemIndex,
    AGoForward, AGoOnCycle, GetNextRowLayoutItemIndexFunc, ACycleChanged);
  Result := (AFocusedItemIndex <> -1) and (not AGoOnCycle or not ACycleChanged or
    FocusNextRecord(FocusedRecordIndex, AGoForward, False, False, False));
  if Result then
  begin
    FocusedItem := GridView.Items[AFocusedItemIndex];
    Result := FocusedItem <> nil;
  end;
end;

function TcxGridTableController.GetNextEditFormItemIndexFunc(AItems: TList;
  AFocusedIndex: Integer; AGoForward, AGoOnCycle: Boolean; out ACycleChanged: Boolean): Integer;
begin
  ACycleChanged := False;
  ...
  begin
    ...
    if (Result < 0) and (FFocusedItemKind in [fikNone, fikGridItem]) then
    begin
      Result := -1;
      FocusedItemKind := fikCancelButton;
    end;
...
begin
  if InplaceEditForm.Modified then
    Result := fikUpdateButton
  else
    Result := fikCancelButton;
end;

function TcxGridTableController.GetNextRowLayoutItemIndexFunc(AItems: TList;
  AFocusedIndex: Integer; AGoForward, AGoOnCycle: Boolean; out ACycleChanged: Boolean): Integer;
begin
  Result := AFocusedIndex;
  if Result <> -1 then
    ...
    begin
      Inc(Result);
      ACycleChanged := Result > AItems.Count - 1;
      if AGoOnCycle and ACycleChanged then
        Result := 0;
    end
    else
    begin
      Dec(Result);
      ACycleChanged := Result < 0;
      if AGoOnCycle and ACycleChanged then
        Result := AItems.Count - 1;
    end
  else
    if AGoForward then
      ...
      Result := AItems.Count - 1;
  Result := EnsureRange(Result, 0, AItems.Count - 1);
end;

function TcxGridTableController.GetNextLayoutItemIndex(AContainerViewInfo: TcxGridLayoutContainerViewInfo;
  AFocusedIndex: Integer; AGoForward, AGoOnCycle: Boolean; AFunc: TcxGridGetNextItemFunc; out ACycleChanged: Boolean): Integer;
var
  I: Integer;
  AItems: TList;
begin
  AItems := TList.Create;
  try
    AContainerViewInfo.PopulateTabOrderList(AItems);
    for I := AItems.Count - 1 downto 0 do
      if not TcxCustomGridColumn(AItems[I]).CanFocus(FocusedRow) then
        AItems.Delete(I);
    Result := -1;
    if AFocusedIndex <> -1 then
      Result := AItems.IndexOf(GridView.Items[AFocusedIndex]);
    Result := AFunc(AItems, Result, AGoForward, AGoOnCycle, ACycleChanged);
    if Result <> -1 then
      Result := TcxCustomGridColumn(AItems[Result]).Index;
  finally
    AItems.Free;
  end;
end;

function TcxGridTableController.RowLayoutSmartCellNavigate(AFocusedItem: TcxCustomGridColumn;
  ADirection: TcxNeighbor; AStartNewCycle: Boolean): Boolean;
var
  I: Integer;
  AItems: TList;
  AViewInfo: TdxCustomLayoutItemViewInfo;
  AContainer: TcxGridLayoutContainerViewInfo;
begin
  AItems := TList.Create;
  try
    AContainer := RowLayoutController.Container.GetActualViewInfo;
    AContainer.FindItemViewInfo(AFocusedItem.RowLayoutItem, AViewInfo);
    AContainer.PopulateSmartOrderList(AItems, AViewInfo, ADirection, AStartNewCycle);
    for I := AItems.Count - 1 downto 0 do
      if not TcxCustomGridColumn(AItems[I]).CanFocus(FocusedRow) then
        AItems.Delete(I);
    Result := AItems.Count > 0;
    if Result then
      FocusedItem := TcxCustomGridColumn(AItems[0]);
  finally
    AItems.Free;
  end;
end;
...
begin
  Result := not GridView.IsInplaceEditFormVisible and inherited SupportsPullFocusing or CellMultiSelect;
end;
...
function TcxGridTableController.CreateCustomizationController: TcxCustomGridCustomizationController;
begin
  Result := TcxGridTableCustomizationController.Create(GridView);
end;
...
begin
  if GridView.IsInplaceEditFormVisible and not CloseInplaceEditFormOnRecordInserting then
    Exit;
  inherited CreateNewRecord(AtEnd);
  if (GridView.OptionsBehavior.EditMode = emModalEditForm) and (dceInsert in DataController.EditState) and
    FocusedRow.IsData and not FocusedRow.IsSpecial then
      TcxGridDataRow(FocusedRow).EditFormVisible := True;
end;
...
begin
  if GridView.IsInplaceEditFormVisible and (GridView.OptionsBehavior.EditMode <> emModalEditForm) then
    Result := FocusNextEditFormItem(AGoForward)
  else
...
begin
  if GridView.IsInplaceEditFormVisible and (GridView.OptionsBehavior.EditMode <> emModalEditForm) then
    ...
    if GridView.RowLayout.Active and ((FocusedRow = nil) or FocusedRow.IsData and not FocusedRow.IsSpecial) then
      Result := FocusNextRowLayoutItem(AFocusedItemIndex, AGoForward, AGoToNextRecordOnCycle, ACycleChanged)
    else
      Result := inherited FocusNextItem(AFocusedItemIndex, AGoForward, AGoOnCycle, AGoToNextRecordOnCycle,
        AFollowVisualOrder, ANeedNormalizeSelection);
end;
...
var
  AWasFullyVisible: Boolean;
  ADataRowViewInfo: TcxGridDataRowViewInfo;
  ACellBounds, AScrollableAreaBounds: TRect;
  AColumn: TcxCustomGridColumn absolute AItem;

  function GetCellBounds: TRect;
  var
    ACell: TcxCustomGridCellViewInfo;
  begin
    if GridView.RowLayout.Active and (FocusedRow <> nil) and FocusedRow.IsData and not FocusedRow.IsSpecial then
      if (FocusedRow.ViewInfo <> nil) and (TcxGridDataRowViewInfo(FocusedRow.ViewInfo).LayoutViewInfo <> nil) then
        ACell := TcxGridDataRowViewInfo(FocusedRow.ViewInfo).LayoutViewInfo.FindCellViewInfo(AColumn)
      else
        ACell := nil
    else
      ACell := ViewInfo.HeaderViewInfo[AColumn.VisibleIndex];
    if ACell <> nil then
      Result := ACell.Bounds
    else
      Result := cxEmptyRect;
  ...
  begin
    SetLeftPosition(LeftPos + ADelta, IsScrollAnimationEnabled and not EditingController.EditingItemSetting);
  end;
  ...
  begin
    if UseRightToLeftAlignment then
      ADelta := - (AColumnBounds.Right - AScrollableAreaBounds.Right)
    else
      ADelta := AColumnBounds.Right - AScrollableAreaBounds.Right;
    Scroll(ADelta);
  end;
  ...
  begin
    if UseRightToLeftAlignment then
      ADelta := AScrollableAreaBounds.Left - AColumnBounds.Left
    else
      ADelta := - (AScrollableAreaBounds.Left - AColumnBounds.Left);
    Scroll(ADelta);
  end;
...
begin
  if not CanMakeItemVisible(AColumn) then
    Exit;
  MakeFocusedRecordVisible;
  if GridView.IsInplaceEditFormVisible then
  begin
    ADataRowViewInfo := TcxGridDataRowViewInfo(FocusedRecord.ViewInfo);
    if (ADataRowViewInfo <> nil) and (ADataRowViewInfo.InplaceEditFormAreaViewInfo <> nil) then
      ADataRowViewInfo.InplaceEditFormAreaViewInfo.MakeItemVisible(AColumn)
  end
  else
    if AColumn.CanScroll then
    begin
      ACellBounds := GetCellBounds;
      AScrollableAreaBounds := ViewInfo.GetBoundsForMakeItemVisible(AColumn);
      AWasFullyVisible := True;
      if ACellBounds.Width >= AScrollableAreaBounds.Width then
      begin
        if UseRightToLeftAlignment then
          ScrollLeft(ACellBounds, AScrollableAreaBounds)
        else
          ScrollRight(ACellBounds, AScrollableAreaBounds);
        AWasFullyVisible := False;
      end
      else
      begin
        if ACellBounds.Right > AScrollableAreaBounds.Right then
        begin
          ScrollLeft(ACellBounds, AScrollableAreaBounds);
          ACellBounds := GetCellBounds;
          AWasFullyVisible := False;
        end;
        if ACellBounds.Left < AScrollableAreaBounds.Left then
        begin
          ScrollRight(ACellBounds, AScrollableAreaBounds);
          AWasFullyVisible := False;
        end;
      end;
      if FColumnScrollAnimationHelper.IsAnimationActive and AWasFullyVisible then
        FColumnScrollAnimationHelper.StopAnimation(True);
    end;
end;
...
procedure TcxGridTableController.ShowRowLayoutCustomizationDialog;
begin
  RowLayoutController.ShowCustomizationForm;
end;
...
begin
  if ViewInfo.ScrollableAreaWidth > 0 then
    ...
    APos := -1;
  if Assigned(Site) and TcxControlMainScrollbarsHelperAccess(TcxGridSiteAccess(Site).ScrollbarsHelper).HasIdenticalScrollBarVisibilityCalculationAttempts then
  begin
    Controller.SetScrollBarInfo(sbHorizontal, 0, 0, 1, 0, 0, False, True);
    Controller.SetScrollBarInfo(sbVertical, 0, 0, 1, 0, 0, False, True);
  end
  else
  begin
...
var
  AGoForward, AFocusedRecordChanged, AWasSmartNavigation: Boolean;
  AFocusedColumn: TcxGridColumn;
  AGridViewLink: TcxObjectLink;
  ...
  case Key of
    VK_LEFT, VK_RIGHT:
      begin
        ...
        if CanRowLayoutSmartCellNavigate then
        begin
          ADirection := TcxNeighbor(IfThen(AGoForward, Ord(nRight), Ord(nLeft)));
          if RowLayoutSmartCellNavigate(FocusedColumn, ADirection, False) then
            Exit;
        end
        else
    ...
    VK_UP, VK_DOWN:
      if GridView.IsInplaceEditFormVisible then
      begin
        FocusNextEditFormItem(Key = VK_DOWN);
        Exit;
      end;
    VK_PRIOR:
  ...
  try
    case Key of
      ...
      VK_UP:
          ...
          begin
            if CanRowLayoutSmartCellNavigate and RowLayoutSmartCellNavigate(FocusedColumn, nTop, False) then
              Exit;
            if IsSpecialRowFocused then
              AFocusedRowIndex := -1
            else
            ...
            AKey := Key;
            AllowRecordScrollAnimationOnMakeVisible(
              procedure
              begin
                NeedUpdateSiteOnFocusedRecordChanged := True;
                try
                  AWasSmartNavigation := False;
                  AFocusedColumn := FocusedColumn;
                  if CanRowLayoutSmartCellNavigate then
                    AWasSmartNavigation := RowLayoutSmartCellNavigate(FocusedColumn, nTop, True);
                  AFocusedRecordChanged := FocusNextRecord(AFocusedRowIndex, False, False, not (ssShift in Shift), not (ssShift in Shift));
                  if AWasSmartNavigation and (not AFocusedRecordChanged or not CanRowLayoutSmartCellNavigate) then
                    FocusedColumn := AFocusedColumn;
                  if not AFocusedRecordChanged and IsSpecialRowFocused then
                    AKey := 0;
                finally
      ...
      VK_DOWN:
          ...
          begin
            if CanRowLayoutSmartCellNavigate and RowLayoutSmartCellNavigate(FocusedColumn, nBottom, False) then
              Exit;
            NeedUpdateSiteOnFocusedRecordChanged := True;
            try
              AllowRecordScrollAnimationOnMakeVisible(
                ...
                begin
                  ...
                  AFocusedColumn := FocusedColumn;
                  if CanRowLayoutSmartCellNavigate then
                    AWasSmartNavigation := RowLayoutSmartCellNavigate(FocusedColumn, nBottom, True);
                  AFocusedRecordChanged := FocusNextRecord(FocusedRowIndex, True, False, not (ssShift in Shift), not (ssShift in Shift));
                  if AWasSmartNavigation and (not AFocusedRecordChanged or not CanRowLayoutSmartCellNavigate) then
                    FocusedColumn := AFocusedColumn;
                end);
            finally
...
var
  AViewInfo: TcxGridColumnHeaderGlyphViewInfo;
begin
  AViewInfo := ViewInfo;
  TdxImageDrawer.DrawUncachedImage(Canvas.Handle, AViewInfo.Bounds, AViewInfo.Bounds,
    AViewInfo.Glyph, AViewInfo.Images, AViewInfo.ImageIndex, idmNormal,
    False, 0, clNone, True, AViewInfo.ColorPalette);
end;
...
begin
  ...
  begin
    ...
    begin
      AExcludeRightBorder := ExcludeRightBorder;
      ABorders := Borders;
      if not IsRightToLeftConverted then
        ABorder := bRight
      else
        ABorder := bLeft;
      if AExcludeRightBorder then
        Exclude(ABorders, ABorder)
      else
        Include(ABorders, ABorder);

      LookAndFeelPainter.DrawScaledHeader(Self.Canvas, Bounds, TextAreaBounds, Neighbors,
        ABorders, AState, AlignmentHorz, AlignmentVert, MultiLinePainting, ShowEndEllipsis,
        Text, Params.Font, Params.TextColor, Params.Color, ScaleFactor,
        ViewInfo.GridViewInfo.HeaderViewInfo.DrawColumnBackgroundHandler, AExcludeRightBorder,
        ViewInfo.Container.Kind = ckGroupByBox, GridViewInfo.GridLineWidth);
    end;
    if NeedFixedHighlight then
      DrawFixedHighlight;
  end;
  DrawAreas;
end;

procedure TcxGridColumnHeaderPainter.DrawFixedHighlight;
begin
  ViewInfo.GridViewInfo.Painter.DrawFixedHighlight(ViewInfo.ClientBounds);
end;
...
procedure TcxGridHeaderPainter.DrawItems;
const
  AOrderedFixedKinds: array[0..3] of TcxGridColumnFixedKind = (fkLeftDynamic, fkLeft, fkRight, fkNone);
var
  I: Integer;
  AFixedKind: TcxGridColumnFixedKind;
begin
  for I := Low(AOrderedFixedKinds) to High(AOrderedFixedKinds) do
  begin
    AFixedKind := AOrderedFixedKinds[I];
    if ViewInfo.ItemCount[AFixedKind] > 0 then
    begin
      ViewInfo.GridViewInfo.Painter.DrawFixedSeparator(ViewInfo.SeparatorBounds[AFixedKind], AFixedKind = fkRight, True);
      DrawItemsByFixedKind(AFixedKind);
    end;
  end;
end;

procedure TcxGridHeaderPainter.DrawItemsByFixedKind(AKind: TcxGridColumnFixedKind);
var
  I: Integer;
...
begin
  for I := 0 to ViewInfo.Count - 1 do
  begin
    AItem := ViewInfo.InternalItems[I];
    if (AItem <> nil) and (AItem.FixedKind = AKind) then
      AItem.Paint;
  end;
end;
...
function TcxGridHeaderPainter.GetViewInfo: TcxGridHeaderViewInfo;
begin
  Result := TcxGridHeaderViewInfo(inherited ViewInfo);
end;

{ TcxGridGroupByBoxFindPanelShowButtonPainter }

procedure TcxGridGroupByBoxSearchButtonPainter.DrawContent;
begin
  ViewInfo.LookAndFeelPainter.DrawScaledGroupByBoxSearchButtonGlyph(Canvas, ViewInfo.GlyphBounds,
    ViewInfo.ButtonState, ScaleFactor, ViewInfo.Params.Color);
end;

function TcxGridGroupByBoxSearchButtonPainter.GetViewInfo: TcxGridGroupByBoxSearchButtonViewInfo;
begin
  Result := TcxGridGroupByBoxSearchButtonViewInfo(inherited ViewInfo);
end;
...
begin
  with ViewInfo do
    LookAndFeelPainter.DrawScaledGroupByBox(Canvas, R, Transparent, Params.Color, BackgroundBitmap, ScaleFactor, BordersScaleFactor);
end;
...
begin
  inherited DrawContent;
  DrawConnectors;
  if ViewInfo.IsSearchButtonVisible then
    ViewInfo.SearchButtonViewInfo.Paint(Canvas);
  if ViewInfo.GridViewInfo.FindPanelViewInfo.Location = fplGroupByBox then
    ViewInfo.GridViewInfo.FindPanelViewInfo.Paint(Canvas);
end;
...
var
  ...
  J: Boolean;
begin
  Canvas.Brush.Color := ViewInfo.LookAndFeelPainter.DefaultGroupByBoxLineColor;
  if ViewInfo.IsSingleLine then
  begin
    for I := 0 to ViewInfo.Count - 2 do
      Canvas.FillRect(ViewInfo.LinkLineBounds[I, True]);
  end
  else
    for I := 0 to ViewInfo.Count - 2 do
      for J := Low(J) to High(J) do
        Canvas.FillRect(ViewInfo.LinkLineBounds[I, J]);
end;
...
begin
  ViewInfo.LookAndFeelPainter.DrawFooterCell(Canvas, ViewInfo.Bounds, ViewInfo.AlignmentHorz,
    ...
    ViewInfo.Params.Color, DrawBackgroundHandler, ViewInfo.Borders, ScaleFactor, ViewInfo.GridView.BordersScaleFactor);
end;

{ TcxGridFooterPainter }

destructor TcxGridFooterPainter.Destroy;
begin
  FreeAndNil(FLeftFixedRegion);
  FreeAndNil(FRightFixedRegion);
  inherited Destroy;
end;
...
procedure TcxGridFooterPainter.DrawBackground;
begin
  ViewInfo.LookAndFeelPainter.DrawFooterContent(Canvas, ViewInfo.Bounds, ViewInfo.Params, ViewInfo.GetBorders,
    ViewInfo.ScaleFactor, ViewInfo.GridView.BordersScaleFactor);
end;
...
procedure TcxGridFooterPainter.DrawItems;
begin
  inherited DrawItems;
  if FLeftFixedRegion <> nil then
    Canvas.SetClipRegion(FLeftFixedRegion, roAdd, False);
  if FRightFixedRegion <> nil then
    Canvas.SetClipRegion(FRightFixedRegion, roAdd, False);
end;

procedure TcxGridFooterPainter.DrawItemsByFixedKind(AKind: TcxGridColumnFixedKind);
begin
  inherited DrawItemsByFixedKind(AKind);
  case AKind of
    fkLeft:
      ExcludeLeftFixedRegion;
    fkRight:
      ExcludeRightFixedRegion;
  end;
end;
...
procedure TcxGridFooterPainter.ExcludeLeftFixedRegion;
var
  ABounds: TRect;
begin
  ABounds := ViewInfo.Bounds;
  if ViewInfo.IsRightToLeftConverted then
    ABounds.Left := ViewInfo.GetLeftFixedAreaLastBound
  else
    ABounds.Right := ViewInfo.GetLeftFixedAreaLastBound;
  FLeftFixedRegion := TcxRegion.Create(ABounds);
  FLeftFixedRegion.Combine(Canvas.GetClipRegion, roIntersect);
  Canvas.ExcludeClipRect(ABounds);
end;

procedure TcxGridFooterPainter.ExcludeRightFixedRegion;
var
  ABounds: TRect;
begin
  ABounds := ViewInfo.Bounds;
  if ViewInfo.IsRightToLeftConverted then
    ABounds.Right := ViewInfo.GetRightFixedAreaFirstBound
  else
    ABounds.Left := ViewInfo.GetRightFixedAreaFirstBound;
  FRightFixedRegion := TcxRegion.Create(ABounds);
  FRightFixedRegion.Combine(Canvas.GetClipRegion, roIntersect);
  Canvas.ExcludeClipRect(ABounds);
end;
...
var
  ...
  ABorder: TcxBorder;
begin
  if not ViewInfo.GridView.UseLightBorders then
  ...
  begin
    if not ViewInfo.IsRightToLeftConverted then
      ...
      ABorder := bLeft;
    ABorders := [bBottom, ABorder];
  end;

  if not ViewInfo.UseLightBorders then
  begin
    ViewInfo.LookAndFeelPainter.DrawScaledHeader(Canvas, ViewInfo.Bounds, ViewInfo.Bounds, [nRight], ABorders,
  ...
  begin
    ViewInfo.LookAndFeelPainter.DrawScaledHeader(Canvas, ViewInfo.Bounds, ViewInfo.Bounds, [nRight], ABorders,
      GridCellStateToButtonState(ViewInfo.State), taLeftJustify, vaTop, False,
      False, '', nil, clNone, ViewInfo.Params.Color, ViewInfo.ScaleFactor, DrawBackgroundHandler,
      False, False, ViewInfo.GridViewInfo.GridLineWidth);
  end;
...
begin
  if not ViewInfo.GridView.UseLightBorders then
    ViewInfo.LookAndFeelPainter.DrawScaledIndicatorItem(Canvas, ViewInfo.Bounds, ViewInfo.GetImageAreaBounds,
      ViewInfo.IndicatorKind, ViewInfo.Params.Color, ScaleFactor, DrawBackgroundHandler, ViewInfo.GetNeighbors, True,
      ViewInfo.IsRightToLeftConverted, ViewInfo.GridViewInfo.GridLineWidth)
  else
    ViewInfo.LookAndFeelPainter.DrawScaledIndicatorItem(Canvas, ViewInfo.Bounds, ViewInfo.GetImageAreaBounds,
      ViewInfo.IndicatorKind, ViewInfo.Params.Color, ScaleFactor, DrawBackgroundHandler, ViewInfo.GetNeighbors, False,
      ViewInfo.IsRightToLeftConverted, ViewInfo.GridViewInfo.GridLineWidth);
end;
...
begin
  ViewInfo.LookAndFeelPainter.DrawFooterPanel(Canvas, ViewInfo.Bounds, ViewInfo.Params, ViewInfo.Borders, ScaleFactor,
    ViewInfo.GridView.BordersScaleFactor);
end;
...
begin
  ...
  begin
    R := CalculateLevelIndentSpaceBounds(ALevel, ABounds);
    ...
    begin
      ...
      FillRect(R, GridViewInfo.LevelIndentBackgroundBitmap);
    ExcludeRectFromBackground(R);

    Brush.Color := GridViewInfo.LevelSeparatorColor;
    R := CalculateLevelIndentVertLineBounds(ALevel, ABounds);
    FillRect(R);
    ExcludeRectFromBackground(R);

    Brush.Color := GridViewInfo.GridLineColor;
    R := CalculateLevelIndentHorzLineBounds(ALevel, ABounds);
    FillRect(R);
    ExcludeRectFromBackground(R);
  end;
end;
...
function TcxCustomGridRowPainter.NeedsPainting: Boolean;
begin
  Result := inherited NeedsPainting and not ViewInfo.Overlapped;
end;
...
class procedure TcxGridRowsPainter.DrawDataRowCellsByFixedKind(
  ARowViewInfo: TcxCustomGridRowViewInfo; AKind: TcxGridColumnFixedKind);
var
  I: Integer;
  ACellViewInfo: TcxGridDataCellViewInfoAccess;
  ADataRowViewInfo: TcxGridDataRowViewInfo absolute ARowViewInfo;
begin
  for I := 0 to ADataRowViewInfo.CellViewInfoCount - 1 do
  begin
    ACellViewInfo := TcxGridDataCellViewInfoAccess(ADataRowViewInfo.InternalCellViewInfos[I]);
    if (ACellViewInfo <> nil) and (ACellViewInfo.FixedKind = AKind) then
      ACellViewInfo.Paint;
  end;
end;
...
var
  I: Integer;
  AFixedKind: TcxGridColumnFixedKind;
  ADataRowViewInfo: TcxGridDataRowViewInfoAccess;
begin
  ADataRowViewInfo := TcxGridDataRowViewInfoAccess(ARowViewInfo);
  for I := Low(AOrderedFixedKinds) to High(AOrderedFixedKinds) do
  begin
    AFixedKind := AOrderedFixedKinds[I];
    if ADataRowViewInfo.GridViewInfo.HeaderViewInfo.ItemCount[AFixedKind] > 0 then
    begin
      ARowViewInfo.GridViewInfo.Painter.DrawFixedSeparator(ADataRowViewInfo.CellSeparatorBounds[AFixedKind], AFixedKind = fkRight, False);
      DrawDataRowCellsByFixedKind(ARowViewInfo, AFixedKind);
    end;
  end;
  ADataRowViewInfo.CellsAreaViewInfo.Paint;
end;
...
procedure TcxGridTablePainter.DrawFixedHighlight(ABounds: TRect);
var
  ACanvas: TdxGPGraphics;
begin
  ACanvas := dxGpBeginPaint(Canvas.Handle, ABounds);
  try
    ACanvas.FillRectangle(ABounds, ViewInfo.FixedHighlightColor);
  finally
    dxGpEndPaint(ACanvas);
  end;
end;

procedure TcxGridTablePainter.DrawFixedSeparator(ABounds: TRect; AIsRight: Boolean; AIsHeader: Boolean);
var
  APartBounds: TRect;
begin
  if not GridView.UseLightBorders then
  begin
    if not ABounds.IsEmpty then
    begin
      Canvas.FillRect(ABounds, ViewInfo.FixedSeparatorColor);
      Canvas.ExcludeClipRect(ABounds);
    end;
  end
  else
  begin
    if not ABounds.IsZero then
    begin
      if not ABounds.IsEmpty then
      begin
        Canvas.FillRect(ABounds, ViewInfo.FixedSeparatorColor);
      ...
      begin
        if not ViewInfo.IsRightToLeftConverted then
          APartBounds := Rect(ABounds.Left, ABounds.Top, ABounds.Left + 1, ABounds.Bottom)
        else
          APartBounds := Rect(ABounds.Left + 1, ABounds.Top, ABounds.Left + 2, ABounds.Bottom);
        Canvas.FillRect(APartBounds, ViewInfo.GridLineColor);
        Canvas.ExcludeClipRect(APartBounds);
      end;
    end;
  end;
end;
...
begin
  Result := inherited CanOffset(AItemsOffset, DX, DY) and
   ...
   (ViewInfo.RecordsViewInfo.GroupBackgroundBitmap = nil) and
    not GridView.IsInplaceEditFormVisible and
    not GridView.RowLayout.Active and
    not GridView.IsFixedGroupsMode and
    ((AItemsOffset <> 0) or
      not GridView.IsMaster and
      (ViewInfo.HeaderViewInfo.ColumnBackgroundBitmap = nil) and
      (not GridView.FilterRow.Visible or (GridView.FilterRow.InfoText = '')) and
      (not GridView.NewItemRow.Visible or (GridView.NewItemRow.InfoText = '')) and
      (GridView.VisibleColumnCountByFixedKind[fkLeftDynamic] = 0) and
      ((GridView.VisibleFixedColumnCount = 0) or (GridView.GroupedColumnCount = 0) and not GridView.Preview.Active)) and
...
var
  I, ALineWidth: Integer;
  N: array of DWORD;
  P: array of TPoint;
  R: TRect;
begin
  ALineWidth := ViewInfo.GridLineWidth;
  case ALineWidth of
    0:
      ;
    1:
      begin
        ...
        with Canvas do
        begin
          Pen.Width := ViewInfo.GridLineWidth;
    ...
    begin
      R := PRect(FGridLines[I])^;
      Canvas.FillRect(R, ViewInfo.GridLineColor);
    end;
  end;
end;
...
begin
  if ViewInfo.FindPanelViewInfo.IsPart then
    DrawFindPanel;
  DrawGroupByBox;
  DrawFilterBar;
  DrawIndicator;
...
procedure TcxGridTablePainter.RepaintOnOffset;
begin
  Site.Invalidate;
  Site.Update;
end;

{ TcxGridTableCustomFindPanelViewInfo }

constructor TcxGridTableFindPanelViewInfo.Create(AGridViewInfo: TcxCustomGridTableViewInfo);
begin
  inherited Create(AGridViewInfo);
  FLocation := GetLocation;
end;

function TcxGridTableFindPanelViewInfo.CalculateBounds: TRect;
begin
  if Location = fplGroupByBox then
    Result := GridViewInfo.GroupByBoxViewInfo.CalculateFitElementBounds(CalculateHeight, CalculateWidth)
  else
    Result := inherited CalculateBounds
end;

function TcxGridTableFindPanelViewInfo.CalculateWidth: Integer;
begin
  if Location = fplGroupByBox then
    Result := WidthInGroupByBox
  else
    Result := inherited CalculateWidth;
end;

function TcxGridTableFindPanelViewInfo.CalculateWidthInGroupByBox: Integer;
begin
  Result := CalculateBestWidth;
end;

function TcxGridTableFindPanelViewInfo.GetButtonsOffset: Integer;
begin
  if GridView.FindPanel.Location = fplGroupByBox then
    Result := GroupByBoxHorOffset
  else
    Result := inherited GetButtonsOffset;
end;

function TcxGridTableFindPanelViewInfo.GetFirstOffset: Integer;
begin
  if GridView.FindPanel.Location = fplGroupByBox then
    Result := GroupByBoxLeftOffset
  else
    Result := inherited GetFirstOffset;
end;

function TcxGridTableFindPanelViewInfo.GetLocation: TcxGridTableFindPanelLocation;
begin
  Result := GridView.FindPanel.Location;
  if (Result = fplGroupByBox) and not (GridViewInfo.GroupByBoxViewInfo.Visible and
    GridViewInfo.GroupByBoxViewInfo.CanFitElement(WidthInGroupByBox)) then
    Result := fplSeparatePanel;
end;

function TcxGridTableFindPanelViewInfo.GetIsPart: Boolean;
begin
  Result := Location = fplSeparatePanel;
end;

function TcxGridTableFindPanelViewInfo.GetVisible: Boolean;
begin
  Result := inherited GetVisible and ((Location = fplSeparatePanel) or
    (Location = fplGroupByBox) and GridViewInfo.GroupByBoxViewInfo.Visible);
end;

function TcxGridTableFindPanelViewInfo.HasBackground: Boolean;
begin
  Result := IsPart;
end;

function TcxGridTableFindPanelViewInfo.HasBorders: Boolean;
begin
  Result := IsPart;
end;

function TcxGridTableFindPanelViewInfo.GetGridView: TcxGridTableView;
begin
  Result := TcxGridTableView(inherited GridView);
end;

function TcxGridTableFindPanelViewInfo.GetGridViewInfo: TcxGridTableViewInfo;
begin
  Result := TcxGridTableViewInfo(inherited GridViewInfo);
end;

function TcxGridTableFindPanelViewInfo.GetWidthInGroupByBox: Integer;
begin
  if FWidthInGroupByBoxAssigned then
    Result := FWidthInGroupByBox
  else
  begin
    Result := CalculateWidthInGroupByBox;
    FWidthInGroupByBox := Result;
    FWidthInGroupByBoxAssigned := True;
  end;
end;
...
begin
  Result := TcxGridColumnHeaderViewInfo(FItems.List[Index]);
end;
...
begin
  ...
  begin
    ...
    FItems.List[Index] := Result;
    FIndex.AddOrSetValue(Result.Column, Result);
  end;
end;
...
begin
  ...
  FIndex := TDictionary<TcxCustomGridColumn, TcxGridColumnHeaderViewInfo>.Create(ColumnCount);
end;
...
begin
  CalculateParams;
  Result := GetItemClass.GetCellHeight(GridViewInfo.GetFontHeight(Params.Font), LookAndFeelPainter, ScaleFactor, BordersScaleFactor);
end;
...
var
  I: Integer;
begin
  if FIndex.TryGetValue(AColumn, Result) then
    Exit;
...
begin
  if not AColumn.Options.AutoWidthSizable and AColumn.GridView.HasColumnAutoWidth then
    Result := AColumn.Width
  else
    Result := AColumn.MinWidth;
  Result := Result + GetColumnIndentWidth(AColumn);
end;

function TcxGridColumnContainerViewInfo.GetColumnIndentWidth(AColumn: TcxGridColumn): Integer;
begin
  Result := GetColumnAdditionalWidth(AColumn) + GetItemAdditionWidth(FindItem(AColumn));
end;
...
function TcxGridColumnContainerViewInfo.GetZone(const P: TPoint; ATrackingBounds: TRect;
  AFirst, ALast: Integer): TcxGridItemContainerZone;
var
  I, AEndPos, ALastBound: Integer;
begin
  Result := nil;
  if not Visible or not PtInRect(ATrackingBounds, P) then Exit;
  if IsRightToLeftConverted then
    ALastBound := ATrackingBounds.Left
  else
    ALastBound := ATrackingBounds.Right;
  for I := AFirst to ALast do
  begin
    if I = ALast then
      AEndPos := ALastBound
    else
      AEndPos := Items[I].Bounds.CenterPoint.X;
    if IsRightToLeftConverted then
      ATrackingBounds.Left := AEndPos
    else
      ATrackingBounds.Right := AEndPos;
    if PtInRect(ATrackingBounds, P) then
    begin
      Result := TcxGridItemContainerZone.Create(I);
      Break;
    end;
    if IsRightToLeftConverted then
      ATrackingBounds.Right := ATrackingBounds.Left
    else
      ATrackingBounds.Left := ATrackingBounds.Right;
  end;
end;

function TcxGridColumnContainerViewInfo.GetZonesTrackingBounds: TRect;
begin
  Result := Bounds;
  if not IsRightToLeftConverted then
...
begin
  Result := GetZone(P, ZonesTrackingBounds, 0, Count);
end;
...
procedure TcxGridColumnHeaderAreaViewInfo.Calculate(const ABounds: TRect; var ATextAreaBounds: TRect);
...
function TcxGridColumnHeaderGlyphViewInfo.GetColorPalette: IdxColorPalette;
begin
  Result := LookAndFeelPainter.HeaderGlyphColorPalette(cxbsNormal);
end;
...
begin
  Result := inherited GetIsVisibleForPainting and ((GridView.OptionsSelection.CheckBoxPosition = cbpIndicator) or
    not GridView.OptionsSelection.ShowCheckBoxesDynamically or (CheckBoxState in [cbsChecked, cbsGrayed]) or
    (State = gcsPressed) or IsHotTracked or RowViewInfo.Focused or Controller.IsRecordHot(RowViewInfo));
end;
...
function TcxGridRowCheckBoxViewInfo.GetController: TcxGridTableController;
begin
  Result := TcxGridTableController(inherited Controller);
end;
...
function TcxGridColumnHeaderViewInfo.GetFixedKind: TcxGridColumnFixedKind;
begin
  Result := Column.FixedKind;
  if (Result = fkLeftDynamic) and not FIsDynamicFixed then
    Result := fkNone;
end;
...
function TcxGridColumnHeaderViewInfo.GetIsFixedByWidth: Boolean;
begin
  Result := FColumn.IsFixedByWidth;
end;
...
function TcxGridColumnHeaderViewInfo.ExcludeLeftBorder: Boolean;
begin
  Result := False;
end;

function TcxGridColumnHeaderViewInfo.ExcludeRightBorder: Boolean;
begin
  if not UseLightBorders then
    Result := False
  else
  begin
    if not IsRightToLeftConverted then
      Result := Column.IsMostRight and (GridView.OptionsView.ColumnAutoWidth or (GridViewInfo.Bounds.Right <= Bounds.Right)) and
        ((GridViewInfo.Bounds.Right = Bounds.Right) or (not GridViewInfo.ForceIncludeRightmostBorder))
    else
      Result := Column.IsMostRight and (GridView.OptionsView.ColumnAutoWidth or (GridViewInfo.Bounds.Left >= Bounds.Left)) and
        ((GridViewInfo.Bounds.Left = Bounds.Left) or (not GridViewInfo.ForceIncludeRightmostBorder))
  end;
end;
...
var
  I, ADelta, ATouchableElementHeight: Integer;
begin
  Result := GetTextCellHeight(GridViewInfo, LookAndFeelPainter);
  if HasGlyph then
  begin
    for I := FAreaViewInfos.Count - 1 downto 0 do
      if AreaViewInfos[I] is TcxGridColumnHeaderGlyphViewInfo then
      begin
        Result := Max(Result, GetCellHeight(
          TcxGridColumnHeaderGlyphViewInfo(AreaViewInfos[I]).CalculateHeight, LookAndFeelPainter, ScaleFactor, BordersScaleFactor));
        Break;
      end;
  end;
  ...
  begin
    ...
    ADelta :=  2 * ScaleFactor.Apply(cxGridCellTextOffset);
    Inc(ADelta, TdxVisualRefinements.HeaderPadding.ScaledHeight(ScaleFactor));
    Dec(ATouchableElementHeight, ADelta);
    dxAdjustToTouchableSize(ATouchableElementHeight, ScaleFactor);
    Inc(ATouchableElementHeight, ADelta);
...
var
  I: Integer;
begin
  FTextAreaBounds := ContentBounds;
  FTextAreaBounds.Deflate(GetContentIndents);
  for I := 0 to AreaViewInfoCount - 1 do
...
begin
   if IsFixed then
    AMinWidth := CalculateWidth
  else
    AMinWidth := Column.MinWidth + FContainer.GetColumnIndentWidth(Column);
  if Value < AMinWidth then Value := AMinWidth;
...
function TcxGridColumnHeaderViewInfo.EmulateMouseMoveAfterCalculate: Boolean;
begin
  Result := True;
end;

function TcxGridColumnHeaderViewInfo.FindArea(AAreaClass: TcxGridColumnHeaderAreaViewInfoClass;
  ACompareExactClassTypes: Boolean = True): TcxGridColumnHeaderAreaViewInfo;
var
  I: Integer;
begin
  for I := 0 to AreaViewInfoCount - 1 do
  begin
    ...
    if (Result.ClassType = AAreaClass) or (not ACompareExactClassTypes and Result.InheritsFrom(AAreaClass)) then
      Exit;
  end;
  Result := nil;
end;

procedure TcxGridColumnHeaderViewInfo.FixDynamically;
begin
  FIsDynamicFixed := True;
end;
...
begin
  Result := Column.AutoWidthSizable;
end;
...
begin
  Result := GetCellBorderWidth(LookAndFeelPainter, ScaleFactor, BordersScaleFactor, AIndex);
end;
...
class function TcxGridColumnHeaderViewInfo.GetCellBorderWidth(ALookAndFeelPainter: TcxCustomLookAndFeelPainter;
  AScaleFactor, ABordersScaleFactor: TdxScaleFactor; ABorder: TcxBorder): Integer;
begin
  Result := ABordersScaleFactor.Apply(ALookAndFeelPainter.HeaderBorderSize);
end;
...
class function TcxGridColumnHeaderViewInfo.GetCellHeight(ATextHeight: Integer;
  ...
  AScaleFactor, ABordersScaleFactor: TdxScaleFactor): Integer;
begin
  Result := ATextHeight +
    GetCellBorderWidth(ALookAndFeelPainter, AScaleFactor, ABordersScaleFactor, bTop) +
    GetCellBorderWidth(ALookAndFeelPainter, AScaleFactor, ABordersScaleFactor, bBottom) +
...
function TcxGridColumnHeaderViewInfo.GetContentIndents: TRect;
begin
...
begin
  ...
  if GridView.HasColumnAutoWidth then
  begin
    Result := GridViewInfo.ClientWidth;
    if Column.VisibleIndex < GridView.LastAutoWidthSizableColumnVisibleIndex then
      for I := 0 to FContainer.Count - 1 do
      begin
        if I < Column.VisibleIndex then
...
function TcxGridColumnHeaderViewInfo.NeedFixedHighlight: Boolean;
begin
  Result := Column.NeedFixedHighlight;
end;
...
procedure TcxGridColumnHeaderViewInfo.Calculate(ALeftBound, ATopBound: Integer;
  ...
  procedure CheckHiddenBorders(var AAdditionalWidthAtLeft, AAdditionalHeightAtTop: Integer; AIsFirstColumnInRightFixedBands: Boolean);
  begin
    CalculateParams;
    FHiddenBorders := cxBordersAll - Borders;
    if AIsFirstColumnInRightFixedBands then
    begin
      if FHiddenBorders <> [] then
      begin
        if bTop in FHiddenBorders then
        begin
    ...
    begin
      if FHiddenBorders <> [] then
      begin
        if bLeft in FHiddenBorders then
        begin
...
begin
  ...
  if not UseLightBorders then
    CheckHiddenBorders(FAdditionalWidthAtLeft, FAdditionalHeightAtTop, False)
  else
    CheckHiddenBorders(FAdditionalWidthAtLeft, FAdditionalHeightAtTop, ExcludeLeftBorder);
  inherited;
...
begin
  inherited DoRightToLeftConversion(ABounds);
  FHiddenBorders := TdxRightToLeftLayoutConverter.ConvertBorders(FHiddenBorders);
...
begin
  inherited;
  FDynamicFixedItems := TList.Create;
  FSpecific := GetViewInfoSpecificClass.Create(Self);
end;

destructor TcxGridHeaderViewInfo.Destroy;
begin
  FSpecific.Free;
  FDynamicFixedItems.Free;
  inherited;
end;
...
procedure TcxGridHeaderViewInfo.AddDynamicFixedItem(AItem: TcxGridColumnHeaderViewInfo);
begin
  FDynamicFixedItems.Add(AItem);
  AItem.FixDynamically;
end;
...
const
  ASeparatorFixedKinds: array[0..1] of TcxGridColumnFixedKind = (fkLeft, fkRight);
var
  I: Integer;
...
begin
  ...
  try
    ...
    for I := Low(ASeparatorFixedKinds) to High(ASeparatorFixedKinds) do
      AAutoWidthObject.AvailableWidth := AAutoWidthObject.AvailableWidth - SeparatorActualWidth[ASeparatorFixedKinds[I]];
...
begin
  if CanCalculateAutoWidths then
    CalculateColumnAutoWidths;
end;

procedure TcxGridHeaderViewInfo.CalculateDynamicFixedItems(ALeft, ATop, ACheckedLeft: Integer);
var
  I, AWidth: Integer;
  AItem: TcxGridColumnHeaderViewInfo;
begin
  AItem := FirstItem[fkNone];
  Inc(ACheckedLeft, IfThen(bLeft in AItem.HiddenBorders, 0, AItem.BorderWidth[bLeft]));
  for I := FirstItemIndex[fkNone] to LastItemIndex[fkNone] do
  begin
    AItem := Items[I];
    if (AItem.Column.FixedKind = fkLeftDynamic) and (AItem.ClientBounds.Left < ACheckedLeft) then
    begin
      AddDynamicFixedItem(AItem);
      AItem.BeforeRecalculation;
      AWidth := AItem.CalculateWidth;
      AItem.Calculate(ALeft, ATop, AWidth, ItemHeight);
      Inc(ALeft, AWidth);
      ACheckedLeft := ALeft;
      AItem.AfterRecalculation;
      if GridViewInfo.FixedSeparatorWidth > 0 then
      begin
        Dec(ACheckedLeft, Min(AWidth, AItem.BorderSize[bRight]));
        CalculateSeparatorBounds(fkLeftDynamic, ACheckedLeft, ATop, GridViewInfo.FixedSeparatorWidth);
        FSeparatorsBounds[fkLeft] := cxEmptyRect;
      end;
    end;
  end;
end;
...
var
  ABounds: TRect;
  ADynamicLeft, ACheckedLeft: Integer;
begin
  ResetSeparatorsBounds;
  ABounds := ItemsAreaBounds;
  if ItemCount[fkLeft] > 0 then
  begin
    CalculateLeftFixedItems(ABounds.Left, ABounds.Top);
    ADynamicLeft := ABounds.Left;
    if GridViewInfo.FixedSeparatorWidth > 0 then
    begin
      Dec(ABounds.Left, Min(GridViewInfo.FixedSeparatorWidth, LastItem[fkLeft].BorderSize[bRight]));
      CalculateSeparatorBounds(fkLeft, ABounds.Left, ABounds.Top, GridViewInfo.FixedSeparatorWidth);
    end;
    ACheckedLeft := ABounds.Left;
  end
  else
  begin
    ADynamicLeft := GridViewInfo.ClientBounds.Left;
    ACheckedLeft := ADynamicLeft;
  end;
  if ItemCount[fkRight] > 0 then
  begin
    CalculateRightFixedItems(ABounds.Right, ABounds.Top);
    if GridViewInfo.FixedSeparatorWidth > 0 then
    begin
      Dec(ABounds.Right, GridViewInfo.FixedSeparatorWidth);
      CalculateSeparatorBounds(fkRight, ABounds.Right, ABounds.Top, GridViewInfo.FixedSeparatorWidth);
    end;
  end;
  if ItemCount[fkNone] > 0 then
    CalculateScrollableItems(ABounds.Left, ABounds.Top);
  if GridView.VisibleColumnCountByFixedKind[fkLeftDynamic] > 0 then
    CalculateDynamicFixedItems(ADynamicLeft, ABounds.Top, ACheckedLeft);
end;

procedure TcxGridHeaderViewInfo.CalculateLeftFixedItems(var ALeft: Integer; ATop: Integer);
var
  I, AWidth: Integer;
  AItem: TcxGridColumnHeaderViewInfo;
begin
  for I := FirstItemIndex[fkLeft] to LastItemIndex[fkLeft] do
  begin
    AItem := Items[I];
    AWidth := AItem.CalculateWidth;
    AItem.Calculate(ALeft, ATop, AWidth, ItemHeight);
...
procedure TcxGridHeaderViewInfo.CalculateRightFixedItems(var ARight: Integer; ATop: Integer);
var
  I, AWidth: Integer;
  AItem: TcxGridColumnHeaderViewInfo;
begin
  for I := LastItemIndex[fkRight] downto FirstItemIndex[fkRight] do
  begin
    AItem := Items[I];
    AWidth := AItem.CalculateWidth;
    Dec(ARight, AWidth);
    AItem.Calculate(ARight, ATop, AWidth, ItemHeight);
  end;
end;

procedure TcxGridHeaderViewInfo.CalculateScrollableItems(ALeft, ATop: Integer);
var
  I, AWidth: Integer;
  AItem: TcxGridColumnHeaderViewInfo;
begin
  if (ItemCount[fkLeft] > 0) and not GridView.RowLayout.Active then
    Dec(ALeft, GridViewInfo.LeftPos);
  for I := FirstItemIndex[fkNone] to LastItemIndex[fkNone] do
  begin
    AItem := Items[I];
    AWidth := AItem.CalculateWidth;
...
procedure TcxGridHeaderViewInfo.CalculateSeparatorBounds(AFixedKind: TcxGridColumnFixedKind;
  var ALeft: Integer; ATop, AWidth: Integer);
begin
  FSeparatorsBounds[AFixedKind] := cxRectBounds(ALeft, ATop, AWidth, ItemHeight);
  Inc(ALeft, AWidth);
end;
...
begin
  if GridView.RowLayout.Active then
    Result := GridViewInfo.HeaderWidth
  else
...
begin
  Result := GridView.HasColumnAutoWidth;
end;
...
begin
  ...
  begin
    Result := [];
    if not IsColumnLeft(AColumn) or GridViewInfo.HasFirstBorderOverlap then
...
function TcxGridHeaderViewInfo.GetDynamicFixedAreaLastBound: Integer;
var
  ABounds: TRect;
begin
  ABounds := SeparatorBounds[fkLeftDynamic];
  if ABounds.IsEmpty then
    ABounds := LastItem[fkLeftDynamic].Bounds;
  Result := IfThen(IsRightToLeftConverted, ABounds.Left, ABounds.Right);
end;
...
begin
  Result := (ItemCount[fkLeft] = 0) and not GridView.RowLayout.Active;
end;

function TcxGridHeaderViewInfo.GetItemAreaBounds(AItem: TcxGridColumnHeaderViewInfo): TRect;
begin
  Result := GetItemAreaBoundsByFixedKind(AItem.Column.FixedKind);
end;

function TcxGridHeaderViewInfo.GetItemAreaBoundsByFixedKind(AKind: TcxGridColumnFixedKind): TRect;
var
  ABounds: TRect;
begin
  Result := GridViewInfo.ScrollableAreaBoundsHorz;
  ABounds := GridViewInfo.ScrollableAreaBounds;
  case AKind of
    fkLeft:
      if IsRightToLeftConverted then
      begin
        ...
        if ItemCount[fkLeft] = 0 then
          ...
          Result.Left := IfThen(SeparatorBounds[fkLeft].IsEmpty, LastItem[fkLeft].Bounds.Left,
            SeparatorBounds[fkLeft].Right);
        Result.Left := Max(Result.Left, ABounds.Left);
      end
      else
      begin
        Result.Left := ABounds.Left;
        if ItemCount[fkLeft] = 0 then
          ...
          Result.Right := IfThen(SeparatorBounds[fkLeft].IsEmpty, LastItem[fkLeft].Bounds.Right,
            SeparatorBounds[fkLeft].Left);
        Result.Right := Min(Result.Right, ABounds.Right);
      end;
    fkRight:
      if IsRightToLeftConverted then
      begin
        ...
        if ItemCount[fkRight] = 0 then
          ...
          Result.Right := IfThen(SeparatorBounds[fkRight].IsEmpty, FirstItem[fkRight].Bounds.Right,
            SeparatorBounds[fkRight].Left);
        Result.Right := Min(Result.Right, ABounds.Right);
        if ItemCount[fkLeft] > 0 then
          Result.Right := Min(Max(Result.Right, GetLeftFixedAreaLastBound), Result.Left);
      end
      else
      begin
        Result.Right := ABounds.Right;
        if ItemCount[fkRight] = 0 then
          ...
          Result.Left := IfThen(SeparatorBounds[fkRight].IsEmpty, FirstItem[fkRight].Bounds.Left,
            SeparatorBounds[fkRight].Right);
        Result.Left := Max(Result.Left, ABounds.Left);
        if ItemCount[fkLeft] > 0 then
          Result.Left := Min(Max(Result.Left, GetLeftFixedAreaLastBound), Result.Right);
      end;
  end;
end;
...
function TcxGridHeaderViewInfo.GetItemsHitTest(const P: TPoint): TcxCustomGridHitTest;
const
  AOrderedFixedKinds: array[0..3] of TcxGridColumnFixedKind = (fkLeftDynamic, fkLeft, fkRight, fkNone);
var
  I: Integer;
...
begin
  ...
  begin
    AFixedKind := AOrderedFixedKinds[I];
    if ItemCount[AFixedKind] > 0 then
    begin
      Result := GetItemsHitTestByFixedKind(P, AFixedKind);
      if Result <> nil then
        Exit;
    end;
  end;
...
function TcxGridHeaderViewInfo.GetItemsHitTestByFixedKind(const P: TPoint;
  AKind: TcxGridColumnFixedKind): TcxCustomGridHitTest;
var
  I: Integer;
  AItem: TcxGridColumnHeaderViewInfo;
begin
...
function TcxGridHeaderViewInfo.GetLeftFixedAreaLastBound: Integer;
var
  ABounds: TRect;
begin
  ABounds := SeparatorBounds[fkLeft];
  if ABounds.IsEmpty then
    ABounds := LastItem[fkLeft].Bounds;
  Result := IfThen(IsRightToLeftConverted, ABounds.Left, ABounds.Right);
end;
...
function TcxGridHeaderViewInfo.GetRightFixedAreaFirstBound: Integer;
var
  ABounds: TRect;
begin
  ABounds := SeparatorBounds[fkRight];
  if ABounds.IsEmpty then
    ABounds := FirstItem[fkRight].Bounds;
  Result := IfThen(IsRightToLeftConverted, ABounds.Right, ABounds.Left);
end;

function TcxGridHeaderViewInfo.GetViewInfoSpecificClass: TcxGridHeaderViewInfoSpecificClass;
begin
  Result := GridViewInfo.GetHeaderViewInfoSpecificClass;
end;
...
begin
  ...
  for I := Low(ASeparatorFixedKinds) to High(ASeparatorFixedKinds) do
    Inc(Result, SeparatorActualWidth[ASeparatorFixedKinds[I]]);
end;

function TcxGridHeaderViewInfo.GetZoneByFixedKind(const P: TPoint; AKind: TcxGridColumnFixedKind): TcxGridItemContainerZone;
var
  AFirstIndex, ALastIndex: Integer;
begin
  AFirstIndex := 0;
  ALastIndex := GridView.VisibleColumnCount;
  case AKind of
    fkLeft:
      if ItemCount[fkLeft] > 0 then
        ALastIndex := LastItemIndex[fkLeft] + 1
      else
        ALastIndex := AFirstIndex;
    fkRight:
      if ItemCount[fkRight] > 0 then
        AFirstIndex := FirstItemIndex[fkRight]
      else
        AFirstIndex := ALastIndex;
    else 
      if ItemCount[fkNone] > 0 then
      begin
        if ItemCount[fkLeftDynamic] > 0 then
          AFirstIndex := LastItemIndex[fkLeftDynamic]
        else
          AFirstIndex := FirstItemIndex[fkNone];
        ALastIndex := LastItemIndex[fkNone] + 1;
      end
      else
      begin
        if ItemCount[fkLeft] > 0 then
          AFirstIndex := LastItemIndex[fkLeft] + 1;
        ALastIndex := AFirstIndex;
      end;
  end;
  Result := GetZone(P, GetZonesTrackingBoundsByFixedKind(AKind), AFirstIndex, ALastIndex);
end;

function TcxGridHeaderViewInfo.GetZonesClientBoundsByFixedKind(AKind: TcxGridColumnFixedKind): TRect;
begin
  Result := GetItemAreaBoundsByFixedKind(AKind);
  if (AKind in [fkNone, fkLeftDynamic]) and (ItemCount[fkLeftDynamic] > 0) then
    if IsRightToLeftConverted then
      Result.Right := Max(GetDynamicFixedAreaLastBound, Result.Left)
    else
      Result.Left := Min(GetDynamicFixedAreaLastBound, Result.Right);
end;

function TcxGridHeaderViewInfo.GetZonesTrackingBounds: TRect;
begin
  Result := inherited GetZonesTrackingBounds;
  Result.Left := GridViewInfo.ClientBounds.Left;
  InflateRect(Result, 0, ColumnHeaderMovingZoneSize);
end;

function TcxGridHeaderViewInfo.GetZonesTrackingBoundsByFixedKind(AKind: TcxGridColumnFixedKind): TRect;
var
  ABounds: TRect;
begin
  Result := ZonesTrackingBounds;
  if (AKind <> fkLeftDynamic) and (ItemCount[AKind] > 0) then
  begin
    ABounds := GetZonesClientBoundsByFixedKind(AKind);
    if not ABounds.IsEmpty then
    begin
      Result.Left := ABounds.Left;
      Result.Right := ABounds.Right;
...
function TcxGridHeaderViewInfo.IsColumnLeft(AColumn: TcxCustomGridColumn): Boolean;
begin
  Result := AColumn.IsLeft or (Items[AColumn.VisibleIndex] = FirstItem[fkLeftDynamic]) and (ItemCount[fkLeft] = 0);
end;
...
procedure TcxGridHeaderViewInfo.ResetSeparatorsBounds;
var
  AFixedKind: TcxGridColumnFixedKind;
begin
  for AFixedKind := Low(FSeparatorsBounds) to High(FSeparatorsBounds) do
    FSeparatorsBounds[AFixedKind] := cxEmptyRect;
end;
...
begin
  ...
  for AFixedKind := Low(FSeparatorsBounds) to High(FSeparatorsBounds) do
    if not FSeparatorsBounds[AFixedKind].IsEmpty then
      FSeparatorsBounds[AFixedKind] := TdxRightToLeftLayoutConverter.ConvertRect(FSeparatorsBounds[AFixedKind], ABounds);
end;

function TcxGridHeaderViewInfo.GetDynamicFixedItemCount: Integer;
begin
  Result := FDynamicFixedItems.Count;
end;

function TcxGridHeaderViewInfo.GetDynamicFixedItem(AIndex: Integer): TcxGridColumnHeaderViewInfo;
begin
  Result := TcxGridColumnHeaderViewInfo(FDynamicFixedItems[AIndex]);
end;

function TcxGridHeaderViewInfo.GetFirstItem(AFixedKind: TcxGridColumnFixedKind): TcxGridColumnHeaderViewInfo;
var
  AIndex: Integer;
begin
  AIndex := FirstItemIndex[AFixedKind];
  if AIndex <> -1 then
    Result := Items[AIndex]
  else
    Result := nil;
...
function TcxGridHeaderViewInfo.GetFirstItemIndex(AFixedKind: TcxGridColumnFixedKind): Integer;
var
  AIndex, ACount: Integer;
begin
  Result := -1;
  ACount := ItemCount[AFixedKind];
  if ACount = 0 then
    Exit;
  case AFixedKind of
    fkLeft:
      Result := 0;
    fkRight:
      Result := GridView.VisibleColumnCount - ACount;
    fkLeftDynamic:
      Result := DynamicFixedItems[0].Index;
    else 
    begin
      AIndex := LastItemIndex[fkLeft];
      if AIndex <> -1 then
        Result := AIndex + 1
      else
        Result := 0;
...
function TcxGridHeaderViewInfo.GetItemCount(AFixedKind: TcxGridColumnFixedKind): Integer;
begin
  case AFixedKind of
    fkLeft, fkRight:
      Result := GridView.VisibleColumnCountByFixedKind[AFixedKind];
    fkLeftDynamic:
      Result := DynamicFixedItemCount
    else
      Result := GridView.VisibleScrollableColumnCount;
  end;
end;

function TcxGridHeaderViewInfo.GetLastItem(AFixedKind: TcxGridColumnFixedKind): TcxGridColumnHeaderViewInfo;
var
  AIndex: Integer;
begin
  AIndex := LastItemIndex[AFixedKind];
  if AIndex <> -1 then
    Result := Items[AIndex]
  else
    Result := nil;
...
function TcxGridHeaderViewInfo.GetLastItemIndex(AFixedKind: TcxGridColumnFixedKind): Integer;
var
  AIndex, ACount: Integer;
begin
  Result := -1;
  ...
  case AFixedKind of
    fkLeft:
      Result := ACount - 1;
    fkRight:
      Result := GridView.VisibleColumnCount - 1;
    fkLeftDynamic:
      Result := DynamicFixedItems[ACount - 1].Index;
    else 
    begin
      AIndex := FirstItemIndex[fkRight];
      if AIndex <> -1 then
        Result := AIndex - 1
      else
        Result := GridView.VisibleColumnCount - 1;
...
function TcxGridHeaderViewInfo.GetSeparatorBounds(AFixedKind: TcxGridColumnFixedKind): TRect;
begin
  Result := FSeparatorsBounds[AFixedKind];
end;

function TcxGridHeaderViewInfo.GetSeparatorActualWidth(AFixedKind: TcxGridColumnFixedKind): Integer;
var
  ANeighborBorderWidth: Integer;
  AItem: TcxGridColumnHeaderViewInfo;
begin
  if (AFixedKind in [fkLeft, fkRight]) and (ItemCount[AFixedKind] > 0) then
  begin
    if AFixedKind = fkLeft then
      ANeighborBorderWidth := LastItem[fkLeft].BorderSize[bRight]
    else
      begin
        AItem := LastItem[fkNone];
        if AItem <> nil then
          ANeighborBorderWidth := AItem.BorderSize[bRight]
        else
          ANeighborBorderWidth := 0;
      end;
    Result := GridViewInfo.GetFixedSeparatorActualWidth(ANeighborBorderWidth);
  end
  else
    Result := 0;
end;
...
begin
  Result := Container.IsColumnWidthCalculating;
end;
...
{ TcxGridGroupByBoxFindPanelShowButtonViewInfo }

constructor TcxGridGroupByBoxSearchButtonViewInfo.Create(AGroupByBoxViewInfo: TcxGridGroupByBoxViewInfo);
begin
  inherited Create(AGroupByBoxViewInfo.GridViewInfo);
  FGroupByBoxViewInfo := AGroupByBoxViewInfo;
end;

procedure TcxGridGroupByBoxSearchButtonViewInfo.Calculate(ALeftBound, ATopBound: Integer;
  AWidth: Integer = -1; AHeight: Integer = -1);
begin
  inherited Calculate(ALeftBound, ATopBound, AWidth, AHeight);
  FGlyphBounds := cxRectCenter(Bounds, GetGlyphSize);
end;

procedure TcxGridGroupByBoxSearchButtonViewInfo.DoRightToLeftConversion(const ABounds: TRect);
begin
  inherited DoRightToLeftConversion(ABounds);
  FGlyphBounds := TdxRightToLeftLayoutConverter.ConvertRect(GlyphBounds, ABounds);
end;

function TcxGridGroupByBoxSearchButtonViewInfo.CalculateHeight: Integer;
begin
  Result := GetGlyphSize.cy + 2 * GetMargin;
end;

function TcxGridGroupByBoxSearchButtonViewInfo.CalculateWidth: Integer;
begin
  Result := GetGlyphSize.cx + 2 * GetMargin;
end;

procedure TcxGridGroupByBoxSearchButtonViewInfo.Click;
begin
  inherited Click;
  GridView.Controller.ShowFindPanel;
end;

function TcxGridGroupByBoxSearchButtonViewInfo.GetFocused: Boolean;
begin
  Result := False;
end;

function TcxGridGroupByBoxSearchButtonViewInfo.GetGlyphSize: TSize;
begin
  Result := ScaleFactor.Apply(cxSize(GlyphSize));
  dxAdjustToTouchableSize(Result, ScaleFactor);
end;

function TcxGridGroupByBoxSearchButtonViewInfo.GetHitTestClass: TcxCustomGridHitTestClass;
begin
  Result := TcxGridGroupByBoxSearchButtonHitTest;
end;

function TcxGridGroupByBoxSearchButtonViewInfo.GetMargin: Integer;
begin
  Result := ScaleFactor.Apply(Margin);
end;

function TcxGridGroupByBoxSearchButtonViewInfo.GetPainterClass: TcxCustomGridCellPainterClass;
begin
  Result := TcxGridGroupByBoxSearchButtonPainter;
end;

procedure TcxGridGroupByBoxSearchButtonViewInfo.GetViewParams(var AParams: TcxViewParams);
begin
  GridView.Styles.GetViewParams(vsGroupByBox, Self, nil, AParams);
  AParams.Color := AParams.TextColor;
end;

function TcxGridGroupByBoxSearchButtonViewInfo.GetGridView: TcxGridTableView;
begin
  Result := TcxGridTableView(inherited GridView);
end;
...
constructor TcxGridGroupByBoxViewInfo.Create(AGridViewInfo: TcxCustomGridTableViewInfo);
begin
  inherited Create(AGridViewInfo);
  FSearchButtonViewInfo := CreateSearchButtonViewInfo;
end;

destructor TcxGridGroupByBoxViewInfo.Destroy;
begin
  FreeAndNil(FSearchButtonViewInfo);
  inherited Destroy;
end;
...
function TcxGridGroupByBoxViewInfo.GetIsSearchButtonVisible: Boolean;
begin
  if FIsSearchButtonVisibleAssigned then
    Result := FIsSearchButtonVisible
  else
  begin
    Result := CanShowSearchButton and not Controller.IsFindPanelVisible and CanFitElement(SearchButtonViewInfo.Width);
    FIsSearchButtonVisible := Result;
    FIsSearchButtonVisibleAssigned := True;
  end;
end;
...
function TcxGridGroupByBoxViewInfo.CalculateFitElementBounds(AElementHeight, AElementWidth: Integer): TRect;
begin
  Result := Bounds;
  Result.Bottom := Result.Top + CalculateMinHeight;
  Result := cxRectCenterVertically(Result, AElementHeight);
  Result.Left := Result.Right - AElementWidth;
end;

function TcxGridGroupByBoxViewInfo.CalculateHeaderAreaHeight(ALevelCount: Integer): Integer;

  function GetTextHeight: Integer;
  begin
    CalculateParams;
    Result := GridViewInfo.GetFontHeight(Params.Font);
    GetCellTextAreaSize(Result, ScaleFactor, False, not LookAndFeelPainter.ApplyEditorAdvancedMode);
    Inc(Result, 2);
  end;
...
begin
  ...
  if ALevelCount = 0 then
    Inc(Result, GetTextHeight)
  else
    Inc(Result, ItemHeight div 2 * (ALevelCount + 1) + Byte(Odd(ItemHeight)));
end;
...
var
  ALevelCount: Integer;
begin
  ALevelCount := GridView.DataController.Groups.LevelCount;
  if (ALevelCount <> 0) and IsSingleLine then
    ALevelCount := 1;
  Result := CalculateHeaderAreaHeight(ALevelCount);
  if CanShowFindPanel then
    Result := Max(Result, GridViewInfo.FindPanelViewInfo.CalculateHeight);
  if CanShowSearchButton then
    Result := Max(Result, SearchButtonViewInfo.Height);
end;
...
function TcxGridGroupByBoxViewInfo.CalculateMinHeight: Integer;
begin
  Result := CalculateHeaderAreaHeight(0);
  if CanShowFindPanel then
    Result := Max(Result, GridViewInfo.FindPanelViewInfo.CalculateHeight);
  if CanShowSearchButton then
    Result := Max(Result, SearchButtonViewInfo.Height);
...
function TcxGridGroupByBoxViewInfo.CanFitElement(AElementWidth: Integer): Boolean;
var
  I, AOffset, AAvailableWidth: Integer;
begin
  AAvailableWidth := CalculateBounds.Width - AElementWidth;
  if Count = 0 then
    Result := AAvailableWidth - cxGridCellTextOffset - TextWidth - GroupByBoxLeftOffset > 0
  else
  begin
    Result := True;
    for I := 0 to Count - 1 do
    begin
      if I = 0 then
        AOffset := GroupByBoxLeftOffset
      else
        if not Items[I].Column.IsChildInMergedGroup then
          AOffset := GroupByBoxHorOffset
        else
          AOffset := 0;
      Dec(AAvailableWidth, AOffset);
      Result := AAvailableWidth >= 0;
      if not Result then
        Exit;
      Dec(AAvailableWidth, Items[I].CalculateWidth);
      Result := AAvailableWidth >= 0;
      if not Result then
        Exit;
    end;
...
function TcxGridGroupByBoxViewInfo.CanShowFindPanel: Boolean;
begin
  Result := (GridViewInfo.FindPanelViewInfo.Location = fplGroupByBox) and (GridView.FindPanel.DisplayMode <> fpdmNever);
end;

function TcxGridGroupByBoxViewInfo.CanShowSearchButton: Boolean;
begin
  Result := (GridView.FindPanel.Location = fplGroupByBox) and (GridView.FindPanel.DisplayMode = fpdmManual);
end;

function TcxGridGroupByBoxViewInfo.CreateSearchButtonViewInfo: TcxGridGroupByBoxSearchButtonViewInfo;
begin
  Result := TcxGridGroupByBoxSearchButtonViewInfo.Create(Self);
end;
...
begin
  Inc(FColumnWidthCalculatingCount);
  try
    Result := Items[AColumn.GroupIndex].GetBestFitWidth;
  finally
    Dec(FColumnWidthCalculatingCount);
  end;
end;
...
begin
  ...
  Result.Deflate(ScaleFactor.Apply(cxGridCellTextOffset));
  if not LookAndFeelPainter.ApplyEditorAdvancedMode then
    TdxVisualRefinements.Padding.Deflate(Result, ScaleFactor);
  Inc(Result.Left, GroupByBoxLeftOffset);
end;
...
function TcxGridGroupByBoxViewInfo.IsColumnWidthCalculating: Boolean;
begin
  Result := FColumnWidthCalculatingCount > 0;
end;
...
begin
  ...
  if IsSearchButtonVisible then
  begin
    ARect := CalculateFitElementBounds(SearchButtonViewInfo.Height, SearchButtonViewInfo.Width);
    SearchButtonViewInfo.Calculate(ARect);
  end;
  if GridViewInfo.FindPanelViewInfo.Location = fplGroupByBox then
    GridViewInfo.FindPanelViewInfo.MainCalculate;
end;
...
begin
  ...
  for I := 0 to Count - 1 do
    Items[I].RightToLeftConversion(ABounds);
  if IsSearchButtonVisible then
    SearchButtonViewInfo.RightToLeftConversion(ABounds);
  if (GridViewInfo.FindPanelViewInfo.Location = fplGroupByBox) and GridViewInfo.FindPanelViewInfo.Visible then
    GridViewInfo.FindPanelViewInfo.RightToLeftConversion(ABounds);
end;

function TcxGridGroupByBoxViewInfo.GetHitTest(const P: TPoint): TcxCustomGridHitTest;
var
  AHitTest: TcxCustomGridHitTest;
begin
  Result := inherited GetHitTest(P);
  ...
  begin
    if IsSearchButtonVisible then
    begin
      AHitTest := SearchButtonViewInfo.GetHitTest(P);
      if AHitTest <> nil then
        Exit(AHitTest);
    end;
    if (GridViewInfo.FindPanelViewInfo.Location = fplGroupByBox) and GridViewInfo.FindPanelViewInfo.Visible then
    begin
      AHitTest := GridViewInfo.FindPanelViewInfo.GetHitTest(P);
      if AHitTest <> nil then
        Exit(AHitTest);
    end;
  end;
...
begin
  inherited;
  ABounds.Offset(0, -Container.SeparatorWidth);
end;

procedure TcxGridFooterCellViewInfo.CalculateTextAreaBounds;
var
  I: Integer;
begin
...
function TcxGridFooterCellViewInfo.CalculateWidth: Integer;
begin
  if FWidth = -1 then
  begin
    FWidth := Container.GetColumnWidth(Column);
    FWidth := Container.Specific.CalcCellWidth(Column, FWidth);
  end;
  Result := FWidth;
end;
...
function TcxGridFooterCellViewInfo.GetBorderWidth(
  AIndex: TcxBorder): Integer;
var
  AFooterCellBorderSizeRect: TRect;
begin
  if (AIndex in Borders) then
  begin
    AFooterCellBorderSizeRect := LookAndFeelPainter.FooterCellBordersSizeRect(Borders, ScaleFactor, GridView.BordersScaleFactor);
    case AIndex of
      bLeft: Result := AFooterCellBorderSizeRect.Left;
      bTop: Result := AFooterCellBorderSizeRect.Top;
      bRight: Result := AFooterCellBorderSizeRect.Right;
    else
      Result := AFooterCellBorderSizeRect.Bottom;
    end;
  end
  else
    Result := 0;
...
class function TcxGridFooterCellViewInfo.GetCellBorderWidth(ALookAndFeelPainter: TcxCustomLookAndFeelPainter;
  AScaleFactor, ABordersScaleFactor: TdxScaleFactor; ABorder: TcxBorder): Integer;
var
  AFooterCellBorderSizeRect: TRect;
begin
  AFooterCellBorderSizeRect := ALookAndFeelPainter.FooterCellBordersSizeRect([ABorder], AScaleFactor, ABordersScaleFactor);
  case ABorder of
    bLeft: Result := AFooterCellBorderSizeRect.Left;
    bTop: Result := AFooterCellBorderSizeRect.Top;
    bRight: Result := AFooterCellBorderSizeRect.Right;
...
class function TcxGridFooterCellViewInfo.GetCellHeight(ATextHeight: Integer;
  ALookAndFeelPainter: TcxCustomLookAndFeelPainter;
  AScaleFactor, ABordersScaleFactor: TdxScaleFactor): Integer;
begin
  Result := ATextHeight;
...
function TcxGridFooterCellViewInfo.GetContentIndents: TRect;
begin
  Result := GridView.LookAndFeelPainter.FooterCellTextOffsets(GetBorders, ScaleFactor, GridView.BordersScaleFactor);
end;
...
function TcxGridFooterCellViewInfo.GetTextHeightWithOffset: Integer;
begin
  Result := TextHeight + TdxVisualRefinements.Padding.ScaledHeight(ScaleFactor);
end;

function TcxGridFooterCellViewInfo.GetTextWidthWithOffset: Integer;
begin
  Result :=
    TextWidth +
    ScaleFactor.Apply(cxGridCellTextOffset) * 2 +
    TdxVisualRefinements.Padding.ScaledWidth(ScaleFactor);
end;
...
var
  AFooterCellContentOffsets: TRect;
begin
  AFooterCellContentOffsets := LookAndFeelPainter.FooterCellContentOffsets(Borders, True, ScaleFactor, BordersScaleFactor);
  Result := inherited GetBestFitWidth + AFooterCellContentOffsets.Left + AFooterCellContentOffsets.Right;
end;
...
var
  ...
  ARowHeight: Integer;
begin
  FFooterPanelContentOffsets := LookAndFeelPainter.FooterPanelContentOffsets(
    GetBorders, False, ScaleFactor, GridView.BordersScaleFactor);
  FAllBordersSizeRect := LookAndFeelPainter.FooterPanelBordersSizeRect(
    cxBordersAll, ScaleFactor, GridView.BordersScaleFactor);

  CalculateParams;
  ARowCount := RowCount;
  ARowHeight := RowHeight;
  Result := ARowHeight + (ARowCount - 1) * (ARowHeight + Specific.GetVerticalSpaceBetweenItems) +
    BorderWidth[bTop] + BorderWidth[bBottom] + FooterPanelContentOffsets.Top + FooterPanelContentOffsets.Bottom;
end;

function TcxGridFooterViewInfo.CalculateItemHeight: Integer;
var
  AFooterCellTextOffsets: TRect;
begin
  AFooterCellTextOffsets := LookAndFeelPainter.FooterCellTextOffsets(cxBordersAll, ScaleFactor, GridView.BordersScaleFactor);
  Result := inherited CalculateItemHeight + AFooterCellTextOffsets.Top + AFooterCellTextOffsets.Bottom;
end;
...
begin
  ...
  begin
    case AIndex of
      bLeft:
        Result := AllBordersSizeRect.Left;
      bTop:
        Result := AllBordersSizeRect.Top;
      bRight:
        Result := AllBordersSizeRect.Right;
    else
      Result := AllBordersSizeRect.Bottom;
    end;
  end
  else
    Result := 0;
...
function TcxGridFooterViewInfo.GetDynamicFixedAreaLastBound: Integer;
begin
  Result := GridViewInfo.HeaderViewInfo.GetDynamicFixedAreaLastBound;
end;
...
begin
  if MultipleSummaries then
    ...
    Result := Specific.CalcCellHeight(AColumn.VisibleIndex, ItemHeight);
end;
...
begin
  if AColumn.IsMostLeft and not GridViewInfo.IsRightToLeftConverted then
    Result := ItemsAreaBounds.Left + BorderWidth[bLeft] + FooterPanelContentOffsets.Left
  else
  begin
    Result := GridViewInfo.HeaderViewInfo[AColumn.VisibleIndex].RealBounds.Left;
    if AColumn.IsMostLeft then
      Inc(Result, BorderWidth[bLeft] + FooterPanelContentOffsets.Left)
    else if AColumn.IsLeft and not IsRightToLeftConverted then
      Inc(Result, GridViewInfo.BorderOverlapSize + Specific.GetHorizontalSpaceBetweenItems div 2)
    else
      Inc(Result, Specific.GetHorizontalSpaceBetweenItems div 2);
  end;
end;
...
begin
  Result := FooterPanelContentOffsets.Top + ItemsAreaBounds.Top +
    GridViewInfo.GetCellLineIndex(AColumn.VisibleIndex) * (ItemHeight + Specific.GetVerticalSpaceBetweenItems);
end;

function TcxGridFooterViewInfo.GetItemTopBound(AIndex: Integer): Integer;
begin
  Result := GetItemTopBound(Items[AIndex].Column) + GetItemRowIndex(AIndex) * (RowHeight + Specific.GetVerticalSpaceBetweenItems);
end;
...
function TcxGridFooterViewInfo.GetLeftFixedAreaLastBound: Integer;
begin
  Result := GridViewInfo.HeaderViewInfo.GetLeftFixedAreaLastBound;
end;
...
function TcxGridFooterViewInfo.GetRightFixedAreaFirstBound: Integer;
begin
  Result := GridViewInfo.HeaderViewInfo.GetRightFixedAreaFirstBound;
end;
...
begin
  ...
  begin
    ...
    if UseLightBorders then
      Result := GridView.BordersScaleFactor.Apply(Result);
  end
  else
    Result := 0;
end;

function TcxGridFooterViewInfo.GetSpecific: TcxGridFooterViewInfoSpecific;
begin
  Result := TcxGridFooterViewInfoSpecific(inherited Specific);
end;
...
function TcxGridFooterViewInfo.GetViewInfoSpecificClass: TcxGridHeaderViewInfoSpecificClass;
begin
  Result := GridViewInfo.GetFooterViewInfoSpecificClass;
end;
...
function TcxGridFooterViewInfo.IsColumnLeft(AColumn: TcxCustomGridColumn): Boolean;
begin
  Result := GridViewInfo.HeaderViewInfo.IsColumnLeft(AColumn);
end;
...
begin
  ...
  begin
    case AIndex of
      bLeft:
        Result := GridViewInfo.FooterViewInfo.AllBordersSizeRect.Left;
      bTop:
        Result := GridViewInfo.FooterViewInfo.AllBordersSizeRect.Top;
      bRight:
        Result := GridViewInfo.FooterViewInfo.AllBordersSizeRect.Right
    else
      Result := GridViewInfo.FooterViewInfo.AllBordersSizeRect.Bottom;
    end;
  end
  else
    Result := 0;
...
begin
  if GridView.OptionsView.IndicatorWidth = 0 then
    Result := ScaleFactor.Apply(LookAndFeelPainter.GridDefaultIndicatorWidth)
  else
    Result := ScaleFactor.Apply(GridView.OptionsView.IndicatorWidth);
end;
...
procedure TcxGridIndicatorViewInfo.MoveItem(ACurIndex, ANewIndex: Integer);
begin
...
begin
  ...
  begin
    if (GridViewInfo.HeaderViewInfo.ItemCount[fkLeftDynamic] > 0) and
      (GridViewInfo.HeaderViewInfo.Items[0].FixedKind = fkLeftDynamic) then
        Result.Left := GridViewInfo.ClientBounds.Left;
...
function TcxCustomGridRowViewInfo.GetDataRelativeIndent: Integer;
begin
  Result := LevelIndent;
end;
...
begin
  Result := RecordsViewInfo.GetRowFocusRectBounds(Self);
end;
...
function TcxCustomGridRowViewInfo.GetHotTrack: Boolean;
begin
  Result := inherited GetHotTrack or GridRecord.ShowCheckBox and
    (GridView.OptionsSelection.CheckBoxPosition = cbpFirstColumn);
end;
...
procedure TcxCustomGridRowViewInfo.MarkAsOverlapped;
begin
  FOverlapped := True;
end;

function TcxCustomGridRowViewInfo.NeedToggleExpandRecord(AHitTest: TcxCustomGridHitTest;
  AButton: TMouseButton; AShift: TShiftState): Boolean;
begin
...
function TcxCustomGridRowViewInfo.PtInCaller(const P: TPoint): Boolean;
begin
  Result := inherited PtInCaller(P) or IndicatorItem.PtInCaller(P);
end;
...
function TcxGridRowsViewInfo.GetGridLineWidth: Integer;
begin
...
begin
  ...
  try
    ...
    CheckOverlapRows(AFixedGroups);
    PostFixedGroups(AFixedGroups);
    CorrectFixedGroupIndicatorsOrder(AFixedGroups);
    RecalculateFixedGroups(AFixedGroups);
    UpdateVisibleCount;
  finally
    AFixedGroups.Free;
  end;
...
procedure TcxGridRowsViewInfo.CheckOverlapRows(AFixedGroups: TdxFastList);
var
  I: Integer;
  AViewInfo: TcxCustomGridRowViewInfo;
begin
  if AFixedGroups.Empty then Exit;
  AViewInfo := TcxCustomGridRowViewInfo(AFixedGroups.Last);
  for I := Count - 1 downto 0 do
    if Items[I].Bounds.Bottom <= AViewInfo.Bounds.Bottom then
      Items[I].MarkAsOverlapped;
end;

procedure TcxGridRowsViewInfo.CorrectFixedGroupIndicatorsOrder(AFixedGroups: TdxFastList);

  function GetFirstScrollableIndex: Integer;
  var
    AIndicatorItem: TcxCustomGridIndicatorItemViewInfo;
    ARowIndicatorItem: TcxGridIndicatorRowItemViewInfo absolute AIndicatorItem;
  begin
    for Result := 0 to GridViewInfo.IndicatorViewInfo.Count - 1 do
    begin
      AIndicatorItem := GridViewInfo.IndicatorViewInfo.Items[Result];
      if (AIndicatorItem is TcxGridIndicatorRowItemViewInfo) and not ARowIndicatorItem.GridRecord.IsSpecial and
        (not ARowIndicatorItem.GridRecord.IsData or (ARowIndicatorItem.GridRecord.FixedState = rfsNotFixed)) then
          Exit;
    end;
    Result := -1;
  end;

  function GetIndex(AIndicatorItem: TcxCustomGridIndicatorItemViewInfo): Integer;
  begin
    for Result := GridViewInfo.IndicatorViewInfo.Count - 1 downto 0 do
      if GridViewInfo.IndicatorViewInfo.Items[Result] = AIndicatorItem then
        Exit;
    Result := -1;
  end;
...
var
  I, AIndex: Integer;
  AGroup: TcxGridGroupRowViewInfo;
begin
  AIndex := GetFirstScrollableIndex;
  if AIndex = -1 then
    Exit;
  for I := 0 to AFixedGroups.Count - 1 do
  begin
    AGroup := TcxGridGroupRowViewInfo(AFixedGroups[I]);
    GridViewInfo.IndicatorViewInfo.MoveItem(GetIndex(AGroup.IndicatorItem), AIndex);
    Inc(AIndex);
  end;
end;
...
begin
  ...
  ARowIndex := GridViewInfo.FirstRecordIndex;
  ARow := ViewData.Rows[ARowIndex];
  ...
  begin
    ...
    if (AGroup.Index = ARowIndex) and not AGroup.Expanded then
      Break;
...
procedure TcxGridRowsViewInfo.RecalculateFixedGroups(AFixedGroups: TdxFastList);
var
  I: Integer;
begin
  for I := 0 to AFixedGroups.Count - 1 do
    TcxCustomGridRowViewInfo(AFixedGroups[I]).Recalculate;
end;
...
begin
  ...
  FSpecialRowHeight := CalculateSpecialRowHeight;
  FSpecialDataRowHeight := CalculateSpecialDataRowHeight;
...
function TcxGridRowsViewInfo.CalculateDataRowHeight(AConsideLayout: Boolean = True): Integer;
begin
  if AConsideLayout then
    Result := FRowHeight
  else
    Result := CalculateRowHeight(False);
end;

function TcxGridRowsViewInfo.CalculateSpecialDataRowHeight: Integer;
begin
  if GridView.RowLayout.Active then
    Result := CalculateDataRowHeight(False)
  else
    Result := FDataRowHeight;
end;

function TcxGridRowsViewInfo.CalculateSpecialRowHeight: Integer;
begin
  if GridView.RowLayout.Active then
    Result := CalculateRowHeight(False)
  else
...
begin
  ...
  for I := 0 to GridView.GroupedColumnCount - 1 do
    if GridView.GroupedColumns[I].ShowGroupValuesWithImages then
      Result := Max(Result, ScaleFactor.Apply(GridView.GroupedColumns[I].GetImageComboBoxProperties.Images.Height));
end;
...
function TcxGridRowsViewInfo.CalculateRowDefaultHeight(AConsideLayout: Boolean = True): Integer;
var
  I, ACellHeight: Integer;
  AParams: TcxViewParams;
begin
  if GridView.RowLayout.Active and AConsideLayout then
    Result := GridView.RowLayoutController.ContainerDefaultHeight
  else
  begin
    if GridView.VisibleColumnCount = 0 then
    begin
    ...
    begin
      ...
      begin
        ...
        ACellHeight := HeaderViewInfo[I].Column.CalculateDefaultCellHeight(Canvas, AParams.Font);
...
function TcxGridRowsViewInfo.CalculateRowHeight(AConsideLayout: Boolean = True): Integer;
begin
  if IsDataRowHeightAssigned then
    Result := GridView.OptionsView.DataRowHeight
  else
  begin
    Result := CalculateRowDefaultHeight(AConsideLayout);
    dxAdjustToTouchableSize(Result, ScaleFactor);
  end;
end;
...
begin
  ...
  begin
    ...
    if GridViewInfo.CalculateDown and (I = MaxCount - 1) then
      Dec(AContentBottom, GridView.GetHorizontalScrollBarAreaHeight);
...
begin
  Result := APosition;
  AViewInfo := TcxCustomGridRowViewInfo(GetRealItem(ARow));
  if AViewInfo = nil then
    Exit;
  if Controller.IsPixelBasedScrollDataPos then
    if (AViewInfo.Index = 0) and (AViewInfo.Bounds.Top < ContentBounds.Top) then
      Result := Result + AViewInfo.Bounds.Top - ContentBounds.Top
    else
      for I := 0 to Count - 1 do
      begin
        AItem := Items[I];
        if (AItem = AViewInfo) or (AItem.FixedState = rfsNotFixed) then
          Break;
        if AViewInfo.Bounds.Top < AItem.Bounds.Bottom then
          Result := Result + AViewInfo.Bounds.Top - AItem.Bounds.Bottom;
      end
  else
    if (AViewInfo.FixedState = rfsNotFixed) and (GetFixedGroupsCount > 0) then
      for I := Count - 1 downto 0 do
        if (Items[I].Bounds.Top < GetFixedGroupsBottomBound) or (AViewInfo.Index = I) then
        begin
          if (Items[I].Bounds.Top < GetFixedGroupsBottomBound) then
            Result := Result + AViewInfo.Index - I - 1;
          Break;
        end;
end;
...
function TcxGridRowsViewInfo.GetCellAreaBounds(ACellViewInfo: TcxGridTableDataCellViewInfo): TRect;
begin
  if ACellViewInfo.IsMain and not GridView.RowLayout.Active then
    Result := GridViewInfo.HeaderViewInfo[ACellViewInfo.Item.VisibleIndex].GetAreaBounds
  else
    Result := inherited GetCellAreaBounds(ACellViewInfo);
end;
...
function TcxGridRowsViewInfo.GetDataRowCellsHitTestByFixedKind(const P: TPoint;
  ARowViewInfo: TcxCustomGridRowViewInfo; AKind: TcxGridColumnFixedKind): TcxCustomGridHitTest;
var
  I: Integer;
  ACellViewInfo: TcxGridDataCellViewInfoAccess;
  ADataRowViewInfo: TcxGridDataRowViewInfo absolute ARowViewInfo;
begin
  ...
  begin
    ACellViewInfo := TcxGridDataCellViewInfoAccess(ADataRowViewInfo.InternalCellViewInfos[I]);
    if (ACellViewInfo <> nil) and (ACellViewInfo.FixedKind = AKind) then
      Result := ACellViewInfo.GetHitTest(P);
    if Result <> nil then
      Exit;
  end;
end;
...
begin
  Result := HeaderViewInfo.IsScrollable or GridView.RowLayout.Active;
end;
...
function TcxGridRowsViewInfo.GetNextNonOverlappedIndex(AIndex: Integer): Integer;
begin
  for Result := AIndex + 1 to Count - 1 do
    if not Items[Result].Overlapped then
      Exit;
  Result := -1;
end;
...
function TcxGridRowsViewInfo.GetRowFocusRectBounds(ARowViewInfo: TcxCustomGridRowViewInfo): TRect;
begin
  Result := ARowViewInfo.Bounds;
  Result.Left := ARowViewInfo.DataIndent;
  if ARowViewInfo.GridRecord.IsData and (HeaderViewInfo.ItemCount[fkLeftDynamic] > 0) and
    (HeaderViewInfo.ItemCount[fkLeft] = 0) then
  begin
    Inc(Result.Left, GridViewInfo.LeftPos);
    if HeaderViewInfo.FirstItemIndex[fkLeftDynamic] > 0 then
      Dec(Result.Left, ARowViewInfo.LevelIndent);
  end;
  if GridLines <> glNone then
    Dec(Result.Right, GridLineWidth);
  Inc(Result.Top, ARowViewInfo.TopPartHeight);
  Result.Bottom := Result.Top + ARowViewInfo.DataHeight;
  if GridLines in [glBoth, glHorizontal] then
...
begin
  ...
  if (GridView.VisibleFixedColumnCount > 0) and not GridView.RowLayout.Active then
  begin
    AClientWidth := GridViewInfo.ClientWidth;
    if Result > AClientWidth then
      if (HeaderViewInfo.ItemCount[fkLeft] > 0) and (HeaderViewInfo.ItemCount[fkRight] > 0) then
        Result := AClientWidth
      else
        if HeaderViewInfo.ItemCount[fkLeft] > 0 then
          Dec(Result, GridViewInfo.LeftPos)
        else
          if HeaderViewInfo.ItemCount[fkRight] > 0 then
...
begin
  ...
  begin
    AItem := Items[I];
    if (AItem.FixedState = rfsNotFixed) and (ContentBounds.Bottom >= AItem.Bounds.Bottom) then
      Inc(FVisibleCount);
  end;
  if not GridViewInfo.CalculateDown and (AVisibleCount < VisibleCount) then
    FVisibleCount := AVisibleCount;
...
begin
  ...
  GetCellTextAreaSize(Result, ScaleFactor, False, not GridViewInfo.LookAndFeelPainter.ApplyEditorAdvancedMode);
  if GridView.OptionsView.GroupRowStyle = grsStandard then
    Result := GetCellHeight(Result)
  else
  begin
  ...
  if GridView.OptionsView.GroupRowStyle = grsOffice11 then
    Result := Max(Result, CalculateGroupImagesHeight);
  Inc(Result, cxMarginsHeight(GridViewInfo.LookAndFeelPainter.DefaultGroupContentOffsets(ScaleFactor)));
end;
...
function TcxGridRowsViewInfo.GetDataRowCellsHitTest(const P: TPoint; ARowViewInfo: TcxCustomGridRowViewInfo): TcxCustomGridHitTest;
const
  AOrderedFixedKinds: array[0..3] of TcxGridColumnFixedKind = (fkLeftDynamic, fkLeft, fkRight, fkNone);
var
  I: Integer;
...
begin
  ...
  begin
    AFixedKind := AOrderedFixedKinds[I];
    if GridViewInfo.HeaderViewInfo.ItemCount[AFixedKind] > 0 then
    begin
      Result := GetDataRowCellsHitTestByFixedKind(P, ARowViewInfo, AFixedKind);
      if Result <> nil then
        Exit;
    end;
  end;
...
function TcxGridTableViewInfo.GetFindPanelViewInfo: TcxGridTableFindPanelViewInfo;
begin
  Result := TcxGridTableFindPanelViewInfo(inherited FindPanelViewInfo);
end;

function TcxGridTableViewInfo.GetFixedSeparatorWidth: Integer;
begin
  Result := GridView.OptionsView.FixedColumnSeparatorWidth;
  if GridView.UseLightBorders and (Result > 0) and (Result < GridLineWidth) then
...
function TcxGridTableViewInfo.GetHeaderWidth: Integer;
begin
  Result := HeaderViewInfo.Width;
  if (Result = 0) and GridView.HasColumnAutoWidth then
    Result := ClientWidth;
end;
...
function TcxGridTableViewInfo.GetPainter: TcxGridTablePainter;
begin
...
begin
  ...
  if GridView.HasRowLayoutController then
    GridView.RowLayoutController.InvalidateDefaultContainer;
end;
...
//  inherited; - because of new item row view info in banded view
  ...
  if FindPanelViewInfo.IsPart then
...
var
  AAvailableWidth, ARequiredWidth: Integer;
begin
  if GridView.RowLayout.Active then
  begin
    AAvailableWidth := ClientWidth - GridView.GetVerticalScrollBarAreaWidth;
    ARequiredWidth := VisualLevelCount * LevelIndent + GridView.RowLayoutController.ContainerDefaultWidth;
    Result := Max(AAvailableWidth, ARequiredWidth);
  end
  else
    Result := HeaderWidth;
end;
...
begin
  FExpandButtonIndent := ScaleFactor.Apply(Painter.LookAndFeelPainter.DefaultGridExpandButtonIndent);
...
begin
  MainCalculate(System.Classes.Bounds(cxInvisibleCoordinate, 0, AMaxSize.X, AMaxSize.Y));
  if VisibleRecordCount = 0 then
    AHeight := GetNonRecordsAreaHeight(False) + RecordsViewInfo.DataRowHeight
  else
  begin
    AHeight := RecordsViewInfo.Items[RecordsViewInfo.Count - 1].Bounds.Bottom + PartsBottomHeight;
    if RecordsViewInfo.HasFixedDataRows then
      Inc(AHeight, RecordsViewInfo.FixedDataRowsViewInfo.GetBottomContentHeight);
    Inc(AHeight, GridView.GetHorizontalScrollBarAreaHeight);
  end;
...
procedure TcxGridTableViewInfo.CalculateParams;
begin
  FGridLineColor := GridView.OptionsView.GetGridLineColor;
  FForceIncludeRightmostBorder := GetForceIncludeRightmostBorder;
end;
...
function TcxGridTableViewInfo.CreateFindPanelViewInfo: TcxGridFindPanelViewInfo;
begin
  Result := TcxGridTableFindPanelViewInfo.Create(Self);
end;
...
function TcxGridTableViewInfo.GetBoundsForMakeItemVisible(AColumn: TcxCustomGridColumn): TRect;
var
  I, ADelta: Integer;
  AColumnHeader, ADynamicColumnHeader: TcxGridColumnHeaderViewInfo;
begin
  Result := ScrollableAreaBoundsHorz;
  for I := HeaderViewInfo.DynamicFixedItemCount - 1 downto 0 do
  begin
    ADynamicColumnHeader := HeaderViewInfo.DynamicFixedItems[I];
    ADelta := AColumn.VisibleIndex - ADynamicColumnHeader.Column.VisibleIndex;
    if ADelta > 0 then
    begin
      if ViewInfo.IsRightToLeftConverted then
        Result.Right := ADynamicColumnHeader.Bounds.Left
      else
        Result.Left := ADynamicColumnHeader.Bounds.Right;
      if ADelta > 1 then
        if ViewInfo.IsRightToLeftConverted then
        begin
          Dec(Result.Right, GetFixedSeparatorActualWidth(ADynamicColumnHeader.BorderSize[bLeft]));
          Inc(Result.Right, HeaderViewInfo[AColumn.VisibleIndex].BorderSize[bRight]);
        end
        else
        begin
          Inc(Result.Left, GetFixedSeparatorActualWidth(ADynamicColumnHeader.BorderSize[bRight]));
          Dec(Result.Left, HeaderViewInfo[AColumn.VisibleIndex].BorderSize[bLeft]);
        end
      else
        if I = 0 then
        begin
          if HeaderViewInfo.ItemCount[fkLeft] > 0 then
            if ViewInfo.IsRightToLeftConverted then
              Dec(Result.Right, HeaderViewInfo.SeparatorActualWidth[fkLeft])
            else
              Inc(Result.Left, HeaderViewInfo.SeparatorActualWidth[fkLeft]);
          AColumnHeader := HeaderViewInfo.FirstItem[fkNone];
          if ViewInfo.IsRightToLeftConverted then
            Inc(Result.Right, IfThen((bRight in AColumnHeader.HiddenBorders), AColumnHeader.BorderWidth[bRight], 0))
          else
            Dec(Result.Left, IfThen((bLeft in AColumnHeader.HiddenBorders), AColumnHeader.BorderWidth[bLeft], 0));
        end;
      Break;
    end;
  end;
...
function TcxGridTableViewInfo.CanOffset(ARecordCountDelta, APixelScrollRecordOffsetDelta, DX, DY: Integer): Boolean;
begin
  Result := inherited CanOffset(ARecordCountDelta, APixelScrollRecordOffsetDelta, DX, DY) and
    ((GridView.VisibleFixedColumnCount = 0) and (GridView.VisibleColumnCountByFixedKind[fkLeftDynamic] = 0) or (DX = 0));
end;
...
begin
  ...
  begin
    ...
    Result := Screen.DesktopHeight div RecordsViewInfo.DataRowHeight + 2;
end;
...
function TcxGridTableViewInfo.GetFixedHighlightColor: TdxAlphaColor;
var
  AAlpha: Byte;
begin
  if GridView.OptionsView.FixedColumnHighlightColor <> TdxAlphaColors.Default then
  begin
    AAlpha := Min(dxGetAlpha(GridView.OptionsView.FixedColumnHighlightColor), GetFixedHighlightColorMaxAlpha);
    Result := TdxAlphaColors.FromArgb(AAlpha, GridView.OptionsView.FixedColumnHighlightColor);
  end
  else
    Result := LookAndFeelPainter.DefaultFixedColumnHighlightColor;
end;

function TcxGridTableViewInfo.GetFixedHighlightColorMaxAlpha: Byte;
begin
  Result := 150;
end;

function TcxGridTableViewInfo.GetFixedSeparatorActualWidth(ANeighborBorderWidth: Integer): Integer;
begin
  Result := Max(FixedSeparatorWidth - ANeighborBorderWidth, 0);
end;

function TcxGridTableViewInfo.GetFixedSeparatorColor: TColor;
begin
  if GridView.OptionsView.FixedColumnSeparatorColor <> clDefault then
    Result := GridView.OptionsView.FixedColumnSeparatorColor
  else
...
begin
  if GridView.UseLightBorders then
    Result := GridView.BordersScaleFactor.Apply(1)
  else
    Result := 1;
end;
...
begin
  ...
  if (AAutoHeight = eahEditor) and not GridView.RowLayout.Active then 
...
begin
  ...
  if not GridView.IsInplaceEditFormVisible then
  begin
    Result.Left := ScrollableAreaBoundsHorz.Left;
    Result.Right := ScrollableAreaBoundsHorz.Right;
  end;
  if (Controller.FocusedRow <> nil) and not Controller.FocusedRow.IsSpecial then
  begin
    if RecordsViewInfo.HasFilterRow then
    ...
    if RecordsViewInfo.HasFixedDataRows then
      if Controller.FocusedRow.FixedState <> rfsFixedToBottom then
      begin
        Dec(Result.Bottom, RecordsViewInfo.FixedDataRowsViewInfo.GetBottomContentHeight);
        if Controller.FocusedRow.FixedState = rfsNotFixed then
...
begin
  ...
  if HeaderViewInfo.ItemCount[fkLeft] > 0 then
    if IsRightToLeftConverted then
      Result.Right := Min(Result.Right, HeaderViewInfo.GetLeftFixedAreaLastBound)
    else
      Result.Left := Max(Result.Left, HeaderViewInfo.GetLeftFixedAreaLastBound);
  if HeaderViewInfo.ItemCount[fkRight] > 0 then
    if IsRightToLeftConverted then
      Result.Left := Max(Result.Left, HeaderViewInfo.GetRightFixedAreaFirstBound)
    else
      Result.Right := Min(Result.Right, HeaderViewInfo.GetRightFixedAreaFirstBound);
end;
...
begin
  Result := DataWidth;
  if NeedIncludeVerticalScrollBarAreaInScrollContent then
...
function TcxGridTableViewInfo.IsCellPartVisible(ACell: TcxGridTableDataCellViewInfo): Boolean;
var
  AFinishBound: Integer;
  ARow: TcxGridDataRowViewInfo;
begin
  Result := inherited IsCellPartVisible(ACell) or GridView.IsFixedGroupsMode and Controller.IsRecordPixelScrolling and
    not Controller.FocusedRow.IsSpecial and (RecordsViewInfo.GetFixedGroupsBottomBound > ACell.EditBounds.Top);
  if Result then Exit;
  ARow := TcxGridDataRowViewInfo(Controller.FocusedRecord.ViewInfo);
  Result := GridView.IsInplaceEditFormVisible and (ARow <> nil) and ARow.IsInplaceEditFormCellPartVisible(ACell);
  if not Result and (HeaderViewInfo.ItemCount[fkLeftDynamic] > 0) then
  begin
    AFinishBound := HeaderViewInfo.GetDynamicFixedAreaLastBound;
    if IsRightToLeftConverted then
      Result := (ACell.Bounds.Right > AFinishBound) and (ACell.EditBounds.Right > AFinishBound) 
    else
      Result := (ACell.Bounds.Left < AFinishBound) and (ACell.EditBounds.Left < AFinishBound); 
  end;
end;

function TcxGridTableViewInfo.NeedIncludeVerticalScrollBarAreaInScrollContent: Boolean;
begin
  Result := ((HeaderViewInfo.Width <> 0) or not GridView.HasColumnAutoWidth) and (HeaderViewInfo.ItemCount[fkRight] = 0);
end;
...
begin
  Result := GridView.VisibleFixedColumnCount = 0;
end;
...
function TcxGridTableViewInfo.GetFooterViewInfoSpecificClass: TcxGridFooterViewInfoSpecificClass;
begin
  Result := TcxGridFooterViewInfoSpecific;
end;

function TcxGridTableViewInfo.GetForceIncludeRightmostBorder: Boolean;
begin
  Result := GridView.OptionsView.ColumnAutoWidth and
    Assigned(GridView.LookAndFeel) and (GridView.LookAndFeel.ScrollbarMode = sbmHybrid) and
    Assigned(GridView.Site) and Assigned(GridView.Site.VScrollBar) and GridView.Site.VScrollBar.Enabled;
end;
...
function TcxGridTableViewInfo.GetCellLineIndex(AIndex: Integer): Integer;
begin
  Result := 0;
end;
...
begin
  ...
  try
    ...
    if GridRecord.GetInternalActiveDetailGridView(ADetailView) then
    begin
      if ADetailView.ViewInfoCache <> nil then
        ADetailView.ViewInfoCache.UnassignValues(AKeepMaster);
    end;
  finally
...
procedure TcxGridColumnStyles.GetDefaultViewParams(Index: Integer; AData: TObject; out AParams: TcxViewParams);
begin
  case Index of
    isFooter:
      GridView.Styles.GetFooterParams(
        TcxGridRowFooterCellPos(AData).Row, Item,
        TcxGridRowFooterCellPos(AData).FooterGroupLevel,
        TcxGridRowFooterCellPos(AData).SummaryItem, AParams);

    isGroupSummary:
      GridView.Styles.GetGroupSummaryParams(
        TcxGridGroupSummaryInfo(AData).Row,
...
begin
  ...
  FVisibleForRowLayout := bDefault;
end;

destructor TcxCustomGridColumn.Destroy;
begin
  RowLayoutItem := nil;
  inherited Destroy;
end;
...
procedure TcxCustomGridColumn.SetFixedKind(Value: TcxGridColumnFixedKind);
var
  APrevFixedKind: TcxGridColumnFixedKind;
begin
  if (FFixedKind <> Value) and GridView.SupportsColumnFixing then
  begin
    APrevFixedKind := FFixedKind;
    FFixedKind := Value;
    GridView.ColumnFixedKindChanged(FFixedKind, APrevFixedKind);
  end;
end;
...
procedure TcxCustomGridColumn.SetRowLayoutItem(const Value: TcxGridTableRowLayoutItem);
begin
  if FRowLayoutItem <> Value then
  begin
    if FRowLayoutItem <> nil then
    begin
      if FRowLayoutItem.IsContainerRestoring then
        FRowLayoutItem.RemoveFreeNotification(Self)
      else
        FRowLayoutItem.Free;
    end;
    FRowLayoutItem := Value;
    if FRowLayoutItem <> nil then
    begin
      FRowLayoutItem.GridViewItem := Self;
      FRowLayoutItem.FreeNotification(Self);
    end;
  end;
end;
...
procedure TcxCustomGridColumn.SetVisibleForRowLayout(AValue: TdxDefaultBoolean);
begin
  if AValue <> FVisibleForRowLayout then
  begin
    FVisibleForRowLayout := AValue;
    CheckAccessibilityForRowLayout;
    Changed(ticProperty);
  end;
end;
...
begin
  ...
  if Operation = opRemove then
    if AComponent = FLayoutItem then
      FLayoutItem := nil
    else
      if AComponent = FRowLayoutItem then
        FRowLayoutItem := nil;
end;
...
function TcxCustomGridColumn.CreateUIAProvider: IdxUIAProvider;
begin
  Result := dxUIACreateGridTableViewColumnHeaderProvider(Self);
end;
...
begin
  if GridView.HasColumnAutoWidth then
    GridView.ViewInfo.HeaderViewInfo.AssignColumnWidths;
end;
...
begin
  Result := GridView.InplaceEditForm.Container.CanCreateLayoutItem(Self);
end;

function TcxCustomGridColumn.CanCreateRowLayoutItem: Boolean;
begin
  Result := GridView.RowLayoutController.Container.CanCreateLayoutItem(Self);
end;

function TcxCustomGridColumn.CanDataCellScroll: Boolean;
begin
  Result := inherited CanDataCellScroll or GridView.IsInplaceEditFormVisible;
end;
...
begin
  Result := (not GridView.IsInplaceEditFormVisible or AVisually) and inherited CanFilter(AVisually);
end;

function TcxCustomGridColumn.CanFilterMRUValueItems: Boolean;
begin
  Result := not GridView.IsInplaceEditFormVisible and inherited CanFilterMRUValueItems;
end;

function TcxCustomGridColumn.CanFilterUsingChecks: Boolean;
begin
  Result := not GridView.IsInplaceEditFormVisible and inherited CanFilterUsingChecks;
end;
...
begin
  Result := not GridView.IsInplaceEditFormVisible and inherited CanGroup;
end;
...
begin
  Result := not GridView.IsInplaceEditFormVisible and inherited CanIncSearch;
end;

function TcxCustomGridColumn.CanScroll: Boolean;
begin
  Result := inherited CanScroll and not GridView.IsInplaceEditFormVisible and
    (GridView.RowLayout.Active or (VisibleIndex <> -1) and
    (GridView.ViewInfo.HeaderViewInfo[VisibleIndex].FixedKind = fkNone));
end;

function TcxCustomGridColumn.CanSearchInGroupRows: Boolean;
begin
  Result := GridView.FindPanel.SearchInGroupRows and Options.FilteringWithFindPanel and (GroupIndex > -1);
end;
...
begin
  Result := not GridView.IsInplaceEditFormVisible and inherited CanSort;
end;
...
begin
  ...
  if RowLayoutItem <> nil then
    RowLayoutItem.Caption := Caption;
...
begin
  if IsLoading or IsDestroying then
    Exit;
  if GridView.EditForm.UseDefaultLayout then
    GridView.InplaceEditForm.Container.RecreateLayout
  else
...
procedure TcxCustomGridColumn.CheckAccessibilityForRowLayout;
begin
  if IsLoading or IsDestroying then
    Exit;
  if GridView.RowLayout.UseDefaultLayout then
    GridView.RowLayoutController.Container.RecreateLayout
  else
    if not IsVisibleForRowLayout then
      DestroyRowLayoutItem
    else
      if CanCreateRowLayoutItem then
        CreateRowLayoutItem;
end;

procedure TcxCustomGridColumn.CheckUsingInFindCriteria;
begin
  if IsDestroying then
    Exit;
  inherited CheckUsingInFindCriteria;
  if CanSearchInGroupRows then
    DataController.FindCriteria.AddGroupItem(Index)
  else
    DataController.FindCriteria.RemoveGroupItem(Index);
end;
...
procedure TcxCustomGridColumn.CreateLayoutItem;
begin
  GridView.InplaceEditForm.Container.CreateLayoutItem(Self);
end;

procedure TcxCustomGridColumn.CreateRowLayoutItem;
begin
  GridView.RowLayoutController.Container.CreateLayoutItem(Self);
end;
...
procedure TcxCustomGridColumn.DestroyRowLayoutItem;
begin
  RowLayoutItem := nil;
end;
...
begin
  if Controller.IsFilterRowFocused then
  begin
    if (GridView.FilterRow.ApplyChanges = fracOnCellExit) or Controller.EditingController.HasImmediateFilterValue then
    begin
      AValue := Value;
      if UseBeginWithMask then
        Controller.AddBeginsWithMask(AValue);
      ViewData.FilterRow.Values[Index] := AValue;
      Controller.EditingController.HasImmediateFilterValue := False;
    end;
...
function TcxCustomGridColumn.GetAutoWidthSizable: Boolean;
begin
  Result := Options.AutoWidthSizable and not IsFixedByWidth;
end;
...
function TcxCustomGridColumn.IsDefaultVisibleForEditForm: Boolean;
begin
  Result := Visible;
end;

function TcxCustomGridColumn.IsDefaultVisibleForRowLayout: Boolean;
begin
  Result := Visible;
end;
...
function TcxCustomGridColumn.IsRowLayoutItemStored: Boolean;
begin
  Result := not GridView.RowLayout.UseDefaultLayout and (RowLayoutItem <> nil);
end;

function TcxCustomGridColumn.IsSearchable: Boolean;
begin
  if IsPreview then
    Result := GridView.FindPanel.SearchInPreview and GridView.Preview.Visible
  else
    if GridView.RowLayout.Active then
      Result := (RowLayoutItem <> nil) and not RowLayoutItem.IsAvailable
    else
      Result := inherited IsSearchable;
end;
...
begin
  Result := dxDefaultBooleanToBoolean(VisibleForEditForm, IsDefaultVisibleForEditForm) and not IsPreview;
end;

function TcxCustomGridColumn.IsVisibleForFocus(ARecord: TcxCustomGridRecord): Boolean;
begin
  Result := inherited IsVisibleForFocus(ARecord) or (ARecord <> nil) and ARecord.IsData and
    TcxGridDataRow(ARecord).EditFormVisible and (LayoutItem <> nil) and LayoutItem.ActuallyVisible or
    GridView.RowLayout.Active and (RowLayoutItem <> nil) and RowLayoutItem.ActuallyVisible;
end;

function TcxCustomGridColumn.IsVisibleForRowLayout: Boolean;
begin
  Result := dxDefaultBooleanToBoolean(VisibleForRowLayout, IsDefaultVisibleForRowLayout) and not IsPreview;
end;

function TcxCustomGridColumn.NeedFixedHighlight: Boolean;
begin
  Result := (FixedKind <> fkNone) and GridView.OptionsView.HighlightFixedColumns and
    (GridView.OptionsView.FixedColumnHighlightColor <> TdxAlphaColors.Empty);
end;
...
begin
  Result := Options.ShowGroupValuesWithImages and
    Safe.Cast<TcxImageComboBoxProperties>(GetProperties, AProperties) and (AProperties.Images <> nil);
end;
...
function TcxCustomGridColumn.TryResetFocusToNext: Boolean;
begin
  if GridView.RowLayout.Active then
    Result := not GridView.IsAssigningItems and Controller.FocusNextItem(Index, True, True, False,
      not Controller.IsMultiSelectPersistent)
  else
    Result := inherited TryResetFocusToNext;
end;
...
procedure TcxGridColumn.Assign(Source: TPersistent);
  ...
  function FindLayoutItem(AGridColumn: TcxGridColumn; AContainer: TcxGridCustomLayoutContainer): TcxGridCustomLayoutItem;
  var
    I: Integer;
  begin
    for I := 0 to AContainer.AbsoluteItemCount - 1 do
      if AContainer.AbsoluteItems[I] is TcxGridCustomLayoutItem then
      begin
        Result := TcxGridCustomLayoutItem(AContainer.AbsoluteItems[I]);
        if Result.GridViewItem = AGridColumn then
          Exit;
      end;
    Result := nil;
  end;
...
var
  AGridColumn: TcxGridColumn;
  ALayoutItem: TcxGridCustomLayoutItem;
begin
  if Source is TcxGridColumn then
  begin
    AGridColumn := TcxGridColumn(Source);
    FixedKind := AGridColumn.FixedKind;
    FooterAlignmentHorz := AGridColumn.FooterAlignmentHorz;
    GroupSummaryAlignment := AGridColumn.GroupSummaryAlignment;
    HeaderGlyph := AGridColumn.HeaderGlyph;
    HeaderGlyphAlignmentHorz := AGridColumn.HeaderGlyphAlignmentHorz;
    HeaderGlyphAlignmentVert := AGridColumn.HeaderGlyphAlignmentVert;
    HeaderImageIndex := AGridColumn.HeaderImageIndex;
    InternalSetFilterRowOperator(AGridColumn.InternalGetFilterRowOperator);
    Summary := AGridColumn.Summary;
    VisibleForEditForm := AGridColumn.VisibleForEditForm;
    VisibleForRowLayout := AGridColumn.VisibleForRowLayout;
    IsChildInMergedGroup := AGridColumn.IsChildInMergedGroup;
    OnCompareRowValuesForCellMerging := AGridColumn.OnCompareRowValuesForCellMerging;
    OnCompareValuesForCellMerging := AGridColumn.OnCompareValuesForCellMerging;
    OnCustomDrawFooterCell := AGridColumn.OnCustomDrawFooterCell;
    OnCustomDrawGroupSummaryCell := AGridColumn.OnCustomDrawGroupSummaryCell;
    OnCustomDrawHeader := AGridColumn.OnCustomDrawHeader;
    OnHeaderClick := AGridColumn.OnHeaderClick;
    ALayoutItem := FindLayoutItem(AGridColumn, GridView.InplaceEditForm.Container);
    if ALayoutItem <> nil then
      LayoutItem := TcxGridInplaceEditFormLayoutItem(ALayoutItem);
    ALayoutItem := FindLayoutItem(AGridColumn, GridView.RowLayoutController.Container);
    if ALayoutItem <> nil then
      RowLayoutItem := TcxGridTableRowLayoutItem(ALayoutItem);
  end;
  inherited Assign(Source);
end;
...
procedure TcxGridColumn.ChangeGrouping(AGroupIndex: Integer; AMergeWithLeftColumn: Boolean = False;
  AMergeWithRightColumn: Boolean = False);
begin
  inherited ChangeGrouping(AGroupIndex, AMergeWithLeftColumn, AMergeWithRightColumn);
  CheckUsingInFindCriteria;
end;

function TcxGridColumn.GetIsFixedByWidth: Boolean;
begin
  Result := inherited GetIsFixedByWidth or (Controller.ForcingWidthItem <> nil) and
...
function TcxGridColumn.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;
...
begin
  if (ADefaultIndex = NBDI_FILTER) and GridView.IsInplaceEditFormVisible then
    Result := False
  else
...
procedure TcxGridTableFindPanelOptions.Assign(Source: TPersistent);
var
  AOptions: TcxGridTableFindPanelOptions absolute Source;
begin
  inherited Assign(Source);
  if Source is TcxGridTableFindPanelOptions then
  begin
    Location := AOptions.Location;
    SearchInGroupRows := AOptions.SearchInGroupRows;
    SearchInPreview := AOptions.SearchInPreview;
  end;
end;

function TcxGridTableFindPanelOptions.GetGridView: TcxGridTableView;
begin
  Result := TcxGridTableView(inherited GridView);
end;

procedure TcxGridTableFindPanelOptions.SetLocation(const AValue: TcxGridTableFindPanelLocation);
begin
  if AValue <> Location then
  begin
    FLocation := AValue;
    GridView.Controller.FindPanel.UpdateEdit;
    Changed(vcSize);
  end;
end;

procedure TcxGridTableFindPanelOptions.SetSearchInGroupRows(AValue: Boolean);
var
  I: Integer;
begin
  if AValue <> SearchInGroupRows then
  begin
    FSearchInGroupRows := AValue;
    GridView.BeginUpdate;
    try
      for I := 0 to GridView.GroupedColumnCount - 1 do
        GridView.GroupedColumns[I].CheckUsingInFindCriteria;
    finally
      GridView.EndUpdate;
    end;
  end;
...
procedure TcxGridTableFindPanelOptions.SetSearchInPreview(AValue: Boolean);
begin
  if AValue <> SearchInPreview then
  begin
    FSearchInPreview := AValue;
    if GridView.Preview.Column <> nil then
      GridView.Preview.Column.CheckUsingInFindCriteria;
  end;
end;
...
begin
  ...
  begin
    if GridView.InplaceEditForm.Visible and not GridView.InplaceEditForm.Close then
      Exit;
    FEditMode := AValue;
    if EditMode = emModalEditForm then
      GridView.InplaceEditForm.DisplayMode := efdmModal
    else
      GridView.InplaceEditForm.DisplayMode := efdmInplace;
    if EditMode = emInplace then
...
function TcxGridTableFiltering.GetColumnFilteredItemsListShowFilteredItemsOnly: Boolean;
begin
  Result := ItemFilteredItemsListShowFilteredItemsOnly;
end;
...
procedure TcxGridTableFiltering.SetColumnFilteredItemsListShowFilteredItemsOnly(AValue: Boolean);
begin
  ItemFilteredItemsListShowFilteredItemsOnly := AValue;
end;
...
function TcxGridTableOptionsCustomize.GetColumnExpressionEditing: Boolean;
begin
  Result := ItemExpressionEditing;
end;
...
procedure TcxGridTableOptionsCustomize.SetColumnExpressionEditing(Value: Boolean);
begin
  ItemExpressionEditing := Value;
end;
...
begin
  ...
  FFixedColumnHighlightColor := TdxAlphaColors.Default;
  FFixedColumnSeparatorColor := clDefault;
  FFixedColumnSeparatorWidth := cxGridDefaultFixedColumnSeparatorWidth;
...
begin
  if not (GridView.AssigningPattern or
    (Assigned(GridView.Control) and TcxCustomGridAccess(GridView.Control).IsScaleChanging)) then
...
procedure TcxGridTableOptionsView.SetFixedColumnHighlightColor(Value: TdxAlphaColor);
begin
  if FFixedColumnHighlightColor <> Value then
  begin
    FFixedColumnHighlightColor := Value;
    Changed(vcSize);
  end;
end;

procedure TcxGridTableOptionsView.SetFixedColumnSeparatorColor(Value: TColor);
begin
  if FFixedColumnSeparatorColor <> Value then
  begin
    FFixedColumnSeparatorColor := Value;
    Changed(vcSize);
  end;
end;

procedure TcxGridTableOptionsView.SetFixedColumnSeparatorWidth(Value: Integer);
begin
  Value := Max(Value, 0);
  if FFixedColumnSeparatorWidth <> Value then
  begin
    FFixedColumnSeparatorWidth := Value;
    Changed(vcSize);
  end;
end;
...
procedure TcxGridTableOptionsView.SetHeaderFilterButtonShowMode(Value: TcxGridItemFilterButtonShowMode);
begin
...
procedure TcxGridTableOptionsView.SetHighlightFixedColumns(AValue: Boolean);
begin
  if HighlightFixedColumns <> AValue then
  begin
    FHighlightFixedColumns := AValue;
    Changed(vcSize);
  end;
end;
...
begin
  inherited;
  FixedColumnSeparatorWidth := MulDiv(FixedColumnSeparatorWidth, M, D);
...
begin
  if Source is TcxGridTableOptionsView then
    ...
    begin
      ...
      Self.FixedColumnHighlightColor := FixedColumnHighlightColor;
      Self.FixedColumnSeparatorColor := FixedColumnSeparatorColor;
      Self.FixedColumnSeparatorWidth := FixedColumnSeparatorWidth;
      Self.Footer := Footer;
      ...
      Self.HighlightFixedColumns := HighlightFixedColumns;
      Self.Indicator := Indicator;
...
procedure TcxGridPreview.SetColumn(Value: TcxGridColumn);
  ...
  begin
    AColumn.CheckAccessibilityForEditForm;
    AColumn.CheckAccessibilityForRowLayout;
    AColumn.CheckUsingInFindCriteria;
...
begin
  if (Value <> nil) and (Value.GridView <> GridView) then
    Value := nil;
  ...
  begin
    ...
    try
      if FColumn <> nil then
      begin
        FColumn.FIsPreview := False;
        ColumnVisibilityChanged(FColumn);
      end;
      FColumn := Value;
      if FColumn <> nil then
      begin
        FColumn.FIsPreview := True;
        ColumnVisibilityChanged(FColumn);
      end;
    finally
      GridView.EndUpdate;
...
begin
  ...
  begin
    FVisible := Value;
    if Column <> nil then
      Column.CheckUsingInFindCriteria;
    Changed(vcSize);
  end;
end;
...
begin
  ...
  begin
    ...
    GetCellTextAreaSize(Result, AGridViewInfo.ScaleFactor, False);
...
begin
  ...
  with AParams, LookAndFeelPainter do
    case Index of
      ...
      vsInplaceEditFormItem, vsInplaceEditFormItemHotTrack:
      begin
        if Index = vsInplaceEditFormItemHotTrack then
          AState := cxbsHot
        else
          AState := cxbsNormal;
        TextColor := DefaultLayoutViewContentTextColor(AState);
      end;
      vsInplaceEditFormGroup, vsRowLayoutGroup:
        begin
          TextColor := clDefault;
          Color := clDefault;
        end;
      vsContent:
        if GridView.RowLayout.Active then
          TextColor := DefaultDataRowLayoutContentTextColor(cxbsNormal);
      vsHotTrack:
        if GridView.RowLayout.Active then
          TextColor := DefaultDataRowLayoutContentTextColor(cxbsHot);
      vsSelection, vsInactive:
      begin
        if Index = vsInactive then
          AState := cxbsDisabled
        else
          AState := cxbsPressed;
        if IsInplaceEditFormItemSelectionParams then
          TextColor := DefaultLayoutViewContentTextColor(AState)
        else
          if GridView.RowLayout.Active then
            TextColor := DefaultDataRowLayoutContentTextColor(AState);
      end;
    end;
end;

procedure TcxGridTableViewStyles.GetRowLayoutGroupParams(out AParams: TcxViewParams);
begin
  GetViewParams(vsRowLayoutGroup, nil, nil, AParams);
end;

procedure TcxGridTableViewStyles.GetRowLayoutItemParams(ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem;
  out AParams: TcxViewParams; AUseViewInfo: Boolean = False; ACellViewInfo: TcxGridTableCellViewInfo = nil);
begin
  GetDataCellParams(ARecord, AItem, AParams, AUseViewInfo, ACellViewInfo);
end;

function TcxGridTableViewStyles.IsOddEvenStylesSupported: Boolean;
begin
  Result := inherited IsOddEvenStylesSupported and ((GridView <> nil) and
    (GridView.RowLayout <> nil) and not GridView.RowLayout.Active);
end;
...
begin
  AItem := ASummaryItem;
  if AItem = nil then
    AItem := ARow.GroupSummaryItems;
  if GridView.DrawDataCellSelected(ARow, nil, True) then
    GetSelectionParams(ARow, AItem, AParams)
  else
    if GridView.DrawDataCellHotTracked(ARow, nil) then
      GetHotTrackParams(ARow, AItem, AParams)
    else
...
begin
  ...
  if (ARecord <> nil) and GridView.DrawRecordFocused(ARecord) and (AItem <> nil) and
    AItem.Focused and GridView.EditForm.ItemHotTrack then
  begin
    FIsInplaceEditFormItemSelectionParams := True;
    try
      GetSelectionParams(ARecord, AItem, AParams)
    finally
      FIsInplaceEditFormItemSelectionParams := False;
    end;
  end
  else
  begin
...
begin
  FreeAndNil(FDataViewStateStream);
  inherited Destroy;
end;
...
function TcxGridTableView.GetFindPanel: TcxGridTableFindPanelOptions;
begin
  Result := TcxGridTableFindPanelOptions(inherited FindPanel);
end;
...
begin
  if not HasInplaceEditForm then
...
function TcxGridTableView.GetRowLayoutController: TcxGridTableRowLayoutController;
begin
  if not HasRowLayoutController then
    FRowLayoutController := GetRowLayoutControllerClass.Create(Self);
  Result := FRowLayoutController;
end;
...
function TcxGridTableView.GetVisibleColumnCountByFixedKind(AKind: TcxGridColumnFixedKind): Integer;
begin
  case AKind of
    fkLeft:
      Result := FVisibleLeftFixedColumnCount;
    fkRight:
      Result := FVisibleRightFixedColumnCount;
    fkLeftDynamic:
      Result := FVisibleDynamicFixedColumnCount;
    else
      Result := VisibleColumnCount - FVisibleLeftFixedColumnCount - FVisibleRightFixedColumnCount -
        FVisibleDynamicFixedColumnCount;
  end;
end;

function TcxGridTableView.GetVisibleFixedColumnCount: Integer;
begin
  Result := VisibleColumnCountByFixedKind[fkLeft] + VisibleColumnCountByFixedKind[fkRight];
end;

function TcxGridTableView.GetVisibleScrollableColumnCount: Integer;
begin
  Result := VisibleColumnCount - VisibleFixedColumnCount;
end;

procedure TcxGridTableView.SetAutomation(const AValue: TdxGridTableViewAutomationSettings);
begin
  inherited Automation := AValue;
end;
...
procedure TcxGridTableView.SetFindPanel(AValue: TcxGridTableFindPanelOptions);
begin
  inherited FindPanel := AValue;
end;
...
procedure TcxGridTableView.SetRowLayout(AValue: TcxGridRowLayoutOptions);
begin
  FRowLayout.Assign(AValue);
end;
...
procedure TcxGridTableView.SetOnDetachedEditFormInitialize(Value: TcxGridTableViewDetachedEditFormEvent);
begin
  if not dxSameMethods(FOnDetachedEditFormInitialize, Value) then
  begin
    FOnDetachedEditFormInitialize := Value;
    Changed(vcProperty);
  end;
end;
...
begin
  ...
  begin
    ...
    if OptionsCustomize.DataRowSizing then
      Add('DataRowHeight');
    if (gsoUseDataViewState in StorageOptions) then
      Add('DataViewState');
    if OptionsCustomize.GroupRowSizing then
      Add('GroupRowHeight');
    Add('EditFormUseDefaultLayout');
    Add('RowUseDefaultLayout');
...
procedure TcxGridTableView.GetPropertyValue(const AName: string; var AValue: Variant);
  ...
  function GetDataViewStateAsString: string;
  var
    AStream: TStringStream;
  begin
    AStream := TStringStream.Create;
    try
      StoreDataViewStateToStream(AStream, StorageOptions);
...
begin
  if AName = 'DataViewState' then
    AValue := GetDataViewStateAsString
  else
          ...
          if AName = 'DataRowHeight' then
            ...
            if AName = 'GroupRowHeight' then
              ...
              if AName = 'EditFormUseDefaultLayout' then
                ...
                if AName = 'RowUseDefaultLayout' then
                  AValue := RowLayout.UseDefaultLayout
                else
...
procedure TcxGridTableView.SetPropertyValue(const AName: string; const AValue: Variant);
  ...
  procedure SetDataViewStateAsString(const AValue: string);
  var
    AStream: TStringStream;
  begin
    AStream := TStringStream.Create(AValue);
    try
      RestoreDataViewStateFromStream(AStream, StorageOptions);
    finally
      AStream.Free;
    end;
  end;
...
begin
  if (gsoUseDataViewState in StorageOptions) and (AName = 'DataViewState') then
    SetDataViewStateAsString(AValue)
  else
  if AName = 'Footer' then
          ...
          if (AName = 'DataRowHeight') and OptionsCustomize.DataRowSizing then
            ...
            if (AName = 'GroupRowHeight') and OptionsCustomize.GroupRowSizing then
              ...
              if AName = 'EditFormUseDefaultLayout' then
                ...
                if AName = 'RowUseDefaultLayout' then
                  RowLayout.UseDefaultLayout := AValue
                else
                  if AName = 'ColumnsQuickCustomizationSorted' then
...
begin
  ...
  if not RowLayout.UseDefaultLayout then
    AChildren.AddObject('RowLayout', RowLayoutController.Container);
end;
...
begin
  Result := 'Personalización de columnas...';  //..
end;
...
function TcxGridTableView.GetAutomation: TdxGridTableViewAutomationSettings;
begin
  Unsafe.Cast(inherited Automation, Result);
end;

function TcxGridTableView.GetAutomationSettingsClass: TdxAutomationElementSettingsClass;
begin
  Result := TdxGridTableViewAutomationSettings;
end;
...
begin
  ...
  FEditForm := GetEditFormClass.Create(Self);
  FRowLayout := GetRowLayoutClass.Create(Self);
end;

function TcxGridTableView.CreateUIAProvider: IdxUIAProvider;
begin
  Result := dxUIACreateGridTableViewProvider(Self);
end;

procedure TcxGridTableView.DestroyOptions;
begin
  FreeAndNil(FRowLayout);
...
procedure TcxGridTableView.Notification(AComponent: TComponent; AOperation: TOperation);
begin
  inherited;
  if AOperation = opRemove then
  begin
    if (Preview <> nil) and (Preview.Column = AComponent) then
      Preview.Column := nil;
  end;
end;
...
begin
  ...
  if HasInplaceEditForm then
    InplaceEditForm.UpdateLayoutLookAndFeel;
  if HasRowLayoutController then
    RowLayoutController.UpdateLayoutLookAndFeel;
end;

procedure TcxGridTableView.AfterRestoring;
begin
  InplaceEditForm.AfterRestoring;
  RowLayoutController.AfterRestoring;
  inherited;
end;

procedure TcxGridTableView.BeforeRestoring;
begin
  RowLayoutController.BeforeRestoring;
...
begin
  ...
  if (dceEdit in DataController.EditState) and (OptionsBehavior.EditMode in [emInplaceEditForm,
    emInplaceEditFormHideCurrentRow]) and Controller.FocusedRow.IsData then
      TcxGridDataRow(Controller.FocusedRow).EditFormVisible := True;
end;

function TcxGridTableView.CanCellMerging: Boolean;
begin
  Result := AllowCellMerging and not FPreview.Active and not RowLayout.Active and (OptionsView.RowSeparatorWidth = 0);
end;
...
begin
  Result := not IsUpdateLocked and (not IsMaster and not IsInplaceEditFormVisible or (GroupedColumnCount = 0));
end;

procedure TcxGridTableView.CheckVisibleDynamicFixedColumnCount;
var
  I: Integer;
begin
  for I := 0 to VisibleColumnCount - 1 do
    if VisibleColumns[I].FixedKind = fkLeftDynamic then
      Inc(FVisibleDynamicFixedColumnCount);
end;

procedure TcxGridTableView.ColumnFixedKindChanged(AKind, APrevKind: TcxGridColumnFixedKind);
var
  AIsFixed, AWasFixed: Boolean;
begin
  AWasFixed := APrevKind in [fkLeft, fkRight];
  AIsFixed := AKind in [fkLeft, fkRight];
  if AIsFixed and not AWasFixed then
    Inc(FFixedColumnCount)
  else if AWasFixed and not AIsFixed then
    Dec(FFixedColumnCount);
  if AKind = fkLeftDynamic  then
    Inc(FDynamicFixedColumnCount)
  else if APrevKind = fkLeftDynamic then
    Dec(FDynamicFixedColumnCount);
  RefreshVisibleItemsList;
  Changed(vcSize);
end;

function TcxGridTableView.CreateFindPanel: TcxGridFindPanelOptions;
begin
  Result := TcxGridTableFindPanelOptions.Create(Self);
end;
...
begin
  if ASource is TcxGridTableView then
    ...
    begin
      ...
      Self.OnDetachedEditFormInitialize := OnDetachedEditFormInitialize;
      if not Self.AssigningSettings then
      begin
        Self.InplaceEditForm.AssignStructure(InplaceEditForm);
        Self.RowLayoutController.AssignStructure(RowLayoutController);
      end;
    end;
...
procedure TcxGridTableView.FocusedItemChanged(APrevFocusedItem, AFocusedItem: TcxCustomGridTableItem);
begin
  inherited FocusedItemChanged(APrevFocusedItem, AFocusedItem);
  UIANotify(TdxUIAGridNotification.FocusedCellChanged);
end;

function TcxGridTableView.GetLastAutoWidthSizableColumnVisibleIndex: Integer;
var
  I: Integer;
begin
  Result := -1;
  for I := VisibleColumnCount - 1 downto 0 do
    if VisibleColumns[I].AutoWidthSizable then
      Exit(I);
end;
...
begin
  Result := inherited GetIsControlFocused or
    (Controller.HasFilterRowOperatorMenu and Controller.FilterRowOperatorMenu.Visible) or
    (Controller.HasDataRowFixingMenu and Controller.DataRowFixingMenu.Visible) or
    ((OptionsBehavior.EditMode = emModalEditForm) and IsInplaceEditFormVisible);
end;
...
begin
  Result := inherited GetResizeOnBoundsChange or HasColumnAutoWidth or Preview.Active or IsMaster;
end;
...
begin
  if IsInplaceEditFormVisible and (InplaceEditForm.Container <> nil) and
...
function TcxGridTableView.HasColumnAutoWidth: Boolean;
begin
  Result := OptionsView.ColumnAutoWidth or RowLayout.Active;
end;

function TcxGridTableView.HasInplaceEditForm: Boolean;
begin
  Result := FInplaceEditForm <> nil;
end;

function TcxGridTableView.HasRowLayoutController: Boolean;
begin
  Result := FRowLayoutController <> nil;
end;

procedure TcxGridTableView.Init;
begin
  if HasInplaceEditForm or not EditForm.UseDefaultLayout then
    InplaceEditForm.Init;
  if HasRowLayoutController or not RowLayout.UseDefaultLayout or RowLayout.Active then
    RowLayoutController.Init;
  inherited Init;
end;
...
function TcxGridTableView.IsInplaceEditFormVisible: Boolean;
begin
  Result := HasInplaceEditForm and InplaceEditForm.Visible;
end;
...
procedure TcxGridTableView.NavigatorEditExecute;
var
  ARow: TcxCustomGridRow;
begin
  inherited NavigatorEditExecute;
  if (OptionsBehavior.EditMode = emModalEditForm) and (dceEdit in DataController.EditState) then
  begin
    ARow := Controller.FocusedRow;
    if ARow.IsData and not ARow.IsSpecial then
      TcxGridDataRow(ARow).EditFormVisible := True;
  end;
end;
...
var
  I: Integer;
  AItem: TcxGridIndicatorHeaderItemViewInfo;
  APreviousCheckBoxState: TcxCheckBoxState;
begin
  Result := inherited NeedChangeLayoutOnSelectionChanged(AInfo) or
    (cbvGroupRow in OptionsSelection.CheckBoxVisibility) and (dcoMultiSelectionSyncGroupWithChildren in DataController.Options);
  if not Result and (cbvColumnHeader in OptionsSelection.CheckBoxVisibility) then
  begin
    for I := 0 to ViewInfo.IndicatorViewInfo.Count - 1 do
    begin
      if Safe.Cast(ViewInfo.IndicatorViewInfo.Items[I], AItem) then
      begin
        if Assigned(AItem.CheckBox) then
        begin
          APreviousCheckBoxState := TcxGridCheckBoxViewInfoAccess(AItem.CheckBox).CheckBoxState;
          if APreviousCheckBoxState <> TcxGridCheckBoxViewInfoAccess(AItem.CheckBox).CalculateCheckBoxState then
            Result := True;
          Break;
        end;
      end;
...
function TcxGridTableView.NeedFocusFirstItemAfterInit: Boolean;
begin
  Result := inherited NeedFocusFirstItemAfterInit or RowLayout.Active;
end;

procedure TcxGridTableView.RecordChanged(ARecordIndex: Integer);
begin
  inherited RecordChanged(ARecordIndex);
  if IsInplaceEditFormVisible then
    InplaceEditForm.ValuesChanged;
end;

procedure TcxGridTableView.RefreshVisibleItemsList;
begin
  if IsAssigningItems then
    Exit;
  FVisibleLeftFixedColumnCount := 0;
  FVisibleRightFixedColumnCount := 0;
  FVisibleDynamicFixedColumnCount := 0;
  inherited RefreshVisibleItemsList;
  FLastAutoWidthSizableColumnVisibleIndex := GetLastAutoWidthSizableColumnVisibleIndex;
  if SupportsColumnFixing then
  begin
    if FDynamicFixedColumnCount > 0 then
      CheckVisibleDynamicFixedColumnCount;
    if FFixedColumnCount > 0 then
      ReorderVisibleItemListByFixedKind;
  end;
end;
...
procedure TcxGridTableView.ReorderVisibleItemListByFixedKind;
var
  I: Integer;
  AList: TList;
  AColumn: TcxGridColumn;
begin
  ...
  try
    for I := 0 to VisibleColumnCount - 1 do
    begin
      AColumn := VisibleColumns[I];
      case AColumn.FixedKind of
        fkLeft:
          begin
            AList.Insert(FVisibleLeftFixedColumnCount, AColumn);
            Inc(FVisibleLeftFixedColumnCount);
          end;
        fkRight:
          begin
            AList.Add(AColumn);
            Inc(FVisibleRightFixedColumnCount);
          end
        else 
          AList.Insert(AList.Count - FVisibleRightFixedColumnCount, AColumn);
      end;
    end;
    VisibleItemsList.Assign(AList);
    AssignVisibleItemIndexes;
  finally
    AList.Free;
  end;
end;
...
begin
  ...
  EditForm.ChangeScale(M, D);
  RowLayout.ChangeScale(M, D);
  if HasInplaceEditForm then
    InplaceEditForm.ChangeScale(M, D);
  if HasRowLayoutController then
    RowLayoutController.ChangeScale(M, D);
end;

procedure TcxGridTableOptionsView.ScaleFactorChanged;
begin
...
begin
  ...
  if HasRowLayoutController then
    RowLayoutController.CheckContainerName(AOldName, Name);
end;

function TcxGridTableView.SupportsColumnFixing: Boolean;
begin
  Result := True;
end;

function TcxGridTableView.SupportsHotTrack: Boolean;
begin
  Result := inherited SupportsHotTrack and not HasCellMerging;
end;

function TcxGridTableView.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;

procedure TcxGridTableView.UIANotify(ANotification: TdxUIAGridNotification);
var
  AOwnerControl: TcxControl;
  ATableViewProvider: IdxUIAGridTableViewProvider;
begin
  if Safe.Cast(OwnerControl, AOwnerControl) and AOwnerControl.HandleAllocated then
  begin
    ATableViewProvider := UIAProvider as IdxUIAGridTableViewProvider;
    if Assigned(ATableViewProvider) then
    begin
      case ANotification of
        TdxUIAGridNotification.FocusedCellChanged:
          begin
            ATableViewProvider.FocusedCellChanged;
          end;
        TdxUIAGridNotification.RowsChanged:
          ATableViewProvider.RowsChanged;
      end;
    end;
  end;
end;
...
begin
  ...
  if OptionsBehavior.EditMode in [emInplaceEditForm, emInplaceEditFormHideCurrentRow] then
    UpdateInplaceEditForm(AInfo);
end;

procedure TcxGridTableView.UpdateFocusedRecord(AInfo: TcxUpdateControlInfo);
var
  ANeedUpdateInplaceEditForm, AIsInserting: Boolean;
begin
  AIsInserting := dceInsert in DataController.EditState;
  ANeedUpdateInplaceEditForm := OptionsBehavior.EditMode in [emInplaceEditForm, emInplaceEditFormHideCurrentRow];
  if ANeedUpdateInplaceEditForm and not AIsInserting then
    UpdateInplaceEditForm(AInfo);
  inherited UpdateFocusedRecord(AInfo);
  if ANeedUpdateInplaceEditForm and AIsInserting then
    UpdateInplaceEditForm(AInfo);
end;
...
begin
  ...
  begin
    ...
    if (AInfo is TcxDataChangedInfo) and (TcxDataChangedInfo(AInfo).Kind in [dcTotal, dcRecord]) and
       (DataController.EditState = []) and IsInplaceEditFormVisible then
      InplaceEditForm.Close;
end;
...
function TcxGridTableView.GetDataViewStateStorageClass: TCustomGridViewStorageClass;
begin
  Result := TGridTableViewStorage;
end;
...
function TcxGridTableView.GetEditFormClass: TcxGridEditFormOptionsClass;
begin
  Result := TcxGridEditFormOptions;
end;
...
function TcxGridTableView.GetRowLayoutClass: TcxGridRowLayoutOptionsClass;
begin
  Result := TcxGridRowLayoutOptions;
end;

function TcxGridTableView.GetRowLayoutControllerClass: TcxGridTableRowLayoutControllerClass;
begin
  Result := TcxGridTableRowLayoutController;
end;
...
begin
  if Controller.CellMultiSelect and TcxCustomGridRow(ARecord).SupportsCellMultiSelect then
    Result := (AItem <> nil) and TcxGridColumn(AItem).Selected and
      ((ACellViewInfo = nil) or TcxGridTableCellViewInfoAccess(ACellViewInfo).AllowDrawSelected)
  else
...
procedure TcxGridTableView.DoCustomDrawRowLayoutRowBackground(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean);
begin
  RowLayout.DoCustomDrawRowBackground(ACanvas, AViewInfo, ADone);
end;

procedure TcxGridTableView.DoCustomDrawRowLayoutRowHotTrack(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean);
begin
  RowLayout.DoCustomDrawRowHotTrack(ACanvas, AViewInfo, ADone);
end;

procedure TcxGridTableView.DoCustomDrawRowLayoutRowSelection(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomGridCellViewInfo; var ADone: Boolean);
begin
  RowLayout.DoCustomDrawRowSelection(ACanvas, AViewInfo, ADone);
end;

procedure TcxGridTableView.DoDetachedEditFormInitialize(AForm: TForm);
begin
  if Assigned(FOnDetachedEditFormInitialize) then
    FOnDetachedEditFormInitialize(Self, AForm);
end;
...
function TcxGridTableView.HasCustomDrawRowLayoutRowHotTrack: Boolean;
begin
  Result := RowLayout.HasCustomDrawRowHotTrack;
end;

function TcxGridTableView.HasCustomDrawRowLayoutRowSelection: Boolean;
begin
  Result := RowLayout.HasCustomDrawRowSelection;
end;
...
begin
  ...
  if Root <> Owner then
    Exit;
  if not EditForm.UseDefaultLayout then
    InplaceEditForm.Store(Proc);
  if not RowLayout.UseDefaultLayout then
    RowLayoutController.Store(Proc);
end;
...
begin
  Result := inherited IsEqualHeightRecords and not ViewInfo.RecordsViewInfo.HasLastHorzGridLine(nil) and
    IsPreviewHeightFixed and (GroupedColumnCount = 0) and not IsMaster and not IsInplaceEditFormVisible;
end;
...
{ TcxGridFooterViewInfoSpecific }

constructor TcxGridFooterViewInfoSpecific.Create(
  AContainerViewInfo: TcxGridHeaderViewInfo);
begin
  Assert(AContainerViewInfo is TcxGridFooterViewInfo);
  inherited;
end;

function TcxGridFooterViewInfoSpecific.CalcCellHeight(AIndex,
  ACellHeight: Integer): Integer;
begin
  Result := ACellHeight;
end;

function TcxGridFooterViewInfoSpecific.CalcCellWidth(Column: TcxGridColumn;
  ACellWidth: Integer): Integer;
var
  AFooterPanelContentOffsets: TRect;
  ASpace: Integer;
begin
  AFooterPanelContentOffsets := ContainerViewInfo.FooterPanelContentOffsets;
  ASpace := GetHorizontalSpaceBetweenItems;
  Result := ACellWidth;
  if not (Column.IsMostLeft or Column.IsMostRight) then
    Dec(Result, ASpace)
  else
  begin
    if Column.IsMostLeft and Column.IsMostRight then
      Dec(Result, AFooterPanelContentOffsets.Left + AFooterPanelContentOffsets.Right)
    else if Column.IsMostLeft then
      Dec(Result, ContainerViewInfo.BorderWidth[bLeft] + AFooterPanelContentOffsets.Left + (ASpace - ContainerViewInfo.GridViewInfo.GridLineWidth) div 2)
    else
      Dec(Result, ContainerViewInfo.BorderWidth[bRight] + AFooterPanelContentOffsets.Right + (ASpace - ContainerViewInfo.GridViewInfo.GridLineWidth) div 2 - ASpace mod 2);
  end;
end;

function TcxGridFooterViewInfoSpecific.GetFooterContainerViewInfo: TcxGridFooterViewInfo;
begin
  Result := TcxGridFooterViewInfo(FContainerViewInfo);
end;

function TcxGridFooterViewInfoSpecific.GetHorizontalSpaceBetweenItems: Integer;
var
  AFooterPanelContentOffsets: TRect;
begin
  AFooterPanelContentOffsets := ContainerViewInfo.FooterPanelContentOffsets;
  Result := (AFooterPanelContentOffsets.Left + AFooterPanelContentOffsets.Right) div 2;
end;

function TcxGridFooterViewInfoSpecific.GetVerticalSpaceBetweenItems: Integer;
var
  AFooterPanelContentOffsets: TRect;
begin
  AFooterPanelContentOffsets := ContainerViewInfo.FooterPanelContentOffsets;
  Result := (AFooterPanelContentOffsets.Top + AFooterPanelContentOffsets.Bottom) div 2;
end;

{ TdxGridTableViewAutomationSettings }

procedure TdxGridTableViewAutomationSettings.AssignTo(ADest: TPersistent);
var
  ADestination: TdxGridTableViewAutomationSettings;
begin
  if Safe.Cast(ADest, ADestination) then
  begin
    inherited AssignTo(ADestination);
    ADestination.OnCalculateRowProperty := OnCalculateRowProperty;
    ADestination.OnInitializeRowProperties := OnInitializeRowProperties;
  end
  else
    inherited AssignTo(ADestination);
end;

procedure TdxGridTableViewAutomationSettings.DoCalculateRowAutomationProperty(
  ARow: TcxGridDataRow; AProperty: TdxAutomationProperty;
  AProperties: TdxAutomationProperties);
begin
  if Assigned(FOnCalculateRowProperty) then
    FOnCalculateRowProperty(ARow, AProperty, AProperties);
end;

procedure TdxGridTableViewAutomationSettings.DoInitializeRowAutomationProperties(
  ARow: TcxGridDataRow; AProperties: TdxAutomationProperties);
begin
  if Assigned(FOnInitializeRowProperties) then
    FOnInitializeRowProperties(ARow, AProperties);
end;

function TdxGridTableViewAutomationSettings.IsAutomationUnitNeededInUses: Boolean;
begin
  Result := inherited IsAutomationUnitNeededInUses or
    Assigned(TMethod(FOnInitializeRowProperties).Code) or Assigned(TMethod(FOnCalculateRowProperty).Code)
end;

initialization
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.InitializationStarted(dxThisUnitName, SysInit.HInstance);{$ENDIF}
  cxGridRegisteredViews.Register(TcxGridTableView, 'Table');
  System.Classes.RegisterClasses([TcxGridColumn, TcxGridTableViewStyleSheet, TcxGridTableRowLayoutItem,
    TcxGridTableRowLayoutGroup, TcxGridTableRowLayoutSerializationOwner]);
