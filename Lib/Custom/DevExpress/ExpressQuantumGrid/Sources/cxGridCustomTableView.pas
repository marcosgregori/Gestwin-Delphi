...
uses
  System.UITypes,
  System.Variants, Winapi.Windows, Winapi.Messages, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.ImgList, Vcl.Forms, Vcl.StdCtrls, Vcl.Dialogs, Winapi.MultiMon, System.SysUtils,
  ...
  dxCoreClasses, dxGdiPlusClasses, dxScrollAnimation, cxMemo, cxDropDownEdit, cxMRUEdit,
  cxTextEdit, dxIncrementalFiltering, dxUIElementPopupWindow, cxFindPanel, cxDataControllerSpreadSheetExpressionProvider,
  dxSpreadSheetTypes, dxSpreadSheetStyles, dxSpreadSheetConditionalFormatting, cxDataControllerConditionalFormatting,
  dxFilterPopupWindow, dxFilterValueContainer, cxDateUtils, dxDateRanges, dxFilterBox, dxScrollbarAnnotations,
  dxCoreGraphics, dxCustomFunction, dxUIABase, dxUIAClasses;
...
const
  ...
  htFindPanelNextButton = htCustomGridTableBase + 13;
  htFindPanelPreviousButton = htCustomGridTableBase + 14;
  ...
  vsHotTrack = vsCustomTableFirst + 11;
...
type
  ...
  TcxGridConditionalFormattingProvider = class;
  ...
  protected
    function IsEqualCore(AChange: TcxCustomGridChange): Boolean; override;
  public
  ...
  TcxGridFindPanelNextButtonHitTest = class(TcxCustomGridViewHitTest)
  protected
    class function GetHitTestCode: Integer; override;
  end;

  TcxGridFindPanelPreviousButtonHitTest = class(TcxCustomGridViewHitTest)
  protected
    class function GetHitTestCode: Integer; override;
  end;
  ...
  TcxGridDataFilterCriteriaItem = class(TcxDataFilterCriteriaItem)
  protected
    function IsItemLinkSupportsMultiThreading: Boolean; override;
  end;

  TcxGridDataFilterCriteria = class(TcxDataFilterCriteria)
  protected
  ...
  protected
    ...
    function CreateExpressionProvider: TcxDataCustomExpressionProvider; override;
  ...
  protected
    ...
    procedure Load(AItem: TcxCustomGridTableItem; AInitSortByDisplayText: Boolean; AUseFilteredValues: Boolean;
      AAddValueItems: Boolean; AUniqueOnly: Boolean; AFilteredValuesShowFilteredItemsOnly: Boolean); reintroduce; overload;
  public
  ...
  private
    ...
    function GetFilterCriteriaItem: TcxFilterCriteriaItem; inline;
    function GetFiltered: Boolean; inline;
    function GetGridView: TcxCustomGridTableView;
    function GetStoredExpression: string;
    ...
    procedure SetStoredExpression(const AValue: string);
    procedure SetValueType(const Value: string);
    procedure SetValueTypeClass(Value: TcxValueTypeClass);
    //
    procedure ReadIsNullValueType(AReader: TReader);
    procedure WriteIsNullValueType(AWriter: TWriter);
  protected
    procedure AddCustomFunctionToFilter(AParent: TcxFilterCriteriaItemList; const ACustomFunctionName: string;
      const AValue: Variant; const ADisplayText: string; AReplaceExistent: Boolean = True);
    procedure DefineProperties(Filer: TFiler); override;
    ...
    procedure SetFilterActiveCustomFunction(const ACustomFunctionName: string; const AValue: Variant; const ADisplayText: string);
    ...
    property StoredExpression: string read GetStoredExpression write SetStoredExpression;
  public
    ...
    function GetDefaultValuesProvider(AProperties: TcxCustomEditProperties = nil): TcxCustomEditDefaultValuesProvider;
  ...
  private
    ...
    function GetRecordIndex: Integer; inline;
    procedure SetExpanded(Value: Boolean);
    procedure SetFocused(Value: Boolean);
  protected
    FPixelScrollPosition: Integer;
    FUIAProvider: IdxUIAProvider;
    RecordInfo: TcxRowInfo;
    // UI Automation
    function CreateUIAProvider: IdxUIAProvider; virtual;
    procedure UIANotifyRowsChanged; virtual;
    // IdxAutomationElement
    function SupportsUIAutomation: Boolean; virtual;
    function GetAutomationSettings: TdxAutomationElementSettings; virtual;
    function GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider; virtual;
    //
    ...
    function HasErrorData: Boolean; virtual;
  ...
  public
    ...
    function UIAProvider(AIsCreating: Boolean = False): IdxUIAProvider; // for internal use
  ...
  protected
    function AddCustomFunctionToFilter(AParent: TcxFilterCriteriaItemList; AItem: TcxCustomGridTableItem;
      const ACustomFunctionName: string; const AValue: Variant; ADisplayText: string = '';
  ...
  protected
    ...
    function NeedCheckScrolling: Boolean; virtual;
    property Controller: TcxCustomGridTableController read GetController;
  ...
  private
    ...
    FCreatingNewRecord : Boolean; //..
    FInsertingRecord : Boolean; //..
  ...
  protected
    ...
    function GetMultilineEdit: TcxAutoHeightInplaceEdit; virtual;
    procedure MultilineEditTextChanged; override;
    procedure SetupMultilineEdit(AEdit: TcxAutoHeightInplaceEdit);
    ...
    function CreateEditAutoHeightProperties: TcxAutoHeightInplaceEditProperties; virtual;
    ...
    procedure InitMultilineEditorMinHeight; virtual;
    ...
    procedure PrepareEditViewInfo;
    procedure RecalculateEditViewInfo; override;
    ...
    procedure EditExecuteAction(Action: TBasicAction; var Handled: Boolean); override;
    ...
    procedure EditUpdateAction(Action: TBasicAction; var Handled: Boolean); override;
  ...
  public
    ...
    procedure RemoveEdit(AProperties: TcxCustomEditProperties); override;
  ...
  TcxCustomGridTableRecordScrollAnimationHelper = class(TdxRecordPixelScrollAnimationHelper)
  strict private
    FIsScrollingByRecord: Boolean;
    FOwner: TcxCustomGridTableController;
  protected
    procedure AfterAnimationEnded(AIsRaiseEvent: Boolean); override;
    procedure BeforeAnimationStarted; override;
    function CalculateAnimatedPosition(var APosition: Integer): TdxRecordPixelScrollData; override;
    function CalculateScrollDelta(AValue: TdxRecordPixelScrollData): Integer; override;
    function GetOwnerPosition: TdxRecordPixelScrollData; override;
    procedure PositionChanged; override;
    procedure SetTargetPosition(AValue: TdxRecordPixelScrollData); override;
    procedure UpdateOwnerPosition(AValue: TdxRecordPixelScrollData); override;
  public
    constructor Create(AOwner: TcxCustomGridTableController);
  end;

  TcxCustomGridTableController = class(TcxCustomGridController)
  strict private
    FAllowRecordScrollAnimationOnMakeVisible: Boolean;
    FIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition: Boolean;
  private
    ...
    FHotRecord: TcxCustomGridRecordViewInfo;
    ...
    FRecordScrollAnimationHelper: TcxCustomGridTableRecordScrollAnimationHelper;
    FScrollDirection: TcxDirection;
    FScrollTimer: TcxTimer;
    FTracking: Boolean;
    FUnselectingRecordIndex: Integer;
    FFirstAvailableItem: TcxCustomGridTableItem; //..
    FFirstAvailableItemIndex: Integer; //..
    ...
    function GetGridView: TcxCustomGridTableView; inline;
    function GetIncSearchingFromAnyPos: Boolean;
    ...
    function GetTargetRecordScrollIndex: Integer;
    function GetTargetRecordScrollOffset: Integer;
  protected
    procedure AfterPaint; override;
    function AllowPan(AScrollKind: TScrollBarKind): Boolean; override;
    procedure AssignFocusedRecord; inline;
    ...
    procedure AllowRecordScrollAnimationOnMakeVisible(AProc: TProc);
    ...
    procedure DoSetTopRecordIndexWithOffset(ATopRecordIndex, ATopRecordOffset: Integer);
    ...
    function GetActuallyFocusedItemIndex: Integer; virtual;
    ...
    function GetLastVisibleRecordIndex: Integer; virtual;
    ...
    function GetPageVisibleRecordCount(AFirstRecordIndex: Integer; APixelScrollRecordOffset: Integer; ACalculateDown: Boolean = True): Integer;
    ...
    function GetVisibleRecordCount(AFirstRecordIndex: Integer; APixelScrollRecordOffset: Integer; ACalculateDown: Boolean = True): Integer; virtual;
    ...
    function IsRecordScrollable(ARecordIndex: Integer): Boolean; virtual;
    ...
    procedure SetTopRecordIndexWithOffset(ATopRecordIndex, ATopRecordOffset: Integer; AAnimated: Boolean = False); overload;
    procedure ScrollAnimationEnded; virtual;
    procedure ScrollAnimationStarted;
    ...
    //hot tracked record view info
    function IsRecordHot(AViewInfo: TcxCustomGridRecordViewInfo): Boolean;
    procedure ResetHotRecord;
    procedure SetHotRecord(AValue: TcxCustomGridRecordViewInfo);
    ...
    function CanProcessMultiSelect(AHitTest: TcxCustomGridHitTest; AShift: TShiftState): Boolean; overload; virtual;
    function CanProcessMultiSelect(AKey: Word; AShift: TShiftState; AFocusedRecordChanged: Boolean): Boolean; overload; virtual;
    ...
    function IsRecordScrollAnimationActive: Boolean;
    function IsScrollAnimationActive: Boolean; virtual;
    function IsScrollAnimationEnabled: Boolean;
    procedure StopScrollAnimation; virtual;
    ...
    procedure DoKeyAutoRepeat; override;
    ...
    property ActuallyFocusedItemIndex: Integer read GetActuallyFocusedItemIndex;
    ...
    property BlockRecordKeyboardHandling: Boolean read FBlockRecordKeyboardHandling write FBlockRecordKeyboardHandling;
    ...
    property IncSearchingFromAnyPos: Boolean read GetIncSearchingFromAnyPos;
    property IsReadyForImmediateEditing: Boolean read FIsReadyForImmediateEditing write FIsReadyForImmediateEditing;
    ...
    property TargetRecordScrollIndex: Integer read GetTargetRecordScrollIndex;
    property TargetRecordScrollOffset: Integer read GetTargetRecordScrollOffset;
  ...
  public
    ...
    property FocusedRecord: TcxCustomGridRecord read FFocusedRecord write SetFocusedRecord;
    ...
    property FirstAvailableItem: TcxCustomGridTableItem read FFirstAvailableItem;  //..
    property FirstAvailableItemIndex: Integer read FFirstAvailableItemIndex; //..
  end;
  ...
  protected
    ...
    procedure ExcludeRectFromBackground(const R: TRect);
    procedure ExcludeFromBackground; virtual;
  ...
  protected
    ...
    procedure RepaintOnOffset; virtual;
  public
  ...
  protected
    ...
    property IsPart: Boolean read FIsPart;
  public
  ...
  protected
    ...
    function DoCalculateHeight: Integer; virtual;
    function DoCalculateWidth: Integer; virtual;
  ...
  protected
    ...
    function GetFocused: Boolean; override;
    function GetHitTestClass: TcxCustomGridHitTestClass; override;
    function GetKind: TcxFindPanelItemKind; virtual;
    function GetVisible: Boolean; override;

    property Container: TcxGridFindPanelButtonsViewInfo read GetContainer;
    property Controller: TcxCustomGridTableController read GetController;
    property Kind: TcxFindPanelItemKind read GetKind;
  end;
  ...
  protected
    function GetAdornerTargetElementName: string; override;
    function GetHitTestClass: TcxCustomGridHitTestClass; override;
    function GetKind: TcxFindPanelItemKind; override;
  end;
  ...
  TcxGridFindPanelNextButtonViewInfo = class(TcxGridFindPanelButtonViewInfo)
  protected
    function GetAdornerTargetElementName: string; override;
    function GetHitTestClass: TcxCustomGridHitTestClass; override;
    function GetKind: TcxFindPanelItemKind; override;
  ...
  TcxGridFindPanelPreviousButtonViewInfo = class(TcxGridFindPanelButtonViewInfo)
  protected
    function GetAdornerTargetElementName: string; override;
    function GetHitTestClass: TcxCustomGridHitTestClass; override;
    function GetKind: TcxFindPanelItemKind; override;
  ...
  private
    ...
    FOwner: TcxGridPartContainerViewInfo;
  ...
  protected
    ...
    function CalculateBestWidth: Integer; virtual;
    function IsItemCalculatedSingly(AItem: TcxGridPartCustomCellViewInfo): Boolean; virtual;
    function GetButtonsOffset: Integer; virtual;
    function GetFirstOffset: Integer; virtual;
    function GetTopBound(AItem: TcxGridPartCustomCellViewInfo; ABounds: TRect): Integer; virtual;
    function GetWidth(AAlignment: TcxGridFilterButtonAlignment): Integer; virtual;
    function GetEnabled: Boolean; virtual;
    function GetVertOffset: Integer; virtual;

    property GridView: TcxCustomGridTableView read GetGridView;
    property Owner: TcxGridPartContainerViewInfo read FOwner;
    property ScaleFactor: TdxScaleFactor read GetScaleFactor;
  public
    constructor Create(AOwner: TcxGridPartContainerViewInfo); virtual;
    destructor Destroy; override;
  ...
  protected
    ...
    function CalculateBestWidth: Integer; override;
    procedure CalculateEditViewInfo(const ABounds: TRect);
    function GetEditMaxWidth: Integer; virtual;
    function GetEditMinWidth: Integer; virtual;
    function GetWidth(AAlignment: TcxGridFilterButtonAlignment): Integer; override;
    function IsItemCalculatedSingly(AItem: TcxGridPartCustomCellViewInfo): Boolean; override;
    procedure OffsetRightAlignedItemsToEditViewInfo; virtual;
  ...
  private
    FTokenCriteriaViewInfo: TdxFilterBoxTokenCriteriaViewInfo;

    function GetButtonsViewInfo: TcxGridFilterButtonsViewInfo;
    function GetFilter: TcxDataFilterCriteria;
    function GetFiltering: TcxCustomGridTableFiltering;
    function GetMainFilter: TcxDataFilterCriteria;

    //IdxFilterBoxTokenCriteriaViewInfoOwner
    function IdxFilterBoxTokenCriteriaViewInfoOwner.GetControl = GetTokenCriteriaControl;
    function IdxFilterBoxTokenCriteriaViewInfoOwner.GetCriteria = GetTokenCriteria;
    function IdxFilterBoxTokenCriteriaViewInfoOwner.GetOptions = GetTokenCriteriaOptions;
    procedure IdxFilterBoxTokenCriteriaViewInfoOwner.GetViewParams = GetTokenCriteriaParams;
    function GetTokenCriteria: TdxFilterBoxTokenCriteria;
    function GetTokenCriteriaControl: TcxControl;
    function GetTokenCriteriaOptions: IdxFilterBoxTokenCriteriaOptions;
    procedure GetTokenCriteriaParams(var AParams: TcxViewParams);
  protected
    function CalculateHeight: Integer; override;
    function CalculateTokenCriteriaViewInfoBounds: TRect; virtual;
    function CreateButtonsViewInfo: TcxGridPartContainerButtonsViewInfo; override;
    function CreateTokenCriteriaViewInfo: TdxFilterBoxTokenCriteriaViewInfo; virtual;
    ...
    function IsTokenCriteriaVisible: Boolean; virtual;
    ...
    property MainFilter: TcxDataFilterCriteria read GetMainFilter;
    property TokenCriteriaViewInfo: TdxFilterBoxTokenCriteriaViewInfo read FTokenCriteriaViewInfo;
  public
    constructor Create(AGridViewInfo: TcxCustomGridTableViewInfo); override;
    destructor Destroy; override;
  ...
  protected
    ...
    function GetHitTestClass: TcxCustomGridHitTestClass; override;
    function GetPainterClass: TcxCustomGridCellPainterClass; override;
    function GetVertOffset: Integer; override;
    procedure GetViewParams(var AParams: TcxViewParams); override;
    function GetVisible: Boolean; override;
  ...
  protected
    function AllowDrawHotTracked: Boolean; virtual;
    function AllowDrawSelected: Boolean; virtual;
    function AlwaysDrawSelectedWhenFocused: Boolean; virtual;
    function CalculateHotTracked: Boolean; virtual;
    function CalculateSelected: Boolean; virtual;
    procedure CalculateTextSelectionInfo; override;
    function CanHighlightFindText: Boolean; virtual;
    function DrawHotTrackedWhenRecordHotTracked: Boolean; virtual;
    function GetAlwaysSelected: Boolean; virtual;
    function GetCanvas: TcxCanvas; override;
    function GetHitTestClass: TcxCustomGridHitTestClass; override;
    function GetHotTrack: Boolean; override;
    procedure GetFindTextSelectionInfo(out ASelStart: Integer; out AHighlightedText: string); virtual;
    function GetTransparent: Boolean; override;
    procedure HotTrackChanged(AHotTracked: Boolean); override;
    function HotTrackSelection: Boolean; virtual;
    procedure InitHitTest(AHitTest: TcxCustomGridHitTest); override;
    function NeedHighlightFindText: Boolean; virtual;
    procedure PrepareFindTextSelColor; virtual;
  ...
  public
    ...
    procedure Calculate(ALeftBound, ATopBound: Integer; AWidth: Integer = -1; AHeight: Integer = -1); override;
    function CanDrawHotTracked: Boolean; virtual;
  ...
  private
    ...
    FStyle: TcxEditStyle;
    FUsedForPaintingHiddenEditor: Boolean;
    FWasFocusedBeforeClick: Boolean;

    procedure AfterDrawCellBackgroundHandler(Sender: TcxCustomEditViewInfo; ACanvas: TcxCanvas);
    procedure AfterDrawCellValueHandler(Sender: TcxCustomEditViewInfo; ACanvas: TcxCanvas);
    ...
    procedure SetUsedForPaintingHiddenEditor(AValue: Boolean);
  protected
    ...
    procedure CalculateEditViewInfo(AEditViewInfo: TcxCustomEditViewInfo; const AMousePos: TPoint); virtual;
    function CalculateHeight: Integer; override;
    function CalculateSelected: Boolean; override;
    function CanActivateEditOnMouseDown(AHitTest: TcxCustomGridHitTest; AButton: TMouseButton; AShift: TShiftState): Boolean; virtual;
    function CanAutoHeight: Boolean; virtual;
    function CanDrawSelectedWhenEditing: Boolean; virtual;
    function CanHighlightEditViewDataFindText: Boolean; virtual;
    ...
    procedure GetEditViewDataFindTextSelectionInfo(out ASelStart: Integer; out AHighlightedText: string); virtual;
    function GetEditViewDataTextSelectionInfo(out AParams: TcxViewParams; out ASelStart, ASelLength: Integer): Boolean;
    ...
    function GetText: string; override;
    function GetTextAreaBounds: TRect; override;
    function GetUsedForPaintingHiddenEditor: Boolean; virtual;
    function GetValue: Variant; virtual;
    procedure GetViewParams(var AParams: TcxViewParams); override;
    function HasCustomDraw: Boolean; override;
    function HasFocusRect: Boolean; virtual;
    procedure InitEditSizeProperties(out AEditSizeProperties: TcxEditSizeProperties); virtual;
    procedure InitHitTest(AHitTest: TcxCustomGridHitTest); override;
    procedure InitializeByHiddenEditor(AEditViewInfo: TcxCustomEditViewInfo; AEdit: TcxCustomEdit); virtual;
    procedure InitViewDataTextSelection; virtual;
    function InvalidateOnStateChange: Boolean; override;
    function IsTextSelected: Boolean; virtual;
    function NeedHighlightEditViewDataFindText: Boolean; virtual;
    ...
    function NeedShowHint(const AMousePos: TPoint; out AHintText: TCaption; out AIsHintMultiLine: Boolean; out ATextRect: TRect): Boolean; override;
    ...
    procedure InitEditInfo; virtual;
    function SupportsEditing: Boolean; virtual;
    procedure UpdateEdit;
    procedure UpdateEditInfo; virtual;
    ...
    property ClientExtent: TRect read FClientExtent;
    ...
    property UsedForPaintingHiddenEditor: Boolean read FUsedForPaintingHiddenEditor write SetUsedForPaintingHiddenEditor;
  ...
  public
    ...
    property Focused: Boolean read GetFocused;
    property IsMain: Boolean read GetIsMain; //for internal use only
    property Item: TcxCustomGridTableItem read FItem;
  ...
  protected
    ...
    function AllowDrawHotTracked: Boolean; virtual;
    function AllowDrawSelected: Boolean; virtual;
    procedure BeforeOffset; override;
    procedure CalculateExpandButtonBounds(var ABounds: TRect); virtual; abstract;
    function CalculateHotTracked: Boolean; virtual;
    procedure CalculateInPlace(ALeft, ATop: Integer); virtual;
  ...
  protected
    ...
    function GetPixelScrollOffset(AFirstRecordIndex, AFirstRecordOffset: Integer; ALastRecordIndex, ALastRecordOffset: Integer): Integer;
    ...
    function GetCellAreaBounds(ACellViewInfo: TcxGridTableDataCellViewInfo): TRect; virtual;
  ...
  protected
    ...
    function IsCellPartVisible(ACell: TcxGridTableDataCellViewInfo): Boolean; virtual;
  ...
  private
    ...
    FFilteringFilteredItemsListShowFilteredItemsOnly: Boolean;
    ...
    procedure SetFilteringFilteredItemsListShowFilteredItemsOnly(AValue: Boolean);
  ...
  protected
    ...
    property ExpressionEditing: Boolean read FExpressionEditing write FExpressionEditing default True;
  ...
  public
    ...
    property FilteringFilteredItemsListShowFilteredItemsOnly: Boolean read FFilteringFilteredItemsListShowFilteredItemsOnly
      write SetFilteringFilteredItemsListShowFilteredItemsOnly default True;
  ...
  TcxGridGetSelectionStyleEvent = procedure(Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
    AItem: TObject; {$IFDEF BCBCOMPATIBLE}var{$ELSE}out{$ENDIF} AStyle: TcxStyle) of object;
  ...
  TcxGridGetFilterImagesEvent = procedure(Sender: TcxCustomGridTableItem;
    AValueList: TcxFilterValueList; var AImages: TCustomImageList) of object;
  ...
  private
    ...
    FAutomation: TdxAutomationElementSettings;
    FAutomationEvents: TNotifyEvent;
    ...
    FEditViewData: TcxCustomEditViewData;
    FEmptyEditorHeight: Integer;
    FFilteringDateRanges: TcxGridFilteringDateRanges;
    FFilterImages: TCustomImageList;
    FGridView: TcxCustomGridTableView;
    ...
    FRepositoryItemTempProperties: TcxCustomEditProperties;
    FSavedVisible: Boolean;
    FStyles: TcxCustomGridTableItemStyles;
    FUIAProvider: IdxUIAProvider;
    FVisible: Boolean;
    FVisibleForCustomization: Boolean;
    FVisibleForExpressionEditor: TdxDefaultBoolean;
    ...
    FOnGetFilterImages: TcxGridGetFilterImagesEvent;
    ...
    function GetEditing: Boolean;
    function GetEditingProperties: TcxCustomEditProperties;
    function GetEmptyEditorHeight: Integer;
    ...
    procedure SetAutomation(const AValue: TdxAutomationElementSettings);
    ...
    procedure SetOnGetFilterImages(Value: TcxGridGetFilterImagesEvent);
  ...
  protected
    ...
    procedure SetFilterActiveValueIndexes(AValues: TcxFilterValueList; const AIndexes: TdxIntegerIndexes;
      AAddToMRUItemsList: Boolean = False);
    //IdxFilterImagesProvider
    function IdxFilterImagesProvider.GetImages = GetFilterImages;
    function GetFilterImages: TCustomImageList; virtual;
    procedure PopulateFilterImages(AValues: TcxFilterValueList); virtual;
    //IdxExcelFilterableComponent
    ...
    procedure CalculateVisualParams; virtual;
    function CanAutoHeight: Boolean; virtual;
    function CanEdit: Boolean; virtual;
    ...
    procedure ChangeGrouping(AGroupIndex: Integer; AMergeWithLeftColumn: Boolean = False; AMergeWithRightColumn: Boolean = False); virtual;
    procedure ChangeSortIndex(Value: Integer);
    procedure CheckUsingInFindCriteria; virtual;
    procedure CheckWidthValue(var Value: Integer);
    procedure DataChanged; virtual;
    procedure DateTimeGroupingChanged; virtual;
    procedure DoGetFilterImages(AValueList: TcxFilterValueList; var AImages: TCustomImageList); virtual;
    procedure DoRefreshStylesCache; virtual;
    ...
    function GetIsFixedByWidth: Boolean; virtual;
    ...
    function IsDefaultVisibleForExpressionEditor: Boolean;
    ...
    function IsVisibleForFocus(ARecord: TcxCustomGridRecord): Boolean; virtual;
    ...
    function IsVisibleForExpressionEditor: Boolean; virtual;
    ...
    function ShowFilteredItemsOnlyForFilteredValues: Boolean; virtual;
    ...
    function TryResetFocusToNext: Boolean; virtual;
    ...
    function CreateEditViewData(AProperties: TcxCustomEditProperties): TcxCustomEditViewData; overload;
    function CreateEditViewData(AProperties: TcxCustomEditProperties; ACellStyle: TcxEditStyle; AScaleFactor: TdxScaleFactor = nil): TcxCustomEditViewData; overload;
    ...
    property Automation: TdxAutomationElementSettings read FAutomation write SetAutomation;
    property Controller: TcxCustomGridTableController read GetController;
    property DataController: TcxCustomDataController read GetDataController;
    ...
    property EmptyEditorHeight: Integer read FEmptyEditorHeight;
    ...
    property IsFixedByWidth: Boolean read GetIsFixedByWidth;
  ...
  public
    ...
    function CalculateDefaultCellHeight(ACanvas: TcxCanvas; AFont: TFont; ADrawAsStandalone: Boolean = False; AScaleFactor: TdxScaleFactor = nil): Integer; virtual;
    ...
    function UIAProvider: IdxUIAProvider; // for internal use
  ...
  published
    ...
    property AutomationEvents: TNotifyEvent read FAutomationEvents write FAutomationEvents; // for internal use
    ...
    property VisibleForExpressionEditor: TdxDefaultBoolean read FVisibleForExpressionEditor
      write FVisibleForExpressionEditor default bDefault;
    ...
    property OnGetFilterImages: TcxGridGetFilterImagesEvent read FOnGetFilterImages write SetOnGetFilterImages;
  ...
  TcxGridFilterBoxTokenCriteriaOptions = class(TcxCustomGridOptions,
    IdxFilterBoxTokenCriteriaOptions)
  strict private
    FItemRemoval: Boolean;

    function GetItemRemoval: Boolean;
    procedure SetItemRemoval(AValue: Boolean);
  public
    constructor Create(AGridView: TcxCustomGridView); override;

    procedure Assign(Source: TPersistent); override;
  published
    property ItemRemoval: Boolean read GetItemRemoval write SetItemRemoval default True;
  end;
  ...
  private
    ...
    FTokenCriteria: TcxGridFilterBoxTokenCriteriaOptions;
    FVisible: TcxGridFilterVisible;

    function GetGridView: TcxCustomGridTableView;
    procedure SetButtonPosition(const Value: TcxGridFilterButtonAlignment);
    procedure SetCriteriaDisplayStyle(Value: TcxFilterCriteriaDisplayStyle);
    ...
    procedure SetTokenCriteria(Value: TcxGridFilterBoxTokenCriteriaOptions);
    procedure SetVisible(Value: TcxGridFilterVisible);
  protected
    function CreateTokenCriteria: TcxGridFilterBoxTokenCriteriaOptions; virtual;
  ...
  published
    ...
    property TokenCriteria: TcxGridFilterBoxTokenCriteriaOptions read FTokenCriteria write SetTokenCriteria;
  ...
  protected
    ...
    function DoGetItemCaption(AItem: TcxFindPanelItemKind): string; override;
    procedure FocusControlContent; override;
    procedure GetControlContentViewParams(var AParams: TcxViewParams); override;
    function GetCriteria: TcxDataFindCriteria; override;
  ...
  private
    ...
    function GetLayout: TcxFindPanelLayout;
    ...
    function GetShowNextButton: Boolean;
    function GetShowPreviousButton: Boolean;
  ...
  published
    ...
    property Behavior: TcxDataFindCriteriaBehavior read GetBehavior write SetBehavior default fcbDefault;
    ...
    property Layout: TcxFindPanelLayout read GetLayout write SetLayout default fplDefault;
    ...
    property ShowNextButton: Boolean read GetShowNextButton write SetShowNextButton default True;
    property ShowPreviousButton: Boolean read GetShowPreviousButton write SetShowPreviousButton default True;
  ...
  private
    ...
    FItemFilteredItemsListShowFilteredItemsOnly: Boolean;
    ...
    procedure SetItemFilteredItemsListShowFilteredItemsOnly(AValue: Boolean);
  ...
  protected
    ...
    property ItemFilteredItemsListShowFilteredItemsOnly: Boolean read FItemFilteredItemsListShowFilteredItemsOnly
      write SetItemFilteredItemsListShowFilteredItemsOnly default True;
  ...
  published
    property ExpressionEditing: Boolean read GetExpressionEditing write SetExpressionEditing default False;
  ...
  private
    ...
    procedure SetHotTrackSelection(Value: Boolean);
  ...
  protected
    function GetDefaultHotTrack: Boolean; virtual;
    function GetDefaultHotTrackSelection: Boolean; virtual;
    ...
    property HotTrackSelection: Boolean read FHotTrackSelection write SetHotTrackSelection default True;
  ...
  private
    FItemExpressionEditing: Boolean;
  ...
  protected
    property ItemExpressionEditing: Boolean read FItemExpressionEditing write FItemExpressionEditing default False;
  ...
  TcxGridScrollbarAnnotations = class(TdxScrollbarAnnotations) // for internal use
  strict private
    FGridView: TcxCustomGridTableView;
  protected
    function GetCurrentRecordIndex: Integer; override;
    function GetDataPixelScrollSize: Integer; override;
    function GetOptions: TdxScrollbarAnnotationOptions; override;
    function GetRecordIndexByScrollableRecordIndex(AIndex: Integer): Integer; override;
    procedure Populate(AKind: TdxScrollbarAnnotationKind; AList: TdxScrollbarAnnotationRowIndexList); override;
    procedure SetCurrentRecordIndex(AIndex: Integer); override;
  public
    constructor Create(AOwner: TcxCustomGridTableView); virtual;
    procedure GetHint(AAnnotationRowIndexLists: TdxScrollbarAnnotationRowIndexLists; var AHint: string); override;
    function GetLastIndexInScrollBand(ARecordIndex: Integer): Integer; override;
    function GetRecordPixelScrollPosition(ARecordIndex: Integer): Integer; override;
    function GetScrollableRecordCount: Integer; override;
    function GetScrollableRecordIndexByRecordIndex(AIndex: Integer): Integer; override;
    function IsRecordIndexBasedRendering: Boolean; override;
  end;

  TcxGridScrollbarAnnotationOptionsHelper = class(TdxScrollbarAnnotationOptions) // for internal use
  strict private
    FOwner: TcxCustomGridTableView;
  protected
    procedure Changed; override;
    procedure CheckScrollbarAnnotations; override;
    function IsVisible(AKind: Integer): Boolean; override;
    procedure RefreshScrollbarAnnotations(AChangedAnnotationKinds: TdxScrollbarAnnotationKinds = dxAllScrollbarAnnotationKinds); override;
  public
    constructor Create(AOwner: TcxCustomGridTableView); virtual;
  end;

  TcxGridScrollbarAnnotationOptions = class(TcxCustomGridOptions)
  strict private
    FHelper: TcxGridScrollbarAnnotationOptionsHelper;
  ...
  protected
    function CreateHelper: TcxGridScrollbarAnnotationOptionsHelper; virtual;
    function GetOwner: TPersistent; override;
    property Helper: TcxGridScrollbarAnnotationOptionsHelper read FHelper;
  public
    constructor Create(AGridView: TcxCustomGridView); override;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
  published
    property Active: Boolean index 0 read GetValue write SetValue stored IsValueStored;
    ...
    property ShowErrors: Boolean index 4 read GetValue write SetValue stored IsValueStored;
    property ShowSearchResults: Boolean index 3 read GetValue write SetValue stored IsValueStored;
    property ShowFocusedRow: Boolean index 2 read GetValue write SetValue stored IsValueStored;
    property ShowSelectedRows: Boolean index 1 read GetValue write SetValue stored IsValueStored;
  end;
  ...
  private
    FActuallyUseOddEvenStyles: Boolean;
    FUseOddEvenStyles: TdxDefaultBoolean;
    FOnGetContentStyle: TcxGridGetCellStyleEvent;
    FOnGetHotTrackStyle: TcxGridGetSelectionStyleEvent;
    FOnGetInactiveStyle: TcxGridGetSelectionStyleEvent;
    FOnGetSelectionStyle: TcxGridGetSelectionStyleEvent;

    function GetGridViewValue: TcxCustomGridTableView; inline;
    procedure SetUseOddEvenStyles(const AValue: TdxDefaultBoolean);
    procedure SetOnGetContentStyle(Value: TcxGridGetCellStyleEvent);
    procedure SetOnGetHotTrackStyle(Value: TcxGridGetSelectionStyleEvent);
    procedure SetOnGetInactiveStyle(Value: TcxGridGetSelectionStyleEvent);
    procedure SetOnGetSelectionStyle(Value: TcxGridGetSelectionStyleEvent);
  protected
    ...
    procedure GetDefaultViewParams(Index: Integer; AData: TObject; out AParams: TcxViewParams); override;
    procedure GetHotTrackParams(ARecord: TcxCustomGridRecord; AItem: TObject; out AParams: TcxViewParams); virtual;
    procedure GetSelectionParams(ARecord: TcxCustomGridRecord; AItem: TObject; out AParams: TcxViewParams); virtual;
    function IsOddEvenStylesSupported: Boolean; virtual;
    procedure RefreshCache; override;
    //
    property ActuallyUseOddEvenStyles: Boolean read FActuallyUseOddEvenStyles;
    property HotTrack: TcxStyle index vsHotTrack read GetValue write SetValue;
    property OnGetHotTrackStyle: TcxGridGetSelectionStyleEvent read FOnGetHotTrackStyle write SetOnGetHotTrackStyle;
  public
    constructor Create(AOwner: TPersistent); override;
    procedure Assign(Source: TPersistent); override;
    ...
    procedure GetDataCellParams(ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem; out AParams: TcxViewParams;
      AUseViewInfo: Boolean = False; ACellViewInfo: TcxGridTableCellViewInfo = nil; AIgnoreSelection: Boolean = False;
      AIgnoreHotTrack: Boolean = False); virtual;
    procedure GetRecordContentParams(ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem; out AParams: TcxViewParams); virtual;
  ...
  published
    ...
    property OnGetContentStyle: TcxGridGetCellStyleEvent read FOnGetContentStyle write SetOnGetContentStyle;
    property OnGetInactiveStyle: TcxGridGetSelectionStyleEvent read FOnGetInactiveStyle write SetOnGetInactiveStyle;
    property OnGetSelectionStyle: TcxGridGetSelectionStyleEvent read FOnGetSelectionStyle write SetOnGetSelectionStyle;
  end;

  { TcxGridConditionalFormattingProvider }

  TcxGridConditionalFormattingProviderClass = class of TcxGridConditionalFormattingProvider;
  ...
  protected
    ...
    procedure DoOwnerBeginUpdate; override;
    procedure DoOwnerEndUpdate; override;
    ...
    function GetOwner: TComponent; override;
    function GetScaleFactor: TdxScaleFactor; override;
    function IsItemVisible(AItem: TObject): Boolean; override;
  ...
  private
    function GetItem(Index: TdxListIndex): TcxCustomGridTableItem;
    procedure SetItem(Index: TdxListIndex; Value: TcxCustomGridTableItem);
  public
    property Items[Index: TdxListIndex]: TcxCustomGridTableItem read GetItem write SetItem; default;
  end;
  ...
  TcxGridPopulateCustomScrollbarAnnotationRowIndexList = procedure(Sender: TcxCustomGridTableView;
  ...
  TcxGridGetScrollbarAnnotationHint = procedure(Sender: TcxCustomGridTableView;
  ...
  TcxFocusFirstAvailableItemEvent = function(Sender:TcxCustomGridTableController) : Boolean of object; //..
  ...
  private
    ...
    FFilterBoxTokenCriteria: TdxFilterBoxTokenCriteria;
    FFiltering: TcxCustomGridTableFiltering;
    ...
    FItems: TList;
    FLinkedFilterControls: TList;
    FLoadedChangedScrollbarAnnotationKinds: TdxScrollbarAnnotationKinds;
    ...
    FScrollbarAnnotationHelper: TcxGridScrollbarAnnotations;
    FScrollbarAnnotations: TcxGridScrollbarAnnotationOptions;
    FVisibleItems: TList;
    ...
    FOnPopulateCustomScrollbarAnnotationRowIndexList: TcxGridPopulateCustomScrollbarAnnotationRowIndexList;
    FOnGetDragDropText: TcxGridGetDragDropTextEvent;
    FOnGetScrollbarAnnotationHint: TcxGridGetScrollbarAnnotationHint;
    FOnInitFilteringDateRanges: TcxGridInitDateRangesEvent;
    FOnInitGroupingDateRanges: TcxGridInitDateRangesEvent;
    FOnInitEdit: TcxGridInitEditEvent;
    FOnInitEditValue: TcxGridInitEditValueEvent;
    FOnQueryCustomFunctions: TcxQueryCustomFunctions;
    FOnSelectionChanged: TcxGridCustomTableViewEvent;
    FOnTopRecordIndexChanged: TNotifyEvent;
    FOnUpdateEdit: TcxGridInitEditEvent;
    FSubClassEvents: TNotifyEvent;
    FOnFocusFirstAvailableItem: TcxFocusFirstAvailableItemEvent; //..
    ...
    function GetFilterBoxTokenCriteria: TdxFilterBoxTokenCriteria;
    ...
    procedure SetScrollbarAnnotations(AValue: TcxGridScrollbarAnnotationOptions);
    ...
    procedure SetOnPopulateCustomScrollbarAnnotationRowIndexList(const Value: TcxGridPopulateCustomScrollbarAnnotationRowIndexList);
    procedure SetOnGetDragDropText(Value: TcxGridGetDragDropTextEvent);
    procedure SetOnGetScrollbarAnnotationHint(Value: TcxGridGetScrollbarAnnotationHint);
    procedure SetOnInitFilteringDateRanges(Value: TcxGridInitDateRangesEvent);
    procedure SetOnInitGroupingDateRanges(Value: TcxGridInitDateRangesEvent);
    procedure SetOnInitEdit(Value: TcxGridInitEditEvent);
    procedure SetOnFocusFirstAvailableItem(Value: TcxFocusFirstAvailableItemEvent); //..
    procedure SetOnInitEditValue(Value: TcxGridInitEditValueEvent);
    procedure SetOnQueryCustomFunctions(Value: TcxQueryCustomFunctions);
  ...
  protected
    ...
    // IcxFilterControlEx
    function IcxFilterControlEx.AllowExpressionEditing = AllowFilterExpressionEditing;
    function IcxFilterControlEx.GetExpressionprovider = GetExpressionprovider;
    procedure IcxFilterControlEx.RegisterListener = RegisterFilterControl;
    procedure IcxFilterControlEx.UnregisterListener = UnregisterFilterControl;
    function IcxFilterControlEx.UseTokens = UseFilterTokens;
    function AllowFilterExpressionEditing: Boolean;
    function GetExpressionprovider: TcxCustomExpressionProvider;
    procedure RegisterFilterControl(AControl: TcxCustomFilterControl);
    procedure UnregisterFilterControl(AControl: TcxCustomFilterControl);
    procedure UpdateLinkedFilterControls;
    function UseFilterTokens: Boolean;
    // IcxNavigator
    ...
    procedure NavigatorCreateNewRecordExecute(AtEnd: Boolean); virtual;
    procedure NavigatorEditExecute; virtual;
    // IcxDataControllerConditionalFormattingProviderOwner
    ...
    procedure CheckItemsUsingInFindCriteria; virtual;
    ...
    function GetCurrentScrollbarAnnotationRecordIndex: Integer; virtual;
    ...
    function GetPostingLockedStateMode: TcxGridShowLockedStateImageMode;
    procedure GetScrollbarAnnotationHint(AAnnotationRowIndexLists: TdxScrollbarAnnotationRowIndexLists; var AHint: string); virtual;
    ...
    function IsScrollAnimationEnabled: Boolean;
    function IsScrollbarAnnotationVisible(AKind: Integer): Boolean; virtual;
    procedure LoadingComplete; override;
    function NeedFocusFirstItemAfterInit: Boolean; virtual;
    procedure Offset(ARecordCountDelta, APixelScrollRecordOffsetDelta, DX, DY: Integer); virtual;
    procedure PopulateScrollbarAnnotations(AKind: TdxScrollbarAnnotationKind; AList: TdxScrollbarAnnotationRowIndexList); virtual;
    procedure ChangeScale(M, D: Integer); override;
    ...
    procedure SetCurrentScrollbarAnnotationRecordIndex(AIndex: Integer); virtual;
    procedure SetName(const NewName: TComponentName); override;
    function SpaceForHorizontalScrollbarNeeded: Boolean; override;
    function SupportsHotTrack: Boolean; virtual;
    ...
    procedure UpdateFilterBoxTokenCriteria; virtual;
    ...
    function CreateFilterBoxTokenCriteria: TdxFilterBoxTokenCriteria; virtual;
    function CreateFindPanel: TcxGridFindPanelOptions; virtual;
    function CreateScrollbarAnnotationHelper: TcxGridScrollbarAnnotations; virtual;
    function CreateScrollbarAnnotations: TcxGridScrollbarAnnotationOptions; virtual;

    procedure AddItem(AItem: TcxCustomGridTableItem); virtual;
    procedure RemoveItem(AItem: TcxCustomGridTableItem); virtual;
    procedure AssignVisibleItemIndexes;
    ...
    procedure FilterChanged; virtual;
    procedure FindCriteriaChanged(AInfo: TcxFindCriteriaChangedInfo); virtual;
    ...
    function DrawDataCellHotTracked(ARecord: TcxCustomGridRecord;
      ACellViewInfo: TcxGridTableCellViewInfo = nil): Boolean; virtual;
    ...
    procedure DoQueryCustomFunctions(AItem: TcxCustomGridTableItem; ACustomFunctions: TStrings); virtual;
    ...
    procedure UpdateItemsVisualParams; virtual;

    function CreateConditionalFormattingProvider: TcxGridConditionalFormattingProvider; virtual;
    function GetConditionalFormattingProviderClass: TcxGridConditionalFormattingProviderClass; virtual;
    ...
    property FilterBoxTokenCriteria: TdxFilterBoxTokenCriteria read GetFilterBoxTokenCriteria;
    ...
    property OnInitGroupingDateRanges: TcxGridInitDateRangesEvent read FOnInitGroupingDateRanges write SetOnInitGroupingDateRanges;
    property OnQueryCustomFunctions: TcxQueryCustomFunctions read FOnQueryCustomFunctions write SetOnQueryCustomFunctions; // for internal use
  public
    destructor Destroy; override;
    ...
    function GoToNextScrollbarAnnotation(AKinds: TdxScrollbarAnnotationKinds = dxAllScrollbarAnnotationKinds;
  ...
  published
    ...
    property ScrollbarAnnotations: TcxGridScrollbarAnnotationOptions read FScrollbarAnnotations write SetScrollbarAnnotations;
    ...
    property OnPopulateCustomScrollbarAnnotationRowIndexList: TcxGridPopulateCustomScrollbarAnnotationRowIndexList read FOnPopulateCustomScrollbarAnnotationRowIndexList write SetOnPopulateCustomScrollbarAnnotationRowIndexList;
    property OnGetDragDropText: TcxGridGetDragDropTextEvent read FOnGetDragDropText write SetOnGetDragDropText;
    property OnGetScrollbarAnnotationHint: TcxGridGetScrollbarAnnotationHint read FOnGetScrollbarAnnotationHint write SetOnGetScrollbarAnnotationHint;
    ...
    property OnFocusFirstAvailableItem: TcxFocusFirstAvailableItemEvent read FOnFocusFirstAvailableItem write SetOnFocusFirstAvailableItem; //..
  end;
  ...
  public
    ...
    class function CanIgnoreTimeForFiltering(AInstance: TcxCustomGridTableItem): Boolean; static;
...
uses
  System.Types, System.DateUtils, System.Math, System.TypInfo, System.RTLConsts, Vcl.Clipbrd,
  ...
  cxEditDataRegisteredRepositoryItems, cxFilterConsts, dxThreading,
  cxGrid, cxGridStrs, cxGridLevel, dxDPIAwareUtils, cxLibraryStrs, dxMessageDialog, cxImageComboBox,
  cxMaskEdit;

const
  dxThisUnitName = 'cxGridCustomTableView';
...
constructor TcxGridFilterBoxTokenCriteriaOptions.Create(AGridView: TcxCustomGridView);
begin
  inherited Create(AGridView);
  FItemRemoval := True;
end;

procedure TcxGridFilterBoxTokenCriteriaOptions.Assign(Source: TPersistent);
var
  ASource: TcxGridFilterBoxTokenCriteriaOptions;
begin
  inherited Assign(Source);
  if Source is TcxGridFilterBoxTokenCriteriaOptions then
  begin
    ASource := TcxGridFilterBoxTokenCriteriaOptions(Source);
    ItemRemoval := ASource.ItemRemoval;
  end;
end;

function TcxGridFilterBoxTokenCriteriaOptions.GetItemRemoval: Boolean;
begin
  Result := FItemRemoval;
end;

procedure TcxGridFilterBoxTokenCriteriaOptions.SetItemRemoval(AValue: Boolean);
begin
  if FItemRemoval <> AValue then
  begin
    FItemRemoval := AValue;
    Changed(vcSize);
  end;
...
begin
  inherited;
  FTokenCriteria := CreateTokenCriteria;
...
destructor TcxGridFilterBox.Destroy;
begin
  FreeAndNil(FTokenCriteria);
  inherited Destroy;
end;
...
begin
  ...
  if Source is TcxGridFilterBox then
    ...
    begin
      Self.CriteriaDisplayStyle := CriteriaDisplayStyle;
      ...
      Self.TokenCriteria := TokenCriteria;
    end;
end;

function TcxGridFilterBox.CreateTokenCriteria: TcxGridFilterBoxTokenCriteriaOptions;
begin
  Result := TcxGridFilterBoxTokenCriteriaOptions.Create(GridView);
end;

function TcxGridFilterBox.UseTokens: Boolean;
begin
  Result := (CriteriaDisplayStyle = fcdsTokens) or (CriteriaDisplayStyle = fcdsDefault) and
    (dxFilterCriteriaDisplayStyle = fcdsTokens);
end;

function TcxGridFilterBox.GetGridView: TcxCustomGridTableView;
begin
  Result := TcxCustomGridTableView(inherited GridView);
end;

procedure TcxGridFilterBox.SetButtonPosition(const Value: TcxGridFilterButtonAlignment);
begin
...
procedure TcxGridFilterBox.SetCriteriaDisplayStyle(Value: TcxFilterCriteriaDisplayStyle);
begin
  if CriteriaDisplayStyle <> Value then
  begin
    FCriteriaDisplayStyle := Value;
    if not UseTokens  then
      GridView.DestroyFilterBoxTokenCriteria;
    GridView.UpdateLinkedFilterControls;
    Changed(vcSize);
  end;
end;
...
procedure TcxGridFilterBox.SetTokenCriteria(Value: TcxGridFilterBoxTokenCriteriaOptions);
begin
  TokenCriteria.Assign(Value);
end;
...
function TcxGridFindPanel.DoGetItemCaption(AItem: TcxFindPanelItemKind): string;
begin
  case AItem of
    fpikFindButton:
    ...
    fpikNextButton:
      Result := cxGetResourceString(@scxGridFindPanelNextButtonCaption);
    fpikPreviousButton:
      Result := cxGetResourceString(@scxGridFindPanelPreviousButtonCaption);
    else
      Result := inherited GetItemCaption(AItem);
  end;
end;

procedure TcxGridFindPanel.FocusControlContent;
var
  AFocusedRecord: TcxCustomGridRecord;
  ARecordToFocus: TcxCustomGridRecord;
begin
  AFocusedRecord := Controller.FocusedRecord;
  if ((AFocusedRecord = nil) or not AFocusedRecord.Visible) and (Controller.TopRecordIndex >= 0) then
  begin
    ARecordToFocus := GridView.ViewData.Records[Controller.TopRecordIndex];
    if ARecordToFocus <> nil then
    begin
      ARecordToFocus.Focused := True;
      Controller.MakeFocusedRecordVisible;
    end;
  end;
  inherited FocusControlContent;
end;

procedure TcxGridFindPanel.GetControlContentViewParams(var AParams: TcxViewParams);
begin
  GridView.Styles.GetViewParams(vsContent, nil, nil, AParams);
end;

function TcxGridFindPanel.GetCriteria: TcxDataFindCriteria;
begin
  Result := GridView.DataController.FindCriteria;
end;
...
function TcxGridFindPanel.GetLookAndFeel: TcxLookAndFeel;
begin
...
function TcxGridFindPanel.GetControl: TcxControl;
begin
...
procedure TcxGridFindPanel.Invalidate(ARecalculate: Boolean = False);
begin
  if GridView.ViewInfo.FindPanelViewInfo.Calculated then
  begin
    if ARecalculate then
      GridView.ViewInfo.FindPanelViewInfo.Recalculate;
    GridView.ViewInfo.FindPanelViewInfo.Invalidate;
  end;
end;
...
function TcxGridFindPanelOptions.GetBehavior: TcxDataFindCriteriaBehavior;
begin
  Result := Options.Behavior;
end;
...
function TcxGridFindPanelOptions.GetLayout: TcxFindPanelLayout;
begin
...
function TcxGridFindPanelOptions.GetShowNextButton: Boolean;
begin
  Result := Options.ShowNextButton;
end;

function TcxGridFindPanelOptions.GetShowPreviousButton: Boolean;
begin
  Result := Options.ShowPreviousButton;
end;
...
procedure TcxGridFindPanelOptions.SetBehavior(AValue: TcxDataFindCriteriaBehavior);
begin
  Options.Behavior := AValue;
end;
...
procedure TcxGridFindPanelOptions.SetLayout(AValue: TcxFindPanelLayout);
begin
  Options.Layout := AValue;
end;
...
procedure TcxGridFindPanelOptions.SetShowNextButton(AValue: Boolean);
begin
  Options.ShowNextButton := AValue;
end;

procedure TcxGridFindPanelOptions.SetShowPreviousButton(AValue: Boolean);
begin
  Options.ShowPreviousButton := AValue;
end;
...
begin
  ...
  dxUIAShutdownProvider(FUIAProvider);
  ...
  FreeAndNil(FRepositoryItemTempProperties);
...
function TcxCustomGridTableItem.GetCustomFunctions(ACustomFunctions: TStrings): string;
begin
  GridView.DoQueryCustomFunctions(Self, ACustomFunctions);
end;

procedure TcxCustomGridTableItem.SetFilterActiveCustomFunction(const ACustomFunctionName: string;
  const AValue: Variant; const ADisplayText: string);
begin
  DataBinding.SetFilterActiveCustomFunction(ACustomFunctionName, AValue, ADisplayText);
end;
...
function TcxCustomGridTableItem.GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider;
begin
  Result := dxUIAGetProvider(FUIAProvider, CreateUIAProvider, SupportsUIAutomation and not IsDestroying, AIsWMGetObjectCall, False);
end;
...
procedure TcxCustomGridTableItem.SetAutomation(const AValue: TdxAutomationElementSettings);
begin
  Automation.Assign(AValue);
end;
...
procedure TcxCustomGridTableItem.SetOnGetFilterImages(Value: TcxGridGetFilterImagesEvent);
begin
  if not dxSameMethods(FOnGetFilterImages, Value) then
  begin
    FOnGetFilterImages := Value;
    Changed(ticProperty);
  end;
end;
...
begin
  ...
  Filer.DefineProperty('IsCaptionAssigned', ReadIsCaptionAssigned, WriteIsCaptionAssigned, IsCaptionAssigned and (Caption = ''));
end;
...
begin
  if FRepositoryItemTempProperties <> nil then
    PropertiesValueChanged; 
  PropertiesChanged;
end;
          ...
          if AName = 'SortIndex' then
            ...
            if AName = 'WasVisibleBeforeGrouping' then
              ...
              if AName = 'Expression' then
                AValue := DataBinding.StoredExpression
              else
        ...
        if AName = 'SortOrder' then
          ...
          if AName = 'SortIndex' then
            ...
            if AName = 'WasVisibleBeforeGrouping' then
              ...
              if AName = 'Expression' then
                DataBinding.StoredExpression := AValue
              else
...
function TcxCustomGridTableItem.GetFilterImages: TCustomImageList;
begin
  Result := FFilterImages;
end;

procedure TcxCustomGridTableItem.PopulateFilterImages(AValues: TcxFilterValueList);
var
  I: Integer;
  AItems: TcxImageComboBoxProperties;
  AItem: TcxImageComboBoxItem;
begin
  if Safe.Cast<TcxImageComboBoxProperties>(GetPropertiesValue, AItems) then
  begin
    FFilterImages := AItems.Images;
    for I := 0 to AValues.Count - 1 do
    begin
      if AValues[i].Kind = fviValue then
      begin
        AItem := AItems.FindItemByValue(AValues[i].Value);
        if AItem <> nil then
          AValues[i].ImageIndex := AItem.ImageIndex;
      end;
    end;
  end
  else
    FFilterImages := nil;
  DoGetFilterImages(AValues, FFilterImages);
end;
...
function TcxCustomGridTableItem.CanUseInExpression: Boolean;
begin
  Result := IsVisibleForExpressionEditor;
end;

function TcxCustomGridTableItem.GetReferenceName: string;
begin
  Result := GetAlternateCaption;
end;
...
function TcxCustomGridTableItem.CreateUIAProvider: IdxUIAProvider;
begin
  Result := nil;
end;
...
procedure TcxCustomGridTableItem.BeforeDestruction;
begin
  if Assigned(FUIAProvider) then
    FUIAProvider.BeginShutdown;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            APoint := TPoint.Create(Index, ARecord.Index);
            AWidth := GridView.ConditionalFormattingProvider.CalculateBestFitWidth(APoint, AWidth);
          end;
...
procedure TcxCustomGridTableItem.CalculateVisualParams;
begin
  FEmptyEditorHeight := GetEmptyEditorHeight;
end;
...
begin
  Result := CanFocus(Controller.FocusedRecord) and Editable and not IsExpression and
...
begin
  Result := Options.FilteringWithFindPanel and IsSearchable;
end;

function TcxCustomGridTableItem.CanFocus(ARecord: TcxCustomGridRecord): Boolean;
begin
  Result := IsVisibleForFocus(ARecord) and FOptions.Focusing and
...
procedure TcxCustomGridTableItem.CheckUsingInFindCriteria;
begin
  if IsDestroying then
    Exit;
  if CanSearch then
    DataController.FindCriteria.AddItem(Index)
  else
    DataController.FindCriteria.RemoveItem(Index);
end;
...
procedure TcxCustomGridTableItem.DoGetFilterImages(AValueList: TcxFilterValueList; var AImages: TCustomImageList);
begin
  if Assigned(FOnGetFilterImages) then
    FOnGetFilterImages(Self, AValueList, AImages);
end;

procedure TcxCustomGridTableItem.DoRefreshStylesCache;
begin
  FStyles.RefreshCache;
end;
...
begin
  Result := CanDataCellScroll and (FocusedCellViewInfo <> nil) and GridView.ViewInfo.IsCellPartVisible(FocusedCellViewInfo);
end;
...
function TcxCustomGridTableItem.GetEmptyEditorHeight: Integer;
begin
  Result := TdxVisualRefinements.Padding.ScaledHeight(GetScaleFactor);
end;
...
procedure TcxCustomGridTableItem.GetFilterDisplayText(const AValue: Variant; var ADisplayText: string);
begin
...
function TcxCustomGridTableItem.GetIsFixedByWidth: Boolean;
begin
...
begin
  ...
  begin
    ...
    AProperties.DefaultValuesProvider := GetDefaultValuesProvider(AProperties);
    AProperties.LockUpdate(False);
  end;
end;

function TcxCustomGridTableItem.IsDefaultVisibleForExpressionEditor: Boolean;
begin
  Result := Visible;
end;

function TcxCustomGridTableItem.IsExpression: Boolean;
begin
  Result := DataController.IsItemExpression(Index);
end;

function TcxCustomGridTableItem.IsSearchable: Boolean;
begin
  Result := Visible;
end;
...
function TcxCustomGridTableItem.IsVisibleForFocus(ARecord: TcxCustomGridRecord): Boolean;
begin
...
begin
  Result := IsVisibleForFocus(Controller.FocusedRecord);
end;
...
function TcxCustomGridTableItem.IsVisibleForExpressionEditor: Boolean;
begin
  Result := dxDefaultBooleanToBoolean(VisibleForExpressionEditor, IsDefaultVisibleForExpressionEditor);
end;
...
procedure TcxCustomGridTableItem.ResetFocus;
begin
  if GridView.IsDestroying or GridView.IsAssigningItems or not TryResetFocusToNext then
...
function TcxCustomGridTableItem.ShowFilteredItemsOnlyForFilteredValues: Boolean;
begin
  Result := GridView.Filtering.ItemFilteredItemsListShowFilteredItemsOnly and
    Options.FilteringFilteredItemsListShowFilteredItemsOnly;
end;
...
function TcxCustomGridTableItem.SupportsUIAutomation: Boolean;
begin
  Result := False;
end;

function TcxCustomGridTableItem.TryResetFocusToNext: Boolean;
begin
  Result := Controller.FocusNextItem(VisibleIndex, True, True, False, not Controller.IsMultiSelectPersistent)
end;

function TcxCustomGridTableItem.UIAProvider: IdxUIAProvider;
begin
  Result := GetUIAProvider(False);
end;
...
procedure TcxCustomGridTableItem.InitStyle(AStyle: TcxCustomEditStyle; const AParams: TcxViewParams; AFocused: Boolean);
begin
  AStyle.Color := AParams.Color;
  AStyle.Font := AParams.Font;
  AStyle.StyleData.FontColor := AParams.TextColor;
  AStyle.HotTrack := True;

  if ShowButtons(AFocused) then
    AStyle.ButtonTransparency := ebtNone
  else
...
begin
  if AProperties <> nil then
  begin
    Result := CreateEditViewData(AProperties, GetCellStyle);
...
function TcxCustomGridTableItem.CreateEditViewData(AProperties: TcxCustomEditProperties; ACellStyle: TcxEditStyle; AScaleFactor: TdxScaleFactor = nil): TcxCustomEditViewData;
begin
  if AScaleFactor = nil then
    AScaleFactor := GetScaleFactor;
  Result := AProperties.CreateViewData(ACellStyle, True, AScaleFactor);
end;
...
begin
  if Source is TcxCustomGridTableItem then
    ...
    begin
      ...
      Self.BestFitMaxWidth := Self.GetScaleFactor.Apply(BestFitMaxWidth, GetScaleFactor);
      Self.DataBinding := DataBinding;
      Self.FFilterImages := FFilterImages;
      ...
      Self.MinWidth := Self.GetScaleFactor.Apply(MinWidth, GetScaleFactor);
      ...
      Self.Width := Self.GetScaleFactor.Apply(Width, GetScaleFactor);
      ...
      Self.OnGetFilterImages := OnGetFilterImages;
...
procedure TcxCustomGridTableItem.ApplyBestFit(ACheckSizingAbility: Boolean = False; AFireEvents: Boolean = False);
begin
...
function TcxCustomGridTableItem.CalculateDefaultCellHeight(ACanvas: TcxCanvas; AFont: TFont; ADrawAsStandalone: Boolean = False; AScaleFactor: TdxScaleFactor = nil): Integer;
var
...
begin
  ...
  try
    ...
    AEditSizeProperties := cxDefaultEditSizeProperties;
    AViewData := CreateEditViewData(GetProperties, AEditStyle, AScaleFactor); 
    try
      AViewData.ForceDrawAsStandalone := ADrawAsStandalone;
      Result := AViewData.GetEditSize(ACanvas, Null, AEditSizeProperties).cy;
    finally
      FreeAndNil(AViewData);
    end;
    if Result <> EmptyEditorHeight then
      Inc(Result, 2 * cxGridEditOffset)
    else
      Result := 0;
  finally
...
function TcxCustomGridTableItem.GetAutomationSettings: TdxAutomationElementSettings;
begin
  Result := Automation;
end;

function TcxCustomGridTableItem.GetDefaultValuesProvider(AProperties: TcxCustomEditProperties = nil): TcxCustomEditDefaultValuesProvider;
begin
  if FDataBinding = nil then
    Result := nil
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      FLastUsedDefaultRepositoryItem.AddListener(Self);
    end;
  end;
end;
...
procedure TcxCustomGridTableItem.ShowExpressionEditor;
begin
  if DataController.ExpressionProvider <> nil then
    DataController.ExpressionProvider.EditExpression(Index);
end;
...
class function TcxCustomGridTableItemAccess.CanIgnoreTimeForFiltering(AInstance: TcxCustomGridTableItem): Boolean;
begin
  Result := AInstance.CanIgnoreTimeForFiltering;
end;
...
begin
  ...
  ViewInfo.LookAndFeelPainter.DrawFilterCustomizeButton(Canvas, ViewInfo.Bounds, ViewInfo.Text, ViewInfo.ButtonState, ScaleFactor);
  DrawFocusRect;
end;
...
begin
  with ViewInfo do
    LookAndFeelPainter.DrawFilterPanel(Canvas, R, Transparent, Params.Color, BackgroundBitmap,
      ViewInfo.Borders, ScaleFactor, ViewInfo.GridView.BordersScaleFactor);
end;

procedure TcxGridFilterPainter.DrawContent;
begin
  inherited DrawContent;
  if ViewInfo.IsTokenCriteriaVisible then
    ViewInfo.TokenCriteriaViewInfo.Paint(Canvas);
end;

procedure TcxGridFilterPainter.DrawText;
begin
  if ViewInfo.IsTokenCriteriaVisible then
    Exit;
  inherited DrawText;
...
begin
  if ViewInfo.HasBorders then
    ViewInfo.LookAndFeelPainter.DrawFindPanelBorder(Canvas, ViewInfo.Bounds, ViewInfo.Borders, ScaleFactor,
...
procedure TcxCustomGridRecordPainter.ExcludeRectFromBackground(const R: TRect);
begin
  ViewInfo.GridViewInfo.Painter.ExcludeFromBackground(R);
end;

procedure TcxCustomGridRecordPainter.ExcludeFromBackground;
begin
  ExcludeRectFromBackground(ViewInfo.GetBackgroundBounds);
end;
...
constructor TcxCustomGridRecordsPainter.Create(ACanvas: TcxCanvas; AViewInfo: TcxCustomGridRecordsViewInfo);
begin
  inherited Create;
  FCanvas := ACanvas;
  FViewInfo := AViewInfo;
...
var
  AItems: TcxCustomGridRecordViewInfo;
  I: Integer;
begin
  for I := 0 to ViewInfo.Count - 1 do
  begin
    AItems := ViewInfo.Items[I];
    if (AItems <> nil) and AItems.Calculated then
      AItems.Paint;
  end;
end;
...
procedure TcxCustomGridTablePainter.RepaintOnOffset;
begin
  GridView.ViewChanged;
end;
...
begin
  ...
  FItemsListBox.Align := alClient;
  FItemsListBox.Parent := FItemsPage;
...
begin
  ...
  ADesktopWorkArea := GetDesktopWorkArea(OwnerScreenBounds);
...
begin
  AItemIndex := -1;
  AItemIndex2 := -1;
  TdxCustomCheckListBoxAccess(CheckListBox).SelectedIndexes.Clear;
  for I := 0 to AItems.Count - 1 do
  begin
...
function TcxCustomGridTableViewData.AddCustomFunctionToFilter(AParent: TcxFilterCriteriaItemList;
  AItem: TcxCustomGridTableItem; const ACustomFunctionName: string; const AValue: Variant;
  ADisplayText: string = ''; AReplaceExistent: Boolean = True): TcxFilterCriteriaItem;
begin
  if AReplaceExistent then
    DataController.Filter.RemoveItemByItemLink(AItem);
  Result := DataController.Filter.AddItem(AParent, AItem, ACustomFunctionName, AValue, ADisplayText);
end;
...
procedure TcxCustomGridTableViewData.AssignFocusedRecord;
begin
  Controller.AssignFocusedRecord;
end;
...
begin
  ...
  try
    ...
    TcxDataControllerAccess(DataController).IsBrowsing := True;
    try
      for I := 0 to RecordCount - 1 do
        Records[I].Expand(ARecurse);
    finally
      TcxDataControllerAccess(DataController).IsBrowsing := False;
    end;
  finally
    EndUpdate;
  end;
...
function TcxCustomGridTableViewData.GetRecordByRecordIndex(ARecordIndex: Integer): TcxCustomGridRecord;
  ...
  function FindRecordByRecordIndex(ARecordIndex: Integer): TcxCustomGridRecord;
  var
    I: Integer;
  begin
  ...
  if HasNewItemRecord and (ARecordIndex = DataController.NewItemRecordIndex) then
    ...
    if (0 <= ARecordIndex) and (ARecordIndex < DataController.RecordCount) then
      if GridView.IsUpdateLocked or not DataController.IsRowInfoValid then
        Result := FindRecordByRecordIndex(ARecordIndex)
      else
      begin
        AIndex := DataController.GetRowIndexByRecordIndex(ARecordIndex, False);
        Result := GetRecordByIndex(AIndex);
      end
    else
      Result := nil;
end;
...
begin
  ...
  AssignEditingRecord;
  AssignFocusedRecord;
  GridView.RefreshScrollbarAnnotations(dxAllScrollbarAnnotationKinds);
...
begin
  if (ADisplayText = '') and not (VarIsNull(AValue) or VarIsArray(AValue)) then
    ADisplayText := AItem.GetProperties.GetDisplayText(AValue);
...
begin
  ...
  Result := AUseCustomValue or (AProperties.GetEditValueSource(False) = evsValue) or
    (AProperties.GetEditValueSource(False) = evsText) and AProperties.IsValueFormattedByProperties;
  if not Result then
    Exit;
  if AUseCustomValue or (AProperties.GetEditValueSource(False) = evsValue) then
    AValue := DataController.Values[ARecordIndex, AItemIndex]
  else
    AValue := TcxDataControllerAccess(DataController).GetInternalDisplayText(ARecordIndex, AItemIndex);
...
constructor TcxCustomGridTableItemStyles.Create(AOwner: TPersistent);
begin
  FItem := AOwner as TcxCustomGridTableItem;
...
procedure TcxCustomGridTableItemStyles.GetDefaultViewParams(Index: Integer; AData: TObject; out AParams: TcxViewParams);
begin
  if Index = isContent then
...
procedure TcxCustomGridTableItemStyles.GetContentParams(ARecord: TcxCustomGridRecord; out AParams: TcxViewParams);
var
  AStyle: TcxStyle;
begin
  AStyle := nil;
...
procedure TcxCustomGridPartViewInfo.AfterConstruction;
begin
...
procedure TcxCustomGridPartViewInfo.BeforeDestruction;
begin
  if IsPart then
...
function TcxGridPartCustomCellViewInfo.DoCalculateHeight: Integer;
begin
  Result := 0;
end;

function TcxGridPartCustomCellViewInfo.DoCalculateWidth: Integer;
begin
  Result := 0;
end;
...
function TcxGridPartCustomCellViewInfo.GetHeight: Integer;
begin
...
function TcxGridPartCustomCellViewInfo.GetWidth: Integer;
begin
...
function TcxGridPinViewInfo.CaptureMouseOnPress: Boolean;
begin
  Result := False;
end;
...
procedure TcxGridPartContainerItemViewInfo.DoMouseLeave;
begin
  if not Enabled then
    Exit;
...
begin
  AWidth := EnsureRange(AWidth, Container.GetEditMinWidth, Container.GetEditMaxWidth);
end;
...
begin
  Result := TextHeight + LookAndFeelPainter.GetFilterCustomizeButtonContentOffsets(ScaleFactor).Height;
end;

function TcxGridPartButtonViewInfo.DoCalculateWidth: Integer;
begin
  Result := TextWidth + LookAndFeelPainter.GetFilterCustomizeButtonContentOffsets(ScaleFactor).Width;
end;
...
begin
  inherited Click;
  Controller.FindPanel.ItemExecute(fpikCloseButton);
end;

function TcxGridFindPanelCloseButtonViewInfo.GetFocused: Boolean;
begin
  Result := Controller.FindPanel.IsItemFocused(Kind) and GridView.IsControlFocused;
end;
...
function TcxGridFindPanelCloseButtonViewInfo.GetKind: TcxFindPanelItemKind;
begin
  Result := fpikCloseButton;
end;

function TcxGridFindPanelCloseButtonViewInfo.GetVisible: Boolean;
begin
  Result := Controller.FindPanel.IsItemVisible(Kind);
end;
...
procedure TcxGridFindPanelButtonViewInfo.Click;
begin
  Controller.AllowRecordScrollAnimationOnMakeVisible(
    procedure
    begin
      FindPanel.ItemExecute(Kind);
    end);
end;

function TcxGridFindPanelButtonViewInfo.DoCalculateHeight: Integer;
begin
  Result := Max(inherited DoCalculateHeight, ScaleFactor.Apply(FindPanel.ButtonMinHeight));
end;

function TcxGridFindPanelButtonViewInfo.DoCalculateWidth: Integer;
begin
  Result := Max(inherited DoCalculateWidth, ScaleFactor.Apply(FindPanel.ButtonMinWidth));
end;
...
function TcxGridFindPanelButtonViewInfo.GetEnabled: Boolean;
begin
  Result := FindPanel.IsItemEnabled(Kind);
end;

function TcxGridFindPanelButtonViewInfo.GetFocused: Boolean;
begin
  Result := FindPanel.IsItemFocused(Kind) and GridView.IsControlFocused;
end;

function TcxGridFindPanelButtonViewInfo.GetKind: TcxFindPanelItemKind;
begin
  Result := fpikNone;
end;

function TcxGridFindPanelButtonViewInfo.GetText: string;
begin
  Result := FindPanel.GetItemCaption(Kind);
end;

function TcxGridFindPanelButtonViewInfo.GetVisible: Boolean;
begin
  Result := FindPanel.IsItemVisible(Kind);
end;
...
function TcxGridFindPanelButtonViewInfo.GetFindPanel: TcxGridFindPanel;
begin
  Result := Controller.FindPanel;
end;
...
function TcxGridFindPanelFindButtonViewInfo.GetKind: TcxFindPanelItemKind;
begin
  Result := fpikFindButton;
end;
...
function TcxGridFindPanelClearButtonViewInfo.GetKind: TcxFindPanelItemKind;
begin
  Result := fpikClearButton;
end;

{ TcxGridFindPanelNextButtonViewInfo }

function TcxGridFindPanelNextButtonViewInfo.GetAdornerTargetElementName: string;
begin
  Result := 'NextButton';
end;

function TcxGridFindPanelNextButtonViewInfo.GetHitTestClass: TcxCustomGridHitTestClass;
begin
  Result := TcxGridFindPanelNextButtonHitTest;
end;

function TcxGridFindPanelNextButtonViewInfo.GetKind: TcxFindPanelItemKind;
begin
  Result := fpikNextButton;
end;

{ TcxGridFindPanelPreviousButtonViewInfo }

function TcxGridFindPanelPreviousButtonViewInfo.GetAdornerTargetElementName: string;
begin
  Result := 'Previous';
end;

function TcxGridFindPanelPreviousButtonViewInfo.GetHitTestClass: TcxCustomGridHitTestClass;
begin
  Result := TcxGridFindPanelPreviousButtonHitTest;
end;

function TcxGridFindPanelPreviousButtonViewInfo.GetKind: TcxFindPanelItemKind;
begin
  Result := fpikPreviousButton;
end;
...
begin
  Result := not Container.FilterViewInfo.Filter.IsEmpty;
end;
...
begin
  Result := Container.FilterViewInfo.MainFilter;
end;
...
begin
  Result := Container.FilterViewInfo.IsMRUItemsListAvailable;
end;
...
constructor TcxGridPartContainerButtonsViewInfo.Create(AOwner: TcxGridPartContainerViewInfo);
begin
  inherited Create;
  FOwner := AOwner;
...
begin
  ...
  begin
    AItem := Items[I];
    if AItem.Visible and not IsItemCalculatedSingly(AItem) then
    begin
      if AItem.Alignment = fbaLeft then
        ALeft := ALeftMargin
      else
        ALeft := ARightMargin - AItem.CalculateWidth;
      AItem.Calculate(ALeft, GetTopBound(AItem, ABounds));
      if AItem.Alignment = fbaLeft then
        ALeftMargin := AItem.Bounds.Right + GetButtonsOffset
      else
        ARightMargin := AItem.Bounds.Left - GetButtonsOffset;
    end;
    end;
end;
...
function TcxGridPartContainerButtonsViewInfo.CalculateBestWidth: Integer;
begin
  Result := WidthLeftPart + WidthRightPart;
end;

function TcxGridPartContainerButtonsViewInfo.IsItemCalculatedSingly(AItem: TcxGridPartCustomCellViewInfo): Boolean;
begin
  Result := False;
end;
...
begin
  Result := Owner.GetButtonsOffset;
  Inc(Result, ScaleFactor.Apply(TdxVisualRefinements.Padding.Left));
end;

function TcxGridPartContainerButtonsViewInfo.GetFirstOffset: Integer;
begin
  Result := Owner.GetFirstOffset;
end;

function TcxGridPartContainerButtonsViewInfo.GetTopBound(AItem: TcxGridPartCustomCellViewInfo; ABounds: TRect): Integer;
begin
  Result := MulDiv(ABounds.Top + ABounds.Bottom - AItem.CalculateHeight, 1, 2);
end;
...
begin
  ...
  if Result <> 0 then
  begin
    Inc(Result, 2 * GetVertOffset);
    Inc(Result, TdxVisualRefinements.Padding.ScaledHeight(ScaleFactor));
  end;
end;
...
begin
  ...
  begin
    AItem := Items[I];
    if AItem.Visible and not IsItemCalculatedSingly(AItem) and (AItem.Alignment = AAlignment) then
...
function TcxGridPartContainerButtonsViewInfo.GetVertOffset: Integer;
begin
  Result := Owner.GetVertOffset;
end;
...
begin
  ...
  inherited Calculate(ABounds);
  OffsetRightAlignedItemsToEditViewInfo;
end;
...
begin
  ...
  AddItem(TcxGridFindPanelNextButtonViewInfo);
  AddItem(TcxGridFindPanelPreviousButtonViewInfo);
  AddItem(TcxGridFindPanelClearButtonViewInfo);
  AddItem(TcxGridFindPanelFindButtonViewInfo);
end;
...
function TcxGridFindPanelButtonsViewInfo.CalculateBestWidth: Integer;
begin
  Result := inherited CalculateBestWidth + GetEditMaxWidth;
end;
...
var
  AWidth: Integer;
begin
  AWidth := Max(0, ABounds.Width - WidthLeftPart - WidthRightPart);
  EditViewInfo.Calculate(ABounds.Left + WidthLeftPart, GetTopBound(EditViewInfo, ABounds), AWidth);
end;

function TcxGridFindPanelButtonsViewInfo.GetEditMaxWidth: Integer;
begin
  Result := FindPanelViewInfo.GetEditMaxWidth;
end;

function TcxGridFindPanelButtonsViewInfo.GetEditMinWidth: Integer;
begin
  Result := FindPanelViewInfo.GetEditMinWidth;
end;
...
function TcxGridFindPanelButtonsViewInfo.IsItemCalculatedSingly(AItem: TcxGridPartCustomCellViewInfo): Boolean;
begin
  Result := AItem = EditViewInfo;
end;

procedure TcxGridFindPanelButtonsViewInfo.OffsetRightAlignedItemsToEditViewInfo;
var
  AItem: TcxGridPartContainerItemViewInfo;
  AOffsetAssigned: Boolean;
  I, AOffset: Integer;
begin
  AOffset := 0;
  AOffsetAssigned := False;
  for I := Count - 1 downto 0 do
  begin
    AItem := Items[I];
    if AItem.Visible and (AItem.Alignment = fbaRight) then
    begin
      if not AOffsetAssigned then
      begin
        AOffset := EditViewInfo.Bounds.Right + GetButtonsOffset - AItem.Bounds.Left;
        AOffsetAssigned := True;
      end;
      AItem.Offset(AOffset, 0);
    end;
    end;
end;
...
function TcxGridPartContainerViewInfo.GetButtonsOffset: Integer;
begin
  Result := 0;
end;

function TcxGridPartContainerViewInfo.GetFirstOffset: Integer;
begin
  Result := 0;
end;
...
function TcxGridPartContainerViewInfo.GetVertOffset: Integer;
begin
...
begin
  inherited Create(AGridViewInfo);
  if GridView.FilterBox.UseTokens then
    FTokenCriteriaViewInfo := CreateTokenCriteriaViewInfo;
end;

destructor TcxGridFilterViewInfo.Destroy;
begin
  FreeAndNil(FTokenCriteriaViewInfo);
  inherited Destroy;
end;

procedure TcxGridFilterViewInfo.Calculate(ALeftBound: Integer; ATopBound: Integer;
  AWidth: Integer = -1; AHeight: Integer = -1);
begin
  inherited Calculate(ALeftBound, ATopBound, AWidth, AHeight);
  if IsTokenCriteriaVisible then
    TokenCriteriaViewInfo.Calculate(CalculateTokenCriteriaViewInfoBounds);
end;

procedure TcxGridFilterViewInfo.DoRightToLeftConversion(const ABounds: TRect);
begin
  inherited DoRightToLeftConversion(ABounds);
  if IsTokenCriteriaVisible then
    TokenCriteriaViewInfo.RightToLeftConversion(ABounds);
end;

function TcxGridFilterViewInfo.MouseDown(AHitTest: TcxCustomGridHitTest;
  AButton: TMouseButton; AShift: TShiftState): Boolean;
begin
  if IsTokenCriteriaVisible and TokenCriteriaViewInfo.HasMouse(AHitTest.Pos) then
  begin
    TokenCriteriaViewInfo.MouseDown(AHitTest.Pos);
    Result := True;
  end
  else
    Result := inherited MouseDown(AHitTest, AButton, AShift);
...
function TcxGridFilterViewInfo.MouseMove(AHitTest: TcxCustomGridHitTest; AShift: TShiftState): Boolean;
begin
  if IsTokenCriteriaVisible and TokenCriteriaViewInfo.HasMouse(AHitTest.Pos) then
  begin
    TokenCriteriaViewInfo.MouseMove(AHitTest.Pos);
    Result := True;
  end
  else
    Result := inherited MouseMove(AHitTest, AShift);
...
function TcxGridFilterViewInfo.MouseUp(AHitTest: TcxCustomGridHitTest;
  AButton: TMouseButton; AShift: TShiftState): Boolean;
begin
  if IsTokenCriteriaVisible and TokenCriteriaViewInfo.HasMouse(AHitTest.Pos) then
  begin
    TokenCriteriaViewInfo.MouseUp(AHitTest.Pos);
    Result := True;
  end
  else
    Result := inherited MouseUp(AHitTest, AButton, AShift);
...
begin
  CalculateParams;
  Result := Max(inherited CalculateHeight, TextHeightWithOffset + 2 * FilterTextOffset);
  if TokenCriteriaViewInfo <> nil then
    Result := Max(Result, TokenCriteriaViewInfo.ContentHeight + 2 * GetVertOffset);
end;

function TcxGridFilterViewInfo.CalculateTokenCriteriaViewInfoBounds: TRect;
begin
  Result := ContentBounds;
  Result.Left := Result.Left + FButtonsViewInfo.WidthLeftPart + FilterTextOffset;
  Result.Right := Result.Right - FButtonsViewInfo.WidthRightPart - FilterTextOffset;
end;
...
function TcxGridFilterViewInfo.CreateTokenCriteriaViewInfo: TdxFilterBoxTokenCriteriaViewInfo;
begin
  Result := TdxFilterBoxTokenCriteriaViewInfo.Create(Self);
end;
...
function TcxGridFilterViewInfo.GetButtonsOffset: Integer;
begin
  Result := ScaleFactor.Apply(FilterButtonsOffset);
end;

function TcxGridFilterViewInfo.GetFirstOffset: Integer;
begin
...
begin
  Result := IsMRUItemsListAvailable;
end;
...
function TcxGridFilterViewInfo.IsMRUItemsListAvailable: Boolean;
begin
  Result := GridView.Filtering.IsMRUItemsListAvailable and not GridView.FilterBox.UseTokens;
end;

function TcxGridFilterViewInfo.IsTokenCriteriaVisible: Boolean;
begin
  Result := (TokenCriteriaViewInfo <> nil) and not Filter.IsEmpty;
end;
...
begin
  if TokenCriteriaViewInfo <> nil then
    Result := TokenCriteriaViewInfo.Criteria
  else
    Result := MainFilter;
end;
...
function TcxGridFilterViewInfo.GetMainFilter: TcxDataFilterCriteria;
begin
  Result := GridView.DataController.Filter;
end;

function TcxGridFilterViewInfo.GetTokenCriteria: TdxFilterBoxTokenCriteria;
begin
  Result := GridView.FilterBoxTokenCriteria;
end;

function TcxGridFilterViewInfo.GetTokenCriteriaControl: TcxControl;
begin
  Result := GridView.Site;
end;

function TcxGridFilterViewInfo.GetTokenCriteriaOptions: IdxFilterBoxTokenCriteriaOptions;
begin
  Result := GridView.FilterBox.TokenCriteria;
end;

procedure TcxGridFilterViewInfo.GetTokenCriteriaParams(var AParams: TcxViewParams);
begin
...
function TcxGridFindPanelViewInfo.CalculateBestWidth: Integer;
begin
  Result := ButtonsViewInfo.CalculateBestWidth;
end;

function TcxGridFindPanelViewInfo.CalculateHeight: Integer;
begin
  CalculateParams;
...
function TcxGridFindPanelViewInfo.GetButtonsOffset: Integer;
begin
  Result := FindPanel.GetButtonsOffset;
end;

function TcxGridFindPanelViewInfo.GetEditMaxWidth: Integer;
begin
  Result := ScaleFactor.Apply(FindPanel.GetEditMaxWidth);
end;

function TcxGridFindPanelViewInfo.GetEditMinWidth: Integer;
begin
  Result := ScaleFactor.Apply(FindPanel.EditMinWidth);
end;

function TcxGridFindPanelViewInfo.GetFirstOffset: Integer;
begin
  Result := FindPanel.GetButtonsFirstOffset;
end;
...
function TcxGridFindPanelViewInfo.GetVertOffset: Integer;
begin
  Result := FindPanel.GetButtonsVertOffset;
end;
...
function TcxGridFindPanelViewInfo.HasBorders: Boolean;
begin
  Result := True;
end;

procedure TcxGridFindPanelViewInfo.HideEdit;
begin
  ButtonsViewInfo.EditViewInfo.HideEdit;
end;

procedure TcxGridFindPanelViewInfo.ShowEdit;
begin
...
begin
  ...
  FGridViewInfo := FRecordViewInfo.GridViewInfo;
end;
...
function TcxGridTableCellViewInfo.GetHotTracked: Boolean;
begin
  if not FHotTrackedCalculated then
  begin
    FHotTracked := CalculateHotTracked;
    FHotTrackedCalculated := True;
  end;
  Result := FHotTracked;
end;
...
function TcxGridTableCellViewInfo.AllowDrawHotTracked: Boolean;
begin
  Result := HotTrackSelection or not Selected;
end;

function TcxGridTableCellViewInfo.AllowDrawSelected: Boolean;
begin
  Result := not HotTrackSelection or not HotTracked;
end;

function TcxGridTableCellViewInfo.AlwaysDrawSelectedWhenFocused: Boolean;
begin
  Result := not GridView.OptionsSelection.InvertSelect;
end;

function TcxGridTableCellViewInfo.CalculateHotTracked: Boolean;
begin
  Result := GridView.DrawDataCellHotTracked(GridRecord, Self);
end;
...
procedure TcxGridTableCellViewInfo.CalculateTextSelectionInfo;
var
  ASelStart: Integer;
  AHighlightedText: string;
begin
  inherited CalculateTextSelectionInfo;
  if (TextForPainting <> '') and NeedHighlightFindText then
  begin
    GetFindTextSelectionInfo(ASelStart, AHighlightedText);
    if ASelStart <> -1 then
    begin
      TextSelStart := ASelStart;
      TextSelLength := Length(AHighlightedText);
      PrepareFindTextSelColor;
    end;
  end;
end;

function TcxGridTableCellViewInfo.CanHighlightFindText: Boolean;
begin
  Result := False;
end;

function TcxGridTableCellViewInfo.DrawHotTrackedWhenRecordHotTracked: Boolean;
begin
  Result := GridView.OptionsSelection.InvertSelect or not GridView.OptionsSelection.CellSelect or
...
begin
  Result := inherited GetHotTrack or CanDrawHotTracked or RecordViewInfo.HotTrack;
end;

procedure TcxGridTableCellViewInfo.GetFindTextSelectionInfo(out ASelStart: Integer; out AHighlightedText: string);
begin
  ASelStart := -1;
  AHighlightedText := '';
end;
...
procedure TcxGridTableCellViewInfo.HotTrackChanged(AHotTracked: Boolean);
begin
  inherited HotTrackChanged(AHotTracked);
  if RecordViewInfo.HotTrack then
    RecordViewInfo.HotTrackChanged(AHotTracked);
end;

function TcxGridTableCellViewInfo.HotTrackSelection: Boolean;
begin
  Result := GridView.OptionsBehavior.HotTrackSelection;
end;
...
function TcxGridTableCellViewInfo.NeedHighlightFindText: Boolean;
begin
  Result := GridView.NeedHighlightFindText and CanHighlightFindText;
end;

procedure TcxGridTableCellViewInfo.PrepareFindTextSelColor;
var
  AHighlightParams: TcxViewParams;
  AParams: TcxViewParams;
begin
  GetViewParams(AParams);
  GridView.Styles.GetViewParams(vsSearchResultHighlight, nil, nil, AHighlightParams);
  cxCalculateTextSelectionColors(AHighlightParams.TextColor, AHighlightParams.Color, AParams.Color);
  TextSelColor := AHighlightParams.TextColor;
  TextSelBkColor := AHighlightParams.Color;
end;

procedure TcxGridTableCellViewInfo.BeforeRecalculation;
begin
  inherited BeforeRecalculation;
  FSelectedCalculated := False;
  FHotTrackedCalculated := False;
end;
...
function TcxGridTableCellViewInfo.CanDrawHotTracked: Boolean;
begin
  Result := GridView.OptionsBehavior.HotTrack and GridView.SupportsHotTrack;
end;
...
procedure TcxGridTableDataCellViewInfo.AfterDrawCellBackgroundHandler(Sender: TcxCustomEditViewInfo; ACanvas: TcxCanvas);
begin
  if FConditionalFormattingProvider <> nil then
    FConditionalFormattingProvider.AfterDrawCellBackground(Self, Sender.BackgroundColor, ACanvas);
end;

procedure TcxGridTableDataCellViewInfo.AfterDrawCellValueHandler(Sender: TcxCustomEditViewInfo; ACanvas: TcxCanvas);
begin
  if FConditionalFormattingProvider <> nil then
    FConditionalFormattingProvider.AfterDrawCellValue(Self, ACanvas);
end;

procedure TcxGridTableDataCellViewInfo.CalculateEditorBoundsHandler(AViewInfo: TcxCustomEditViewInfo; var R: TRect);
begin
  if FConditionalFormattingProvider <> nil then
    FConditionalFormattingProvider.CalculateCellEditorBounds(Self, R);
end;

procedure TcxGridTableDataCellViewInfo.CanDrawEditValueHandler(Sender: TcxCustomEditViewInfo; var Allow: Boolean);
begin
  if FConditionalFormattingProvider <> nil then
    FConditionalFormattingProvider.CanDrawCellValue(Self, Allow);
end;
...
procedure TcxGridTableDataCellViewInfo.SetUsedForPaintingHiddenEditor(AValue: Boolean);
begin
  if AValue <> UsedForPaintingHiddenEditor then
  begin
    FUsedForPaintingHiddenEditor := AValue;
    UpdateEditInfo;
  end;
end;
...
function TcxGridTableDataCellViewInfo.AllowDrawHotTracked: Boolean;
begin
  Result := inherited AllowDrawHotTracked and not Editing;
end;

function TcxGridTableDataCellViewInfo.AllowDrawSelected: Boolean;
begin
  Result := inherited AllowDrawSelected and (not Editing or CanDrawSelectedWhenEditing);
end;
...
procedure TcxGridTableDataCellViewInfo.InitializeByHiddenEditor(AEditViewInfo: TcxCustomEditViewInfo; AEdit: TcxCustomEdit);
begin
  Style.ButtonTransparency := ebtNone;
  TcxCustomEditAccess(AEdit).InitializeInfoForHiddenInplaceEditor(EditViewData, AEditViewInfo);
end;

procedure TcxGridTableDataCellViewInfo.CalculateEditViewInfo(AEditViewInfo: TcxCustomEditViewInfo; const AMousePos: TPoint);

  function CanDrawTwoOrMoreLines(AProperties: TcxCustomEditProperties; AEditHeight: Integer): Boolean;
  var
    APadding: TdxPadding;
  begin
    if (AProperties is TcxMaskEditProperties) or (AProperties is TcxTextEditProperties) then
    begin
      if GridView.LookAndFeelPainter.SupportsEditorPadding(True) then
        APadding := GridView.LookAndFeelPainter.GetEditorPadding(True, ScaleFactor)
      else
        APadding := TdxVisualRefinements.Padding.GetScaled(ScaleFactor);
      Result := AEditHeight >= TdxTextMeasurer.TextLineHeight(Params.Font) * 2 + APadding.Height + cxTextOffset * 2;
    end
    else
      Result := True;
  end;
...
begin
  ...
  try
    AControl := Control;
    FEditViewData.UseRightToLeftAlignment := AControl.UseRightToLeftAlignment;
    FEditViewData.UseRightToLeftReading := AControl.UseRightToLeftReading;
    FEditViewData.UseRightToLeftScrollBar := AControl.UseRightToLeftScrollBar;
    ...
    AEditBounds := EditViewDataBounds;

    if UsedForPaintingHiddenEditor then
      InitializeByHiddenEditor(AEditViewInfo, Controller.EditingController.Edit)
    else
    begin
      FEditViewData.PaintOptions := [];
      if AutoHeight or (MultiLine and CanDrawTwoOrMoreLines(EditViewData.Properties, AEditBounds.Height)) then
      begin
      ...
      InitViewDataTextSelection;
      EditViewData.EditValueToDrawValue(DisplayValue, AEditViewInfo);
    end;

    if FConditionalFormattingProvider <> nil then
      FConditionalFormattingProvider.CalculateStyleViewInfo(Self);
    FEditViewData.Calculate(Canvas, AEditBounds, AMousePos, cxmbNone, [], AEditViewInfo, True);
  finally
    DestroyEditViewData;
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      InitEditSizeProperties(AEditSizeProperties);
      if AEditSizeProperties.Width > 0 then
      begin
        Result := GetEditSize(AEditSizeProperties).cy;
        FClientExtent := FEditViewData.ClientExtent;
      end
      else
        Result := 0;
      if Result <> Item.EmptyEditorHeight then
      begin
        Inc(FClientExtent.Top, cxGridEditOffset);
        Inc(FClientExtent.Bottom, cxGridEditOffset);
        Inc(Result, 2 * cxGridEditOffset);
      end
      else
        if not SupportsZeroHeight then
          Result := Item.CalculateDefaultCellHeight(Canvas, Params.Font)
        else
          Result := 0;
    finally
      DestroyEditViewData;
...
function TcxGridTableDataCellViewInfo.CanDrawSelectedWhenEditing: Boolean;
begin
  Result := False;
end;

function TcxGridTableDataCellViewInfo.CanHighlightEditViewDataFindText: Boolean;
begin
  Result := Item.CanSearch and (RecordViewInfo.GridRecord.RecordIndex >= 0);
end;
...
begin
  AControl := Control;
  Result := (AControl <> nil) and AControl.HandleAllocated and not (csDestroyingHandle in AControl.ControlState);
end;
...
procedure TcxGridTableDataCellViewInfo.DoMouseLeave;
begin
  inherited DoMouseLeave;
  if HotTrack then
...
begin
  Result := RecordViewInfo.RecordsViewInfo.GetCellAreaBounds(Self);
end;
...
begin
  ...
  Result.Deflate(cxGridEditOffset, cxGridEditOffset);
...
begin
  R.Init(cxGridEditOffset, cxGridEditOffset, cxGridEditOffset, cxGridEditOffset);
end;

procedure TcxGridTableDataCellViewInfo.GetEditViewDataFindTextSelectionInfo(out ASelStart: Integer; out AHighlightedText: string);
begin
  ASelStart := GridView.DataController.FindCriteria.GetTextStartPositionByRowIndex(
    GridRecord.Index, Item.Index, AHighlightedText);
end;

function TcxGridTableDataCellViewInfo.GetEditViewDataTextSelectionInfo(out AParams: TcxViewParams; out ASelStart, ASelLength: Integer): Boolean;
var
  AText: string;
  APos, AStyleIndex: Integer;
begin
  AStyleIndex := 0;
  Result := False;
  if NeedHighlightEditViewDataFindText then
  begin
    GetEditViewDataFindTextSelectionInfo(ASelStart, AText);
    ...
    begin
      ASelStart := 0;
      AText := Controller.IncSearchingText;
      ASelLength := Length(AText);
      if Controller.IncSearchingFromAnyPos then
      begin
        APos := TdxDiacriticStringOptions.AnsiTextPos(AText, DisplayValue);
        if APos > 0  then
          ASelStart := APos - 1;
      end;
...
function TcxGridTableDataCellViewInfo.GetIsMain: Boolean;
begin
  Result := True;
end;
...
begin
  if UsedForPaintingHiddenEditor then
    Result := FItem.GetPropertiesForEdit
  else
...
function TcxGridTableDataCellViewInfo.GetUsedForPaintingHiddenEditor: Boolean;
begin
  Result := Controller.EditingController.HasHiddenEditor and Editing;
end;
...
procedure TcxGridTableDataCellViewInfo.InitEditSizeProperties(out AEditSizeProperties: TcxEditSizeProperties);
begin
  AEditSizeProperties.InitValues(MaxLineCount, -1, TextAreaBounds.Width, nil);
  Inc(AEditSizeProperties.Width, 2 * (ScaleFactor.Apply(cxGridCellTextOffset) - cxGridEditOffset));
end;
...
procedure TcxGridTableDataCellViewInfo.InitViewDataTextSelection;
var
  AParams: TcxViewParams;
  ASelStart, ASelLength: Integer;
begin
  if GetEditViewDataTextSelectionInfo(AParams, ASelStart, ASelLength) then
  begin
    EditViewData.SelStart := ASelStart;
    EditViewData.SelLength := ASelLength;
    EditViewData.SelBackgroundColor := AParams.Color;
    EditViewData.SelTextColor := AParams.TextColor;
  end
  else
    EditViewData.SelLength := 0;
end;
...
function TcxGridTableDataCellViewInfo.NeedHighlightEditViewDataFindText: Boolean;
begin
  Result := GridView.NeedHighlightFindText and CanHighlightEditViewDataFindText;
end;

function TcxGridTableDataCellViewInfo.NeedLocalCopyOfEditViewData: Boolean;
begin
  Result := UsedForPaintingHiddenEditor;
end;
...
begin
  AView := GridView;
  if AView.HasConditionalFormattingProvider then
    FConditionalFormattingProvider := AView.ConditionalFormattingProvider
  else
    FConditionalFormattingProvider := nil;
...
begin
  ...
  Result.Owner := Self;
  Result.OnAfterDrawBackground := AfterDrawCellBackgroundHandler;
  Result.OnAfterDrawValue := AfterDrawCellValueHandler;
  Result.OnCanDrawEditValue := CanDrawEditValueHandler;
  Result.OnCalculateEditorBounds := CalculateEditorBoundsHandler;
end;
...
procedure TcxGridTableDataCellViewInfo.InitEditInfo;
begin
...
procedure TcxGridTableDataCellViewInfo.UpdateEditInfo;
begin
...
procedure TcxGridTableDataCellViewInfo.Calculate(ALeftBound, ATopBound: Integer; AWidth: Integer = -1; AHeight: Integer = -1);
begin
  UsedForPaintingHiddenEditor := GetUsedForPaintingHiddenEditor;
  inherited Calculate(ALeftBound, ATopBound, AWidth, AHeight);
  CalculateEditViewInfo(FEditViewInfo, MousePos);
  Text := GetText;
  if not GridViewInfo.IsInternalUse and Editing then
    UpdateEdit;
end;
...
function TcxGridTableDataCellViewInfo.MouseDown(AHitTest: TcxCustomGridHitTest; AButton: TMouseButton; AShift: TShiftState): Boolean;
var
  AGridViewLink, ASelfLink: TcxObjectLink;
  AItem: TcxCustomGridTableItem;
  AActivateEdit, AHotTrack, APostCheckEdit: Boolean;
  AOrigin, AMousePos: TPoint;
begin
        ...
        begin
          ...
          if AItem.Focused then
            ...
            begin
              ...
              if AHotTrack then
              begin
                AMousePos := AHitTest.Pos;
                AHitTest := AItem.Controller.ViewInfo.GetHitTest(AMousePos);
                if AHitTest.ViewInfo <> nil then
                  AMousePos.Offset(AHitTest.ViewInfo.Bounds.Left - AOrigin.X, AHitTest.ViewInfo.Bounds.Top - AOrigin.Y);
                AItem.Controller.EditingController.ShowEdit(AItem, AShift, AMousePos.X, AMousePos.Y);
              end
              else
...
begin
  if Controller.IsRecordHot(Self) then
    Controller.ResetHotRecord;
  if not FIsRecordViewInfoAssigned and (FRecord <> nil) then
  begin
    if Assigned(FRecord.FUIAProvider) then
    begin
      dxUIAShutdownProvider(FRecord.FUIAProvider);
      FRecord.FUIAProvider := nil;
    end;
    FRecord.UIANotifyRowsChanged;
...
function TcxCustomGridRecordViewInfo.GetController: TcxCustomGridTableController;
begin
  Result := TcxCustomGridTableController(inherited Controller);
end;
...
function TcxCustomGridRecordViewInfo.GetHotTracked: Boolean;
begin
  if not FHotTrackedCalculated then
  begin
    FHotTracked := CalculateHotTracked;
...
function TcxCustomGridRecordViewInfo.AllowDrawHotTracked: Boolean;
begin
  Result := not GridView.Controller.IsScrollAnimationActive and
    ((GridView.OptionsBehavior.HotTrackSelection or not Selected) and
    (not GridRecord.Focused or not GridView.Controller.IsEditing) or
    not GridView.OptionsSelection.InvertSelect and not GridView.OptionsSelection.MultiSelect and
    GridRecord.HasCells and GridRecord.CanFocusCells);
end;

function TcxCustomGridRecordViewInfo.AllowDrawSelected: Boolean;
begin
  Result := not GridView.OptionsBehavior.HotTrackSelection or not HotTracked or
    not GridView.OptionsSelection.InvertSelect and not GridView.OptionsSelection.MultiSelect and
    GridRecord.HasCells and GridRecord.CanFocusCells;
end;

procedure TcxCustomGridRecordViewInfo.BeforeOffset;
begin
  ResetState;
end;

function TcxCustomGridRecordViewInfo.CalculateHotTracked: Boolean;
begin
  Result := CanDrawHotTracked and AllowDrawHotTracked and Controller.IsRecordHot(Self);
end;

procedure TcxCustomGridRecordViewInfo.CalculateInPlace(ALeft, ATop: Integer);
begin
  MainCalculate(ALeft, ATop);
  if GridView.UseRightToLeftAlignment then
    RightToLeftConversion(RecordsViewInfo.Bounds);
end;
...
begin
  Result := GridView.DrawRecordSelected(GridRecord) and AllowDrawSelected;
end;

function TcxCustomGridRecordViewInfo.CanDrawHotTracked: Boolean;
begin
  Result := GridView.OptionsBehavior.HotTrack and GridView.SupportsHotTrack;
end;
...
function TcxCustomGridRecordViewInfo.GetHotTrack: Boolean;
begin
  Result := inherited GetHotTrack or CanDrawHotTracked;
end;
...
procedure TcxCustomGridRecordViewInfo.HotTrackChanged(AHotTracked: Boolean);
begin
  inherited HotTrackChanged(AHotTracked);
  if AHotTracked then
    Controller.SetHotRecord(Self)
  else
    if Controller.IsRecordHot(Self) and not IsHotTracked then
      Controller.SetHotRecord(nil);
end;
...
function TcxCustomGridRecordViewInfo.InvalidateOnStateChange: Boolean;
begin
  Result := False;
end;
...
procedure TcxCustomGridRecordViewInfo.Calculate(ALeftBound, ATopBound: Integer; AWidth: Integer = -1; AHeight: Integer = -1);
begin
  FSelectedCalculated := False;
  FHotTrackedCalculated := False;
  inherited Calculate(ALeftBound, ATopBound, AWidth, AHeight);
...
function TcxCustomGridRecordViewInfo.MouseDown(AHitTest: TcxCustomGridHitTest;
  AButton: TMouseButton; AShift: TShiftState): Boolean;
begin
  Result := (AButton = mbLeft) and (AHitTest.HitTestCode = htExpandButton);
  if Result then
    GridRecord.ToggleExpanded
  else
    Result := (AButton <> mbMiddle) and IsClickHitTest(AHitTest) and GridView.Site.IsFocused and
      Click(AHitTest, AButton, AShift);
end;
...
function TcxCustomGridRecordsViewInfo.GetPixelScrollOffset(AFirstRecordIndex, AFirstRecordOffset: Integer;
  ALastRecordIndex, ALastRecordOffset: Integer): Integer;
var
  AIndex: Integer;
  ARecordSize: Integer;
  ASign: Integer;
begin
  Result := 0;
  AFirstRecordIndex := Controller.GetFirstIndexInScrollBand(AFirstRecordIndex);
  ALastRecordIndex := Controller.GetFirstIndexInScrollBand(ALastRecordIndex);
  if (AFirstRecordIndex > ALastRecordIndex) or
    (AFirstRecordIndex = ALastRecordIndex) and (AFirstRecordOffset < ALastRecordOffset) then
  begin
    ExchangeLongWords(AFirstRecordIndex, ALastRecordIndex);
    ExchangeLongWords(AFirstRecordOffset, ALastRecordOffset);
    ASign := -1;
  end
  else
    ASign := 1;
  AIndex := AFirstRecordIndex;
  while AIndex < ALastRecordIndex do
  begin
    ARecordSize := GetRecordScrollSize(AIndex);
    Inc(Result, ARecordSize);
    Inc(AIndex, Controller.GetRecordCountInScrollBand(AIndex));
  end;
  Inc(Result, AFirstRecordOffset - ALastRecordOffset);
  Result := ASign*Abs(Result);
end;
...
function TcxCustomGridRecordsViewInfo.GetCellAreaBounds(ACellViewInfo: TcxGridTableDataCellViewInfo): TRect;
begin
  ...
  begin
    ...
    for I := I to I + Abs(AItemCountDelta) - 1 do
      ...
      begin
        ...
        Items[I].CalculateInPlace(GetItemLeftBound(I), GetItemTopBound(I));
      end;
  end;
...
function TcxGridCustomTableNavigatorViewInfo.GetScaleFactor: TdxScaleFactor;
begin
  Result := FGridViewInfo.ScaleFactor;
end;
...
procedure TcxNavigatorSiteViewInfo.DoMouseLeave;
begin
  inherited DoMouseLeave;
  if not GridView.IsDestroying then
...
begin
  ...
  GridView.UpdateItemsVisualParams;
end;
...
function TcxCustomGridTableViewInfo.IsCellPartVisible(ACell: TcxGridTableDataCellViewInfo): Boolean;
begin
  Result := not ScrollableAreaBoundsForEdit.Contains(ACell.EditBounds);
end;
...
procedure TcxGridFilterValueList.Load(AItem: TcxCustomGridTableItem; AInitSortByDisplayText: Boolean;
  AUseFilteredValues: Boolean; AAddValueItems: Boolean; AUniqueOnly: Boolean; AFilteredValuesShowFilteredItemsOnly: Boolean);
begin
  inherited Load(AItem.Index, AInitSortByDisplayText, AUseFilteredValues, AAddValueItems,
    AUniqueOnly, AFilteredValuesShowFilteredItemsOnly);
...
begin
  Load(AItem, AInitSortByDisplayText, AUseFilteredValues, AAddValueItems, True, True);
end;
...
function TcxGridRecordChange.IsEqualCore(AChange: TcxCustomGridChange): Boolean;
begin
  Result := inherited IsEqualCore(AChange) and
...
begin
  with GridView as TcxCustomGridTableView do
    ...
    begin
      ...
      RefreshScrollbarAnnotations([dxFocusedRowScrollbarAnnotationID]);
...
function TcxGridItemDataBinding.GetExpression: string;
begin
  if csWriting in Item.ComponentState then
    Result := StoredExpression
  else
    Result := DataController.ItemExpressions[Item.Index];
end;
...
function TcxGridItemDataBinding.GetStoredExpression: string;
begin
  Result := DataController.ItemExpressions[Item.Index];
  if Result <> '' then
    Result := InvariantExpressionFlag + DataController.ExpressionProvider.ExpressionToInvariantExpression(Result);
end;
...
procedure TcxGridItemDataBinding.SetExpression(const AValue: string);
begin
  if csReading in Item.ComponentState then
    StoredExpression := AValue
  else
    DataController.ItemExpressions[Item.Index] := AValue;
end;

procedure TcxGridItemDataBinding.SetStoredExpression(const AValue: string);
var
  AExpression: string;
begin
  AExpression := AValue;
  if (AExpression <> '') and (AExpression[1] = InvariantExpressionFlag) then
  begin
    Delete(AExpression, 1, 1);
    AExpression := DataController.ExpressionProvider.InvariantExpressionToExpression(AExpression);
  end;
  DataController.ItemExpressions[Item.Index] := AExpression;
end;
...
procedure TcxGridItemDataBinding.ReadIsNullValueType(AReader: TReader);
begin
  if AReader.ReadBoolean then
    ValueTypeClass := nil;
end;

procedure TcxGridItemDataBinding.WriteIsNullValueType(AWriter: TWriter);
begin
  AWriter.WriteBoolean(ValueTypeClass = nil);
end;

procedure TcxGridItemDataBinding.AddCustomFunctionToFilter(AParent: TcxFilterCriteriaItemList;
  const ACustomFunctionName: string; const AValue: Variant; const ADisplayText: string; AReplaceExistent: Boolean = True);
begin
  GridView.ViewData.AddCustomFunctionToFilter(AParent, FItem, ACustomFunctionName, AValue, ADisplayText, AReplaceExistent);
end;

procedure TcxGridItemDataBinding.DefineProperties(Filer: TFiler);
begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('IsNullValueType', ReadIsNullValueType, WriteIsNullValueType, ValueTypeClass = nil);
end;
...
begin
  AValueList.Load(Item, AInitSortByDisplayText,
    ...
    Item.UseValueItemsForFilterValueList, AUniqueOnly, Item.ShowFilteredItemsOnlyForFilteredValues);
...
procedure TcxGridItemDataBinding.SetFilterActiveCustomFunction(const ACustomFunctionName: string;
  const AValue: Variant; const ADisplayText: string);
begin
  GridView.BeginFilteringUpdate;
  try
    Filter.BeginUpdate;
    try
      AddCustomFunctionToFilter(nil, ACustomFunctionName, AValue, ADisplayText);
      Filter.Active := True;
    finally
      Filter.EndUpdate;
    end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        if AInfo.CustomFunctionName <> '' then
          AddCustomFunctionToFilter(AList, AInfo.CustomFunctionName, AInfo.Value, AInfo.DisplayText)
        else
...
function TcxGridItemDataBinding.GetDefaultValuesProvider(AProperties: TcxCustomEditProperties = nil): TcxCustomEditDefaultValuesProvider;
begin
...
function TcxCustomGridRecord.CreateUIAProvider: IdxUIAProvider;
begin
  Result := nil;
end;
...
function TcxCustomGridRecord.GetAutomationSettings: TdxAutomationElementSettings;
begin
  Result := nil;
end;
...
begin
  ...
  begin
    RecordInfo.Expanded := False;
    RecordInfo.Level := 0;
    RecordInfo.RecordIndex := DataController.NewItemRecordIndex;
  end
  else
...
procedure TcxCustomGridRecord.UIANotifyRowsChanged;
begin
end;

function TcxCustomGridRecord.UIAProvider(AIsCreating: Boolean): IdxUIAProvider;
begin
  if AIsCreating then
    Result := GetUIAProvider(False)
  else
...
function TcxCustomGridRecord.GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider;
begin
  Result := dxUIAGetProvider(FUIAProvider, CreateUIAProvider, SupportsUIAutomation and not FIsDestroying, AIsWMGetObjectCall, False);
end;
...
function TcxCustomGridRecord.HasErrorData: Boolean;
var
  I: Integer;
  AErrorData: TcxEditValidateInfo;
begin
  ...
  begin
    GridView.VisibleItems[I].DoValidateDrawValue(Self, Values[GridView.VisibleItems[I].Index], AErrorData);
    Result := TcxEditValidateInfoAccess(AErrorData).IsError;
    if Result then
      Break;
  end;
  AErrorData.Free;
...
function TcxCustomGridRecord.SupportsUIAutomation: Boolean;
begin
  Result := False;
end;
...
procedure TcxCustomGridRecord.BeforeDestruction;
begin
  FIsDestroying := True;
  inherited BeforeDestruction;
end;
...
begin
  inherited;
  if NeedCheckScrolling then
    Controller.CheckScrolling(P);
end;

function TcxCustomGridTableMovingObject.NeedCheckScrolling: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  FDragOpening := True;
  FDragScrolling := True;
  FHotTrack := GetDefaultHotTrack;
  FHotTrackSelection := GetDefaultHotTrackSelection;
...
procedure TcxCustomGridTableOptionsBehavior.SetHotTrack(Value: Boolean);
begin
  if Value <> HotTrack then
  begin
    FHotTrack := Value;
    Changed(vcLayout);
  end;
...
procedure TcxCustomGridTableOptionsBehavior.SetHotTrackSelection(Value: Boolean);
begin
  if Value <> HotTrackSelection then
  begin
    FHotTrackSelection := Value;
    Changed(vcLayout);
  end;
end;
...
function TcxCustomGridTableOptionsBehavior.GetDefaultHotTrack: Boolean;
begin
  Result := False;
end;

function TcxCustomGridTableOptionsBehavior.GetDefaultHotTrackSelection: Boolean;
begin
  Result := True;
end;
...
begin
  if Source is TcxCustomGridTableOptionsBehavior then
    ...
    begin
      ...
      Self.HotTrackSelection := HotTrackSelection;
...
begin
  if Source is TcxCustomGridTableOptionsCustomize then
    ...
    begin
      Self.ItemExpressionEditing := ItemExpressionEditing;
...
{ TcxGridScrollbarAnnotations }

constructor TcxGridScrollbarAnnotations.Create(AOwner: TcxCustomGridTableView);
begin
  inherited Create;
  FGridView := AOwner;
end;

procedure TcxGridScrollbarAnnotations.GetHint(AAnnotationRowIndexLists: TdxScrollbarAnnotationRowIndexLists; var AHint: string);
begin
  FGridView.GetScrollbarAnnotationHint(AAnnotationRowIndexLists, AHint);
end;

function TcxGridScrollbarAnnotations.GetScrollableRecordCount: Integer;
begin
  Result := FGridView.Controller.ScrollRecordCount;
end;

function TcxGridScrollbarAnnotations.GetScrollableRecordIndexByRecordIndex(AIndex: Integer): Integer;
begin
  Result := AIndex - FGridView.Controller.FirstScrollRecordIndex;
end;

function TcxGridScrollbarAnnotations.GetCurrentRecordIndex: Integer;
begin
  Result := FGridView.GetCurrentScrollbarAnnotationRecordIndex;
end;

function TcxGridScrollbarAnnotations.GetDataPixelScrollSize: Integer;
begin
  Result := FGridView.Controller.GetPixelScrollSize;
end;

function TcxGridScrollbarAnnotations.GetLastIndexInScrollBand(ARecordIndex: Integer): Integer;
begin
  Result := FGridView.Controller.GetLastIndexInScrollBand(ARecordIndex);
end;

function TcxGridScrollbarAnnotations.GetOptions: TdxScrollbarAnnotationOptions;
begin
  Result := FGridView.ScrollbarAnnotations.Helper;
end;

function TcxGridScrollbarAnnotations.GetRecordIndexByScrollableRecordIndex(AIndex: Integer): Integer;
begin
  Result := AIndex + FGridView.Controller.FirstScrollRecordIndex;
end;

function TcxGridScrollbarAnnotations.GetRecordPixelScrollPosition(ARecordIndex: Integer): Integer;
begin
  Result := FGridView.ViewData.Records[ARecordIndex].PixelScrollPosition;
end;

function TcxGridScrollbarAnnotations.IsRecordIndexBasedRendering: Boolean;
begin
  Result := not FGridView.Controller.IsPixelBasedScrollDataPos or FGridView.IsEqualScrollSizeRecords;
end;

procedure TcxGridScrollbarAnnotations.Populate(AKind: TdxScrollbarAnnotationKind; AList: TdxScrollbarAnnotationRowIndexList);
begin
  FGridView.PopulateScrollbarAnnotations(AKind, AList);
end;

procedure TcxGridScrollbarAnnotations.SetCurrentRecordIndex(AIndex: Integer);
begin
  FGridView.SetCurrentScrollbarAnnotationRecordIndex(AIndex);
end;

{ TcxGridScrollbarAnnotationOptionsHelper }

constructor TcxGridScrollbarAnnotationOptionsHelper.Create(
...
procedure TcxGridScrollbarAnnotationOptionsHelper.Changed;
begin
  FOwner.Changed(vcProperty);
end;

procedure TcxGridScrollbarAnnotationOptionsHelper.CheckScrollbarAnnotations;
begin
  FOwner.CheckScrollbarAnnotations;
end;

function TcxGridScrollbarAnnotationOptionsHelper.IsVisible(AKind: Integer): Boolean;
begin
  Result := inherited IsVisible(AKind) and FOwner.IsScrollbarAnnotationVisible(AKind);
end;

procedure TcxGridScrollbarAnnotationOptionsHelper.RefreshScrollbarAnnotations(
  AChangedAnnotationKinds: TdxScrollbarAnnotationKinds);
begin
  FOwner.RefreshScrollbarAnnotations(AChangedAnnotationKinds);
end;

{ TcxGridScrollbarAnnotationOptions }

constructor TcxGridScrollbarAnnotationOptions.Create(AGridView: TcxCustomGridView);
begin
  inherited Create(AGridView);
  FHelper := CreateHelper;
end;

destructor TcxGridScrollbarAnnotationOptions.Destroy;
begin
  FreeAndNil(FHelper);
  inherited Destroy;
end;

procedure TcxGridScrollbarAnnotationOptions.Assign(Source: TPersistent);
begin
  if Source is TcxGridScrollbarAnnotationOptions then
    FHelper.Assign(TcxGridScrollbarAnnotationOptions(Source).FHelper);
  inherited;
end;

function TcxGridScrollbarAnnotationOptions.GetCustomScrollbarAnnotations: TdxCustomScrollbarAnnotations;
begin
  Result := FHelper.CustomAnnotations;
end;

function TcxGridScrollbarAnnotationOptions.CreateHelper: TcxGridScrollbarAnnotationOptionsHelper;
begin
  Result := TcxGridScrollbarAnnotationOptionsHelper.Create(GridView as TcxCustomGridTableView);
end;

function TcxGridScrollbarAnnotationOptions.GetOwner: TPersistent;
begin
  Result := GridView;
end;

function TcxGridScrollbarAnnotationOptions.GetValue(const Index: Integer): Boolean;
begin
  Result := FHelper.GetValue(Index);
end;

function TcxGridScrollbarAnnotationOptions.IsValueStored(Index: Integer): Boolean;
begin
  Result := FHelper.GetDefaultValue(Index) <> GetValue(Index);
end;

procedure TcxGridScrollbarAnnotationOptions.SetCustomScrollbarAnnotations(const Value: TdxCustomScrollbarAnnotations);
begin
  FHelper.CustomAnnotations := Value;
end;

procedure TcxGridScrollbarAnnotationOptions.SetValue(const Index: Integer; const Value: Boolean);
begin
  FHelper.SetValue(Index, Value);
end;
...
begin
  ...
  if Source is TcxCustomGridTableViewStyles then
    ...
    begin
      ...
      Self.OnGetContentStyle := OnGetContentStyle;
      Self.OnGetHotTrackStyle := OnGetHotTrackStyle;
      Self.OnGetInactiveStyle := OnGetInactiveStyle;
      Self.OnGetSelectionStyle := OnGetSelectionStyle;
      Self.FUseOddEvenStyles := UseOddEvenStyles;
      Self.FActuallyUseOddEvenStyles := ActuallyUseOddEvenStyles;
    end;
end;

procedure TcxCustomGridTableViewStyles.GetConditionalFormattingContentParams(
  ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem; var AParams: TcxViewParams);
begin
  ...
  begin
    with GridView.ConditionalFormattingProvider do
      CalculateViewParams(AParams, ARecord.RecordIndex, AItem.Index);
  end;
end;
...
procedure TcxCustomGridTableViewStyles.SetOnGetHotTrackStyle(Value: TcxGridGetSelectionStyleEvent);
begin
  if not dxSameMethods(FOnGetHotTrackStyle, Value) then
  begin
    FOnGetHotTrackStyle := Value;
    GridView.Changed(vcProperty);
  end;
end;

procedure TcxCustomGridTableViewStyles.SetOnGetInactiveStyle(Value: TcxGridGetSelectionStyleEvent);
begin
  if not dxSameMethods(FOnGetInactiveStyle, Value) then
  begin
    FOnGetInactiveStyle := Value;
    GridView.Changed(vcProperty);
  end;
end;

procedure TcxCustomGridTableViewStyles.SetOnGetSelectionStyle(Value: TcxGridGetSelectionStyleEvent);
begin
  if not dxSameMethods(FOnGetSelectionStyle, Value) then
  begin
    FOnGetSelectionStyle := Value;
    GridView.Changed(vcProperty);
  end;
end;

procedure TcxCustomGridTableViewStyles.GetDefaultViewParams(Index: Integer; AData: TObject; out AParams: TcxViewParams);
const
...
begin
  ...
  with AParams, LookAndFeelPainter do
    case Index of
      vsContent:
        if ActuallyUseOddEvenStyles and (AData is TcxCustomGridRecord) then
          GetDefaultViewParams(StyleIndexes[Odd(TcxCustomGridRecord(AData).Index)], AData, AParams)
        else
        begin
      ...
      vsIncSearch, vsSearchResultHighlight:
        begin
          Color := DefaultSearchResultHighlightColor;
          TextColor := DefaultSearchResultHighlightTextColor;
        end;
      vsNavigator:
        begin
      ...
      vsSelection, vsHotTrack:
        begin
          if AData <> nil then
            with TcxGridCellPos(AData) do
              GetCellContentParams(GridRecord, Item, AParams);
          if Index = vsSelection then
          begin
          ...
          begin
            Color := DefaultHotTrackColor;
            TextColor := DefaultHotTrackTextColor;
          end;
        end;
    end;
end;

procedure TcxCustomGridTableViewStyles.GetHotTrackParams(ARecord: TcxCustomGridRecord;
...
begin
  ...
  try
    AStyle := nil;
    if (ARecord <> nil) and Assigned(FOnGetHotTrackStyle) then
      FOnGetHotTrackStyle(GridView, ARecord, AItem, AStyle);
    GetViewParams(vsHotTrack, ACellPos, AStyle, AParams)
  finally
    ACellPos.Free;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      if (ARecord <> nil) and Assigned(FOnGetSelectionStyle) then
        FOnGetSelectionStyle(GridView, ARecord, AItem, AStyle);
      GetViewParams(vsSelection, ACellPos, AStyle, AParams)
    end
    else
    begin
      if (ARecord <> nil) and Assigned(FOnGetInactiveStyle) then
        FOnGetInactiveStyle(GridView, ARecord, AItem, AStyle);
      GetViewParams(vsInactive, ACellPos, AStyle, AParams);
    end;
  finally
    ACellPos.Free;
  end;
...
function TcxCustomGridTableViewStyles.IsOddEvenStylesSupported: Boolean;
begin
  Result := True;
end;

procedure TcxCustomGridTableViewStyles.RefreshCache;
begin
  inherited RefreshCache;
  FActuallyUseOddEvenStyles := IsOddEvenStylesSupported and ((UseOddEvenStyles = bTrue) or
    (UseOddEvenStyles = bDefault) and ((ContentOdd <> nil) or (ContentEven <> nil) or
    (LookAndFeelPainter <> nil) and LookAndFeelPainter.GridLikeControlDefaultUseOddEvenStyle));
end;

procedure TcxCustomGridTableViewStyles.GetCellContentParams(
  ARecord: TcxCustomGridRecord; AItem: TObject; out AParams: TcxViewParams);
begin
...
procedure TcxCustomGridTableViewStyles.GetDataCellContentParams(
  ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem; out AParams: TcxViewParams);
begin
  if AItem <> nil then
...
procedure TcxCustomGridTableViewStyles.GetDataCellParams(ARecord: TcxCustomGridRecord;
  ...
  AIgnoreSelection: Boolean = False; AIgnoreHotTrack: Boolean = False);
var
  AContentParamsDone: Boolean;
begin
  AContentParamsDone := False;
  if (ARecord <> nil) and (AItem <> nil) and GridView.HasConditionalFormattingProvider then
  begin
    GetDataCellContentParams(ARecord, AItem, AParams);
    AContentParamsDone := True;
  end;
  if not AIgnoreSelection and ((ACellViewInfo <> nil) and ACellViewInfo.Selected or
    (ACellViewInfo = nil) and GridView.DrawDataCellSelected(ARecord, AItem, AUseViewInfo)) then
      GetSelectionParams(ARecord, AItem, AParams)
  else
    if not AIgnoreHotTrack and ((ACellViewInfo <> nil) and ACellViewInfo.HotTracked or
      (ACellViewInfo = nil) and AUseViewInfo and GridView.DrawDataCellHotTracked(ARecord)) then
        GetHotTrackParams(ARecord, AItem, AParams)
    else
      if not AContentParamsDone then
        GetDataCellContentParams(ARecord, AItem, AParams);
end;
...
procedure TcxGridConditionalFormattingProvider.DoOwnerBeginUpdate;
begin
  FOwner.BeginUpdate(lsimImmediate);
end;

procedure TcxGridConditionalFormattingProvider.DoOwnerEndUpdate;
begin
  FOwner.EndUpdate;
end;

function TcxGridConditionalFormattingProvider.GetCellBounds(ACellViewInfo: TObject): TRect;
var
...
function TcxGridConditionalFormattingProvider.GetFocusedItemIndex: Integer;
var
  AFocusedItem: TcxCustomGridTableItem;
begin
  AFocusedItem := Owner.Controller.FocusedItem;
  if AFocusedItem <> nil then
    Result := AFocusedItem.Index
  else
    Result := -1;
end;
...
function TcxGridConditionalFormattingProvider.GetItemDisplayName(AItem: TObject): string;
var
  ATableItem: TcxCustomGridTableItem absolute AItem;
begin
  Result := ATableItem.Caption;
end;
...
begin
  if FOwner.Site = nil then
    ...
    Result := Vcl.Forms.GetParentForm(FOwner.Site);
end;
...
var
  ACell: TcxGridTableDataCellViewInfo absolute ACellViewInfo;
begin
  Result := TPoint.Create(ACell.Item.Index, ACell.GridRecord.RecordIndex);
end;
...
function TcxGridConditionalFormattingProvider.IsItemVisible(AItem: TObject): Boolean;
var
  AGridItem: TcxCustomGridTableItem absolute AItem;
begin
  Result := AGridItem.Visible or AGridItem.VisibleForCustomization;
end;
...
function TcxGridDetailModeConditionalFormattingProvider.IsIndependent: Boolean;
begin
  Result := False;
end;
...
function TcxGridOpenTableItemList.GetItem(Index: TdxListIndex): TcxCustomGridTableItem;
begin
  Result := TcxCustomGridTableItem(inherited Items[Index]);
end;

procedure TcxGridOpenTableItemList.SetItem(Index: TdxListIndex; Value: TcxCustomGridTableItem);
begin
...
function TcxCustomGridTableView.GetFilterBoxTokenCriteria: TdxFilterBoxTokenCriteria;
begin
  if FFilterBoxTokenCriteria = nil then
  begin
    FFilterBoxTokenCriteria := CreateFilterBoxTokenCriteria;
    UpdateFilterBoxTokenCriteria;
  end;
  Result := FFilterBoxTokenCriteria;
end;
...
begin
  Result := TcxCustomGridTableItem(FItems.List[Index]);
end;
...
procedure TcxCustomGridTableView.SetScrollbarAnnotations(AValue: TcxGridScrollbarAnnotationOptions);
begin
  FScrollbarAnnotations.Assign(AValue);
end;
...
procedure TcxCustomGridTableView.SetOnPopulateCustomScrollbarAnnotationRowIndexList(const Value: TcxGridPopulateCustomScrollbarAnnotationRowIndexList);
begin
  if not dxSameMethods(FOnPopulateCustomScrollbarAnnotationRowIndexList, Value) then
  begin
    FOnPopulateCustomScrollbarAnnotationRowIndexList := Value;
    Changed(vcProperty);
  end;
end;

procedure TcxCustomGridTableView.SetOnQueryCustomFunctions(Value: TcxQueryCustomFunctions);
begin
  if not dxSameMethods(FOnQueryCustomFunctions, Value) then
  begin
    FOnQueryCustomFunctions := Value;
    Changed(vcProperty);
  end;
end;
...
procedure TcxCustomGridTableView.SetOnGetScrollbarAnnotationHint(Value: TcxGridGetScrollbarAnnotationHint);
begin
  if not dxSameMethods(FOnGetScrollbarAnnotationHint, Value) then
  begin
    FOnGetScrollbarAnnotationHint := Value;
    Changed(vcProperty);
  end;
end;
...
//..
procedure TcxCustomGridTableView.SetOnFocusFirstAvailableItem( Value : TcxFocusFirstAvailableItemEvent ); //..
begin
  if dxSameMethods(FOnFocusFirstAvailableItem, Value) then
  begin
    FOnFocusFirstAvailableItem := Value;
    Changed(vcProperty);
  end;
end;
//..
...
procedure TcxCustomGridTableView.CheckScrollbarAnnotations;
begin
  if not IsDesigning and FScrollbarAnnotations.Active then
  begin
    FScrollbarAnnotationHelper := CreateScrollbarAnnotationHelper;
    TcxGridSiteAccess(Site).AddMainScrollBarsAsAnnotationRenderers;
  end
  else
    FreeAndNil(FScrollbarAnnotationHelper);
end;
...
procedure TcxCustomGridTableView.InvalidateRecordPixelScrollSizeDependendScrollbarAnnotations;
begin
  if (FScrollbarAnnotationHelper <> nil) and not FScrollbarAnnotationHelper.IsRecordIndexBasedRendering then
    FScrollbarAnnotationHelper.InvalidateRenderers;
end;
...
function TcxCustomGridTableView.AllowFilterExpressionEditing: Boolean;
begin
  Result := Filtering.ExpressionEditing;
end;

function TcxCustomGridTableView.GetExpressionprovider: TcxCustomExpressionProvider;
begin
  Result := DataController.ExpressionProvider;
end;

procedure TcxCustomGridTableView.RegisterFilterControl(AControl: TcxCustomFilterControl);
begin
  if FLinkedFilterControls.IndexOf(AControl) = -1 then
    FLinkedFilterControls.Add(AControl)
end;

procedure TcxCustomGridTableView.UnregisterFilterControl(AControl: TcxCustomFilterControl);
begin
  FLinkedFilterControls.Remove(AControl);
end;

procedure TcxCustomGridTableView.UpdateLinkedFilterControls;
var
  I: Integer;
begin
  for I := 0 to FLinkedFilterControls.Count - 1 do
    TcxCustomFilterControl(FLinkedFilterControls[I]).LayoutChanged;
end;

function TcxCustomGridTableView.UseFilterTokens: Boolean;
begin
  Result := FilterBox.UseTokens;
end;
...
begin
  case AButtonIndex of
    ...
    NBDI_INSERT, NBDI_APPEND:
      NavigatorCreateNewRecordExecute(AButtonIndex = NBDI_APPEND);
    NBDI_DELETE:
      Controller.DeleteSelection;
    NBDI_EDIT:
      NavigatorEditExecute;
    NBDI_POST:
      begin
...
procedure TcxCustomGridTableView.NavigatorEditExecute;
begin
  DataController.Edit;
  Controller.FocusedItem.Editing := True;
end;

procedure TcxCustomGridTableView.NavigatorCreateNewRecordExecute(AtEnd: Boolean);
begin
  Controller.CreateNewRecord(AtEnd);
end;
...
begin
  ...
  RefreshScrollbarAnnotations(FLoadedChangedScrollbarAnnotationKinds);
end;
...
procedure TcxCustomGridTableView.DestroyFilterBoxTokenCriteria;
begin
  FreeAndNil(FFilterBoxTokenCriteria);
end;
...
procedure TcxCustomGridTableView.DoRefreshStylesCache;
var
  I: Integer;
begin
  inherited;
  for I := 0 to ItemCount - 1 do
    Items[I].DoRefreshStylesCache;
end;

procedure TcxCustomGridTableView.DoStylesChanged;
begin
  inherited DoStylesChanged;
  if Controller.IsFindPanelVisible then
...
procedure TcxCustomGridTableView.CheckItemsUsingInFindCriteria;
var
  I: Integer;
begin
  BeginUpdate;
  try
    for I := 0 to ItemCount - 1 do
      Items[I].CheckUsingInFindCriteria;
  finally
    EndUpdate;
  end;
end;
...
procedure TcxCustomGridTableView.DoAssign(ASource: TcxCustomGridView);
  ...
  begin
    ATableView := TcxCustomGridTableView(ASource);
    BeginAssignItems;
    try
      for I := 0 to ATableView.ItemCount - 1 do
      begin
        AItem := FindItemByID(ATableView.Items[I].ID);
        if AItem = nil then
        begin
          AItem := CreateItem;
          AItem.FID := ATableView.Items[I].ID;
        end;
        AItem.Index := I;
        AItem.Assign(ATableView.Items[I]);
      end;
      for I := ItemCount - 1 downto ATableView.ItemCount do
      begin
        if Items[I].VisibleIndex >= 0 then
          FVisibleItems.Remove(Items[I]);
        Items[I].Free;
      end;
    finally
      EndAssignItems;
    end;
    FNextID := ATableView.FNextID;
  end;
    ...
    try
      ...
      begin
        ...
        if HasConditionalFormattingProvider and not Self.AssigningPattern and
          ...
          Self.ConditionalFormatting.Assign(ConditionalFormatting);
        Self.ScrollbarAnnotations := ScrollbarAnnotations;
        ...
        Self.OnPopulateCustomScrollbarAnnotationRowIndexList := OnPopulateCustomScrollbarAnnotationRowIndexList;
        Self.OnGetDragDropText := OnGetDragDropText;
        Self.OnGetScrollbarAnnotationHint := OnGetScrollbarAnnotationHint;
        Self.OnInitFilteringDateRanges := OnInitFilteringDateRanges;
        Self.OnInitGroupingDateRanges := OnInitGroupingDateRanges;
        ...
        Self.OnFocusFirstAvailableItem := OnFocusFirstAvailableItem; //..
      end;
    finally
      EndUpdate;
    end;
...
function TcxCustomGridTableView.GetCurrentScrollbarAnnotationRecordIndex: Integer;
begin
...
begin
  Result := inherited GetIsControlFocused or
    ...
    Controller.IsFindPanelVisible and Controller.FindPanel.IsEditFocused;
end;
...
function TcxCustomGridTableView.GetPostingLockedStateMode: TcxGridShowLockedStateImageMode;
var
  AGridControl: TcxCustomGridAccess;
begin
  AGridControl := TcxCustomGridAccess(Control);
  if (AGridControl = nil) or not AGridControl.CanUseLockedStateImage then
    Result := lsimNever
  else
    Result := OptionsBehavior.ShowLockedStateImageOptions.Posting;
end;

procedure TcxCustomGridTableView.GetScrollbarAnnotationHint(AAnnotationRowIndexLists: TdxScrollbarAnnotationRowIndexLists; var AHint: string);
begin
  if Assigned(FOnGetScrollbarAnnotationHint) then
    FOnGetScrollbarAnnotationHint(Self, AAnnotationRowIndexLists, AHint);
end;

function TcxCustomGridTableView.GetScrollbarAnnotationHelper: TdxScrollbarAnnotations;
begin
  Result := FScrollbarAnnotationHelper;
end;
...
begin
  inherited Init;
  if NeedFocusFirstItemAfterInit then
    Controller.FocusFirstAvailableItem;
end;

function TcxCustomGridTableView.IsDataScrollbar(AKind: TScrollBarKind): Boolean;
begin
...
begin
  Result := not DataController.IsGridMode and
    ...
    ((OptionsBehavior.RecordScrollMode = rsmDefault) and (cxIsTouchModeEnabled or IsScrollAnimationEnabled)));
end;

function TcxCustomGridTableView.IsScrollAnimationEnabled: Boolean;
begin
  Result := (LookAndFeel <> nil) and ((LookAndFeel.ScrollMode = scmSmooth) or
    (LookAndFeel.ScrollMode = scmDefault) and (cxDefaultLookAndFeelScrollMode = scmSmooth));
end;

function TcxCustomGridTableView.IsScrollbarAnnotationVisible(AKind: Integer): Boolean;
begin
  case AKind of
    dxFirstScrollbarAnnotationID..dxFirstPredefinedScrollbarAnnotationID-1:
      Result := Assigned(OnPopulateCustomScrollbarAnnotationRowIndexList);
    dxErrorScrollbarAnnotationID:
      Result := not DataController.IsGridMode;
    dxSearchResultScrollbarAnnotationID:
      Result := not DataController.IsGridMode and DataController.FindCriteria.IsActive;
...
function TcxCustomGridTableView.NeedFocusFirstItemAfterInit: Boolean;
begin
  Result := Controller.FocusedItem = nil;
end;
...
procedure TcxCustomGridTableView.PopulateScrollbarAnnotations(
  AKind: TdxScrollbarAnnotationKind; AList: TdxScrollbarAnnotationRowIndexList);
var
  I, AIndex: Integer;
  ARecord: TcxCustomGridRecord;
begin
  case AKind of
    dxFirstScrollbarAnnotationID..dxFirstPredefinedScrollbarAnnotationID-1:
      OnPopulateCustomScrollbarAnnotationRowIndexList(Self, AKind, AList);
    dxErrorScrollbarAnnotationID:
      for I := Controller.FirstScrollRecordIndex to Controller.LastScrollRecordIndex do
        if ViewData.Records[I].HasErrorData then
          AList.Add(I);
    dxSearchResultScrollbarAnnotationID:
      for I := 0 to DataController.FindCriteria.MatchCount - 1 do
      begin
        AIndex := DataController.FindCriteria.MatchRowIndexes[I];
        if Controller.IsRecordScrollable(AIndex) then
          AList.Add(AIndex);
      end;
    dxFocusedRowScrollbarAnnotationID:
      AList.Add(Controller.FocusedRecordIndex);
    dxSelectedRowScrollbarAnnotationID:
      for I := 0 to Controller.SelectedRecordCount - 1 do
      begin
        ARecord := Controller.SelectedRecords[I];
        if ARecord <> nil then
          AList.Add(ARecord.Index);
      end;
  end;
end;
...
procedure TcxCustomGridTableView.RefreshScrollbarAnnotations(AChangedAnnotationKinds: TdxScrollbarAnnotationKinds = dxAllScrollbarAnnotationKinds);
begin
  if IsLoading then
    FLoadedChangedScrollbarAnnotationKinds := FLoadedChangedScrollbarAnnotationKinds + AChangedAnnotationKinds
  else
    if FScrollbarAnnotationHelper <> nil then
      FScrollbarAnnotationHelper.Refresh(AChangedAnnotationKinds);
end;
...
procedure TcxCustomGridTableView.SetControl(Value: TcxControl);
begin
  if Controller <> nil then
    Controller.StopScrollAnimation;
  inherited SetControl(Value);
end;

procedure TcxCustomGridTableView.SetCurrentScrollbarAnnotationRecordIndex(AIndex: Integer);
begin
  Controller.AllowRecordScrollAnimationOnMakeVisible(
    procedure
    begin
      Controller.FocusedRecordIndex := AIndex;
    end);
end;
...
function TcxCustomGridTableView.SupportsHotTrack: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  if AInfo is TcxFindCriteriaChangedInfo then
    FindCriteriaChanged(TcxFindCriteriaChangedInfo(AInfo));
end;
...
procedure TcxCustomGridTableView.UpdateFilterBoxTokenCriteria;
begin
  FilterBoxTokenCriteria.UpdateBySourceCriteria;
end;
...
begin
  ...
  FLinkedFilterControls := TList.Create;
end;

procedure TcxCustomGridTableView.DestroyHandlers;
begin
  FreeAndNil(FLinkedFilterControls);
  DestroyFilterBoxTokenCriteria;
  FreeAndNil(FScrollbarAnnotationHelper);
...
begin
  ...
  FScrollbarAnnotations := CreateScrollbarAnnotations;
end;

procedure TcxCustomGridTableView.DestroyOptions;
begin
  FreeAndNil(FScrollbarAnnotations);
...
function TcxCustomGridTableView.CreateFilterBoxTokenCriteria: TdxFilterBoxTokenCriteria;
begin
  Result := TdxFilterBoxTokenCriteria.Create(DataController.Filter);
end;
...
function TcxCustomGridTableView.CreateScrollbarAnnotationHelper: TcxGridScrollbarAnnotations;
begin
  Result := TcxGridScrollbarAnnotations.Create(Self);
end;

function TcxCustomGridTableView.CreateScrollbarAnnotations: TcxGridScrollbarAnnotationOptions;
begin
  Result := TcxGridScrollbarAnnotationOptions.Create(Self);
end;
...
begin
  ...
  AItem.Visible := True;
  RefreshFilterableItemsList;
  DataController.UpdateExpressionItems;
  AItem.CheckUsingInFindCriteria;
end;
...
procedure TcxCustomGridTableView.AssignVisibleItemIndexes;
var
  I: Integer;
begin
  for I := 0 to FVisibleItems.Count - 1 do
    TcxCustomGridTableItem(FVisibleItems.List[I]).FVisibleIndex := I;
end;
...
begin
  ...
  begin
    ...
    if AItem.ActuallyVisible then
      AItem.FVisibleIndex := FVisibleItems.Add(AItem)
    else
...
begin
  ...
  Controller.EditingController.UpdateEditValue;
  if Controller.IsFindPanelVisible then
    Controller.FindPanel.UpdateViewInfo;
  Synchronize;
end;
...
begin
  ...
  RefreshScrollbarAnnotations(dxAllScrollbarAnnotationKinds);
  SizeChanged;
  Controller.EditingController.UpdateEditValue;
  if Controller.IsFindPanelVisible then
    Controller.FindPanel.UpdateViewInfo;
...
procedure TcxCustomGridTableView.FindCriteriaChanged(AInfo: TcxFindCriteriaChangedInfo);
begin
  if Controller.IsFindPanelVisible then
    Controller.FindPanel.CriteriaChanged(TcxFindCriteriaChangedInfo(AInfo).Changes);
  RefreshScrollbarAnnotations([dxSearchResultScrollbarAnnotationID]);
  LayoutChanged;
end;
...
var
  I: Integer;
  A110B35036260310402776A1A2512106E: TcxCustomGridRecord;
begin
  if NeedChangeLayoutOnSelectionChanged(AInfo) then
    ...
    begin
      A110B35036260310402776A1A2512106E := ViewData.GetRecordByIndex(AInfo.RowIndexes[I]);
      if A110B35036260310402776A1A2512106E <> nil then A110B35036260310402776A1A2512106E.Invalidate;
    end;
  if not IsLoading then
    NotifySelectionChanged(TcxSelectionChangedInfo(AInfo.Clone));
  RefreshScrollbarAnnotations([dxSelectedRowScrollbarAnnotationID]);
end;
...
function TcxCustomGridTableView.CalculateDataCellSelected(ARecord: TcxCustomGridRecord;
  ...
  begin
    if AUseViewInfo and (ARecord.ViewInfo <> nil) then
      Result := ARecord.ViewInfo.Focused
    else
      Result := DrawRecordFocused(ARecord);
  end;

  function GetCellFocused: Boolean;
  begin
    Result := (AItem <> nil) and AItem.Focused and GetRecordFocused;
  end;

begin
  Result := (not ARecord.HasCells or not ARecord.CanFocusCells or OptionsSelection.InvertSelect and
    (not GetCellFocused or (ACellViewInfo <> nil) and ACellViewInfo.AlwaysDrawSelectedWhenFocused) or
    not OptionsSelection.InvertSelect and (OptionsSelection.MultiSelect or GetCellFocused)) and
    ((ACellViewInfo = nil) or ACellViewInfo.AllowDrawSelected);
end;

function TcxCustomGridTableView.DrawDataCellHotTracked(ARecord: TcxCustomGridRecord;
  ACellViewInfo: TcxGridTableCellViewInfo = nil): Boolean;

  function GetRecordHotTracked: Boolean;
  begin
    Result := (ARecord <> nil) and (ARecord.ViewInfo <> nil) and ARecord.ViewInfo.HotTracked;
  end;

  function GetCellHotTracked: Boolean;
  begin
    Result := ACellViewInfo.DrawHotTrackedWhenRecordHotTracked or ACellViewInfo.IsHotTracked;
  end;

begin
  Result := (ACellViewInfo <> nil) and ACellViewInfo.CanDrawHotTracked and ACellViewInfo.AllowDrawHotTracked and
    GetRecordHotTracked and GetCellHotTracked or (ACellViewInfo = nil) and GetRecordHotTracked;
end;
...
function TcxCustomGridTableView.DrawDataCellSelected(ARecord: TcxCustomGridRecord;
  ...
  begin
    Result := (ACellViewInfo <> nil) and ACellViewInfo.AlwaysSelected and ACellViewInfo.AllowDrawSelected or
      CalculateDataCellSelected(ARecord, AItem, AUseViewInfo, ACellViewInfo);
  end;
...
begin
  Result := (ARecord.Selected and (Focused or OptionsSelection.MultiSelect) or
    ARecord.DragHighlighted) and DrawSelection;
end;
...
procedure TcxCustomGridTableView.DoQueryCustomFunctions(AItem: TcxCustomGridTableItem; ACustomFunctions: TStrings);
begin
  if Assigned(FOnQueryCustomFunctions) then
  begin
    FOnQueryCustomFunctions(AItem, AItem.FilterCaption, ACustomFunctions); // PropertyName == FilterCaption ??
  end;
end;
...
function TcxCustomGridTableView.NeedHighlightFindText: Boolean;
begin
  Result := DataController.FindCriteria.IsActive and FindPanel.HighlightSearchResults;
end;

procedure TcxCustomGridTableView.UpdateItemsVisualParams;
var
  I: Integer;
begin
  for I := 0 to VisibleItemCount - 1 do
    VisibleItems[I].CalculateVisualParams;
end;
...
procedure TcxCustomGridTableView.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if not IsDestroying and (Operation = opRemove) then
  begin
    if AComponent = Navigator.Buttons.Images then
      Navigator.Buttons.Images := nil;
  end;
end;
...
begin
  if PatternGridView <> Self then
    ...
    Result := GetConditionalFormattingProviderClass.Create(Self);
end;

function TcxCustomGridTableView.GetConditionalFormattingProviderClass: TcxGridConditionalFormattingProviderClass;
begin
  Result := TcxGridConditionalFormattingProvider;
end;
...
function TcxCustomGridTableView.GoToNextScrollbarAnnotation(AKinds: TdxScrollbarAnnotationKinds = dxAllScrollbarAnnotationKinds;
  AGoForward: Boolean = True; AGoOnCycle: Boolean = False): Boolean;
begin
  Result := (FScrollbarAnnotationHelper <> nil) and
    FScrollbarAnnotationHelper.GoToNext(AKinds, AGoForward, AGoOnCycle);
end;
...
{ TcxGridFindPanelNextButtonHitTest }

class function TcxGridFindPanelNextButtonHitTest.GetHitTestCode: Integer;
begin
  Result := htFindPanelNextButton;
end;

class function TcxGridFindPanelPreviousButtonHitTest.GetHitTestCode: Integer;
begin
  Result := htFindPanelPreviousButton;
end;
...
{ TcxGridDataFilterCriteriaItem }

function TcxGridDataFilterCriteriaItem.IsItemLinkSupportsMultiThreading: Boolean;
var
  AProperties: TcxCustomEditProperties;
begin
  AProperties := TcxCustomGridTableItem(ItemLink).GetProperties;
  Result := TcxCustomEditPropertiesAccess(AProperties).SupportsMultiThreading(CompareByDisplayValue);
end;

{ TcxGridDataFilterCriteria }

function TcxGridDataFilterCriteria.GetItemClass: TcxFilterCriteriaItemClass;
begin
  Result := TcxGridDataFilterCriteriaItem;
end;
...
function TcxGridDefaultValuesProvider.IsOnGetTextAssigned: Boolean;
begin
  Result := IsDisplayFormatDefined(False);
end;

{ TcxGridDataController }

constructor TcxGridDataController.Create(AOwner: TComponent);
begin
  FGridView := AOwner as TcxCustomGridTableView;
  inherited;
end;
...
function TcxGridDataController.CreateExpressionProvider: TcxDataCustomExpressionProvider;
begin
  Result := TcxDataControllerSpreadSheetExpressionProvider.Create(Self);
end;
...
function TcxGridDataController.GetFilterCriteriaClass: TcxDataFilterCriteriaClass;
begin
  Result := TcxGridDataFilterCriteria;
end;
...
begin
  ...
  FItemFilteredItemsListShowFilteredItemsOnly := True;
end;
...
function TcxCustomGridTableFiltering.GetExpressionEditing: Boolean;
begin
  Result := FExpressionEditing and TcxFilterCriteriaAccess(GridView.DataController.Filter).IsExpressionsSupported;
end;
...
procedure TcxCustomGridTableFiltering.SetExpressionEditing(Value: Boolean);
begin
  if FExpressionEditing <> Value then
  begin
    FExpressionEditing := Value;
    GridView.UpdateLinkedFilterControls;
    Changed(vcProperty);
  end;
end;
...
procedure TcxCustomGridTableFiltering.SetItemFilteredItemsListShowFilteredItemsOnly(AValue: Boolean);
begin
  if AValue <> ItemFilteredItemsListShowFilteredItemsOnly then
  begin
    FItemFilteredItemsListShowFilteredItemsOnly := AValue;
    Changed(vcProperty);
  end;
end;
...
begin
  if Source is TcxCustomGridTableFiltering then
    ...
    begin
      Self.ExpressionEditing := ExpressionEditing;
      ...
      Self.ItemFilteredItemsListShowFilteredItemsOnly := ItemFilteredItemsListShowFilteredItemsOnly;
...
begin
  inherited Create(AItem);
  FEditing := True;
  FExpressionEditing := True;
  FFiltering := True;
  FFilteringAddValueItems := True;
  FFilteringFilteredItemsList := True;
  FFilteringFilteredItemsListShowFilteredItemsOnly := True;
...
procedure TcxCustomGridTableItemOptions.SetFilteringFilteredItemsListShowFilteredItemsOnly(AValue: Boolean);
begin
  if AValue <> FilteringFilteredItemsListShowFilteredItemsOnly then
  begin
    FFilteringFilteredItemsListShowFilteredItemsOnly := AValue;
    Changed(ticProperty);
  end;
end;
...
begin
  if Source is TcxCustomGridTableItemOptions then
    ...
    begin
      ...
      Self.FilteringFilteredItemsListShowFilteredItemsOnly := FilteringFilteredItemsListShowFilteredItemsOnly;
...
begin
  ...
  begin
    if EditingItemSetting then Exit;
    EditingItemSetting := True;
    try
      if Value <> nil then
      begin
...
function TcxGridEditingController.GetMultilineEdit: TcxAutoHeightInplaceEdit;
var
  AProperties: TcxAutoHeightInplaceEditProperties;
begin
  AProperties := CreateEditAutoHeightProperties;
  try
    Result := EditList.GetEdit(AProperties) as TcxAutoHeightInplaceEdit;
    Result.SetEditingController(Self);
  finally
    AProperties.Free;
  end;
  Result.LockChangeEvents(True);
  try
    SetupMultilineEdit(Result);
  finally
    Result.LockChangeEvents(False, False);
  end;
end;
...
begin
  ...
  begin
    ...
    if MultiLineEdit.BoundsRect = R then
      Exit;
    ...
    begin
      ...
      begin
        ...
        if R.Height > MultiLineEdit.Height then
          MultiLineEdit.Height := R.Height; 
      end;
    end
    else
...
procedure TcxGridEditingController.SetupMultilineEdit(AEdit: TcxAutoHeightInplaceEdit);
begin
  AEdit.AutoHeight := AutoHeight;
  AEdit.BorderColor := GridView.OptionsView.EditAutoHeightBorderColor;
  if AEdit.BorderColor <> clNone then
  begin
    if AEdit.BorderColor = clDefault then
      AEdit.BorderColor := GridView.OptionsView.GetGridLineColor;
  end;
  if Assigned(Edit.RepositoryItem) then
    with AEdit.ActiveProperties do
    begin
  ...
  AEdit.ActiveProperties.ValidateOnEnter := False;
end;
...
function TcxGridEditingController.CreateEditAutoHeightProperties: TcxAutoHeightInplaceEditProperties;
begin
  Result := TcxAutoHeightInplaceEditProperties.Create(nil);
  Result.Assign(FEdit.ActiveProperties);
end;
...
begin
  ...
  begin
    FMultiLineEdit := GetMultilineEdit;
...
begin
  ...
  begin
    Edit.Deactivate; 
    PostEditingData;
    Edit.DoPostEditValueNotification;  //..
...
begin
     If   HideEditOnFocusedRecordChange
     then HideEdit(GridView.DataController.IsEditing and not FInsertingRecord )  //..
     else FUpdateEditStyleNeeded := True;
end;
...
begin
  ...
  begin
    FMultiLineEdit.SetMaxLineCount(EditingCellViewInfo.MaxLineCount);
    InitMultilineEditorMinHeight;
  end;
end;

procedure TcxGridEditingController.InitMultilineEditorMinHeight;
var
  ARowViewInfo: TcxCustomGridRecordViewInfo;
begin
...
begin
  ...
  try
    ...
    if EditPreparing or EditHiding or (AItem = nil) or (Controller.FocusedRecord = nil) or FInsertingRecord then //..
      Exit;
    FEditPreparing := True;
    try
      if AItem.Editing and not EditingItemSetting then
      begin
...
procedure TcxGridEditingController.PrepareEditViewInfo;
begin
  Edit.ViewInfo.Association := EditingCellViewInfo.EditViewInfo.Association;
  Edit.ViewInfo.ForceDrawAsStandalone := EditingCellViewInfo.EditViewInfo.ForceDrawAsStandalone;
  Edit.ViewInfo.ParentBackgroundProvider := EditingCellViewInfo.EditViewInfo.ParentBackgroundProvider;
end;

procedure TcxGridEditingController.RecalculateEditViewInfo;
var
  ACellViewInfo: TcxGridTableDataCellViewInfo;
begin
  ACellViewInfo := EditingCellViewInfo;
  if ACellViewInfo <> nil then
    ACellViewInfo.Recalculate;
end;
...
procedure TcxGridEditingController.EditExecuteAction(Action: TBasicAction; var Handled: Boolean);
begin
  Handled := GridView.DataController.ExecuteAction(Action);
end;
...
procedure TcxGridEditingController.EditUpdateAction(Action: TBasicAction; var Handled: Boolean);
begin
  Handled := GridView.DataController.UpdateAction(Action);
end;
...
begin
  inherited RemoveEdit(AProperties);
  if FMultilineEdit <> nil then
    inherited RemoveEdit(FMultilineEdit.ActiveProperties);
end;
...
{ TcxCustomGridTableRecordScrollAnimationHelper }

constructor TcxCustomGridTableRecordScrollAnimationHelper.Create(
  AOwner: TcxCustomGridTableController);
begin
  inherited Create;
  FOwner := AOwner;
end;

function TcxCustomGridTableRecordScrollAnimationHelper.CalculateScrollDelta(AValue: TdxRecordPixelScrollData): Integer;
var
  AFirstIndex, ALastIndex: Integer;
begin
  AFirstIndex := GetOwnerPosition.Index;
  ALastIndex := AValue.Index;
  FIsScrollingByRecord := Abs(ALastIndex - AFirstIndex) > FOwner.ViewInfo.RecordsViewInfo.GetPixelScrollContentSize;
  if FIsScrollingByRecord then
    Result := ALastIndex - AFirstIndex
  else
    Result := FOwner.ViewInfo.RecordsViewInfo.GetPixelScrollOffset(GetOwnerPosition.Index, GetOwnerPosition.ScrollOffset,
      AValue.Index, AValue.ScrollOffset);
end;

procedure TcxCustomGridTableRecordScrollAnimationHelper.AfterAnimationEnded(AIsRaiseEvent: Boolean);
begin
  inherited AfterAnimationEnded(AIsRaiseEvent);
  FOwner.ScrollAnimationEnded;
end;

procedure TcxCustomGridTableRecordScrollAnimationHelper.BeforeAnimationStarted;
begin
  FOwner.ScrollAnimationStarted;
  inherited BeforeAnimationStarted;
end;

function TcxCustomGridTableRecordScrollAnimationHelper.CalculateAnimatedPosition(var APosition: Integer): TdxRecordPixelScrollData;
var
  AOverPan: Boolean;
  ADelta: Integer;
begin
  if APosition = Abs(AnimationLength) then
    Result := TargetPosition
  else
  begin
    Result := GetOwnerPosition;
    ADelta := Sign(AnimationLength)*(APosition - PrevAnimatedPosition);
    if FIsScrollingByRecord then
    begin
      Result.Index := Result.Index + ADelta;
      Result.ScrollOffset := 0;
    end
    else
      FOwner.ViewInfo.RecordsViewInfo.CalculatePixelScrollInfo(Result.Index, Result.ScrollOffset,
        MaxInt, MinInt, -ADelta, AOverPan);
  end;
end;

function TcxCustomGridTableRecordScrollAnimationHelper.GetOwnerPosition: TdxRecordPixelScrollData;
begin
  Result := TdxRecordPixelScrollData.Create(FOwner.InternalTopRecordIndex, FOwner.PixelScrollRecordOffset);
end;

procedure TcxCustomGridTableRecordScrollAnimationHelper.PositionChanged;
begin
  FOwner.GridView.DoTopRecordIndexChanged;
end;

procedure TcxCustomGridTableRecordScrollAnimationHelper.SetTargetPosition(AValue: TdxRecordPixelScrollData);
begin
  AValue.Index := FOwner.GetAdjustedTopRecordIndex(AValue.Index);
  inherited SetTargetPosition(AValue);
end;

procedure TcxCustomGridTableRecordScrollAnimationHelper.UpdateOwnerPosition(AValue: TdxRecordPixelScrollData);
begin
  FOwner.DoSetTopRecordIndexWithOffset(AValue.Index, AValue.ScrollOffset); 
  FOwner.Site.Update;
end;
...
begin
  ...
  FRecordScrollAnimationHelper := TcxCustomGridTableRecordScrollAnimationHelper.Create(Self);
end;

destructor TcxCustomGridTableController.Destroy;
begin
  FreeAndNil(FRecordScrollAnimationHelper);
  FFilterPopup.Free;
...
function TcxCustomGridTableController.GetAdjustedTopRecordIndex(ATopRecordIndex: Integer): Integer;
begin
...
function TcxCustomGridTableController.GetIncSearchingFromAnyPos: Boolean;
begin
  Result := DataController.Search.SearchingFromAnyPos;
end;
...
begin
  ...
  begin
    ...
    if (FFocusedItem <> nil) and not FEditingController.EditingItemSetting then
...
var
  ...
  ANeedUseLockedStatePaint: Boolean;
begin
  if CanChangeFocusedRecordIndex(FocusedRecordIndex, Value) then
  begin
    AIndexesAreEqual := FocusedRecordIndex = Value;

    ANeedUseLockedStatePaint := (GridView.GetPostingLockedStateMode <> lsimNever) and DataController.CanFocusedRecordIndexChangePostData;
    if ANeedUseLockedStatePaint then
      TcxCustomGridTableViewAccess.BeginLockedStatePaintOnPost(GridView);
    try
      if not DataController.ChangeFocusedRowIndex(Value) then
        Exit;
      AssignFocusedRecord;
    finally
      if ANeedUseLockedStatePaint then
...
begin
  ...
  FScrollTimer.Interval := GetScrollDataTimeInterval(FScrollDirection);
...
begin
  ...
  begin
    ...
    begin
      Site.Update;
      TcxControlAccess(Site).DragAndDrop(Site.ScreenToClient(GetMouseCursorPos), AAccepted);
    end;
  end;
end;
...
procedure TcxCustomGridTableController.AssignFocusedRecord;
begin
  FFocusedRecord := GetFocusedRecord;
end;
...
procedure TcxCustomGridTableController.AllowRecordScrollAnimationOnMakeVisible(AProc: TProc);
begin
  FAllowRecordScrollAnimationOnMakeVisible := True;
  try
    AProc;
  finally
    FAllowRecordScrollAnimationOnMakeVisible := False;
  end;
end;
...
begin
  Result := (ARecord <> nil) and (GridView.Control <> nil) and IsRecordScrollable(ARecord.Index);
  if Result then
  begin
...
var
  ARecordIndex, ARecordOffset, APrevRecordIndex, APrevRecordOffset: Integer;
begin
  ...
  begin
    ...
    APrevRecordIndex := ARecordIndex;
    APrevRecordOffset := ARecordOffset;
    CheckTopRecordIndexAndOffset(ARecordIndex, ARecordOffset);
    if (ARecordIndex <> APrevRecordIndex) or (ARecordOffset <> APrevRecordOffset) then
...
begin
  ...
  try
    ...
    AIndex := GetAdjustedTopRecordIndex(AIndex);
...
var
  APrevIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition: Boolean;
  I: Integer;
begin
  APrevIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition := FIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition;
  if not IsPixelBasedScrollDataPos then
  begin
    Result := ScrollBarRecordCount;
    FIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition := False;
  end
  else
  begin
    ...
    begin
      ...
      FIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition := True;
    end;
    Inc(Result, GridView.GetHorizontalScrollBarAreaHeight);
  end;
  if not FIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition and
    APrevIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition then
  begin
    for I := FirstScrollRecordIndex to LastScrollRecordIndex do
      ViewData.Records[I].FPixelScrollPosition := 0;
    if GridView.GetScrollbarAnnotationHelper <> nil then
      GridView.GetScrollbarAnnotationHelper.InvalidateRenderers;
  end;
end;
...
function TcxCustomGridTableController.IsRecordScrollAnimationActive: Boolean;
begin
  Result := FRecordScrollAnimationHelper.IsAnimationActive;
end;

function TcxCustomGridTableController.IsScrollAnimationActive: Boolean;
begin
  Result := IsRecordScrollAnimationActive;
end;

function TcxCustomGridTableController.IsScrollAnimationEnabled: Boolean;
begin
  Result := GridView.IsScrollAnimationEnabled and not IsNavigationKeyAutoRepeat and IsRecordPixelScrolling;
end;

procedure TcxCustomGridTableController.StopScrollAnimation;
begin
  if IsRecordScrollAnimationActive then
    FRecordScrollAnimationHelper.StopAnimation(False);
end;
...
procedure TcxCustomGridTableController.DoSetTopRecordIndexWithOffset(ATopRecordIndex, ATopRecordOffset: Integer);
var
...
function TcxCustomGridTableController.GetActuallyFocusedItemIndex: Integer;
begin
...
begin
  ...
  if IsFindPanelVisible and FindPanel.IsFocused then
...
begin
  ...
  if (0 <= ARecordIndex) and (ARecordIndex < ViewData.RecordCount) then
    Result := ViewData.Records[ARecordIndex]
  else
    Result := nil;
end;
...
begin
  Result := LastScrollRecordIndex - GetPageVisibleRecordCount(LastScrollRecordIndex, 0, False) + 1;
end;
...
function TcxCustomGridTableController.GetPageVisibleRecordCount(AFirstRecordIndex: Integer;
  APixelScrollRecordOffset: Integer; ACalculateDown: Boolean = True): Integer;
begin
  Result := GetVisibleRecordCount(AFirstRecordIndex, APixelScrollRecordOffset, ACalculateDown);
...
function TcxCustomGridTableController.GetTargetRecordScrollIndex: Integer;
begin
  Result := FRecordScrollAnimationHelper.TargetPosition.Index;
end;

function TcxCustomGridTableController.GetTargetRecordScrollOffset: Integer;
begin
  Result := FRecordScrollAnimationHelper.TargetPosition.ScrollOffset;
end;
...
begin
  Result := ABottomRecordIndex - GetPageVisibleRecordCount(ABottomRecordIndex, 0, False) + 1;
end;

function TcxCustomGridTableController.GetVisibleRecordCount(AFirstRecordIndex: Integer; APixelScrollRecordOffset: Integer;
  ...
  if IsRectEmpty(ViewInfo.Bounds) then
    ...
    begin
      ...
      try
        ...
        AViewInfo.PixelScrollRecordOffset := APixelScrollRecordOffset;
...
begin
  if FCheckEditNeeded then
  begin
    CheckEdit;
    GridView.Invalidate(False);
  end;
end;
...
begin
  if ACell <> nil then
  begin
    FClickedDataCellInfo.Cell := ACell;
    FClickedDataCellInfo.Item := ACell.Item;
    FClickedDataCellInfo.RecordIndex := ACell.GridRecord.RecordIndex;
    end
  else
...
procedure TcxCustomGridTableController.SetTopRecordIndexWithOffset(ATopRecordIndex, ATopRecordOffset: Integer; AAnimated: Boolean = False);
var
  APrevTopRecordIndex: Integer;
begin
  if AAnimated then
    FRecordScrollAnimationHelper.SetPosition(TdxRecordPixelScrollData.Create(ATopRecordIndex, ATopRecordOffset))
  else
  begin
    if IsRecordScrollAnimationActive then
    begin
     APrevTopRecordIndex := FRecordScrollAnimationHelper.StartPosition.Index;
     FRecordScrollAnimationHelper.StopAnimation(False);
    end
    else
     APrevTopRecordIndex := InternalTopRecordIndex;
    DoSetTopRecordIndexWithOffset(ATopRecordIndex, ATopRecordOffset);
    if InternalTopRecordIndex <> APrevTopRecordIndex then
      GridView.DoTopRecordIndexChanged;
  end;
end;

procedure TcxCustomGridTableController.ScrollAnimationEnded;
begin
  if not IsScrollAnimationActive then
    AfterScrolling;
end;

procedure TcxCustomGridTableController.ScrollAnimationStarted;
begin
  if not IsScrollAnimationActive then
    BeforeScrolling;
end;
...
begin
  ...
  begin
    ...
    FGridModeBufferCountUpdateTimer.Interval := 1;
    FGridModeBufferCountUpdateTimer.OnTimer := GridModeBufferCountUpdateTimerHandler;
  end;
end;
...
var
  ...
  AIsAnyRowHeightChanged: Boolean;
begin
  Result := 0;
  AIsAnyRowHeightChanged := False;
  if ScrollRecordCount > 0 then
  ...
  begin
    ...
    AIsAnyRowHeightChanged := AIsAnyRowHeightChanged or (ViewData.Records[I].FPixelScrollPosition <> Result);
  ...
  if AIsAnyRowHeightChanged then
    GridView.InvalidateRecordPixelScrollSizeDependendScrollbarAnnotations;
end;
...
function TcxCustomGridTableController.IsRecordHot(AViewInfo: TcxCustomGridRecordViewInfo): Boolean;
begin
  Result := AViewInfo = FHotRecord;
end;

procedure TcxCustomGridTableController.ResetHotRecord;
begin
  FHotRecord := nil;
end;

procedure TcxCustomGridTableController.SetHotRecord(AValue: TcxCustomGridRecordViewInfo);
var
  APrevHotRecord: TcxCustomGridRecordViewInfo;
begin
  APrevHotRecord := FHotRecord;
  FHotRecord := AValue;
  if (APrevHotRecord <> nil) and (APrevHotRecord <> FHotRecord) then
    APrevHotRecord.Update;
  if FHotRecord <> nil then
    FHotRecord.Update;
end;
...
begin
  if FocusedRecord <> nil then
    FocusedRecord.Selected := not FocusedRecord.Selected;
end;
...
procedure TcxCustomGridTableController.FocusNextPage(ASyncSelection: Boolean);
  ...
  var
    AOverPan: Boolean;
    AOffset, ADelta: Integer;
    AMaxRecordIndex, AMaxRecordOffset: Integer;
  begin
    if IsRecordScrollAnimationActive then
    begin
      Result := TargetRecordScrollIndex;
      AOffset := TargetRecordScrollOffset;
      ADelta := Min(-ViewInfo.RecordsViewInfo.GetPixelScrollContentSize + 1, 0);
      AMaxRecordIndex := LastScrollRecordIndex;
      AMaxRecordOffset := 0;
      ViewInfo.RecordsViewInfo.CalculatePixelScrollInfo(Result, AOffset,
        AMaxRecordIndex, AMaxRecordOffset, ADelta, AOverPan);
      Result := GetLastIndexInScrollBand(Result);
    end
    else
      Result := LastVisibleRecordIndex;
  end;
...
begin
  ...
  try
    MakeFocusedRecordVisible;
    if FocusedRecordIndex = GetLastVisibleRecordIndex then
      ShowNextPage;
    FocusRecord(GetLastVisibleRecordIndex, ASyncSelection);
...
procedure TcxCustomGridTableController.FocusPrevPage(ASyncSelection: Boolean);
  ...
  function InternalGetTopRecordIndex: Integer;
  begin
    if IsRecordScrollAnimationActive then
      Result := TargetRecordScrollIndex
    else
  ...
  function InternalIsFirstPageRecordFocused: Boolean;
  begin
    if IsRecordScrollAnimationActive then
      Result := FocusedRecordIndex = TargetRecordScrollIndex  // IsFirstPageRecordFocused overrided !!!
    else
      Result := IsFirstPageRecordFocused;
  end;
...
begin
  ...
  try
    MakeFocusedRecordVisible;
    if InternalIsFirstPageRecordFocused then
      ShowPrevPage;
    FocusRecord(InternalGetTopRecordIndex, ASyncSelection);
    if not MultiSelect then
      Site.Update;
  finally
    EditingController.ShowEditAfterScrollContent;
...
begin
  ...
  try
    ...
    begin
      if IsRecordScrollAnimationActive then
      begin
        ARecordIndex := TargetRecordScrollIndex;
        ARecordOffset := TargetRecordScrollOffset;
      end
      else
      begin
        ARecordIndex := InternalTopRecordIndex;
      ...
      if AForward then
        ...
        if not IsRecordPixelScrolling or (ARecordOffset = 0) then
          Dec(ARecordIndex, ACount);
      if IsRecordPixelScrolling then
      begin
        ARecordOffset := 0;
        CheckTopRecordIndexAndOffset(ARecordIndex, ARecordOffset);
        SetTopRecordIndexWithOffset(ARecordIndex, ARecordOffset, IsScrollAnimationEnabled);
      end
      else
...
begin
  ...
  begin
    if IsRecordScrollAnimationActive then
      ATopRecordIndex := TargetRecordScrollIndex + GetPageVisibleRecordCount(TargetRecordScrollIndex, TargetRecordScrollOffset, True)
    else
...
begin
  ...
  begin
    ...
    ATopRecordOffset := TargetRecordScrollOffset;
    Dec(ATopRecordIndex, GetPageVisibleRecordCount(ATopRecordIndex, ATopRecordOffset, False));
  end
  else
  begin
    ATopRecordIndex := InternalTopRecordIndex;
    ATopRecordOffset := PixelScrollRecordOffset;
    Dec(ATopRecordIndex, GetPageRecordCount);
  end;
  CheckTopRecordIndexAndOffset(ATopRecordIndex, ATopRecordOffset);
  SetTopRecordIndexWithOffset(ATopRecordIndex, 0, IsScrollAnimationEnabled and IsRecordPixelScrolling);
end;
...
begin
  ...
  FPullFocusingScrollingTimer.Interval := PullFocusingScrollingTimeInterval;
  FPullFocusingScrollingTimer.OnTimer := PullFocusingScrollingTimerHandler;
end;
...
begin
  ...
  FDragScrollTimer.Interval := GetDragScrollInterval;
...
begin
  Result := inherited HasFocusedControls or
    ...
    IsFindPanelVisible and FindPanel.IsEditFocused;
end;
...
function TcxCustomGridTableController.IsRecordScrollable(ARecordIndex: Integer): Boolean;
begin
  Result := (ARecordIndex >= FirstScrollRecordIndex) and (ARecordIndex <= LastScrollRecordIndex);
end;
...
begin
  ...
  case Key of
    ...
    VK_F3:
      if (DataController.FindCriteria.Behavior = fcbSearch) and DataController.FindCriteria.IsActive then
      begin
        AllowRecordScrollAnimationOnMakeVisible(
        procedure
        begin
          if Shift = [ssShift] then
            DataController.FindCriteria.GoToPreviousMatch
          else
            DataController.FindCriteria.GoToNextMatch;
        end);
        Key := 0;
      end;
    Ord('A'):
...
begin
  if IsFindPanelVisible then
    FindPanel.FocusedItem := fpikNone;
...
var
  ...
  var
    I: Integer;
  begin
    if ViewInfo.LookAndFeelPainter.SupportsNativeFocusRect then
  ...
  begin
    ...
    ACanvas.DrawText(FDragDropText, R, 0);
  ...
  try
    try
      ...
      begin
        ...
        var AEdit := EditingController.Edit;  //..

        If   Assigned( AEdit )
        then AModifiedAfterEnter := AEdit.ModifiedAfterEnter
        else AModifiedAfterEnter := False;

        EditingController.UpdateValue;

        If   AModifiedAfterEnter and Assigned( AEdit )
        then begin
             // Cuando se sale directamente, por ejemplo con cursor abajo, no se producía la notificación del PostEditValue
             AEdit.ModifiedAfterEnter := AModifiedAfterEnter;
             AEdit.DoPostEditValueNotification; //..
             end;
...
begin
  try
    ...
    FEditingController.FCreatingNewRecord := True;  //..

     GridView.Focused := True;

       try
         FEditingController.FInsertingRecord := True;  //..
         If   AtEnd
         then begin
              If   CanAppend( False )
              then DataController.Append;
              end
         else If   CanInsert( False )
              then begin
                   DataController.Insert;
                   If   MultiSelect and ( ViewData.EditingRecord<>nil )
                   then ViewData.EditingRecord.Selected := True;
                   end;

       finally
         FEditingController.FInsertingRecord := False;
         end;

     If   (dceInsert in DataController.EditState) and
          GridView.OptionsBehavior.FocusFirstCellOnNewRecord
     then begin
          FocusFirstAvailableItem;
          FEditingController.ShowEdit;  //..
		        end;
     finally
         FEditingController.FCreatingNewRecord := False;
         end;
end;
//..
...
begin
  ...
  if not GridView.OptionsData.DeletingConfirmation or
    (dxMessageDlg(GetConfirmationText, mtConfirmation, [mbOK, mbCancel], 0) = mrOk) then
  begin
    if IsEditing then 
...
//..
procedure TcxCustomGridTableController.DoKeyAutoRepeat;
begin
  StopScrollAnimation;
end;
...
begin
     if   Assigned(GridView.FOnFocusFirstAvailableItem )  //..
     then Result := GridView.FOnFocusFirstAvailableItem(Self)
     else Result := FocusNextItem(-1, True, False, False, not IsMultiSelectPersistent);
     FFirstAvailableItemIndex := FocusedItemIndex;
     FFirstAvailableItem := FocusedItem;
end;
//..
...
begin
  if FocusedRecordHasCells(True) then
    Result := FocusNextItem(ActuallyFocusedItemIndex, AGoForward, False,
...
begin
  ...
  Result := CanFocusNextItem(AFocusedItemIndex, ANextItemIndex, AGoForward,
    AGoOnCycle, AGoToNextRecordOnCycle);
     If   Result
     then begin
          If   ACycleChanged and AGoToNextRecordOnCycle
          then begin
               FEditingController.HideEdit( GridView.DataController.IsEditing ); //..
               If   FocusedRecord.IsNewItemRecord
               then begin
                    If   CanDataPost 
					               then DataController.Post
                    else DataController.Cancel;
                    If   ViewData.NewItemRecord <> nil
                    then ViewData.NewItemRecord.Focused := True;
					               Result := True;
                    end
               else begin
                    Result := FocusNextRecord(FocusedRecordIndex, AGoForward, False, False, False);
                    Exit;    //..  Para evitar que se fije el focus en ANextItemIndex
                    end;
               end
          else Result := True;
          If   Result
          then GridView.VisibleItems[ANextItemIndex].Focused := True;
		  If   ANeedNormalizeSelection 
		  then DoNormalSelection;
  end;
end;
...
begin
  ...
  AAnimated := FAllowRecordScrollAnimationOnMakeVisible and IsScrollAnimationEnabled and
    not EditingController.EditingItemSetting;
  if not IsRecordFullyVisibleAtTop(AIndex) then
  begin
    AWasFullyVisible := False;
    if IsRecordPartVisibleAtTop(AIndex) then
      SetTopRecordIndexWithOffset(InternalTopRecordIndex, 0, AAnimated)
    else
      SetTopRecordIndexWithOffset(AIndex, 0, AAnimated);
  end;
        ...
        begin
          ...
          begin
            ...
            if AIndex = ATopIndex then
              ATopOffset := 0;
            SetTopRecordIndexWithOffset(ATopIndex, ATopOffset, AAnimated);
            AWasFullyVisible := False;
          end
          else
  ...
  if IsRecordScrollAnimationActive and AWasFullyVisible then
    FRecordScrollAnimationHelper.StopAnimation(True);
end;
