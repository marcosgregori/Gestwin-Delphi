...
uses
  ...
  System.Classes, Vcl.Controls, Vcl.Graphics, Vcl.ImgList, Vcl.Forms, Vcl.StdCtrls,
...
type
  ...
  public
    constructor Create(AGridView: TcxCustomGridView; AUpdateGridViewOnly: Boolean = False; AKeepMaster: Boolean = False); reintroduce; virtual;
    procedure Execute; override;
    function IsCompatibleWith(AChange: TcxCustomGridChange): Boolean; override;
    property KeepMaster: Boolean read FKeepMaster;
    property UpdateGridViewOnly: Boolean read FUpdateGridViewOnly write FUpdateGridViewOnly;
  end;
  ...
  TcxGridViewChangedEvent = procedure(AView: TcxCustomGridView; AChangeKind: TcxGridViewChangeNotificationKind ) of object;  //..
  ...
  private
    ...
    FViews: TList;
    FOnGridViewChanged : TcxGridViewChangedEvent; //..
    FOnStructureNavigatorChanged : TNotifyEvent;  //..
    ...
    procedure DestroyChanges(AChanges: TdxFastObjectList);
  ...
  protected
    ...
    function CanUseLockedStateImage: Boolean;
    ...
    procedure DoUpdate(AChanges: TdxFastObjectList);
  ...
  public
    ...
    property OnStructureNavigatorChanged : TNotifyEvent read FOnStructureNavigatorChanged write FOnStructureNavigatorChanged; //..
    property OnGridViewChanged : TcxGridViewChangedEvent read FOnGridViewChanged write FOnGridViewChanged; //..
...
uses
  System.RTLConsts, System.SysUtils, System.Math, cxGridCustomTableView, dxOffice11, cxGeometry, cxGridStrs,
  dxUIAGridProvider;

const
  dxThisUnitName = 'cxGrid';
...
function TcxGridSizeChange.IsEqualCore(AChange: TcxCustomGridChange): Boolean;
begin
...
function TcxGridViewChange.IsEqualCore(AChange: TcxCustomGridChange): Boolean;
begin
  Result := inherited IsEqualCore(AChange) and
...
begin
  ...
  FViews := TList.Create;
  FChanges := TcxGridChanges.Create(False);
  FTabStop := True;
...
procedure TcxCustomGrid.SetAutomation(const AValue: TdxAutomationElementSettings);
begin
...
procedure TcxCustomGrid.DestroyChanges(AChanges: TdxFastObjectList);
begin
  AChanges.OwnsObjects := True;
  AChanges.Clear;
  AChanges.OwnsObjects := False;
end;
...
begin
  for I := FChanges.Count - 1 downto 0 do
    if (TcxCustomGridChange(FChanges.List[I]) is TcxCustomGridViewChange) and
      (TcxCustomGridViewChange(FChanges.List[I]).GridView = AView) then
    begin
      FChanges[I].Free;
      FChanges.Delete(I);
    end;
...
function TcxCustomGrid.CreateUIAProvider: IdxUIAProvider;
begin
  Result := dxUIACreateCustomGridUIAProvider(Self);
end;
...
procedure TcxCustomGrid.CMRecreateWnd(var Message: TMessage);
var
  AWasFocused: Boolean;
begin
  AWasFocused := IsFocused;
  inherited;
  if AWasFocused and (WindowHandle <> 0) then
...
function TcxCustomGrid.GetAutomationSettings: TdxAutomationElementSettings;
begin
  Result := FAutomation;
end;
...
begin
  ...
  if ActiveView <> nil then
    TcxCustomGridViewAccess.LookAndFeelChanged(ActiveView);
  SizeChanged;
end;
...
function TcxCustomGrid.CanUseLockedStateImage: Boolean;
begin
  Result := LockedStatePaintHelper.CanControlPaint and
    LockedStatePaintHelper.CanCreateLockedImage;
end;
...
procedure TcxCustomGrid.DoUpdate(AChanges: TdxFastObjectList);
var
  AFinalChanges: TdxFastObjectList;

  procedure ValidateChanges(AList: TdxFastObjectList);

    procedure DeleteChange(AIndex: Integer);
    begin
       TObject(AList.List[AIndex]).Free;
       AList.Delete(AIndex);
    end;
  ...
  begin
    ...
    begin
      AChange := TcxCustomGridChange(AList.List[I]);
      for J := I + 1 to AList.Count - 1 do
        if AChange.IsCompatibleWith(TcxCustomGridChange(AList.List[J])) then
        begin
          DeleteChange(I);
          Dec(I);
          Break;
  ...
  procedure BuildFinalChangesList(AChanges: TdxFastObjectList);
  begin
    ValidateChanges(AChanges);
    if AChanges.Count > 0 then
    try
      AFinalChanges.InsertRange(0, AChanges);
    finally
      AChanges.Clear;
    end;
  end;
  ...
  var
    ALockChanges, AUnlockChanges: Boolean;
    AChange: TcxCustomGridChange;
  begin
    if AFinalChanges.Count = 0 then
      Exit;
    ALockChanges := AFinalChanges.Count > 1;
    if ALockChanges then
      BeginUpdate;
    try
      while AFinalChanges.Count > 0 do
      begin
        AChange := TcxCustomGridChange(AFinalChanges.List[0]);
        AUnlockChanges := ALockChanges and not AChange.CanExecuteWhenLocked;
        if AUnlockChanges then
        begin
          if FChanges.Count > 0 then
            BuildFinalChangesList(FChanges);
          Dec(FUpdateLockCount);
        end;
        AChange := TcxCustomGridChange(AFinalChanges.List[0]);
        AFinalChanges.Delete(0);
        try
          DoChange(AChange);
        finally
          if AUnlockChanges then
...
begin
  if AChanges.Count = 0 then
    Exit;
  Inc(FUpdateCount);
  AFinalChanges := TdxFastObjectList.Create(False);
  try
    BuildFinalChangesList(AChanges);
    DoFinalChanges;
  finally
    Dec(FUpdateCount);
    AFinalChanges.Free;
...
begin
  ...
  then If   Assigned( FOnStructureNavigatorChanged )  //..
       then FOnStructureNavigatorChanged( Self );
end;
...
begin
  StructureNavigatorChanged;
  If   Assigned( FOnGridViewChanged )  //..
  then FOnGridViewChanged( AView, AChangeKind );
end;
...
procedure TcxCustomGrid.ScaleFactorChanged;
var
  I: Integer;
begin
  try
    ...
    for I := 0 to ViewCount - 1 do
      TcxCustomGridViewAccess.FinishScaleFactorChange(Views[I]);
  finally
    EndUpdate;
  end;
end;

procedure TcxCustomGrid.ScaleFactorChanging;
begin
  BeginUpdate;
...
function TcxGrid.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;
