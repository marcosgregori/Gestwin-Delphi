...
uses
  System.Types, Winapi.Windows, Winapi.Messages, System.Classes, Vcl.Graphics,
  Vcl.Forms, Vcl.Controls {after Vcl.Forms for D12},
  Vcl.StdCtrls, Vcl.Menus, Vcl.ImgList, dxCore, dxTypeHelpers, dxXMLDoc,
  ...
  dxIncrementalFiltering, dxUIElementPopupWindow, cxGeometry, dxForms, dxScrollbarAnnotations,
  dxUIAClasses, dxUIABase;
...
type
  TcxGridStorageOption = (gsoUseFilter, gsoUseSummary, gsoUseDataViewState,
    gsoExpanded, gsoFocusedRecord, gsoFocusedItem, gsoSelected,
    gsoTopRecord, gsoFocusedView, gsoDetail);

  TcxGridStorageOptions = set of TcxGridStorageOption;
  TcxGridViewChangeNotificationKind = (vcnName{, vsRemoved});

const
  cxGridStoreAllDataViewStates = [gsoUseDataViewState,
    gsoExpanded, gsoFocusedRecord, gsoFocusedItem, gsoSelected,
    gsoTopRecord, gsoFocusedView, gsoDetail];
...
type
  ...
  protected
    procedure UpdateItemHeight; virtual; abstract;
  ...
  protected
    ...
    function SupportsIDETitle: Boolean; override;
  ...
  protected
    ...
    function NeedPlaceHintBelowCursor: Boolean; override;
  ...
  TcxCustomGridCustomizationController = class(TcxGridViewHandler)
  strict private
    FCustomization: Boolean;
    FForm: TForm;
    FFormBounds: TRect;

    function GetFormIntf: IcxGridCustomizationForm;
    procedure SetCustomization(Value: Boolean);
  protected
    function CanCustomize: Boolean; virtual;
    procedure CheckFormBounds(var R: TRect); virtual;
    ...
    function GetFormBounds(AScaleFactor: TdxScaleFactor): TRect; virtual;
    function GetFormDefaultHeight: Integer; virtual;
    function GetFormDefaultWidth: Integer; virtual;
    procedure HideForm;
    procedure InitializeForm(AForm: TForm); virtual;
    procedure ShowForm;

    property Form: TForm read FForm;
    property FormBounds: TRect read FFormBounds write FFormBounds;
    property FormIntf: IcxGridCustomizationForm read GetFormIntf;
  public
    property Customization: Boolean read FCustomization write SetCustomization;
  end;

  TcxGridCustomizationController = class(TcxCustomGridCustomizationController)
  protected
  ...
  private
    FCustomizationController: TcxCustomGridCustomizationController;
    ...
    FIsDblClick: Boolean;
    FIsFocusing: Boolean;
    FIsNavigationKeyAutoRepeat: Boolean;
    FIsScrolling: Boolean;

    function GetCustomization: Boolean;
    function GetCustomizationForm: TForm;
    ...
    procedure SetCustomization(Value: Boolean);
    procedure SetCustomizationFormBounds(const AValue: TRect);
    ...
    procedure CheckKeyAutoRepeat(ACharCode: Word; ARepeatCount: Cardinal);
    function IsNavigationKey(ACharCode: Word): Boolean;
    procedure ResetKeyAutoRepeat(ACharCode: Word);
  protected
    procedure AfterPaint; virtual;
    ...
    procedure DoKeyAutoRepeat; virtual;
    ...
    function GetMouseWheelMajorScrollBarKind: TScrollBarKind; virtual;
    ...
    // customization
    function CreateCustomizationController: TcxCustomGridCustomizationController; virtual;
    ...
    property CustomizationController: TcxCustomGridCustomizationController read FCustomizationController;
    ...
    property IsNavigationKeyAutoRepeat: Boolean read FIsNavigationKeyAutoRepeat;
  ...
  public
    ...
    property CustomizationForm: TForm read GetCustomizationForm;
    property CustomizationFormBounds: TRect read GetCustomizationFormBounds write SetCustomizationFormBounds;
  ...
  protected
    ...
    function DrawBackgroundBeforePaint: Boolean; virtual;
  ...
  private
    ...
    function AllowBoundsChangedNotification: Boolean;
  ...
  protected
    ...
    function NeedToCheckIdenticalScrollBarVisibilityCalculationAttempts: Boolean; override;
  ...
  private
    ...
    FIsStateResetting: Boolean;
    FLinkedHitTest: TcxCustomGridHitTest;
    FParamsCalculated: Boolean;
    FScaleFactor: TdxScaleFactor;
    FState: TcxGridCellState;
    FText: string;
    FTextSelBkColor: TColor;
    FTextSelColor: TColor;
    FTextSelLength: Integer;
    FTextSelStart: Integer;
    FVisible: Boolean;
  ...
  protected
    ...
    function CalculateTextWidth: Integer;
    function CalculateTextHeight(AForPainting: Boolean): Integer;
    procedure CalculateTextSelectionInfo; virtual;
    ...
    function GetActualScaleFactor: TdxScaleFactor; virtual;
    ...
    function GetBordersScaleFactor: TdxScaleFactor; virtual;
    ...
    class function GetCellHeight(ATextHeight: Integer; ALookAndFeelPainter: TcxCustomLookAndFeelPainter;
      AScaleFactor, ABordersScaleFactor: TdxScaleFactor): Integer; virtual;
    ...
    procedure HotTrackChanged(AHotTracked: Boolean); virtual;
    ...
    function IsChildrenHotTracked: Boolean; virtual;
    ...
    function RecalculateOnStateChange: Boolean; virtual;
    procedure ResetChildrenState; virtual;
    ...
    procedure UpdateOnStateChange; virtual;
    function GetUseLightBorders: Boolean; virtual;
    ...
    property TextSelBkColor: TColor read FTextSelBkColor write FTextSelBkColor;
    property TextSelColor: TColor read FTextSelColor write FTextSelColor;
    property TextSelStart: Integer read FTextSelStart write FTextSelStart;
    property TextSelLength: Integer read FTextSelLength write FTextSelLength;
  public
    Bounds: TRect;
    MultiLine: Boolean;
    ...
    procedure Update;
    function UseRightToLeftAlignment: Boolean;
    function IsForcedVisibleForPainting: Boolean; virtual;
    ...
    property BordersScaleFactor: TdxScaleFactor read GetBordersScaleFactor; // for internal use
  ...
  TcxCustomGridViewCellViewInfo = class(TcxCustomGridCellViewInfo, IcxHintableObject)
  private
    ...
    function GetLookAndFeelPainter: TcxCustomLookAndFeelPainter;
  protected
    function GetUseLightBorders: Boolean; override;
    // IdxAdornerTargetElement
    ...
    function GetActualScaleFactor: TdxScaleFactor; override;
    function GetCanvas: TcxCanvas; override;
    function GetControl: TcxControl; override;
    function GetBordersScaleFactor: TdxScaleFactor; override;
  ...
  protected
    ...
    procedure RefreshCache; virtual;
    //
    property LookAndFeelPainter: TcxCustomLookAndFeelPainter read FLookAndFeelPainter;
  public
    procedure AfterConstruction; override;
    //
    property GridView: TcxCustomGridView read FGridView;
  ...
  TCustomGridViewStorage = class; // for internal use only
  TCustomGridViewStorageClass = class of TCustomGridViewStorage;
  ...
  TcxCustomGridView = class(TcxControlChildComponent,
    ...
    IdxAdornerTargetElement,
    IdxAutomationElement)
  private class var FResourcesInitialized: Boolean;
  private
    ...
    function GetIsDetail: Boolean; inline;
    ...
    function GetLookAndFeelPainter: TcxCustomLookAndFeelPainter;
    function GetMasterDataController: TcxCustomDataController; inline;
    function GetMasterGridRecordIndex: Integer;
    function GetMasterGridView: TcxCustomGridView; inline;
  ...
  protected
    ...
    function GetDataViewStateStorageClass: TCustomGridViewStorageClass; virtual;
    function CreateDataViewStateStorage: TCustomGridViewStorage; virtual;
    // IcxStoredParent
    ...
    function CheckVisibility(APotentional: Boolean): Boolean; virtual;
    ...
    procedure FinishScaleFactorChange; virtual;
    ...
    function UseOptimalHeightWhenDetail: Boolean; virtual;
  ...
  public
    ...
    function UIAProvider: IdxUIAProvider; overload; // for internal use
    ...
    property MasterDataController: TcxCustomDataController read GetMasterDataController;
    ...
    property OwnerControl: TcxControl read GetOwnerControl; // for internal use
    ...
    property UseLightBorders: Boolean read GetUseLightBorders; // for internal use
  ...
  public
    ...
    class procedure FinishScaleFactorChange(AInstance: TcxCustomGridView); static;
  ...
  { TCustomGridViewStorage }

  TCustomGridViewStorage = class
  private
    FOptions: TcxGridStorageOptions;
    FGridView: TcxCustomGridView;
  protected
    procedure SaveStateToNode(AParentNode: TdxXMLNode); virtual;
    procedure RestoreStateFromNode(AParentNode: TdxXMLNode); virtual;
  public
    procedure SaveToStream(AView: TcxCustomGridView; AStream: TStream); virtual;
    procedure RestoreFromStream(AView: TcxCustomGridView; AStream: TStream); virtual;

    property Options: TcxGridStorageOptions read FOptions write FOptions;
    property GridView: TcxCustomGridView read FGridView write FGridView;
  end;
...
function cxGridRegisteredViews: TcxRegisteredClasses;
...
uses
  System.UITypes,
  System.RTLConsts, 
  System.SysUtils, System.Math, cxScrollBar, cxVariants, cxLibraryConsts,
  cxGrid, cxGridLevel, cxGridStrs, cxDrawTextUtils, cxEditUtils, dxDPIAwareUtils, dxCoreGraphics, dxMessageDialog,
  dxUIAGridProvider;

const
  dxThisUnitName = 'cxGridCustomView';
...
function cxGridRegisteredViews: TcxRegisteredClasses;
begin
  Result := cxInternalGridRegisteredViews;
end;
...
function TcxCustomGridViewChange.IsEqualCore(AChange: TcxCustomGridChange): Boolean;
begin
  Result := FGridView = TcxCustomGridViewChange(AChange).GridView;
end;
...
begin
  ...
  if (odFocused in State) and LookAndFeelPainter.SupportsNativeFocusRect then
...
procedure TcxCustomGridItemsInnerListBox.LookAndFeelChanged(Sender: TcxLookAndFeel; AChangedValues: TcxLookAndFeelValues);
begin
  CalculateBorderStyle;
  CalculateItemHeight;
  inherited;
...
begin
  ...
  HandleNeeded;
  ScaleForCurrentDPI;
  Controller.CustomizationController.InitializeForm(Self);
  CalculateConsts;
  CreateControls;
end;
...
begin
  FOffset := TdxTextMeasurer.TextLineHeight(Canvas.Handle) div 6;
end;
...
function TcxCustomGridCustomizationForm.SupportsIDETitle: Boolean;
begin
  Result := False; 
end;
...
procedure TcxGridPopupListBox.InternalMouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited InternalMouseDown(Button, Shift, X, Y);
  if (ItemIndex >= 0) and ItemRect(ItemIndex).Contains(X, Y) then
    Click;
end;
...
function TcxGridHintHelper.NeedPlaceHintBelowCursor: Boolean;
begin
  Result := False;
end;
...
{ TcxCustomGridCustomizationHelper }

function TcxCustomGridCustomizationController.CanCustomize: Boolean;
begin
  Result := not (GridView.IsPattern or not Site.HandleAllocated);
end;

procedure TcxCustomGridCustomizationController.CheckFormBounds(var R: TRect);
begin
end;

procedure TcxCustomGridCustomizationController.CustomizationChanged;
begin
...
procedure TcxCustomGridCustomizationController.DoCreateForm;
begin
  if FForm = nil then
    FForm := CreateForm;
end;

procedure TcxCustomGridCustomizationController.DoCustomization;
begin
  if Control <> nil then
...
function TcxCustomGridCustomizationController.GetFormBounds(AScaleFactor: TdxScaleFactor): TRect;
begin
  if IsRectEmpty(FFormBounds) then
  begin
    Result.BottomRight := Site.ClientToScreen(Site.ClientRect.BottomRight);
    Result.Left := Result.Right - AScaleFactor.Apply(GetFormDefaultWidth);
    Result.Top := Result.Bottom - AScaleFactor.Apply(GetFormDefaultHeight);
...
function TcxCustomGridCustomizationController.GetFormDefaultHeight: Integer;
begin
  Result := cxGridCustomizationFormDefaultHeight;
end;

function TcxCustomGridCustomizationController.GetFormDefaultWidth: Integer;
begin
  Result := cxGridCustomizationFormDefaultWidth;
end;

procedure TcxCustomGridCustomizationController.HideForm;
begin
...
procedure TcxCustomGridCustomizationController.InitializeForm(AForm: TForm);
begin
  AForm.Caption := cxGetResourceString(@scxGridCustomizationFormCaption);
  AForm.BoundsRect := GetFormBounds(dxGetScaleFactor(AForm));
  if Control <> nil then 
  begin
    dxAssignFont(AForm.Font, TcxControlAccess(Control).Font, dxGetScaleFactor(AForm), dxGetScaleFactor(Control));
    AForm.BiDiMode := Control.BiDiMode;
  end;
end;

procedure TcxCustomGridCustomizationController.ShowForm;
begin
  DoCreateForm;
  FForm.Show;
end;

function TcxCustomGridCustomizationController.GetFormIntf: IcxGridCustomizationForm;
begin
  if Form = nil then
    Result := nil
  else
    Supports(Form, IcxGridCustomizationForm, Result);
end;

procedure TcxCustomGridCustomizationController.SetCustomization(Value: Boolean);
begin
  if Value and not CanCustomize then
    Exit;
...
function TcxGridCustomizationController.CreateForm: TForm;
begin
  Result := Controller.GetCustomizationFormClass.Create(Controller);
end;
...
function TcxCustomGridController.GetCustomization: Boolean;
begin
  Result := CustomizationController.Customization;
end;

function TcxCustomGridController.GetCustomizationForm: TForm;
begin
  Result := CustomizationController.Form;
end;

function TcxCustomGridController.GetCustomizationFormBounds: TRect;
begin
  Result := CustomizationController.FormBounds;
end;
...
begin
  CustomizationController.Customization := Value;
end;

procedure TcxCustomGridController.SetCustomizationFormBounds(const AValue: TRect);
begin
  CustomizationController.FormBounds := AValue;
end;
...
procedure TcxCustomGridController.CheckKeyAutoRepeat(ACharCode: Word; ARepeatCount: Cardinal);
begin
  if IsNavigationKey(ACharCode) and (ARepeatCount > 1) then
  begin
    FIsNavigationKeyAutoRepeat := True;
    DoKeyAutoRepeat;
  end;
end;

function TcxCustomGridController.IsNavigationKey(ACharCode: Word): Boolean;
begin
  Result := ACharCode in [VK_NEXT, VK_PRIOR, VK_UP, VK_DOWN, VK_LEFT, VK_RIGHT];
end;

procedure TcxCustomGridController.ResetKeyAutoRepeat(ACharCode: Word);
begin
  if IsNavigationKey(ACharCode) then
    FIsNavigationKeyAutoRepeat := False;
end;
...
procedure TcxCustomGridController.DoKeyAutoRepeat;
begin
end;
...
function TcxCustomGridController.GetMouseWheelMajorScrollBarKind: TScrollBarKind;
begin
  Result := sbVertical;
end;
...
begin
  if CustomizationController.FormIntf <> nil then
    CustomizationController.FormIntf.GridViewChanged;
end;
...
function TcxCustomGridController.CreateCustomizationController: TcxCustomGridCustomizationController;
begin
  Result := TcxGridCustomizationController.Create(GridView);
end;
...
begin
  if CustomizationController.FormIntf <> nil then
    CustomizationController.FormIntf.RefreshData;
end;
...
begin
  if CanDrawDesignSelection then
    DoDrawDesignSelection(Canvas, ViewInfo);
end;
...
begin
  if not ViewInfo.Transparent then
    Canvas.FillRect(R, ViewInfo.Params.Color)
  else
    if ViewInfo.BackgroundBitmap <> nil then
      Canvas.FillRect(R, ViewInfo.BackgroundBitmap);
end;
...
begin
  Canvas.SetBrushColor(ViewInfo.BorderColor[ABorder]);
  Canvas.FillRect(ViewInfo.BorderBounds[ABorder]);
end;
...
var
  ARect: TRect;
  ADrawTextFlags, ATextOutFlags: Integer;
begin
  if ViewInfo.TextForPainting = '' then
    Exit;
  ARect := ViewInfo.TextAreaBounds;  // can call font change
  ViewInfo.PrepareCanvas(Canvas);
  Canvas.Brush.Style := bsClear;
  PrepareCanvasForDrawText;
  ADrawTextFlags := ViewInfo.GetTextAttributes(True);
  if ViewInfo.GetDrawTextRotationAngle = ra0 then
  begin
    ATextOutFlags := DrawTextFlagsTocxTextOutFlags(ADrawTextFlags);
    cxTextOut(Canvas.Canvas, ViewInfo.TextForPainting, ARect, ATextOutFlags, ViewInfo.TextSelStart, ViewInfo.TextSelLength,
      nil, ViewInfo.TextSelBkColor, ViewInfo.TextSelColor)
  end
  else
    Canvas.DrawText(ViewInfo.TextForPainting, ARect, ADrawTextFlags, True, ViewInfo.GetDrawTextRotationAngle);
...
begin
  if not ViewInfo.UseLightBorders then
    ...
    Result := ViewInfo.Calculated and (ViewInfo.IsForcedVisibleForPainting or Canvas.RectVisible(ViewInfo.GetAreaBoundsForPainting));
end;
...
procedure TcxCustomGridPainter.DrawBackground;
  ...
  begin
    FillRgn(Canvas.Handle, FBackgroundRegion.Handle, TdxSolidBrushCache.Get(ViewInfo.BackgroundColor));
  end;
  ...
  begin
    ...
    try
      Canvas.SetClipRegion(FBackgroundRegion, roIntersect, False);
      Canvas.FillRect(ViewInfo.Bounds, ViewInfo.BackgroundBitmap);
    finally
...
begin
  if ViewInfo.BackgroundBitmap <> nil then
    DrawBackgroundUsingBitmap
  else
    DrawBackgroundUsingBrush;
end;

function TcxCustomGridPainter.DrawBackgroundBeforePaint: Boolean;
begin
  Result := False;
end;
...
begin
  if not DrawBackgroundBeforePaint then
    DrawBackground;
...
begin
  ...
  if DrawBackgroundBeforePaint then
    DrawBackground;
...
function TcxGridSite.AllowBoundsChangedNotification: Boolean;
var
...
function TcxGridSite.CreateUIAProvider: IdxUIAProvider;
begin
  Result := dxUIACreateGridSiteProvider(Self);
end;
...
begin
  ...
  if ((APrevSize.cx <> 0) or (APrevSize.cy <> 0)) and
    ...
    if AllowBoundsChangedNotification then
...
function TcxGridSite.GetScrollbarAnnotationHelper: TdxScrollbarAnnotations;
begin
  Result := GridView.GetScrollbarAnnotationHelper;
end;
...
function TcxGridSite.IsDataScrollbar(AKind: TScrollBarKind): Boolean;
begin
  Result := GridView.IsDataScrollbar(AKind);
end;
...
function TcxGridSite.GetAutomationSettings: TdxAutomationElementSettings;
begin
  Result := nil;
end;
...
const
  MinorScrollBar: array[TScrollBarKind] of TScrollBarKind = (sbVertical, sbHorizontal);
  ScrollingKind: array[TScrollBarKind] of TcxMouseWheelScrollingKind = (mwskHorizontal, mwskVertical);
var
  AMajorScrollBar, AMinorScrollBar: TScrollBarKind;
begin
  AMajorScrollBar := Controller.GetMouseWheelMajorScrollBarKind;
  if IsScrollBarActive(AMajorScrollBar) then
    Result := ScrollingKind[AMajorScrollBar]
  else
  begin
    AMinorScrollBar := MinorScrollBar[AMajorScrollBar];
    if IsScrollBarActive(AMinorScrollBar) then
      Result := ScrollingKind[AMinorScrollBar]
    else
      Result := mwskNone;
  end;
end;
...
begin
  ...
  begin
    with ANewBounds do
      Winapi.Windows.MoveWindow(Handle, Left, Top, Right - Left, Bottom - Top,
        ANewBounds.Left <> cxInvisibleCoordinate);
  end;
end;

function TcxGridSite.NeedToCheckIdenticalScrollBarVisibilityCalculationAttempts: Boolean;
begin
  Result := True;
end;
...
function TcxGridSite.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;
...
begin
  if FBoundsChangedTimer <> nil then
    Exit;
...
begin
  if not ViewInfo.IsCalculating then
...
procedure TcxGridSite.WMKeyDown(var Message: TWMKeyDown);
begin
  Controller.CheckKeyAutoRepeat(Message.CharCode, Word(Message.KeyData));
  inherited;
end;

procedure TcxGridSite.WMKeyUp(var Message: TWMKeyUp);
begin
  Controller.ResetKeyAutoRepeat(Message.CharCode);
  inherited;
end;
...
begin
  inherited Create(nil);
  FVisible := True;
  FScaleFactor := GetActualScaleFactor;
end;

function TcxCustomGridCellViewInfo.CreateUIAProvider: IdxUIAProvider;
begin
  Result := nil;
end;
...
begin
  Result := TextWidth;
  GetCellTextAreaSize(Result, ScaleFactor, True);
end;
...
begin
  if not MouseCapture and ((FState = gcsSelected) or not IsCheck and (FState = gcsPressed)) then
    DoMouseLeave;
end;

procedure TcxCustomGridCellViewInfo.DoMouseLeave;
begin
  State := gcsNone;
end;

function TcxCustomGridCellViewInfo.GetActualScaleFactor: TdxScaleFactor;
begin
...
function TcxCustomGridCellViewInfo.GetBordersScaleFactor: TdxScaleFactor;
begin
...
procedure TcxCustomGridCellViewInfo.AfterOffset;
begin
  State := gcsNone;
  MouseLeave;
end;
...
procedure TcxCustomGridCellViewInfo.BeforeOffset;
begin
//do nothing
end;
...
function TcxCustomGridCellViewInfo.CalculateTextWidth: Integer;
var
  R: TRect;
  ADrawTextFlags, ATextOutFlags: Integer;
begin
  ...
  begin
    ...
    ADrawTextFlags := GetTextAttributes(False) and not (cxAlignRight or cxAlignCenter) or cxAlignLeft;
    ATextOutFlags := DrawTextFlagsTocxTextOutFlags(ADrawTextFlags) or CXTO_CALCRECT;
    cxTextOut(Canvas.Handle, Text, R, ATextOutFlags);
...
function TcxCustomGridCellViewInfo.CalculateTextHeight(AForPainting: Boolean): Integer;
var
  R: TRect;
...
begin
  ...
  begin
    PrepareCanvas(Canvas);
    R := Rect(0, 0, cxMaxRectSize, cxMaxRectSize);
    ATextAreaBounds := TextAreaBounds;
    if ATextAreaBounds.Width > 0 then
      R.Width := ATextAreaBounds.Width;
    ADrawTextFlags := GetTextAttributes(AForPainting) and not (cxAlignBottom or cxAlignVCenter) or cxAlignTop;
    ATextOutFlags := DrawTextFlagsTocxTextOutFlags(ADrawTextFlags) or CXTO_CALCRECT;
    cxTextOut(Canvas.Handle, Text, R, ATextOutFlags);
...
procedure TcxCustomGridCellViewInfo.CalculateTextSelectionInfo;
begin
  FTextSelStart := -1;
  FTextSelLength := -1;
  FTextSelColor := clNone;
  FTextSelBkColor := clNone;
end;
...
begin
  ...
  case AIndex of
    bLeft:
      begin
        Result.Right := Result.Left;
        Dec(Result.Left, BorderWidth[AIndex]);
      end;
    bTop:
      begin
        Result.Bottom := Result.Top;
        Dec(Result.Top, BorderWidth[AIndex]);
      end;
    bRight:
      begin
        Result.Left := Result.Right;
        Inc(Result.Right, BorderWidth[AIndex]);
      end;
    bBottom:
      begin
        Result.Top := Result.Bottom;
        Inc(Result.Bottom, BorderWidth[AIndex]);
      end;
  end;
  if AIndex in [bLeft, bRight] then
  begin
    if bTop in Borders then
      Dec(Result.Top, BorderWidth[bTop]);
    if bBottom in Borders then
      Inc(Result.Bottom, BorderWidth[bBottom]);
  end;
end;
...
class function TcxCustomGridCellViewInfo.GetCellHeight(ATextHeight: Integer;
  ALookAndFeelPainter: TcxCustomLookAndFeelPainter; AScaleFactor, ABordersScaleFactor: TdxScaleFactor): Integer;
begin
  Result := ATextHeight;
  GetCellTextAreaSize(Result, AScaleFactor, False);
end;
...
begin
  ...
  Result := GetCellHeight(Result, ALookAndFeelPainter, ScaleFactor, BordersScaleFactor);
end;
...
function TcxCustomGridCellViewInfo.GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider;
begin
...
function TcxCustomGridCellViewInfo.GetUseLightBorders: Boolean;
begin
  Result := False;
end;
...
procedure TcxCustomGridCellViewInfo.HotTrackChanged(AHotTracked: Boolean);
begin
//do nothing
end;
...
function TcxCustomGridCellViewInfo.IsChildrenHotTracked: Boolean;
begin
  Result := False;
end;
...
begin
  Result := (State = gcsSelected) or ACheckChildren and IsChildrenHotTracked;
end;
...
function TcxCustomGridCellViewInfo.RecalculateOnStateChange: Boolean;
begin
  Result := False;
end;

procedure TcxCustomGridCellViewInfo.ResetChildrenState;
begin
//do nothing
end;

procedure TcxCustomGridCellViewInfo.ResetState;
begin
  FIsStateResetting := True;
  try
    ResetChildrenState;
    State := gcsNone;
  finally
    FIsStateResetting := False;
  end;
end;
...
begin
  ...
  if IsHotTrackChanged(APrevState) then
    HotTrackChanged(State = gcsSelected);
  case State of
    gcsNone:
...
function TcxCustomGridCellViewInfo.SupportsUIAutomation: Boolean;
begin
  Result := False;
end;

procedure TcxCustomGridCellViewInfo.UpdateOnStateChange;
begin
  if FIsStateResetting then
    Exit;
  if RecalculateOnStateChange then
    Recalculate;
...
function TcxCustomGridCellViewInfo.IsForcedVisibleForPainting: Boolean;
begin
  Result := False;
end;
...
begin
  inherited;
  if not CloseDropDownWindowOnDestruction and DropDownWindowExists and DropDownWindow.Visible and IsDropDownWindowOwner then
  begin
    DropDownWindow.Owner := Self;
    State := gcsPressed;
  end;
...
function TcxCustomGridCellViewInfo.GetAutomationSettings: TdxAutomationElementSettings;
begin
  Result := nil;
end;
...
function TcxCustomGridCellViewInfo.UIAProvider(AIsCreating: Boolean = False): IdxUIAProvider;
begin
  if AIsCreating then
...
function TcxCustomGridViewCellViewInfo.GetBordersScaleFactor: TdxScaleFactor;
begin
  Result := GridView.BordersScaleFactor;
end;

function TcxCustomGridViewCellViewInfo.GetUseLightBorders: Boolean;
begin
  Result := GridView.UseLightBorders;
end;
...
begin
  ...
  if EmulateMouseMoveAfterCalculate and HotTrack and GridViewInfo.Visible and Control.HandleAllocated and
    (Control.Handle = WindowFromPoint(Control.ClientToScreen(GridViewInfo.MousePos))) then
  begin
...
function TcxCustomGridViewCellViewInfo.GetActualScaleFactor: TdxScaleFactor;
begin
...
begin
  TdxTextMeasurer.TextMetrics(AFont, AMetrics);
end;
...
begin
  ...
  FIsCalculating := True;
  try
    if (Control <> nil) and (FSite.Parent = nil) then
      FSite.LookAndFeel.MasterLookAndFeel := TcxControlAccess(Control).LookAndFeel;
...
var
  AWasRightToLeftConverted: Boolean;
begin
  AWasRightToLeftConverted := IsRightToLeftConverted;
  MainCalculate(FSite.BoundsRect);
  if AWasRightToLeftConverted then
...
procedure TcxCustomGridViewInfoCache.Reset;
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
...
procedure TcxCustomGridOptions.ScaleFactorChanged;
begin
end;
...
procedure TcxCustomGridStyles.AfterConstruction;
begin
  inherited;
  RefreshCache;
end;
...
procedure TcxCustomGridStyles.GetDefaultViewParams(Index: Integer; AData: TObject; out AParams: TcxViewParams);
var
  AGrid: TcxCustomGrid;
begin
  inherited;
...
procedure TcxCustomGridStyles.RefreshCache;
begin
  FGridView := GetGridView;
  if GridView <> nil then
    FLookAndFeelPainter := GridView.LookAndFeelPainter
  else
...
begin
  CreateEx(AControl, AControl.IsDesigning);
...
begin
  if IsDetail then
    Result := (MasterDataController as IcxCustomGridDataController).GridView
  else
    Result := nil;
end;

function TcxCustomGridView.GetMasterDataController: TcxCustomDataController;
begin
  Result := DataController.GetMasterDataController;
end;

function TcxCustomGridView.GetMasterGridRecordIndex: Integer;
begin
  if MasterRecordIndex <> -1 then
    Result := MasterDataController.GetRowIndexByRecordIndex(MasterRecordIndex, False)
  else
    Result := -1;
end;
...
procedure TcxCustomGridView.SetAutomation(const AValue: TdxAutomationElementSettings);
begin
...
function TcxCustomGridView.CreateUIAProvider: IdxUIAProvider;
begin
  Result := nil;
end;
...
function TcxCustomGridView.GetAutomationSettings: TdxAutomationElementSettings;
begin
  Result := Automation;
end;

function TcxCustomGridView.GetAutomationSettingsClass: TdxAutomationElementSettingsClass;
begin
...
procedure TcxCustomGridView.FinishScaleFactorChange;
begin
  OptionsView.ScaleFactorChanged;
end;
...
procedure TcxCustomGridView.BeforeDestruction;
begin
  if Assigned(FUIAProvider) then
...
begin
  ...
  try
    ...
    try
      TcxDataControllerAccess(DataController).SetPatternDataController(APattern.DataController);
      Assign(APattern);
    finally
...
begin
  ...
  try
    if not IsStoringNameMode then
      if AOwnerName = '' then
        if Owner = nil then
          AStorage.NamePrefix := Control.Owner.Name
        else
...
procedure TcxCustomGridView.DoRefreshStylesCache;
begin
...
function TcxCustomGridView.CheckVisibility(APotentional: Boolean): Boolean;
var
  AMasterGridView: TcxCustomGridView;
begin
  if (csDestroying in ComponentState) or (FLevel = nil) or IsPattern then
    Exit(False);

  if APotentional then
    Result := TcxGridLevel(FLevel).CanBeVisible
  else
    Result := TcxGridLevel(FLevel).ActuallyVisible;

  if Result and IsDetail then
  begin
    AMasterGridView := MasterGridView;
    Result := AMasterGridView.Visible and AMasterGridView.IsDetailVisible(Self);
  end;
end;
...
function TcxCustomGridView.GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider;
begin
  Result := dxUIAGetProvider(FUIAProvider, CreateUIAProvider, SupportsUIAutomation and not IsDestroying, AIsWMGetObjectCall, False);
end;

function TcxCustomGridView.GetUseLightBorders: Boolean;
var
  ALookAndFeel: TcxLookAndFeel;
begin
  if TdxVisualRefinements.LightBorders then
  begin
    ALookAndFeel := LookAndFeel;
    if not Assigned(ALookAndFeel) then
      ALookAndFeel := RootLookAndFeel;
    Result := (not ALookAndFeel.NativeStyle) and ((ALookAndFeel.Kind = lfUltraFlat) or Assigned(ALookAndFeel.SkinPainter));
  end
  else
    Result := False;
end;
...
function TcxCustomGridView.IsDataScrollbar(AKind: TScrollBarKind): Boolean;
begin
  Result := False;
end;
...
function TcxCustomGridView.SupportsUIAutomation: Boolean;
begin
  Result := False;
end;

procedure TcxCustomGridView.UIANotify(ANotification: TdxUIAGridNotification);
begin
end;

function TcxCustomGridView.UIAProvider: IdxUIAProvider;
begin
  Result := GetUIAProvider(False);
end;
...
begin
  ...
  begin
    ...
    begin
      AMasterGridView := MasterGridView;
      if (AMasterGridView <> nil) and not AMasterGridView.IsUpdateLocked and
        (AMasterGridView.DataController.LockCount = 0) and
        (PatternGridView <> nil) and PatternGridView.HasAsClone(Self)
      then
        AMasterGridView.DetailDataChanged(Self);
    end
    else
...
function TcxCustomGridView.UseOptimalHeightWhenDetail: Boolean;
begin
  Result := False;
end;
...
function TcxCustomGridView.GetOwnerControl: TcxControl;
var
  ALevel: TcxGridLevel;
begin
  if Safe.Cast(Level, ALevel) then
    Result := ALevel.Control
  else
    Result := nil;
end;
...
begin
  if not FResourcesInitialized then
  begin
    InitializeResources;
    FResourcesInitialized := True;
  end;
...
procedure TcxCustomGridView.InitializeResources;
begin
  // Load Images Resources and so on
end;
...
begin
  dxMessageBox(0, Format('%s does not support master-detail relationships!', [ClassName]), 'Error', MB_OK or MB_ICONSTOP or MB_TASKMODAL);
  Abort;
end;
...
begin
  AMasterGridView := MasterGridView;
  Result := (AMasterGridView = AGridView) or (AMasterGridView <> nil) and AMasterGridView.HasAsMaster(AGridView);
end;
...
function TcxCustomGridView.GetDataViewStateStorageClass: TCustomGridViewStorageClass;
begin
  Result := TCustomGridViewStorage;
end;

function TcxCustomGridView.CreateDataViewStateStorage: TCustomGridViewStorage;
begin
  Result := GetDataViewStateStorageClass.Create;
end;

procedure TcxCustomGridView.RestoreDataViewState(AOptions: TcxGridStorageOptions);
begin
  if FDataViewStateStream <> nil then
  begin
    FDataViewStateStream.Position := 0;
    RestoreDataViewStateFromStream(FDataViewStateStream, AOptions);
    FreeAndNil(FDataViewStateStream);
  end;
end;

procedure TcxCustomGridView.RestoreDataViewStateFromStream(AStream: TStream; AOptions: TcxGridStorageOptions);
var
  AStorage: TCustomGridViewStorage;
begin
  AStorage := CreateDataViewStateStorage;
  try
    AStorage.Options := AOptions;
    AStorage.RestoreFromStream(Self, AStream);
  finally
    AStorage.Free;
  end;
end;

procedure TcxCustomGridView.StoreDataViewState(AOptions: TcxGridStorageOptions);
begin
  if FDataViewStateStream = nil then
    FDataViewStateStream := TMemoryStream.Create;
  FDataViewStateStream.Clear;
  StoreDataViewStateToStream(FDataViewStateStream, AOptions);
end;

procedure TcxCustomGridView.StoreDataViewStateToStream(AStream: TStream; AOptions: TcxGridStorageOptions);
var
  AStorage: TCustomGridViewStorage;
begin
  AStorage := CreateDataViewStateStorage;
  try
    AStorage.Options := AOptions;
    AStorage.SaveToStream(Self, AStream);
  finally
    AStorage.Free;
  end;
end;
...
function TcxCustomGridView.SizeChanged(AUpdateSelfOnly: Boolean = False; AKeepMaster: Boolean = False): Boolean;
begin
  Result := Changed(TcxGridSizeChange.Create(Self, AUpdateSelfOnly or (Control = nil){7}, AKeepMaster));
end;
...
function TcxCustomGridView.GetBordersScaleFactor: TdxScaleFactor;
begin
...
function TcxCustomGridView.GetScrollbarAnnotationHelper: TdxScrollbarAnnotations;
begin
  Result := nil;
end;
...
procedure TCustomGridViewStorage.RestoreStateFromNode(AParentNode: TdxXMLNode);
begin
end;

procedure TCustomGridViewStorage.RestoreFromStream(AView: TcxCustomGridView; AStream: TStream);
var
  ADocument: TdxXMLDocument;
  AParentNode: TdxXMLNode;
  I: Integer;
begin
  GridView := AView;
  ADocument := nil;
  try
    ...
    AParentNode := ADocument.FindChild(['Components']);
    for I := 0 to AParentNode.Count - 1 do
      if AParentNode.Items[I].Attributes.GetValueAsString('ComponentName') = GridView.Name then
      begin
        RestoreStateFromNode(AParentNode.Items[I].First);
        Break;
      end;
  finally
    ADocument.Free;
...
procedure TCustomGridViewStorage.SaveStateToNode(AParentNode: TdxXMLNode);
begin
end;

procedure TCustomGridViewStorage.SaveToStream(AView: TcxCustomGridView; AStream: TStream);
var
  ADocument: TdxXMLDocument;
  AParentNode: TdxXMLNode;
  ANode: TdxXMLNode;
begin
  ...
  try
    ...
    ANode.Attributes.SetValueAsString('ComponentName', GridView.Name);
    SaveStateToNode(ANode);
...
class procedure TcxCustomGridViewAccess.FinishScaleFactorChange(AInstance: TcxCustomGridView);
begin
  AInstance.FinishScaleFactorChange;
end;
