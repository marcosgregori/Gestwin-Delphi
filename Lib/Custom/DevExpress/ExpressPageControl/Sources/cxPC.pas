...
uses
  System.UITypes,
  Winapi.Windows, Winapi.Messages, System.Classes, Vcl.Controls, Vcl.Forms, Vcl.Graphics, Vcl.ImgList, System.SysUtils, System.Contnrs, Vcl.Menus, Vcl.Themes,
...
type
  ...
  TcxPCTabulationCycle = ( tcDefault, tcComplete, tcNone ); //..
  ...
  TcxPCOutTabImageAndTextData = record // for internal use
    TabImageRect: TRect;
    TabTextRect: TRect;
  ...
  TcxPCTabsPosition = record // for internal use
  ...
  TcxPCWOffset = record // for internal use
    Left, Right: Integer;
  end;

  TcxPCDistance = record // for internal use
    dw, dh: Integer;
  end;

  TcxPCIndexInterval = record // for internal use
    Left, Right: Integer;
  end;
  TcxPCLineIndexBoundsArray = array of TcxPCIndexInterval; // for internal use

  TcxPCTabIndex = Integer;

  TcxPCTabPropertyChanged = (tpcNotSpecified, tpcColor, tpcEnabled, tpcFocused, // for internal use
    tpcHighlighted, tpcHotTrack, tpcIsMainTab, tpcLayout, tpcPressed,
    tpcSelected, tpcTracking, tpcVisible);

  TcxPCTabNotification = (tnDeleting); // for internal use

  TcxPCImageListRotatedImagesElement = record // for internal use
    BackgroundColor: TColor;
    Bitmap: TcxBitmap;
    IsBackgroundColorSpecified: Boolean;
  end;
  TcxPCImageListRotatedImagesElementArray = array of TcxPCImageListRotatedImagesElement; // for internal use

  { IcxTabControl }

  IcxTabControl = interface(IcxControlComponentState) // for internal use
  ...
  TcxPCCustomElementViewInfo = class(TcxIUnknownObject, IcxHintableObject) // for internal use
  strict private
    FBounds: TRect;
  ...
  TcxPCCustomButtonViewInfo = class(TcxPCCustomElementViewInfo) // for internal use
  strict private
  ...
  TcxPCCustomTabButtonViewInfo = class(TcxPCCustomButtonViewInfo) // for internal use
  strict private
  ...
  TcxPCTabButtonViewInfo = class(TcxPCCustomTabButtonViewInfo) // for internal use
  strict private
    FButton: TcxPCButton;
  protected
  ...
  TcxPCTabCloseButtonViewInfo = class(TcxPCCustomTabButtonViewInfo) // for internal use
  protected
    procedure DoClick; override;
    function GetHitTestIndex: Integer; override;
  public
  ...
  TcxPCCustomHeaderButtonViewInfo = class(TcxPCCustomButtonViewInfo) // for internal use
  private
  ...
  TcxPCHeaderButtonViewInfo = class(TcxPCCustomHeaderButtonViewInfo) // for internal use
  private
    FButton: TcxPCButton;
  protected
    procedure DoClick; override;
  ...
  TcxPCNavigatorButtonViewInfo = class(TcxPCCustomHeaderButtonViewInfo) // for internal use
  private
  ...
  TcxPCButtonViewInfos = class // for internal use
  private
  ...
  TcxTabViewInfo = class(TcxPCCustomElementViewInfo) // for internal use
  private
  ...
  TcxPCNewButtonViewInfo = class(TcxTabViewInfo) // for internal use
  private
  ...
  TcxTabsViewInfo = class // for internal use
  private
  ...
  TcxCustomTabControlViewInfo = class // for internal use
  private
  ...
  protected
    ...
    function ArrowButtonClick(ANavigatorButton: TcxPCNavigatorButton): Boolean; virtual;
  ...
  public
    ...
    property IControl: IcxTabControl read FIControl; // for internal use
  ...
  TcxPCCustomPainter = class // for internal use
  strict private
  ...
  protected
    ...
    function GetTabVerticalCorrection(const AFullRect: TRect; ATabPosition: TcxTabPosition): TRect; virtual;
    ...
    function GetScaledDrawImageOffset(ATabVisibleIndex: Integer): TRect;
    ...
    function GetScaledDrawTextHOffset(ATabVisibleIndex: Integer): TRect;
    ...
    function IsEnoughSpaceForClientPage: Boolean;
  ...
  public
    ...
    class function HasTabPressedState: Boolean; virtual;
  ...
  TcxPCImageList = class // for internal use
  strict private
  ...
  TcxPCCustomGoDialog = class // for internal use
  private
  ...
  TcxTabControlDragAndDropObject = class(TcxDragAndDropObject) // for internal use
  private
  ...
  TcxCustomTabControlHintHelper = class(TcxCustomHintHelper) // for internal use
  private
    FController: TcxCustomTabControlController;
  protected
  ...
  private
    ...
    function HasTabPressedState: Boolean;
  ...
  TcxCustomTabControlPropertiesChangedType = (pctHard, pctMedium, pctSimple, pctLight); // for internal use
  ...
  TcxPCNewEvent = procedure(Sender: TObject; AIndex: Integer) of object; // deprecated
  ...
  public
    ...
    property Painter: TcxPCCustomPainter read FPainter; // for internal use
    property ParentBackground;
    property Properties: TcxCustomTabControlProperties read FProperties write SetProperties;
    property TabStop default True;
    property ViewInfo: TcxCustomTabControlViewInfo read FViewInfo; // for internal use
  end;
  ...
  TcxPageControlViewInfo = class(TcxCustomTabControlViewInfo) // for internal use
  private
  ...
  private
    ...
    FOnPageChanging: TcxPageChangingEvent;
    FTabulationCycle : TcxPCTabulationCycle;  //..
  ...
  published
    ...
    property TabulationCycle : TcxPCTabulationCycle read FTabulationCycle write FTabulationCycle default tcDefault; //..
    property TabWidth;
...
constructor TcxCustomTabControlHintHelper.Create(AController: TcxCustomTabControlController);
begin
  inherited Create;
...
procedure TcxPageControlProperties.MoveTab(ACurrentIndex, ANewIndex: Integer);
begin
  Pages[ACurrentIndex].PageIndex := ANewIndex;
end;
...
begin
  ...
  case InternalGetTextRotationAngle(ControlViewInfo) of
    ra0:
      begin
        Inc(R.Left, AContentHorizontalOffset);
        if AIsTabHasImage then
        begin
          FImageRect.Left := R.Left + ControlViewInfo.ImageBorder;
          FImageRect.Right := FImageRect.Left + ControlViewInfo.GetTabImageSize.cx;
        end;
        Inc(R.Left, AImageAreaWidth + AImageTextDistance);

        if not FIsTextTooLong and AIsAssignedImagesAndRotate then
          Inc(R.Left, GetContentHorizontalOffset(cxRectWidth(R), ATextWidth));
        FTextRect.Left := R.Left;
        FTextRect.Right := FTextRect.Left + ATextWidth;
        if ControlViewInfo.UseRightToLeftAlignment and (ControlViewInfo.TabPosition in [tpRight, tpLeft]) and
          ControlViewInfo.ActuallyRotate then
        begin
          FImageRect := TdxRightToLeftLayoutConverter.ConvertRect(FImageRect, AContentRect);
          FTextRect := TdxRightToLeftLayoutConverter.ConvertRect(FTextRect, AContentRect);
        end;
      end;
    raPlus90:
      begin
        Dec(R.Bottom, AContentHorizontalOffset);
        if AIsTabHasImage then
        begin
          FImageRect.Bottom := R.Bottom - ControlViewInfo.ImageBorder;
          FImageRect.Top := FImageRect.Bottom - ControlViewInfo.GetTabImageSize.cx;
        end;
        Dec(R.Bottom, AImageAreaWidth + AImageTextDistance);

        if not FIsTextTooLong and AIsAssignedImagesAndRotate then
          Dec(R.Bottom,
            GetContentHorizontalOffset(cxRectHeight(R), ATextWidth));
        FTextRect.Bottom := R.Bottom;
        FTextRect.Top := FTextRect.Bottom - ATextWidth;
      end;
    raMinus90:
      begin
        Inc(R.Top, AContentHorizontalOffset);
        if AIsTabHasImage then
        begin
          FImageRect.Top := R.Top + ControlViewInfo.ImageBorder;
          FImageRect.Bottom := FImageRect.Top + ControlViewInfo.GetTabImageSize.cx;
        end;
        Inc(R.Top, AImageAreaWidth + AImageTextDistance);
    ...
    case InternalGetTextRotationAngle(ControlViewInfo) of
      ...
      raPlus90:
        begin
          ...
          if Painter.HasTabPressedState then
          begin
...
function TcxTabViewInfo.HasPressedState: Boolean;
begin
  Result := ControlViewInfo.Painter.HasTabPressedState;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        cxDrawText(cxMeasureCanvas.Handle, Caption, R, Painter.GetCTFlags(VisibleIndex));
        FTextSize := cxSize(R);
      end
      else
        FTextSize := cxSize(cxGetTextRect(cxMeasureCanvas.Font, Caption, ALineCount, True))
    end
    else
      FTextSize := cxTextSize(cxMeasureCanvas.Font, Caption, Painter.GetCTFlags(VisibleIndex));
  end;
  Result := FTextSize;
end;
...
var
  ...
  procedure ValidateTabVisibleIndexes;
  begin
...
begin
  ...
  begin
    if TabsViewInfo[ATabVisibleIndex].HasPressedState and not IControl.IsDesigning then
...
begin
  if (ATabVisibleIndex <> -1) and
    TabsViewInfo[ATabVisibleIndex].HasPressedState and (ATabVisibleIndex = FPressedTabVisibleIndex) then
...
begin
  if Supports(IControl, IInterfaceComponentReference, AComponentReference) and
    (AComponentReference.GetComponent is TWinControl)
  then
    Result := AComponentReference.GetComponent
  else
    Result := nil;
end;
...
procedure TcxPCImageList.Draw(ACanvas: TcxCanvas; const R: TRect; AIndex: Integer;
  ...
  begin
    TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, R, R, nil, GetImages(AHot), AIndex,
  ...
  begin
    ...
    Result.Rotate(cxOppositeRotationAngle[ImageRotationAngle]);
...
begin
  ...
  if HasTabPressedState and (ATabVisibleIndex = FMouseDownTabVisibleIndex) then
...
begin
  ...
  if ATabVisibleIndex <> -1 then
  begin
    if not (HasTabPressedState or not Properties.ActivateFocusedTab) or
      (Key in [VK_TAB, VK_RETURN, VK_SPACE]) then
      TabClick(ATabVisibleIndex, [], False)
    else
    begin
      if not HasTabPressedState then
...
begin
  ...
  begin
    ...
    try
      if not AMouseEvent or HasTabPressedState then
...
begin
  ...
  begin
    FMouseDownTabVisibleIndex := -1;
    if HasTabPressedState and not IControl.IsDesigning then
...
function TcxCustomTabControlController.HasTabPressedState: Boolean;
begin
  Result := ViewInfo.Painter.HasTabPressedState;
end;
...
procedure TcxCustomTabControlProperties.SetTabIndex(Value: Integer);
  ...
  function HaveSelectableTab: Boolean;
  begin
    Result := (Tabs.NewButton.Visible and (Tabs.VisibleTabsCount > 1))
      or  (not Tabs.NewButton.Visible and (Tabs.VisibleTabsCount > 0))
  end;
...
begin
  ...
  if (Value <> -1) and TabIndexTabMustBeVisible and (not Tabs[Value].Visible) then
    if HaveSelectableTab then
      Value := Tabs.VisibleTabs[Tabs.VisibleTabsCount - 1].VisibleIndex
    else
      Value := -1;
  if Value = TabIndex then
    Exit;
  if not CanChange(Value) then
    DoSetTabIndex(Min(TabIndex, Tabs.Count - 1))
  else
  begin
    UnselectTabs;
    DoSetTabIndex(Value);
...
begin
  if Focused or HandleAllocated and Winapi.Windows.IsChild(Handle, Winapi.Windows.GetFocus) then
  begin
...
begin
  ...
  if TdxFormHelper.CanResetScalingFlags(Self, M, D) then
    FScalingFlags := [];
  inherited;
end;
...
begin
  Result := cxNullRect;
  if IsEnoughSpaceForClientPage then
  begin
...
class function TcxPCCustomPainter.HasTabPressedState: Boolean;
begin
  Result := False;
end;
...
procedure TcxPCCustomPainter.CorrectTabContentVerticalOffset(ATabVisibleIndex: Integer; var ADrawOffset: TRect);
  ...
  procedure CorrectTabContentVerticalOffset;
  begin
    if HasTabPressedState then
    begin
...
begin
  CorrectTabContentVerticalOffset;
...
function TcxPCCustomPainter.GetTabVerticalCorrection(const AFullRect: TRect; ATabPosition: TcxTabPosition): TRect;
begin
  Result := GetExtendedRect(AFullRect, Rect(0, 0, 0, 0), ATabPosition);
end;
    ...
    begin
      ...
      try
        ...
        try
          ...
          TdxImageDrawer.DrawBitmap(ACanvas.Handle, ABitmap, AButtonRect, cxNullPoint);
        finally
...
begin
  ...
  ADrawImageOffset := GetDrawImageOffset(ATabVisibleIndex);
  Result.Top := ADrawImageOffset.Top;
  Result.Bottom := ADrawImageOffset.Bottom;
  Dec(Result.Bottom);
end;
...
function TcxPCCustomPainter.GetScaledDrawImageOffset(ATabVisibleIndex: Integer): TRect;
begin
  Result := ScaleFactor.Apply(GetDrawImageOffset(ATabVisibleIndex));
end;

function TcxPCCustomPainter.GetScaledDrawTextHOffset(ATabVisibleIndex: Integer): TRect;
begin
  Result := ScaleFactor.Apply(GetDrawTextHOffset(ATabVisibleIndex));
end;
...
function TcxPCCustomPainter.GetHighlightedTextColor(ATabVisibleIndex: Integer; ATextColor: TColor): TColor;
var
  AColor: TColor;
begin
  if IsNativePainting then
  begin
    if cxGetThemeColor(totTab, TABP_TABITEM, GetTabNativeState(ATabVisibleIndex), TMT_TEXTCOLOR, AColor) then
      Result := AColor
    else
      Result := ATextColor;
  end
...
begin
  R := Rect(0, 0, 100, 100);
  GetThemeBackgroundContentRect(OpenTheme(totTab, ScaleFactor), 0, TABP_PANE, 0, R, Result);
...
begin
  ...
  begin
    ...
    begin
      ...
      DrawThemeText(OpenTheme(totTab, ScaleFactor), ACanvas.Canvas.Handle, APartId,
...
begin
  ...
  begin
    if IntersectWithCurrentClipRegion and
      (not Winapi.Windows.RectVisible(Handle, ClipR)) then Exit;
    SaveClipRegion;
...
function TcxPCCustomPainter.IsEnoughSpaceForClientPage: Boolean;
