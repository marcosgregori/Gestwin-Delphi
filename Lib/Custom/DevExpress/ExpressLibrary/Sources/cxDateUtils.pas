...
uses
  System.Variants, System.SysUtils, Winapi.Windows, Vcl.Controls, System.Classes, Vcl.Graphics, System.Win.Registry,
...
type
  ...
  {$SCOPEDENUMS ON}
  TdxDateTimeKind = (Date, Time, DateTime);
  {$SCOPEDENUMS OFF}
  ...
  TdxDayOfWeekVCL = 1..7;    //1 = Sunday..7 = Saturday

  TgxDuration = LongWord; //..
...
type
  ...
  TcxDurationFormat = (dfHourMinSec, dfHourMin, dfHour);  //..
  ...
  public
    property Items[Index: Integer]: TcxEra read GetItem write SetItem; default; // for internal use
  end;
  ...
  public
    ...
    function FromDateTime(AYear, AMonth, ADay: Cardinal; AHours, AMinutes, ASeconds: Byte; AMilliseconds: Word): TcxDateTime; overload; virtual;
    ...
    function GetWeekNumber(ADate: TDateTime; AStartOfWeek: TDay; AFirstWeekOfYear: TcxFirstWeekOfYear): Cardinal; overload; virtual;
    function GetWeekNumber(ADate: TcxDateTime; AStartOfWeek: TDay; AFirstWeekOfYear: TcxFirstWeekOfYear): Cardinal; overload; virtual; abstract;
    ...
    function ToDateTime(AYear, AMonth, ADay: Cardinal; AHours, AMinutes, ASeconds: Byte; AMilliseconds: Word): TDateTime; overload; virtual;
    function ToDateTime(const ADate: TcxDate): TDateTime; overload; virtual;
    function ToDateTime(const ADateTime: TcxDateTime): TDateTime; overload; virtual; abstract;

    function GetDayNumber(const S: string): Cardinal;
    function GetMonthNumber(AYear: Integer; const S: string): Cardinal; overload;
    function GetMonthNumber(AYear: Integer; const S: string; const AFormat: TFormatSettings): Cardinal; overload;
    function GetYearNumber(const S: string): Cardinal;
    function TryGetDayNumber(const S: string; out AValue: Cardinal): Boolean; virtual;
    function TryGetMonthNumber(AYear: Integer; const S: string; const AFormat: TFormatSettings; out AValue: Cardinal): Boolean; virtual;
    function TryGetYearNumber(const S: string; out AValue: Cardinal): Boolean; virtual;
    ...
    property DefaultEra: TcxEra read GetDefaultEra; // for internal use
    property Eras: TcxEras read FEras; // for internal use
  ...
  strict private
    ...
    class procedure ReadTimeZoneInfoFromRegistry(ARegistry: TRegistry; out AInfo: TcxTimeZoneInformation); static;
  private
  ...
  public
    ...
    class function AddDSTInfo(var ATimeZoneInfo: TcxTimeZoneInformation; const AYear: Integer; const AStandardDate, ADaylightDate: TSystemTime): Integer; overload; static;
    class function AddDSTInfo(var ATimeZoneInfo: TcxTimeZoneInformation; const AYear: Integer; const AStandardDate, ADaylightDate: TDateTime): Integer; overload; static;
    class function AddDSTInfo(const ATimeZone: Integer; const AYear: Integer; const AStandardDate, ADaylightDate: TDateTime): Integer; overload; static;
    class function CalculateTransitionDate(const AYear: Word;const ADSTDateTime: TSystemTime): TDateTime; static;
    class function ConvertTimeZoneToTimeZone(const ADateTime: TDateTime; ASourceTimeZone, ADestTimeZone: Integer; AUseDayLightBias: Boolean = True): TDateTime; static;
    class function ConvertToGlobalTime(const ADateTime: TDateTime; ATimeZone: Integer = -1): TDateTime; static;
    class function ConvertToLocalTime(const ADateTime: TDateTime; ATimeZone: Integer = -1): TDateTime; static;
    ...
    class function TimeZoneBias(AIndex: Integer): TDateTime; overload; static;
    class function TimeZoneBias(AIndex: Integer; ADate: TDateTime; AUseDayLightBias: Boolean): TDateTime; overload; static;
...
function dxGetNowInUTC(TrimMilliseconds: Boolean = False): TDateTime;
...
function GetDateTimeKind(const AValue: TDateTime): TdxDateTimeKind;
...
function cxGetDateOrder(const ADateFormat: string): TcxDateOrder;
...
function cxEncodeDuration( H, M, S : Word ) : TgxDuration; //..
procedure cxDecodeDuration( Duration : TgxDuration; var H : Word; var M : Word; var S : Word );
function cxStrToDuration( const ADurationStr : String; AFormat : TcxDurationFormat = dfHourMinSec ) : TgxDuration; //..
function cxTryStrToDuration( const ADurationStr : String; out AValue : TgxDuration) : Boolean; //..
function cxFormatDuration( DurationFormat : TcxDurationFormat; Value : TgxDuration; TimeSeparator : Char = ':' ) : String;
function cxDurationToTime( Duration : TgxDuration; Format : TcxDurationFormat ) : TTime;
...
function cxFloatToDateTime(const AValue: Double): TDateTime;
...
function cxTryStrToDate(const ADateStr: string; const AFormat: TFormatSettings;
  out ADateTime: TcxDateTime; ACalendar: TcxCustomCalendarTable = nil): Boolean; overload;
function cxTryStrToDate(const ADateStr: string; const AFormat: TFormatSettings; const ADateSeparator: Char;
  out ADateTime: TcxDateTime; ACalendar: TcxCustomCalendarTable = nil): Boolean; overload;
...
function cxIsWeekendDay(const ADate: TDateTime): Boolean;
...
uses
  System.RTLConsts, System.DateUtils, System.StrUtils, System.Math, System.SysConst,
  cxLibraryStrs, cxClasses, dxCalendarUtils, cxFormats, cxGraphics;

const
  dxThisUnitName = 'cxDateUtils';
...
function CalendarList: TdxFastObjectList;
begin
  ...
  begin
    case AFormatInfo.Items[I].Kind of
      ...
      dtikDay:
        begin
          Result := doDMY;
          if Length(AFormatInfo.Items[I].Data) <= 2 then
            Break;
        end;
    end;
  end;
...
begin
  if cxIsGregorianCalendar(ACalendar) and cxFormatController.UseDelphiDateTimeFormats then
    Exit(dxFormatSettings.LongMonthNames[AMonth]);
...
function cxFloatToDateTime(const AValue: Double): TDateTime;
begin
  if (AValue < MinDateTime) or (AValue > MaxDateTime) then
    raise EConvertError.CreateResFmt(@SInvalidDateTimeFloat, [AValue]);
  Result := RoundTo(AValue, -9);
end;
...
function GetEraYearOffset(const AFormat: TFormatSettings; const Name: string): Integer;
begin
  Result := FormatSettings.GetEraYearOffset(Name);
end;
...
begin
  ALength := Length(S);
  while (APos <= ALength) and dxCharInSet(S[APos], ['0'..'9']) do
  begin
...
procedure SkipBlanks(const S: string; var APos: Integer);
begin
  if APos <= 0 then
    Exit;
  while (APos < Length(S)) and (S[APos] = ' ') do
    Inc(APos);
end;

function ScanPart(const S: string; var APos: Integer; const AEndScan: Char): string; overload;
var
  J: Integer;
  L: Integer;
begin
  J := APos;
  L := Length(S);
  while (APos <= L) and (Ord(S[APos]) <> Ord(AEndScan)) do
    Inc(APos);
  if (J > 1) or (APos - J <> L) then
    Result := Copy(S, J, APos - J)
  else
    Result := S;
end;

function ScanPart(const S: string; var APos: Integer; AEndScan: TdxAnsiCharSet): string; overload;
begin
  Result := '';
...
function cxStrToDate(const ADateStr: string; const AFormat: TFormatSettings; ACalendar: TcxCustomCalendarTable = nil): TcxDateTime;
begin
  if not cxTryStrToDate(ADateStr, AFormat, Result, ACalendar) then
    raise EInvalidArgument.CreateFmt('Cannot convert the "%s" to date time', [ADateStr]);
end;

function cxTryStrToDateNumber(const S: string; out V: Cardinal): Boolean;
var
  I, E: Integer;
begin
  Val(S, I, E);
...
function cxTryStrToDate(const ADateStr: string; const AFormat: TFormatSettings;
  out ADateTime: TcxDateTime; ACalendar: TcxCustomCalendarTable = nil): Boolean;
begin
  Result := cxTryStrToDate(ADateStr, AFormat, AFormat.DateSeparator, ADateTime, ACalendar);
end;
...
function cxTryStrToDate(const ADateStr: string; const AFormat: TFormatSettings; const ADateSeparator: Char;
  ...
  function NeedMoreScanMonthStr(var APos: Integer): Boolean;
  begin
    Result := False;
    if ADateSeparator = ' ' then
    begin
      ScanBlanks(ADateStr, APos);
      Result := (APos < Length(ADateStr)) and not dxCharInSet(ADateStr[APos], ['0'..'9']);
    end;
  end;
...
begin
  ...
  if AFormat.ShortDateFormat <> '' then
  begin
    if AFormat.ShortDateFormat[1] = 'g' then  // skip over prefix text
    begin
      ScanNumber(ADateStr, APos);
      AEraYearOffset := GetEraYearOffset(AFormat, Trim(Copy(ADateStr, 1, APos - 1)));
    end
    else
      if AnsiPos('e', AFormat.ShortDateFormat) > 0 then
      begin
        if High(AFormat.EraInfo) >= 0 then
          AEraYearOffset := AFormat.EraInfo[High(AFormat.EraInfo)].EraOffset;
      end;
  end;

  APart1 := ScanPart(ADateStr, APos, ADateSeparator);
  if APos >= ALength then Exit(False);

  Inc(APos);
  APart2 := ScanPart(ADateStr, APos, ADateSeparator);
  if APos >= ALength then Exit(False);

  Inc(APos);
  SkipBlanks(ADateStr, APos);
  APart3 := ScanPart(ADateStr, APos, ' ');

  case GetDateOrder(AFormat.ShortDateFormat) of
    doMDY:
      begin
        if NeedMoreScanMonthStr(APos) then
        begin
          APart1 := APart1 + ' ' + APart2;
          APart2 := APart3;
          APart3 := ScanPart(ADateStr, APos, ' ');
        end;
        if not ACalendar.TryGetDayNumber(APart2, ADateTime.Day) then
          Exit(False);
        if not ACalendar.TryGetYearNumber(APart3, ADateTime.Year) then
          Exit(False);
        if not ACalendar.TryGetMonthNumber(ADateTime.Year, APart1, AFormat, ADateTime.Month) then
          Exit(False);
      end;
    ...
    doDMY:
      begin
        ...
        if not ACalendar.TryGetDayNumber(APart1, ADateTime.Day) then
          Exit(False);
        if not ACalendar.TryGetYearNumber(APart3, ADateTime.Year) then
          Exit(False);
        if not ACalendar.TryGetMonthNumber(ADateTime.Year, APart2, AFormat, ADateTime.Month) then
          Exit(False);
      end;
    ...
    doYMD:
      begin
        ...
        if not ACalendar.TryGetDayNumber(APart3, ADateTime.Day) then
          Exit(False);
        if not ACalendar.TryGetYearNumber(APart1, ADateTime.Year) then
          Exit(False);
        if not ACalendar.TryGetMonthNumber(ADateTime.Year, APart2, AFormat, ADateTime.Month) then
          Exit(False);
      end;
  ...
  ADateTime.Era := ACalendar.GetEra(AEraYearOffset + 1);
  ...
  if APos < ALength then
  begin
    if not TryStrToTime(Copy(ADateStr, APos, MaxInt), ATime, AFormat) then
      Exit(False);
...
begin
     APos := 1;
     APart1 := ScanPart(ADurationStr, APos, [cxFormatController.LocalFormatSettings.TimeSeparator]);
     Inc(APos);
     APart2 := ScanPart(ADurationStr, APos, [cxFormatController.LocalFormatSettings.TimeSeparator]);
     Inc(APos);
     APart3 := ScanPart(ADurationStr, APos, [' ']);
     case AFormat of
       dfHourMinSec :
         begin
         H := StrToInt( APart1 );
     ...
     Result := cxEncodeDuration( H, M, S );
end;

function cxFormatDuration( DurationFormat : TcxDurationFormat;
                           Value          : TgxDuration;
...
begin
     cxDecodeDuration( Value, H, M, S );
     case DurationFormat of
       dfHour       : Result := Format( '%.2d', [ H ] );
       dfHourMin    : Result := Format( '%.2d' + TimeSeparator + '%.2d', [ H, M ] );
       dfHourMinSec : Result := Format( '%.2d' + TimeSeparator + '%.2d' + TimeSeparator + '%.2d', [ H, M, S ] );
       end;
end;

function cxGetTimeDuration( ANow, AThen: TTime ) : TgxDuration;
begin
     Result := SecondsBetween( ANow, AThen );
end;
...
begin
     cxDecodeDuration( Duration, H, M, S );
...
begin
     Result := True;
     try
       AValue := cxStrToDuration( ADurationStr );
     except
       Result := False;
       end;
end;
...
begin
  Result := nil;
  for I := 0 to CalendarList.Count - 1 do
    if TcxCustomCalendarTable(CalendarList[I]).CalendarID = ACalendarID then
    begin
      Result := TcxCustomCalendarTable(CalendarList[I]);
      Break;
    end;
end;
...
function cxGetDateOrder(const ADateFormat: string): TcxDateOrder;
begin
  Result := GetDateOrder(ADateFormat);
end;
...
procedure CorrectTextForDateTimeConversion(var AText: string;
  ...
  begin
    S := StringReplace(S, ASubStr, DupeString(' ', Length(ASubStr)),
  ...
  procedure RemoveStringsThatInFormatInfo(var S: WideString; const ADateTimeFormatInfo: TcxDateTimeFormatInfo);
  var
  ...
  procedure RemoveDayNames(var S: WideString);
  var
    I: Integer;
  begin
    for I := 1 to 7 do
    begin
      S := StringReplace(S, dxFormatSettings.LongDayNames[I], '', [rfIgnoreCase, rfReplaceAll]);
      S := StringReplace(S, dxFormatSettings.ShortDayNames[I], '', [rfIgnoreCase, rfReplaceAll]);
    end;
  end;

  procedure CheckStringByDateFormatInfo(var S: WideString; const ADateTimeFormatInfo: TcxDateTimeFormatInfo);
  const
    ...
    begin
      ...
      begin
        case ADateTimeFormatInfo.Items[I].Kind of
          dtikMonth:
            AMonth := ScanPart(S, APos, [' ', cxFormatController.LocalFormatSettings.DateSeparator, dxFormatSettings.DateSeparator]);
          dtikDay:
            begin
              ABeginPos := APos;
              ScanNumber(S, APos);
...
begin
  ...
  begin
    ...
    ALength := Length(S);
    if (ALength > 0) and (S[ALength] = cxFormatController.LocalFormatSettings.TimeSeparator) then
      S := S + '00';
  end;
  AText := S;
end;
...
function InternalDateTimeToText(ADateTime: TDateTime; AFourDigitYearNeeded, AUseDelphiDateTimeFormats, AShowZeroTime: Boolean): string;
  ...
  begin
    ...
    begin
      ...
      if AShowZeroTime or (dxTimeOf(ADateTime) <> 0) then
...
var
  ...
  ADateStr: array[0..100] of Char;
begin
  if ADateTime = NullDate then
    Result := ''
  else
    if AUseDelphiDateTimeFormats then
      DateTimeToString(Result, GetDateTimeFormat, ADateTime)
    else
    begin
      DateTimeToSystemTime(ADateTime, ASystemTime);
      if GetDateFormat(GetThreadLocale, 0, @ASystemTime, PChar(GetDateTimeFormat), ADateStr, 100) <> 0 then
      begin
        Result := ADateStr;
        if AShowZeroTime or (dxTimeOf(ADateTime) <> 0) then
        begin
          GetTimeFormat(GetThreadLocale, 0, @ASystemTime, nil, ADateStr, 100);
          Result := Result + ' ' + ADateStr;
        end;
      end
      else
        try
          Result := VarFromDateTime(ADateTime);
        except
          on EVariantError do
            Result := '';
        end;
...
function cxDateTimeToText(ADate: TDateTime; AFourDigitYearNeeded: Boolean = False; AUseDelphiDateTimeFormats: Boolean = False): string;
begin
  Result := InternalDateTimeToText(ADate, AFourDigitYearNeeded, AUseDelphiDateTimeFormats, False);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Result := InternalDateTimeToText(ADate, AFourDigitYearNeeded, cxFormatController.UseDelphiDateTimeFormats, AIsDateTimeEdit and cxFormatController.DateEditShowZeroTime);
    end
    else
...
function cxIsWeekendDay(const ADate: TDateTime): Boolean;
begin
  Result := dxDayOfWeek(ADate) in [dSunday, dSaturday];
end;

{ TcxEra }

constructor TcxEra.Create(AEra: Integer; AStartDate: TDateTime; AYearOffset, AMinEraYear, AMaxEraYear: Integer);
begin
  Era := AEra;
...
procedure TcxCustomCalendarTable.YearToGregorianYear(var AYear: Cardinal; AEra: Integer);
begin
  if AEra = -1 then
...
function TcxCustomCalendarTable.AddDays(ADate: TcxDateTime; ACountDays: Integer): TDateTime;
begin
  Result := ToDateTime(ADate) + ACountDays;
  CheckDateTime(Result);
end;

function TcxCustomCalendarTable.AddMonths(ADate: TDateTime; ACountMonths: Integer): TDateTime;
begin
  Result := AddMonths(FromDateTime(ADate), ACountMonths);
end;

function TcxCustomCalendarTable.AddMonths(ADate: TcxDateTime; ACountMonths: Integer): TDateTime;
var
  ASwap: Integer;
...
function TcxCustomCalendarTable.AddWeeks(ADate: TDateTime; ACountWeeks: Integer): TDateTime;
begin
  Result := AddWeeks(FromDateTime(ADate), ACountWeeks);
end;

function TcxCustomCalendarTable.AddWeeks(ADate: TcxDateTime; ACountWeeks: Integer): TDateTime;
begin
  Result := AddDays(ADate, ACountWeeks * 7);
  CheckDateTime(Result);
end;

function TcxCustomCalendarTable.AddYears(ADate: TDateTime; ACountYears: Integer): TDateTime;
begin
  Result := AddYears(FromDateTime(ADate), ACountYears);
end;

function TcxCustomCalendarTable.AddYears(ADate: TcxDateTime; ACountYears: Integer): TDateTime;
var
  ACurrentYear: Integer;
  ACurrentEra: Integer;
begin
  if IsNotValid(ADate, Result) then
    Exit;
  ACurrentYear := Integer(ADate.Year) + ACountYears;
  ACurrentEra := ADate.Era;
  AdjustYear(ACurrentYear, ACurrentEra);
  if not IsValidYear(ACurrentEra, ACurrentYear) then
  begin
    Result := MinSupportedDate;
    Exit;
  end;
  if not IsValidMonth(ACurrentEra, ACurrentYear, ADate.Month) then
    ADate.Month := GetMonthsInYear(ACurrentEra, ACurrentYear);
  if not IsValidDay(ACurrentEra, ACurrentYear, ADate.Month, ADate.Day) then
    ADate.Day := GetDaysInMonth(ACurrentEra, ACurrentYear, ADate.Month);
  AdjustYear(ACurrentYear, ACurrentEra, ADate.Month, ADate.Day);
  ADate.Year := ACurrentYear;
  ADate.Era := ACurrentEra;
  Result := ToDateTime(ADate);
  CheckDateTime(Result);
...
function TcxCustomCalendarTable.FromDateTime(AYear, AMonth, ADay: Cardinal): TcxDateTime;
begin
  Result := FromDateTime(AYear, AMonth, ADay, 0, 0, 0, 0);
end;

function TcxCustomCalendarTable.FromDateTime(AYear, AMonth,
  ADay: Cardinal; AHours, AMinutes, ASeconds: Byte; AMilliseconds: Word): TcxDateTime;
begin
...
function TcxCustomCalendarTable.GetWeekNumber(ADate: TDateTime; AStartOfWeek: TDay; AFirstWeekOfYear: TcxFirstWeekOfYear): Cardinal;
begin
...
function TcxCustomCalendarTable.IsValidDay(AYear, AMonth, ADay: Cardinal): Boolean;
begin
  Result := IsValidDay(-1, AYear, AMonth, ADay);
end;

function TcxCustomCalendarTable.IsValidDay(AEra: Integer;AYear, AMonth, ADay: Cardinal): Boolean;
begin
...
function TcxCustomCalendarTable.IsValidMonth(AYear, AMonth: Cardinal): Boolean;
begin
  Result := IsValidMonth(-1, AYear, AMonth);
end;

function TcxCustomCalendarTable.IsValidMonth(AEra: Integer; AYear, AMonth: Cardinal): Boolean;
begin
...
function TcxCustomCalendarTable.ToDateTime(const ADate: TcxDate): TDateTime;
var
...
function TcxCustomCalendarTable.ToDateTime(AYear, AMonth, ADay: Cardinal; AHours, AMinutes, ASeconds: Byte; AMilliseconds: Word): TDateTime;
var
  ADateTime: TcxDateTime;
begin
  ADateTime.Era := -1;
  ADateTime.Year := AYear;
  ADateTime.Month := AMonth;
  ADateTime.Day := ADay;
  ADateTime.Hours := AHours;
  ADateTime.Minutes := AMinutes;
  ADateTime.Seconds := ASeconds;
  ADateTime.Milliseconds := AMilliseconds;
  Result := ToDateTime(ADateTime);
end;

function TcxCustomCalendarTable.GetDayNumber(const S: string): Cardinal;
begin
  if not TryGetDayNumber(S, Result) then
    raise EInvalidArgument.CreateFmt(SInvalidInteger, [S]);
end;

function TcxCustomCalendarTable.GetMonthNumber(AYear: Integer; const S: string): Cardinal;
begin
  Result := GetMonthNumber(AYear, S, FormatSettings);
end;

function TcxCustomCalendarTable.GetMonthNumber(AYear: Integer; const S: string; const AFormat: TFormatSettings): Cardinal;
begin
  if not TryGetMonthNumber(AYear, S, AFormat, Result) then
    raise EInvalidArgument.CreateFmt(SInvalidInteger, [S]);
end;

function TcxCustomCalendarTable.GetYearNumber(const S: string): Cardinal;
begin
  if not TryGetYearNumber(S, Result) then
    raise EInvalidArgument.CreateFmt(SInvalidInteger, [S]);
end;

function TcxCustomCalendarTable.TryGetDayNumber(const S: string; out AValue: Cardinal): Boolean;
begin
  Result := cxTryStrToDateNumber(S, AValue) and InRange(AValue, 1, MaxDayNumber);
end;

function TcxCustomCalendarTable.TryGetMonthNumber(AYear: Integer;
  const S: string; const AFormat: TFormatSettings; out AValue: Cardinal): Boolean;
var
  I: Integer;
begin
  Result := cxTryStrToDateNumber(S, AValue);
  if not Result then
    for I := 1 to 12 do
    begin
      if AnsiSameText(S, AFormat.LongMonthNames[I]) or AnsiSameText(S, AFormat.ShortMonthNames[I]) then
      begin
        AValue := I;
        Exit(True);
      end;
...
function TcxCustomCalendarTable.TryGetYearNumber(const S: string; out AValue: Cardinal): Boolean;
var
  AAge: Integer;
  ARightDigitYear: Integer;
begin
  Result := cxTryStrToDateNumber(S, AValue);
  if Result and (Length(S) <= 2) then
  begin
    if FTwoDigitYearMax = 0 then
      cxGetCalendarInfo(GetThreadLocale, GetCalendarID, CAL_ITWODIGITYEARMAX or CAL_RETURN_NUMBER, nil, 0, @FTwoDigitYearMax);

    AAge := FTwoDigitYearMax div 100;
    ARightDigitYear := FTwoDigitYearMax - AAge * 100;
    if AValue <= Cardinal(ARightDigitYear) then
      Inc(AValue, (AAge * 100))
    else
      Inc(AValue, (AAge - 1) * 100);
  end;
end;
...
class function TdxTimeZoneHelper.AddDSTInfo(var ATimeZoneInfo: TcxTimeZoneInformation; const AYear: Integer; const AStandardDate, ADaylightDate: TSystemTime): Integer;
begin
...
class function TdxTimeZoneHelper.AddDSTInfo(var ATimeZoneInfo: TcxTimeZoneInformation; const AYear: Integer; const AStandardDate, ADaylightDate: TDateTime): Integer;
begin
...
class function TdxTimeZoneHelper.AddDSTInfo(const ATimeZone: Integer; const AYear: Integer; const AStandardDate, ADaylightDate: TDateTime): Integer;
begin
  Result := AddDSTInfo(FTimeZoneInformations[CheckTimeIndex(ATimeZone)],
    AYear, AStandardDate, ADaylightDate);
end;

class function TdxTimeZoneHelper.CalculateTransitionDate(const AYear: Word; const ADSTDateTime: TSystemTime): TDateTime;
var
  ADay: Integer;
  ADaysInMonth: Word;
begin
...
class function TdxTimeZoneHelper.ConvertTimeZoneToTimeZone(const ADateTime: TDateTime; ASourceTimeZone, ADestTimeZone: Integer; AUseDayLightBias: Boolean = True): TDateTime;
begin
  Result := ADateTime + TimeZoneBias(ASourceTimeZone) - TimeZoneBias(ADestTimeZone);
  if AUseDayLightBias then
    Result := Result + (TimeZoneDaylightBias(ADateTime, ASourceTimeZone) - TimeZoneDaylightBias(ADateTime, ADestTimeZone)) * OneMinute;
end;

class function TdxTimeZoneHelper.ConvertToGlobalTime(const ADateTime: TDateTime; ATimeZone: Integer = -1): TDateTime;
begin
  Result := ADateTime + TimeZoneBias(CheckTimeIndex(ATimeZone));
end;

class function TdxTimeZoneHelper.ConvertToLocalTime(const ADateTime: TDateTime; ATimeZone: Integer = -1): TDateTime;
begin
  Result := ADateTime - TimeZoneBias(CheckTimeIndex(ATimeZone));
end;
...
class function TdxTimeZoneHelper.cxTZInfoToTZInfo(const AInfo: TcxTimeZoneInformation): TTimeZoneInformation;
begin
...
class function TdxTimeZoneHelper.TZInfoTocxTZInfo(const AInfo: TTimeZoneInformation): TcxTimeZoneInformation;
begin
  FillChar(Result, SizeOf(Result), 0);
...
class function TdxTimeZoneHelper.IsDaylightDateTime(ATimeZone: Integer; ADateTime: TDateTime): Boolean;
var
  AYear: Word;
  AIndex: Integer;
...
class function TdxTimeZoneHelper.TimeZoneBias(AIndex: Integer; ADate: TDateTime; AUseDayLightBias: Boolean): TDateTime;
var
  ATimeBias: Integer;
begin
  if AIndex = -1 then
    AIndex := CurrentTimeZone;
  ATimeBias := TimeZoneInfo(AIndex).TZI.Bias;
  if AUseDayLightBias then
    ATimeBias := ATimeBias + TimeZoneDaylightBias(ADate, AIndex);
  Result := ATimeBias * OneMinute;
end;
...
class function TdxTimeZoneHelper.TimeZoneDaylightBias(ADateTime: TDateTime; ATimeZone: Integer): Integer;
begin
  Result := 0;
  if ATimeZone < 0 then
...
class procedure TdxTimeZoneHelper.ReadTimeZoneInfoFromRegistry(ARegistry: TRegistry; out AInfo: TcxTimeZoneInformation);
type
...
begin
  if dxDateOf(AValue) = AValue then
    System.SysUtils.DateTimeToString(Result, AIso8601UtcDateFormat, AValue)
  else
    System.SysUtils.DateTimeToString(Result, AIso8601UtcDateTimeFormat, AValue);
end;
...
function dxGetNowInUTC(TrimMilliseconds: Boolean): TDateTime;
var
  AR: DWORD;
  ACurTimeZoneInfo: TTimeZoneInformation;
  Hour, Min, Sec, MSec: Word;
begin
  Result := Now;
  if TrimMilliseconds then
  begin
    DecodeTime(Frac(Result), Hour, Min, Sec, MSec);
    Result := Int(Result) + EncodeTime(Hour, Min, Sec, 0);
  end;

  AR := GetTimeZoneInformation(ACurTimeZoneInfo);
  Result := Result + ACurTimeZoneInfo.Bias / MinsPerDay;
  if AR = TIME_ZONE_ID_DAYLIGHT then
    Result := Result + ACurTimeZoneInfo.DaylightBias / MinsPerDay;
end;
...
function GetDateTimeKind(const AValue: TDateTime): TdxDateTimeKind;
const
  dxEpsilonTime = OneMillisecond / 10;
begin
  if Trunc(AValue) = 0 then
    Result := TdxDateTimeKind.Time
  else
    if Frac(AValue) < dxEpsilonTime then 
      Result := TdxDateTimeKind.Date
    else
      Result := TdxDateTimeKind.DateTime;
end;
...
initialization
  ...
  dxUnitsLoader.AddUnit(SysInit.HInstance, dxThisUnitName, InitializeUtils, FinalizeUtils);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.InitializationFinished(dxThisUnitName, SysInit.HInstance);{$ENDIF}
finalization
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.FinalizationStarted(dxThisUnitName, SysInit.HInstance);{$ENDIF}
  dxUnitsLoader.RemoveUnit(SysInit.HInstance, dxThisUnitName, FinalizeUtils);
