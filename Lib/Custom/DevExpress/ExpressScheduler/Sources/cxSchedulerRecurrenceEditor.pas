...
uses
  ...
  Vcl.ComCtrls, cxLookAndFeelPainters, cxButtons, cxCheckBox, cxRadioGroup,
...
type
  ...
  public
    ...
    property TimeZoneDaylightSaving: Boolean read FTimeZoneDaylightSaving write FTimeZoneDaylightSaving; 
  ...
  TcxSchedulerRecurrenceEventEditorForm = class(TcxSchedulerCustomRecurrenceEventEditor)
    ...
    procedure cbDurationPropertiesValidate(Sender: TcxCustomEdit;  //..
...
uses
  System.Variants, System.DateUtils, cxSchedulerStrs, cxFormats, cxSchedulerDialogs, dxMessageDialog;
...
begin
  Result := not FNeedCheckLossOfExceptions or
    (dxMessageDlg(cxGetResourceString(@scxConfirmLostExceptions), mtWarning, [mbOK, mbCancel], 0) = mrOk);
end;
...
begin
  ...
  SetDateWithoutChangeEvent(deStart, dxDateOf(AStart));
  SetPatternFromEvent;
...
begin
  ...
  begin
    ...
    begin
      dxMessageDlg(cxGetResourceString(@scxInvalidNumber), mtError, [mbOK], 0);
      Text := '1';
    end
    else
    ...
    begin
      case GetRecurrenceInfoValidStatus of
        ...
        rvsReplaceOccurrenceDate:
          if CheckLostExceptions and (dxMessageDlg(Format(cxGetResourceString(@scxReplaceOccurrenceDate),
            [meNumOfDay.Text]), mtWarning, [mbOK,mbCancel], 0) = mrOk) then ModalResult := mrOk;
        rvsInvalidDuration:
          dxMessageDlg(cxGetResourceString(@scxInvalidRecurrenceDuration), mtWarning, [mbOK], 0);
        else
