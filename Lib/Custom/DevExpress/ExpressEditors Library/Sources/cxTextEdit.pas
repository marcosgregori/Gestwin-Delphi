...
uses
  System.UITypes,
  Winapi.Windows, Winapi.Messages, System.Variants, System.Classes, Vcl.Clipbrd, Vcl.Controls, Vcl.Forms, Vcl.Graphics,
  Vcl.Menus, Vcl.StdCtrls, System.SysUtils, System.Generics.Defaults, System.Generics.Collections,
  ...
  dxUIABase, dxUIAClasses, dxUIATextControl, dxUIAListControlProvider, dxAI;
...
type
  ...
  TcxCustomInnerTextEdit = class(TCustomEdit, IUnknown,
    IcxContainerInnerControl, IcxInnerEditHelper, IdxAutomationElement)
  private
  ...
  protected
    ...
    procedure DoSelectItem;
    function GetBorderExtentSize: TSize;
    ...
    function GetScrollBarDefaultAreaSize: TSize;
...
{ TcxCustomTextEditProperties }
  ...
  private
    ...
    FOnGetAICommands: TdxOnGetAICommands;
  ...
  protected
    ...
    function NeedsValidation( DisplayValue : TcxEditValue ) : Boolean; override; //..
  ...
  public
    ...
    //..
    procedure FormatDisplayValue(var DisplayValue: TcxEditValue; AEdit: TcxCustomEdit; AForceFormatting : Boolean = False ); override;
    ...
    property OnGetAICommands: TdxOnGetAICommands read FOnGetAICommands write FOnGetAICommands;
  ...
  TcxCustomTextEdit = class(TcxCustomEdit,
    ...
    IdxAITextControl)
  private type
    TUIACaretState = record
      Caret: Integer;
      Selection: TdxUIATextRange;
      function Equal(const AState: TUIACaretState): Boolean;
    end;
  private
    FAIPopupMenu: HMENU;
    ...
    FText: TCaption;
    FUIATextControlHelper: IdxUIATextControl;
    //
  ...
  protected
    FAICommandsMsgIDs: TDictionary<Cardinal, TdxAICommand>;
    FAICommands: TdxAICommandList;
    ...
    procedure CaretMoved;
    function CreateUIAProvider: IdxUIAProvider; override;
    function CreateUIATextControlHelper: IdxUIATextControl; virtual;
    function GetInnerEditUIAProvider(AAllowCreate: Boolean): IdxUIAProvider; override;
    function GetUIATextControlHelper: IdxUIATextControl;
    procedure UIATextChanged; virtual;
    ...
    procedure CheckRecreateInnerControlOnStyleChanged; override;
    ...
    function NeedRecreateInnerControlOnStyleChanged: Boolean; virtual;
    ...
    //IdxAIControl
    function GetAICommands: TdxAICommandList;

    //IdxAITextControl
    function getText(const AAICommandID: string; ASelectedOnly: Boolean): string; virtual; // GetText conflicts with C++ 64 TControl.GetText
    procedure putText(const AAICommandID: string; const AText: string; AInsertMode: TdxAITextInsertMode); virtual;
    function CanChangeText: Boolean;

    procedure BuildAISubmenu(AMenu: HMENU; const AAICommands: TdxAICommandList);
    procedure ExecuteAIMenuItem(AMessageID: Cardinal);
    function HasAIMenuItem(AMessageID: Cardinal): Boolean;
    procedure PopulateAICommands(AAICommands: TdxAICommandList); virtual;
    procedure ResetPopupMenu; //delete
    function SupportsAI: Boolean; virtual;
    procedure ValidateContextMenu(AMenu: HMENU); virtual;

    property AIPopupMenu: HMENU read FAIPopupMenu;
  ...
  { TcxCustomTextEditUIATextControl }

  TcxCustomTextEditUIATextControl = class(TdxCustomEditUIATextControl) // for internal use
  private
    FControl: TcxCustomTextEdit;
  protected
    function GetPasswordChar: Char; override;
    ...
    property Control: TcxCustomTextEdit read FControl;
  public
    constructor Create(AControl: TcxCustomTextEdit);
  end;
...
uses
  Vcl.Themes, Data.FMTBcd, System.Types, System.Math, dxCharacters, dxCoreGraphics,
  cxEditConsts, cxEditUtils, dxThemeConsts, dxThemeManager, cxDWMApi,
  dxIncrementalFiltering, dxTypeHelpers, dxPopupMenus, dxUIATextControlProvider, System.Character,
  dxUIA, dxDPIAwareUtils, dxAI.Commands.Consts, dxThreading;
...
type
  ...
  { TdxUIAEditListBoxContainerProvider }

  TdxUIAEditListBoxContainerProvider = class(TdxUIACustomListControlContainerProvider<TcxEditListBoxContainer>)
  protected
    function InternalGetFocusable: Boolean; override;
  end;
...
begin
  AViewInfo.TextRect := cxRectContent(AViewInfo.ClientRect, AViewData.GetDrawTextOffset);
  AMeasureTextHeight := TdxTextMeasurer.TextLineHeight(ACanvas.Handle);
  if not AViewInfo.IsInplace and (cxRectHeight(AViewInfo.TextRect) < AMeasureTextHeight) then
    AViewInfo.TextRect.Bottom := Min(AViewInfo.ClientRect.Bottom, AMeasureTextHeight + AViewInfo.TextRect.Top);
end;
...
begin
  ...
  if (not AViewInfo.IsInplace)
    and ((AViewInfo.Painter = nil) or (AViewInfo.Painter.SupportsEditorBorders)) then  
  begin
    if ((AViewInfo.NativeState = TC_NONE) or AViewInfo.DrawSelectionBar) and not AViewInfo.Transparent and (AViewInfo.EditingStyle <> esNoEdit) then
...
function StrToFloatEx(S: string; var AValue: Double): Boolean;
  ...
  {$IFDEF DELPHI103RIO}
    ...
    {$IFDEF DELPHI11}
      if S.EndsWith('e', True {IgnoreCase}) or S.EndsWith('e+', True) or S.EndsWith('e-', True) then
        Result := TextToFloat(S + '0', AResultValue)
      else
...
function TdxUIAEditListBoxContainerProvider.InternalGetFocusable: Boolean;
begin
  Result := False;
end;
...
{$IFDEF DELPHI104}
function TcxCustomInnerTextEdit.IsCustomStyleActive: Boolean;
begin
  Result := False;
end;
{$ENDIF}

function TcxCustomInnerTextEdit.UIAProvider: IdxUIAProvider;
begin
...
procedure TcxCustomInnerTextEdit.BeforeDestruction;
begin
...
function TcxCustomInnerTextEdit.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIAInnerEditProvider.Create(GetContainerUIATextControlHelper);
end;

function TcxCustomInnerTextEdit.GetContainerUIATextControlHelper: IdxUIATextControl;
begin
...
function TcxCustomInnerTextEdit.NeedAccessibilityHelper: Boolean;
begin
...
begin
  ...
  case Message.Msg of
   ...
   WM_UNINITMENUPOPUP:
     begin
       inherited WndProc(Message);
       if Message.WParam = Container.AIPopupMenu then 
         Container.ResetPopupMenu;
     end;
...
var  Cp : TPoint;
begin
     If   Assigned( Container.PopupMenu )
     then begin
          Cp := Point( Width + 2, ( Height div 2 ) - 11 );  // 11 es la mitad de la altura del frame
          Cp := ClientToScreen( Cp );
          Cp := Application.MainForm.ScreenToClient( Cp );
          TPopupMenu( Container.PopupMenu ).Popup( Cp.X, Cp.Y );
          end;
end;
...
begin
  ...
  If   Assigned( Container.PopupMenu )
  then ShowWindow( TPopupMenu( Container.PopupMenu ).Handle, SW_HIDE );
end;
...
procedure TcxCustomInnerTextEdit.UpdateStyleElements;
begin
end;

procedure TcxCustomInnerTextEdit.CaretMoved;
begin
...
function TcxCustomInnerTextEdit.GetAutomationSettings: TdxAutomationElementSettings;
begin
  Result := nil;
end;
...
function TcxCustomInnerTextEdit.GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider;
begin
...
function TcxCustomInnerTextEdit.SupportsUIAutomation: Boolean;
begin
...
procedure TcxCustomInnerTextEdit.WMDestroy(var Message: TWMDestroy);
begin
  if Assigned(FUIAProvider) then
  begin
...
begin
  ...
  Container.UpdateKeyPressed( AKey, AShiftState ); //..
  SaveTextEditState(Helper, True, APrevState);
  FInternalUpdating := False;
  inherited;
...
procedure TcxCustomInnerTextEdit.WMKeyUp(var Message: TWMKeyUp);
begin
  inherited;
  CaretMoved;
end;
...
begin
  ...
  begin
    ...
    ASelStart := TdxDiacriticStringOptions.AnsiTextPos(Edit.LookupData.SearchText, AText) - 1;
...
function TcxEditListBoxContainer.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIAEditListBoxContainerProvider.Create(Self);
end;
...
function TcxCustomTextEditLookupData.GetScrollBarDefaultAreaSize: TSize;
begin
  Result.Init(ListContainer.GetVScrollBarDefaultAreaWidth, ListContainer.GetHScrollBarDefaultAreaHeight);
end;
...
procedure TcxCustomTextEditLookupData.UpdateFilteredLookupItems(const ASearchText: string);
  ...
  begin
    Result := (ASearchText = '') or
      IsLikeTypeFiltering and (Pos(ASearchText, AItemText) <> 0) or
      not IsLikeTypeFiltering and (Pos(ASearchText, AItemText) = 1);
  end;

var
  I: Integer;
  AItem, APattern: string;
begin
  FSearchText := ASearchText;
  APattern := TdxDiacriticStringOptions.Normalize(ASearchText.ToUpper);
  FFilteredLookupItems.Clear;
  for I := 0 to GetItemCount - 1 do
  begin
    AItem := TdxDiacriticStringOptions.Normalize(GetItem(I).ToUpper);
    if IsSuitable(APattern, AItem) then
      FFilteredLookupItems.Add(I);
  end;
end;
  ...
  if L > 0 then
    ...
    begin
      ...
      if IsLikeTypeFiltering and IsFilterActive then
        AFound := TdxDiacriticStringOptions.AnsiTextPos(AText, S) <> 0
      else
...
function TcxCustomTextEditLookupData.GetBorderExtentSize: TSize;
var
  ABorderExtent: TRect;
begin
  ABorderExtent := ListContainer.GetBorderExtent;
  Result.Init(ABorderExtent.Left + ABorderExtent.Right, ABorderExtent.Top + ABorderExtent.Bottom);
end;
...
begin
  Result := Safe.Cast<TcxCustomTextEditProperties>(EditProperties, AProperties) and
...
procedure TcxCustomTextEditProperties.FormatDisplayValue( var DisplayValue     : TcxEditValue;
                                                              AEdit            : TcxCustomEdit;
                                                              AForceFormatting : Boolean = False );

var  PrevDisplayValue : TcxEditValue;

begin
     If   ( FTextEditPad<>tpNone ) and ( Length( VarToStr( DisplayValue ) )<MaxLength ) and ( AEdit.Editing or AForceFormatting )
     then begin
...
begin
     If   IsEditValueNumeric and IsValueBoundsDefined
     then CheckDisplayValueBounds( ADisplayValue, AErrorText, AError, AEdit );
     If   not AError   //..
     then inherited ValidateDisplayValue( ADisplayValue, AErrorText, AError, AEdit );
end;
...
begin
  ...
  if AProperties is TcxCustomTextEditProperties then
    ...
    begin
      ...
      Self.OnGetAICommands := OnGetAICommands;
...
function TcxCustomTextEdit.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIACustomTextEditProvider.Create(GetUIATextControlHelper);
end;

function TcxCustomTextEdit.CreateUIATextControlHelper: IdxUIATextControl;
begin
  Result := TcxCustomTextEditUIATextControl.Create(Self);
end;
...
function TcxCustomTextEdit.getText(const AAICommandID: string; ASelectedOnly: Boolean): string;
begin
...
procedure TcxCustomTextEdit.CheckRecreateInnerControlOnStyleChanged;
begin
  if HasInnerEdit and not IsDestroying and not FIsCreating and NeedRecreateInnerControlOnStyleChanged then
    cxRecreateControlWnd(InnerControl);
end;

function TcxCustomTextEdit.NeedRecreateInnerControlOnStyleChanged: Boolean;
begin
  Result := InnerControl.HandleAllocated and (ActiveProperties.PasswordChar = #0) and (ActiveProperties.EchoMode = eemPassword);
end;
...
procedure TcxCustomTextEdit.PopulateAICommands(AAICommands: TdxAICommandList);
begin
  AAICommands.Add([
...
procedure TcxCustomTextEdit.putText(const AAICommandID: string; const AText: string; AInsertMode: TdxAITextInsertMode);
var
  AInnerTextEdit: TcxCustomInnerTextEdit;
begin
  AInnerTextEdit := TcxCustomInnerTextEdit(InnerControl);
  case AInsertMode of
    TdxAITextInsertMode.Above:
      begin
        SelLength := 0;
        AInnerTextEdit.Perform(EM_REPLACESEL, 0, PCHAR(AText + #13#10));
      end;
    TdxAITextInsertMode.Below:
      begin
        SelStart := SelStart + SelLength;
        AInnerTextEdit.Perform(EM_REPLACESEL, 0, PCHAR(#13#10 + AText));
      end;
...
function TcxCustomTextEdit.CanChangeText: Boolean;
begin
  Result := not RealReadOnly;
end;
...
procedure TcxCustomTextEdit.CaretMoved;
begin
  dxUIARaiseEvent(GetInnerEditUIAProvider(False), UIA_Text_TextSelectionChangedEventId);
  dxUIARaiseEvent(FUIAProvider, UIA_Text_TextSelectionChangedEventId, UIA_TextPatternId);
end;
...
function TcxCustomTextEdit.GetInnerEditUIAProvider(AAllowCreate: Boolean): IdxUIAProvider;
begin
  if Assigned(Unsafe.Cast<TcxCustomInnerTextEdit>(InnerControl).FUIAProvider) or AAllowCreate then
    Result := Unsafe.Cast<TcxCustomInnerTextEdit>(InnerControl).UIAProvider
  else
    Result := nil;
end;
...
function TcxCustomTextEdit.GetAICommands: TdxAICommandList;
begin
  if FAICommands = nil then
    FAICommands := TdxAICommandList.Create
  else
    FAICommands.Clear;
  PopulateAICommands(FAICommands);
  if Assigned(ActiveProperties.FOnGetAICommands) then
    ActiveProperties.FOnGetAICommands(Self, FAICommands);
  Result := FAICommands;
end;

procedure AddMenuItem(AMenu: HMENU; const ACaption: string; AMenuCommand: Cardinal; AEnabled: Boolean = True);
var
  AInfo: TMenuItemInfo;
begin
  ZeroMemory(@AInfo, SizeOf(TMenuItemInfo));
  AInfo.cbSize := SizeOf(TMenuItemInfo);
  AInfo.fMask := MIIM_ID or MIIM_STRING or MIIM_FTYPE; 
  if not AEnabled then
  begin
    AInfo.fMask := AInfo.fMask or MIIM_STATE;
    AInfo.fState := MFS_DISABLED;
  end;
  AInfo.dwTypeData := @ACaption[1];
  AInfo.cch := Length(ACaption);
  AInfo.wID := AMenuCommand;
  InsertMenuItem(AMenu, GetMenuItemCount(AMenu), True, AInfo);
end;

procedure AddMenuSeparator(AMenu: HMENU);
var
  AInfo: TMenuItemInfo;
begin
  ZeroMemory(@AInfo, SizeOf(TMenuItemInfo));
  AInfo.cbSize := SizeOf(TMenuItemInfo);
  AInfo.fMask := MIIM_FTYPE;
  AInfo.fType := MFT_SEPARATOR;
  InsertMenuItem(AMenu, GetMenuItemCount(AMenu), True, AInfo);
end;

function AddSubMenu(AMenu: HMENU; const ACaption: string; AEnabled: Boolean = True): HMENU;
var
  AInfo: TMenuItemInfo;
begin
  ...
  ZeroMemory(@AInfo, SizeOf(TMenuItemInfo));
  AInfo.cbSize := SizeOf(TMenuItemInfo);
  AInfo.fMask := MIIM_STRING or MIIM_SUBMENU or MIIM_FTYPE; 
  if not AEnabled then
  begin
    AInfo.fMask := AInfo.fMask or MIIM_STATE;
    AInfo.fState := MFS_DISABLED;
  ...
  AInfo.dwTypeData := @ACaption[1];
  AInfo.hSubMenu := Result;
  AInfo.cch := Length(ACaption);
  InsertMenuItem(AMenu, GetMenuItemCount(AMenu), True, AInfo);
end;

procedure TcxCustomTextEdit.BuildAISubmenu(AMenu: HMENU; const AAICommands: TdxAICommandList);
var
  I: Integer;
  AMessageID: Cardinal;
  AAICommand: TdxAICommand;
  ASubMenuItems: TDictionary<string, HMENU>;
  AParentMenuItem: HMENU;
  AAICommandInfo: TdxAICommandList.TCommand;
  AHasText: Boolean;
begin
  if not SupportsAI or (AAICommands.Count = 0) then
    Exit;

  AddMenuSeparator(AMenu);

  FAIPopupMenu := AddSubMenu(AMenu, cxGetResourceString(@dxSAIAssistantCaption), True);

  AMessageID := DXM_POPUPMENUAICOMMANDFIRST;
  FreeAndNil(FAICommandsMsgIDs);
  FAICommandsMsgIDs := TDictionary<Cardinal, TdxAICommand>.Create;
  ASubMenuItems := TDictionary<string, HMENU>.Create;
  try
    ...
    begin
      ...
      begin
        ...
        begin
          AParentMenuItem := AddSubMenu(FAIPopupMenu, AAICommand.Category, AHasText);
          ASubMenuItems.Add(AAICommand.Category, AParentMenuItem);
        end;
      end
      else
        AParentMenuItem := FAIPopupMenu;
...
procedure TcxCustomTextEdit.ExecuteAIMenuItem(AMessageID: Cardinal);
var
  AAICommand: TdxAICommand;
  AControl: IdxAITextControl;
begin
  if TdxAIChatClients.HasChatClient and Supports(Self, IdxAITextControl, AControl)
    and FAICommandsMsgIDs.TryGetValue(AMessageID, AAICommand) then
    AAICommand.Execute(AControl);
  FreeAndNil(FAICommandsMsgIDs);
end;

function TcxCustomTextEdit.HasAIMenuItem(AMessageID: Cardinal): Boolean;
begin
  Result := FAICommandsMsgIDs.ContainsKey(AMessageID);
end;

procedure TcxCustomTextEdit.ResetPopupMenu;
begin
  FAIPopupMenu := 0;
end;

function TcxCustomTextEdit.SupportsAI: Boolean;
begin
  Result := False;
end;

procedure TcxCustomTextEdit.ValidateContextMenu(AMenu: HMENU);
begin
  BuildAISubmenu(AMenu, GetAICommands);
end;
...
function TcxCustomTextEdit.GetUIATextControlHelper: IdxUIATextControl;
begin
  if not Assigned(FUIATextControlHelper) then
    FUIATextControlHelper := CreateUIATextControlHelper;
  Result := FUIATextControlHelper;
end;
...
procedure TcxCustomTextEdit.UIATextChanged;
begin
  if HandleAllocated then
  begin
    dxUIARaiseEvent(GetInnerEditUIAProvider(False), UIA_Text_TextChangedEventId);
    dxUIARaiseEvent(FUIAProvider, UIA_Text_TextChangedEventId, UIA_TextPatternId);
  end;
end;
...
function TcxTextEdit.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;
...
constructor TcxCustomTextEditUIATextControl.Create(AControl: TcxCustomTextEdit);
begin
  inherited Create(AControl.InnerControl as TCustomEdit);
  FControl := AControl;
end;

function TcxCustomTextEditUIATextControl.GetFocused: Boolean;
begin
  Result := FControl.Focused;
end;

function TcxCustomTextEditUIATextControl.GetInnerControl: TWinControl;
begin
  Result := FControl.InnerControl;
end;

function TcxCustomTextEditUIATextControl.GetInstance: TWinControl;
begin
  Result := FControl;
end;

function TcxCustomTextEditUIATextControl.GetPasswordChar: Char;
begin
  Result := GetWindowsPasswordChar(FControl.LookAndFeel.NativeStyle, FControl.ActiveProperties.PasswordChar);
end;

function TcxCustomTextEditUIATextControl.GetTextWindow: TWinControl;
begin
  Result := FControl.InnerControl;
end;

function TcxCustomTextEditUIATextControl.IsPassword: Boolean;
begin
  Result := FControl.ActiveProperties.EchoMode = eemPassword;
end;

{ TcxCustomTextEdit.TUIACaretState }

function TcxCustomTextEdit.TUIACaretState.Equal(
  const AState: TUIACaretState): Boolean;
begin
  Result := (Caret = AState.Caret) and (Selection = AState.Selection);
end;
