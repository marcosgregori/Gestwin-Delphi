...
uses
  System.UITypes,
  System.Types, Winapi.Windows, Winapi.Messages, System.Variants, Vcl.Forms, Vcl.ComCtrls, System.Classes, Vcl.Controls, Vcl.Graphics, System.Generics.Defaults, System.Generics.Collections,
  Vcl.Menus, Vcl.StdCtrls, System.SysUtils, Vcl.ActnList, Vcl.ImgList,
  ...
  cxAccessibility, dxUIAClasses, dxUIABase;
...
{$SCOPEDENUMS ON}
  TcxEditButtonMode = (Button, Glyph);
  TcxEditButtonHotTrackMode = (Default, Editor, None);
{$SCOPEDENUMS OFF}
...
type
  ...
  TcxEditValidationOptions = set of (evoRaiseException, evoShowErrorIcon, evoAllowLoseFocus, evoValidateAlways);  //..
  ...
  TcxEditButtonViewInfoData = record
    ...
    Mode: TcxEditButtonMode;
    HotTrackMode: TcxEditButtonHotTrackMode;
    TransparentBackground: Boolean;
    DrawState: TcxEditButtonState;
    RawState: TcxEditButtonState;
  end;
  ...
  public
    ...
    //
    property EditOwner: IcxEditOwner read GetEditOwner; // for internal use
  ...
  public
    ...
    property ViewInfo: TcxEditButtonViewInfo read FViewInfo; // for internal use
  end;

  { TdxEditChildAccessibilityHelper }

  TdxEditChildAccessibilityHelper = class(TcxAccessibilityHelper)
  protected
    FLocalId: Integer;
  ...
  TcxEditButtonViewInfo = class(TInterfacedPersistent, IdxAutomationElement)
  strict private
  ...
  protected
    ...
    function SupportsUIAutomation: Boolean;
    function GetAutomationSettings: TdxAutomationElementSettings;
    function GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider;
    //
  ...
  public
    ...
    Data: TcxEditButtonViewInfoData; // for internal use
    EditViewInfo: TcxCustomEditViewInfo;
    FadingHelper: TcxEditButtonFadingHelper; // for internal use
    Glyph: TdxSmartGlyph;
  ...
  private
    ...
    procedure SetHotTrackMode(Value: TcxEditButtonHotTrackMode);
    ...
    procedure SetMode(Value: TcxEditButtonMode);
  ...
  protected
    ...
    function GetAutomationSettingsClass: TdxAutomationElementSettingsClass; virtual;
    ...
    property HotTrackMode: TcxEditButtonHotTrackMode read FHotTrackMode write SetHotTrackMode default TcxEditButtonHotTrackMode.Default;
    ...
    property Mode: TcxEditButtonMode read FMode write SetMode default TcxEditButtonMode.Button;
  ...
  public
    destructor Destroy; override;
    function HasErrorIcon: Boolean; inline;
  ...
  IdxInplaceEditParentBackgroundProvider = interface
    ['{A4537E0B-97D6-4D7C-98AB-CA2C019BDCBD}']
    procedure DrawParentBackground(ACanvas: TcxCanvas; const AViewInfo: TcxCustomEditViewInfo);
    function UseParentBackground: Boolean; 
  end;

  TcxCustomEditViewInfo = class(TcxContainerViewInfo)
  strict private type
  {$REGION 'strict private type'}
    TAfterDrawEvent = procedure(Sender: TcxCustomEditViewInfo; ACanvas: TcxCanvas) of object;
  ...
  private
    ...
    FOnAfterDrawBackground: TAfterDrawEvent;
    FOnAfterDrawValue: TAfterDrawEvent;
    FOnCalculateEditorBounds: TCalculateEditorBoundsEvent;
    FOnCanDrawEditValue: TCanDrawEditValueEvent;
  ...
  protected
    ...
    function CalculateBackgroundColor(AStyle: TcxContainerStyle; AEnabled: Boolean; AContainer: TcxContainer = nil): TColor; override;
    ...
    function GetActualUnscaledPadding(AHorzAlignment: TAlignment): TdxPadding;
    ...
    function GetDrawReadOnly: Boolean;
    ...
    function ShouldUseStyleColorPalette(AStyle: TcxContainerStyle): Boolean; override;
    ...
    procedure DrawComplexFrameEx(ACanvas: TcxCanvas; var ARect: TRect; ALeftTopColor, ARightBottomColor: TColor); virtual;
    ...
    procedure FrameRectEx(ACanvas: TcxCanvas; var ARect: TRect; AColor: TColor); virtual;
  ...
  public
    ...
    Focused: Boolean;
    Font: TFont;
    ForceDrawAsStandalone: Boolean; // for internal use
    HasBackground: Boolean;
    ...
    ParentBackgroundProvider: TObject; // for internal use
    ...
    Transparent: Boolean;
    WindowHandle: HWND;
    UseInplacePadding: Boolean; // for internal use
    ...
    procedure UpdateStyle(AStyle: TcxContainerStyle); override;

    // Draw
    function CanDrawEditValue: Boolean; virtual;
    function DrawAsInplace: Boolean; inline; // for internal use
    ...
    procedure DrawBorderedBackground(ACanvas: TcxCanvas; const R: TRect); virtual; 
    ...
    function GetUnscaledPadding: TdxPadding; virtual; // for internal use //#lcm: WXI
    function HasPadding: Boolean; virtual; // for internal use
    function PainterPaddingAvailable: Boolean; virtual; // for internal use //#lcm: WXI
    function SupportsBorders: Boolean; virtual; // for internal use //#lcm: WXI
    function SupportsRoundedContainerBorders: Boolean; virtual; // for internal use //#lcm: WXI
    ...
    //
    property OnAfterDrawBackground: TAfterDrawEvent read FOnAfterDrawBackground write FOnAfterDrawBackground;
    property OnAfterDrawValue: TAfterDrawEvent read FOnAfterDrawValue write FOnAfterDrawValue;
    property OnCalculateEditorBounds: TCalculateEditorBoundsEvent read FOnCalculateEditorBounds write FOnCalculateEditorBounds;
    property OnCanDrawEditValue: TCanDrawEditValueEvent read FOnCanDrawEditValue write FOnCanDrawEditValue;
    //
  ...
  TcxEditSizeProperties = record
    ...
    procedure InitValues(AMaxLineCount, AHeight, AWidth: Integer; AScaleFactor: TdxScaleFactor = nil);
  end;
  ...
  protected
    ...
    //
    procedure AdjustPadding(var APadding: TdxPadding); virtual;
    function CalculatePaintOptions: TcxEditPaintOptions; virtual;
    function CalculateReadOnly: Boolean; virtual;
    function CalculateStyleReadOnly: Boolean;
    function CalculateDefaultReadOnly: Boolean;
    ...
    function GetDefaultContainerState(const ABounds: TRect; const P: TPoint;
      Button: TcxMouseButton; Shift: TShiftState; AIsMouseEvent: Boolean): TcxContainerState;
  ...
  public
    ...
    ClientExtent: TRect; // for internal use
    ContainerState: TcxContainerState;
    ContentOffset: TRect;
    EditContentParams: TcxEditContentParams; // for internal use
    Enabled: Boolean;
    Focused: Boolean;
    ForceDrawAsStandalone: Boolean; // for internal use
    HorzAlignment: TcxEditHorzAlignment;
    HScrollBar: IcxControlScrollBar; // for internal use
    InnerEdit: IcxCustomInnerEdit; // for internal use
    ...
    PreviewMode: Boolean;
    RefinementsTuner: IdxVisualRefinementsTuner; // for internal use
    Selected: Boolean;
    ...
    VScrollBar: IcxControlScrollBar; // for internal use
    WindowHandle: HWND;
    ...
    function IgnorePainterBorderSettings: Boolean; virtual;
    ...
    property ScaleFactor: TdxScaleFactor read FScaleFactor;
    property Style: TcxCustomEditStyle read FStyle write FStyle;
  ...
  strict private
    ...
    function GetStyleReadOnly: TcxEditStyle;
    ...
    procedure SetStyleReadOnly(Value: TcxEditStyle);
  protected
  ...
  published
    ...
    property StyleReadOnly: TcxEditStyle read GetStyleReadOnly write SetStyleReadOnly;
...
//    property GradientButtons;
  ...
  private
    ...
    FOnButtonGlyphDrawParameters: TcxEditButtonGlyphDrawParametersEvent;
    FOnChange: TNotifyEvent;
    ...
    procedure SetDefaultValuesProvider(Value: TcxCustomEditDefaultValuesProvider);
  ...
  protected
    ...
    // IdxFreeNotify
    procedure DestroyFreeNotificator;
    ...
    function GetAutomationSettingsClass: TdxAutomationElementSettingsClass; virtual;
    function GetButtonIndexByVisibleIndex(AButtonVisibleIndex: Integer): Integer;
    ...
    function IsMinValueStored: Boolean;
    procedure InitiateActions; virtual;
    function SupportsMultiThreading(AUseDisplayValue: Boolean): Boolean; virtual;
	function NeedsValidation( DisplayValue : TcxEditValue ) : Boolean; virtual; //..	
    class function ShowAtDesignTime: Boolean; virtual;  
  ...
  public
    ...
    function IsAutomationUnitNeededInUses: Boolean; virtual; // for internal use
    ...
    procedure FormatDisplayValue(var DisplayValue: TcxEditValue; AEdit: TcxCustomEdit; AForceFormatting : Boolean = False ); virtual; //..
    ...
    function CreateViewData(AStyle: TcxCustomEditStyle; AIsInplace: Boolean; APreviewMode: Boolean = False): TcxCustomEditViewData; overload; virtual; {$IF CompilerVersion > 28}deprecated;{$IFEND}
    function CreateViewData(AStyle: TcxCustomEditStyle; AIsInplace: Boolean; AScaleFactor: TdxScaleFactor;
      APreviewMode: Boolean = False): TcxCustomEditViewData; overload; virtual;
    ...
    property DefaultValuesProvider: TcxCustomEditDefaultValuesProvider read FDefaultValuesProvider write SetDefaultValuesProvider; // for internal use
    ...
    property OnButtonGlyphDrawParameters: TcxEditButtonGlyphDrawParametersEvent read FOnButtonGlyphDrawParameters write FOnButtonGlyphDrawParameters;
  ...
  strict private
    FDefaultValuesProvider: TcxCustomEditDefaultValuesProvider;
    FOnExecuteAction: TActionEvent;
    FOnUpdateAction: TActionEvent;
  protected
    FEdit: TcxCustomEdit;
    ...
    property Edit: TcxCustomEdit read FEdit;
    property OnExecuteAction: TActionEvent read FOnExecuteAction write FOnExecuteAction;
    property OnUpdateAction: TActionEvent read FOnUpdateAction write FOnUpdateAction;
  public
    ...
    property DefaultValuesProvider: TcxCustomEditDefaultValuesProvider read FDefaultValuesProvider; // for internal use
  ...
  protected
    // UI Automation
    function GetInnerEditUIAProvider(AAllowCreate: Boolean): IdxUIAProvider; virtual;
  ...
  TdxCachedSize = record // for internal use
    Hash: Integer;
    Size: TSize;
  end;

  { TdxEditAccessibilityHelper }

  TdxEditAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
  ...
  protected
    ...
    procedure AddAccessibilityChild(AChild: IcxAccessibilityHelper);
    procedure DoButtonClick(AVisibleIndex: Integer); virtual;
    function GetButtonName(AIndex: Integer): string; virtual;

    property AccessibilityChildren: TList read FAccessibilityChildren;
    property Edit: TcxCustomEdit read GetEdit;
  ...
  TdxEditAccessibilityHelperClass = class of TdxEditAccessibilityHelper;
  ...
  private
    ...
    function GetIAccessibilityHelper: IcxAccessibilityHelper;
    function GetIInnerEditAccessibilityHelper: IcxAccessibilityHelper;
    function GetInternalStyle(AState: TcxContainerStateItem): TcxCustomEditStyle;
    ...
    procedure SetModifiedOnEnter(Value: Boolean); //..
  ...
  protected
    ...
    procedure VisibleChanging; override; //..
       ...
       // IdxAutomationElement
    function GetAutomationSettings: TdxAutomationElementSettings; override;
    // IdxAutomationElementWithRepositoryItem
    function GetRepositoryAutomationSettings: TdxAutomationElementSettings;
    // TWinControl
    ...
    procedure DoButtonAction(AButtonVisibleIndex: Integer); virtual;
    ...
    function HandleMouseWheel(Shift: TShiftState): Boolean; virtual;
    procedure HidePopupWindowOnScroll; virtual;
    function NeedAccessibilityHelper: Boolean; virtual;
    procedure PopulateSizeProperties(var AEditSizeProperties: TcxEditSizeProperties); virtual;
    procedure InitializeNonShareableProperties; virtual;
    procedure DisconnectNonShareableProperties; virtual;

    // initialize & prepare
    procedure Initialize; virtual;
    procedure InitContentParams;
    procedure InitializeEditData; virtual;
    procedure InitializeInfoForHiddenInplaceEditor(AEditViewData: TcxCustomEditViewData;
      AEditViewInfo: TcxCustomEditViewInfo); virtual;
    ...
    procedure UpdateInfoForHiddenInplaceEditor(AViewData: TcxCustomEditViewData; AEditViewInfo: TcxCustomEditViewInfo); virtual;

    // internal
    procedure InternalCanResize(var ANewWidth, ANewHeight: Integer); virtual; // for internal use
    ...
    // validation
    function IsOnValidateEventAssigned: Boolean; virtual;
    function CanShowValidationErrorOnPostEditValue: Boolean; virtual;
    procedure DisableValidate;
    procedure DoOnValidate(var ADisplayValue: TcxEditValue; var AErrorText: TCaption; var AError: Boolean); virtual;
    ...
    function GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass; virtual;
    ...
    // property DataBinding: TcxEditDataBinding read FDataBinding write SetDataBinding;  // Trasladado
    ...
    property IInnerEditAccessibilityHelper: IcxAccessibilityHelper read GetIInnerEditAccessibilityHelper;
    // property IsEditValidated: Boolean read FIsEditValidated write FIsEditValidated;  // Trasladado
    ...
    // property Properties: TcxCustomEditProperties read FProperties write SetProperties;  // Trasladado
  ...
  public
    ContentParams: TcxEditContentParams; // for internal use
    ...
    procedure DoPostEditValueNotification; //..
    ...
    function GetUserValidateAction( out UserIsCancelingForm : Boolean ) : Boolean; overload; //..
    function GetUserValidateAction : Boolean; overload; //..
    procedure UpdateKeyPressed( Key : Word; Shift : TShiftState ); //..
    procedure ClearAndPost; //..
    procedure PrepareEditValue(const ADisplayValue: TcxEditValue;
      out AEditValue: TcxEditValue; AEditFocused: Boolean); virtual;
    procedure Reset;
    ...
    property DisplayingValue: TcxEditValue read GetDisplayValue; //..
    ...
    property ExternalPostingValue : Boolean read FExternalPostingValue; //..
    property Editing: Boolean read GetEditing; //..
    property IsEditValidated: Boolean read FIsEditValidated write FIsEditValidated; //..
    property IsPosting: Boolean read FIsPosting;
    property IsAKeyPressed: Boolean read FIsKeyPressed write FIsKeyPressed; //..
    property KeyPressed : Word read FKeyPressed; //..
    ...
    property ModifiedOnEnter: Boolean read FModifiedOnEnter write SetModifiedOnEnter; //..
    property Properties: TcxCustomEditProperties read FProperties write SetProperties; //..
    property Style: TcxEditStyle read GetStyle write SetStyle;
    property StyleDisabled: TcxEditStyle read GetStyleDisabled write SetStyleDisabled;
    property StyleFocused: TcxEditStyle read GetStyleFocused write SetStyleFocused;
  ...
  private
    ...
    function FindItem(AProperties: TcxCustomEditProperties; ACanUseFreeEditors: Boolean): Integer; overload;
  ...
  private
    ...
    FHideEditOnScrollContentCount: Integer;
  ...
  protected
    ...
    procedure DoHideEditOnScrollContent; virtual;
    procedure DoShowEditAfterScrollContent; virtual;
    ...
    function HasHiddenEditor: Boolean;
    procedure HideEditOnScrollContent;
    procedure HideInplaceEditor; virtual;
    procedure InitEdit; virtual;
    function IsEditVisible: Boolean;
    function IsHiddenOnScrolling: Boolean;
    ...
    procedure RecalculateEditViewInfo; virtual;
    procedure ShowEditAfterScrollContent;
    ...
    function SupportsPadding: Boolean; virtual;
    ...
    procedure EditExecuteAction(Action: TBasicAction; var Handled: Boolean); virtual;
    ...
    procedure EditUpdateAction(Action: TBasicAction; var Handled: Boolean); virtual;
    ...
    property EditingItemSetting: Boolean read FEditingItemSetting write FEditingItemSetting;
...
const
  cxDefaultEditSizeProperties: TcxEditSizeProperties =    (MaxLineCount: 0; Height: -1; Width: -1; ScaleFactor: nil;);
  cxSingleLineEditSizeProperties: TcxEditSizeProperties = (MaxLineCount: 1; Height: -1; Width: -1; ScaleFactor: nil;);


var
  FEditControlNotification : Integer = 0; //..
...
procedure cxCalculateTextSelectionColors(var ATextSelectionColor, ATextSelectionBackgroundColor: TColor; ABackgroundColor: TColor);
...
procedure RegisterEditPropertiesIcon(AEditProperties: TClass; const AnIconName: string; AModuleInstance: HINST); // For internal use
function LoadEditorComponentIcon(const AName: string; AGlyph: TdxSmartGlyph; ATargetDPI: Integer): string; // For internal use

procedure ClearEditControlKeyPressed; //..
...
uses
  System.TypInfo, cxDateUtils, cxEditConsts, cxEditUtils, cxFilterConsts, dxOffice11,
  dxThemeConsts, cxDWMApi, System.Math, cxLibraryConsts, dxHash, dxTypeHelpers, dxGenerics,
  dxUIATextControlProvider;

const
  dxThisUnitName = 'cxEdit';
...
var
  ...
  FcxEditErrorIcon: TdxSmartGlyph;
  FcxEditWarningIcon: TdxSmartGlyph;
  FcxEditInfoIcon: TdxSmartGlyph;
  FEditControlKeyPressed : TcxCustomEdit;  //..  El componente sobre el que se ha pulsado la última tecla y su valor

type
  TdxEditButtonAccessibilityHelper = class(TdxEditChildAccessibilityHelper)
  strict private
    function GetEditButtonViewInfo: TcxEditButtonViewInfo;
  protected
    ...
    property EditButtonViewInfo: TcxEditButtonViewInfo read GetEditButtonViewInfo;
  public
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect; override;
  end;

  TEditPropertyIconInfo = record
    EditPropertyClassName: string;
    IconName: string;
    ModuleInstance: HINST;
  end;

  TEditPropertyIconList = class(TList<TEditPropertyIconInfo>)
  public
    constructor Create;
    function GetIconModuleInstance(AnIconName: string): HINST;
  end;

constructor TEditPropertyIconList.Create;
begin
  inherited Create{$IFDEF DELPHI12}(IComparer<TEditPropertyIconInfo>(TdxComparer<TEditPropertyIconInfo>.Default)){$ENDIF};
end;

function TEditPropertyIconList.GetIconModuleInstance(AnIconName: string): HINST;
var
  AInfo: TEditPropertyIconInfo;
begin
  Result := 0;
  for AInfo in Self do
    if (AInfo.IconName = AnIconName) and (AInfo.ModuleInstance <> 0) then
      Exit(AInfo.ModuleInstance);
end;

var
  FEditPropertyIcons: TEditPropertyIconList;

procedure RegisterEditPropertiesIcon(AEditProperties: TClass; const AnIconName: string; AModuleInstance: HINST);
var
  AInfo: TEditPropertyIconInfo;
begin
  if not Assigned(FEditPropertyIcons) then
    FEditPropertyIcons := TEditPropertyIconList.Create;
  AInfo.EditPropertyClassName := AEditProperties.ClassName;
  AInfo.IconName := AnIconName;
  if AModuleInstance = 0 then
    AInfo.ModuleInstance := FEditPropertyIcons.GetIconModuleInstance(AnIconName)
  else
    AInfo.ModuleInstance := AModuleInstance;
  FEditPropertyIcons.Add(AInfo);
end;

function LoadEditorComponentIcon(const AName: string; AGlyph: TdxSmartGlyph; ATargetDPI: Integer): string;
var
  AInfo: TEditPropertyIconInfo;
begin
  for AInfo in FEditPropertyIcons do
    if AInfo.EditPropertyClassName = AName then
    begin
      if ATargetDPI <= 96 then
      begin
        AGlyph.LoadFromResource(AInfo.ModuleInstance, AInfo.IconName + '16', RT_BITMAP);
        AGlyph.SourceDPI := 96;
      end
      else if ATargetDPI <= 144 then
      begin
        AGlyph.LoadFromResource(AInfo.ModuleInstance, AInfo.IconName, RT_BITMAP); // 24 px
        AGlyph.SourceDPI := 144;
      end
      else
      begin
        AGlyph.LoadFromResource(AInfo.ModuleInstance, AInfo.IconName + '32', RT_BITMAP); // 24 px
        AGlyph.SourceDPI := 196;
      end;
      Exit;
    end;
end;
...
begin
  if FInplaceEditLists <> nil then
    for I := 0 to FInplaceEditLists.Count - 1 do
      TcxInplaceEditList(FInplaceEditLists.List[I]).DisconnectProperties(AProperties);
end;
...
procedure cxCalculateTextSelectionColors(var ATextSelectionColor, ATextSelectionBackgroundColor: TColor; ABackgroundColor: TColor);
begin
  if ATextSelectionBackgroundColor = clDefault then
  begin
    if ATextSelectionColor = clDefault then
    begin
      if ABackgroundColor = clHighlight then
      begin
        ATextSelectionBackgroundColor := clBlack;
        ATextSelectionColor := clWhite;
      end
      else
      begin
        ATextSelectionBackgroundColor := clHighlight;
        ATextSelectionColor := clHighlightText;
      end;
    end
    else
      ATextSelectionBackgroundColor := dxInvertColor(ATextSelectionColor);
  end
  else
    if ATextSelectionColor = clDefault then
      ATextSelectionColor := dxInvertColor(ATextSelectionBackgroundColor)
end;
...
begin
  ...
  FAutomation := GetAutomationSettingsClass.Create(Self);
  FAutoSelect := True;
  ...
  FInnerAlignment := CreateAlignment;
...
function TcxCustomEditProperties.CreateViewData(AStyle: TcxCustomEditStyle; AIsInplace: Boolean; AScaleFactor: TdxScaleFactor;
  APreviewMode: Boolean): TcxCustomEditViewData;
begin
  Result := GetViewDataClass.Create(Self, AStyle, AIsInplace);
  Result.PreviewMode := APreviewMode;
  if Assigned(AScaleFactor) then
    Result.ScaleFactor.Assign(AScaleFactor);
end;
...
begin
  Result := CreateViewData(AStyle, AIsInplace, dxDefaultScaleFactor, APreviewMode)
end;
...
begin
  AViewData := TcxCustomEditViewData(CreateViewData(AEditStyle, AIsInplace, AEditSizeProperties.ScaleFactor));
  try
    Result := AViewData.GetEditConstantPartSize(ACanvas, AEditSizeProperties, MinContentSize);
  finally
...
function TcxCustomEditProperties.IsEditValueValid(var AEditValue: TcxEditValue; AEditFocused: Boolean): Boolean;
begin
  Result := True;
end;
...
//..
procedure TcxCustomEditProperties.FormatDisplayValue(var DisplayValue: TcxEditValue; AEdit: TcxCustomEdit; AForceFormatting : Boolean = False );
begin
end;
//..
...
procedure TcxCustomEditProperties.DestroyFreeNotificator;
begin
  FreeAndNil(FFreeNotificator);
end;
...
begin
  ...
  begin
    Result := not ReadOnly;
    if Result and (DefaultValuesProvider <> nil) then
      Result := DefaultValuesProvider.DefaultCanModify and DefaultValuesProvider.CanSetEditMode;
  end;
end;
...
function TcxCustomEditProperties.CreateAlignment: TcxEditAlignment;
begin
  Result := TcxEditAlignment.Create(Self, GetDefaultVertAlignment);
end;
...
function TcxCustomEditProperties.GetAutomationSettingsClass: TdxAutomationElementSettingsClass;
begin
  Result := TdxAutomationElementSettings;
end;

function TcxCustomEditProperties.GetDefaultHorzAlignment: TAlignment;
begin
  if DefaultValuesProvider <> nil then
    Result := DefaultValuesProvider.DefaultAlignment
  else
...
begin
  if AssignedValues.MaxValue then
    ...
    if DefaultValuesProvider = nil then
      ...
      Result := DefaultValuesProvider.DefaultMaxValue;
end;
...
begin
  if AssignedValues.MinValue then
    ...
    if DefaultValuesProvider = nil then
      ...
      Result := DefaultValuesProvider.DefaultMinValue;
end;
...
begin
  Result := evsValue;
  if not AEditFocused and (dfoSupports in DisplayFormatOptions) and (DefaultValuesProvider <> nil) and
      DefaultValuesProvider.IsDisplayFormatDefined(not(dfoNoCurrencyValue in DisplayFormatOptions)) then
    Result := evsText;
end;
...
begin
  Result := not IsValueFormattedByProperties and (dfoSupports in DisplayFormatOptions) and
    ...
    (DefaultValuesProvider <> nil) and
    DefaultValuesProvider.IsDisplayFormatDefined(not(dfoNoCurrencyValue in DisplayFormatOptions));
end;
...
procedure TcxCustomEditProperties.DeleteLinks;
begin
  if Assigned(Images) and Assigned(FImageChangeLink) then
...
begin
  ...
  Automation := AProperties.Automation;
  AutoSelect := AProperties.AutoSelect;
  ScaleFactor.Assign(AProperties.ScaleFactor);
  ...
  FDefaultValuesProvider := AProperties.FDefaultValuesProvider;

  FInnerAlignment.Assign(AProperties.FInnerAlignment);
  ...
  OnButtonGlyphDrawParameters := AProperties.OnButtonGlyphDrawParameters;
...
begin
  ...
  APrevDisplayValue := ADisplayValue;  //.. Considero el pad como parte de la introducción del usuario.
  ...
  begin
    ...
    begin
      ...
      if not AIsUserErrorDisplayValue then
        ADisplayValue := APrevDisplayValue; //.. AEdit.DisplayValue;
    end;
  end;
end;
...
function TcxCustomEditProperties.GetButtonIndexByVisibleIndex(
  AButtonVisibleIndex: Integer): Integer;
var
  I, ACurrentVisibleIndex: Integer;
begin
  ACurrentVisibleIndex := -1;
  ...
  begin
    ...
    begin
      ...
      begin
        Inc(ACurrentVisibleIndex);
        if ACurrentVisibleIndex = AButtonVisibleIndex then
          Exit(ACurrentVisibleIndex);
      end;
    end;
  end;
  Result := -1;
...
function TcxCustomEditProperties.GetFreeNotificator: TcxFreeNotificator;
begin
...
begin
  if AssignedValues.ReadOnly then
    ...
    if DefaultValuesProvider = nil then
      ...
      Result := DefaultValuesProvider.DefaultReadOnly;
end;
...
function TcxCustomEditProperties.SupportsMultiThreading(AUseDisplayValue: Boolean): Boolean;
begin
...
function TcxCustomEditProperties.IsAutomationUnitNeededInUses: Boolean;
var
  AButton: TcxEditButton;
  I: Integer;
begin
  Result := Assigned(FAutomation) and FAutomation.IsAutomationUnitNeededInUses;
  if not Result then
  begin
    for I := 0 to Buttons.Count - 1 do
    begin
      AButton := Buttons[I];
      if Assigned(AButton.Automation) and AButton.Automation.IsAutomationUnitNeededInUses then
        Exit(True);
    end;
  end;
end;
...
procedure TcxCustomEditProperties.SetAutomation(
  const AValue: TdxAutomationElementSettings);
begin
...
procedure TcxCustomEditProperties.SetDefaultValuesProvider(Value: TcxCustomEditDefaultValuesProvider);
begin
  FDefaultValuesProvider := Value;
...
class function TcxCustomEditProperties.ShowAtDesignTime: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  begin
    for I := 0 to FCreatedEditPropertiesList.Count - 1 do
      with TcxCustomEditProperties(FCreatedEditPropertiesList.List[I]) do
      begin
        if DefaultValuesProvider = Self then
...
function TdxEditChildAccessibilityHelper.GetLocalId(AChildID: TcxAccessibleSimpleChildElementID = 0): Integer;
begin
  Result := FLocalId;
end;

{ TdxEditButtonAccessibilityHelper }

function TdxEditButtonAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if Parent <> nil then
    Result := cxRectOffset(EditButtonViewInfo.Bounds, Parent.GetScreenBounds(0).TopLeft)
  else
    Result := EditButtonViewInfo.Bounds;
end;

procedure TdxEditButtonAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if Parent is TdxEditAccessibilityHelper then
    TdxEditAccessibilityHelper(Parent).DoButtonClick(EditButtonViewInfo.ButtonVisibleIndex);
end;

function TdxEditButtonAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Push';
end;

function TdxEditButtonAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := EditButtonViewInfo.Data.Caption;
  if (Result = '') and (Parent is TdxEditAccessibilityHelper) then
    Result := TdxEditAccessibilityHelper(Parent).GetButtonName(EditButtonViewInfo.ButtonIndex);
end;

function TdxEditButtonAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  if EditButtonViewInfo.EditViewInfo.Edit <> nil then
    Result := EditButtonViewInfo.EditViewInfo.Edit.IAccessibilityHelper.GetHelper
  else
    Result := nil;
end;

function TdxEditButtonAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_PUSHBUTTON;
end;

function TdxEditButtonAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE;
end;

function TdxEditButtonAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopLocation, aopDefaultAction];
end;

function TdxEditButtonAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxEditButtonAccessibilityHelper.GetEditButtonViewInfo: TcxEditButtonViewInfo;
begin
  Result := TcxEditButtonViewInfo(OwnerObject);
end;
...
procedure TcxEditButtonViewInfo.BeforeDestruction;
begin
  FIsDestroying := True;
...
function TcxEditButtonViewInfo.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIAEditButtonProvider.Create(Self);
end;

function TcxEditButtonViewInfo.GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider;
begin
...
function TcxEditButtonViewInfo.GetAutomationSettings: TdxAutomationElementSettings;
begin
  if Assigned(UIAProvider()) and (ButtonIndex < EditViewInfo.EditProperties.Buttons.Count) then
    Result := EditViewInfo.EditProperties.Buttons[ButtonIndex].Automation
  else
    Result := nil;
end;

function TcxEditButtonViewInfo.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;

function TcxEditButtonViewInfo.UIAProvider: IdxUIAProvider;
begin
  Result := GetUIAProvider(False);
end;

function TcxEditButtonViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := TdxEditButtonAccessibilityHelper.Create(Self);
...
begin
  ...
  try
    ...
    FVisible := True;
    FImageIndex := -1;
    FHotTrackMode := TcxEditButtonHotTrackMode.Default;
    FMode := TcxEditButtonMode.Button;
    FTransparent := False;
    FAutomation := GetAutomationSettingsClass.Create(Self);
  finally
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Self.Width := Width;
        Self.Hint := Hint;
        Self.HotTrackMode := HotTrackMode;
        Self.Mode := Mode;
...
procedure TcxCustomEditButton.SetHotTrackMode(
  Value: TcxEditButtonHotTrackMode);
begin
  if FHotTrackMode <> Value then
  begin
    FHotTrackMode := Value;
    Changed(False);
  end;
...
procedure TcxCustomEditButton.SetMode(Value: TcxEditButtonMode);
begin
  if FMode <> Value then
  begin
    FMode := Value;
...
procedure TcxCustomEditButton.SetTransparent(Value: Boolean);
begin
...
procedure TcxCustomEditButton.SetAutomation(
  const AValue: TdxAutomationElementSettings);
begin
  Automation.Assign(AValue);
end;
...
function TcxCustomEditButton.GetAutomationSettingsClass: TdxAutomationElementSettingsClass;
begin
  Result := TdxAutomationElementSettings;
end;
...
begin
  if FFreeNotificator <> nil then
  begin
    FFreeNotificator.RemoveSender(Sender);
    FFreeNotificator.Check(FFreeNotificator);
  end;
end;
...
function TcxCustomEditButtons.GetFreeNotificator: TcxFreeNotificator;
begin
  if FFreeNotificator = nil then
  begin
    FFreeNotificator := TcxFreeNotificator.Create(nil);
...
procedure TcxEditRepository.RemoveItems(AItemClass: TcxEditRepositoryItemClass);
var
...
begin
  ...
  begin
    AItem := Items[I];
    if AItem.InheritsFrom(AItemClass) then
      AItem.Free;
  end;
end;
...
procedure TcxInplaceEditList.DisconnectProperties(AProperties: TcxCustomEditProperties);
var
  AItemIndex: Integer;
  AEdit: TcxCustomEdit;
begin
  ...
  begin
    if not FItems[AItemIndex].Properties.AllowRepositorySharing then
    begin
      AEdit := FItems[AItemIndex].Edit;
      AEdit.LockChangeEvents(True);
      try
        AEdit.DisconnectNonShareableProperties;
      finally
...
function TcxInplaceEditList.FindItem(AProperties: TcxCustomEditProperties; ACanUseFreeEditors: Boolean): Integer;
var
  I: Integer;
begin
  Result := -1;
...
var
  AItem: TcxEditListItem;
  I: Integer;
begin
  for I := 0 to Count - 1 do
  begin
    AItem := FItems[I];
    if (AItem.Properties = nil) and (AItem.Edit.ClassType = APropertiesClass.GetContainerClass) then
      Exit(I);
  end;
  Result := -1;
end;
...
begin
  ...
  try
    ...
    if not AProperties.AllowRepositorySharing then
      AEdit.InitializeNonShareableProperties;
  finally
...
function TcxEditValidateInfo.HasErrorIcon: Boolean;
begin
  Result := (ErrorType <> eetNone) and (FErrorIcon <> nil) and not FErrorIcon.Empty;
end;
...
function TcxCustomEditViewInfo.DrawAsInplace: Boolean;
begin
  Result := IsInplace and not ForceDrawAsStandalone;
end;
...
procedure TcxCustomEditViewInfo.UpdateStyle(AStyle: TcxContainerStyle);
begin
  inherited;
  UseSkinnedColors := UseSkinnedColors and not DrawAsInplace;
end;

procedure TcxCustomEditViewInfo.CalculateEditorBounds;
begin
  FEditorBounds := Bounds;
  if Assigned(FOnCalculateEditorBounds) then
    FOnCalculateEditorBounds(Self, FEditorBounds);
end;
...
function TcxCustomEditViewInfo.ShouldUseStyleColorPalette(AStyle: TcxContainerStyle): Boolean;
begin
  Result := not IsInplace and AStyle.IsColorAssigned and not Painter.SupportsEditorBorders;
end;

function TcxCustomEditViewInfo.CalculateBackgroundColor(
  AStyle: TcxContainerStyle; AEnabled: Boolean; AContainer: TcxContainer = nil): TColor;
const
  NativePartMap: array [Boolean] of Integer = (EP_EDITTEXT, EP_BACKGROUND);
var
  AEdit: TcxCustomEdit absolute AContainer;
begin
  if (AEdit <> nil) and AEdit.DefaultParentColor then
    Exit(AStyle.Color);

  if UseSkinnedColors and (AEdit <> nil) and not IsStyleValueAssigned(AStyle, csvColor) then
    Result := Painter.DefaultEditorBackgroundColorEx(AEdit.GetEditStateColorKind)
  else
    Result := clDefault;

  if Result = clDefault then
  begin
    if NativeStyle and not (AEnabled or AStyle.IsValueAssigned(csvColor)) then
      Result := cxGetThemeColor(totEdit, NativePartMap[IsWinVistaOrLater], NativeState, TMT_FILLCOLOR);
  end;
...
begin
  Result := True;
  if Assigned(FOnCanDrawEditValue) then
    FOnCanDrawEditValue(Self, Result);
end;
...
function TcxCustomEditViewInfo.GetDrawReadOnly: Boolean;
begin
  if not Painter.SupportsEditorBorders then
    Result := ReadOnly and (BorderState <> TcxContainerBorderState.cbsDisabled) 
  else
    Result := ReadOnly;
end;

procedure TcxCustomEditViewInfo.DrawBorderedBackground(ACanvas: TcxCanvas; const R: TRect);
var
  [unsafe] APalette: IdxColorPalette;
begin
  GetBackgroundStyleColorPalette(APalette);
  Painter.DrawScaledEditorBackground(ACanvas, R, DrawAsInplace, GetDrawReadOnly, BackgroundColor,
    BorderState, ScaleFactor, APalette);
end;
...
var
  ASize: TSize;
begin
  if ErrorData.HasErrorIcon then
  begin
    ...
    TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, cxRectCenter(FErrorBounds, ASize), FErrorBounds, ErrorData.ErrorIcon, nil, -1, idmNormal, True, 0, clNone, True, Painter.ActiveColorPalette);
  end;
end;
...
begin
  ...
  begin
    if AViewInfo.Data.NativeStyle then
      ...
      if UseSkins then
        DrawLookAndFeelButtonBackground(ACanvas, AViewInfo, ARect)
      else
...
begin
  ...
  APenColor := TextColor;
  if Edit = nil then
    ABrushColor := ButtonColorsMap[AButtonViewInfo.Data.DrawState = ebsDisabled]
  else
...
begin
  ...
  if AViewInfo.Data.NativeState <> TC_NONE then
    ...
    begin
      ...
      if (AViewInfo.Data.DrawState in [ebsPressed, ebsSelected]) and (AButtonStyle = btsSimple) then
...
procedure TcxCustomEditViewInfo.DrawButtonContent(ACanvas: TcxCanvas;
  ...
  begin
    ...
    if AViewInfo.Data.DrawState <> ebsDisabled then
  ...
  begin
    if AContentRect.Right - AContentRect.Left < 12 then
      ...
      AContentSize.cy := ScaleFactor.GetDiscreteFactor.Apply(2);
  ...
  procedure GetButtonGlyphDrawParameters(AButtonViewInfo: TcxEditButtonViewInfo; var AGlyph: TGraphic; var APalette: IdxColorPalette);
  ...
  begin
    ...
    APalette := nil;
    ASender := GetSender;
    if (EditProperties <> nil) and Assigned(EditProperties.OnButtonGlyphDrawParameters) and (ASender <> nil) then
      EditProperties.OnButtonGlyphDrawParameters(ASender, AButtonViewInfo.ButtonIndex,
        AButtonViewInfo.Data.State, AGlyph, APalette);
  end;
  ...
  begin
    GetButtonGlyphDrawParameters(AViewInfo, AGlyph, APalette);
    if AGlyph = nil then
      AGlyph := AViewInfo.Glyph;
    if APalette = nil then
      APalette := GetButtonColorPalette(AViewInfo.Data.DrawState);
    if IsImageAssigned(AGlyph, AViewInfo.Images, AViewInfo.ImageIndex) then
    begin
      ASize := dxGetImageSize(AGlyph, AViewInfo.Images, 0, ScaleFactor);
      P := GetContentPosition(ASize, ANeedOffsetContent);
      AImageRect := Rect(P.X, P.Y, P.X + ASize.cx, P.Y + ASize.cy);
      TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, AImageRect, AContentRect,
        AGlyph, AViewInfo.Images, AViewInfo.ImageIndex,
        EnabledImageDrawModeMap[not (AViewInfo.Data.DrawState = ebsDisabled)],
        False, 0, clDefault, True, APalette);
    end;
  end;
  ...
  begin
    ...
    R := AContentRect;
    if not DrawAsInplace then 
      InflateRect(R, -1, -1)
    else
      InflateRect(R, -1, 0);
    Dec(R.Right);

    if (AViewInfo.Data.DrawState = ebsDisabled) and not UseSkins then
    begin
    ...
    begin
      if (AViewInfo.Data.Style = btsHotFlat) and (AViewInfo.Data.DrawState in [ebsPressed, ebsSelected]) then
        AColor := APenColor
      else
        AColor := cxGetActualColor(AViewInfo.Data.TextColor, TextColor);
  ...
  try
    ...
    begin
      Painter.DrawScaledEditorButtonGlyph(ACanvas, AContentRect,
        ...
        EditBtnStateToButtonState[AViewInfo.Data.DrawState], AViewInfo.ScaleFactor,
...
begin
  ...
  begin
    ...
    ATheme := OpenTheme(AThemedObjectType, ScaleFactor);
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := SupportsRoundedContainerBorders;
  end;
end;
...
begin
  Result := False;
  for I := 0 to Length(ButtonsInfo) - 1 do
    if ButtonsInfo[I].Data.DrawState = ebsSelected then
    begin
      Result := True;
      Break;
    end;
...
function TcxCustomEditViewInfo.GetUnscaledPadding: TdxPadding;
begin
  if PainterPaddingAvailable then
    Result := Painter.GetEditorPadding(DrawAsInplace, nil) 
  else
    Result := TdxVisualRefinements.Padding;
end;

function TcxCustomEditViewInfo.HasPadding: Boolean;
begin
  if IsInplace then
    Result := UseInplacePadding
  else
    Result := PainterPaddingAvailable or TdxVisualRefinements.UsePaddingForStandaloneEditors;
end;

function TcxCustomEditViewInfo.PainterPaddingAvailable: Boolean;
begin
  Result := (Painter <> nil) and Painter.SupportsEditorPadding(DrawAsInplace);
end;

function TcxCustomEditViewInfo.SupportsBorders: Boolean;
begin
  Result := PaintOnGlass or Painter.SupportsEditorBorders;
end;

function TcxCustomEditViewInfo.SupportsRoundedContainerBorders: Boolean;
begin
  Result := Painter.SupportsRoundedContainerBorders(-1, DrawAsInplace); 
end;
...
function TcxCustomEditViewInfo.GetActualUnscaledPadding(AHorzAlignment: TAlignment): TdxPadding;

  procedure AdjustPaddingForButtons(var APadding: TdxPadding);
  var
    I, ACount: Integer;
    AButtonViewInfo: TcxEditButtonViewInfo;
    ADropLeftIndent: Boolean;
    ADropRightIndent: Boolean;
  begin
    ACount := Length(ButtonsInfo);
    if ACount = 0 then
      Exit;
    ADropLeftIndent := False;
    ADropRightIndent := False;
    if UseRightToLeftAlignment then
      ChangeBiDiModeAlignment(AHorzAlignment);

    for I := 0 to ACount - 1 do
    begin
      AButtonViewInfo := ButtonsInfo[I];
      if AButtonViewInfo.Data.LeftAlignment then
        ADropLeftIndent := AHorzAlignment <> taLeftJustify
      else
        ADropRightIndent := AHorzAlignment <> taRightJustify;
    end;
    if not UseRightToLeftAlignment then
    begin
      if ADropLeftIndent then
        APadding.Left := 0;
      if ADropRightIndent then
        APadding.Right := 0;
    end
    else
    begin
...
begin
  if HasPadding then
  begin
    Result := GetUnscaledPadding;
    AdjustPaddingForButtons(Result);
  end
  else
...
var
  I: Integer;
begin
  if ErrorData.IsError and FErrorBounds.Contains(P) then
    Result := ecpErrorIcon
  else
    if BorderRect.Contains(P) then
      Result := ecpControl
    else
      Result := ecpNone;

  for I := Low(ButtonsInfo) to High(ButtonsInfo) do
    if ButtonsInfo[I].Bounds.Contains(P) then
      Exit(I);
end;
...
begin
  ...
  try
    ...
    if not AViewInfo.Data.TransparentBackground then
      DrawButtonBorder(ACanvas, AViewInfo, ABackgroundRect, AContentRect, APenColor, ABrushColor);
    if not AViewInfo.Data.TransparentBackground and not IsRectEmpty(ABackgroundRect) then
...
procedure TcxCustomEditViewInfo.DrawEditButton(ACanvas: TcxCanvas; AButtonVisibleIndex: Integer);
  ...
  begin
    Result := (AViewInfo.Data.DrawState = ebsPressed) and
...
begin
  if Assigned(FOnAfterDrawBackground) then
    FOnAfterDrawBackground(Self, ACanvas);
end;

procedure TcxCustomEditViewInfo.DoAfterDrawValue(ACanvas: TcxCanvas);
begin
  if Assigned(FOnAfterDrawValue) then
    FOnAfterDrawValue(Self, ACanvas);
end;
...
begin
  ...
  if AButtonViewInfo.Data.DrawState = ebsPressed then
  begin
...
procedure TcxCustomEditViewInfo.DrawComplexFrameEx(ACanvas: TcxCanvas; var ARect: TRect; ALeftTopColor,
...
begin
  ...
  begin
    ABrushColor := ABrushColorA[Data.DrawState];
    ...
    if Data.DrawState in [ebsPressed, ebsSelected] then
      if Data.Transparent and (Data.DrawState = ebsSelected) then
        APenColor := clBtnShadow
      else
        APenColor := clWindow
    else
...
procedure TcxCustomEditViewInfo.DrawNativeButtonBorder(ACanvas: TcxCanvas; AButtonViewInfo: TcxEditButtonViewInfo;
  ...
  var
    ATheme: TdxTheme;
  begin
    ATheme := OpenTheme(AThemeObject, ScaleFactor);
...
begin
  if AButtonViewInfo.Data.Transparent then
    ...
    if AButtonViewInfo.Data.DrawState = ebsDisabled then
      ABrushColor := clBtnFace
    else
      if AButtonViewInfo.Data.DrawState = ebsNormal then
        ...
        ABrushColor := GetEditButtonHighlightColor(
          AButtonViewInfo.Data.DrawState = ebsPressed, AIsOffice11Style);

  AHighlightColor := GetEditBorderHighlightColor(AIsOffice11Style);

  if (AButtonViewInfo.Data.DrawState in [ebsDisabled, ebsNormal]) or
  ...
  begin
      if not(AButtonViewInfo.Data.DrawState in [ebsDisabled, ebsNormal]) then
...
procedure TcxCustomEditViewInfo.FrameRectEx(ACanvas: TcxCanvas; var ARect: TRect; AColor: TColor);
begin
  ACanvas.FrameRect(ARect, AColor);
  InflateRect(ARect, -1, -1);
end;

procedure TcxCustomEditViewInfo.DrawLookAndFeelButtonBackground(ACanvas: TcxCanvas; AButtonViewInfo: TcxEditButtonViewInfo;
  const ARect: TRect);
begin
  Painter.DrawScaledEditorButton(ACanvas, ARect, EditBtnKindToEditBtnPainterKind[AButtonViewInfo.Data.Kind],
    EditBtnStateToButtonState[AButtonViewInfo.Data.DrawState], AButtonViewInfo.ScaleFactor,
...
procedure TcxCustomEditViewInfo.DrawUsualButtonBackground(ACanvas: TcxCanvas; AButtonViewInfo: TcxEditButtonViewInfo; const ARect: TRect; ABrushColor: TColor);
  ...
  begin
    AGradientDrawing := (AButtonViewInfo.Data.Style = btsOffice11) and
      ...
      (AButtonViewInfo.Data.DrawState <> ebsDisabled);
    if AGradientDrawing then
      case AButtonViewInfo.Data.DrawState of
        ebsNormal:
          begin
...
begin
  Result := NativeStyle or (UseSkins and (Painter <> nil) and
    Painter.SupportsEditorBorders and StyleColorAssigned);
end;
...
begin
  ...
  Result.Offset(P.X, P.Y + cxGetCursorSize.Height);
end;
...
begin
  RefinementsTuner := nil;
  FreeAndNil(FScaleFactor);
  inherited Destroy;
end;

function TcxCustomEditViewData.GetPainter: TcxCustomLookAndFeelPainter;
begin
  Result := Style.LookAndFeel.Painter;
end;
...
begin
  ...
  if (not IsInplace or ForceDrawAsStandalone) and not NativeStyle and
  ...
  AViewInfo.PaintOnGlass := PaintOnGlass;
  if IsInplace then 
  ...
  AViewInfo.ReadOnly := CalculateReadOnly;
  ...
  ClientExtent := GetClientExtent(ACanvas, AViewInfo);
  AViewInfo.ClientRect := cxRectContent(AViewInfo.ClientRect, ClientExtent);
...
procedure TcxCustomEditViewData.CalculateButtonsViewInfo(ACanvas: TcxCanvas;
  ...
  procedure CalculateButtonStates(APrevPressedButton: Integer);
    ...
    function GetDrawState(AButtonViewInfo: TcxEditButtonViewInfo; AState: TcxEditButtonState): TcxEditButtonState;
    begin
      case AState of
          ebsDisabled: Result := ebsDisabled;
          ebsPressed:
          begin
            if AButtonViewInfo.Data.Mode = TcxEditButtonMode.Glyph then
              Result := GetDrawState(AButtonViewInfo, ebsSelected)
            else
              Result := ebsPressed;
          end;
          ebsSelected:
          begin
            if AButtonViewInfo.Data.HotTrackMode = TcxEditButtonHotTrackMode.None then
              Result := GetDrawState(AButtonViewInfo, ebsNormal)
            else
              Result := ebsSelected;
          end;
      else
    ...
    procedure CalculateDrawState(AButtonViewInfo: TcxEditButtonViewInfo);
    begin
      AButtonViewInfo.Data.DrawState := GetDrawState(AButtonViewInfo, AButtonViewInfo.Data.State);
    end;

    procedure CorrectByButtonMode(AButtonViewInfo: TcxEditButtonViewInfo; AButtonVisibleIndex: Integer);
    begin
      if (AButtonViewInfo.Data.State = ebsPressed) and (AButtonViewInfo.Data.Mode = TcxEditButtonMode.Glyph) then
      begin
        AButtonViewInfo.Data.State := ebsSelected;
        AViewInfo.SelectedButton := AViewInfo.PressedButton;
        AViewInfo.PressedButton := -1;
      end
      else
        if (AButtonViewInfo.Data.State = ebsNormal) and
          (AButtonViewInfo.Data.HotTrackMode = TcxEditButtonHotTrackMode.Editor) and
          (not IsDesigning and (Edit <> nil) and PtInRect(Edit.VisibleBounds, P)) then
            AButtonViewInfo.Data.State := ebsSelected;
    end;
      ...
      if not Enabled or not Properties.Buttons[AButtonViewInfo.ButtonIndex].Enabled then
        ...
        begin
          ACapturePressing := (Button = cxmbNone) and (ssLeft in Shift) and
            (
              (AButtonViewInfo.Data.State = ebsNormal) or
              ((AButtonViewInfo.Data.State = ebsSelected) and (AButtonViewInfo.Data.HotTrackMode = TcxEditButtonHotTrackMode.Editor)) or
              ((AButtonViewInfo.Data.State = ebsSelected) and (AButtonViewInfo.Data.Mode = TcxEditButtonMode.Glyph))
            ) and (GetCaptureButtonVisibleIndex = AButtonVisibleIndex);
          ...
          AButtonViewInfo.Data.State := ebsNormal;
      AButtonViewInfo.Data.RawState := AButtonViewInfo.Data.State;
      CorrectByButtonMode(AButtonViewInfo, AButtonVisibleIndex);
      CalculateDrawState(AButtonViewInfo);
  ...
  begin
    ...
    begin
      ...
      if AViewInfo.UseSkins then
        Data.TextColor := AViewInfo.Painter.EditButtonTextColor(EditBtnStateToButtonState[Data.State]);
      if Data.TextColor = clDefault then
    ...
    Data.Mode := AButton.Mode;
    Data.HotTrackMode := AButton.HotTrackMode;
    Data.TransparentBackground := AButton.Transparent;
  end;
end;
...
begin
  ...
  begin
    ANativeStyle := IsNativeStyle(Style.LookAndFeel);
    if Style.LookAndFeel.SkinPainter <> nil then
      ...
      if Style.TransparentBorder then
        ...
        if not Style.HasBorder or ANativeStyle and (Style.BaseStyle.BorderStyle = ebsNone) or (Edit <> nil) and Edit.Transparent then
          Result := cxEmptyRect
        else
...
begin
  ...
  begin
    ...
    ABorderWidth := Painter.GetContainerBorderWidth(TcxContainerBorderStyle(ABorderStyle));
...
begin
  if AButtonViewInfo.Data.ComboBoxStyle then
    Result := OpenTheme(totComboBox, ScaleFactor)
  else
    Result := OpenTheme(totButton, ScaleFactor);
end;
...
var
  R, CR: TRect;
begin
  if IsInplace and not ForceDrawAsStandalone then
    Result := ContentOffset
  else
    if NativeStyle then
    begin
      if not Style.TransparentBorder and (Style.BaseStyle.BorderStyle = ebsNone) then
        ...
        begin
          R := Rect(0, 0, 100, 100);
          GetThemeBackgroundContentRect(OpenTheme(totEdit, ScaleFactor), ACanvas.Handle, EP_EDITTEXT, ETS_NORMAL, R, CR);
          Result := CR;
      ...
      if not Painter.SupportsEditorBorders then 
        Result := Painter.GetEditorButtonsPadding(ScaleFactor)
      else
...
function TcxCustomEditViewData.GetClientExtent(ACanvas: TcxCanvas; AViewInfo: TcxCustomEditViewInfo): TRect;
  ...
  procedure AdjustPaddingForButtons(var APadding: TdxPadding; AAdjustLeft: Boolean; AAdjustRight: Boolean);
  begin
    if not UseRightToLeftAlignment then
    begin
      if AAdjustLeft then
        APadding.Left := 0;
      if AAdjustRight then
        APadding.Right := 0;
    end
    else
    begin
...
begin
  ...
  AActualPadding := AViewInfo.GetActualUnscaledPadding(HorzAlignment);
  AdjustPadding(AActualPadding);
  AActualPadding.InflatePadding(Result, ScaleFactor, UseRightToLeftScrollBar);
end;
...
begin
  ...
  begin
    ...
    ATempViewInfo.ForceDrawAsStandalone := ForceDrawAsStandalone;
  end
  else
    ATempViewInfo := AViewInfo;
  try
    if AViewInfo = nil then
      Calculate(ACanvas, Rect(0, 0, cxMaxRectSize, cxMaxRectSize), cxInvalidPoint, cxmbNone, [], ATempViewInfo, False);
    Result := InternalGetEditConstantPartSize(ACanvas, IsInplace, AEditSizeProperties, MinContentSize, ATempViewInfo);
  finally
    if AViewInfo = nil then
...
begin
  ...
  begin
    Result.cx := 0;
    Result.cy := TdxTextMeasurer.TextLineHeight(Style.GetVisibleFont) + Self.GetEditContentSizeCorrection.cy;
  end
  else
...
begin
  Result.cx := EditContentParams.Offsets.Left + EditContentParams.Offsets.Right + EditContentParams.SizeCorrection.cx;
  Result.cy := EditContentParams.Offsets.Top + EditContentParams.Offsets.Bottom + EditContentParams.SizeCorrection.cy;
end;
...
function TcxCustomEditViewData.HasBorders: Boolean;
begin
  if (Painter = nil) or Painter.SupportsEditorBorders or IgnorePainterBorderSettings then
    Result := Style.HasBorder and ((Style.BorderStyle <> ebsNone) or Style.TransparentBorder)
  else
    Result := False;
end;
...
begin
  if (Painter <> nil) and (not Painter.SupportsEditorShadow) then
    Exit(False);
  Result := Style.Shadow and not IsNativeStyle(Style.LookAndFeel);
end;

function TcxCustomEditViewData.IgnoreButtonWhileStretching(AButtonVisibleIndex: Integer): Boolean;
begin
  Result := False;
end;

function TcxCustomEditViewData.IgnorePainterBorderSettings: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  if AButtonViewInfo.Data.NativePart = TC_NONE then
    ...
    AButtonViewInfo.Data.NativeState := ButtonStateMap[
      AButtonViewInfo.Data.ComboBoxStyle, AButtonViewInfo.Data.DrawState];
end;

procedure TcxCustomEditViewData.AdjustPadding(var APadding: TdxPadding);
begin
  if RefinementsTuner <> nil then
    RefinementsTuner.AdjustPadding(APadding);
  if not IsTouchScrollUIMode and (Edit <> nil) then
  begin
    if Edit.IsScrollBarActive(sbHorizontal) then
      APadding.Bottom := 0;
    if Edit.IsScrollBarActive(sbVertical) then
      APadding.Right := 0;
  end;
end;
...
function TcxCustomEditViewData.CalculateReadOnly: Boolean;
begin
  Result := CalculateDefaultReadOnly;
end;

function TcxCustomEditViewData.CalculateStyleReadOnly: Boolean;
begin
  Result := (Edit <> nil) and (Edit.Style.ReadOnly);
end;

function TcxCustomEditViewData.CalculateDefaultReadOnly: Boolean;
begin
  Result := FProperties.ReadOnly and not IsInplace;
end;
...
begin
  ...
  if not IsInplace then
    AViewInfo.BorderColor := AViewInfo.CalculateBorderColor(Style, Enabled, IsDesigning, Edit);
  AViewInfo.BackgroundColor := AViewInfo.CalculateBackgroundColor(Style, Enabled, Edit);
  AViewInfo.TextColor := AViewInfo.CalculateTextColor(Style, Enabled, Edit);
  AViewInfo.StyleColorAssigned := Style.IsColorAssigned;
  AViewInfo.UseStyleColorPalette := AViewInfo.ShouldUseStyleColorPalette(Style);
end;

function TcxCustomEditViewData.CanPressButton(AViewInfo: TcxCustomEditViewInfo; AButtonVisibleIndex: Integer): Boolean;
begin
  Result := True;
end;
...
begin
  Result := GetDefaultContainerState(ABounds, P, Button, Shift, AIsMouseEvent);
end;

function TcxCustomEditViewData.GetDefaultContainerState(const ABounds: TRect;
  const P: TPoint; Button: TcxMouseButton; Shift: TShiftState;
  AIsMouseEvent: Boolean): TcxContainerState;
  ...
  procedure AddHotTrackIfNecessary(var AState: TcxContainerState);
  begin
    if PtInRect(GetEditVisibleBounds, P) and
      (cxShiftStateMoveOnly(Shift) or (Edit <> nil) and (GetCaptureControl = Edit)) then
        Include(AState, csHotTrack);
  end;

  function GetStyleReadOnlyAsDisabled: Boolean;
  begin
    if Edit <> nil then
      Result := Edit.GetDefaultStyleReadOnlyAsDisabled
    else
      Result := cxDefaultStyleReadOnlyAsDisabled;
  end;
...
begin
  if Enabled then
  begin
    AIsReadOnly := CalculateReadOnly and CalculateStyleReadOnly;
    if IsDesigning then
    begin
      if AIsReadOnly then
        Result := [csReadOnly]
      ...
      if GetStyleReadOnlyAsDisabled then
      begin
        if AIsReadOnly then
          Result := [csReadOnly]
        else
...
begin
  if (Painter <> nil) and Painter.ApplyEditorAdvancedMode then
    Result.Empty
  else     
    Result := {ScaleFactor.Apply}(EditContentDefaultOffsets[IsInplace]); 
end;
...
begin
  ...
  AParams.SizeCorrection.cx := Max(EditContentDefaultOffsets[False].Left,
    EditContentMaxTotalDefaultHorzOffset - (AParams.Offsets.Left + AParams.Offsets.Right));
  if not NativeStyle then
    AParams.SizeCorrection.cy := 0
  else
    AParams.SizeCorrection.cy := GetNativeInnerTextEditContentHeightCorrection(Properties, IsInplace);
...
begin
  ...
  begin
    ...
    Result.cx := Result.cx + GetErrorIconWidth(AViewInfo.ErrorData);
  end
  else
  begin
...
begin
  Result := AErrorData.HasErrorIcon and (evoShowErrorIcon in Properties.ValidationOptions);
end;
...
function TcxEditStyleController.GetStyleReadOnly: TcxEditStyle;
begin
  Result := TcxEditStyle(FStyles[csReadOnly]);
end;
...
procedure TcxEditStyleController.SetStyleReadOnly(Value: TcxEditStyle);
begin
  FStyles[csReadOnly] := Value;
end;
...
begin
  Result := [svButtonTransparency, svEdges, svFont, svGradient,
    ...
    svShadow, svTransparentBorder, svReadOnly];
end;
...
begin
  Result := (ParentStyle <> nil) and TcxCustomEditStyle(ParentStyle).Gradient;
end;

function TcxCustomEditStyle.DefaultGradientButtons: Boolean;
begin
  Result := (ParentStyle <> nil) and TcxCustomEditStyle(ParentStyle).GradientButtons;
end;
...
begin
  inherited Create;
  FEdit := AEdit;
  FDefaultValuesProvider := GetDefaultValuesProviderClass.Create(nil);
end;
...
begin
  ...
  if Assigned(FOnExecuteAction) then
    FOnExecuteAction(Action, Result);
end;
...
procedure TcxCustomEdit.SetModifiedOnEnter(Value: Boolean);
begin
    FModifiedOnEnter := Value;
end;
...
begin
  Result := DefaultValuesProvider.IsDataAvailable;
end;
...
function TcxDefaultEditStyleController.GetStyleReadOnly: TcxEditStyle;
begin
  Result := DefaultEditStyleController.StyleReadOnly;
end;
...
procedure TcxDefaultEditStyleController.SetStyleReadOnly(Value: TcxEditStyle);
begin
  DefaultEditStyleController.StyleReadOnly := Value;
end;
...
begin
  Result := IsEditing and
    not (FEditHiding or FInitiatingEditing or FEdit.IsPosting or FEdit.IsEditValidating or FEdit.InternalSettingValue ); //..
end;

procedure TcxCustomEditingController.CheckEditUpdatePost;
begin
  if FEditUpdateNeeded and (FHideEditOnScrollContentCount = 0) then
    DoUpdateEdit;
end;
...
procedure TcxCustomEditingController.DoHideEditOnScrollContent;
var
  ABoundsForValidate: TRect;
begin
  if Edit.Left = cxInvisibleCoordinate then
    Exit;
  ABoundsForValidate := Edit.BoundsRect;
  Edit.HidePopupWindowOnScroll;
  Edit.Left := cxInvisibleCoordinate; 
  RecalculateEditViewInfo;
  Winapi.Windows.ValidateRect(Edit.Parent.Handle, ABoundsForValidate);
end;

procedure TcxCustomEditingController.DoShowEditAfterScrollContent;
begin
  DoUpdateEdit;
end;
...
function TcxCustomEditingController.SupportsPadding: Boolean;
begin
  Result := True;
end;
...
function TcxCustomEditingController.HasHiddenEditor: Boolean;
begin
  Result := IsEditing and Edit.HandleAllocated and (Edit.Left = cxInvisibleCoordinate);
end;

procedure TcxCustomEditingController.HideEditOnScrollContent;
begin
  if (FHideEditOnScrollContentCount = 0) and IsEditing then
    DoHideEditOnScrollContent;
  Inc(FHideEditOnScrollContentCount);
end;
...
begin
  ...
  try
    ...
    FEdit.ViewInfo.UseInplacePadding := SupportsPadding;
  finally
...
begin
  Result := IsEditing and FEdit.Visible and (FEdit.Left <> cxInvisibleCoordinate) and not IsHiddenOnScrolling;
end;

function TcxCustomEditingController.IsHiddenOnScrolling: Boolean;
begin
  Result := HasHiddenEditor and (FHideEditOnScrollContentCount > 0);
end;
...
procedure TcxCustomEditingController.RecalculateEditViewInfo;
begin
end;

procedure TcxCustomEditingController.ShowEditAfterScrollContent;
begin
  Dec(FHideEditOnScrollContentCount);
  if (FHideEditOnScrollContentCount = 0) and IsEditing then
    DoShowEditAfterScrollContent;
end;
...
begin
 if   IsEditing and FEdit.EditModified then
	begin
      if   not FEdit.InternalSettingValue then //..
...
begin
  ...
  begin
    ...
    DataBinding.OnExecuteAction := EditExecuteAction;
    DataBinding.OnUpdateAction := EditUpdateAction;
...
procedure TcxCustomEditingController.EditExecuteAction(Action: TBasicAction; var Handled: Boolean);
begin
  Handled := False;
end;
...
procedure TcxCustomEditingController.EditUpdateAction(Action: TBasicAction; var Handled: Boolean);
begin
  Handled := False;
end;
...
begin
  Result := System.TypInfo.GetPropInfo(Properties, APropertyName) <> nil;
end;
...
function TcxCustomEditContainer.GetInnerEditUIAProvider(AAllowCreate: Boolean): IdxUIAProvider;
begin
  Result := nil;
end;
...
constructor TdxEditAccessibilityHelper.Create(AOwnerObject: TObject);
begin
  inherited;
  FAccessibilityChildren := TList.Create;
end;

destructor TdxEditAccessibilityHelper.Destroy;
begin
  FreeAndNil(FAccessibilityChildren);
  inherited;
end;

function TdxEditAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if Visible then
    Result := cxRectSetOrigin(ViewInfo.Bounds, Edit.ClientToScreen(ViewInfo.Bounds.TopLeft))
  else
    Result := cxNullRect;
end;

function TdxEditAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := False;
end;

function TdxEditAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
  Result := IcxAccessibilityHelper(FAccessibilityChildren[AIndex]).GetHelper;
end;

function TdxEditAccessibilityHelper.GetChildCount: Integer;
var
  I: Integer;
begin
  FAccessibilityChildren.Clear;
  if Edit.IInnerEditAccessibilityHelper <> nil then
    AddAccessibilityChild(Edit.IInnerEditAccessibilityHelper);
  for I := 0 to High(ViewInfo.ButtonsInfo) do
    AddAccessibilityChild(ViewInfo.ButtonsInfo[I].IAccessibilityHelper);

  Result := FAccessibilityChildren.Count;
end;

function TdxEditAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := Edit.Caption;
end;

function TdxEditAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  if Visible then
    Result := Edit.Handle
  else
...
function TdxEditAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_CLIENT;
end;

function TdxEditAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE;
  if not Edit.Enabled then
    Result := Result or cxSTATE_SYSTEM_UNAVAILABLE;
  if Edit.Focused and ((Edit.IInnerEditAccessibilityHelper = nil) or
    (Edit.IInnerEditAccessibilityHelper.GetHelper.Role[0] = cxROLE_SYSTEM_STATICTEXT)) then
      Result := Result or cxSTATE_SYSTEM_FOCUSED;
  if (Edit.Parent = nil) or (ViewInfo = nil) or not Edit.Visible then
    Result := Result or cxSTATE_SYSTEM_INVISIBLE
  else
    Result := Result or cxSTATE_SYSTEM_NORMAL;
end;

function TdxEditAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation, aopValue];
end;

function TdxEditAccessibilityHelper.GetValue(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := VarToStr(Edit.EditValue);
end;

function TdxEditAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

procedure TdxEditAccessibilityHelper.AddAccessibilityChild(AChild: IcxAccessibilityHelper);
begin
  TdxEditChildAccessibilityHelper(AChild.GetHelper).FLocalId := FAccessibilityChildren.Count + 1;
  FAccessibilityChildren.Add(Pointer(AChild));
end;

procedure TdxEditAccessibilityHelper.DoButtonClick(AVisibleIndex: Integer);
begin
  Edit.DoButtonClick(AVisibleIndex);
end;

function TdxEditAccessibilityHelper.GetButtonName(AIndex: Integer): string;
begin
  Result := '';
end;

function TdxEditAccessibilityHelper.GetEdit: TcxCustomEdit;
begin
  Result := TcxCustomEdit(OwnerObject);
end;

function TdxEditAccessibilityHelper.GetViewInfo: TcxCustomEditViewInfo;
begin
  Result := Edit.ViewInfo;
end;
...
begin
  ...
  if not FIsInplace then
    Properties.DefaultValuesProvider := nil;
...
//..
function TcxCustomEdit.GetEditing : Boolean;
begin
     Result := FEditing and not IsDestroying;
end;
...
procedure TcxCustomEdit.WMKeyDown(var Message: TWMKeyDown);
begin
     inherited;
end;
...
function TcxCustomEdit.GetRepositoryAutomationSettings: TdxAutomationElementSettings;
begin
  if Assigned(FRepositoryItem) and Assigned(FRepositoryItem.Properties) then
    Result := FRepositoryItem.Properties.Automation
  else
    Result := nil;
end;
...
begin
     If   CanModify
     then try
            // Esta variable indica que se está realizando un post desde una función externa, y no desde la edición del cxCustomEdit
            FExternalPostingValue := True;
            BeforePosting;
            DoEditing;
            ...
            FExternalPostingValue := False;
            If   not IsEditValidated
...
var  UserIsCancelingForm : Boolean;

begin
     Result := GetUserValidateAction( UserIsCancelingForm );
end;

function TcxCustomEdit.GetUserValidateAction( out UserIsCancelingForm : Boolean ) : Boolean;

var  AParentForm : TCustomForm;
     UserIsvalidatingForm : Boolean;

begin
     AParentForm := GetParentForm( Self );

     // ¿Está pulsando un botón de validación o de cancelación del form?

     UserIsValidatingForm := False;
     UserIsCancelingForm := False;
     If   { not IsInplace and } ( Assigned ( AParentForm ) and Assigned( AParentForm.ActiveControl ) and ( AParentForm.ActiveControl is TCustomButton ) )
     then begin
          var FormButton := TCustomButton( AParentForm.ActiveControl );
          UserIsValidatingForm := FormButton.ModalResult=mrOk;
          UserIsCancelingForm := FormButton.ModalResult=mrCancel;
          end;

     // UserIsValidatingForm := not IsInplace and ( Assigned ( AParentForm ) and Assigned( AParentForm.ActiveControl ) and ( AParentForm.ActiveControl is TCustomButton ) and ( TCustomButton( AParentForm.ActiveControl ).ModalResult=mrOk ) );

     // Con la comprobación de FIsKeyPressed intento evitar que el control en el que se ha producido la pulsación sea otro y se aplique a éste

     Result := Editing and
               ( UserIsValidatingForm or
               ( FIsKeyPressed and ( ( ( FKeyPressed=VK_Return ) ) or ( ( FKeyPressed=Vk_Tab ) and not ( ssShift in FKeyShift ) ) or ( IsInplace and ( FKeyPressed in [ VK_Left, VK_Right, VK_Up, VK_Down ] ) ) ) ) );
end;

procedure TcxCustomEdit.UpdateKeyPressed( Key   : Word;
                                          Shift : TShiftState );
begin
     FIsKeyPressed := True;
     FKeyPressed := Key;
...
var
  ...
  APrevValidationError,
  AUserIsCancelingForm,
  AUserValidateAction : Boolean;

  function GetValueToValidate: TcxEditValue;
  begin
    {
    if FModifiedOnEnter or FModifiedAfterEnter or VarIsNull(EditValue) then  //..
    }
      Result := DisplayValue;
    {
    else
  ...
  begin
       If   AUserValidateAction or AForceValidation
       then If   Enabled and Assigned( ActiveProperties.DefaultValuesProvider )
            then If   ( ActiveProperties.Required or ActiveProperties.DefaultValuesProvider.DefaultRequired ) and ValueIsEmpty( ADisplayValue )
                 then begin
                      ClearDescription;
                      FValidationErrorText := cxSEditValueRequired;
                      FValidationError := True;
                      end;
  end;
...
//..
  DisableUserAction;
  try
  AUserValidateAction := GetUserValidateAction( AUserIsCancelingForm );
  If   not AForceValidation and
       not ( AUserValidateAction and ActiveProperties.NeedsValidation( DisplayValue ) ) and
       ( AUserIsCancelingForm or
         FIsEditValidated or
         not Enabled or
         FCancelValidation or
         ( FLockValidate<>0 ) or
         ( not FModifiedAfterEnter and not AUserValidateAction ) or

         //** 24.10.2023 Cuando se pulsa [F2], o simplemente se entra en modo edición, el registro no se edita (dsEdit) automáticamente y el
         //              usuario puede validar la entrada (por ejemplo con [Tab]). Sin embargo el campo no consta como validado (FIsEditValidated)
         //              porque no se ha editado el registro. No debe  validarse si no se ha modificado.

         ( IsInplace and not EditModified ) or // Antes : ( ( FIsEditValidated and not EditModified and not ActiveProperties.Required ) ) )

         not CanModify )
  then begin

       // Cuando se pone un valor por defecto (muchas veces incompleto) al entrar en el edit de una rejilla y no se acaba validando
       // porque el usuario retrocede o sale directamente utilizando el ratón, hay que limpiarlo de nuevo para evitar
       // que se guarde un valor no validado.

       If   ( not FIsEditValidated and FModifiedOnEnter ) or AUserIsCancelingForm
       then begin
            FModifiedOnEnter := False;
            If   IsInplace and CanModify and CanPostEditValue
            then Reset;
            end;
  ...
  try
    ...
    If   ActiveProperties.CanValidate
    then begin

         CheckRequired;

         If    not FValidationError
	        then  begin
               DoValidateDisplayValue(ADisplayValue, FValidationErrorText, FValidationError);
               If   not FValidationError
               then CheckRequired;  // Se puede haber suprimido el contenido en la validación
               end;

         AValidationOptions := ActiveProperties.FInternalValidationOptions;
         If   ((FValidationError <> APrevValidationError) or FValidationError) and HandleAllocated
		       then PostMessage(Handle, DXM_SHORTREFRESHCONTAINER, 0, 0);
         end;
//..
    ...
    begin
      if (FModifiedOnEnter or FModifiedAfterEnter or not ValueEquals(ADisplayValue, GetValueToValidate) or not FIsEditValidated ) and
         not (IsUserAction and not FEditing) then //.. DoEditing? Esto provoca que se entre en modo edición incluso despues de un Post
      begin
...
function TcxCustomEdit.GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass;
begin
  Result := TdxEditAccessibilityHelper;
end;
...
function TcxCustomEdit.GetBackgroundStyle: TcxControlBackgroundStyle;
begin
  Result := bgNone;
end;
...
function TcxCustomEdit.GetPaintBlackOpaqueOnGlass: Boolean;
begin
  Result := IsTransparentBackground; 
end;
...
begin
  ...
  then begin
       FOnShowEditControlPopup( Self, ecpMouseClicked );
       Exit;
  end;
 //..
  ...
  begin
    ...
    if (AButtonVisibleIndex <> -1) and (ViewInfo.ButtonsInfo[AButtonVisibleIndex].Data.RawState = ebsPressed) then
...
begin
  ...
  then If   AComponent = FRepositoryItem
       then RepositoryItem := nil
       else If   AComponent=FCaptionLabel
...
begin
  ...
  begin
    ...
    begin
      if ViewInfo.ParentBackgroundProvider = nil then 
        ViewInfo.ParentBackgroundProvider := Parent;  
...
begin
     ...
     then If   not FCaptionLabel.Style.ReadOnly  //.. Una forma como otra de evitar que se desactive
          then FCaptionLabel.Enabled := Value;

     If   Assigned( FDescriptionLabel )
     then If   not FDescriptionLabel.Style.ReadOnly
          then FDescriptionLabel.Enabled := Value;
...
procedure TcxCustomEdit.VisualRefinementsListenerChanged;
begin
  AdjustInnerEditPosition;
  ContainerStyleChanged(nil);
end;

procedure TcxCustomEdit.ScaleFactorChanged;
begin
...
procedure TcxCustomEdit.LookAndFeelChanged(Sender: TcxLookAndFeel; AChangedValues: TcxLookAndFeelValues);
begin
  if not FIsCreating then
    InitContentParams;
...
begin
  ...
  begin
    ...
    AIsSizeChanged := (ANewWidth <> Width) or (ANewHeight <> Height);
    SetBounds(ALeft, ATop, ANewWidth, ANewHeight);
    AdjustInnerEditPosition;
    if AIsSizeChanged then
      cxRecalculateAutoSize(Parent);
  end;
end;
...
begin
  ...
  if (APart = ecpNone) or (APart = ecpControl) then
    Result := Hint
  else
...
begin
  Result := CanAutoSize and CanAutoHeight and AutoHeight and not ((akTop in Anchors) and (akBottom in Anchors));
end;

function TcxCustomEdit.IsAutoWidth: Boolean;
begin
  Result := CanAutoSize and CanAutoWidth and AutoWidth and not ((akLeft in Anchors) and (akRight in Anchors));
end;
...
begin
  Result := not (CanAutoSize and CanAutoHeight);
end;
  ...
  if IsInplace then
    ...
    begin
      ...
      begin
        ...
        Result := Result and DataBinding.DefaultValuesProvider.DefaultCanModify and
          DataBinding.DefaultValuesProvider.CanSetEditMode;
      end;
    end;
end;
...
begin
  Result := ActiveProperties.CreateViewData(ActiveStyle, IsInplace, ScaleFactor);
  ...
  begin
    ...
    Result.ForceDrawAsStandalone := ViewInfo.ForceDrawAsStandalone;
  end;
...
procedure TcxCustomEdit.DisconnectNonShareableProperties;
begin
  InternalStoreEditValue(Null);
end;
...
procedure TcxCustomEdit.DoButtonAction(AButtonVisibleIndex: Integer);
var
...
begin
  DisableUserAction;
  try
    DoButtonAction(AButtonVisibleIndex);
  finally
    EnableUserAction;
  end;
end;
...
begin
  if IsLoading or IsDestroying then
    Exit;
  //..
  if   Editing and not IsInplace and not FInternalSettingValue and not FDescriptionCleared
...
begin  
  ...
  FIsHiding := True;
  try
    If   IsDestroying or FValidateErrorProcessing 
    then Exit;

    If   not ( FCancelValidation or
               ClosingWindow or
               // No se puede ampliar a los controles estáticos porque si no se ha modificado antes ningún valor del registro no se
               // realiza la validación aunque el control sera Required (es porque, al no haber modificaciones previas, CanPostEditValue es False)
               ( IsInplace and ( ( not EditModified and not ActiveProperties.Required ) or not CanPostEditValue ) ) )
    then begin
         LockChangeEvents(True);
         try
           If   not InternalValidateEdit
           then Exit;
           If   not IsInplace and CanPostEditValue
           then InternalPostEditValue;
         finally
           LockChangeEvents( False );
           end;
         end;

    If   UpdateContentOnFocusChanging
...
begin
     ...
     try
       ...
       If   not FValidateErrorProcessing 
       then begin
            inherited DoEnter;
            If   UpdateContentOnFocusChanging 
            then DataBinding.UpdateDisplayValue;
       ...
       If   ActiveProperties.IsResetEditClass 
       then PrevEditValue := EditValue;
       
       DoOnEnter; //..
       
       If   not FValidateErrorProcessing and    //..
            UpdateContentOnFocusChanging 
       then SynchronizeDisplayValue;

     finally

       // No considero que un cambio desde Unassigned a Null sea una modificacion

       FModifiedOnEnter := not ( VarIsNull( EditValue ) and ( PrevEditValue=Unassigned ) ) and not VarEqualsExact( EditValue, PrevEditValue );
...
procedure TcxCustomEdit.DoPostEditValueNotification;
begin
     If   EditModified
     then DoPostEditValue;
end;
...
begin
  Result := not IsDesigning and (ActiveProperties.UseMouseWheel or (ssCtrl in Shift)) and Focused;
end;

procedure TcxCustomEdit.HidePopupWindowOnScroll;
begin
end;

function TcxCustomEdit.NeedAccessibilityHelper: Boolean;
begin
  Result := not (dxUIAutomationEnabled and SupportsUIAutomation);
end;
...
begin
  ...
  CreateDblClickTimer;
  if not IsInplace then
    Properties.FDefaultValuesProvider := FDataBinding.DefaultValuesProvider;
...
procedure TcxCustomEdit.InitializeInfoForHiddenInplaceEditor(AEditViewData: TcxCustomEditViewData;
  AEditViewInfo: TcxCustomEditViewInfo);
begin
  AEditViewData.Focused := Focused;
  if Properties.UseLeftAlignmentOnEditing then
    AEditViewData.HorzAlignment := taLeftJustify;
  AEditViewData.SelBackgroundColor := clHighlight;
  AEditViewData.SelTextColor := clHighlightText;
  UpdateInfoForHiddenInplaceEditor(AEditViewData, AEditViewInfo);
end;
...
procedure TcxCustomEdit.InitializeNonShareableProperties;
begin
end;
...
begin
  ...
  AViewData.PaintOnGlass := csGlassPaint in ControlState;
...
procedure TcxCustomEdit.UpdateInfoForHiddenInplaceEditor(AViewData: TcxCustomEditViewData; AEditViewInfo: TcxCustomEditViewInfo);
begin
  AViewData.EditValueToDrawValue(EditingValue, AEditViewInfo);
end;
...
begin
  ...
  begin
    ...
    Result.FDefaultValuesProvider := DataBinding.DefaultValuesProvider;
  end;
end;
...
begin
     try
       ...
       If   IsUserAction and not DoEditing
       then Exit;
       ...
       then begin
            If   FEditing and not FIsKeyPressed
...
begin
  Result := DataBinding.DefaultValuesProvider.IsDataStorage;
end;
...
begin
  Result := not ModifiedAfterEnter and (csPaintCopy in ControlState) and IsDBEdit and IsParentDBCtrlGrid and not cxIsTransparentBackgroundDrawing(Self);
end;
...
begin
  with ActiveProperties do
    Result := (DefaultValuesProvider = nil) or
      DefaultValuesProvider.IsValidChar(AChar) or
      DefaultValuesProvider.IsOnSetTextAssigned;
end;
...
begin
  Result := IsDestroying or ((GetInnerEditClass <> nil) and (InnerEdit = nil));
end;
...
begin
  Result := not(not DataBinding.Modified and CanKeyDownModifyEdit(Key, Shift) and not DoEditing);
end;
...
function TcxCustomEdit.GetAutomationSettings: TdxAutomationElementSettings;
begin
  if Assigned(UIAProvider()) then
    Result := Properties.Automation
  else
    Result := nil;
end;
...
function TcxCustomEdit.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
...
function TcxCustomEdit.GetIInnerEditAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FInnerEdit <> nil then
    Result := FInnerEdit.GetIAccessibilityHelper
  else
    Result := nil;
end;
...
function TcxCustomEdit.GetStyleReadOnly: TcxEditStyle;
begin
  Result := TcxEditStyle(FStyles.StyleReadOnly);
end;

procedure TcxCustomEdit.IMEComposition(var AMessage: TMessage);
begin
  if HasInnerControl then
    AMessage.Result := SendMessage(InnerControl.Handle, AMessage.Msg, AMessage.WParam, AMessage.LParam);
end;

procedure TcxCustomEdit.IMEStartComposition;
begin
  if HasInnerControl then
    SendMessage(InnerControl.Handle, WM_IME_STARTCOMPOSITION, 0, 0);
end;
...
begin
  ...
  try
    ...
    FreeAndNil(AViewData);
    if HasOwnScrollbars then 
    begin
      if Supports(Self, IdxScrollingControl, AIntf) then
      begin
        UpdateScrollBars;
        TdxScrollHelper.CheckPositions(AIntf);
      end;
    end;
  end;
end;
...
var
  ...
  AViewData: TcxCustomEditViewData;
begin
  if CanAutoSize and ViewInfo.Calculated then
  begin
    ...
    AViewData := TcxCustomEditViewData(CreateViewData);
    try
      ASize := AViewData.GetEditSize(cxMeasureCanvas, EditValue, AEditSizeProperties, ViewInfo);
    finally
      FreeAndNil(AViewData);
    end;
...
begin
  ...
  begin
    if ComponentState * [csLoading, csDestroying] = [] then
      Properties.OnPropertiesChanged := PropertiesChanged;
    Properties.DefaultValuesProvider := DataBinding.DefaultValuesProvider;
  end
  else
  begin
    Properties.FDefaultValuesProvider := nil;
...
procedure TcxCustomEdit.SetStyleReadOnly(Value: TcxEditStyle);
begin
  FStyles.StyleReadOnly := Value
end;
...
procedure TcxCustomEdit.VisibleChanging;
begin
     If   Assigned( FCaptionLabel )
     then FCaptionLabel.Visible := not Visible;
     If   Assigned( FDescriptionLabel )
     then FDescriptionLabel.Visible := not Visible;
end;
...
procedure TcxCustomEdit.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
  AAccessibilityHelper: TdxEditAccessibilityHelper;
begin
  if NeedAccessibilityHelper and CanReturnAccessibleObject(Message) then
  begin
    ...
    begin
      AAccessibilityHelper := TdxEditAccessibilityHelper(IAccessibilityHelper.GetHelper);
      if (AObjectID > 0) and (AObjectID <= Cardinal(AAccessibilityHelper.AccessibilityChildren.Count)) then
        Message.Result := WMGetObjectResultFromIAccessibilityHelper(Message, IcxAccessibilityHelper(AAccessibilityHelper.AccessibilityChildren[AObjectID - 1]))
      else
        inherited;
    end;
  end
...
function cxEditErrorIcon: TdxSmartGlyph;
begin
  if FcxEditErrorIcon = nil then
  begin
    FcxEditErrorIcon := TdxSmartGlyph.Create;
    if dxUseVectorIcons then
      FcxEditErrorIcon.LoadFromResource(HInstance, 'CXEDIT_GLYPH_ERROR', 'SVG')
    else
      FcxEditErrorIcon.LoadFromResource(HInstance, 'CXEDIT_GLYPH_ERROR', RT_BITMAP);
  end;
  Result := FcxEditErrorIcon;
end;

function cxEditWarningIcon: TdxSmartGlyph;
begin
  if FcxEditWarningIcon = nil then
  begin
    FcxEditWarningIcon := TdxSmartGlyph.Create;
    if dxUseVectorIcons then
      FcxEditWarningIcon.LoadFromResource(HInstance, 'CXEDIT_GLYPH_WARNING', 'SVG')
    else
      FcxEditWarningIcon.LoadFromResource(HInstance, 'CXEDIT_GLYPH_WARNING', RT_BITMAP);
  end;
  Result := FcxEditWarningIcon;
end;

function cxEditInfoIcon: TdxSmartGlyph;
begin
  if FcxEditInfoIcon = nil then
  begin
    FcxEditInfoIcon := TdxSmartGlyph.Create;
    if dxUseVectorIcons then
      FcxEditInfoIcon.LoadFromResource(HInstance, 'CXEDIT_GLYPH_INFO', 'SVG')
    else
      FcxEditInfoIcon.LoadFromResource(HInstance, 'CXEDIT_GLYPH_INFO', RT_BITMAP);
  end;
  Result := FcxEditInfoIcon;
end;

procedure RegisterAssistants;
begin
  dxRegisterCursor(crcxEditMouseWheel, HInstance, 'CXEDIT_MOUSEWHEEL');
...
begin
  dxUnregisterCursor(crcxEditMouseWheel);
  FreeAndNil(FcxEditErrorIcon);
  FreeAndNil(FcxEditWarningIcon);
  FreeAndNil(FcxEditInfoIcon);
...
procedure TcxEditSizeProperties.InitValues(AMaxLineCount, AHeight, AWidth: Integer; AScaleFactor: TdxScaleFactor = nil);
begin
  MaxLineCount := AMaxLineCount;
  Height := AHeight;
  Width := AWidth;
...
finalization
  ...
  dxFreeGlobalObject(FDefaultEditRepository);
  dxFreeGlobalObject(FRegisteredEditProperties);
  dxFreeGlobalObject(FCreatedEditPropertiesList);
  dxFreeGlobalObject(FEditPropertyIcons);
