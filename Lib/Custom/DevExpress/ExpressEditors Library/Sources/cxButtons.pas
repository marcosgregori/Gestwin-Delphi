...
uses
  Winapi.Windows, Winapi.Messages, System.Types, System.Classes, Vcl.Controls, Vcl.Graphics, Vcl.StdCtrls, Vcl.Forms, Vcl.Menus,
  Vcl.ImgList, Vcl.Buttons, Winapi.CommCtrl,
  dxCore, dxMessages, dxThemeManager, cxGeometry, cxGraphics, cxLookAndFeels, dxGDIPlusClasses,
  cxLookAndFeelPainters, cxControls, cxContainer, cxClasses, dxFading, dxCoreGraphics, dxCoreClasses, cxAccessibility,
...
type
  ...
  public
    procedure DrawButton(ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState; AFocused: Boolean; AColor: TColor; APart: TcxButtonPart); overload; virtual;
    procedure DrawButton(ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState; AColor: TColor; APart: TcxButtonPart); overload; virtual;
    ...
    property ViewInfo: TcxButtonViewInfo read GetViewInfo; // for internal use
  end;
  ...
  public
    ...
    property ButtonGlyph: TcxButtonGlyph read GetButtonGlyph; // for internal use
    ...
    property Painter: TcxButtonPainter read FPainter; // for internal use
...
// for navigation
  ...
  {$ENDIF}
    ...
    function GetButtonDefault: Boolean;
...
uses
  System.SysUtils, Vcl.Consts, Vcl.Dialogs, System.Math, Vcl.ActnList,
...
type
  ...
  TdxButtonAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
  ...
  TdxUIACustomButtonProvider = class(TdxUIABaseWindowProvider<TcxCustomButton>, dxUIA.IInvokeProvider)
  protected
    function GetDefaultAccessKey: string; override;
    ...
    // IInvokeProvider
    procedure InternalInvoke; override;
  end;
...
begin
  if (Sender = Images) and not IsGlyphAssigned(Glyph) then
    GlyphChanged;
end;
...
procedure TcxSpeedButtonOptions.InternalSetDown(AValue: Boolean);
  ...
  function CanChangeDownState: Boolean;
  begin
    Result := not Down or FAllowAllUp or (FGroupIndex = 0) or (LockCount <> 0) or Assigned(Button.Action) or
...
begin
  ...
  if (AValue <> Down) and CanChangeDownState then
  begin
    if AValue then
    begin
...
begin
  if not Assigned(Button.Action) or
    (Assigned(Button.Action) and ((GroupIndex <> 0) and (not Down or AllowAllUp) or ((GroupIndex = 0) and Down))) then
...
{ TdxButtonAccessibilityHelper }

function TdxButtonAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID = 0 then
    Result := Button.ViewInfo.Bounds
  else
    Result := Button.ViewInfo.DropDownButtonRect;

  Result := cxRectSetOrigin(Result, Button.ClientToScreen(Result.TopLeft));
end;

function TdxButtonAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := True;
end;

procedure TdxButtonAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if AChildID = 0 then
    Button.Click
  else
    Button.DoDropDownMenu;
end;

function TdxButtonAccessibilityHelper.GetChildCount: Integer;
begin
  if Button.Kind = cxbkDropDownButton then
    Result := 1
  else
    Result := 0;
end;

function TdxButtonAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Click';
end;

function TdxButtonAccessibilityHelper.GetDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := Button.Description;
end;

function TdxButtonAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID = 0 then
  begin
...
function TdxButtonAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := Button.Handle;
end;

function TdxButtonAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID = 1 then
    Result := cxROLE_SYSTEM_BUTTONDROPDOWN
  else
    if (Button.Kind = cxbkDropDownButton) and (AChildID = 0) then
      Result := cxROLE_SYSTEM_SPLITBUTTON
    else
      Result := cxROLE_SYSTEM_PUSHBUTTON;
end;

function TdxButtonAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  ...
  if AChildID = 0 then
  begin
    if Button.SpeedButtonOptions.CanBeFocused then
      Result := Result or cxSTATE_SYSTEM_FOCUSABLE;
    if Button.ViewInfo.State = cxbsHot then
      Result := Result or cxSTATE_SYSTEM_HOTTRACKED;
    if (AChildID = 0) and Button.Focused then
      Result := Result or cxSTATE_SYSTEM_FOCUSED;
  end;
end;

function TdxButtonAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation, aopDefaultAction, aopDescription];
end;

function TdxButtonAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxButtonAccessibilityHelper.GetButton: TcxCustomButton;
begin
  Result := TcxCustomButton(OwnerObject);
end;
...
begin
  ...
  FShowFocusRect := True;
  DoubleBuffered := True;
  ControlStyle := ControlStyle + [csOverrideStylePaint, csReflector, csOpaque];
end;
...
function TcxCustomButton.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := TdxButtonAccessibilityHelper.Create(Self);
...
function TcxCustomButton.IsDestroying: Boolean;
begin
...
function TcxCustomButton.SupportsUIAutomation: Boolean;
begin
  Result := False;
end;
...
begin
  AExecuteClick := (bisPressed in InternalStates) or ([bisPressed, bisMenuButtonPressed] * InternalStates = []);
  if SpeedButtonMode and CanBeFocused and AExecuteClick and not (bisAccelExecute in InternalStates) and
    (not Assigned(Action) or (Assigned(Action) and (not Down or AllowAllUp))) then
    SetDown(not Down);
...
begin
  Result := FFade and ( (Painter.LookAndFeelStyle in [lfsSkin, lfsNative]) and                          //..
...
begin
  ...
  if ACaption = '' then
    ACaption := ' ';
  Result.cx := cxMeasureCanvas.TextWidth(ACaption) + cxMeasureCanvas.TextWidth('R') * 3;
  Result.cy := MulDiv(TdxTextMeasurer.TextLineHeight(cxMeasureCanvas.Handle), 14, 8);
end;

function TcxCustomButton.UIAProvider: IdxUIAProvider;
begin
...
function TcxCustomButton.GetButtonDefault: Boolean;
begin
  Result := FIsPaintDefault;
end;
  ...
  case Kind of
    ...
    cxbkDropDownButton:
      if BiDiMode = bdRightToLeft then
        Result := cxRectSetLeft(ClientRect, ClientRect.Left, ViewInfo.Painter.GetDropDownButtonSize)
      else
...
function TcxCustomButton.GetAutomationSettings: TdxAutomationElementSettings;
begin
...
function TcxCustomButton.GetAutomationSettingsClass: TdxAutomationElementSettingsClass;
begin
...
function TcxCustomButton.GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider;
begin
...
procedure TcxCustomButton.SetAutomation(
...
var
  AExecuteClick, AMenuButtonPressed: Boolean;
begin
  FButtonPressTimer.Enabled := False;
  AMenuButtonPressed := ((bisMenuButtonPressed in InternalStates) and (Kind in [cxbkDropDown, cxbkOfficeDropDown]));
  AExecuteClick := not CanBeFocused and (Button = mbLeft) and (bisHot in InternalStates) and
    ((bisPressed in InternalStates) or AMenuButtonPressed);
  if (GroupIndex <> 0) and AExecuteClick and
    (not Assigned(Action) or (Assigned(Action) and not AMenuButtonPressed and (not Down or AllowAllUp))) then
    SetDown(not Down);
...
function TcxCustomButton.NeedAccessibilityHelper: Boolean;
begin
...
begin
  ...
  FIsPaintDefault := ((DrawItemStruct.itemState and ODS_FOCUS <> 0) or
    (Default and IsDesigning) or FIsFocused) and CanBeFocused and not FSpeedButtonOptions.Flat;  //..
  CheckStartFading(APrevButtonState, GetButtonState);
...
procedure TcxCustomButton.BeforeDestruction;
begin
  if Assigned(FUIAProvider) then
...
begin
  inherited;
  if not IsDesigning and (WindowFromPoint(GetMouseCursorPos) <> Handle) then
...
procedure TcxCustomButton.WMDestroy(var Message: TWMDestroy);
begin
  if not cxIsDestroying(Self, True) and (bisHot in InternalStates) then
  begin
    MouseLeave;
    Perform(WM_MOUSELEAVE, 0, LPARAM(Self));
  end;
  if Assigned(FUIAProvider) then
  begin
...
procedure TcxCustomButton.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
...
function TcxCustomButton.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIACustomButtonProvider.Create(Self);
end;
...
begin
  ...
  begin
    ...
    FTextAreaSize := cxSizeAddMargins(FTextAreaSize, ATextOffsets);
  end;
end;
...
begin
  ...
  begin
    ...
    FTextAreaSize := cxSizeAddMargins(cxRectSize(ATextBounds), ATextOffsets);
  end
  else
    FTextAreaSize := cxNullSize;
end;
...
begin
  FBounds := ABounds;
  FDefault := Button.GetButtonDefault;
  FState := Button.GetButtonState;
  FFocused := Button.Focused;
...
begin
  if HasDropDownButton then
  begin
    Painter.DrawButton(ACanvas, ButtonRect, State, IsDefault, Color, cxbpDropDownLeftPart);
    Painter.DrawButton(ACanvas, DropDownButtonRect, DropDownButtonState, IsDefault, Color, cxbpDropDownRightPart);
  end
  else
    Painter.DrawButton(ACanvas, ButtonRect, State, IsDefault, Color, cxbpButton);
end;
...
procedure TcxButtonViewInfo.DrawGlyph(ACanvas: TcxCanvas);
  ...
  begin
    TdxImageDrawer.DrawImage(ACanvas, AImageBounds, ButtonGlyph.Glyph, ButtonGlyph.Images, ButtonGlyph.ImageIndex,
...
begin
  ...
  Result := cxRectInflate(ButtonRect, -ABorderSize);
end;
...
var
  ADefaultFontColor: TColor;
begin
  ADefaultFontColor := Painter.GetTextColor(State, Button.Font.Color);
  Font.Assign(Button.Font);
  Font.Color := ADefaultFontColor;
  if (Kind = cxbkCommandLink) then
  begin
    Font.Height := MulDiv(Button.Font.Height, 12, 10);
  end;
  DescriptionFont.Assign(Button.Font);
  DescriptionFont.Color := Painter.GetDescriptionTextColor(State, ADefaultFontColor);
...
const
  Map: array[Boolean] of TcxButtonPart = (cxbpButton, cxbpDropDownLeftPart);
begin
  Result := Button.Painter.ButtonColorPalette(State, Map[HasDropDownButton])
end;
...
procedure TcxButtonPainter.DrawButton(ACanvas: TcxCanvas; const R: TRect;
  AState: TcxButtonState; AFocused: Boolean; AColor: TColor;
  APart: TcxButtonPart);
begin
  if Button.Kind = cxbkCommandLink then
    ...
    if Button.SpeedButtonOptions.NeedDrawParts then
      ...
      LookAndFeelPainter.DrawScaledButton(ACanvas, R, AState, AFocused, ScaleFactor, True, False, APart, AColor);
end;

procedure TcxButtonPainter.DrawButton(ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState; AColor: TColor; APart: TcxButtonPart);
begin
  if Button.Kind = cxbkCommandLink then
    DrawCommandLinkButton(ACanvas, R, AState, AColor)
  else
    if Button.SpeedButtonOptions.NeedDrawParts then
      ...
      LookAndFeelPainter.DrawScaledButton(ACanvas, R, AState, ScaleFactor, APart, AColor);
end;
...
begin
  if LookAndFeelPainter.SupportsNativeFocusRect(TdxPaintPartID.Button) then
    ...
    lfsNative:
      begin
        ...
        if (ATheme <> TC_NONE) and IsThemeBackgroundPartiallyTransparent(ATheme, BP_PUSHBUTTON, PBS_NORMAL) then
          ...
          if Button.SpeedButtonOptions.Transparent then
            cxDrawTransparentControlBackground(Button, ACanvas, R);
      end;
...
begin
  ...
  begin
    ...
    R := cxRectInflate(R, -LookAndFeelPainter.ButtonBorderSize(AState));
  end;
  ...
  begin
    ...
    try
      ...
      LookAndFeelPainter.DrawScaledButton(ACanvas, R, AState, ScaleFactor, False, True, APart, AColor)
    finally
      ACanvas.RestoreClipRegion;
    end;
  end;
...
begin
  ...
  begin
    if Button.Kind = cxbkCommandLink then
      ...
      if ViewInfo.HasDropDownButton and ((LookAndFeelStyle <> lfsSkin) or (AState <> cxbsDisabled)) then
        Result := LookAndFeelPainter.ButtonSymbolColor(AState, ADefaultColor, cxbpDropDownLeftPart)
      else
        Result := LookAndFeelPainter.ButtonSymbolColor(AState, ADefaultColor, cxbpButton);
  end;
end;
...
{ TdxUIACustomButtonProvider }

function TdxUIACustomButtonProvider.GetControlTypePropertyValue(
  out AValue: Integer): Boolean;
begin
  AValue := dxUIA.UIA_ButtonControlTypeId;
  Result := True;
end;

function TdxUIACustomButtonProvider.GetDefaultAccessKey: string;
begin
  Result := dxUIAGetAccelledLabelAccessKey(Control.Caption);
end;

function TdxUIACustomButtonProvider.GetDefaultAriaRole: string;
begin
  Result := 'button';
end;

function TdxUIACustomButtonProvider.GetDefaultAutomationName: string;
begin
  Result := dxUIAGetAccelledLabelText(Control.Caption);
end;

function TdxUIACustomButtonProvider.GetDefaultIsContentElement: TdxDefaultBoolean;
begin
...
function TdxUIACustomButtonProvider.GetSupportedPatterns: TArray<Integer>;
begin
  Result := [dxUIA.UIA_InvokePatternId];
end;

procedure TdxUIACustomButtonProvider.InternalInvoke;
begin
  Control.Click;
end;
...
function TcxButton.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;
