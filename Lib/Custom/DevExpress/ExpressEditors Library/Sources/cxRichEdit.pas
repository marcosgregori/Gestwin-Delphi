...
uses
  System.Variants, Winapi.Windows, Winapi.Messages, Winapi.ActiveX, Winapi.OleDlg, Vcl.OleConst, Vcl.OleCtnrs,
  System.Classes, Vcl.Clipbrd, Vcl.ComCtrls, Vcl.Controls, Vcl.Dialogs, Vcl.Forms, Vcl.Graphics, Vcl.Menus,
  Winapi.RichEdit, Vcl.StdCtrls, System.SysUtils, System.Generics.Defaults, System.Generics.Collections,
  ...
  dxDrawRichTextUtils, cxGeometry, dxDPIAwareUtils, cxAccessibility, dxAI;
...
type
  ...
  protected
    ...
    function SupportsMultiThreading(AUseDisplayValue: Boolean): Boolean; override;
  ...
  protected
    function AddMenuItem(const AParentMenuItem: TMenuItem; const ACaption: string; ATag: Integer; AEnabled: Boolean = True): TMenuItem;
    procedure AdjustInnerEdit; override;
    procedure AdjustInnerEditFont;
    ...
    procedure SetScrollBarVisible(AScrollBar: IcxControlScrollBar; AVisible: Boolean); override;
    ...
    function getText(const AAICommandID: string; ASelectedOnly: Boolean): string; override;
    procedure BuildAISubmenu(const AMenu: TMenuItem; const AAICommands: TdxAICommandList);
  ...
  { TdxRichInnerEditAccessibilityHelper }

  TdxRichInnerEditAccessibilityHelper = class(TdxCustomInnerTextEditAccessibilityHelper);
  ...
  private
    ...
    FRightMouseUpProcessing: Boolean;
    ...
    FSelectionBar: Boolean;
    FStreamModes: TcxRichEditStreamModes;
    FUpdatingHideSelectionState: Boolean;
    ...
    procedure ForceUpdateHideSelectionState;
    ...
    procedure WMAfterDpiChange(var Message: TMessage); message WM_DPICHANGED_AFTERPARENT;
    procedure WMBeforeDpiChange(var Message: TMessage); message WM_DPICHANGED_BEFOREPARENT;
  ...
  protected
    ...
    function ScaleViaZoomFactor: Boolean;

    procedure ChangeScale(M, D: Integer{$IFDEF DELPHIBERLIN}; isDpiChange: Boolean{$ENDIF}); override;
  {$IFDEF DELPHI110}
  ...
  {$ENDIF}
    ...
    property RightMouseUpProcessing: Boolean read FRightMouseUpProcessing;
...
function GetAdjustedRichEditText(const ARichEdit: TcxRichInnerEdit): string; overload;
function GetAdjustedRichEditText(const ARichEdit: TcxCustomRichEdit): string; overload;
function GetAdjustedRichEditText(const AText: string; AUseShortLineBreak: Boolean): string; overload;
...
uses
  Winapi.CommDlg, Vcl.Printers, System.Math, System.Types, System.Win.ComObj, Winapi.CommCtrl,
  System.AnsiStrings, 
  dxSpellCheckerCore, cxEditPaintUtils, cxEditUtils, cxVariants, cxDWMAPI, dxUxTheme, dxThemeConsts, dxThemeManager,
  dxCoreGraphics, dxForms;

const
  dxThisUnitName = 'cxRichEdit';
...
type
  ...
  TdxRichEditDllLoader = class
  private
    FRichEditLibrary: array[TcxRichEditClass] of THandle;
  protected
    function GetRichEditClassName(ARichEditClass: TcxRichEditClass; var ARichEditVersion: Integer): string;
  public
    destructor Destroy; override;
  end;
...
var
  FRichRenderer: TcxRichRenderer;
  FRichConverter: TcxRichRenderer;
  FRichEditDllLoader: TdxRichEditDllLoader;
...
begin
  ...
  SetWindowPos(Wnd, 0,
    (GetSystemMetrics(SM_CXSCREEN) - ARect.Width) div 2,
    (GetSystemMetrics(SM_CYSCREEN) - ARect.Height) div 3,
    0, 0, SWP_NOACTIVATE or SWP_NOSIZE or SWP_NOZORDER);
end;

{$IFDEF DELPHI110}
function cxOleDialogHook(Wnd: HWnd; Msg: UINT; AWParam: WPARAM; ALParam: LPARAM): UINT; stdcall;
{$ELSE}
...
function GetAdjustedRichEditText(const ARichEdit: TcxRichInnerEdit): string;
begin
  Result := GetAdjustedRichEditText(ARichEdit.Text, ARichEdit.RichVersion >= 200);
end;

function GetAdjustedRichEditText(const ARichEdit: TcxCustomRichEdit): string;
begin
  Result := GetAdjustedRichEditText(ARichEdit.InnerRich);
end;
...
function RichEditDllLoader: TdxRichEditDllLoader;
begin
  if FRichEditDllLoader = nil then
    FRichEditDllLoader := TdxRichEditDllLoader.Create;
  Result := FRichEditDllLoader;
end;
...
var
  ARichRenderer: TcxRichRenderer;
begin
  if AScaleFactor = nil then
    AScaleFactor := AProperties.ScaleFactor;

  ARichRenderer := RichRenderer;
  ARichRenderer.RichEditClass := TcxCustomRichEditProperties(AProperties).RichEditClass;
  ARichRenderer.MemoMode := TcxCustomRichEditProperties(AProperties).MemoMode;
  ARichRenderer.PlainText := TcxCustomRichEditProperties(AProperties).PlainText;
  if ARichRenderer.Alignment <> TcxCustomRichEditProperties(AProperties).Alignment then
  begin
    ARichRenderer.Alignment := TcxCustomRichEditProperties(AProperties).Alignment;
    ARichRenderer.Paragraph.Alignment := ARichRenderer.Alignment; 
  end;
  ARichRenderer.AutoURLDetect := TcxCustomRichEditProperties(AProperties).AutoURLDetect;
  ARichRenderer.AllowObjects := TcxCustomRichEditProperties(AProperties).AllowObjects;
  ARichRenderer.AdvancedTypography := TcxCustomRichEditProperties(AProperties).AdvancedTypography;
  ARichRenderer.HandleNeeded;
  ARichRenderer.ZoomFactor := TcxCustomRichEditProperties(AProperties).ZoomFactor;
  ARichRenderer.ScaleFactor.Assign(AScaleFactor);

  SetRichDefAttributes(ARichRenderer, AFont, ATextColor);

  if ARichRenderer.MemoMode then
    ARichRenderer.Text := AText
  else
    dxRichLoadFromString(ARichRenderer.RichLines, AText);

  SendMessage(ARichRenderer.Handle, EM_SETBKGNDCOLOR, 0, ColorToRGB(AColor));
end;

procedure cxDrawRichEdit(ADC: HDC; const ARect: TRect; ARichHandle: HWND;
  AMinCharIndex, AMaxCharIndex: Integer; ACalculateHeight: Boolean; out AHeight: Integer);
begin
  dxDrawRichEdit(ADC, cxRectSetNullOrigin(ARect), ARichHandle, dxDefaultScaleFactor, AMinCharIndex, AMaxCharIndex, ACalculateHeight, AHeight);
end;

function cxRichEditCalculateBestFit(AText: string; AProperties: TcxCustomRichEditProperties; AFont: TFont; AScaleFactor: TdxScaleFactor): Integer;
var
...
begin
  ...
  if RichRenderer.RichVersion >= 200 then
  begin
    ZeroMemory(@AParagraph, SizeOf(AParagraph));
    AParagraph.cbSize := SizeOf(AParagraph);
    AParagraph.dwMask := PFM_RTLPARA or PFM_ALIGNMENT;
    AParagraph.wAlignment := PFA_LEFT;
    AParagraph.wReserved := 0;
    cxSendStructMessage(RichRenderer.Handle, EM_SETPARAFORMAT, 0, AParagraph);
  end
  else
    RichRenderer.Paragraph.Alignment := taLeftJustify;

{$REGION 'T1060430'}
  AText := GetAdjustedRichEditText(RichRenderer);
  if (AText <> '') and dxCharInSet(AText[Length(AText)], dxSpaceCharacters) then
  begin
    RichRenderer.SelStart := Length(AText) - 1;
    RichRenderer.SelLength := 1;
    RichRenderer.SelText := '_';
  end;
{$ENDREGION}

  Result := RichRenderer.RequestSizeRect.Width;
end;

procedure cxDrawRichEdit(ADC: HDC; const ARect: TRect; const AText: string; AProperties: TcxCustomRichEditProperties;
  AFont: TFont; AColor: TColor; ATextColor: TColor; ACalculateHeight: Boolean; out AHeight: Integer;
...
begin
  ...
  begin
    if (Screen.ActiveControl <> nil) and Screen.ActiveControl.HandleAllocated then
      Params.WndParent := Screen.ActiveControl.Handle
    else
...
{ TdxRichEditDllLoader }

destructor TdxRichEditDllLoader.Destroy;
var
  ARichClass: TcxRichEditClass;
begin
  for ARichClass := Low(FRichEditLibrary) to High(FRichEditLibrary) do
  begin
    if FRichEditLibrary[ARichClass] > HINSTANCE_ERROR then
      FreeLibrary(FRichEditLibrary[ARichClass]);
  end;
  inherited;
end;

function TdxRichEditDllLoader.GetRichEditClassName(ARichEditClass: TcxRichEditClass; var ARichEditVersion: Integer): string;
const
  cxRichEditClassNames: array [TcxRichEditClass] of PChar = ('RICHEDIT', 'RICHEDIT20W', 'RICHEDIT50W');
  cxRichEditVersions: array [TcxRichEditClass] of Integer = (100, 200, 410);
  cxRichEditDLLNames: array [TcxRichEditClass] of string = ('Riched32.dll', 'Riched20.dll', 'Msftedit.dll');
var
  ARichClass: TcxRichEditClass;
  AWndClass: TWndClass;
begin
  Result := '';
  for ARichClass := ARichEditClass downto cxMinVersionRichEditClass do
  begin
    if FRichEditLibrary[ARichClass] < HINSTANCE_ERROR then
      FRichEditLibrary[ARichClass] := LoadLibrary(PChar(cxRichEditDLLNames[ARichClass]));

    if GetClassInfo(HInstance, cxRichEditClassNames[ARichClass], AWndClass) then
    begin
      ARichEditVersion := cxRichEditVersions[ARichClass];
      Result := cxRichEditClassNames[ARichClass];
      Exit;
    end;
  end;
...
begin
  if ALock then
    ...
    if Edit.FLockBoundsCount > 0 then
...
function TcxRichInnerEditHelper.CallDefWndProc(AMsg: UINT; WParam: WPARAM; LParam: LPARAM): LRESULT;
begin
...
function TcxRichInnerEditHelper.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
...
begin
  ...
  FRichEdit.ApplyZoomFactor;
  if FRichEdit.Container <> nil then
    FRichEdit.Container.EditModified := False;
end;
...
begin
  ...
  begin
    Result := Text;
    while (Length(Result) > 0) and dxCharInSet(Result[Length(Result)], dxBreakLineCharacters) do
...
begin
  ...
  begin
    ...
    AActualAutoURLDetect := FAutoURLDetect and not MemoMode and ((Container = nil) or not Container.IsDesigning);
  {$IFDEF DELPHI110}
    EnableURLs := AActualAutoURLDetect;
  {$ELSE}
    SendMessage(Handle, EM_AUTOURLDETECT, WPARAM(AActualAutoURLDetect), 0);
  {$ENDIF}
...
begin
  ...
  begin
    ...
    InternalSetMemoMode;
    dxForceUpdateControlDPI(Self);
    ApplyZoomFactor;
  end;
  if Container <> nil then
    Container.LockChangeEvents(False, False);
end;
...
begin
     If   Assigned( Container.Properties.OnAnnotationRequest )      //..
     then Container.Properties.OnAnnotationRequest( Container )
     else begin
          inherited DblClick;
...
begin
  ...
  if (Key = VK_RETURN) and Assigned(TdxSpellCheckerInstance.ISpellChecker) then
    Invalidate;
  If   ( RichVersion>=200 ) and ( Key=VK_RETURN ) and not WantReturns
  then If   not( ssShift in KeyboardStateToShiftState ) and             //.. Shift + VK_RETURN sí que inserta un retorno de carro
            not( ssCtrl in KeyboardStateToShiftState )
       then Key := 0;
end;
...
begin
  ...
  if ( not WantTabs and (Key = Char(VK_TAB)) ) or
     ( not WantReturns and (Key = Char(VK_RETURN)) ) then //..
    Key := #0;                                  //..
...
procedure TcxRichInnerEdit.RichCreateParams(var Params: TCreateParams; out ARichVersion: Integer);
begin
  CreateSubClass(Params, PChar(RichEditDllLoader.GetRichEditClassName(RichEditClass, ARichVersion)));
end;
...
begin
  ...
  FRIghtMouseUpProcessing := Message.Msg = WM_RBUTTONUP;
  inherited WndProc(Message);
  FRIghtMouseUpProcessing := False;
end;
...
function TcxRichInnerEdit.ScaleViaZoomFactor: Boolean;
begin
  Result := not dxIsProcessDPIPerMonitorV2Aware or (RichEditClass <> recRichEdit41)
end;

procedure TcxRichInnerEdit.ChangeScale(M, D: Integer{$IFDEF DELPHIBERLIN}; isDpiChange: Boolean{$ENDIF});
begin
  if not (FIsInDpiChange or ScaleViaZoomFactor) then
  begin
    if HandleAllocated then
      SendMessage(Handle, WM_DPICHANGED_BEFOREPARENT, 0, 0);
    try
      inherited ChangeScale(M, D{$IFDEF DELPHIBERLIN}, isDpiChange{$ENDIF});
      ScaleFactor.Change(M, D);
    finally
      if HandleAllocated then
        SendMessage(Handle, WM_DPICHANGED_AFTERPARENT, 0, 0);
    end;
  end
  else
  begin
...
{$IFDEF DELPHI110}
procedure TcxRichInnerEdit.UpdateEditMargins;
begin
  // do nothing
end;
{$ENDIF}
...
function TcxRichInnerEdit.GetAccessibilityHelperClass: TdxCustomInnerTextEditAccessibilityHelperClass;
begin
  Result := TdxRichInnerEditAccessibilityHelper;
end;

procedure TcxRichInnerEdit.ForceUpdateHideSelectionState;
begin
  FUpdatingHideSelectionState := True;
  try
    SendMessage(Handle, WM_SETREDRAW, 0, 0);
    SendMessage(Handle, WM_KILLFOCUS, Container.Handle, 0);
    SendMessage(Handle, WM_SETFOCUS, 0, 0);
  finally
    SendMessage(Handle, WM_SETREDRAW, 1, 0);
    FUpdatingHideSelectionState := False;
  end;
end;
...
function TcxRichInnerEdit.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
...
procedure TcxRichInnerEdit.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
...
begin
  inherited;
  if FUpdatingHideSelectionState then
    Exit;
...
begin
  ...
  begin
    ...
    Winapi.Windows.DeleteObject(ARgn);
  end;
end;
...
begin
  if (Container <> nil) and (Message.WParam = SCF_SELECTION) and (SelLength > 0) then
    Container.DoEditing;
  inherited;
end;
...
procedure TcxRichInnerEdit.WMAfterDpiChange(var Message: TMessage);
begin
  inherited;
  FIsInDpiChange := True;
end;

procedure TcxRichInnerEdit.WMBeforeDpiChange(var Message: TMessage);
begin
  inherited;
  FIsInDpiChange := False;
end;
...
procedure TcxRichInnerEdit.EMHideSelection(var Message: TMessage);
begin
  inherited;
  if HideSelection and (RichEditClass = recRichEdit41) and Focused then
    ForceUpdateHideSelectionState;
end;
...
begin
  if Assigned(Properties.OnProtectChange) then
    Properties.OnProtectChange(Self, AStartPos, AEndPos, AAllowChange);
  if RepositoryItem <> nil then
    if Assigned(ActiveProperties.OnProtectChange) then
      ActiveProperties.OnProtectChange(Self, AStartPos, AEndPos, AAllowChange);
end;
...
begin
  if IsDestroying then
    Exit;
  if Assigned(Properties.OnSaveClipboard) then
    Properties.OnSaveClipboard(Self, ANumObjects, ANumChars, ASaveClipboard);
  if RepositoryItem <> nil then
    if Assigned(ActiveProperties.OnSaveClipboard) then
      ActiveProperties.OnSaveClipboard(Self, ANumObjects, ANumChars, ASaveClipboard);
end;

procedure TcxCustomRichEdit.EditPopupMenuClick(Sender: TObject);
var
  AMessageID: TdxNativeInt;
begin
  AMessageID := TdxNativeInt(TMenuItem(Sender).Tag);
  case AMessageID of
    -1: Undo;
    -2: InnerRich.Redo;
    -3: CutToClipboard;
    ...
    if (AMessageID >= DXM_POPUPMENUAICOMMANDFIRST) and (AMessageID <= DXM_POPUPMENUAICOMMANDLAST) then
      ExecuteAIMenuItem(AMessageID);
  end;
end;
...
function TcxCustomRichEdit.AddMenuItem(const AParentMenuItem: TMenuItem; const ACaption: string; ATag: Integer; AEnabled: Boolean = True): TMenuItem;
begin
  Result := TMenuItem.Create(Self);
  Result.Caption := ACaption;
  Result.Enabled := AEnabled;
  Result.Tag := ATag;
  Result.OnClick := EditPopupMenuClick;
...
function TcxCustomRichEditViewData.InternalGetEditContentSize(ACanvas: TcxCanvas;
...
begin
  if VarIsSoftNull(AEditValue) then
    Exit(inherited InternalGetEditContentSize(ACanvas, AEditValue, AEditSizeProperties));

  AText := dxVariantToString(AEditValue);

  AWidth := AEditSizeProperties.Width;
  if AWidth = -1 then
    AWidth := cxRichEditCalculateBestFit(AText, Properties, Style.Font, ScaleFactor);
  Result.Width := AWidth;
  ...
  try
    cxDrawRichEdit(ADC, Rect(0, 0, AWidth, 0), AText, Properties, Style.Font, clWhite, clBlack, True, AHeight, ScaleFactor);
  finally
    DeleteDC(ADC);
  end;

  if AHeight > 0 then
    Inc(AHeight, GetEditContentSizeCorrection.Height);
...
var
  P: TPoint;
begin
  if FEdit.RightMouseUpProcessing then
  begin
    P := GetMouseCursorPos;
...
function TcxCustomRichEditProperties.GetDisplayText(const AEditValue: TcxEditValue; AFullText: Boolean = False;
...
function TcxCustomRichEditProperties.GetEditValueSource(AEditFocused: Boolean): TcxDataEditValueSource;
begin
  Result := evsValue;
end;
...
function TcxCustomRichEditProperties.SupportsMultiThreading(AUseDisplayValue: Boolean): Boolean;
begin
  Result := False;
end;
...
procedure TcxCustomRichEditProperties.SetAllowObjects(const Value: Boolean);
begin
  if FAllowObjects <> Value then
  begin
    FAllowObjects := Value;
...
procedure TcxCustomRichEditProperties.SetAutoURLDetect(const Value: Boolean);
begin
  if Value <> FAutoURLDetect then
  begin
    FAutoURLDetect := Value;
...
begin
  inherited Create;
  PrevDrawBitmapSize.Width := -1;
  PrevDrawBitmapSize.Height := -1;
end;
...
begin
  if IsInplace or (BorderStyle = ebsNone) or not IsWinVistaOrLater then
    ...
    dxDrawThemeBackground(ACanvas.Handle, totEdit, ScaleFactor, Bounds, EP_EDITTEXT, ETS_NORMAL);
end;
...
procedure TcxCustomRichEditViewInfo.DrawText(ACanvas: TcxCanvas);
  ...
  begin
    ...
    begin
      if (DrawBitmap = 0) or (PrevDrawBitmapSize.Width <> TextRect.Width) or (PrevDrawBitmapSize.Height <> TextRect.Height) then
      begin
        if DrawBitmap <> 0 then
          DeleteObject(DrawBitmap);
        DrawBitmap := CreateCompatibleBitmap(ACanvas.Handle, TextRect.Width, TextRect.Height);
      end;
      ADC := CreateCompatibleDC(ACanvas.Handle);
      APrevBitmap := 0;
      try
...
begin
  ACanvas.Lock;
  try
    if IsInplace or IsDBEditPaintCopyDrawing then
    begin
      DrawText(ACanvas);
...
procedure TcxCustomRichEdit.InternalSetEditValue(const Value: TcxEditValue; AValidateEditValue: Boolean);
begin
...
function TcxCustomRichEdit.getText(const AAICommandID: string; ASelectedOnly: Boolean): string;
begin
  Result := InnerRich.SelText;
end;
...
procedure TcxCustomRichEdit.SetScrollBarVisible(AScrollBar: IcxControlScrollBar; AVisible: Boolean);
var
  AWasVisible: Boolean;
begin
  AWasVisible := AScrollBar.Visible;
  inherited SetScrollBarVisible(AScrollBar, AVisible);
  if AWasVisible <> AVisible then
...
procedure TcxCustomRichEdit.UpdateDefaultFont;
begin
...
procedure TcxCustomRichEdit.BuildAISubmenu(const AMenu: TMenuItem; const AAICommands: TdxAICommandList);
var
  I: Integer;
  AMessageID: Cardinal;
  AAICommand: TdxAICommand;
  ASubMenuItems: TDictionary<string, TMenuItem>;
  AParentMenuItem: TMenuItem;
  AAIMenuItem: TMenuItem;
  AAICommandInfo: TdxAICommandList.TCommand;
  AHasText: Boolean;
begin
  if not SupportsAI or (AAICommands.Count = 0) or (AMenu = nil) then
    Exit;

  AAIMenuItem := AMenu.Find(cxGetResourceString(@dxSAIAssistantCaption));
  if AAIMenuItem = nil then
    AddMenuItem(AMenu, '-', MaxInt);
  FreeAndNil(AAIMenuItem);
  AAIMenuItem := AddMenuItem(AMenu, cxGetResourceString(@dxSAIAssistantCaption), MaxInt);

  AMessageID := DXM_POPUPMENUAICOMMANDFIRST;
  FreeAndNil(FAICommandsMsgIDs);
  FAICommandsMsgIDs := TDictionary<Cardinal, TdxAICommand>.Create;
  ASubMenuItems := TDictionary<string, TMenuItem>.Create;
  try
    AHasText := getText('', True).Trim <> '';
    for I := 0 to AAICommands.Count - 1 do
    begin
      AAICommandInfo := AAICommands.GetCommandInfoByIndex(I);
      AAICommand := TdxAICommands.FindByID(AAICommandInfo.ID);
      if AAICommand = nil then
        Continue;
      if AAICommand.Category <> '' then
      begin
        if not ASubMenuItems.TryGetValue(AAICommand.Category, AParentMenuItem) then
        begin
          AParentMenuItem := AddMenuItem(AAIMenuItem, AAICommand.Category, MaxInt, AHasText);
          ASubMenuItems.Add(AAICommand.Category, AParentMenuItem);
        end;
      end
      else
        AParentMenuItem := AAIMenuItem;
      AddMenuItem(AParentMenuItem, AAICommand.Caption, AMessageID, AAICommandInfo.Available and AHasText);
      FAICommandsMsgIDs.Add(AMessageID, AAICommand);
      Inc(AMessageID);
      if AMessageID > DXM_POPUPMENUAICOMMANDLAST then
        Break;
    end;
  finally
    FreeAndNil(ASubMenuItems);
  end;
end;

procedure TcxCustomRichEdit.AdjustInnerEditFont;
var
  AVisibleFont: TFont;
begin
  if not IsInplace then
  begin
    AVisibleFont := VisibleFont;
    if not dxAreFontsEqual(InnerRich.Font, AVisibleFont) then
      InnerRich.Font := AVisibleFont;
  end;
end;
...
begin
  if Assigned(Properties.OnResizeRequest) then
    Properties.OnResizeRequest(Self, R);
  if RepositoryItem <> nil then
    if Assigned(ActiveProperties.OnResizeRequest) then
      ActiveProperties.OnResizeRequest(Self, R);
end;

procedure TcxCustomRichEdit.DoOnSelectionChange;
begin
  if Assigned(Properties.OnSelectionChange) then
    Properties.OnSelectionChange(Self);
  if RepositoryItem <> nil then
    if Assigned(ActiveProperties.OnSelectionChange) then
      ActiveProperties.OnSelectionChange(Self);
end;
...
begin
  ...
  begin
    ...
    AParentMenuItem := TPopupMenu(FEditPopupMenu).Items;
    AddMenuItem(AParentMenuItem, cxGetResourceString(@cxSEditRichEditUndoCaption), -1);
    AddMenuItem(AParentMenuItem, cxGetResourceString(@cxSEditRichEditRedoCaption), -2);
    AddMenuItem(AParentMenuItem, '-', MaxInt);
    AddMenuItem(AParentMenuItem, cxGetResourceString(@cxSEditRichEditCutCaption), -3);
    AddMenuItem(AParentMenuItem, cxGetResourceString(@cxSEditRichEditCopyCaption), -4);
    AddMenuItem(AParentMenuItem, cxGetResourceString(@cxSEditRichEditPasteCaption), -5);
    AddMenuItem(AParentMenuItem, cxGetResourceString(@cxSEditRichEditDeleteCaption), -6);
    AddMenuItem(AParentMenuItem, '-', MaxInt);
    AddMenuItem(AParentMenuItem, cxGetResourceString(@cxSEditRichEditSelectAllCaption), -7);
  end;
  BuildAISubmenu(TPopupMenu(FEditPopupMenu).Items, GetAICommands); 
...
begin
  ...
  InnerRich.ReadOnly := RealReadOnly; 
end;

procedure TcxCustomRichEdit.UpdateInfoForHiddenInplaceEditor(
  AViewData: TcxCustomEditViewData; AEditViewInfo: TcxCustomEditViewInfo);
var
  ATextViewData: TcxCustomTextEditViewData;
begin
  ATextViewData := TcxCustomTextEditViewData(AViewData);
  ATextViewData.DisplayValueToDrawValue(VarToStr(EditValue), AEditViewInfo);
...
class procedure TcxCustomRichEdit.RegisterConversionFormat(const AExtension: string; AConversionClass: TConversionClass);
var
  AConversionFormat: PcxConversionFormat;
begin
...
begin
  GetRegisteredEditProperties.Register(TcxRichEditProperties, cxGetResourceString(@scxSEditRepositoryRichEditItem));
...
begin
  ...
  dxFreeGlobalObject(FRichEditDllLoader);
end;
