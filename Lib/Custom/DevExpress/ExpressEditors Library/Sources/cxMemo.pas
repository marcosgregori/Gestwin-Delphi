...
uses
  Winapi.Windows, Winapi.Messages, System.Types, System.Classes, Vcl.Controls, Vcl.Forms, Vcl.Graphics, Vcl.Menus, Vcl.StdCtrls, System.SysUtils,
  ...
  cxAccessibility, dxAI, dxUIABase, dxUIATextControl, dxUIAClasses;
...
type
  ...
  private
    ...
    function GetProperties: TcxCustomMemoProperties;
    function GetViewInfo: TcxCustomMemoViewInfo;
    procedure SetCaretPos(const Value: TPoint);
  ...
  protected
    // UI Automation
    function CreateUIATextControlHelper: IdxUIATextControl; override;
    function GetInnerEditUIAProvider(AAllowCreate: Boolean): IdxUIAProvider; override;
    //
    ...
    procedure AdjustScrollBarPosition(const AScrollBar: IcxControlScrollBar); override;
    procedure AdjustScrollBarPositions; override;
    ...
    function GetDefaultInteractiveGestureOptions: TInteractiveGestureOptions; override;
    ...
    function NeedRecreateInnerControlOnStyleChanged: Boolean; override;
    function NeedsScrollBars: Boolean; override;
    procedure putText(const AAICommandID: string; const AText: string; AInsertMode: TdxAITextInsertMode); override;
    ...
    function SupportsAI: Boolean; override;
    function TabsNeeded: Boolean; override;
    procedure CreateParams(var Params: TCreateParams); override;
    procedure WndProc(var Message: TMessage); override;
    function CanMemoKeyModifyEdit(Key: Word; Shift: TShiftState; AIsKeyPress: Boolean): Boolean; virtual;
    procedure InternalSynchronizeEditValue;

    property InnerMemo: IcxInnerMemo read GetInnerMemo;
    property ViewInfo: TcxCustomMemoViewInfo read GetViewInfo;
  public
  ...
  { TdxCustomInnerMemoAccessibilityHelper }

  TdxCustomInnerMemoAccessibilityHelper = class(TdxCustomInnerTextEditAccessibilityHelper);

  { TcxCustomInnerMemo }

  TcxCustomInnerMemo = class(TMemo, IUnknown,
    IcxContainerInnerControl, IcxInnerEditHelper, IcxPaintControlsHelper, IdxAutomationElement)
  private
    FAutoSelect: Boolean;
  ...
  protected
    ...
    function GetContainerUIATextControlHelper: IdxUIATextControl;
    // IdxAutomationElement
  ...
  private
    function GetEdit: TcxAutoHeightInplaceEdit;
  protected
    procedure DrawEditorBorder(ACanvas: TcxCanvas); virtual;
  ...
  TcxAutoHeightInplaceEdit = class(TcxMemo) // for internal use
  private
    ...
    FLineHeight: Integer;
    FMaxLineCountTextHeight: Integer;
    FPrevHeight: Integer;
    procedure WMCommand(var Message: TWMCommand); message WM_COMMAND;
  protected
    ...
    property MaxLineCountTextHeight: Integer read FMaxLineCountTextHeight;
  public
    constructor Create(AEditingController: TcxCustomEditingController); reintroduce; virtual;
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    procedure SetEditingController(AController: TcxCustomEditingController); // for internal use
    procedure SetMaxLineCount(AValue: Integer); // for internal use
...
uses
  System.Variants, Vcl.Clipbrd, System.Math, System.UITypes, dxCoreGraphics, cxEditConsts, cxEditUtils, cxScrollBar,
  dxDPIAwareUtils, dxMessages, dxThreading, dxPopupMenus, dxUIATextControlProvider;

const
  dxThisUnitName = 'cxMemo';
...
type
  ...
  TcxCustomMemoUIATextControl = class(TdxCustomEditUIATextControl) // for internal use
  private
    FControl: TcxCustomMemo;
  protected
    function GetTextWindow: TWinControl; override;
    function IsMultiline: Boolean; override;
    // IdxUIATextControl
    function IsPassword: Boolean; override;

    function GetFocused: Boolean; override;
    function GetInstance: TWinControl; override;
    function GetInnerControl: TWinControl; override;

    property Control: TcxCustomMemo read FControl;
  public
    constructor Create(AControl: TcxCustomMemo);
  end;
...
procedure TcxCustomMemoViewData.AdjustPadding(var APadding: TdxPadding);
var
  ABackPadding: TRect;
  AScrollPadding: TRect;
begin
  if Painter.SupportsMemoPadding and not IsTouchScrollUIMode and (Edit <> nil) then
  begin
    ABackPadding := Painter.GetMemoBackgroundPadding(IsInplace, dxDefaultScaleFactor);

    if TcxCustomEditAccess(Edit).IsScrollBarActive(sbHorizontal) then
    begin
      AScrollPadding := Painter.GetMemoScrollBarPadding(IsInplace, sbHorizontal, nil);
      APadding.Bottom := AScrollPadding.Bottom; 
    end;
    if TcxCustomEditAccess(Edit).IsScrollBarActive(sbVertical) then
    begin
      AScrollPadding := Painter.GetMemoScrollBarPadding(IsInplace, sbVertical, nil);
      APadding.Right := AScrollPadding.Right; 
    end;
  end
  else
...
procedure TcxCustomMemoViewData.Calculate(ACanvas: TcxCanvas;
...
var
  APadding: TdxPadding;
  AViewInfo: TcxCustomMemoViewInfo;
  AButtonViewInfo: TcxEditButtonViewInfo;
  I: Integer;
begin
  inherited Calculate(ACanvas, ABounds, P, Button, Shift, ViewInfo, AIsMouseEvent);
  AViewInfo := TcxCustomMemoViewInfo(ViewInfo);
  if AViewInfo.Painter.SupportsEditorPadding(AViewInfo.IsInplace) then
  begin
    APadding := AViewInfo.Painter.GetMemoBackgroundPadding(AViewInfo.IsInplace, AViewInfo.ScaleFactor);
    AViewInfo.MemoAreaBounds := ABounds;
    AViewInfo.MemoAreaBounds.Deflate(APadding);
  end
  else
    AViewInfo.MemoAreaBounds := AViewInfo.BorderRect;
  ...
  begin
    ...
    if AButtonViewInfo.Data.LeftAlignment then
      AViewInfo.MemoAreaBounds.Left := Max(AViewInfo.MemoAreaBounds.Left, AButtonViewInfo.Bounds.Right)
    else
      AViewInfo.MemoAreaBounds.Right := Min(AViewInfo.MemoAreaBounds.Right, AButtonViewInfo.Bounds.Left);
  end;
end;
...
begin
  with EditContentDefaultOffsets[IsInplace] do
...
begin
  ...
  if (ADisplayText <> '') and dxCharInSet(ADisplayText[Length(ADisplayText)], dxBreakLineCharacters) then
    ADisplayText := ADisplayText + dxCRLF;
...
begin
  if (DefaultValuesProvider <> nil) and DefaultValuesProvider.IsBlob then
    Result := evsValue
  else
    Result := evsText;
end;
...
var
  AMemoProperties: TcxCustomMemoProperties;
begin
  inherited;
  if AProperties is TcxCustomMemoProperties then
  begin
    AMemoProperties := TcxCustomMemoProperties(AProperties);
    ScrollBars := AMemoProperties.ScrollBars;
    VisibleLineCount := AMemoProperties.VisibleLineCount;
    WantReturns := AMemoProperties.WantReturns;
    WantTabs := AMemoProperties.WantTabs;
    WordWrap := AMemoProperties.WordWrap;
  end;
end;
...
var
  AInnerEditBounds, AInnerContentBounds: TRect;
...
begin
  ...
  try
    AInnerContentBounds := ViewInfo.ClientRect;
    AInnerEditBounds := AInnerContentBounds;
    if LookAndFeelPainter.SupportsEditorBorders then
      AInnerEditBounds.Inflate(cxContainerMaxBorderWidth);
    ...
    AlignControls(InnerEdit.Control, AInnerContentBounds);
  finally
    FInnerEditPositionAdjusting := False;
  end;
end;

procedure TcxCustomMemo.AdjustInnerControlScrollBarBounds(AScrollBarKind: TScrollBarKind; var ABounds: TRect);
var
  APadding, AScrollBarPadding: TRect;
  AScrollBarAdjustment: Integer;
begin
  if not IsPopupScrollBars and LookAndFeelPainter.SupportsEditorPadding(IsInplace) then
  begin
    APadding := LookAndFeelPainter.GetMemoBackgroundPadding(IsInplace, ScaleFactor);
    AScrollBarPadding := LookAndFeelPainter.GetMemoScrollBarPadding(IsInplace, AScrollBarKind, ScaleFactor);
    ...
    begin
      ...
      AScrollBarAdjustment := APadding.Right - AScrollBarPadding.Right;
      if UseRightToLeftScrollBar then
        AScrollBarAdjustment := -AScrollBarAdjustment;
      Inc(ABounds.Right, AScrollBarAdjustment);
      Inc(ABounds.Left, AScrollBarAdjustment);
    end
    else
    begin
      ...
      AScrollBarAdjustment := APadding.Bottom - AScrollBarPadding.Bottom;
      Inc(ABounds.Top, AScrollBarAdjustment);
      Inc(ABounds.Bottom, AScrollBarAdjustment);
    end;
  end;
end;

procedure TcxCustomMemo.AdjustScrollBarPosition(const AScrollBar: IcxControlScrollBar);
var
...
begin
  ...
  begin
    ...
    if not dxIsProcessDPIPerMonitorV2Aware then
    begin
    ...
    AdjustInnerControlScrollBarBounds(AScrollBar.Kind, R);
    AScrollBar.BoundsRect := R;
...
procedure TcxCustomMemo.AdjustScrollBarPositions;
var
  AScrollBarInfo: TcxScrollBarInfo;
...
begin
  ...
  begin
    R := ViewInfo.MemoAreaBounds;
    R.Top := R.Bottom  - AScrollBarSize.cy;
    if AVScrollBarVisible then
      R.Right := R.Right - AScrollBarSize.cx;
    AdjustInnerControlScrollBarBounds(sbHorizontal, R);
    if UseRightToLeftScrollBar then
      R := TdxRightToLeftLayoutConverter.ConvertRect(R, ViewInfo.MemoAreaBounds);
  ...
  begin
    R := ViewInfo.MemoAreaBounds;
    R.Left := R.Right  - AScrollBarSize.cx;
    if AHScrollBarVisible then
      R.Bottom := R.Bottom - AScrollBarSize.cy;
    AdjustInnerControlScrollBarBounds(sbVertical, R);
    if UseRightToLeftScrollBar then
      R := TdxRightToLeftLayoutConverter.ConvertRect(R, ViewInfo.MemoAreaBounds);
...
function TcxCustomMemo.GetBorderExtent: TRect;
var
  AScrollBarPadding: TRect;
  AVScrollbarInfo, AHScrollbarInfo: TScrollBarInfo;
  AHScrollbarVisible, AVScrollbarVisible: Boolean;
begin
  if LookAndFeelPainter.SupportsEditorPadding(IsInplace) then
  begin
    Result := LookAndFeelPainter.GetMemoBackgroundPadding(False, ScaleFactor);
    ...
    if AVScrollbarVisible then
    begin
      AScrollBarPadding := LookAndFeelPainter.GetMemoScrollBarPadding(False, sbVertical, ScaleFactor);
      Result.Right := AScrollBarPadding.Right;
      if AHScrollbarVisible then
        Result.Bottom := AScrollBarPadding.Bottom;
    end;
    if AHScrollbarVisible then
      Result.Bottom := LookAndFeelPainter.GetMemoScrollBarPadding(False, sbVertical, ScaleFactor).Bottom;
  end
  else
    Result := inherited GetBorderExtent;
end;

function TcxCustomMemo.GetDefaultInteractiveGestureOptions: TInteractiveGestureOptions;
begin
  Result := [igoPanInertia, igoPanSingleFingerVertical, igoPanGutter, igoParentPassthrough];
end;
...
function TcxCustomMemo.GetHScrollBarBounds: TRect;
var
  AInnerEditHScrollBarInfo: TcxScrollBarInfo;
  AInnerEditVScrollBarInfo: TcxScrollBarInfo;
begin
  Result := ViewInfo.MemoAreaBounds;
  case GetScrollbarMode of
    ...
    sbmClassic:
      begin
        GetScrollBarInfo(AInnerEditHScrollBarInfo, sbHorizontal);
        Result.Top := Result.Bottom - AInnerEditHScrollBarInfo.rcScrollBar.Height;
        if VScrollBarVisible then
        begin
          GetScrollBarInfo(AInnerEditVScrollBarInfo, sbVertical);
          Result.Right := Result.Right - AInnerEditVScrollBarInfo.rcScrollBar.Width;
        end;
      end;
  else // sbmHybrid
    Result.Top := Result.Bottom - HScrollBar.Height;
  ...
  if UseRightToLeftScrollBar then
    Result := TdxRightToLeftLayoutConverter.ConvertRect(Result, ViewInfo.MemoAreaBounds);
end;
...
function TcxCustomMemo.GetInnerEditUIAProvider(AAllowCreate: Boolean): IdxUIAProvider;
begin
  if Assigned(Unsafe.Cast<TcxCustomInnerMemo>(InnerControl).FUIAProvider) or AAllowCreate then
    Result := Unsafe.Cast<TcxCustomInnerMemo>(InnerControl).UIAProvider
  else
    Result := nil;
end;

function TcxCustomMemo.GetVScrollBarBounds: TRect;
var
  AInnerEditHScrollBarInfo: TcxScrollBarInfo;
  AInnerEditVScrollBarInfo: TcxScrollBarInfo;
begin
  ...
  case GetScrollbarMode of
    ...
    sbmClassic:
      begin
        GetScrollBarInfo(AInnerEditVScrollBarInfo, sbVertical);
        Result.Left := Result.Right - AInnerEditVScrollBarInfo.rcScrollBar.Width;
        if HScrollBarVisible then
        begin
          GetScrollBarInfo(AInnerEditHScrollBarInfo, sbHorizontal);
          Result.Bottom := Result.Bottom - AInnerEditHScrollBarInfo.rcScrollBar.Height;
        end;
      end;
  else // sbmHybrid
    Result.Left := Result.Right - VScrollBar.Width;
...
function TcxCustomMemo.SupportsAI: Boolean;
begin
  Result := TdxAIChatClients.HasChatClient and (TdxAICommands.CommandsCount > 0);
end;
...
function TcxCustomMemo.NeedRecreateInnerControlOnStyleChanged: Boolean;
var
  AInnerControl: TcxCustomInnerMemo;
  ANeedInnerControlBorders: Boolean; 
  ALockRedraw: Boolean;
begin
  AInnerControl := TcxCustomInnerMemo(InnerControl);
  ANeedInnerControlBorders := LookAndFeelPainter.SupportsEditorBorders;
  ALockRedraw := HandleAllocated and IsWindowVisible(Handle);
  if ANeedInnerControlBorders then
  begin
    if AInnerControl.BorderStyle <> bsSingle then
    begin
      if ALockRedraw then
        SendMessage(Handle, WM_SETREDRAW, 0, 0);
      AInnerControl.BorderStyle := bsSingle;
      if ALockRedraw then
  ...
  begin
    if AInnerControl.BorderStyle <> bsNone then
    begin
      if ALockRedraw then
        SendMessage(Handle, WM_SETREDRAW, 0, 0);
      AInnerControl.BorderStyle := bsNone;
      if ALockRedraw then
        SendMessage(Handle, WM_SETREDRAW, 1, 0);
      Exit(False);
    end;
  end;
  Result := inherited NeedRecreateInnerControlOnStyleChanged;
end;
...
procedure TcxCustomMemo.putText(const AAICommandID, AText: string; AInsertMode: TdxAITextInsertMode);
var
  AInnerMemo: TcxCustomInnerMemo;
begin
  AInnerMemo := TcxCustomInnerMemo(InnerControl);
  case AInsertMode of
    TdxAITextInsertMode.Above:
      begin
        SelLength := 0;
        AInnerMemo.Perform(EM_REPLACESEL, 0, PCHAR(AText + #13#10));
      end;
    TdxAITextInsertMode.Below:
      begin
        SelStart := SelStart + SelLength;
        AInnerMemo.Perform(EM_REPLACESEL, 0, PCHAR(#13#10 + AText));
      end;
    TdxAITextInsertMode.Replace:
      SelText := AText;
  else
  end;
end;
...
function TcxCustomMemo.CreateUIATextControlHelper: IdxUIATextControl;
begin
  Result := TcxCustomMemoUIATextControl.Create(Self);
end;
...
function TcxCustomMemo.CanMemoKeyModifyEdit(Key: Word; Shift: TShiftState; AIsKeyPress: Boolean): Boolean;
begin
  Result := (Key = VK_RETURN) and (ActiveProperties.WantReturns or (ssCtrl in Shift))
    or (Key = VK_TAB) and (ActiveProperties.WantTabs or AIsKeyPress)
    or (Key = VK_BACK) and (not cxPointIsEqual(CaretPos, cxNullPoint));
end;
...
function TcxCustomMemo.GetViewInfo: TcxCustomMemoViewInfo;
begin
  Result := TcxCustomMemoViewInfo(FViewInfo);
end;
...
function TcxMemo.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;
...
function TcxCustomInnerMemoHelper.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
...
function TcxCustomInnerMemo.UIAProvider: IdxUIAProvider;
begin
...
procedure TcxCustomInnerMemo.CaretMoved;
begin
  Container.CaretMoved;
end;

procedure TcxCustomInnerMemo.Change;
var
  AContainer: TcxCustomMemo;
begin
  inherited Change;
  AContainer := Container;
  if Assigned(AContainer) and AContainer.SupportsUIAutomation then
    AContainer.UIATextChanged;
end;
...
begin
  ...
  begin
    if (csGlassPaint in ControlState) and (TMessage(Message).Msg = CN_CTLCOLOREDIT) then
    begin
...
function TcxCustomInnerMemo.SupportsUIAutomation: Boolean;
begin
  Result := Assigned(Container) and Container.SupportsUIAutomation;
end;
...
begin
  FInternalUpdating := False;
  if not WantTabs and ((Key = Char(VK_TAB))) then //..  En caso contrario, el caracter #9 se guarda en el EditValue
    Key := #0;                                    //..
...
function TcxCustomInnerMemo.NeedAccessibilityHelper: Boolean;
begin
...
function TcxCustomInnerMemo.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIAInnerMemoProvider.Create(GetContainerUIATextControlHelper);
end;
...
{$IFDEF DELPHI110}
procedure TcxCustomInnerMemo.UpdateEditMargins;
begin
  // do nothing
end;
{$ENDIF}
...
begin
  ...
  case Message.Msg of
    ...
    WM_UAHMEASUREMENUITEM:
      begin
        inherited WndProc(Message);
        Message.Result := TdxPopupMenuController.WMUAHMeasureMenuItem(Handle, Message.WParam, Message.LParam);
      end;
   WM_UAHINITMENU: 
     begin
       inherited WndProc(Message);
       if FValidateContextMenu then
       begin
         AMenu := TdxPopupMenuController.GetHMenuFromMessage(Message);
         if AMenu <> 0 then
           Container.ValidateContextMenu(AMenu);
         FValidateContextMenu := False;
       end;
     end;
   DXM_POPUPMENUAICOMMANDFIRST..DXM_POPUPMENUAICOMMANDLAST:
     begin
       if Container.HasAIMenuItem(Message.Msg) then
       begin
         Container.ExecuteAIMenuItem(Message.Msg);
         Message.Result := 1;
       end
       else
         inherited WndProc(Message);
...
function TcxCustomInnerMemo.GetAccessibilityHelperClass: TdxCustomInnerTextEditAccessibilityHelperClass;
begin
  Result := TdxCustomInnerMemoAccessibilityHelper;
end;

function TcxCustomInnerMemo.GetAutomationSettings: TdxAutomationElementSettings;
begin
  Result := nil;
end;

procedure TcxCustomInnerMemo.BeforeDestruction;
begin
...
function TcxCustomInnerMemo.GetContainerUIATextControlHelper: IdxUIATextControl;
begin
  Result := Container.GetUIATextControlHelper;
end;

function TcxCustomInnerMemo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
...
function TcxCustomInnerMemo.GetIsDestroying: Boolean;
begin
  Result := csDestroying in ComponentState;
end;

function TcxCustomInnerMemo.GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider;
begin
...
procedure TcxCustomInnerMemo.WMDestroy(var Message: TWMDestroy);
begin
  if Assigned(FUIAProvider) then
  begin
    dxUIACleanupWindowProvider(Self, FUIAProvider);
    FUIAProvider := nil;
  end;
  inherited;
end;
...
procedure TcxCustomInnerMemo.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
  if dxUIAHandleWMGetObject(Message, Self, GetUIAProvider, SupportsUIAutomation) then
    Exit;
...
procedure TcxCustomInnerMemo.WMKeyUp(var Message: TWMKeyUp);
begin
  inherited;
  CaretMoved;
end;
...
begin
  if dxUIASkipControlPainting(Self, Message) then
    Exit;
  inherited;
end;
...
procedure TcxCustomInnerMemo.WMStyleChanged(var Message: TMessage);
var
  AMessage: TWMStyleChanged;
  ARTLChanging: Boolean;
  ABiDiMode: TBiDiMode;
begin
  inherited;
  AMessage := TWMStyleChanged(Message);
  if (AMessage.StyleType = UINT_PTR(GWL_EXSTYLE)) then
  begin
    ARTLChanging := AMessage.StyleStruct.styleOld and WS_EX_RTLREADING <> AMessage.StyleStruct.styleNew and WS_EX_RTLREADING;
    if ARTLChanging then
    begin
      if AMessage.StyleStruct.styleNew and WS_EX_RTLREADING <> 0 then
        ABiDiMode := bdRightToLeft
      else
        ABiDiMode := bdLeftToRight;
      TdxUIThreadSyncService.EnqueueInvokeInUIThread(Container,
        procedure ()
        begin
          Container.BiDiMode := ABiDiMode;
        end);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if cxRectIntersect(AFocusRectBounds, Bounds) and Painter.SupportsNativeFocusRect(TdxPaintPartID.Memo) then
      ACanvas.DrawFocusRectEx(AFocusRectBounds, cxBordersAll, True);
  end;
end;

procedure TcxAutoHeightInplaceEditViewInfo.DrawEditorBorder(ACanvas: TcxCanvas);
begin
...
begin
  if Edit.Left = cxInvisibleCoordinate then
    Exit;
  if Edit.AutoHeight = eahEditor then
    DrawEditorBorder(ACanvas);
  inherited InternalPaint(ACanvas);
end;

{ TcxAutoHeightInplaceEditProperties }

constructor TcxAutoHeightInplaceEditProperties.Create(AOwner: TPersistent);
begin
  inherited Create(AOwner);
  WantReturns := False;
  ScrollBars := ssNone;
...
class function TcxAutoHeightInplaceEditProperties.GetContainerClass: TcxContainerClass;
begin
  Result := TcxAutoHeightInplaceEdit;
end;
...
begin
  ...
  SetEditingController(AEditingController);
end;
...
begin
  ...
  if LookAndFeel.Painter.SupportsEditorPadding(True) then
  begin
    APadding := LookAndFeel.Painter.GetEditorPadding(True, ScaleFactor);
    Inc(Result, APadding.Height);
  end
  else
...
procedure TcxAutoHeightInplaceEdit.SetEditingController(AController: TcxCustomEditingController);
begin
  FEditingController := AController;
end;

procedure TcxAutoHeightInplaceEdit.SetMaxLineCount(AValue: Integer);
begin
  FLineHeight := TdxTextMeasurer.TextLineHeight(Style.Font);
  if AValue > 0 then
    FMaxLineCountTextHeight := LineHeight * AValue 
  else
    FMaxLineCountTextHeight := 0;
end;

procedure TcxAutoHeightInplaceEdit.CalculateHeight;
var
  R: TRect;
  ATextHeight, ATextLength: Integer;
  AText: string;
begin
  AText := Text;
  ATextLength := Length(AText);
  if ATextLength = 0 then
    ATextHeight := LineHeight
  else
  begin
    R := InnerControl.ClientRect;
    InflateRect(R, -1, 0);
    ATextHeight := TdxTextMeasurer.TextHeightDT(Style.Font, AText, R, DT_EDITCONTROL or DT_WORDBREAK or DT_CALCRECT or DT_NOPREFIX);
    if (ATextLength >= 2) and (AText[ATextLength - 1] = #13) and (AText[ATextLength] = #10) then
      Inc(ATextHeight, LineHeight);
    if (AutoHeight = eahRow) and (MaxLineCountTextHeight > 0) then
      ATextHeight := Min(ATextHeight, MaxLineCountTextHeight);
  end;
...
{ TcxCustomMemoUIATextControl }

constructor TcxCustomMemoUIATextControl.Create(AControl: TcxCustomMemo);
begin
  inherited Create(AControl.InnerControl as TCustomEdit);
  FControl := AControl;
end;

function TcxCustomMemoUIATextControl.GetFocused: Boolean;
begin
  Result := FControl.Focused;
end;

function TcxCustomMemoUIATextControl.GetInnerControl: TWinControl;
begin
  Result := FControl.InnerControl;
end;

function TcxCustomMemoUIATextControl.GetInstance: TWinControl;
begin
  Result := FControl;
end;

function TcxCustomMemoUIATextControl.GetTextWindow: TWinControl;
begin
  Result := FControl.InnerControl;
end;

function TcxCustomMemoUIATextControl.IsMultiline: Boolean;
begin
  Result := True;
end;

function TcxCustomMemoUIATextControl.IsPassword: Boolean;
begin
  Result := False;
end;
