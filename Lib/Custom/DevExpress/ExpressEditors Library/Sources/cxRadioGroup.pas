...
uses
  System.UITypes,
  System.Types, Winapi.Windows, Winapi.Messages, System.Variants, System.Classes, Vcl.Controls, Vcl.Forms, Vcl.Graphics,
  Vcl.ImgList, Vcl.Menus, Vcl.StdCtrls, System.SysUtils,
  dxCore, dxUxTheme, cxClasses, cxContainer, cxControls, cxDataStorage, cxGraphics, cxLookAndFeels, cxEdit, cxTextEdit,
  cxGroupBox, cxDropDownEdit, cxFilterControlUtils, dxFading, cxGeometry, dxCoreClasses, cxAccessibility,
  cxLookAndFeelPainters, dxUIAClasses, dxUIABase;
...
type
  ...
  protected
    ...
    //IdxVisualRefinementsListener
    procedure VisualRefinementsChanged;
    procedure IdxVisualRefinementsListener.Changed = VisualRefinementsChanged;
  ...
  public
    ...
    property ButtonCount: Integer read GetButtonCount; // for internal use
...
uses
  System.Math, Vcl.ActnList, dxThemeConsts, dxThemeManager, cxVariants, cxEditConsts, cxEditPaintUtils, cxEditUtils, dxDPIAwareUtils,
  Winapi.ActiveX, dxThreading, dxUIA, dxUIAUtils;

const
  dxThisUnitName = 'cxRadioGroup';

type
  TCanvasAccess = class(TCanvas);

  TdxRadioButtonAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
    function GetRadioButton: TcxRadioButton;
  protected
    ...
    property RadioButton: TcxRadioButton read GetRadioButton;
  public
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect; override;
  end;

  TdxUIARadioButtonProvider = class(TdxUIABaseWindowProvider<TcxRadioButton>,
    dxUIA.ISelectionItemProvider)
  protected
    function GetDefaultAriaRole: string; override;
    function GetDefaultAutomationName: string; override;
    ...
    function InternalSelect: Boolean; override;
    function InternalAddToSelection: Boolean; override;
    function InternalRemoveFromSelection: Boolean; override;
    function InternalGetIsSelected: Boolean; override;
    function InternalGetSelectionContainer: IdxUIAProvider; override;
  end;

  TdxUIACustomRadioGroupProvider = class(TdxUIACustomGroupBoxProvider<TcxCustomRadioGroup>, dxUIA.ISelectionProvider)
  protected
    function GetDefaultAriaRole: string; override;
    function GetControlTypePropertyValue(out AValue: Integer): Boolean; override;
    function GetSupportedPatterns: TArray<Integer>; override;
    // ISelectionProvider
    function InternalGetSelection: TdxUIAProviderList; override;
    function InternalGetCanSelectMultiple: Boolean; override;
    function InternalGetIsSelectionRequired: Boolean; override;
  end;
...
{ TdxRadioButtonAccessibilityHelper }

function TdxRadioButtonAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := cxRectOffset(cxRectSetNullOrigin(RadioButton.BoundsRect), RadioButton.ClientToScreen(cxNullPoint));
end;

function TdxRadioButtonAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := RadioButton.Caption;
end;

function TdxRadioButtonAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := RadioButton.Handle;
end;

function TdxRadioButtonAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_RADIOBUTTON;
end;

function TdxRadioButtonAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE;
  if RadioButton.Checked then
    Result := Result or cxSTATE_SYSTEM_CHECKED;
  if not RadioButton.Visible then
...
function TdxRadioButtonAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopLocation, aopFocus];
end;

function TdxRadioButtonAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxRadioButtonAccessibilityHelper.GetRadioButton: TcxRadioButton;
begin
  Result := TcxRadioButton(OwnerObject);
end;
...
begin
  ...
  ControlStyle := ControlStyle + [csOverrideStylePaint, csDoubleClicks];
...
function TcxRadioButton.GetAutomationSettings: TdxAutomationElementSettings;
begin
...
function TcxRadioButton.GetAutomationSettingsClass: TdxAutomationElementSettingsClass;
begin
...
begin
  ...
  begin
    ...
    if not IsNativeStyle then
      Inc(R.Bottom);
    CheckFocusRect(R);
    if FLookAndFeel.Painter.SupportsNativeFocusRect(TdxPaintPartID.RadioButton) then
...
function TcxRadioButton.NeedAccessibilityHelper: Boolean;
begin
...
procedure TcxRadioButton.SetAutomation(
...
begin
  ...
  begin
    ...
    if HandleAllocated then
    begin
      AUIAProvider := UIAProvider;
      if Assigned(AUIAProvider) then
        dxNotifyUIAClientsOfRadioButtonStateChange(AUIAProvider, not FChecked, FChecked);
    end;
  end;
end;
...
procedure TcxRadioButton.VisualRefinementsChanged;
begin
  AdjustSize;
  Invalidate;
end;
...
function TcxRadioButton.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIARadioButtonProvider.Create(Self);
end;
...
function GetTextRectCorrection(AIsInplace, ANativeStyle, AWordWrap: Boolean; AAlignment: TLeftRight): TRect;
const
  AInplaceTextRectCorrection: array [Boolean] of TRect = (
    (Left: 5; Top: 0; Right: 1; Bottom: 0),
    (Left: 3; Top: 0; Right: 0; Bottom: 0)
  );
  AStandardTextRectCorrection: array [Boolean, TLeftRight] of TRect = (
   ((Left: 2; Top: -1; Right: 1; Bottom: 0),
    (Left: 5; Top: -1; Right: 0; Bottom: 0)),
   ((Left: 2; Top: -1; Right: 6; Bottom: 0),
    (Left: 5; Top: -1; Right: 2; Bottom: 0))
    );
  ANativeStyleTextRectCorrection: array [Boolean, TLeftRight] of TRect = (
   ((Left: 2; Top: 0; Right: 1; Bottom: 0),
    (Left: 3; Top: 0; Right: 0; Bottom: 0)),
   ((Left: 2; Top: 0; Right: 3; Bottom: 0),
    (Left: 3; Top: 0; Right: 0; Bottom: 0))
  );
begin
  if AIsInplace then
    Result := AInplaceTextRectCorrection[ANativeStyle]
  else
    if ANativeStyle then
      Result := ANativeStyleTextRectCorrection[AWordWrap, AAlignment]
    else
      Result := AStandardTextRectCorrection[AWordWrap, AAlignment];
  if AIsInplace or TdxVisualRefinements.UsePaddingForStandaloneEditors then
  begin
    Inc(Result.Top, TdxVisualRefinements.Padding.Top);
    Inc(Result.Bottom, TdxVisualRefinements.Padding.Bottom);
  end;
end;

procedure CorrectRadioButtonTextRect(var R: TRect; AIsInplace, ANativeStyle, AWordWrap, AUseRightToLeftAlignment: Boolean;
  AAlignment: TLeftRight; AScaleFactor: TdxScaleFactor);
var
  AOffsets: TRect;
begin
  AOffsets := GetTextRectCorrection(AIsInplace, ANativeStyle, AWordWrap, AAlignment);
  if AUseRightToLeftAlignment then
    AOffsets := TdxRightToLeftLayoutConverter.ConvertOffsets(AOffsets);
  R := cxRectContent(R, AScaleFactor.Apply(AOffsets));
end;

procedure TcxRadioButton.CorrectTextRect(var R: TRect; ANativeStyle: Boolean);
begin
  CorrectRadioButtonTextRect(R, IsInplace, ANativeStyle, WordWrap, UseRightToLeftAlignment, Alignment, ScaleFactor);
end;
...
var
  AGroup: TcxCustomRadioGroup;
  AInnerControl: IcxContainerInnerControl;
begin
  if Supports(Self, IcxContainerInnerControl, AInnerControl) then
  begin
    AGroup := AInnerControl.ControlContainer as TcxCustomRadioGroup;
    ACanvas.Font.Assign(AGroup.FCaptionFont);
    ACanvas.Font.Color := AGroup.VisibleFontColor;
  end
  else
  begin
  ...
  begin
    ...
    try
      if GetBiDiModeDependentAlignment = taRightJustify then
        AButtonOffset := ButtonRect.Left
      else
        AButtonOffset := Width - ButtonRect.Right;
      Canvas.Font := Font;
      if Text <> '' then
      begin
        AOffsets := ScaleFactor.Apply(GetTextRectCorrection(IsInplace, IsNativeStyle, WordWrap, Alignment));
        ARect := cxRectSetWidth(cxNullRect, Max(AWidth - cxRectWidth(ButtonRect) - AButtonOffset - AOffsets.Left - AOffsets.Right, 1));
        cxDrawText(Canvas.Handle, Text, ARect, GetTextFlags);
        if not (Align in [alTop, alBottom]) then
          AWidth := cxRectWidth(ARect) + cxRectWidth(ButtonRect) + AButtonOffset + AOffsets.Left + AOffsets.Right + 2; 
        if not (Align in [alLeft, alRight]) then
        begin
          AHeight := Max(cxRectHeight(ARect), cxRectHeight(ButtonRect)) + ScaleFactor.Apply(4);
          if TdxVisualRefinements.UsePaddingForStandaloneEditors or IsInplace then
            Inc(AHeight, TdxVisualRefinements.Padding.ScaledHeight(ScaleFactor));
        end;
      end
      else
      begin
        if not (Align in [alTop, alBottom]) then
          AWidth := cxRectWidth(ButtonRect) + 2 * AButtonOffset;
        if not (Align in [alLeft, alRight]) then
...
begin
  ...
  begin
    Result := LookAndFeel.Painter.DefaultLabelTextColorEx(cxEditStateColorKindMap[Enabled]);
...
function TcxRadioButton.GetUIAProvider(AIsWMGetObjectCall: Boolean): IdxUIAProvider;
begin
...
function TcxRadioButton.IsDestroying: Boolean;
begin
...
function TcxRadioButton.IsWidthStored: Boolean;
begin
  Result := not AutoSize or WordWrap;
end;

function TcxRadioButton.IsHeightStored: Boolean;
begin
  Result := not AutoSize;
end;
...
function TcxRadioButton.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;

function TcxRadioButton.UIAProvider: IdxUIAProvider;
begin
...
function TcxRadioButton.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := TdxRadioButtonAccessibilityHelper.Create(Self);
  Result := FIAccessibilityHelper;
end;

function TcxRadioButton.GetRadioButtonRect(const ARadioButtonSize: TSize; ANativeStyle: Boolean): TRect;
var
  APadding: TdxPadding;
begin
  if TdxVisualRefinements.UsePaddingForStandaloneEditors or IsInplace then
    APadding := TdxVisualRefinements.Padding
  else
    APadding := TdxPadding.Null;
  Result.Top := (Height - APadding.ScaledHeight(ScaleFactor) - ARadioButtonSize.cy) div 2 + ScaleFactor.Apply(APadding.Top);
  Result.Bottom := Result.Top + ARadioButtonSize.cy;
  if GetBiDiModeDependentAlignment = taRightJustify then
  begin
    if ANativeStyle or IsInplace or (LookAndFeel.SkinPainter <> nil) then
      Result.Left := 0
    else
      Result.Left := 1;
...
procedure TcxRadioButton.WMDestroy(var Message: TWMDestroy);
begin
  if Assigned(FUIAProvider) then
  begin
...
procedure TcxRadioButton.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
...
procedure TcxRadioButton.BeforeDestruction;
begin
  if Assigned(FUIAProvider) then
...
begin
  ...
  if ItemAlignment = taLeftJustify then
    R.Left := AGlyphRect.Right
  else
    R.Right := AGlyphRect.Left;
  CorrectRadioButtonTextRect(R, IsInplace, NativeStyle, False, UseRightToLeftAlignment, taRightJustify, ScaleFactor);
  ACanvas.Brush.Style := bsClear;
  ...
  begin
    ...
    if Painter.SupportsNativeFocusRect(TdxPaintPartID.RadioGroup) then
...
function TcxCustomRadioGroupViewInfo.GetSkinnedTextColor(AKind: TcxEditStateColorKind): TColor;
begin
...
begin
  if UseRightToLeftAlignment then
    TcxCustomRadioGroupViewInfo(AViewInfo).ItemAlignment := taRightJustify
  else
    TcxCustomRadioGroupViewInfo(AViewInfo).ItemAlignment := taLeftJustify;
...
function TcxCustomRadioGroupViewData.GetButtonNativeTheme(AButtonViewInfo: TcxEditButtonViewInfo): TdxTheme;
begin
  Result := OpenTheme(totButton, ScaleFactor);
end;

function TcxCustomRadioGroupViewData.GetButtonSize: TSize;
begin
  Result := Style.LookAndFeel.Painter.RadioButtonSize;
end;
...
destructor TcxRadioGroupItem.Destroy;
begin
  FreeAndNil(FAutomation);
  inherited Destroy;
end;
...
begin
  ...
  begin
    ...
    Automation.Assign(TcxRadioGroupItem(Source).Automation)
  end;
...
procedure TcxRadioGroupItem.SetAutomation(
  const AValue: TdxAutomationElementSettings);
begin
  Automation.Assign(AValue);
end;
...
function TcxCustomRadioGroupProperties.IsAutomationUnitNeededInUses: Boolean;
var
  AItem: TcxRadioGroupItem;
  I: Integer;
begin
  Result := inherited IsAutomationUnitNeededInUses;
  if not Result then
  begin
    for I := 0 to Items.Count - 1 do
    begin
      AItem := Items[I];
      if Assigned(AItem.Automation) and AItem.Automation.IsAutomationUnitNeededInUses then
        Exit(True);
    end;
  end;
end;
...
function TcxCustomRadioGroupButton.GetAutomationSettings: TdxAutomationElementSettings;
begin
  if Assigned(UIAProvider()) then
    Result := RadioGroup.ActiveProperties.Items[RadioGroup.InternalButtons.IndexOf(Self)].Automation
  else
    Result := nil;
end;
...
function TcxCustomRadioGroup.GetButtonCount: Integer;
begin
  Result := InternalButtons.Count;
end;

//..
function TcxCustomRadioGroup.GetSelectedButton: TcxCustomRadioGroupButton;
begin
     Result := GetButton( ItemIndex );
end;
//..
...
function TcxCustomRadioGroup.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIACustomRadioGroupProvider.Create(Self);
end;
...
function TcxRadioGroup.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;
...
var
  ...
  I: Integer;
begin
  ARadioGroupProperties := AEditProperties as TcxCustomRadioGroupProperties;
  with (AProperties as TcxComboBoxProperties).Items do
  begin
    Clear;
  ...
  TcxComboBoxProperties(AProperties).DefaultValuesProvider := nil;
...
{ TdxUIARadioButtonProvider }

function TdxUIARadioButtonProvider.GetControlTypePropertyValue(
  out AValue: Integer): Boolean;
begin
  AValue := dxUIA.UIA_RadioButtonControlTypeId;
  Result := True;
end;

function TdxUIARadioButtonProvider.GetDefaultAriaRole: string;
begin
  Result := 'radio';
end;

function TdxUIARadioButtonProvider.GetDefaultAutomationName: string;
begin
  Result := Control.Caption;
end;

function TdxUIARadioButtonProvider.GetDefaultIsContentElement: TdxDefaultBoolean;
begin
...
function TdxUIARadioButtonProvider.GetSupportedPatterns: TArray<Integer>;
begin
  Result := [dxUIA.UIA_SelectionItemPatternId];
end;

function TdxUIARadioButtonProvider.InternalAddToSelection: Boolean;
begin
  Control.Checked := True;
  Result := True;
end;

function TdxUIARadioButtonProvider.InternalGetIsSelected: Boolean;
begin
  Result := Control.Checked;
end;

function TdxUIARadioButtonProvider.InternalGetSelectionContainer: IdxUIAProvider;
begin
  Result := Parent;
end;

function TdxUIARadioButtonProvider.InternalRemoveFromSelection: Boolean;
begin
  Control.Checked := False;
  Result := True;
end;

function TdxUIARadioButtonProvider.InternalSelect: Boolean;
begin
  Control.Checked := True;
  Result := True;
end;

{ TdxUIACustomRadioGroupProvider }

function TdxUIACustomRadioGroupProvider.GetControlTypePropertyValue(
  out AValue: Integer): Boolean;
begin
  AValue := dxUIA.UIA_GroupControlTypeId;
  Result := True;
end;

function TdxUIACustomRadioGroupProvider.GetDefaultAriaRole: string;
begin
  Result := 'radiogroup';
end;

function TdxUIACustomRadioGroupProvider.GetSupportedPatterns: TArray<Integer>;
begin
  Result := [dxUIA.UIA_SelectionPatternId];
end;

function TdxUIACustomRadioGroupProvider.InternalGetCanSelectMultiple: Boolean;
begin
  Result := False;
end;

function TdxUIACustomRadioGroupProvider.InternalGetIsSelectionRequired: Boolean;
begin
  Result := False;
end;

function TdxUIACustomRadioGroupProvider.InternalGetSelection: TdxUIAProviderList;
var
  AResult: TdxUIAProviderList;
  I: Integer;
  ACount: Integer;
  AButton: TcxCustomRadioGroupButton;
  AButtonProvider: IdxUIAProvider;
begin
  SetLength(AResult, Control.ButtonCount);
  ACount := 0;
  for I := 0 to Control.ButtonCount - 1 do
  begin
    AButton := Control.Buttons[I];
    if AButton.Checked then
    begin
      if AButton.SupportsUIAutomation then
      begin
        AButtonProvider := AButton.UIAProvider;
        if Assigned(AButtonProvider) then
        begin
          AResult[ACount] := AButtonProvider;
          Inc(ACount);
        end;
      end;
    end;
