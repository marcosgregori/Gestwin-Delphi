...
uses
  System.Types, Winapi.Windows, System.Variants, System.Classes, Vcl.Controls, Vcl.Forms, Vcl.Graphics, System.SysUtils,
  Winapi.ActiveX,
  cxClasses, cxContainer, cxControls, cxDataStorage, cxDataUtils, cxEdit, cxGraphics, cxMaskEdit, cxGeometry,
  cxTextEdit, cxVariants, cxLookAndFeelPainters, cxFilterControlUtils, dxUxTheme, dxUIA, dxUIABase,
...
type
  ...
  { TcxSpinEditButtonViewInfo }

  TcxSpinEditButtonViewInfo = class(TcxEditButtonViewInfo)
  protected
  ...
  strict private const
    SpinButtonToEditBtnKind: array[Boolean, 0..3] of TcxEditBtnKind = (
  ...
  protected
    ...
    procedure DrawLookAndFeelButtonBackground(ACanvas: TcxCanvas; AButtonViewInfo: TcxEditButtonViewInfo; const ARect: TRect); override;
  ...
  private
    ...
    FVisible: Boolean;
    function GetProperties: TcxCustomSpinEditProperties;
    procedure SetPosition(AValue: TcxSpinEditButtonsPosition);
    procedure SetShowFastButtons(AValue: Boolean);
  ...
  private
    FValueType: Boolean;
    procedure SetValueType(AValue: Boolean);
  public
    procedure Assign(Source: TPersistent); override;
  ...
  private
    ...
    function DoubleAsValueType(AValue: Double; AValueType: TcxSpinEditValueType): Double;
  ...
  protected
    ...
    function IsValueBoundsValid(AValue: Double): Boolean;
  ...
  public
    ...
    function CreateViewData(AStyle: TcxCustomEditStyle; AIsInplace: Boolean; AScaleFactor: TdxScaleFactor;
      APreviewMode: Boolean = False): TcxCustomEditViewData; override;
  ...
  { TcxCustomSpinEditInnerTextEdit }

  TcxCustomSpinEditInnerTextEdit = class(TcxCustomInnerTextEdit)
  protected
    // UIA
    function CreateUIAProvider: IdxUIAProvider; override;
  end;
  ...
  private
    ...
    procedure SetPressedState(AValue: TcxSpinEditPressedState);
    procedure SetProperties(AValue: TcxCustomSpinEditProperties);
  ...
  protected
    ...
    procedure DoButtonAction(AButtonVisibleIndex: Integer); override;
    ...
    procedure InternalStoreEditValue(const AValue: TcxEditValue); override;
    ...
    function NeedAccessibilityHelper: Boolean; override;
    ...
    procedure SetInternalValue(Value: TcxEditValue);  override; //..
    ...
    property IsCustomText: Boolean read FIsCustomText write FIsCustomText; 
...
uses
  System.Math, dxCore, dxOffice11, dxThemeConsts, dxThemeManager,
  cxEditConsts, cxEditPaintUtils, cxEditUtils, cxFormats, cxDWMApi;

const
  dxThisUnitName = 'cxSpinEdit';
...
type
  ...
  { TdxUIACustomSpinEditProvider }

  TdxUIACustomSpinEditProvider = class(TdxUIACustomTextEditProvider, IRangeValueProvider)
  strict private
    function GetControl: TcxCustomSpinEdit;
  protected
    function GetButtons: TdxUIAProviderList; override;
    function GetControlTypePropertyValue(out AValue: Integer): Boolean; override;
    function GetDefaultAriaRole: string; override;
    function GetSupportedPatterns: TArray<Integer>; override;
    // IRangeValueProvider
    function InternalSetValue(const AValue: Double): Boolean; override;
    function InternalGetValue(out AValue: Double): Boolean; override;
    function InternalGetMaximum: Double; override;
    function InternalGetMinimum: Double; override;
    function InternalGetLargeChange: Double; override;
    function InternalGetSmallChange: Double; override;
    // IValueProvider
    function InternalSetValue(const AValue: string): Boolean; override;
    function InternalGetIsReadOnly: Boolean; override;

    property Control: TcxCustomSpinEdit read GetControl;
  end;

  { TdxUIACustomSpinEditInnerEditProvider }

  TdxUIACustomSpinEditInnerEditProvider = class(TdxUIAInnerEditProvider, IRangeValueProvider)
  strict private
    function GetSpinEdit: TcxCustomSpinEdit;
  protected
    function GetControlTypePropertyValue(out AValue: Integer): Boolean; override;
    function GetDefaultAriaRole: string; override;
    function GetSupportedPatterns: TArray<Integer>; override;
    ...
    // IValueProvider
    function InternalSetValue(const AValue: string): Boolean; override;
    function InternalGetValue(out AValue: string): Boolean; override;
    function InternalGetIsReadOnly: Boolean; override;

    property SpinEdit: TcxCustomSpinEdit read GetSpinEdit;
  end;

  { TdxUIASpinEditButtonProvider }

  TdxUIASpinEditButtonProvider = class(TdxUIAEditButtonProvider)
  protected
    function GetAutomationIdPropertyValue(out AValue: string): Boolean; override;
  end;
...
function TdxUIASpinEditButtonProvider.GetAutomationIdPropertyValue(out AValue: string): Boolean;
const
  AScrollAmountMap: array[TcxSpinEditButton] of ScrollAmount = (
    ScrollAmount_SmallDecrement, // sebBackward
    ScrollAmount_SmallIncrement, // sebForward
    ScrollAmount_LargeDecrement, // sebFastBackward
    ScrollAmount_LargeIncrement // sebFastForward
  );
begin
  AValue := IntToStr(AScrollAmountMap[TcxSpinEditButton(ViewInfo.ButtonIndex)]);
  Result := True;
end;

{ TcxSpinEditButtonViewInfo }

function TcxSpinEditButtonViewInfo.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIASpinEditButtonProvider.Create(Self);
end;
...
begin
  ...
  begin
    Painter.DrawScaledEditorButtonGlyph(ACanvas, AContentRect,
      SpinButtonToEditBtnKind[ButtonsPosition = sbpVert, AViewInfo.ButtonVisibleIndex],
...
procedure TcxSpinEditViewInfo.DrawLookAndFeelButtonBackground(
  ACanvas: TcxCanvas; AButtonViewInfo: TcxEditButtonViewInfo; const ARect: TRect);
begin
  Painter.DrawScaledEditorButton(ACanvas, ARect,
    SpinButtonToEditBtnKind[ButtonsPosition = sbpVert, AButtonViewInfo.ButtonVisibleIndex],
    EditBtnStateToButtonState[AButtonViewInfo.Data.State], AButtonViewInfo.ScaleFactor,
...
begin
  dxDrawThemeBackground(ACanvas.Handle, totSpin, ScaleFactor, ARect, AButtonViewInfo.Data.NativePart, AButtonViewInfo.Data.NativeState);
end;

function TcxSpinEditViewInfo.GetButtonViewInfoClass: TcxEditButtonViewInfoClass;
begin
  Result := TcxSpinEditButtonViewInfo;
end;
...
var
  ...
  begin
    ...
    begin
      APainter := Style.LookAndFeel.SkinPainter;
      if APainter <> nil then
      begin
        if APainter.ApplyEditorAdvancedMode then
        begin
          if ASpinButtonsPosition = sbpVert then
            AButtonWidth := APainter.EditButtonGlyphMinSize(cxbkSpinDownBtn,  AViewInfo.ScaleFactor).cx
          else
            AButtonWidth := APainter.EditButtonGlyphMinSize(cxbkSpinLeftBtn, AViewInfo.ScaleFactor).cx;
        end;
        with APainter.EditButtonSize do
        begin
...
function TcxSpinEditViewData.GetButtonNativeTheme(AButtonViewInfo: TcxEditButtonViewInfo): TdxTheme;
begin
  Result := OpenTheme(totSpin, ScaleFactor);
end;
...
procedure TcxSpinEditButtons.SetPosition(AValue: TcxSpinEditButtonsPosition);
begin
...
procedure TcxSpinEditButtons.SetShowFastButtons(AValue: Boolean);
begin
  if AValue <> FShowFastButtons then
  begin
    FShowFastButtons := AValue;
    Changed;
  end;
end;

procedure TcxSpinEditButtons.SetVisible(AValue: Boolean);
begin
...
procedure TcxSpinEditPropertiesValues.SetValueType(AValue: Boolean);
begin
...
function TcxCustomSpinEditProperties.CreateViewData(AStyle: TcxCustomEditStyle; AIsInplace: Boolean; AScaleFactor: TdxScaleFactor;
  APreviewMode: Boolean): TcxCustomEditViewData;
begin
  Result := inherited CreateViewData(AStyle, AIsInplace, AScaleFactor, APreviewMode);
...
function TcxCustomSpinEditProperties.IsEditValueValid(var AEditValue: TcxEditValue; AEditFocused: Boolean): Boolean;
var
  AValue: TcxEditValue;
begin
  ...
  begin
    ...
    else if VarIsFloat(AEditValue) then
      Result := TryTextToValue(FloatToStr(AEditValue), AValue)
    else
      Result := TryTextToValue(AEditValue, AValue);
  end
  else
    Result :=  {VarIsNumericEx(EditValue) or} VarIsSoftNull(AEditValue) or VarIsDate(AEditValue);
  if not Result then
    Exit;
  try
...
begin
  ...
  begin
	...
	DoPrepareDisplayValue( AEditValue, ADisplayValue, AEdit.Focused );  //..
...
begin
  if AProperties is TcxCustomSpinEditProperties then
    ...
    begin
      Self.FDefaultValuesProvider := FDefaultValuesProvider;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        Buttons[cxSpinBackwardButtonIndex].Automation.Name := cxGetResourceString(@sdxSpinEditBackwardButtonUIAName);
        Buttons[cxSpinForwardButtonIndex].Automation.Name := cxGetResourceString(@sdxSpinEditForwardButtonUIAName);
        if AButtonCount = 4 then
        begin
          Buttons[cxSpinFastBackwardButtonIndex].Automation.Name := cxGetResourceString(@sdxSpinEditFastBackwardButtonUIAName);
          Buttons[cxSpinFastForwardButtonIndex].Automation.Name := cxGetResourceString(@sdxSpinEditFastForwardButtonUIAName);
        end;
      end;
    finally
      EndUpdate(False);
...
begin
  if not AEditFocused and not AssignedValues.DisplayFormat and (DefaultValuesProvider <> nil) and
    DefaultValuesProvider.IsDisplayFormatDefined(True) then
      Result := evsText
  else
    Result := evsValue;
end;
...
function TcxCustomSpinEditProperties.IsValueBoundsValid(AValue: Double): Boolean;
begin
  if ValueType = vtInt then
...
function TcxCustomSpinEditProperties.DoubleAsValueType(AValue: Double; AValueType: TcxSpinEditValueType): Double;
begin
...
begin
  if AssignedValues.ValueType then
    ...
    if DefaultValuesProvider <> nil then
      Result := ASpinEditValueTypeMap[DefaultValuesProvider.DefaultIsFloatValue]
    else
      Result := vtInt;
end;
...
begin
  ...
  begin
    Result := StrToFloatEx(s, AExtendedValue);
...
function TcxCustomSpinEditInnerTextEdit.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIACustomSpinEditInnerEditProvider.Create(GetContainerUIATextControlHelper);
end;
...
function TcxCustomSpinEdit.CreateUIAProvider: IdxUIAProvider;
begin
  Result := TdxUIACustomSpinEditProvider.Create(GetUIATextControlHelper);
end;
...
procedure TcxCustomSpinEdit.DoButtonAction(AButtonVisibleIndex: Integer);
begin
  inherited DoButtonAction(AButtonVisibleIndex);
  Increment(TcxSpinEditButton(AButtonVisibleIndex));
end;

procedure TcxCustomSpinEdit.DoButtonClick(AButtonVisibleIndex: Integer);
begin
  //
end;
...
begin
  ...
  try
    if FPressedState = epsNone then
      ...
      begin
        DoButtonAction(ViewInfo.PressedButton);
        CreateTimer;
      end;
  finally
    EndUserAction;
...
begin
  ...
  if IsTextChar(Key) or (Key = dxVKBack) or (Key = dxVKDelete) then
...
function TcxCustomSpinEdit.GetInnerEditClass: TControlClass;
begin
  Result := TcxCustomSpinEditInnerTextEdit;
end;
...
procedure TcxCustomSpinEdit.InternalStoreEditValue(const AValue: TcxEditValue);
begin
  inherited;
...
begin
  ...
  Result := Result or IsControlChar(AChar) or (AnsiChar(AChar) in ActiveProperties.ValidChars);
  if not ActiveProperties.CanEdit and Result and (not IsControlChar(AChar) or dxCharInSet(AChar, [dxVKBack, dxVKDelete])) then
    Result := False;
end;
...
function TcxCustomSpinEdit.NeedAccessibilityHelper: Boolean;
begin
  Result := False;
end;
...
begin
  Result := True;
  if ADisplayValue = '' then
    if (ActiveProperties.DefaultValuesProvider <> nil) and ActiveProperties.DefaultValuesProvider.DefaultRequired then
...
procedure TcxCustomSpinEdit.SetPressedState(AValue: TcxSpinEditPressedState);
const
...
var
  I: Integer;
begin
  if (Length(ViewInfo.ButtonsInfo) > 0) and (AValue <> FPressedState) then
  begin
    FPressedState := AValue;
...
procedure TcxCustomSpinEdit.SetProperties(AValue: TcxCustomSpinEditProperties);
begin
  Properties.Assign(AValue);
end;

procedure TcxCustomSpinEdit.SetValue(const AValue: Variant);
var
  AEditValue: TcxEditValue;
begin
  if VarIsNumericEx(AValue) then
  begin
    AEditValue := AValue;
    if ActiveProperties.IsEditValueValid(AEditValue, Focused) then
...
function TcxSpinEdit.SupportsUIAutomation: Boolean;
begin
  Result := True;
end;
...
class function TcxFilterSpinEditHelper.GetSupportedFilterOperators(AProperties: TcxCustomEditProperties;
...
class procedure TcxFilterSpinEditHelper.InitializeProperties(AProperties, AEditProperties: TcxCustomEditProperties; AHasButtons: Boolean);
begin
...
function TdxUIACustomSpinEditProvider.GetButtons: TdxUIAProviderList;
const
  ButtonIndexMap: array[0..3] of Integer = (
    1,  // cxSpinBackwardButtonIndex
    0,  // cxSpinForwardButtonIndex
    3,  // cxSpinFastBackwardButtonIndex
    2   // cxSpinFastForwardButtonIndex
  );
var
  AButtonViewInfo: TcxEditButtonViewInfo;
  AButtonUIAProvider: IdxUIAProvider;
begin
  Result := nil;
  if Assigned(Control) and Control.HandleAllocated and Assigned(Control.ViewInfo) then
  begin
    SetLength(Result, Length(Control.ViewInfo.ButtonsInfo));
    for AButtonViewInfo in Control.ViewInfo.ButtonsInfo do
    begin
      AButtonUIAProvider := AButtonViewInfo.UIAProvider;
      if Assigned(AButtonUIAProvider) and AButtonUIAProvider.IsLive then
        Result[ButtonIndexMap[AButtonViewInfo.ButtonIndex]] := AButtonUIAProvider;
    end;
  end;
end;

function TdxUIACustomSpinEditProvider.GetControlTypePropertyValue(out AValue: Integer): Boolean;
begin
  AValue := dxUIA.UIA_SpinnerControlTypeId;
  Result := True;
end;

function TdxUIACustomSpinEditProvider.GetDefaultAriaRole: string;
begin
  Result := 'spinner';
end;

function TdxUIACustomSpinEditProvider.GetSupportedPatterns: TArray<Integer>;
begin
  Result := inherited GetSupportedPatterns + [dxUIA.UIA_RangeValuePatternId];
end;

function TdxUIACustomSpinEditProvider.InternalGetIsReadOnly: Boolean;
begin
  Result := Control.IsReadOnly;
end;

function TdxUIACustomSpinEditProvider.InternalGetLargeChange: Double;
begin
  Result := Control.ActiveProperties.LargeIncrement;
end;

function TdxUIACustomSpinEditProvider.InternalGetMaximum: Double;
begin
  Result := Control.ActiveProperties.MaxValue;
end;

function TdxUIACustomSpinEditProvider.InternalGetMinimum: Double;
begin
  Result := Control.ActiveProperties.MinValue;
end;

function TdxUIACustomSpinEditProvider.InternalGetSmallChange: Double;
begin
  Result := Control.ActiveProperties.Increment;
end;

function TdxUIACustomSpinEditProvider.InternalGetValue(out AValue: Double): Boolean;
begin
  if TextControl.IsPassword then
    Result := False
  else
  begin
    AValue := Control.Value;
    Result := True;
  end;
end;

function TdxUIACustomSpinEditProvider.InternalSetValue(const AValue: Double): Boolean;
begin
  Control.Value := Control.ActiveProperties.PrepareValue(AValue);
  Control.CheckEditorValueBounds;
  Result := True;
end;

function TdxUIACustomSpinEditProvider.InternalSetValue(const AValue: string): Boolean;
var
  AIntValue: Integer;
  AFloatValue: Extended;
begin
  case Control.ActiveProperties.ValueType of
    vtInt:
      if TryStrToInt(AValue, AIntValue) then
      begin
        Control.Value := AIntValue;
        Control.CheckEditorValueBounds;
        Exit(True);
      end;
    vtFloat:
      if TryStrToFloat(AValue, AFloatValue) then
      begin
        Control.Value := AFloatValue;
        Control.CheckEditorValueBounds;
        Exit(True);
      end;
  end;
...
function TdxUIACustomSpinEditProvider.GetControl: TcxCustomSpinEdit;
begin
  Result := Safe.Cast<TcxCustomSpinEdit>(inherited Control);
end;

{ TdxUIACustomSpinEditInnerEditProvider }

function TdxUIACustomSpinEditInnerEditProvider.GetSpinEdit: TcxCustomSpinEdit;
begin
  Result := Safe.Cast<TcxCustomSpinEdit>(TextControl.Instance);
end;

function TdxUIACustomSpinEditInnerEditProvider.GetControlTypePropertyValue(out AValue: Integer): Boolean;
begin
  AValue := dxUIA.UIA_SpinnerControlTypeId;
  Result := True;
end;

function TdxUIACustomSpinEditInnerEditProvider.GetDefaultAriaRole: string;
begin
  Result := 'spinner';
end;

function TdxUIACustomSpinEditInnerEditProvider.GetSupportedPatterns: TArray<Integer>;
begin
  Result := inherited GetSupportedPatterns + [dxUIA.UIA_RangeValuePatternId];
end;

function TdxUIACustomSpinEditInnerEditProvider.InternalGetIsReadOnly: Boolean;
var
  AProvider: IValueProvider;
  pRetVal: Integer;
begin
  Result := Supports(SpinEdit.UIAProvider, IValueProvider, AProvider) and
    (AProvider.Get_IsReadOnly(pRetVal) = S_OK) and (pRetVal = 1);
end;

function TdxUIACustomSpinEditInnerEditProvider.InternalGetLargeChange: Double;
var
  AProvider: IRangeValueProvider;
begin
  if not (Supports(SpinEdit.UIAProvider, IRangeValueProvider, AProvider) and (AProvider.Get_LargeChange(Result) = S_OK)) then
    Result := 0;
end;

function TdxUIACustomSpinEditInnerEditProvider.InternalGetMaximum: Double;
var
  AProvider: IRangeValueProvider;
begin
  if not (Supports(SpinEdit.UIAProvider, IRangeValueProvider, AProvider) and (AProvider.Get_Maximum(Result) = S_OK)) then
    Result := 0;
end;

function TdxUIACustomSpinEditInnerEditProvider.InternalGetMinimum: Double;
var
  AProvider: IRangeValueProvider;
begin
  if not (Supports(SpinEdit.UIAProvider, IRangeValueProvider, AProvider) and (AProvider.Get_Minimum(Result) = S_OK)) then
    Result := 0;
end;

function TdxUIACustomSpinEditInnerEditProvider.InternalGetSmallChange: Double;
var
  AProvider: IRangeValueProvider;
begin
  if not (Supports(SpinEdit.UIAProvider, IRangeValueProvider, AProvider) and (AProvider.Get_SmallChange(Result) = S_OK)) then
    Result := 0;
end;

function TdxUIACustomSpinEditInnerEditProvider.InternalGetValue(out AValue: Double): Boolean;
var
  AProvider: IRangeValueProvider;
begin
  Result := Supports(SpinEdit.UIAProvider, IRangeValueProvider, AProvider) and (AProvider.Get_Value(AValue) = S_OK);
end;

function TdxUIACustomSpinEditInnerEditProvider.InternalGetValue(out AValue: string): Boolean;
var
  AProvider: IValueProvider;
  pRetVal: WideString;
begin
  if Supports(SpinEdit.UIAProvider, IValueProvider, AProvider) and (AProvider.Get_Value(pRetVal) = S_OK) then
  begin
    AValue := pRetVal;
    Result := True;
  end
...
function TdxUIACustomSpinEditInnerEditProvider.InternalSetValue(const AValue: Double): Boolean;
var
  AProvider: IRangeValueProvider;
begin
  Result := Supports(GetSpinEdit.UIAProvider, IRangeValueProvider, AProvider) and (AProvider.SetValue(AValue) = S_OK);
end;

function TdxUIACustomSpinEditInnerEditProvider.InternalSetValue(const AValue: string): Boolean;
var
  AProvider: IValueProvider;
begin
  Result := Supports(SpinEdit.UIAProvider, IValueProvider, AProvider) and (AProvider.SetValue(PWideChar(AValue)) = S_OK);
end;
