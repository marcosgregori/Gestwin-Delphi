...
uses
  Winapi.Windows, System.Classes, Vcl.Controls, Vcl.ExtCtrls, Vcl.Forms, Vcl.Graphics, Vcl.ImgList, Winapi.Messages, System.Types,
...
type
  ...
  protected
    ...
    function SupportsListBoxSkinPadding: Boolean; override;
  ...
  { TcxMCListBoxViewInfo }

  TcxMCListBoxViewInfo = class(TcxCustomListBoxViewInfo)
  protected
    procedure DrawBorder(ACanvas: TcxCanvas; R: TRect); override;
    function UseListBoxBorderedBackground: Boolean; override;
  end;
  ...
  public
    ...
    property Count;
    property InnerListBox: TcxMCInnerListBox read GetInnerListBox; // for internal use
    property ItemIndex; 
...
uses
  System.Variants, System.Math, cxVariants, dxThemeConsts, dxThemeManager, dxUxTheme, cxGeometry, dxCoreGraphics, System.UITypes,
  dxDPIAwareUtils;
...
procedure TcxMCInnerListBox.UpdateItemsLayout;
begin
  RecalcItemRects;
  Invalidate;
end;
...
procedure TcxMCInnerListBox.WMEraseBkgnd(var Message: TWMEraseBkgnd);
var
  AHeight, ACount, ATop: Integer;
  R: TRect;
begin
  ACount := GetListBoxInfo(Handle);
  ATop := TopIndex;
  AHeight := ClientHeight - (ACount - ATop) * GetDefaultItemHeight;
  if AHeight > 0 then
  begin
    R := ClientRect;
    R.Top := R.Bottom - AHeight;
    FillRectByColor(Message.DC, R, Brush.Color);
  end;
  if MCListBox.ShowColumnLines then
  begin
    Canvas.Canvas.Handle := Message.DC;
    try
      DrawLines;
    finally
...
begin
  ...
  begin
    if (not TdxVisualRefinements.LightBorders) or (I <> MCListBox.HeaderSections.Count - 1) or (not MCListBox.HeaderSections.HasAutoSize) then
    begin
...
function TcxMCInnerListBox.SupportsListBoxSkinPadding: Boolean;
begin
  Result := False; 
end;
...
begin
  ...
  LookAndFeel.Painter.DrawScaledHeaderControlSection(Canvas, R, cxEmptyRect, [],
    ABorders, cxbsNormal, taLeftJustify, vaTop, False, False, '', Font,
...
procedure TcxMCListBoxViewInfo.DrawBorder(ACanvas: TcxCanvas; R: TRect);
begin
  if Painter.SupportsEditorBorders then
    inherited DrawBorder(ACanvas, R)
  else
    Painter.DrawBorder(ACanvas, R, Painter.BorderSize);
end;

function TcxMCListBoxViewInfo.UseListBoxBorderedBackground: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FSection.Width := FInnerHeader.Canvas.TextWidth(FSection.Text) + ScaleFactor.Apply(4);
    end;
  end;
end;
...
begin
  ...
  FInnerListBoxContainer.Style.Font := VisibleFont;
end;
...
function TcxMCListBox.GetViewInfoClass: TcxContainerViewInfoClass;
begin
  Result := TcxMCListBoxViewInfo;
end;
...
procedure TcxMCListBox.DrawItem(Control: TWinControl; Index: Integer; ARect: TRect; State: TOwnerDrawState);
  ...
  begin
     if Enabled and (not FIsInternalPaint and (odSelected in State)) or //..
    ...
    begin
         ...
         then begin
              ...
              ABackgroundColor := clInactiveCaption;
              ATextColor := clInactiveCaptionText;
              end;
         //..
    end else
    begin
      ABackgroundColor := ViewInfo.BackgroundColor;
      ATextColor := VisibleFontColor;
    end;
...
begin
  ...
  begin
    ...
    if ShowColumnLines then
      if (not TdxVisualRefinements.LightBorders) or (I <> HeaderSections.Count - 1) or (not HeaderSections.HasAutoSize) then
...
var
  ...
  ACalcRect: TRect;
begin
  Result := Max(ItemHeight, InnerListBox.GetDefaultItemHeight);
  for I := 0 to HeaderSections.Count - 1 do
  begin
...
procedure TcxMCListBox.Paint;
begin
  inherited;
  if FInnerListBoxContainer <> nil then
    FInnerListBoxContainer.UpdateInternalControlsState;
end;
