...
unit dxRichEdit.Dialogs.CustomDialog; // for internal use
...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, dxForms,
  dxCore, cxClasses, cxDropDownEdit, cxRadioGroup, cxTextEdit, dxCoreClasses,
  cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters, dxLayoutLookAndFeels, cxSpinEdit, dxColorEdit,
...
type
  ...
  private
    ...
    procedure ColorButtonClick(Sender: TObject; AButtonIndex: Integer);
  protected
    ...
    procedure InitializeColorButtons(AColorEdit: TdxCustomColorEdit; AAutoResString: TcxResourceStringID = nil);
    ...
    procedure MeasurementUnitEditValidateHandler(Sender: TcxCustomEdit; var DisplayValue: Variant; var ErrorText: TCaption;
      var Error: Boolean); //..
...
uses
  ...
  dxMessageDialog, dxCoreGraphics,
  { cxEdit, } cxDrawTextUtils, dxColorDialog,  //..
...
procedure TdxRichEditCustomDialogForm.ColorButtonClick(Sender: TObject; AButtonIndex: Integer);
var
  AColor: TColor;
begin
  case AButtonIndex of
    1:
      begin
        AColor := (Sender as TdxCustomColorEdit).ColorValue;
        if TdxGlobalColorDialog.Execute(AColor, Handle) then
          TdxCustomColorEdit(Sender).ColorValue := AColor;
      end;
    2:
      (Sender as TdxCustomColorEdit).EditValue := Null;
  end;
end;
...
begin
  ...
  begin
    AEditor.SetFocus;
    dxMessageDlg(cxGetResourceString(@sdxRichEditInvalidNumber), mtWarning, [mbOK], 0);
    Exit(False);
  end;
  if AMinValue = 0 then
  ...
  begin
    AEditor.SetFocus;
    dxMessageDlg(Format(cxGetResourceString(@sdxRichEditInvalidSize), [AMinValue, AMaxValue]), mtWarning, [mbOK], 0);
    Result := False;
  end
  else
    Result := True;
...
procedure TdxRichEditCustomDialogForm.InitializeColorButtons(AColorEdit: TdxCustomColorEdit;
  AAutoResString: TcxResourceStringID);
var
  AItem: TcxEditButton;
begin
  AItem := AColorEdit.Properties.Buttons.Add;
  AItem.Kind := bkEllipsis;
  AItem.Visible := True;
  AColorEdit.Properties.OnButtonClick := ColorButtonClick;

  AItem := AColorEdit.Properties.Buttons.Add;
  AItem.Kind := bkText;
  if AAutoResString = nil then
    AAutoResString := @sdxRichEditFontDialogButtonColorAuto;
  AItem.Caption := cxGetResourceString(AAutoResString);
  AItem.Width := cxTextWidth(AColorEdit.Style.GetVisibleFont, AItem.Caption) + 8 * cxTextSpace;
  AColorEdit.Properties.OnButtonClick := ColorButtonClick;
end;
...
procedure TdxRichEditCustomDialogForm.MeasurementUnitEditValidateHandler(Sender: TcxCustomEdit; var DisplayValue: Variant;  //..
...
begin
  ...
  else if Owner is TdxRichEditCustomDialogForm then
  ...
  begin
    if Safe.Cast<TdxLayoutCxLookAndFeel>(dxLayoutLookAndFeelList1.Items[I], ACustomLookAndFeel) then
