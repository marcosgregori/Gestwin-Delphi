...
unit dxRichEdit.Dialogs.Paragraph; // for internal use
...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, dxLayoutLookAndFeels, cxClasses, Vcl.ActnList, dxCore, cxGraphics, cxControls,
  cxLookAndFeels, cxLookAndFeelPainters, dxLayoutContainer, dxLayoutControl, Vcl.Menus, dxLayoutControlAdapters,
  Vcl.StdCtrls, cxButtons, dxLayoutcxEditAdapters, cxContainer, cxEdit, cxLabel, cxTextEdit, cxMaskEdit, cxDropDownEdit,
...
type
  ...
  TdxRichEditParagraphDialogForm = class(TdxRichEditCustomDialogForm, IdxFormOwner)
    ...
    btnOK: TcxButton;
    btnTabs: TcxButton;
    cbContextualSpacing: TdxLayoutCheckBoxItem;
    cbKeepLinesTogether: TdxLayoutCheckBoxItem;
    cbPageBreakBefore: TdxLayoutCheckBoxItem;
    ...
    procedure seFirstLineIndentPropertiesValidate(Sender: TcxCustomEdit; var DisplayValue: Variant; var ErrorText: TCaption;
      var Error: Boolean); //..
...
procedure TdxRichEditParagraphDialogForm.seFirstLineIndentPropertiesValidate(Sender: TcxCustomEdit; var DisplayValue: Variant;
...
begin
  ...
  begin
    cbContextualSpacing.CheckBoxOptions.AllowGrayed := True;
...
begin
  ...
  begin
    cbKeepLinesTogether.CheckBoxOptions.AllowGrayed := True;
...
begin
  ...
  begin
    cbPageBreakBefore.CheckBoxOptions.AllowGrayed := True;
...
begin
  if not cbContextualSpacing.CheckBoxOptions.AllowGrayed or (cbContextualSpacing.State <> TcxCheckBoxState.cbsGrayed) then
...
begin
  if not cbKeepLinesTogether.CheckBoxOptions.AllowGrayed or (cbKeepLinesTogether.State <> TcxCheckBoxState.cbsGrayed) then
...
begin
  if not cbPageBreakBefore.CheckBoxOptions.AllowGrayed or (cbPageBreakBefore.State <> TcxCheckBoxState.cbsGrayed) then
...
function TdxRichEditParagraphDialogForm.ValidateFromControls: Boolean;
  ...
  begin
    ...
    begin
      ...
      begin
        Result := False;
        dxMessageDlg(cxGetResourceString(@sdxRichEditInvalidMeasurement), mtWarning, [mbOK], 0);
      end
      else
    ...
    if not Result then
    begin
      AItem := dxLayoutControl1.FindItem(AEditor);
      if AItem <> nil then
        AItem.MakeVisible;
