...
type
  ...
  public
    ...
    AjustandoCantidad,
    FijarClases,
    MovimientoDevolucionEnvases : Boolean;
...
resourceString
   ...
   RsDescripcionDevolucionEnvases = 'Devolucion de envases y contenedores';
...
procedure TMntMovForm.ClienteCtrlPropertiesEditRequest(Sender: TcxCustomEdit);
begin
     MntClientes( [ Sender.EditingValue ] );
end;

procedure TMntMovForm.ClienteCtrlPropertiesQueryRequest(Sender: TcxCustomEdit);
begin
     ConsultaClientes( Sender );
end;

procedure TMntMovForm.ClienteCtrlPropertiesValidate(Sender: TcxCustomEdit; var DisplayValue: TcxEditValue; var ErrorText: TCaption; var Error: Boolean);
begin
...
begin
     ...
     then begin
          ...
          then If   ( ( MovimientoFields.TipoMovimiento.Value=tmDevolucionEnvases ) or ArticuloFields.ConEnvase.Value ) and ValueIsEmpty( DisplayValue )
               then begin
                    Error := True;
...
begin
     With LineaMovimientoFields do
       ConsultaClases( TNroClase( GridView.Controller.FocusedColumn.Tag ),
                       CodigoArticulo.Value,
                       CodigoClaseA.Value,
                       CodigoClaseB.Value,
                       CodigoClaseC.Value,
                       MovimientoFields.Propietario.Value, Sender );
end;
...
begin
     With GridViewNroCajas do
       If   Editing and ( GridViewCodigoArticulo.EditValue<>ArticuloManual )
...
procedure TMntMovForm.FormManagerPreparedForm;
begin
     If   Usuario.OpcionUsuario( DataModule00.DmUsuarioFields.Ventas_MostrarStock, DataModule00.DmEmpresaFields.Ventas_VerStock )
...
begin
     ...
     If   not LineaMovimientoTable.ControlsDisabled and not MovimientoDevolucionEnvases
     then begin
          ActualizaCamposCalculadosLinea;
...
begin
     With LineaMovimientoFields do
       ...
       If   MovimientoFields.TipoMovimiento.Value=tmDevolucionEnvases
       then begin
            CodigoArticulo.Value := ArticuloManual;
            Descripcion.Value := RsDescripcionDevolucionEnvases;
            end;
...
begin
     If   not MovimientoDevolucionEnvases
     then FacturaCompras.CalculaImportes( LineaMovimientoFields, ArticuloFields )
end;
...
begin
     ...
     DescripcionCtrl.Enabled := MovimientoTableCanEditRecord( nil );
     ...
     then begin
          ...
          then begin
               ...
               then Movimiento.SetupCamposLibresLineaVentas( GridView, [ GridViewCampoLibre1, GridViewCampoLibre2, GridViewCampoLibre3, GridViewCampoLibre4, GridViewCampoLibre5 ] )
               else Movimiento.SetupCamposLibresLineaCompras( GridView, [ GridViewCampoLibre1, GridViewCampoLibre2, GridViewCampoLibre3, GridViewCampoLibre4, GridViewCampoLibre5 ] );
               end;
          end;
end;
...
begin
     ...
     then try
            ...
            MovimientoDevolucionEnvases := TipoMovimiento=tmDevolucionEnvases;

            If   TipoMovimientoCtrl.Editing
            then If   TipoMovimiento in [ tmReubicacionCarga, tmOferta..tmCapturador, tmConsumos..tmRecuperado ]
                 then begin
                      Error := True;
            ...
            With DataModule00.DmEmpresaFields do
              ...
              var OcultarPrecios := DataModule00.DmUsuarioFields.Compras_OcultarPrecios.Value;

              GridViewFechaCaducidad.Visible := DataModule00.Trazabilidad  and
                                              ( ( ( TipoMovimiento in [ tmInventario, tmRegularizacion, tmEntrada, tmCompra, tmFabricacion ] ) and Compras_FechaCaducidad.Value ) or
                                                ( ( TipoMovimiento in [ tmSalida, tmVenta ] ) and Ventas_FechaCaducidad.Value ) );

              GridViewNumeroSerie.Visible := not MovimientoDevolucionEnvases and ( Ventas_NrosSerie.Value );
              GridViewLoteFabricacion.Visible := not MovimientoDevolucionEnvases and Stock_LotesProductos.Value;
              GridViewTipoLinea.Visible:= not MovimientoDevolucionEnvases and Stock_TiposLineaMovimiento.Value;
              GridViewCodigoAlmacen.Visible := not MovimientoDevolucionEnvases and Stock_MultiAlmacen.Value;
              GridViewUbicacion.Visible := not MovimientoDevolucionEnvases and Stock_Ubicaciones.Value;
              GridViewNroCajas.Visible := Articulo_Cajas.Value;
              GridViewLargo.Visible := not MovimientoDevolucionEnvases and ( Ventas_Cantidades.Value>0 );
              GridViewAncho.Visible := not MovimientoDevolucionEnvases and ( Ventas_Cantidades.Value>0 );
              GridViewAlto.Visible := not MovimientoDevolucionEnvases and ( Ventas_Cantidades.Value>1 );

              var BultosVisibles := DataModule00.Envases and Articulo_Bultos.Value and
                                    ( ( Compras_EditarBultosAlbaran.Value and ( TipoMovimiento=tmCompra ) ) or
                                      ( Ventas_EditarBultosAlbaran.Value and ( TipoMovimiento=tmVenta ) ) or
                                      ( TipoMovimiento=tmDevolucionEnvases ) );

              GridViewCodigoArticulo.Visible := not MovimientoDevolucionEnvases;
              GridViewBultos.Visible := BultosVisibles;
              GridViewCantidad.Visible := not MovimientoDevolucionEnvases;

              GridViewPrecio.Visible := not MovimientoDevolucionEnvases and not OcultarPrecios;
              GridViewDescuento.Visible := not MovimientoDevolucionEnvases and not OcultarPrecios;
              GridViewImporte.Visible := not MovimientoDevolucionEnvases and not OcultarPrecios;

              SetupPanelableCtrl( ClienteCtrl, TipoMovimiento=tmDevolucionEnvases );

              GridViewAlto.Caption := 'Alto';
              GridViewNroCajas.Caption := 'Cajas';
              GridViewBultos.Caption := 'Bultos';

              If   TipoMovimiento in [ tmPresupuesto..tmVenta, tmDevolucionEnvases ]
              then begin

                   If   not ValueIsEmpty( Ventas_NombreCajas.Value )
                   then If   TipoMovimiento=tmDevolucionEnvases
                        then GridViewNroCajas.Caption := Ventas_NombreCajas.Value
                        else If   TipoMovimiento in [ tmPresupuesto..tmVenta ]
              ...
              If   TipoMovimiento in [ tmOferta..tmCompra ]
              then begin

                   If   not ValueIsEmpty( Compras_NombreCajas.Value )
                   then If   Compras_Cantidades.Value=2
                        then GridViewAlto.Caption := Compras_NombreCajas.Value
                        else GridViewNroCajas.Caption := Compras_NombreCajas.Value;
