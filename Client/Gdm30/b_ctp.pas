
unit b_ctp;

interface

uses AppForms, StdCtrls, Buttons, Mask, Controls,
     ExtCtrls, Classes, Forms, Grids, Graphics,

     AppManager,
     AppContainer,
     Menus, cxLookAndFeelPainters, cxButtons, cxCheckBox,
     cxDBEdit, cxControls, cxContainer, cxEdit, cxTextEdit, DB, dxmdaset,
     DataManager, cxGraphics, cxLookAndFeels, dxSkinsCore,
  dxSkinsDefaultPainters, cxLabel, cxGroupBox, dxUIAClasses;

type

  TBoxCtpForm = class(TgxForm)
    FormManager: TgxFormManager;
    dataPanel: TgxEditPanel;
    CodigoProveedorCtrl: TcxDBTextEdit;
    CopiarReferenciasCtrl: TcxDBCheckBox;
    DataSource: TDataSource;
    Data: TgxMemData;
    DataCodigoProveedor: TWideStringField;
    DataCopiarReferencias: TBooleanField;
    Label2: TcxLabel;
    Label1: TcxLabel;
    cxGroupBox1: TcxGroupBox;
    cxLabel1: TcxLabel;
    ButtonsPanel: TgxEditPanel;
    Panel3: TcxGroupBox;
    OkButton: TgBitBtn;
    CloseButton: TgBitBtn;
    procedure FormManagerOkButton;
    procedure CodigoProveedorCtrlPropertiesEditRequest(
      Sender: TcxCustomEdit);
    procedure CodigoProveedorCtrlPropertiesQueryRequest(
      Sender: TcxCustomEdit);
    procedure CodigoProveedorCtrlPropertiesValidate(Sender: TcxCustomEdit;
      var DisplayValue: TcxEditValue; var ErrorText: TCaption;
      var Error: Boolean);
  private
  public

    FOnSelected : TSimpleEvent;
  end;

var  BoxCtpForm: TBoxCtpForm = nil;

procedure CopiaTarifaProveedor( CodigoProveedor : String;
                                OnSelected    : TSimpleEvent );

implementation

{$R *.DFM}

uses   Gim00Fields,
       Gim30Fields,

       dm_pro,

       a_pro,

       cx_pro;

// Solo debe usarse desde a_tpc

resourceString
       rsMsg1  = 'No puede utilizar el mismo código de proveedor como destino.';

var  CodigoProveedorOrigen : String;

procedure CopiaTarifaProveedor( CodigoProveedor : String;
                                OnSelected    : TSimpleEvent );
begin
     CodigoProveedorOrigen := CodigoProveedor;
     CreateEditForm( TboxCtpForm, BoxCtpForm );
     If   Assigned( BoxCtpForm )
     then BoxCtpForm.FOnSelected := OnSelected;
end;

procedure TBoxCtpForm.FormManagerOkButton;
begin
     If   Assigned( FOnSelected )
     then FOnSelected;
end;

procedure TBoxCtpForm.CodigoProveedorCtrlPropertiesEditRequest(Sender: TcxCustomEdit);
begin
     MntProveedores( [ Sender.EditingValue ] );
end;

procedure TBoxCtpForm.CodigoProveedorCtrlPropertiesQueryRequest(Sender: TcxCustomEdit);
begin
     ConsultaProveedores( Sender );
end;

procedure TBoxCtpForm.CodigoProveedorCtrlPropertiesValidate( Sender: TcxCustomEdit; var DisplayValue: TcxEditValue; var ErrorText: TCaption; var Error: Boolean);
begin
     Proveedor.Valida( Sender, DisplayValue, ErrorText, Error );
     If   DisplayValue=CodigoProveedorOrigen
     then begin
          Error := True;
          ErrorText := RsMsg1;
          end;
end;

end.
